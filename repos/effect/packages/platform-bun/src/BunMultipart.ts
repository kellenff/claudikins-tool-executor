 ...
 *
 * This module adapts a Bun `Request` body and headers into the shared `Multipart` model. `stream`
 * returns multipart parts as a `Stream`, while `persisted` collects the form and writes file parts
 * to scoped temporary files through the current `FileSystem`, `Path`, and `Scope` services.
 *
 * @since 4.0.0
 */
...
/**
 * Parses and persists multipart data from a Bun `Request`, requiring file-system, path, and scope
 * services.
 *
 * @since 4.0.0
 * @category Constructors
