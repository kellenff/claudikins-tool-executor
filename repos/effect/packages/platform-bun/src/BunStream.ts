 ...
 *
 * This module is the Bun entry point for adapting runtime streams into Effect's streaming model. It
 * re-exports the shared Node stream adapters for Bun's Node-compatible stream APIs and adds
 * {@link fromReadableStream}, a Web `ReadableStream` adapter that uses Bun's `readMany` reader
 * method to pull batches of values into an Effect `Stream`.
 *
 * @since 4.0.0
 */
