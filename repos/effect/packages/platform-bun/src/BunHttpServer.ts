 ...
 *
 * `make` creates a scoped HTTP server from `Bun.serve`, converting Bun `Request` values into
 * `HttpServerRequest` values and Effect `HttpServerResponse` values back into Web `Response`
 * values. The server supports streaming bodies, multipart requests, file responses through
 * `BunHttpPlatform`, and WebSocket upgrades. This module also provides layers for the server alone,
 * the Bun HTTP support services, the combined server, configurable server options, and a test
...
/**
 * Creates a scoped Bun `HttpServer` from `Bun.serve` options, stopping the server on scope
 * finalization with optional graceful shutdown settings.
 *
 * @since 4.0.0
...
export const make = Effect.fnUntraced(function* <R extends string>(
  ...
  const handlerStack: Array<
    (request: Request, server: BunServer<WebSocketContext>) => Response | Promise<Response>
  > = [
...
/**
 * Layer that provides only `HttpServer` by constructing a scoped Bun server from the supplied serve
 * options.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Layer that provides Bun HTTP support services: `HttpPlatform`, weak ETag generation, and
 * `BunServices`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Layer that provides a Bun `HttpServer` together with the Bun HTTP platform, ETag generator, and
 * Bun services.
 *
 * @since 4.0.0
 * @category Layers
