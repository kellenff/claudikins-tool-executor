 ...
 *
 * This module creates scoped Bun `RedisClient` connections and exposes them as both the portable
 * `Redis` service and the Bun-specific `BunRedis` service for direct access to the raw client. The
 * `layer` helper accepts Redis options directly, while `layerConfig` reads them from Effect config.
 * Both close the underlying client when the layer scope finalizes.
 *
 * @since 4.0.0
 */
...
/**
 * Service tag for Bun Redis integration, exposing the raw `RedisClient` and a `use` helper that
 * maps client promise failures to `RedisError`.
 *
 * @since 4.0.0
 * @category Services
 */
...
/**
 * Creates scoped Bun Redis layers for `Redis.Redis` and `BunRedis`, closing the underlying client
...
/**
 * Creates scoped Bun Redis layers from configurable Redis options, closing the underlying client
 * when the scope finalizes.
 *
 * @since 4.0.0
 * @category Layers
