 ...
 * logging in browser runtimes. The main `layer` builds the shared OpenTelemetry resource from
 * explicit service metadata, then enables only the signal types that have processors or readers
...
/**
 * Configuration for the Web OpenTelemetry layer, including resource metadata and optional tracing,
...
/**
 * Creates a scoped Web OpenTelemetry tracer provider from one or more span processors and shuts it
...
/**
 * Creates a Web OpenTelemetry layer from configuration, providing the resource and enabling
 * tracing, metrics, and logging when configured.
 *
 * **When to use**
 *
 * Use to install browser OpenTelemetry support when service metadata is configured in code and
 * telemetry processors or readers are supplied directly.
 *
 * **Details**
 *
 * The configuration can be provided lazily or effectfully. The layer always provides
 * `Resource.Resource`; tracing, metrics, and logging are installed only when the corresponding
 * processors or readers are non-empty.
 *
 * **Gotchas**
 *
 * Browser resource metadata is explicit; this layer does not read OpenTelemetry environment
 * variables. Empty processor or reader arrays are treated as not configured.
 *
 * @since 4.0.0
 * @category Layers
 */
