 ...
 *
 * This module exports a `Configuration` type and layers for installing tracing, metrics, and
 * logging in Node.js. The main `layer` builds the shared OpenTelemetry resource from environment
 * variables and optional service metadata, then enables only the signal types that have processors
 * or readers configured. `layerTracerProvider` creates a scoped Node tracer provider, and
...
/**
 * Configuration for the Node OpenTelemetry layer, including optional tracing, metrics, logging,
...
/**
 * Creates a scoped Node OpenTelemetry tracer provider from one or more span processors and shuts it
 * down when the layer is released.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Creates a Node OpenTelemetry layer from configuration, enabling tracing, metrics, and logging
 * only when their processors or readers are supplied.
 *
 * **When to use**
 *
 * Use to install OpenTelemetry support for a Node.js Effect application from one configuration
 * object, enabling tracing, metrics, logging, or any combination of those signals based on the
 * processors and readers supplied.
 *
 * **Details**
 *
 * The configuration can be provided lazily or effectfully. The layer always provides
 * `Resource.Resource`, building it from environment variables and any explicit resource metadata in
 * the configuration.
 *
 * **Gotchas**
 *
 * Register Node auto-instrumentations before importing modules that should be patched, because many
 * Node instrumentations hook module loading.
 *
 * @since 4.0.0
 * @category Layers
 */
