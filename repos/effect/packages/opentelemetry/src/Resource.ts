 ...
 *
 * An OpenTelemetry resource identifies the process or service that emits spans, metrics, and logs.
 * This module stores that resource in Effect context and provides layers for creating it from
 * explicit service metadata, from `OTEL_SERVICE_NAME` and `OTEL_RESOURCE_ATTRIBUTES`, or as an
 * empty resource. It also includes `configToAttributes` for turning service metadata into raw
 ...
 * **When to use**
 *
 * Use to provide process, service, and deployment metadata that should be attached to spans,
 * metrics, and logs.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Converts resource configuration into OpenTelemetry attributes, adding service name, optional
 * service version, and telemetry SDK metadata.
 *
 * **When to use**
 *
 * Use to turn explicit service metadata into a raw OpenTelemetry attribute map for lower-level
 * resource construction or merging with environment-derived attributes via `layerFromEnv`.
 *
 * **Details**
 *
 * The returned record copies `attributes` first, then sets `service.name`, `telemetry.sdk.name`,
 * and `telemetry.sdk.language`. `service.version` is included only when `serviceVersion` is
 * provided.
 *
 * **Gotchas**
 *
 * Custom values for `service.name` and `telemetry.sdk.*` are overwritten by this helper. An empty
 * `serviceVersion` is treated as absent.
 *
 * @since 4.0.0
...
/**
 * Creates a `Resource` layer from OpenTelemetry environment variables, optionally merging
