 ...
 *
 * This module exposes Effect metrics as an OpenTelemetry `MetricProducer`. `makeProducer` creates
 * the producer, `registerProducer` attaches it to one or more SDK `MetricReader`s, and `layer`
 * manages that setup in a scoped layer. The `TemporalityPreference` type lets callers choose
 * cumulative or delta metric values.
 *
 * @since 4.0.0
 */
 ...
 * measurements. This is the default behavior. `delta` reports changes since the last export. Each
 * interval is independent with no dependency on previous measurements.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when you need a `MetricProducer` for manually wiring Effect metrics into OpenTelemetry
 * instead of using the scoped `layer` helper.
 *
 * **Details**
 *
 * Requires the current OpenTelemetry `Resource`, captures the current Effect context, and uses
 * cumulative temporality by default. Pass `"delta"` for interval-based values.
 *
 * @since 4.0.0
