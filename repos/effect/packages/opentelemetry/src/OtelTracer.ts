/**
 * Bridges Effect tracing into OpenTelemetry by installing an Effect `Tracer` that creates
 * OpenTelemetry spans, records attributes, events, links, errors, and status, and keeps
 * OpenTelemetry context active while traced effects run. Use this module when an application
 * already has an OpenTelemetry `TracerProvider`, or when the Node and Web SDK layers should expose
 * Effect spans to OTLP, console, or other OpenTelemetry-compatible exporters.
 *
 * The layer constructors wire Effect's tracer service to either the global OpenTelemetry tracer
 * provider or an explicitly provided `OtelTracer`. This module does not create exporters or span
 * processors by itself, so spans are exported only when the provider has been configured by the
 * application or by the Node/Web SDK layers. Parentage is taken from Effect spans first and can
 * also attach to the active OpenTelemetry context, while `makeExternalSpan` and `withSpanContext`
 * are the entry points for continuing an incoming remote trace. Preserve `traceFlags` and
 * `traceState` when building external spans; otherwise sampling defaults to sampled and trace state
...
/**
 * Context service containing OpenTelemetry trace flags used when constructing external span
 * contexts.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Context service containing OpenTelemetry trace state used when constructing external span
 * contexts.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Creates an Effect external span from an OpenTelemetry span context, preserving trace flags and
 * trace state when provided.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Layer that creates an OpenTelemetry tracer from the provided tracer provider and resource
 * metadata.
 *
 * @since 4.0.0
 * @category Layers
 */
export const layerTracer: Layer.Layer<OtelTracer, never, OtelTracerProvider | Resource> =
  Layer.effect(
    OtelTracer,
    Effect.gen(function* () {
...
/**
 * Layer that creates an OpenTelemetry tracer from the global tracer provider and the current
 * resource.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * @category Layers
 */
export const layerGlobal: Layer.Layer<OtelTracer, never, Resource> = Layer.effect(
  Tracer.Tracer,
  make,
 ...
 * @category Layers
 */
export const layerWithoutOtelTracer: Layer.Layer<never, never, OtelTracer> = Layer.effect(
  Tracer.Tracer,
  make,
);

/**
 * Layer that creates an OpenTelemetry tracer from a provider and resource, then installs it as the
 * Effect tracer.
 *
 * **When to use**
 *
 * Use when you already provide an `OtelTracerProvider` and a `Resource`, and want Effect spans
 * backed by a tracer derived from them.
 *
 * @since 4.0.0
 * @category Layers
 ...
 */
export const layer: Layer.Layer<OtelTracer, never, OtelTracerProvider | Resource> =
 ...
 * **Details**
 *
 * This accessor works with both the official OpenTelemetry API, such as `Tracer.layer` and
 * `NodeSdk.layer`, and the lightweight OTLP module, such as `OtlpTracer.layer`. When using OTLP,
 * the returned span is a wrapper that conforms to the OpenTelemetry `Span` interface.
 *
 * @since 4.0.0
 * @category Accessors
 */
export const currentOtelSpan: Effect.Effect<Otel.Span, Cause.NoSuchElementError> = Effect.clockWith(
  (clock) =>
 ...
 * **When to use**
 *
 * Use when you need an effect to continue a trace from a parent span context produced by
 * OpenTelemetry instrumentation outside Effect.
 *
 * @since 4.0.0
