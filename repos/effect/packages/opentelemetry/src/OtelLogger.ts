 ...
 *
 * This module turns Effect log events into OpenTelemetry log records. It maps Effect log levels to
 * OpenTelemetry severity numbers, provides the `OtelLoggerProvider` service, creates an Effect
 * `Logger` with `make`, and offers layers for installing that logger or creating a scoped SDK
 ...
 * **Details**
 *
 * OpenTelemetry log severity numbers are in the range `1` through `24`. This function maps from
 * Effect's log levels instead of using `LogLevel.getOrdinal`, whose internal sort ordinals, such as
 * the `Info` ordinal `20000`, fall outside the OpenTelemetry logs data model and can be treated as
 * `UNSPECIFIED` by validating backends.
 *
 * @since 4.0.0
 * @category Converting
 */
...
/**
 * Creates an Effect logger that emits log records through the configured OpenTelemetry logger
 * provider.
 *
 * @since 4.0.0
...
/**
 * Creates a layer that installs the OpenTelemetry-backed Effect logger, merging with existing
 * loggers by default.
 *
 * **When to use**
 *
 * Use to install the OpenTelemetry-backed Effect logger in an application that has an
 * `OtelLoggerProvider`, so standard Effect logging emits OpenTelemetry log records.
 *
 * **Details**
 *
 * The layer installs the logger created by `make`. `mergeWithExisting` defaults to `true`; set it
 * to `false` to replace the current logger set.
 *
 * @since 4.0.0
 * @category Layers
...
export const layer = (options: {
   ...
   * **Details**
   *
   * If set to `true`, the OpenTelemetry logger will be merged with existing loggers in the
   * application.
   *
   * If set to `false`, the OpenTelemetry logger will replace all existing loggers in the
   * application.
   *
   * Defaults to `true`.
   */
...
/**
 * Creates a scoped OpenTelemetry logger provider from one or more log record processors, using the
 * current `Resource` and flushing and shutting down the provider when the layer is released.
 *
 * @since 4.0.0
 * @category Layers
 */
