/**
 * React hooks for working with Effect atoms from components. The hooks read, write, mount, refresh,
 * and subscribe to atoms from `RegistryContext`, handle `AsyncResult` atoms with React Suspense,
 * and expose helpers for reading and deriving `AtomRef` values.
 *
 * @since 4.0.0
 */
"use client";
 ...
 * **Gotchas**
 *
 * Each atom is initialized at most once for a given registry by this hook, so later calls for the
 * same atom in that registry are ignored.
 *
 * @since 4.0.0
 * @category Hooks
 */
...
/**
 * Subscribes to an atom in the current React registry and returns its current value, optionally
 * mapped through a selector.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * When a selector is provided, the hook maps the atom before subscribing so the component reads the
 * selected value from the current `RegistryContext`.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **Details**
 *
 * The hook uses the current `RegistryContext` and releases the mount through React effect cleanup
 * when the component unmounts or when the registry or atom dependency changes.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **When to use**
 *
 * Use when a React component needs to update a writable atom without rendering from that atom's
 * value.
 *
 * **Details**
 *
 * The hook mounts the atom and returns a setter. In value mode the setter accepts a write value or
 * updater function; for `AsyncResult` atoms, `promise` and `promiseExit` modes return a promise for
 * the success value or full `Exit`.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **When to use**
 *
 * Use to expose a React callback that requests a refresh for an atom without reading or writing its
 * value.
 *
 * **Details**
 *
 * The hook uses the current `RegistryContext`, mounts the atom for the component lifetime, and
 * returns a callback that calls `registry.refresh`.
 *
 * @since 4.0.0
 * @category Hooks
...
/**
 * Subscribes to a writable atom and returns its current value together with a setter for updating
 * it.
 *
 * **When to use**
 *
 * Use when a React component needs both to render the current value of a writable atom and update
 * it from the same component.
 *
 * @since 4.0.0
 * @category Hooks
...
/**
 * Reads an `AsyncResult` atom through React Suspense, suspending while the result is initial or
 * configured as waiting.
 *
 * **When to use**
 *
 * Use when a React component should render only after an `AsyncResult` atom has left its initial
 * state, with loading delegated to a Suspense boundary.
 *
 * **Details**
 *
 * `suspendOnWaiting` defaults to `false`. When `includeFailure` is `true`, a failure result is
 * returned instead of being thrown.
 *
 * **Gotchas**
 *
 * Without `includeFailure`, failure results are thrown with `Cause.squash(result.cause)`, so
 * callers need an error boundary for failures.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **When to use**
 *
 * Use when a React component needs to run a callback for atom changes without reading the atom
 * value during render.
 *
 * **Details**
 *
 * The subscription is installed in a React effect and cleaned up on unmount or dependency change.
 * When `options.immediate` is enabled, the callback receives the current value when the effect
 * subscribes.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **When to use**
 *
 * Use when a React component should render from an `AtomRef.ReadonlyRef` directly instead of
 * reading an atom through the current registry.
 *
 * **Details**
 *
 * The hook subscribes with `ref.subscribe`, triggers re-renders through React state, and returns
 ...
 * **Details**
 *
 * The hook memoizes `ref.prop(prop)` for the `[ref, prop]` dependency pair and returns the property
 * ref so callers can read, set, update, or subscribe to that nested property.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **When to use**
 *
 * Use when a React component needs only the current value of one property from an object-shaped
 * `AtomRef`.
 *
 * **Details**
 *
 * The hook composes `useAtomRefProp(ref, prop)` with `useAtomRef`, so the property ref is memoized
 * for the `[ref, prop]` pair and then subscribed through `ref.subscribe`.
 *
 * @since 4.0.0
 * @category Hooks
