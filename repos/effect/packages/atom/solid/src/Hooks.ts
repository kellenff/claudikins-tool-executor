/**
 * Solid hooks for using Effect Atoms from components and computations. The hooks read and write
 * atoms through the current `RegistryContext`, mount atoms for cleanup, subscribe callbacks, seed
 * initial values, expose `AsyncResult` atoms as Solid resources, and read values from `AtomRef`
 * references.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * For each atom in the current registry, this hook applies the first value supplied through the
 * hook. Later calls for the same atom in that registry are ignored.
 *
 * @since 4.0.0
 * @category Hooks
 */
 ...
 * **Details**
 *
 * The hook uses the current `RegistryContext`, mounts inside a Solid computation, and releases the
 * mount through Solid cleanup when the computation changes or the owner is disposed.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **When to use**
 *
 * Use when a Solid component or computation needs both a reactive accessor for a writable atom and
 * a write function for that same atom.
 *
 * **Details**
 *
 * The setter accepts either a write value or an updater function. For `AsyncResult` atoms,
 * `promise` and `promiseExit` modes return promises for the success value or full `Exit`.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **When to use**
 *
 * Use when a Solid component or computation should render from an `AtomRef.ReadonlyRef` directly
 * instead of reading an atom through the current registry.
 *
 * **Details**
 *
 * The hook accepts a thunk for the ref, reads `ref().value`, subscribes with `ref.subscribe`, and
 * releases the subscription through Solid cleanup when the selected ref changes or the owner is
 * disposed.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **Details**
 *
 * The returned accessor memoizes `ref().prop(prop)`, updating when the source ref thunk produces a
 * different ref.
 *
 * **Gotchas**
 *
 * The `prop` argument is captured as a plain value. Recreate the hook call when the property key
 * should change.
 *
 * @since 4.0.0
 * @category Hooks
 ...
 * **When to use**
 *
 * Use when a Solid component or computation needs the value of one property from an object-shaped
 * `AtomRef` without keeping the intermediate property ref.
 *
 * **Details**
 *
 * The hook composes `useAtomRefProp(ref, prop)` with `useAtomRef`, returning a Solid accessor for
