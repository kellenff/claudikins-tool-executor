 ...
 *
 * This module can create an in-memory SQLite database in the current runtime or connect to a
 * worker-backed database, such as the OPFS worker from `OpfsWorker`. Both clients are exposed as
 * `SqliteClient` and the generic Effect SQL client, serialize access, support database import and
 * export, and can install reactivity hooks from SQLite update notifications. In-memory clients can
 * stream query rows; worker-backed clients cannot. `updateValues` is not supported by this driver.
 *
 * @since 4.0.0
 */
...
/**
 * SQLite WASM client service interface, extending `SqlClient` with database `export` and `import`
 * operations and marking `updateValues` as unsupported for SQLite.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Configuration for an in-memory SQLite WASM client, including optional reactivity hooks, span
 * attributes, and query/result name transforms.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Configuration for a worker-backed SQLite WASM client, including the scoped worker or message
 * port, optional reactivity hooks, span attributes, and query/result name transforms.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a scoped in-memory SQLite WASM client using the memory VFS, serializing access through a
 * semaphore and exposing database `export` and `import` operations.
 *
 * @since 4.0.0
        ...
        executeStream(sql, params, transformRows) {
          ...
          return Stream.suspend(() => Stream.fromIteratorSucceed(stream()[Symbol.iterator]())).pipe(
            ...
            Stream.mapError(
              (cause) =>
                new SqlError({
                  reason: classifyError(cause, "Failed to execute statement", "stream"),
                }),
            ),
          );
        },
        ...
        import(data) {
          return Effect.try({
            try: () => sqlite3.deserialize(db, "main", data, data.length, data.length, 1 | 2),
              ...
              new SqlError({ reason: classifyError(cause, "Failed to import database", "import") }),
          });
        },
      });
    });
...
/**
 * Creates a scoped worker-backed SQLite WASM client, communicating with the configured worker or
 * message port, restarting the scoped connection on worker errors, and exposing database `export`
...
/**
 * Fiber reference that stores transferables to include with worker-backed SQLite WASM query
 * messages.
 *
 * @since 4.0.0
...
/**
 * Runs an effect with the supplied transferables attached to worker-backed SQLite WASM query
 * messages.
 *
 * @since 4.0.0
 * @category Transferables
...
/**
 * Builds a layer from an Effect `Config` value, providing both the in-memory SQLite WASM
 * `SqliteClient` service and the generic `SqlClient` service.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Builds a layer from an in-memory SQLite WASM client configuration, providing both `SqliteClient`
 * and the generic `SqlClient` service.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Builds a layer from a worker-backed SQLite WASM client configuration, providing both
 * `SqliteClient` and the generic `SqlClient` service.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Builds a layer from an Effect `Config` value, providing both the worker-backed SQLite WASM
 * `SqliteClient` service and the generic `SqlClient` service.
 *
 * @since 4.0.0
 * @category Layers
