 ...
 *
 * This module opens `@effect/wa-sqlite` with the OPFS access-handle VFS, then listens on a
 * `MessagePort`-compatible port for the protocol used by `SqliteClient`. It sends a ready message,
 * executes SQL messages, imports and exports database bytes, forwards update-hook notifications,
 * and closes when requested. It is meant to run in a dedicated worker or a `SharedWorker`.
 *
 * @since 4.0.0
 */
...
/**
 * Configuration for the SQLite OPFS worker, including the message port used for the client protocol
 * and the OPFS database name to open.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Runs the SQLite OPFS worker loop, opening the configured database, posting a ready message,
 * handling query/import/export/update-hook messages, and closing when a close message is received.
 *
 * @since 4.0.0
    ...
    const db = yield* Effect.acquireRelease(
      Effect.try({
        try: () => sqlite3.open_v2(options.dbName, undefined, "opfs"),
          ...
          new SqlError({ reason: classifyError(cause, "Failed to open database", "openDatabase") }),
      }),
