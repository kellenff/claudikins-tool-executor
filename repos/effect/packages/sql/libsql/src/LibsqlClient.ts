 ...
 *
 * This module provides a {@link LibsqlClient} and the generic SQL client service for
 * `@libsql/client`. It uses Effect SQL's SQLite compiler, supports managed SDK clients or
 * caller-owned live clients, classifies libSQL and SQLite failures as `SqlError`s, and provides
 * transaction support with savepoints. Streaming queries are not implemented by this driver.
 *
 * @since 4.0.0
 */
...
/**
 * LibSQL-backed SQL client service, extending `SqlClient` with its runtime type marker and client
 * configuration.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Configuration for a libSQL client, either by supplying connection options or an existing live
 * libSQL client.
 *
 * @since 4.0.0
 * @category Models
...
export declare namespace LibsqlClientConfig {
  ...
  /**
   * Connection-based libSQL configuration used to create a managed client, including URL,
   * credentials, sync, integer mode, TLS, and concurrency options.
   *
   * @since 4.0.0
   * @category Models
   */
  export interface Full extends Base {
     ...
     * **Details**
     *
     * The client supports `libsql:`, `http:`/`https:`, `ws:`/`wss:` and `file:` URL. For more
     * infomation, please refer to the project README:
     *
     ...
     * **Details**
     *
     * - `"number"` (default): returns SQLite integers as JavaScript `number`-s (double precision
     *   floats). `number` cannot precisely represent integers larger than 2^53-1 in absolute value,
     *   so attempting to read larger integers will throw a `RangeError`.
     * - `"bigint"`: returns SQLite integers as JavaScript `bigint`-s (arbitrary precision integers).
     *   Bigints can precisely represent all SQLite integers.
     ...
     * **Details**
     *
     * By default, the client performs up to 20 concurrent requests. You can set this option to a
     * higher number to increase the concurrency limit or set it to 0 to disable concurrency limits
     * completely.
     */
  ...
  /**
   * Configuration that uses an existing libSQL client. The supplied `liveClient` is caller-owned
...
/**
 * Creates a scoped libSQL SQL client with transaction support. When given connection options it
 * creates and closes the SDK client; when given `liveClient`, the caller retains ownership.
 *
 * @since 4.0.0
...
/**
 * Creates a layer from a `Config`-wrapped libSQL client configuration, providing both
 * `LibsqlClient` and `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Creates a layer from a concrete libSQL client configuration, providing both `LibsqlClient` and
 * `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
