 ...
 *
 * This module adapts a Durable Object `SqlStorage` handle into both the Durable Object-specific
 * `SqliteClient` service and the generic Effect `SqlClient` service. Use it from inside a Durable
 * Object to run local per-object queries, repositories, migrations, transactional read/write
 * workflows, and tests that exercise Cloudflare's SQLite-backed storage API.
 *
 * Durable Object SQLite storage is scoped to one object id, so each object instance has its own
 * database. Callers can pass the `SqlStorage` handle for normal queries, or the full
 * `DurableObjectStorage` when `withTransaction` or migrations need Cloudflare-managed transactions.
 * This adapter serializes Effect SQL access through one connection; a transaction holds that permit
 * for the lifetime of its scope, so keep transactions short, avoid suspending them across unrelated
 * work, and use them when multi-statement writes must commit atomically. `SqlStorage.exec` returns
 * `ArrayBuffer` values for SQLite blobs, which this client normalizes to `Uint8Array`, and SQLite
...
/**
 * Cloudflare Durable Object SQLite client service, extending `SqlClient` with its configuration.
...
/**
 * Configuration for a Cloudflare Durable Object SQLite client, including either a `SqlStorage`
 * handle or the full `DurableObjectStorage` for transaction support, span attributes, and
 * query/result name transforms.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a scoped Cloudflare Durable Object SQLite client around Durable Object SQLite storage,
 * serializing access and converting returned `ArrayBuffer` values to `Uint8Array`.
 *
 * @since 4.0.0
...
/**
 * Creates a layer from a `Config`-wrapped Durable Object SQLite client configuration, providing
 * both `SqliteClient` and `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Creates a layer from a concrete Durable Object SQLite client configuration, providing both
 * `SqliteClient` and `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
 */
