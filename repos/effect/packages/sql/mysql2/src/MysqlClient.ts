 ...
 *
 * This module provides constructors and layers for a {@link MysqlClient} and the generic Effect SQL
 * client service. `make` creates a managed mysql2 pool, checks the connection with `SELECT 1`, maps
 * mysql2 failures to `SqlError`, supports transaction connections, and exposes streaming queries
 * through mysql2 query streams. It also provides direct and config-backed layers plus a MySQL
 * statement compiler.
 *
 * @since 4.0.0
 */
...
/**
 * Mysql2-backed SQL client service, extending `SqlClient` with its runtime type marker and client
 * configuration.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Configuration for a mysql2 client, including connection URI or connection fields, pool options,
 * span attributes, and query/result name transforms.
 *
 * @since 4.0.0
 * @category Models
 */
export interface MysqlClientConfig {
  ...
  /**
   * Use the text protocol instead of prepared statements, for proxies like Cloudflare Hyperdrive
   * that do not support `COM_STMT_PREPARE`.
   */
...
/**
 * Creates a scoped MySQL client backed by a managed mysql2 pool, verifying connectivity and
 * supporting streaming queries through mysql2 query streams.
 *
 * @since 4.0.0
...
export const make = (
  ...
  Effect.gen(function* () {
    ...
    const transformRows = options.transformResultNames
      ...
      : undefined;
    const defaultMethod: "execute" | "query" =
      options.disablePreparedStatements === true ? "query" : "execute";
...
/**
 * Creates a layer from a `Config`-wrapped MySQL client configuration, providing both `MysqlClient`
 * and `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
...
export const layerConfig = (
  ...
  Layer.effectContext(
    Config.unwrap(config).pipe(
      Effect.flatMap(make),
      Effect.map((client) =>
        Context.make(MysqlClient, client).pipe(Context.add(Client.SqlClient, client)),
      ),
    ),
  ).pipe(Layer.provide(Reactivity.layer));

/**
 * Creates a layer from a concrete MySQL client configuration, providing both `MysqlClient` and
 * `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
  ...
  return asyncPauseResume<any, SqlError>(
    Effect.fnUntraced(function* (emit) {
      ...
      query.on("error", (cause: unknown) =>
        emit.fail(
          new SqlError({ reason: classifyError(cause, "Failed to stream statement", "stream") }),
        ),
      );
