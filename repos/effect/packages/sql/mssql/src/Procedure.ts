 ...
 *
 * This module defines the `Procedure` values consumed by `MssqlClient.call`. `make` starts a
 * procedure definition, `param` and `outputParam` add typed Tedious parameter metadata, `withRows`
 * sets the expected row type, and `compile` binds input values before execution. The module also
 * defines the typed result shape for output parameters and returned rows.
 *
 * @since 4.0.0
 */
...
/**
 * Pipeable definition of a SQL Server stored procedure, tracking its input parameters, output
 * parameters, and result row type.
 *
 * @since 4.0.0
 * @category Models
 */
...
export declare namespace Procedure {
  ...
  /**
   * Result of a SQL Server stored procedure call, containing typed output parameter values and
   * returned rows.
   *
   * @since 4.0.0
   * @category Models
...
/**
 * Binds input values to a SQL Server stored procedure definition, producing a value that can be
 * executed with `MssqlClient.call`.
 *
 * @since 4.0.0
 * @category Combinators
 */
