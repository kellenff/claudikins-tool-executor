 ...
 *
 * This module provides the `MssqlClient` service, constructors, layers, and SQL Server statement
 * compiler. `make` creates a pooled Tedious client, checks the connection with `SELECT 1`, maps SQL
 * Server failures to `SqlError`, and supports transactions with savepoints. The SQL Server-specific
 * service adds typed Tedious parameters with `param`, stored procedure calls with `call`, direct or
 * config-backed layers, and default parameter type mappings. Streaming queries are not implemented
 * by this driver.
 *
 * @since 4.0.0
 */
...
/**
 * Microsoft SQL Server client service, extending `SqlClient` with typed parameter fragments and
...
/**
 * Configuration for a Microsoft SQL Server client, including connection, authentication, pool,
 * parameter type, span attribute, and query/result name transform options.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a scoped Microsoft SQL Server client backed by a connection pool, with transaction and
 * stored procedure support. Streaming queries are not implemented.
 *
 * @since 4.0.0
  ...
  Effect.gen(function* () {
        ...
        conn.connect((cause) => {
          if (cause) {
            resume(
              Effect.fail(
                new SqlError({
                  reason: classifyError(cause, "Failed to connect", "connect", "connection"),
                }),
              ),
            );
          } else {
            ...
            conn.saveTransaction((cause) => {
              if (cause) {
                resume(
                  Effect.fail(
                    new SqlError({
                      reason: classifyError(cause, "Failed to create savepoint", "createSavepoint"),
                    }),
                  ),
                );
              } else {
    ...
    yield* Pool.get(pool).pipe(
      ...
      Effect.mapError(
        (cause) =>
          new SqlError({
            reason: classifyError(cause, "MssqlClient: Failed to connect", "connect", "connection"),
          }),
      ),
      Effect.scoped,
...
/**
 * Creates a layer from a `Config`-wrapped SQL Server client configuration, providing both
 * `MssqlClient` and `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
 */
export const layerConfig: (
  ...
  Layer.effectContext(
    Config.unwrap(config).pipe(
      Effect.flatMap(make),
      Effect.map((client) =>
        Context.make(MssqlClient, client).pipe(Context.add(Client.SqlClient, client)),
      ),
    ),
  ).pipe(Layer.provide(Reactivity.layer));

/**
 * Creates a layer from a concrete SQL Server client configuration, providing both `MssqlClient` and
 * `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates the SQL Server statement compiler, using `@1`-style placeholders, bracket-escaped
