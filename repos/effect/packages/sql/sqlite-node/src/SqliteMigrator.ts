 ...
 *
 * This module re-exports the shared migration loaders and errors, then provides `run` and `layer`
 * helpers that apply pending migration files with the current `SqlClient`. It does not add
 * Node-specific schema dump support; migration execution is handled by the shared SQL migrator.
 *
 * @since 4.0.0
 */
...
/**
 * Runs SQL migrations for a SQLite database using the shared `Migrator` implementation and the
 * current `SqlClient`.
 *
 * @since 4.0.0
...
/**
 * Creates a layer that runs the configured SQLite migrations during layer construction and provides
 * no services.
 *
 * @since 4.0.0
 * @category Constructors
