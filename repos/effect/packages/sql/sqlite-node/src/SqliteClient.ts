 ...
 *
 * This module opens a SQLite database and exposes it as both `SqliteClient` and the generic Effect
 * SQL client. It serializes access through one connection, caches prepared statements, enables WAL
 * mode unless disabled, and supports database backup, and extension loading. Streaming queries and
...
/**
 * Node SQLite client service, extending `SqlClient` with database export, backup, and extension
 * loading helpers. `updateValues` is not supported.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Configuration for a node SQLite client backed by `node:sqlite`, including the database filename,
 * read-only mode, statement cache settings, WAL behavior, span attributes, and query/result name
 * transforms.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Creates a scoped node SQLite client from the supplied configuration, using a single serialized
 * connection with WAL enabled by default and exposing SQLite-specific `export`, `backup`, and
...
export const make = (
  ...
  Effect.gen(function* () {
    ...
    const makeConnection = Effect.gen(function* () {
        ...
        Effect.withFiber<ReadonlyArray<any>, SqlError>((fiber) => {
          ...
          return Effect.try({
            try: () => {
              ...
              return raw
                ? ({ changes: result.changes, lastInsertRowid: result.lastInsertRowid } as any)
                : [];
            },
            catch: (cause) =>
              new SqlError({
      ...
      const runStatementValues = (statement: StatementSync, params: ReadonlyArray<unknown>) =>
        Effect.withFiber<ReadonlyArray<ReadonlyArray<unknown>>, SqlError>((fiber) => {
          ...
          return Effect.try({
            try: () => {
              statement.setReadBigInts(useSafeIntegers);
              if (statement.columns().length > 0) {
                return statement.all(...(params as Array<any>)) as unknown as ReadonlyArray<
      ...
      const runValuesUnprepared = (sql: string, params: ReadonlyArray<unknown>) =>
...
/**
 * Builds a layer from an Effect `Config` value, providing both the node `SqliteClient` service and
...
/**
 * Builds a layer from a node SQLite client configuration, providing both `SqliteClient` and the
 * generic `SqlClient` service.
 *
 * @since 4.0.0
 * @category Layers
 */
