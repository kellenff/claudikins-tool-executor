/**
 * Connects Effect SQL to PGlite, the embedded PostgreSQL-compatible database from
 * `@electric-sql/pglite`.
 *
 * This module can create a managed PGlite instance or wrap an existing one and expose it as both
 * `PgliteClient` and the generic Effect SQL client. The client runs PostgreSQL-style SQL, adds
 * helpers for JSON values and LISTEN/NOTIFY messages, can dump the PGlite data directory, and can
 * refresh PGlite array types. It also provides layers and maps common PostgreSQL-style failures
 * into Effect SQL errors.
 *
 * @since 4.0.0
 */
...
/**
 * PGlite-backed PostgreSQL client service, extending `SqlClient` with access to the PGlite
 * instance, JSON fragments, LISTEN/NOTIFY, data directory dumps, and array type refresh.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Configuration for a PGlite client, either by supplying PGlite creation options or an existing
 * live PGlite client.
 *
 * @since 4.0.0
 * @category Models
...
export declare namespace PgliteClientConfig {
  /**
   * Shared PGlite client options for span attributes, query/result name transformations, and JSON
  ...
  /**
   * Configuration that uses an existing PGlite client. The supplied `liveClient` is caller-owned
   * and is not closed by the Effect client.
   *
   * @since 4.0.0
   * @category Models
   */
  ...
  /**
   * Config-friendly subset of PGlite creation options, including data directory, username,
   * database, relaxed durability, and shared transform options.
   *
   * @since 4.0.0
   * @category Models
   */
...
/**
 * Creates a scoped PGlite SQL client. When no live client is supplied it creates and closes a
 * PGlite instance; when `liveClient` is supplied, the caller retains ownership.
 *
 * @since 4.0.0
        ...
        : yield* Effect.acquireRelease(
            Effect.tryPromise({
              try: async () => {
                ...
                new SqlError({
                  reason: classifyError(cause, "PgliteClient: Failed to connect", "connect"),
                }),
            }),
...
/**
 * Builds a `PgliteClient` around an existing PGlite instance, adding SQL client operations,
 * LISTEN/NOTIFY, dump helpers, and serialized access.
 *
 * @since 4.0.0
 * @category Constructors
 */
          ...
          Effect.acquireRelease(
            Effect.tryPromise({
              try: () =>
                ...
                new SqlError({ reason: classifyError(cause, "Failed to listen", "listen") }),
            }),
      ...
      notify: (channel: string, payload: string) =>
        Effect.tryPromise({
          try: () => pglite.exec(`NOTIFY ${escape(channel)}, ${escapeLiteral(payload)}`),
            ...
            new SqlError({ reason: classifyError(cause, "Failed to notify", "notify") }),
        ...
        semaphore.withPermit(
          Effect.tryPromise({
            try: () => pglite.dumpDataDir(compression),
              ...
              new SqlError({
                reason: classifyError(cause, "Failed to dump data dir", "dumpDataDir"),
              }),
          }),
        ),
      refreshArrayTypes: semaphore.withPermit(
        Effect.tryPromise({
          try: () => pglite.refreshArrayTypes(),
            ...
            new SqlError({
              reason: classifyError(cause, "Failed to refresh array types", "refreshArrayTypes"),
            }),
        }),
      ),
    });
    ...
    return Effect.map(
      Effect.tryPromise({
        try: () => this.pglite.query<any>(sql, params as Array<any>),
          ...
          new SqlError({ reason: classifyError(cause, "Failed to execute statement", method) }),
      }),
      (result) => result.rows,
    );
  }
  ...
  executeRaw(sql: string, params: ReadonlyArray<unknown>) {
    return Effect.tryPromise({
      try: () => this.pglite.query<any>(sql, params as Array<any>),
        ...
        new SqlError({ reason: classifyError(cause, "Failed to execute statement", "executeRaw") }),
    });
  }
    ...
    return Effect.map(
      Effect.tryPromise({
        try: () => this.pglite.query<any>(sql, params as Array<any>, { rowMode: "array" }),
          ...
          new SqlError({
            reason: classifyError(cause, "Failed to execute statement", "executeValuesUnprepared"),
          }),
      }),
      (result) => result.rows as ReadonlyArray<ReadonlyArray<any>>,
    );
...
/**
 * Creates a layer from an effect that acquires a `PgliteClient`, providing both `PgliteClient` and
 * `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates a layer from a `Config`-wrapped PGlite client configuration, providing both
 * `PgliteClient` and `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
 */
export const layerConfig: (
  ...
  layerFrom(
    Effect.flatMap(Config.unwrap(config), (resolved) => make(resolved as PgliteClientConfig)),
  );

/**
 * Creates a layer from a concrete PGlite client configuration, providing both `PgliteClient` and
 * `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates the PGlite statement compiler, using PostgreSQL `$1` placeholders, double-quoted
 * identifiers, returning clauses, and optional JSON value transformation.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * PGlite-specific custom statement fragments supported by the compiler, currently JSON parameter
 * fragments.
 *
 * @since 4.0.0
