 ...
 *
 * This module provides both the ClickHouse-specific {@link ClickhouseClient} service and the generic
 * {@link Client.SqlClient} service. `make` creates a scoped client, checks the connection with
 * `SELECT 1`, maps ClickHouse errors to `SqlError`, and aborts in-flight queries when interrupted.
 * The ClickHouse-specific service adds typed parameters, command execution, insert queries, query
 * id and settings helpers, a statement compiler, and direct or config-backed layers.
 *
 * @since 4.0.0
 */
...
/**
 * ClickHouse-specific `SqlClient` extension with access to its configuration, typed parameter
 * fragments, command-mode execution, insert queries, and per-effect query ID and ClickHouse
 * settings.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Configuration for creating a ClickHouse client, combining `@clickhouse/client` options with
 * optional span attributes and query/result name transforms.
 *
 * @since 4.0.0
...
/**
 * Creates a scoped `ClickhouseClient`, verifies connectivity with `SELECT 1`, closes the underlying
 * client when the scope ends, maps ClickHouse failures to `SqlError`, and aborts plus kills
 * in-flight queries when interrupted.
 *
 * @since 4.0.0
 * @category Constructors
 */
  ...
  Effect.gen(function* () {
    ...
    class ConnectionImpl implements Connection {
      ...
      private runRaw(
        ...
        return Effect.withFiber<Clickhouse.ResultSet<"JSON"> | Clickhouse.CommandResult, SqlError>(
          (fiber) => {
        ...
        return this.runRaw(sql, params, "JSONEachRow").pipe(
          ...
          Stream.mapEffect((chunk) => {
            ...
            return Effect.tryPromise({
              try: () =>
                Promise.all(promises).then((rows) => (transformRows ? transformRows(rows) : rows)),
              catch: (cause) =>
                new SqlError({ reason: classifyError(cause, "Failed to parse row", "parseRow") }),
            });
          }),
...
/**
 * Fiber reference read by the low-level ClickHouse connection to choose query or command execution
 * for statements; defaults to `query`.
 *
 * @since 4.0.0
 * @category References
 */
...
/**
 * Fiber reference for the ClickHouse `query_id` applied to queries and inserts; a random UUID is
 * generated when no query ID is set.
 *
 * @since 4.0.0
 * @category References
 */
...
/**
 * Provides both `ClickhouseClient` and generic `SqlClient` services from a `Config`-backed
...
/**
 * Provides both `ClickhouseClient` and generic `SqlClient` services from a ClickHouse client
 * configuration.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates the SQL statement compiler for ClickHouse, emitting typed `{pN: Type}` placeholders and
 * escaping identifiers with an optional query name transform.
 *
 * @since 4.0.0
 * @category Compiler
 */
