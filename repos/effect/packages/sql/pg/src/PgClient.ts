 ...
 *
 * This module provides constructors and layers for building a PostgreSQL client from pool settings,
 * a managed `pg.Client`, an existing `pg.Pool`, or custom connection code. The client runs Effect
 * SQL queries against PostgreSQL, including transactions and streamed results, and adds helpers for
 * JSON values and LISTEN/NOTIFY messages. It also maps common PostgreSQL failures, such as
 * connection, authentication, constraint, timeout, and deadlock errors, into Effect SQL errors.
 *
 * @since 4.0.0
 */
...
/**
 * PostgreSQL client service, extending `SqlClient` with JSON parameter fragments and LISTEN/NOTIFY
 * helpers.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Configuration for a PostgreSQL client, including connection, TLS, custom stream, application
 * name, type parser, JSON transform, and query/result name transform options.
 *
 * @since 4.0.0
...
/**
 * PostgreSQL pool configuration, extending `PgClientConfig` with idle timeout, pool size, and
...
/**
 * Creates a scoped PostgreSQL client backed by a managed single `pg` client, optionally acquiring a
 * separate client for streaming and LISTEN operations.
 *
 * @since 4.0.0
 * @category Constructors
 */
    ...
    acquire: Effect.acquireRelease(
      Effect.tryPromise({
        try: async () => {
          ...
          new SqlError({ reason: classifyError(cause, "PgClient: Failed to connect", "connect") }),
      }),
...
/**
 * Builds a PostgreSQL client from a scoped `pg` pool acquisition effect, deriving transaction,
 * streaming, and LISTEN/NOTIFY support from that pool.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Builds a PostgreSQL client from a scoped `pg` client acquisition effect, serializing access when
 * sharing the client and optionally using separate clients for streams and LISTEN.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Creates a `PgClient` from SQL connection acquirers, a LISTEN acquirer, client configuration, and
 * transformation options.
 *
 * **When to use**
 *
 * Use to build a PostgreSQL client from custom connection acquisition logic instead of the built-in
 * pool or single-client constructors.
 *
 * @since 4.0.0
 * @category Constructors
 */
    ...
    return Stream.fromChannel(
      Channel.fromTransform(
        Effect.fnUntraced(function* (_, scope) {
          ...
          return Effect.callback<Arr.NonEmptyReadonlyArray<any>, SqlError | Cause.Done>(
            (resume) => {
...
/**
 * Creates a layer from an effect that acquires a `PgClient`, providing both `PgClient` and
 * `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates a layer from a `Config`-wrapped PostgreSQL pool configuration, providing both `PgClient`
 * and `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates a layer from a concrete PostgreSQL pool configuration, providing both `PgClient` and
 * `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates the PostgreSQL statement compiler, using `$1` placeholders, double-quoted identifiers,
 * PostgreSQL returning clauses, and optional JSON value transformation.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * PostgreSQL-specific custom statement fragments supported by the compiler, currently JSON
 * parameter fragments.
 *
 * @since 4.0.0
