 ...
 *
 * This module reuses the shared SQL migrator and connects it to PostgreSQL. It exposes the common
 * migration helpers and adds `run` and `layer` functions that apply pending migration files with
 * the current SQL client. When schema dumps are requested, it uses `pg_dump` and the usual process
...
/**
 * Runs PostgreSQL SQL migrations using the configured clients. Schema dumps use `pg_dump` and
 * require child process, filesystem, and path services.
 *
 * @since 4.0.0
  ...
  dumpSchema(path, table) {
    const pgDump = (args: Array<string>) =>
      Effect.gen(function* () {
        ...
        Effect.mapError(
          (error) => new Migrator.MigrationError({ kind: "Failed", message: error.message }),
        ),
      );
...
/**
 * Creates a layer that runs PostgreSQL migrations during layer construction, including
 * `pg_dump`-based schema dump support when requested.
 *
 * @since 4.0.0
 * @category Layers
 */
