...
describe("Graph", () => {
  it("make", () => {
    ...
    expect(Graph.make("undirected")<string, number>).type.not.toBeCallableWith(() =>
      Promise.resolve(),
    );
  });
  ...
  it("guards", () => {
    ...
    if (Graph.isGraph(known)) {
      expect(known).type.toBe<
        Graph.UndirectedGraph<string, number> | Graph.MutableUndirectedGraph<string, number>
      >();
    }
  ...
  it("compose", () => {
    expect(
      Graph.compose(directedNodes, directedNodes, {
        nodeIdentity: (node) => {
  ...
  it("intersection", () => {
    expect(Graph.intersection(directedNodes, directedNodes)).type.toBe<
      Graph.DirectedGraph<Node, number>
    >();

    expect(pipe(undirectedNodes, Graph.intersection(undirectedNodes))).type.toBe<
      Graph.UndirectedGraph<Node, number>
    >();
  });

  it("difference", () => {
    expect(
      Graph.difference(directedNodes, directedNodes, {
        nodeIdentity: (node) => {
          expect(node).type.toBe<Node>();
          return node.id;
        },
  ...
  it("symmetricDifference", () => {
    expect(
      Graph.symmetricDifference(directedNodes, directedNodes, {
        nodeIdentity: (node) => {
          expect(node).type.toBe<Node>();
          return node.id;
        },
  ...
  it("complement", () => {
    expect(
      Graph.complement(directed, (source, target) => {
  ...
  it("sum", () => {
    ...
    expect(pipe(undirected, Graph.sum(undirected))).type.toBe<
      Graph.UndirectedGraph<string, number>
    >();
  });
});
