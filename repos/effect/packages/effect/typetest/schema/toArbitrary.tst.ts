...
describe("toArbitrary", () => {
  ...
  it("passes recursion metadata in the arbitrary context", () => {
    Schema.String.annotate({
      toArbitrary: () => (fc, context) => {
        expect(context.constraint).type.toBe<
          Schema.Annotations.ToArbitrary.GenerationConstraint | undefined
        >();
  ...
  it("passes type parameter derivations to declaration arbitrary hooks", () => {
    Schema.declareConstructor<ReadonlyArray<string>>()([Schema.String], () => null as any, {
      toArbitrary:
        ([value]) =>
        (fc) => {
