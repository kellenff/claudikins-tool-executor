...
describe("HttpApiBuilder", () => {
  describe("group", () => {
    ...
    describe("handleAll", () => {
      ...
      it("validates partial batches", () => {
        ...
        expect(HttpApiBuilder.group).type.not.toBeCallableWith(Api, "users", incomplete);
        ...
        expect(HttpApiBuilder.group).type.toBeCallableWith(Api, "users", completeInTwoSteps);
    ...
    describe("handler validation", () => {
      it("rejects incomplete synchronous handler collections", () => {
        ...
        expect(HttpApiBuilder.group).type.not.toBeCallableWith(Api, "users", build);
      });
      ...
      it("rejects duplicate handlers", () => {
        ...
        const build = (
          ...
          const afterGetUser = handlers.handle("getUser", ({ params }) =>
            Effect.succeed({ id: params.id }),
          );

          expect(afterGetUser.handle).type.not.toBeCallableWith("getUser", () =>
            Effect.succeed({ id: "id" }),
          );
          expect(afterGetUser.handleRaw).type.not.toBeCallableWith("getUser", () =>
            Effect.succeed({ id: "id" }),
          );
        ...
        expect(HttpApiBuilder.group).type.toBeCallableWith(Api, "users", build);
      });
      ...
      it("limits handle registrations to remaining endpoint identifiers", () => {
        ...
        HttpApiBuilder.group(Api, "users", (handlers) => {
          expect(handlers.handle).type.not.toBeCallableWith("missing", () =>
            Effect.succeed({ id: "id" }),
          );

          const remaining = handlers.handle("getUser", ({ params }) =>
            Effect.succeed({ id: params.id }),
          );

          expect(remaining.handle).type.not.toBeCallableWith("getUser", () =>
            Effect.succeed({ id: "id" }),
          );
    ...
    describe("handleRaw", () => {
      it("preserves request inference", () => {
        ...
        const handlers = HttpApiBuilder.group(Api, "files", (handlers) =>
  ...
  describe("endpoint", () => {
    ...
    it("selects the intended class-like endpoint when identifiers overlap", () => {
      class UsersToken extends Context.Service<
        UsersToken,
        {
          readonly token: "users";
        }
      >()("UsersToken") {}
      class UsersCurrent extends Context.Service<
        UsersCurrent,
        {
          readonly userId: string;
        }
      >()("UsersCurrent") {}
      class AdminsToken extends Context.Service<
        AdminsToken,
        {
          readonly token: "admins";
        }
      >()("AdminsToken") {}
      class AdminsCurrent extends Context.Service<
        AdminsCurrent,
        {
