...
describe("HttpApiClient", () => {
  ...
  describe("group", () => {
    it("selects one group when endpoint identifiers overlap", () => {
      ...
      expect(selectedUsers.lookup).type.not.toBeCallableWith({
  ...
  describe("error", () => {
    it("defaults to client and schema errors", () => {
      ...
      expect(client.group.a()).type.toBe<
        Effect.Effect<void, HttpClientError.HttpClientError | Schema.SchemaError>
      >();
    });
    ...
    it("includes errors decoded from the endpoint schema", () => {
      ...
      expect(client.group.a()).type.toBe<
        Effect.Effect<
          void,
          { readonly a: number } | HttpClientError.HttpClientError | Schema.SchemaError
        >
      >();
    });
  });
  ...
  describe("endpoint", () => {
    it("selects an endpoint and preserves its request and response types", () => {
      ...
      >();
      expect(
        getUser({ params: { id: "1" }, query: { page: 1 }, responseMode: "decoded-and-response" }),
      ).type.toBe<
        Effect.Effect<
      ...
      >();
      expect(
        getUser({ params: { id: "1" }, query: { page: 1 }, responseMode: "response-only" }),
      ).type.toBe<
        Effect.Effect<HttpClientResponse.HttpClientResponse, HttpClientError.HttpClientError>
      >();
    ...
    it("selects within the requested group when endpoint identifiers overlap", () => {
      class UsersEndpointError extends Schema.TaggedErrorClass<UsersEndpointError>()(
        "UsersEndpointError",
        {},
      ) {}
      class AdminsEndpointError extends Schema.TaggedErrorClass<AdminsEndpointError>()(
        "AdminsEndpointError",
        {},
      ) {}
      class UsersClientError extends Schema.TaggedErrorClass<UsersClientError>()(
        "UsersClientError",
        {},
      ) {}
      class AdminsClientError extends Schema.TaggedErrorClass<AdminsClientError>()(
        "AdminsClientError",
        {},
      ) {}

      class UsersMiddleware extends HttpApiMiddleware.Service<
        UsersMiddleware,
        {
      ...
      class AdminsMiddleware extends HttpApiMiddleware.Service<
        AdminsMiddleware,
        {
      ...
      expect<Effect.Services<typeof usersEndpoint>>().type.toBe<
        HttpApiMiddleware.ForClient<UsersMiddleware>
      >();
      expect<Effect.Services<typeof adminsEndpoint>>().type.toBe<
        HttpApiMiddleware.ForClient<AdminsMiddleware>
      >();
        ...
        HttpApiClient.endpoint(Api, {
          ...
          transformClient: (client) => {
            expect(client).type.toBe<
              HttpClient.HttpClient.With<CustomClientError, CustomService>
            >();
            return client;
          },
        }),
  ...
  describe("client middleware", () => {
    it("requires layers and includes errors for required client middleware", () => {
      ...
      expect(client.group.a()).type.toBe<
        Effect.Effect<
          string,
          RequiredClientError | HttpClientError.HttpClientError | Schema.SchemaError
        >
      >();
