...
describe("HttpApiEndpoint", () => {
  ...
  describe("params", () => {
    ...
    it("accepts a field record", () => {
      ...
      expect(endpoint["~Params"]).type.toBe<
        Schema.toCodecStringTree<Schema.Struct<{ id: Schema.Finite }>>
      >();
    });
  ...
  describe("success", () => {
    ...
    it("preserves mixed buffered and stream schemas", () => {
      ...
      const endpoint = HttpApiEndpoint.get("a", "/a", {
        success: [Schema.Struct({ message: Schema.String }), stream],
      });

      expect(endpoint["~Success"]).type.toBe<
        Schema.toCodecJson<Schema.Struct<{ readonly message: Schema.String }>> | typeof stream
      >();
    });
    ...
    it("includes StreamSse event and error schema services", () => {
      ...
      const Error =
        hole<Schema.Codec<StreamError, StreamError, "ErrorDecoding", "ErrorEncoding">>();
      const endpoint = HttpApiEndpoint.get("a", "/a", {
        success: HttpApiSchema.StreamSse({ events: Events, error: Error }),
      });

      expect<HttpApiEndpoint.ServerServices<typeof endpoint>>().type.toBe<
        "EventsEncoding" | "ErrorEncoding"
      >();
      expect<HttpApiEndpoint.ClientServices<typeof endpoint>>().type.toBe<
  ...
  describe("error services", () => {
    it("includes endpoint and middleware decoding services", () => {
      ...
      const EndpointError =
        hole<Schema.Codec<EndpointError, EndpointError, "EndpointErrorDecoding", never>>();
      const MiddlewareError =
        hole<Schema.Codec<MiddlewareError, MiddlewareError, "MiddlewareErrorDecoding", never>>();
    ...
    it("includes endpoint and middleware encoding services", () => {
      ...
      const EndpointError =
        hole<Schema.Codec<EndpointError, EndpointError, never, "EndpointErrorEncoding">>();
      const MiddlewareError =
        hole<Schema.Codec<MiddlewareError, MiddlewareError, never, "MiddlewareErrorEncoding">>();
