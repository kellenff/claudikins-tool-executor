...
describe("Prompt", () => {
  describe("custom", () => {
    ...
    it("with events, process receives ProcessInput<A>", () => {
      Prompt.custom({ count: 0 }, stringEvents, {
        render: () => Effect.succeed(""),
        process: (input, _state) => {
    ...
    it("ProcessInput is a discriminated union narrowed by _tag", () => {
      Prompt.custom({ count: 0 }, objectEvents, {
        render: () => Effect.succeed(""),
        process: (input, _state) => {
          if (input._tag === "Input") {
    ...
    it("returns Prompt<Output>", () => {
      const prompt = Prompt.custom(0, {
        render: () => Effect.succeed(""),
        process: ({ key }, state) =>
          Effect.succeed(
            key.name === "enter"
              ? { _tag: "Submit" as const, value: state }
              : { _tag: "Beep" as const },
          ),
        clear: () => Effect.succeed(""),
      });
    ...
    it("returns Prompt<Output> with events", () => {
      const prompt = Prompt.custom(0, stringEvents, {
        render: () => Effect.succeed(""),
        process: (input, state) =>
          Effect.succeed(
            Match.value(input).pipe(
              ...
              Match.tag("Event", ({ value }) =>
                value === "tick"
                  ? { _tag: "NextFrame" as const, state: state + 1 }
                  : { _tag: "Beep" as const },
              ),
              Match.exhaustive,
            ),
