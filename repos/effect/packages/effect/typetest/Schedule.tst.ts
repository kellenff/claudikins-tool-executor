...
describe("Schedule", () => {
  ...
  it("tap", () => {
    ...
    expect(schedule).type.toBe<
      Schedule.Schedule<number, string, "error" | "tapError", "service" | "tapService">
    >();
  });
  ...
  it("modifyDelay", () => {
    ...
    expect(schedule).type.toBe<
      Schedule.Schedule<
        number,
        string,
        "error" | "modifyDelayError",
        "service" | "modifyDelayService"
      >
    >();
  });
  ...
  it("max", () => {
    const first =
      hole<Schedule.Schedule<number, { readonly first: string }, "firstError", "firstService">>();
    const second =
      hole<
        Schedule.Schedule<boolean, { readonly second: number }, "secondError", "secondService">
      >();
