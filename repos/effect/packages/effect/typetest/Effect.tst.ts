...
describe("Effect.catchReasons", () => {
  ...
  it("keeps other error tags when orElse re-fails", () => {
    const result = pipe(
      mixedEffect,
      Effect.catchReasons("AiError", { RateLimitError: () => Effect.succeed("ok") }, () =>
        Effect.fail(new SimpleError({ code: 1 })),
      ),
    );
    expect(result).type.toBe<Effect.Effect<string, OtherError | SimpleError>>();
...
describe("Effect.catchTags", () => {
  it("removes handled errors when orElse is omitted", () => {
    const result = pipe(mixedEffect, Effect.catchTags({ AiError: () => Effect.succeed("ok") }));
    expect(result).type.toBe<Effect.Effect<string, OtherError>>();
  });
...
describe("Effect.unwrapReason", () => {
  it("replaces parent error with reasons", () => {
    ...
    expect(result).type.toBe<
      Effect.Effect<string, RateLimitError | QuotaExceededError | UnknownAiModelError>
    >();
  });
...
describe("Effect.partition", () => {
  it("data-first", () => {
    ...
    expect(result).type.toBe<
      Effect.Effect<[excluded: Array<string>, satisfying: Array<number>], never, never>
    >();
  });
  ...
  it("data-last", () => {
    ...
    expect(result).type.toBe<
      Effect.Effect<[excluded: Array<number>, satisfying: Array<string>], never, never>
    >();
  });
});
