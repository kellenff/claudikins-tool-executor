...
const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
  handlers
    ...
    .handle("oldPage", () => Effect.succeed(HttpServerResponse.redirect("/new", { status: 302 }))),
);
...
const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
  handlers.handle("getStream", () => Effect.succeed(HttpServerResponse.stream(stream))),
);
...
class AuthInfo extends Context.Service<
  AuthInfo,
  {
...
class LoadAuth extends HttpApiMiddleware.Service<
  LoadAuth,
  {
...
class RequireAuth extends HttpApiMiddleware.Service<
  RequireAuth,
  {
  ...
  // Return the security handlers for the middleware
  {
    ...
    myBearer: (effect, opts) =>
      Effect.provideServiceEffect(
        ...
        CurrentUser,
        Effect.gen(function* () {
          yield* Effect.log("checking bearer token", Redacted.value(opts.credential));
...
const api = HttpApi.make("api")
  ...
  .annotate(HttpApi.AdditionalSchemas, [Schema.String.annotate({ identifier: "MyString" })])
  // Add a description
