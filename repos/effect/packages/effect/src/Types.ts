 ...
 *
 * Everything in this module is type-level only; it does not define runtime values. The types are
 * used throughout Effect to work with tuple lengths, object shapes, tagged unions, reason-tagged
 * errors, mutability, exactness, required keys, concurrency settings, and variance markers.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need a fixed-length array type, especially instead of manually writing `[T, T, T,
 * ...]` for longer tuples.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need a minimum-length array type that still allows additional elements. This is
 ...
 * **When to use**
 *
 * Use to combine all members of a union into a single type with all their properties. This is
 ...
 * **Details**
 *
 * - Uses the `<T>() => T extends X ? 1 : 2` trick for exact equality, distinguishing between `any`,
 *   `unknown`, `never`, and other types.
 ...
 * **When to use**
 *
 * Use to branch type-level logic when at least one key from a candidate key set exists on an object
 * type.
 *
 * **Details**
 *
...
/**
 * Left-biased merge of two object types where keys from `Source` take precedence over `Target` on
 * conflict.
 *
 * **When to use**
 *
...
/**
 * Right-biased merge of two object types where keys from `Source` take precedence over `Target` on
 * conflict.
 *
 * **When to use**
 *
...
/**
 * Recursively removes `readonly` from all properties, including nested objects, arrays, `Map`, and
 * `Set`.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use as a phantom field type to make a type parameter invariant, neither covariant nor
 * contravariant.
 *
 * **Details**
 *
 * A value of type `Invariant<A>` cannot be assigned to `Invariant<B>` unless `A` and `B` are the
 * same type.
 *
 ...
 * **Details**
 *
 * `Covariant<A>` is assignable to `Covariant<B>` when `A extends B`, following the subtype
 * direction.
 *
 ...
 * **Details**
 *
 * `Contravariant<A>` is assignable to `Contravariant<B>` when `B extends A`, following the
 * supertype direction.
 *
 ...
 * **When to use**
 *
 * Use when Effect's type-level machinery needs to represent a type parameter that has not been
 * assigned yet.
 *
 * **Details**
 *
 * Used internally by the Effect type system to indicate that a type parameter has not been assigned
 * a concrete type.
 *
 * @since 4.0.0
 * @category Types
 ...
 * **When to use**
 *
 * Use when Effect's type-level machinery needs to represent an error type that has not been handled
 * yet.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when an error type stores nested sub-errors in a `reason` field and you need that field's
 * full union type as a standalone type.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * This is shorthand for `Tags<ReasonOf<E>>`. It returns `never` if `E` has no `reason` field or the
 * reason has no `_tag`.
 *
 ...
 * **When to use**
 *
 * Use when you need the nested reason variant type itself, selected by `_tag`, rather than the
 ...
 * **When to use**
 *
 * Use to preserve the original error shape while narrowing its nested reason field to the matching
 * variant.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need the remaining nested reason union type after removing variants handled by
 * `_tag`, rather than the enclosing error type.
 *
 * **Details**
 *
 * Returns `never` if `E` has no `reason` field.
