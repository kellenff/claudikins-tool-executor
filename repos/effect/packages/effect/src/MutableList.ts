/**
 * Mutable lists for collecting ordered values and draining them from the front. A `MutableList<A>`
 * can append values to the end, prepend values to the beginning, take one or more values from the
 * front, inspect its contents as an array, filter values, remove values, and clear itself. All
 * operations update the same list object in place and keep its `length` field current. Taking from
 * an empty list returns the `Empty` symbol.
 *
 * @since 4.0.0
 */
...
/**
 * A mutable linked list data structure optimized for high-throughput operations. MutableList
 * provides efficient append/prepend operations and is ideal for producer-consumer patterns, queues,
...
/**
 * The MutableList namespace contains type definitions and utilities for working with mutable linked
 * lists.
 *
...
export declare namespace MutableList {
  /**
   * Storage node used by the exposed `head` and `tail` fields of a `MutableList`.
   *
   * **Details**
   *
   * Most code should treat buckets as an implementation detail and use `MutableList` operations
   * such as `append`, `prepend`, and `take` instead of constructing or mutating buckets directly.
   *
...
/**
 * Defines the unique symbol used to represent an empty result when taking elements from a
 * MutableList. This symbol is returned by `take` when the list is empty, allowing for safe type
 * checking.
 *
 * **When to use**
 *
...
/**
 * Appends an element to the end of the MutableList. This operation is optimized for high-frequency
 * usage.
 *
...
/**
 * Prepends an element to the beginning of the MutableList. This operation is optimized for
...
/**
 * Prepends all elements from an iterable to the beginning of the MutableList. The elements are
 * added in order, so the first element in the iterable becomes the new head of the list.
 *
...
/**
 * Prepends all elements from a ReadonlyArray to the beginning of the MutableList. This is an
 * optimized version that can reuse the array when mutable=true.
 *
 * **When to use**
 *
 * Use when prepending a trusted array directly is worth the optimized path and you control whether
 * the input may be reused.
 *
 * **Gotchas**
 *
 * When mutable=true, the input array may be modified internally. Only use mutable=true when you
 * control the array lifecycle.
 *
...
/**
 * Appends all elements from an iterable to the end of the MutableList. Returns the number of
 * elements added.
 *
...
/**
 * Appends all elements from a ReadonlyArray to the end of the MutableList. This is an optimized
 * version that can reuse the array when mutable=true. Returns the number of elements added.
 *
 * **When to use**
 *
 * Use when appending a trusted array directly is worth the optimized path and you control whether
 * the input may be reused.
 *
 * **Gotchas**
 *
...
/**
 * Removes all elements from the MutableList, resetting it to an empty state. This operation is
 * highly optimized and releases all internal memory.
 *
...
/**
 * Takes up to N elements from the beginning of the MutableList and returns them as an array. The
 * taken elements are removed from the list. This operation is optimized for performance and
 * includes zero-copy optimizations when possible.
 *
 ...
 * **When to use**
 *
 * Use to discard a bounded number of values from the head of a `MutableList` when the removed
 * values are not needed.
 *
 * **Details**
 *
 * If `n` is less than or equal to zero, or the list is empty, the list is left unchanged. If `n` is
 * greater than or equal to the current length, the list is cleared.
 *
 * @since 4.0.0
 * @category Elements
...
/**
 * Takes all elements from the MutableList and returns them as an array. The list becomes empty
 * after this operation. This is equivalent to takeN(list, list.length).
 *
...
/**
 * Takes a single element from the beginning of the MutableList. Returns the element if available,
 * or the Empty symbol if the list is empty. The taken element is removed from the list.
 *
...
/**
 * Copies up to `n` elements from the beginning of the `MutableList` into a new array without
 * modifying the list.
 *
 * **When to use**
 *
...
/**
 * Filters the MutableList in place, keeping only elements that satisfy the predicate. This
 * operation modifies the list and rebuilds its internal structure for efficiency.
 *
...
/**
 * Removes all occurrences of a value from the `MutableList` using JavaScript strict equality
 * semantics.
 *
 * **When to use**
 *
 * Use when in-place removal should use JavaScript identity/strict equality rather than Effect
 * structural equality.
 *
 * **Details**
 *
