...
/**
 * Detects whether a date string already contains timezone info. Without a zone, `new
 * Date("2024-01-01T12:00:00")` is parsed as local time, so `makeUnsafe` appends "Z" to force UTC
 * interpretation. This check prevents appending "Z" to strings that already have a zone (e.g.
 * "2024-01-01T12:00:00Z", "...+05:30", "...GMT"), which would produce invalid dates.
 */
...
export const make: <A extends DateTime.DateTime.Input>(
  input: A,
) => Option.Option<DateTime.DateTime.PreserveZone<A>> = Option.liftThrowable(makeUnsafe);
