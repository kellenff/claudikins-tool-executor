...
function applyChecks(
  ...
  return filters.reduce(
    (acc, filter) =>
      acc.filter((a) => filter.run(a, ast, SchemaAST.defaultParseOptions) === undefined),
    arbitrary,
  );
}
...
function lengthToFastCheckConstraints(
  constraint:
    | { readonly minLength?: number | undefined; readonly maxLength?: number | undefined }
    | undefined,
) {
...
function array(
  ...
  return arrayWithConstraints(
    ...
    item,
    terminal
      ? { ...arrayConstraints, maxLength: arrayConstraints?.minLength ?? 0 }
      : arrayConstraints,
...
function appendArray(
  ...
  return out.chain((as) =>
    as.length < len ? fc.constant(as) : rest.map((rest) => [...as, ...rest]),
  );
}
...
function makeLazy<T>(normal: Lazy<T>, terminal: LazyOption<T>): LazyArbitraryWithContext<T> {
  const out = ((fc, ctx, recursionStack = emptyRecursionStack) =>
    normal(fc, ctx, recursionStack)) as LazyArbitraryWithContext<T>;
...
function reportChecks(
  ...
  function visit(check: SchemaAST.Check<any>, covered: boolean) {
    ...
    const nextCovered =
      covered || arbitrary?.constraint !== undefined || arbitrary?.candidate !== undefined;
    if (check._tag !== "Filter") {
      for (const child of check.checks) {
      ...
      const description =
        typeof meta === "object" && meta !== null && "_tag" in meta && typeof meta._tag === "string"
          ? meta._tag
...
function applyCandidates(
  ...
  const weighted: Array<FastCheck.WeightedArbitrary<any>> =
    base === undefined ? [] : [{ arbitrary: base, weight: 1 }];
...
function recur(
  ...
  if (annotation) {
    ...
    const derive =
      (lazyNormal: boolean): BaseBuilder =>
      (fc, ctx, nextCtx, recursionStack) =>
        normalizeDerivation(
          annotation(
  ...
  switch (ast._tag) {
    ...
    case "Number":
      return same((fc, ctx) => {
        const constraint = ctx.constraint;
        const ordered =
          constraint?.ordered?.order === Order.Number ? constraint.ordered : undefined;
    ...
    case "BigInt":
      return same((fc, ctx) => {
        const ordered =
          ctx.constraint?.ordered?.order === Order.BigInt ? ctx.constraint.ordered : undefined;
    ...
    case "Arrays": {
      ...
      const terminal: LazyOption<any> = (fc, ctx, recursionStack) => {
        ...
        const needsRest =
          Array.isReadonlyArrayNonEmpty(rest) && minLength > length + optionals.length;
        ...
        if (Array.isReadonlyArrayNonEmpty(rest)) {
          ...
          const headArbitrary =
            minRestLength === 0 ? undefined : head.arbitrary.terminal(fc, reset, recursionStack);
    ...
    case "Union": {
      ...
      const terminal: LazyOption<any> = (fc, ctx, recursionStack) =>
        oneOf(
          fc,
          types
            .map((type) => type.terminal(fc, ctx, recursionStack))
            .filter(Predicate.isNotUndefined),
        );
      return makeLazy((fc, ctx, recursionStack) => {
