...
/** @internal */
export const makeInterruptReason = (fiberId?: number | undefined): Cause.Interrupt =>
  new Interrupt(fiberId);
...
/** @internal */
export const causePrettyErrors = <E>(
  self: Cause.Cause<E>,
  options?: {
...
>(
  ...
  <A, E>(
    ...
    onNone?: LazyArg<E>,
  ): Effect.Effect<A, Cause.NoSuchElementError | E> =>
...
export const transposeOption = <A = never, E = never, R = never>(
  ...
  Option.isNone(self) ? succeedNone : map(self.value, Option.some);
...
const asyncFinalizer: (onInterrupt: () => Effect.Effect<void, any, any>) => Primitive =
  makePrimitive({
    op: "AsyncFinalizer",
    [contAll](fiber) {
...
> =>
  suspend(() =>
    fromIteratorUnsafe(args.length === 1 ? args[0]() : (args[1].call(args[0].self) as any)),
  );
...
const makeFn = (
  ...
  const body =
    typeof bodyOrOptions === "function" ? bodyOrOptions : pipeables.pop()!.bind(bodyOrOptions.self);
...
export const andThen: {
  ...
  <A, E, R, B, E2, R2>(
    ...
    flatMap(self, (a) =>
      isEffect(f) ? f : internalCall(() => (f as (a: A) => Effect.Effect<B, E2, R2>)(a)),
    ),
);
...
export const tap: {
  ...
  <A, E, R, B, E2, R2>(
    ...
    flatMap(self, (a) =>
      as(isEffect(f) ? f : internalCall(() => (f as (a: A) => Effect.Effect<B, E2, R2>)(a)), a),
    ),
);
...
/** @internal */
export const exitIsSuccess = <A, E>(self: Exit.Exit<A, E>): self is Exit.Success<A, E> =>
  self._tag === "Success";
...
/** @internal */
export const exitIsFailure = <A, E>(self: Exit.Exit<A, E>): self is Exit.Failure<A, E> =>
  self._tag === "Failure";
...
export const exitMatch: {
  ...
  <A, E, X1, X2>(
    ...
    exitIsSuccess(self) ? options.onSuccess(self.value) : options.onFailure(self.cause),
);
...
export const setContext: {
  ...
  2,
  <A, E, R>(self: Effect.Effect<A, E, R>, context: Context.Context<R>): Effect.Effect<A, E> =>
    updateContext(self, constant(context)),
);
...
export const provideService: {
  ...
  return dual(3, (self, service, impl) => provideServiceImpl(self, service, impl)).apply(
    this,
    arguments as any,
  ) as any;
};
...
/** @internal */
export const orDie = <A, E, R>(self: Effect.Effect<A, E, R>): Effect.Effect<A, never, R> =>
  catch_(self, die);
...
export const scopeMake = (
  finalizerStrategy?: "sequential" | "parallel",
): Effect.Effect<Scope.Closeable> => sync(() => scopeMakeUnsafe(finalizerStrategy));
...
export const cached = <A, E, R>(
  self: Effect.Effect<A, E, R>,
): Effect.Effect<Effect.Effect<A, E, R>> => cachedWithTTL(self, Duration.infinity);
...
const iterateEagerImpl = <S, A, X, E, R, E2>(options: {
  ...
  return (
    ...
    let nextIndex = index;
    const exits: Array<Exit.Exit<X, E> | undefined> | undefined = orderedStep
      ? new Array(end)
      : undefined;
...
export const filterMapOrElse: {
  ...
  <A, E, R, B, X, C, E2, R2>(
    ...
    flatMap(self, (a) => {
      ...
      return (
        Result.isFailure(result) ? orElse(result.failure) : succeed(result.success)
...
export const bindTo: {
  <N extends string>(
    name: N,
  ): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<Record<N, A>, E, R>;
...
export const forkDetach: {
  ...
  <A, E, R>(
    ...
    withFiber((fiber) =>
      succeed(forkUnsafe(fiber, self, options?.startImmediately, true, options?.uninterruptible)),
    ),
);
...
class Latch implements _Latch.Latch {
  ...
  release = withFiber<boolean>((fiber) =>
    this._isOpen ? succeedFalse : this.scheduleUnsafe(fiber),
  );
  openUnsafe() {
...
export const makeSpanUnsafe = <XA, XE>(
  ...
  if (disablePropagation) {
    ...
    const links =
      options?.links !== undefined ? [...linksFromEnv, ...options.links] : linksFromEnv.slice();
...
export const withParentSpan: {
  ...
  let provideStackFrame: <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<A, E, R> =
    identity;
  if (span._tag === "Span") {
...
/** @internal */
export const currentSpan: Effect.Effect<Tracer.Span, Cause.NoSuchElementError> = withFiber(
  (fiber) => {
    const span = fiber.currentSpanLocal;
...
/** @internal */
export const currentParentSpan: Effect.Effect<Tracer.AnySpan, Cause.NoSuchElementError> =
...
class ClockImpl implements Clock.Clock {
  ...
  private sleepMillis(millis: number): Effect.Effect<void> {
    ...
    return callback((resume) => {
      const continuation =
        millis > MAX_TIMER_MILLIS ? this.sleepMillis(millis - MAX_TIMER_MILLIS) : void_;
...
export const consolePretty = (options?: {
  ...
  const showColors =
    typeof options?.colors === "boolean" ? options.colors : processStdoutIsTTY || isBrowser;
