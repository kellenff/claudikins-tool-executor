...
/** @internal */
export const causeFromReasons = <E>(reasons: ReadonlyArray<Cause.Reason<E>>): Cause.Cause<E> =>
  new CauseImpl(reasons);
...
/** @internal */
export const isInterruptReason = <E>(self: Cause.Reason<E>): self is Cause.Interrupt =>
  self._tag === "Interrupt";
...
export const makeExit = <Fn extends (...args: Array<any>) => any, Prop extends string>(options: {
  ...
  const Proto = {
    ...
    [Equal.symbol](this: any, that: any): boolean {
      return (
        isExit(that) && that._tag === this._tag && Equal.equals(this[args], (that as any)[args])
      );
    },
    [Hash.symbol](this: any): number {
