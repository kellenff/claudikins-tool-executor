/**
 * Utility for safely manipulating `Error.stackTraceLimit` in environments where intrinsics may be
 * frozen (e.g., SES / hardened JavaScript or deterministic sandboxes such as Temporal). When the
 * property is non-writable, mutating it throws, so all manipulation here degrades to a best-effort,
 * silent no-op.
 *
 ...
 *
 * The error is constructed inline at each call site (rather than inside a closure here) so the
 * captured stack trace keeps pointing at the real caller instead of this module.
 *
 * @internal
 */
...
/**
 * Check if `Error.stackTraceLimit` is writable. Returns `false` if the property is frozen,
 * non-writable, or `Error` is non-extensible.
 *
 * @internal
 */
...
/**
 * Get the current `Error.stackTraceLimit` value. Returns `undefined` if the property doesn't exist.
 *
 * @internal
 */
 ...
 *
 * Accepts `undefined` so a value read via {@link getStackTraceLimit} can be restored faithfully.
 *
 * @internal
 */
