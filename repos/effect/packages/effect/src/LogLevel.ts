/**
 * Log-level types and helpers used by Effect logging. The module defines all accepted log levels,
 * the concrete emitted severities, the ordered level list, equality and ordering instances,
 * threshold comparison helpers, and an effect for checking whether a level is enabled by the
...
/**
 * Represents every level used by Effect logging, including concrete message severities and the
 * `All` and `None` sentinel levels.
 *
 * **When to use**
 *
 * Use to type values that may be either concrete log message severities or logging configuration
 * sentinels.
 *
 * **Details**
 *
...
/**
 * Log levels that represent actual message severities, excluding the `All` and `None` sentinel
 * levels.
 *
 * **When to use**
 *
 * Use when typing emitted log message severities, such as explicit log calls, current log level
 * references, or error-report severity annotations, where `All` and `None` are not valid values.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Returns all `LogLevel` values in order from `All` through the concrete severities to `None`.
 *
 * **When to use**
 *
 * Use to enumerate or validate all accepted `LogLevel` string values, including the `All` and
 * `None` sentinel levels.
 *
 * **Details**
 *
 * The array order matches the module severity order: `All`, concrete severities from `Fatal` to
 * `Trace`, then `None`.
 *
 * **Gotchas**
 *
 * This list includes `All` and `None`, so it is not limited to concrete emitted severities.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * This order treats "All" as the least restrictive level and "None" as the most restrictive, with
 * Fatal being the most severe actual log level.
 *
 ...
 * **When to use**
 *
 * Use to compare two `LogLevel` values when only the exact same level should match.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to project a `LogLevel` into the numeric sort key used by `LogLevel.Order` when custom
 * ordering code or an integration needs a number instead of an `Order` comparison.
 *
 * **Details**
 *
 * The mapping is `All` to `Number.MIN_SAFE_INTEGER`, `Trace` to `0`, `Debug` to `10000`, `Info` to
 * `20000`, `Warn` to `30000`, `Error` to `40000`, `Fatal` to `50000`, and `None` to
 ...
 * **When to use**
 *
 * Use to implement minimum log-level filtering by checking whether a message level meets a
 * threshold.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to implement maximum log-level filtering by checking whether a level is at or below a
 * threshold.
 *
 * **Details**
 *
