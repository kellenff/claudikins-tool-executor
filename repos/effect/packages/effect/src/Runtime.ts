/**
 * Helpers for turning an `Effect` program into a host application's main entry point. This module
 * is the low-level layer used by platform adapters to run a main effect, observe its fiber, report
 * unhandled failures, and translate the resulting `Exit` into an application or process exit code.
 * It provides `makeRunMain`, the default teardown behavior, and error markers for custom exit codes
 * and already-reported failures. Application code usually calls the platform-provided runner
 ...
 * **When to use**
 *
 * Use when integrating {@link makeRunMain} with a host platform that needs to translate an Effect
 * `Exit` into a process, worker, or application exit code.
 *
 * **Details**
 *
 * A teardown function is called when an Effect program completes, either successfully or with a
 * failure. It determines the appropriate exit code and can perform cleanup before invoking the
 * supplied `onExit` callback.
 *
 ...
 * **When to use**
 *
 * Use as the standard teardown for main programs with conventional process exit codes and support
 * for {@link errorExitCode}.
 *
 * **Details**
 *
 ...
 * - The squashed error's {@link errorExitCode} value for other failures when present.
 * - `1` for other failures.
 *
 * **Gotchas**
 *
 * The `130` code is used only when the Cause contains interruptions and no other failure reasons.
 * Mixed causes use the squashed error path instead.
 *
 ...
 * **Details**
 *
 * The runner executes Effect programs as main entry points. The provided function receives a forked
 * fiber and a teardown callback so it can install platform-specific signal handling, fiber
 * observers, and final exit behavior.
 *
 * Most applications should use a platform-provided runner, such as `NodeRuntime.runMain`, rather
 * than constructing one directly.
 *
 * `disableErrorReporting` disables the automatic log emitted for unreported non-interruption
 * failures. It does not change exit-code calculation or the custom teardown callback.
 *
 * **Gotchas**
 *
 * The setup function is responsible for observing the fiber and eventually invoking teardown.
 * `makeRunMain` also tries to keep the host process alive with a long interval while the main fiber
 * is running; if the host blocks timers, the runner still starts but cannot use that keep-alive
 * fallback.
 *
 ...
 * **When to use**
 *
 * Use when error classes should map failures to a specific process exit code when handled by
 * {@link defaultTeardown}.
 *
 * **Details**
 *
 * Attach this marker as a readonly property on an error object. When the main program fails,
 * {@link defaultTeardown} squashes the Cause and reads the marker from the resulting error value.
 *
 * **Gotchas**
 *
 * The marker is read from the squashed failure value. If a Cause contains multiple failures, the
 * selected squashed error determines the exit code.
 *
 ...
 * **When to use**
 *
 * Use to read a custom failure exit code from an unknown error value, falling back to the default
 * failure code.
 *
 * **Details**
 *
 * Returns the numeric `[Runtime.errorExitCode]` property when it is present on an object. Otherwise
 * returns `1`, the default failure exit code used by `defaultTeardown`.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when you need error classes reported by application code to avoid being logged again by the
 * default main runner.
 *
 * **Details**
 *
 * Set `[Runtime.errorReported]` to `false` on an error object to suppress the runtime log because
 * the error has already been reported. Omitted or non-boolean values are treated as `true`, so
 * failures are logged by default.
 *
 * **Gotchas**
 *
 * This marker controls only automatic error logging. It does not change the failure Cause or the
 * process exit code. `makeRunMain` reads the marker from `Cause.squash(cause)`, so for causes with
 * multiple failures, the squashed error determines whether default logging is suppressed.
 *
 ...
 * **When to use**
 *
 * Use to read whether an unknown error value should be treated as already reported by the default
 * main runner.
 *
 * **Details**
 *
 * Returns a boolean `[Runtime.errorReported]` property when it is present on an object. Otherwise
 * returns `true`, so failures are logged by default.
 *
 * **Gotchas**
 *
 * Non-object values, missing markers, and non-boolean marker values all return `true`.
 *
 * @since 4.0.0
 * @category Accessors
