 ...
 *
 * A hash ring minimizes remapping when nodes are added, removed, or reweighted. This makes it
 * useful for routing requests, partitioning keys, and distributing shards across service instances
 * or storage backends. This module can create rings, add or remove nodes by `PrimaryKey`, route an
 * input string to a node, and compute shard assignments.
 *
 * @since 4.0.0
 */
...
/**
 * A weighted consistent-hashing ring for assigning inputs to nodes with stable remapping as nodes
 * are added or removed.
 *
 * **When to use**
 *
 * Use to maintain a mutable weighted hash ring for routing keys or shards to nodes identified by
 * `PrimaryKey`.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to narrow an `unknown` value before treating it as a `HashRing`, such as values crossing an
 * untyped boundary.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * This is a structural type-id check; it does not validate the ring's `nodes`, `ring`, or weight
 * state.
 *
 * @since 3.19.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use to create an empty weighted consistent-hashing ring with the default or custom virtual-point
 * density.
 *
 * **Details**
 *
 * `baseWeight` controls how many virtual points are added for a node with weight `1`; it defaults
 * to `128` and is clamped to at least `1`.
 *
 * @since 3.19.0
...
/**
 * Adds new nodes to the ring. If a node already exists in the ring, it will be updated. For
 * example, you can use this to update the node's weight.
 *
 * **When to use**
 *
...
/**
 * Adds a new node to the ring. If the node already exists in the ring, it will be updated. For
 * example, you can use this to update the node's weight.
 *
 * **When to use**
 *
 * Use to register one node in a `HashRing` so lookups and shard assignments can return it, or
 * update that node's weight.
 *
 * **Details**
 *
 * Nodes are matched by `PrimaryKey.value`. The weight defaults to `1` and is clamped to at least
 * `0.1`.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use to remove a node that has left the pool so future lookups and shard assignments stop
 * returning it.
 *
 * **Details**
 *
 * Removal matches by `PrimaryKey.value`, so any value with the same primary key removes the same
 * ring member.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when you need to know whether registering a node would update an existing ring member because
 * another node already has the same primary-key identity.
 *
 * **Details**
 *
 * Membership is checked with `self.nodes.has(PrimaryKey.value(node))`, so matching is by primary
 * key, not object identity or weight.
 *
 * @since 3.19.0
 * @category Combinators
...
/**
 * Gets the node which should handle the given input. Returns undefined if the hashring has no
 * elements with weight.
 *
 * **When to use**
 *
