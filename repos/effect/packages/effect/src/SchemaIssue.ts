 ...
 *
 * An `Issue` records what failed and, for nested data, where the failure happened. The Schema
 * system uses these values for missing keys, unexpected keys, invalid types, invalid values, failed
 * filters, failed transformations, and alternatives that did not match. This module also formats
 * issues and supports redaction for sensitive values.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need to narrow an `unknown` value to `Issue` in error-handling code, such as
 ...
 * **When to use**
 *
 * Use when constraining formatter hooks to only handle terminal nodes or when pattern matching on
 * the `_tag` of an issue and only leaf nodes matter.
 *
 * **Details**
 *
 * Members: {@link InvalidType}, {@link InvalidValue}, {@link MissingKey}, {@link UnexpectedKey},
 ...
 * **When to use**
 *
 * Use when typing the error channel in `Effect<A, Issue, R>` results from schema parsing, or when
 * writing custom formatters or issue-tree walkers.
 *
 * **Details**
 *
 * Every node has a `_tag` field for pattern-matching. The union includes both terminal {@link Leaf}
 * types and composite types that wrap inner issues: {@link Filter}, {@link Encoding},
 * {@link Pointer}, {@link Composite}, {@link AnyOf}. All `Issue` instances have a `toString()` that
 * delegates to the default formatter, so `String(issue)` produces a human-readable message.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when you need to inspect a schema issue that records which refinement check rejected the
 * value.
 *
 * **Details**
 *
 ...
 * - `actual` is `Option.some(value)` when the input was present, or `Option.none()` when it was
 *   absent.
...
/**
 * Wraps an inner {@link Issue} with a property-key path, indicating _where_ in a nested structure
 * the error occurred.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use when you need to walk the issue tree for struct/tuple schemas that collect all field errors
 ...
 * **When to use**
 *
 * Use when you need to detect basic type mismatches, such as a wrong primitive or `null` where an
 * object was expected.
 *
 * **Details**
 *
 * - `ast` is the schema node that expected a different type.
 * - `actual` is `Option.some(value)` when the input was present, or `Option.none()` when no value was
 *   provided.
 ...
 * **When to use**
 *
 * Use when you need to detect constraint violations from `Schema.filter`, `Schema.minLength`,
 * `Schema.greaterThan`, or similar checks.
 *
 * **Details**
 *
 ...
 * - The default formatter renders this as `"Invalid data <actual>"` unless a custom `message`
...
/**
 * Represents a schema issue produced when a forbidden operation is encountered during parsing, such
 * as an asynchronous Effect running inside `Schema.decodeUnknownSync`.
 *
 * **When to use**
 *
 ...
 * - `issues` contains per-member failures. When empty, the formatter falls back to the union's
 *   `expected` annotation.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link OneOf} — the opposite: _too many_ members matched
 ...
 * - The default formatter renders this as `"Expected exactly one member to match the input
 *   <actual>"`.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link AnyOf} — the opposite: _no_ members matched
 */
 ...
 * **When to use**
 *
 * Use when you need to retrieve an `Issue`'s offending input value for logging or custom error
 * rendering.
 *
 * **Details**
 *
 * - Returns `Option.none()` for `Pointer` and `MissingKey` (they carry no value).
 * - Returns the existing `Option` for variants that already store `actual` as `Option<unknown>`
 *   (`InvalidType`, `InvalidValue`, `Forbidden`, `Encoding`, `Composite`).
 * - Wraps `actual` with `Option.some` for variants that store it as plain `unknown` (`AnyOf`,
 *   `UnexpectedKey`, `OneOf`, `Filter`).
 *
...
/**
 * A function type that converts an {@link Issue} into a formatted representation. Specialisation of
 * the generic `Formatter` from `Formatter.ts` with `Value` fixed to `Issue`.
 *
 * @since 4.0.0
 * @category Formatter
 ...
 * **When to use**
 *
 * Use when customizing how {@link makeFormatterStandardSchemaV1} renders terminal issues.
 *
 * @since 4.0.0
 * @category Formatter
 ...
 * **When to use**
 *
 * Use when customizing how {@link makeFormatterStandardSchemaV1} renders filter failures.
 *
 * **Details**
 *
 ...
 * - Returns `undefined` when no annotation is found, causing the formatter to fall back to `"Expected
 *   <filter>, got <actual>"`.
 *
 * @since 4.0.0
 * @category Formatter
 ...
 * **When to use**
 *
 * Use when you need schema parse errors in [Standard Schema
 * V1](https://github.com/standard-schema/standard-schema) format, optionally customizing leaf or
 ...
 * - Falls back to {@link defaultLeafHook} / {@link defaultCheckHook} when no hooks are provided.
 *
...
/**
 * Creates a {@link Formatter} that converts an {@link Issue} into a human-readable multi-line
 * string.
 *
 * **When to use**
 *
 * Use when you need to format a `SchemaIssue.Issue` as error messages for logging, CLI output, or
 ...
 *
 * - Flattens the issue tree into `{ message, path }` entries using {@link defaultLeafHook} and
 *   {@link defaultCheckHook}.
