 ...
 *
 * A `LayerMap<K, I, E>` turns a key into a cached service `Context<I>` and exposes that context as
 * either a `Layer` or a scoped effect. Entries can be invalidated explicitly or released after they
 * sit unused. This is useful for keyed resource families such as tenant clients, regional
 * connections, or environment-specific services.
 *
 * @since 3.14.0
 */
 ...
 * **Details**
 *
 * A `LayerMap` builds resources for a key on demand, exposes them as a `Layer` or scoped `Context`,
 * and can invalidate cached resources for a key.
 *
 ...
 * **Details**
 *
 * The record keys become the keys accepted by the returned `LayerMap`, and the record values are
 * the layers built for those keys.
 *
...
>(
  ...
  | Scope.Scope
  | (Layers[keyof Layers] extends Layer.Layer<infer _A, infer _E, infer _R> ? _R : never)
> =>
 ...
 * **When to use**
 *
 * Use as the public type for classes returned by `LayerMap.Service` when an API needs to accept,
 * return, or alias the generated service class and its static helpers.
 *
 * **Details**
 *
 * It combines a `Context.Service` tag for the `LayerMap` with default layers and helper accessors
 * for retrieving, using, and invalidating keyed resources.
 *
 * @since 3.14.0
 * @category Services
...
export declare namespace Service {
  ...
  /**
   * Extracts the services provided by the layers in a `LayerMap.Service` definition.
   *
   * @since 3.14.0
   * @category Services
   */
  export type Success<Options> =
    Layers<Options> extends Layer.Layer<infer _A, infer _E, infer _R> ? _A : never;
  ...
  export type Error<Options> =
    Layers<Options> extends Layer.Layer<infer _A, infer _E, infer _R> ? _E : never;
  ...
  export type Services<Options> =
    Layers<Options> extends Layer.Layer<infer _A, infer _E, infer _R> ? _R : never;
}
