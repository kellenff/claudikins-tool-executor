 ...
 *
 * A `TxHashSet` keeps an immutable `HashSet` inside a `TxRef`, so membership checks and updates can
 * commit atomically with other transactional operations. Use it when several pieces of shared
 * transactional state must change together, such as adding a value only after checking related
 * state. The module includes the usual set operations, including adding, removing, membership
 * checks, set algebra, mapping, filtering, reducing, and conversion back to `HashSet`.
 *
 * @since 2.0.0
 */
...
/**
 * A TxHashSet is a transactional hash set data structure that provides atomic operations on unique
 * values within Effect transactions. It uses an immutable HashSet internally with TxRef for
 * transactional semantics, ensuring all operations are performed atomically.
 *
 * **Details**
 *
 * Mutation operations such as `add`, `remove`, and `clear` update the original TxHashSet and return
 * `Effect<void>` or `Effect<boolean>`. Transform operations such as `union`, `intersection`,
 * `difference`, `map`, and `filter` create new TxHashSet instances and leave the original TxHashSet
 * unchanged.
 *
...
/**
 * The TxHashSet namespace contains type-level utilities and helper types for working with TxHashSet
 * instances.
 *
 ...
 * **Details**
 *
 * This function mutates the original TxHashSet by adding the specified value. It does not return a
 * new TxHashSet reference.
 *
 ...
 * **Details**
 *
 * This function mutates the original TxHashSet by removing the specified value. It does not return
 * a new TxHashSet reference.
 *
 ...
 * **Details**
 *
 * This function mutates the original TxHashSet by clearing all values. It does not return a new
 * TxHashSet reference.
 *
...
/**
 * Creates the difference of two TxHashSets (elements in the first set that are not in the second),
 * returning a new TxHashSet.
 *
...
/**
 * Reduces the TxHashSet to a single value by iterating through the values and applying an
