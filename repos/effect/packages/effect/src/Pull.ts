 ...
 *
 * A `Pull<A, E, Done, R>` is an `Effect` that can produce one `A`, fail with an ordinary error `E`,
 * or signal end-of-input with `Cause.Done<Done>`. The separate done signal lets low-level consumers
 * distinguish normal completion from failure. This module includes type extractors and helpers for
 * detecting, filtering, catching, converting, and matching done signals separately from ordinary
 * failures.
 *
 * @since 4.0.0
 */
...
/**
 * An effectful pull step that either produces a value, fails with `E`, or signals completion with
 * `Cause.Done<Done>`.
 *
 * **When to use**
 *
 * Use to model one low-level pull step when a consumer repeatedly evaluates an effect that may emit
 * a value, fail normally, or signal normal completion through `Cause.Done`.
 *
 * **Details**
 *
 * `Pull` represents completion in the error channel so low-level stream consumers can distinguish
 * ordinary failures from end-of-input and carry a leftover value when needed.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use to derive the value produced by an existing `Pull` when declaring reusable type aliases,
 * low-level stream helpers, or function signatures.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use to derive only the ordinary failure type from a `Pull` when declaring wrappers or APIs that
 ...
 * **When to use**
 *
 * Use to derive the completion leftover type from an existing `Pull` when declaring reusable type
 * aliases or helper signatures that preserve a pull's done value.
 *
 * @since 4.0.0
 * @category Type extractors
 * @see {@link Success} for extracting the pulled value type instead
 ...
 * **When to use**
 *
 * Use to derive the context requirements of a generic or inferred `Pull` without restating its `R`
 * type parameter.
 *
 * @since 4.0.0
 * @category Type extractors
 ...
 * **When to use**
 *
 * Use to describe the ordinary error type that remains after `Cause.Done` completion signals have
 * been handled or filtered out of an error union.
 *
 * @since 4.0.0
 * @category Type extractors
 ...
 * **When to use**
 *
 * Use to recover from a `Cause.Done` completion signal in an effect, such as turning a pull
 * leftover value into a successful recovery effect while preserving ordinary failures.
 *
 * **Details**
 *
 * The handler receives the done leftover value and may recover with a new effect. Non-done errors
 * are preserved.
 *
 * @since 4.0.0
 * @category Done
 ...
 * **When to use**
 *
 * Use when you need to test whether a pull failure cause represents normal completion and only need
 * a boolean result.
 *
 * @since 4.0.0
 * @category Done
 ...
 * **When to use**
 *
 * Use when you need to identify done completion reasons while traversing `cause.reasons`, before
 ...
 * **When to use**
 *
 * Use to separate `Cause.Done` completion from ordinary causes while preserving the typed done
 * value.
 *
 * **Details**
 *
 * Returns a successful `Result` with the `Cause.Done` value when one is present, otherwise returns
 * a failed `Result` containing the non-done cause.
 *
 * @since 4.0.0
 * @category Done
 */
export const filterDone: <E>(
  input: Cause.Cause<E>,
) => Result.Result<Cause.Done.Only<E>, Cause.Cause<ExcludeDone<E>>> = Filter.composePassthrough(
  Cause.findError,
 ...
 * **When to use**
 *
 * Use to detect `Cause.Done` completion in a `Cause` when the completion value is not part of the
 * downstream logic.
 *
 * **Details**
 *
 * Returns a successful `Result` with the done marker when present, otherwise returns a failed
 * `Result` with the non-done cause.
 *
 * @since 4.0.0
 * @category Done
 ...
 * **When to use**
 *
 * Use to select ordinary failure causes for handling while leaving `Cause.Done` completion causes
 * outside that handler.
 *
 * **Details**
 *
 * Returns a successful `Result` with the cause when every failure is non-done; otherwise returns a
 * failed `Result` with the original cause.
 *
 * @since 4.0.0
 * @category Done
 ...
 * **When to use**
 *
 * Use to produce an `Exit` for finalizing a low-level pull workflow when a `Cause.Done` signal
 * should be treated as success and any remaining cause should fail.
 *
 * **Details**
 *
 * If the cause contains a done value, that leftover becomes the successful value. Otherwise the
 * non-done cause becomes the failure cause.
 *
 * @since 4.0.0
 * @category Done
