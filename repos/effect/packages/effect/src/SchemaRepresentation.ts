/**
 * Plain data structures for describing schemas in a serializable form. A `Representation` is not
 * the original `Schema` object; it is a JSON-friendly description of the schema's types, fields,
 * unions, checks, annotations, and references.
 *
 * This module defines the representation node types, document types, and codecs used to validate
 * those documents. It can build representation documents from schema ASTs, turn representation
 * documents back into schemas, convert them to and from JSON Schema documents, and generate
 * TypeScript code artifacts for schema definitions.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * `typeParameters` holds the inner type arguments, such as the `A` in `Option<A>`. `encodedSchema`
 * is the fallback representation when no {@link Reviver} recognizes this declaration.
 * `annotations.typeConstructor` identifies the declaration kind, such as `{ _tag: "effect/Option"
 * }`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * `thunk` points to the actual representation, possibly via a {@link Reference}. `checks` is always
 * empty on `Suspend` nodes.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Reference}
 ...
 * **When to use**
 *
 * Use when a representation should point to a named definition instead of embedding the definition
 * inline.
 *
 * **Details**
 *
 * `$ref` is the key into `Document.references` or `MultiDocument.references`. References are
 ...
 * **Details**
 *
 * `checks` holds string-specific constraints, such as min/max length, pattern, and UUID checks.
 * `contentMediaType` and `contentSchema` indicate that the string contains encoded data, such as
 * `"application/json"` with a nested schema.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link StringMeta}
 ...
 * **Details**
 *
 * `checks` holds number-specific constraints, such as int, finite, min, max, multipleOf, and
 * between checks.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * `elements` are the fixed positional elements, or tuple prefix, and each may be optional. `rest`
 * contains the variadic tail types; a single-element `rest` with no `elements` produces a plain
 * `Array<T>`. `checks` holds array-specific constraints, such as minLength, maxLength, and unique
 * checks.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * `isOptional` indicates whether this element can be absent. `type` is the schema representation
 * for this element's value.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Arrays}
 ...
 * **Details**
 *
 * `propertySignatures` are the explicitly named fields. `indexSignatures` define catch-all
 * key/value types, such as `Record<string, T>`. `checks` holds object-specific constraints, such as
 ...
 * **Details**
 *
 * `name` is the property key, which can be a string, number, or symbol. `isOptional` indicates
 * whether the key can be absent. `isMutable` indicates whether the property is mutable rather than
 * readonly.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * An index signature, such as `[key: string]: number`, within an {@link Objects}.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * `types` are the union members. `mode` controls JSON Schema output as either `"anyOf"` (the
 * default) or mutually exclusive `"oneOf"`.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * Each variant has a `_tag` discriminator. Switch on `_tag` to handle each shape. Most variants
 * carry optional `annotations` and some carry `checks` for validation constraints.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Document}
...
/**
 * A single validation constraint with typed metadata describing the check (e.g. `{ _tag:
 * "isMinLength", minLength: 3 }`).
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Check}
...
/**
 * A group of validation constraints that are logically combined. Contains at least one
 * {@link Check}.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Metadata union for string-specific validation checks (minLength, maxLength, pattern, UUID,
 * trimmed, etc.).
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Metadata union for number-specific validation checks (int, finite, min, max, multipleOf,
 * between).
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Metadata union for object-specific validation checks (minProperties, maxProperties,
 * propertiesLength, propertyNames).
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Objects}
...
/**
 * Metadata union for size-based validation checks (minSize, maxSize, size). Used for collection
 * types like `Set`, `Map`.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * A string-keyed map of named {@link Representation} definitions. Used by {@link Document} and
 * {@link MultiDocument} for `$ref` resolution (analogous to JSON Schema `$defs`).
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Reference}
 ...
 * **When to use**
 *
 * Use when representing a single Schema AST together with its named references before
 * reconstructing a runtime Schema, converting to JSON Schema, or wrapping it as a
 * {@link MultiDocument}.
 *
 * @since 4.0.0
 * @category Models
 ...
 * {@link toCodeDocument} to generate TypeScript code, and {@link toJsonSchemaMultiDocument} to
 ...
 * **When to use**
 *
 * Use to validate recursive annotation metadata trees whose leaves are `null`, `number`, `boolean`,
 * `bigint`, `symbol`, or `string`.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Schema for serializing public `Schema.Annotations.Annotations` values. It filters out internal
 * annotation keys and non-primitive values during encoding.
 *
 * **When to use**
 *
 * Use to serialize schema annotations in representation schemas while retaining only primitive-tree
 * metadata.
 *
 * **Details**
 *
 * Decoding is passthrough. Encoding removes internal annotation keys and values that are not
 * accepted by `$PrimitiveTree`.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **When to use**
 *
 * Use to encode, decode, or validate serialized `BigInt` representation nodes, not application
 * `bigint` values.
 *
 * **Details**
 *
 * Accepts representation nodes with `_tag: "BigInt"`, optional annotations, and bigint-specific
 * validation metadata in `checks`.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Schema for the literal value types allowed in a {@link Literal} node (string, finite number,
 * boolean, or bigint).
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * @category Schemas
 */
export const $SizeMeta = Schema.Union([$IsMinSize, $IsMaxSize, $IsSizeBetween]).annotate({
  identifier: "SizeMeta",
});
...
/**
 * Schema for the full {@link Representation} union. It recursively validates and encodes any
 * representation node.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **When to use**
 *
 * Use when you have multiple Schema ASTs and need one schema representation `MultiDocument` with
 * shared references.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to pass a single schema representation `Document` where an API expects a
 * `MultiDocument`.
 *
 * @since 4.0.0
...
/**
 * A callback that handles {@link Declaration} nodes during reconstruction ({@link toSchema}) or
 * code generation ({@link toCodeDocument}).
 *
 * **Details**
 *
 * Return a value to handle the declaration. Return `undefined` to fall back to default behavior,
 * which uses `encodedSchema` for `toSchema` or the `generation` annotation for `toCodeDocument`.
...
/**
 * Default {@link Reviver} for {@link toSchema} that handles built-in Effect types, including
 * Option, Result, Redacted, Cause, Exit, ReadonlyMap, HashMap, ReadonlySet, Date, Duration, URL,
 * and RegExp.
 *
 * **When to use**
 *
 * Use when you need the default `options.reviver` for {@link toSchema} to reconstruct runtime
 * schemas for built-in Effect declarations.
 *
 * **Details**
 *
 * The reviver returns `undefined` for unrecognized declarations, causing fallback to
 * `encodedSchema`.
 *
 * @since 4.0.0
 * @category Transforming
 ...
 * **When to use**
 *
 * Use when you have a serialized or computed schema representation document and need a runtime
 * Schema for decoding/encoding.
 *
 * **Details**
 *
 * Pass `options.reviver`, such as {@link toSchemaDefaultReviver}, to handle {@link Declaration}
 * nodes for types like `Date` and `Option`. Without a reviver, declarations fall back to their
 * `encodedSchema`. Circular references are handled via lazy `Schema.suspend`.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when you need to produce a standard JSON Schema document from a schema representation
 * `Document`.
 *
 * **Gotchas**
 *
 * JSON Schema generation is best-effort. Some Effect schema representation semantics cannot be
 * represented exactly in JSON Schema, and importing an emitted JSON Schema may produce an
 * equivalent approximation rather than the original representation shape.
 *
 ...
 * **When to use**
 *
 * Use when you need to export related schema representation documents together so shared
 * definitions stay in multi-document JSON Schema form.
 *
 * **Gotchas**
 *
 * JSON Schema generation is best-effort. Some Effect schema representation semantics cannot be
 * represented exactly in JSON Schema, and importing an emitted JSON Schema may produce equivalent
 * approximations rather than the original representation shapes.
 *
 * @since 4.0.0
 * @category Transforming
 * @see {@link MultiDocument}
...
/**
 * A pair of TypeScript source strings for a schema: `runtime` is the executable Schema expression,
 * `Type` is the corresponding TypeScript type.
 *
 * @since 4.0.0
...
/**
 * An auxiliary code artifact produced during code generation — a symbol declaration, an enum
 * declaration, or an import statement.
 *
 * @since 4.0.0
 * @category Code Generation
 * @see {@link CodeDocument}
...
/**
 * The output of {@link toCodeDocument}: generated TypeScript code for one or more schemas plus
 * their shared references and auxiliary artifacts.
 *
 * **Details**
 *
 * `codes` contains one {@link Code} per input representation. `references.nonRecursives` contains
 * topologically sorted non-recursive definitions. `references.recursives` contains definitions
 * involved in cycles. `artifacts` contains symbols, enums, and import statements needed by the
 * code.
 *
 * @since 4.0.0
 * @category Code Generation
 ...
 * **When to use**
 *
 * Use when you need to produce source code for Effect Schema definitions from a schema
 * representation `MultiDocument`.
 *
 * **Details**
 *
 * `options.reviver` can customize code generation for {@link Declaration} nodes. Return `undefined`
 * to fall back to the default logic, which uses `generation` annotations or the encoded schema.
 * References are topologically sorted so non-recursive definitions are emitted before their
...
export function toCodeDocument(
  ...
  options?: {
    /**
     * The reviver can return `undefined` to indicate that the generation should be generated by the
     * default logic
     */
  ...
  function toRuntimeChecks(checks: ReadonlyArray<Check<Meta>>): string {
    return checks
      .map((c) => `.check(${toRuntimeCheck(c)})` + toRuntimeBrand(c.annotations))
      .join("");
  }
...
/**
 * Converts an arbitrary string into a valid (ASCII) JavaScript identifier starting with an
 ...
 * **When to use**
 *
 * Use when you need to import a Draft 2020-12 JSON Schema document into the Effect schema
 * representation system.
 *
 * **Details**
 *
 * `options.onEnter` is an optional hook called on each JSON Schema node before processing, allowing
 * pre-transformation.
 *
 * **Gotchas**
 *
 * JSON Schema import is best-effort. Some JSON Schema constructs do not map exactly to Effect
 * schema representations, and importing a schema previously emitted by `toJsonSchemaDocument` may
 * produce an equivalent approximation rather than the original representation shape.
 *
 * This throws if a `$ref` cannot be resolved within the document's definitions. Circular `$ref`s
 ...
 * **When to use**
 *
 * Use when you need to import a Draft 2020-12 JSON Schema multi-document whose schemas share
 * definitions.
 *
 * **Details**
 *
 ...
 *
 * JSON Schema import is best-effort. Some JSON Schema constructs do not map exactly to Effect
 * schema representations, and importing schemas previously emitted by `toJsonSchemaMultiDocument`
 * may produce equivalent approximations rather than the original representation shapes.
 *
...
export function fromJsonSchemaMultiDocument(
  ...
  function recur(u: unknown): Representation {
    ...
    if (Array.isArray(js.anyOf)) {
      out = combine(
        { _tag: "Union", types: js.anyOf.map((type) => recur(type)), mode: "anyOf" },
        out,
      );
    }
    if (Array.isArray(js.oneOf)) {
      out = combine(
        { _tag: "Union", types: js.oneOf.map((type) => recur(type)), mode: "oneOf" },
        out,
      );
    }
  ...
  function collectProperties(js: JsonSchema.JsonSchema): Array<PropertySignature> {
    const properties: Record<string, unknown> = Predicate.isObject(js.properties)
      ? js.properties
      : {};
  ...
  function unknownToJson(representation: Representation): Representation {
    switch (representation._tag) {
      ...
      case "String": {
        ...
        return contentSchema === representation.contentSchema
          ? representation
          : { ...representation, contentSchema };
      }
      case "Arrays": {
      ...
      case "Objects": {
        ...
        const indexSignatures = SchemaAST.mapOrSame(
...
export type TopologicalSort = {
  /**
   * The definitions that are not recursive. The definitions that depends on other definitions are
   * placed after the definitions they depend on
   */
  readonly nonRecursives: ReadonlyArray<{
    readonly $ref: string;
    readonly representation: Representation;
