 ...
 *
 * Every `Schema` has an AST made from nodes for declarations, primitives, literals, arrays,
 * objects, unions, suspended schemas, checks, annotations, encoding links, and parsing context.
 * Most users work with the higher-level `Schema` module. Use `SchemaAST` when you need to inspect
 * schema nodes, build ASTs programmatically, change encoded or decoded views, collect issues, or
 ...
 * **Details**
 *
 * Every `Schema` has an `.ast` property of this type. Use the guard functions ({@link isString},
 * {@link isObjects}, etc.) to narrow to a specific variant, then access variant-specific fields.
 *
 * - All variants share the {@link Base} fields: `annotations`, `checks`, `encoding`, `context`.
 ...
 * **When to use**
 *
 * Use to recognize an AST node that represents exactly the `null` literal when inspecting,
 * traversing, or transforming schema ASTs.
 *
 * @since 4.0.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use to identify AST nodes that represent the TypeScript `void` type before handling
 * `Void`-specific schema behavior.
 *
 * @since 4.0.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use to detect the AST node for a schema that can never match before handling other schema
 * variants.
 *
 * @since 4.0.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use to detect schema AST nodes that match any string value while inspecting or transforming a
 * Schema AST.
 *
 * @since 4.0.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use to identify the `Boolean` AST variant while inspecting, traversing, or transforming schema
 * definitions.
 *
 * @since 4.0.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use to narrow an `AST` node before handling the `Symbol` variant for schemas that accept any
 ...
 * **When to use**
 *
 * Use to identify the AST node for the TypeScript `object` keyword when inspecting or transforming
 * a Schema AST.
 *
 * @since 3.10.0
 * @category Guards
 ...
 * **Details**
 *
 * A link pairs a target {@link AST} with a `Transformation` or `Middleware` that converts values
 * between the current node and the target.
 *
 ...
 *
 * Links are composed into a non-empty array ({@link Encoding}) attached to AST nodes that have a
 * different encoded representation.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Encoding}
...
/**
 * A non-empty chain of {@link Link} values representing the transformation steps between a schema's
 * decoded (type) form and its encoded (wire) form.
 *
 * **Details**
 *
 * Stored on {@link Base.encoding}. When `undefined`, the node has no encoding transformation (type
 * and encoded forms are identical).
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Link}
 ...
 * **Details**
 *
 * Pass to `Schema.decodeUnknown`, `Schema.encode`, and related APIs to customize error reporting,
 * excess property handling, output key ordering, check execution, and asynchronous parser
 * concurrency.
 *
 * - `errors` — `"first"` (default) stops at the first error; `"all"` collects every error.
 * - `onExcessProperty` — `"ignore"` (default) strips unknown object keys; `"error"` fails;
 *   `"preserve"` keeps them.
 * - `propertyOrder` — `"none"` (default) lets the system choose key order; `"original"` preserves
 *   input key order.
 * - `disableChecks` — skips validation checks while still applying defaults and transformations.
 * - `concurrency` — maximum number of async parse effects to run concurrently; defaults to `1`, or
 *   use `"unbounded"`.
 *
 * @since 3.10.0
 * @category Options
...
export interface ParseOptions {
   ...
   * **Details**
   *
   * The default, `"first"`, stops at the first error. Set the option to `"all"` to collect every
   * parsing error, which can help with debugging or with presenting more complete error messages to
   * a user.
   *
   * @default "first"
   */
   ...
   * **Details**
   *
   * The default, `"ignore"`, strips unspecified properties from the output. Use `"error"` to fail
   * when an excess property is present, or `"preserve"` to keep excess properties in the output.
   *
   * @default "ignore"
   */
  ...
  /**
   * The `propertyOrder` option provides control over the order of object fields in the output. This
   * feature is useful when the sequence of keys is important for the consuming processes or when
   * maintaining the input order enhances readability and usability.
   *
   * **Details**
   *
   * By default, the `propertyOrder` option is set to `"none"`. This means that the internal system
   * decides the order of keys to optimize parsing speed.
   *
   * Setting `propertyOrder` to `"original"` ensures that the keys are ordered as they appear in the
   * input during the decoding/encoding process.
   *
   * **Gotchas**
   *
   * The key order for `"none"` should not be considered stable and may change in future updates
   * without notice.
   *
   * @default "none"
   */
  ...
  /** Whether to disable checks while still applying defaults and transformations. */
 ...
 * **Details**
 *
 * Tracks whether a property key is optional, mutable, has a constructor default, or carries
 * key-level annotations. Typically set by helpers like {@link optionalKey} and
 * `Schema.mutableKey`.
 *
 * - `isOptional` — the property key may be absent from the input.
 * - `isMutable` — the property is `readonly` when `false`.
 * - `defaultValue` — an {@link Encoding} applied during construction to supply missing values.
...
/**
 * Non-empty array of validation {@link Check} values attached to an AST node via
 * {@link Base.checks}.
 *
 * **Details**
 *
 * Checks are run after basic type matching succeeds. They represent refinements like `minLength`,
 * `pattern`, `int`, etc.
 *
 * @since 4.0.0
 * @category Models
 ...
 * - `encoding` — optional {@link Encoding} chain for type ↔ wire transformations.
 ...
 * **Details**
 *
 * - `typeParameters` — inner schemas this declaration is parameterized over (e.g. the element type
 *   for a custom collection).
 * - `run` — factory that receives `typeParameters` and returns a parser that validates or transforms
 *   raw input.
 *
 * @since 3.10.0
 * @category Models
...
export {
   ...
   * **When to use**
   *
   * Use when you need the shared AST node for exact null values while inspecting or constructing
   * schema ASTs.
   *
   * @since 4.0.0
   * @category Constants
...
export {
   ...
   * **When to use**
   *
   * Use when you need the shared AST node for exact undefined values while inspecting or
   * constructing schema ASTs.
   *
   * @since 4.0.0
   * @category Constants
   */
 ...
 * **Details**
 *
 * Parsers built from this node accept any present runtime input and map it to `undefined`. Public
 * schemas built from it may still expose `void` as their typed decoded and encoded representation.
 *
 * @since 4.0.0
 * @category Models
...
export {
   ...
   * **When to use**
   *
   * Use when constructing or comparing AST nodes for TypeScript `void` return values whose result
   * is intentionally ignored.
   *
   * **Details**
   *
   * The node parses any present runtime value as `undefined`; schemas may still expose `void` on
   * their typed decoded and encoded sides.
   *
   * @since 4.0.0
 ...
 * **Details**
 *
 * Parsing always fails. Useful as a placeholder in unions or as the result of narrowing that
 ...
 * **When to use**
 *
 * Use when you need the singleton AST node for the TypeScript `any` type and intentionally want
 * parsing to accept every input value.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * Unlike {@link Any}, this is type-safe: the parsed result is typed as `unknown` rather than `any`.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link unknown}
 ...
 * **When to use**
 *
 * Use when you need the reusable AST singleton for a schema node that accepts every value while
 * keeping parsed values opaque.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * AST node matching the TypeScript `object` type — accepts objects, arrays, and functions (anything
 * non-primitive and non-null).
 *
 * @since 3.10.0
 * @category Models
 ...
 * **When to use**
 *
 * Use to reuse the canonical AST node for the TypeScript `object` keyword when building or
 * comparing `SchemaAST` values directly.
 *
 * @since 3.10.0
 * @category Constructors
 ...
 * **Details**
 *
 * Holds `enums` as an array of `[name, value]` pairs where values are `string | number`. Parsing
 * succeeds when the input matches any enum value.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link isEnum}
...
export class Enum extends Base {
  ...
  getParser() {
    ...
    return fromRefinement(this, (input): input is (typeof this.enums)[number][1] =>
      values.has(input),
    );
  }
  /** @internal */
 ...
 * **Details**
 *
 * Parsing succeeds only when the input is strictly equal (`===`) to the stored `literal`. Numeric
 * literals must be finite — `Infinity`, `-Infinity`, and `NaN` are rejected at construction time.
 *
 ...
 *
 * If the node has an `isFinite` or `isInt` check, the string fallback is skipped since non-finite
 * values cannot occur.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when you need the canonical `SchemaAST` node for schemas that accept any JavaScript number
 * value.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to reuse the standard AST node that accepts either `true` or `false` when constructing schema
 * ASTs directly.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * When serialized to a string-based codec, symbols are converted via `Symbol.keyFor` and must be
 ...
 * **Gotchas**
 *
 * String-based codecs can encode only symbols registered with `Symbol.for`, because the
 * implementation uses `Symbol.keyFor`.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * When serialized to a string-based codec, bigints are converted to/from their decimal string
 * representation.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use to reuse the canonical `BigInt` AST node when constructing, inspecting, or transforming
 * schemas at the AST level.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use when constructing or inspecting AST nodes for tuple or array-like schemas, including rest
 * elements.
 *
 * **Details**
 *
 * - `elements` — positional element types (tuple elements). An element is optional if its
 *   {@link Context.isOptional} is `true`.
 * - `rest` — the rest/variadic element types. When non-empty, the first entry is the "spread" type
 *   (e.g. `...Array<string>`), and subsequent entries are trailing positional elements after the
 *   spread.
 * - `isMutable` — whether the resulting array is `readonly` (`false`) or mutable (`true`).
 *
 * **Gotchas**
 *
 * Construction enforces TypeScript ordering rules: a required element cannot follow an optional
 * one, and an optional element cannot follow a rest element.
 *
  ...
  if (issue === undefined) {
    return Exit.failCause(
      Cause.map(
        exit.cause,
        (issue) =>
          new SchemaIssue.Composite(ast, s.oinput, [new SchemaIssue.Pointer([key], issue)]),
      ),
    );
  }
    ...
    switch (parameter._tag) {
      ...
      case "TemplateLiteral":
        return (stringKeys ??= Object.keys(input)).filter(
          (k) => parameter.matchPart(k, options) !== undefined,
        );
      case "Number":
        return (stringKeys ??= Object.keys(input)).filter(
          (k) => parameter.matchKey(k, options) !== undefined,
        );
      case "Symbol":
 ...
 * **Details**
 *
 * Pairs a `name` (any `PropertyKey`) with a `type` ({@link AST}). The property's optionality and
 * mutability are determined by the `type`'s {@link Context}.
 *
 * @since 3.10.0
 * @category Models
 * @see {@link Objects}
 ...
 * **Details**
 *
 * Used by {@link IndexSignature} when the same key appears multiple times (e.g. from
 * `Schema.extend` or overlapping records). Provides separate `decode` and `encode` combiners that
 * determine how duplicate entries are merged.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * - `parameter` — the key type AST (e.g. {@link String} for `string` keys, {@link TemplateLiteral}
 *   for patterned keys).
 ...
 * **Gotchas**
 *
 * Using `Schema.optionalKey` on the value type is not allowed for index signatures (throws at
 * construction); use `Schema.optional` instead.
 *
 * @since 3.10.0
 * @category Models
 * @see {@link Objects}
 ...
 * **When to use**
 *
 * Use when constructing or inspecting AST nodes for structs or records rather than array-like
 * schemas.
 *
 * **Details**
 *
 * - `propertySignatures` — named properties with their types (struct fields).
 * - `indexSignatures` — index signature entries (record patterns), each with a `parameter` AST for
 *   matching keys and a `type` AST for values.
 *
 * An `Objects` node with no properties and no index signatures performs only a non-nullish check:
 * it accepts any value except `null` and `undefined`, including primitive values.
 *
 * **Gotchas**
 *
...
/**
 * The goal is to reduce the number of a union members that will be checked. This is useful to
 * reduce the number of issues that will be returned.
 *
 * @internal
 */
 ...
 * - `mode` — `"anyOf"` succeeds on the first match (like TypeScript unions); `"oneOf"` requires
 *   exactly one member to match (fails if multiple do).
 *
 * During parsing, members are tried in order. An internal candidate index narrows which members to
 * try based on the runtime type of the input and discriminant ("sentinel") fields, making large
 * unions efficient.
 *
  ...
  getParser(recur: (ast: AST) => SchemaParser.Parser): SchemaParser.Parser {
    ...
    return (oinput, options) => {
      ...
      const eff = parseUnion(
        state,
        candidates,
        concurrency ? { ...concurrency, orderedStep: true } : undefined,
      );
      if (!eff) {
        return state.out
 ...
 * **Details**
 *
 * Wraps a thunk (`() => AST`) that is memoized on first call. Use this to define recursive or
 * mutually recursive schemas without infinite loops at construction time.
 *
 ...
 * - `annotations` — optional filter-level metadata (expected message, meta tags, arbitrary constraint
 *   hints).
 * - `aborted` — when `true`, parsing stops immediately after this filter fails (no further checks
 *   run).
 *
 * Use `.annotate()` to add metadata and `.abort()` to mark as aborting. Combine with another check
 * via `.and()` to form a {@link FilterGroup}.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link FilterGroup}
 ...
 * **Details**
 *
 * Created by calling `.and()` on a {@link Filter} or another `FilterGroup`. All inner checks are
 * run; failures from aborted filters still stop evaluation.
 *
 * @since 4.0.0
 * @category Models
 * @see {@link Filter}
 ...
 * **When to use**
 *
 * Use when string validation should be represented as a schema `Filter` backed by a regular
 * expression.
 *
 * **Details**
 *
 * The filter can be used with `Schema.filter` or attached directly to a `String` AST node through
 * checks. The regular expression source is stored in annotations for serialization and arbitrary
 * generation.
 *
 * **Gotchas**
 *
 * Use a non-global, non-sticky regular expression, or reset `lastIndex` yourself, because
 * `RegExp.test` is stateful for expressions with the `g` or `y` flag.
 *
...
/**
 * Attaches a `Transformation` to the `to` AST, making it decode from the `from` AST and encode back
 * to it.
 *
 * **Details**
 *
 * This is the low-level primitive behind `Schema.transform` and `Schema.transformOrFail`. It
 * appends a {@link Link} to the `to` node's encoding chain.
 *
 ...
 * **Details**
 *
 * Checks `ast.context?.isOptional`. Defaults to `false` when no {@link Context} is set.
 *
 * @since 4.0.0
 * @category Predicates
 * @see {@link optionalKey}
...
/**
 * Strips all encoding transformations from an AST, returning the decoded (type-level)
 * representation.
 *
 * **Details**
 *
 * - Memoized: same input reference → same output reference.
 * - Recursively walks into composite nodes ({@link Arrays}, {@link Objects}, {@link Union},
 *   {@link Suspend}).
 *
 ...
 * **Details**
 *
 * Equivalent to `toType(flip(ast))`. This gives you the AST that describes the shape of the
 ...
 * **Details**
 *
 * After flipping, what was decoding becomes encoding and vice versa. This is the core operation
 * behind `Schema.encode` — encoding a value is decoding with a flipped SchemaAST.
 *
 * - Memoized: same input reference → same output reference.
...
function applyTemplateLiteralPartChecks<A>(
  ast: AST,
  value: A,
  options: ParseOptions,
): A | undefined {
...
const finiteToString = new Link(finiteString, SchemaTransformation.numberFromString);
...
const bigIntToString = new Link(bigIntString, SchemaTransformation.bigintFromString);
 ...
 * **Details**
 *
 * If the node has {@link Checks}, returns annotations from the last check (which is where
 * user-supplied annotations end up after `.pipe(Schema.annotations(...))`). Otherwise returns
 * `Base.annotations` directly.
 *
 ...
 * **Details**
 *
 * Like {@link resolve}, reads from the last check's annotations when checks are present. Returns
 ...
 * **Details**
 *
 * The identifier is typically set by `Schema.annotations({ identifier: "..." })` and is used for
 * error messages and schema identification.
 *
 * @since 4.0.0
 * @category Annotations
 * @see {@link resolve}
...
/** @internal */
export const unknownToStringTree = new Link(StringTree, SchemaTransformation.passthrough());
