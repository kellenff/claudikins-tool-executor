 ...
 *
 * A `Chunk<A>` is useful when you need to build or transform collections without changing the
 * original collection. It is designed for efficient append, prepend, and concatenation. This module
 * includes helpers for creating, reading, slicing, mapping, filtering, sorting, zipping, combining,
...
/**
 * A Chunk is an immutable, ordered collection optimized for efficient concatenation and access
 * patterns.
 *
...
/**
 * Creates an `Equivalence` for chunks that compares chunk lengths and then compares corresponding
 * elements with the provided element equivalence.
 *
...
/**
 * Converts a `Chunk` into an `Array`. If the provided `Chunk` is non-empty (`NonEmptyChunk`), the
 * function will return a `NonEmptyArray`, ensuring the non-empty property is preserved.
 *
...
/**
 * Converts a `Chunk` into a `ReadonlyArray`. If the provided `Chunk` is non-empty
 * (`NonEmptyChunk`), the function will return a `NonEmptyReadonlyArray`, ensuring the non-empty
 ...
 * **When to use**
 *
 * Use when the input array is already known to be non-empty, can be shared with the resulting
 * `Chunk`, and avoiding a copy matters.
 *
 * **Gotchas**
 *
 * Mutating the source array after wrapping can mutate the resulting `Chunk`.
 ...
 * **When to use**
 *
 * Use when reading from a `Chunk` at an index known to be in bounds and direct element access is
...
/**
 * Prepends the specified prefix chunk to the beginning of the specified chunk. If either chunk is
 * non-empty, the result is also a non-empty chunk.
 *
...
/**
 * Concatenates two chunks, combining their elements. If either chunk is non-empty, the result is
 * also a non-empty chunk.
 *
 * **When to use**
 *
...
/**
 * Applies a function to each element in a chunk and returns a new chunk containing the concatenated
 * mapped elements.
 *
 ...
 * **Details**
 *
 * This function processes every element of the given `Chunk`, calling the provided function `f` on
 * each element. It does not return a new value; instead, it is primarily used for side effects,
 * such as logging or accumulating data in an external variable.
 *
 ...
 * **Details**
 *
 * The final chunk may contain fewer than `n` elements. Empty input produces an empty chunk of
 * chunks.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when you know the chunk is non-empty and need the first element directly without handling
 * `Option.none`.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when you know the chunk is non-empty and need the last element directly without handling
 * `Option.none`.
 *
 * **Gotchas**
 *
...
/**
 * Transforms the elements of a chunk using the specified mapping function. If the input chunk is
 * non-empty, the resulting chunk will also be non-empty.
 *
 ...
 * **Details**
 *
 * `n` is floored and normalized to at least `1`. If `n` is greater than or equal to the chunk
 * length, the first result is the original chunk and the second result is empty.
 *
 ...
 * **Details**
 *
 * The chunk size is derived from the input length and `n`; the final chunk may contain fewer
...
/**
 * Splits this chunk on the first element that matches this predicate. Returns a tuple containing
 * two chunks: the first one is before the match, and the second one is from the match onward.
 *
...
/**
 * Applies a function to the element at the specified index safely, creating a new `Chunk`, or
...
/**
 * Changes the element at the specified index safely, creating a new `Chunk`, or returns `None` if
...
export const replace: {
  ...
  3,
  <A, B>(self: Chunk<A>, i: number, b: B): O.Option<Chunk<B | A>> => modify(self, i, () => b),
);
...
/**
 * Returns a function that checks if a `Chunk` contains a given value using the default
 * `Equivalence`.
 *
...
/**
 * Returns a function that checks if a `Chunk` contains a given value using a provided
 ...
 * const containsById = Chunk.containsWith<{ id: number; name: string }>(
 *   (a, b) => a.id === b.id,
 * );
 ...
 * const words = Chunk.make("Apple", "Banana", "Cherry");
 * const containsCaseInsensitive = Chunk.containsWith<string>(
...
/**
 * Creates a `Chunk` of values not included in the other given `Chunk` using the provided
 * `isEquivalent` function. The order and references of result values are determined by the first
 * `Chunk`.
 *
 ...
 * const caseInsensitive = Chunk.differenceWith<string>(
 *   (a, b) => a.toLowerCase() === b.toLowerCase(),
 * );
...
/**
 * Creates a `Chunk` of values not included in the other given `Chunk`. The order and references of
 * result values are determined by the first `Chunk`.
 *
