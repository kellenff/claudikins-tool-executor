 ...
 *
 * This module provides immutable and scoped-mutable graph data structures. A graph can be directed
 * or undirected, and it can store user-defined data on both nodes and edges. The module includes
 * traversal, analysis, path-finding, transformation, and diagram export utilities.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need to keep the identifier for a graph edge so you can later read, update, remove,
 * or compare that edge.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when writing graph-polymorphic types or helpers that need to preserve whether a graph is
 ...
 * **Details**
 *
 * Contains only the runtime marker and shared protocols. Graph storage is kept internal; use module
 * functions such as `nodes`, `edges`, `getNode`, and `getEdge` to inspect graph contents.
 *
 * @since 3.18.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use as the immutable graph model for code that queries, traverses, transforms, or analyzes graph
 * structure without mutating it.
 *
 * @since 3.18.0
 * @category Models
 ...
 * **When to use**
 *
 * Use as the immutable graph type when edge direction is part of the model and traversal or
 * neighbor queries should follow source-to-target edges.
 *
 * **Details**
 *
 * `DirectedGraph<N, E>` is a `Graph<N, E, "directed">` with node data of type `N` and edge data of
 * type `E`.
 *
 * @since 3.18.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when modeling relationships where each edge connects both endpoints without a
 ...
 * **When to use**
 *
 * Use when annotating a temporary graph value that can be changed in place and whose edges have
 * source-to-target direction.
 *
 * @since 3.18.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when annotating a temporary graph value that can be changed in place and whose edges connect
 * both endpoints without direction.
 *
 * @since 3.18.0
 * @category Models
...
/**
 * Error thrown by graph operations when the requested graph structure is invalid, such as
 * referencing a missing node or using unsupported edge weights.
 *
 * **When to use**
 *
 * Use when handling failures thrown by graph operations that reject invalid graph structure or
 * unsupported algorithm inputs.
 *
 * @since 3.18.0
 * @category Errors
 */
...
/**
 * Returns `true` if a value has the graph runtime type identifier, narrowing it to an immutable or
 * mutable graph.
 *
 * **When to use**
 *
 ...
 * **Gotchas**
 *
 * This guard checks the shared graph runtime type identifier and does not distinguish immutable
 * graphs from mutable graphs or directed graphs from undirected graphs.
 *
 * @since 4.0.0
 * @category Guards
 */
 ...
 * **When to use**
 *
 * Use when the graph kind is selected dynamically. Prefer `directed` or `undirected` when the kind
 * is known statically.
 *
 ...
 * **Details**
 *
 * Finalizes the mutable handle. Later public mutation operations on that handle fail with a
 * `GraphError`.
 *
...
class EdgeIdentity<NI, EI> implements Equal.Equal {
  ...
  constructor(type: Kind, source: NI, target: NI, identity: EI) {
    this.type = type;
    this.source = source;
    this.target = target;
  ...
  [Equal.symbol](that: Equal.Equal): boolean {
    if (
      ...
      this.type !== that.type ||
      !Equal.equals(this.identity, that.identity)
    ) {
      return false;
    }
 ...
 * identities of its endpoint nodes and the graph kind.
 *
 * **Gotchas**
 *
 * Edge identity defines set membership, not edge multiplicity. Parallel edges with the same
 * endpoint identities and projected edge identity are treated as the same member by graph set
 * operations.
 *
 * @since 4.0.0
 * @category Models
...
/** @internal */
const nodeIdentityAt = <N, I>(maps: NodeMaps<N, I>, index: NodeIndex): I =>
  maps.byIndex.get(index) as I;
 ...
 * **Details**
 *
 * Nodes and edges present in both graphs use data from `that`. The result has the same graph kind
 * as `self`. Throws a `GraphError` when the graph kinds do not match. `nodeIdentity` and
 * `edgeIdentity` default to the complete node and edge data. Edge identity also includes the
 * endpoint identities.
 *
 * `G1 ∪ G2 = {V1 ∪ V2, E1 ∪ E2}`
 *
 * **Gotchas**
 *
 * Nodes with equal identities in one input graph are coalesced. The last node supplies the data,
 * and redirected edges can collapse or become self-loops. Parallel edges with equal identities are
 * also coalesced, with the last edge supplying the data.
 *
 ...
 * **Details**
 *
 * Node data comes from `self`, and edge data comes from `that`. The result has the same graph kind
 * as `self`. Throws a `GraphError` when the graph kinds do not match. `nodeIdentity` and
 * `edgeIdentity` default to the complete node and edge data. Edge identity also includes the
 * endpoint identities.
 *
 ...
 *
 * Nodes with equal identities in one input graph are coalesced. The last node supplies the data,
 * and redirected edges can collapse or become self-loops. The result contains at most one edge for
 * each shared edge identity.
 *
  ...
  <N, E, T extends Kind, NI, EI>(
    ...
    for (const edge of thatImpl.edges.values()) {
      ...
      if (MutableHashMap.has(nodes, sourceIdentity) && MutableHashMap.has(nodes, targetIdentity)) {
        const edgeIdentity = new EdgeIdentity(
          that.type,
          sourceIdentity,
          targetIdentity,
          getEdgeIdentity(edge.data),
        );
 ...
 * **Details**
 *
 * All nodes from `self` are preserved. Edges are matched by endpoint and edge identities. The
 * result has the same graph kind as `self`. Throws a `GraphError` when the graph kinds do not
 * match. `nodeIdentity` and `edgeIdentity` default to the complete node and edge data.
 *
 * `G1 \ G2 = {V1, E1 \ E2}`
 *
 * **Gotchas**
 *
 * Nodes with equal identities in one input graph are coalesced. The last node supplies the data,
 * and redirected edges can collapse or become self-loops. If `that` contains an edge identity,
 * every parallel edge with that identity is removed from `self`.
 *
 ...
 * **Details**
 *
 * Keeps nodes from both graphs. Overlapping nodes use data from `that`. The result has the same
 * graph kind as `self`. Throws a `GraphError` when the graph kinds do not match. `nodeIdentity` and
 * `edgeIdentity` default to the complete node and edge data. Edge identity also includes the
 * endpoint identities.
 *
 ...
 * **Gotchas**
 *
 * Edges with different projected identities are distinct. Nodes with equal identities in one input
 * graph are coalesced. The last node supplies the data, and redirected edges can collapse or become
 * self-loops. Parallel edges with equal identities are coalesced before the graphs are compared.
 *
 ...
 * **Details**
 *
 * Adds every missing edge between distinct nodes. The `createEdge` function receives the source and
 * target node data for each added edge. The result has the same graph kind as `self`.
 *
 * `G' = {V, (V x V) \ E}`
 *
 ...
 * **Details**
 *
 * `radius` limits the edge distance from the center node and defaults to `1`. `direction` controls
 * how directed edges are traversed and defaults to `"outgoing"`.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The `radius` option is the maximum edge distance from `nodeIndex` and defaults to `1`. The
 * `direction` option controls directed graph traversal and defaults to `"outgoing"`. The result has
 * the same graph kind as `self` and keeps all original edges whose endpoints are both reached.
 ...
 * **Details**
 *
 * Copies all nodes and edges from both graphs without merging equal node data. The result has the
 * same graph kind as `self`. Throws a `GraphError` when the graph kinds do not match.
 *
 ...
 * **Details**
 *
 * The returned index is allocated from the graph's next node index. The mutable graph stores the
 * node data and initializes empty incoming and outgoing edge indexes for the new node.
 *
 * **Gotchas**
 *
...
/** @internal */
const rebuildAdjacency = <N, E, T extends Kind = "directed">(mutable: GraphImpl<N, E, T>): void => {
...
/**
 * Filters and optionally transforms nodes in a mutable graph using a predicate function. Nodes that
 * return Option.none are removed along with all their connected edges.
 *
...
/**
 * Filters and optionally transforms edges in a mutable graph using a predicate function. Edges that
 * return Option.none are removed from the graph.
 *
...
/**
 * Filters nodes by removing those that don't match the predicate. This function modifies the
 * mutable graph in place.
 *
...
/**
 * Filters edges by removing those that don't match the predicate. This function modifies the
 * mutable graph in place.
 *
 ...
 * **When to use**
 *
 * Use to connect two existing nodes in a mutable graph while storing edge data and receiving the
 * new edge identifier.
 *
 * **Details**
 *
 * Creates an `Edge` with the source, target, and data at the next edge index, updates adjacency
 * indexes, and increments the graph's next edge index. Undirected graphs register the same edge for
 * both endpoints.
 *
 * **Gotchas**
 *
 * The source and target nodes must already exist in the mutable graph; missing endpoints throw a
 * `GraphError`.
 *
...
export const hasEdge: {
  ...
  <N, E, T extends Kind = "directed">(
    ...
    for (const edgeIndex of adjacencyList) {
      ...
      if (edge !== undefined) {
        const neighbor =
          graph.type === "undirected" && edge.target === source ? edge.source : edge.target;
        if (neighbor === target) {
          return true;
        }
      }
 ...
 * **Details**
 *
 * For directed graphs, neighbors are the targets of outgoing edges. For undirected graphs,
 * neighbors are the other endpoints of incident edges.
 *
 ...
 * **When to use**
 *
 * Use when maintaining existing code that already passes an explicit traversal direction. New code
 * should prefer `successors` or `predecessors`.
 *
 * **Gotchas**
 *
 * Throws a `GraphError` when used with an undirected graph.
 ...
 * ```
 *
 * @deprecated Use {@link successors} for outgoing neighbors or {@link predecessors} for incoming
 *   neighbors.
 * @since 3.18.0
 * @category Queries
 * @see {@link successors} for outgoing neighbors in a directed graph
 ...
 * **Details**
 *
 * These options customize node labels, edge labels, and graph naming in DOT format compatible with
 * GraphViz tools.
 *
...
export interface GraphVizOptions<N, E> {
  /** Function to generate custom labels for nodes. Defaults to String(data) if not provided. */
  readonly nodeLabel?: (data: N) => string;

  /** Function to generate custom labels for edges. Defaults to String(data) if not provided. */
  readonly edgeLabel?: (data: E) => string;

  /** Name for the DOT graph. Defaults to "G" if not provided. */
 ...
 *
 * When not specified, automatically selects based on graph type: directed graphs use "flowchart",
 * undirected graphs use "graph".
 *
 ...
 * **Details**
 *
 * These options customize node labels, edge labels, diagram type, layout direction, node shapes,
 * and graph naming in Mermaid format.
 *
...
export interface MermaidOptions<N, E> {
  ...
  /**
   * Diagram type override. If not specified, automatically detects: - "flowchart" for directed
   * graphs - "graph" for undirected graphs
   */
  readonly diagramType?: MermaidDiagramType;

  /** Direction for diagram layout. Defaults to "TD" (Top Down) if not provided. */
  readonly direction?: MermaidDirection;

  /**
   * Function to determine node shape for each node. Defaults to "rectangle" for all nodes if not
   * provided.
   */
 ...
 * **Details**
 *
 * Mermaid is a popular diagram-as-code tool that generates flowcharts and other visualizations from
 * text-based definitions. This function converts Effect Graph structures to valid Mermaid syntax
 * for use in documentation, web applications, and visualization tools.
 *
 ...
 *   edgeLabel: (edge) => edge,
 *   nodeShape: (dep) =>
 *     dep.type === "framework" ? "hexagon" : dep.type === "tool" ? "diamond" : "rectangle",
 *   direction: "TB",
 * });
 *
 ...
 * **Details**
 *
 * `"outgoing"` selects edges whose source is the node, while `"incoming"` selects edges whose
 * target is the node.
 *
 * @since 3.18.0
 * @category Models
 ...
 * **Details**
 *
 * `"outgoing"` follows edges from source to target, `"incoming"` follows them from target to
 * source, and `"undirected"` allows traversal in either direction.
 *
 ...
 * **Details**
 *
 * Uses depth-first search to detect back edges, which indicate cycles. For directed graphs, any
 * back edge creates a cycle. For undirected graphs, a back edge that doesn't use the same edge used
 * to enter the current node creates a cycle.
 *
...
export const isAcyclic = <N, E, T extends Kind = "directed">(
  ...
  if (graph.type === "undirected") {
    ...
    for (const startNode of impl.nodes.keys()) {
      ...
      const stack: Array<{ node: NodeIndex; incoming: EdgeIndex | null }> = [
        { node: startNode, incoming: null },
      ];
 ...
 * **Details**
 *
 * A bipartite graph is one whose vertices can be divided into two disjoint sets such that no two
 * vertices within the same set are adjacent. Uses BFS coloring to determine bipartiteness.
 *
...
/**
 * Get neighbors for undirected graphs by checking both adjacency and reverse adjacency. For
 * undirected graphs, we need to find the other endpoint of each edge incident to the node.
 */
...
const getTraversableNeighbor = <N, E, T extends Kind>(
  ...
  graph.type === "undirected" && edge.target === current ? edge.source : edge.target;

/**
 * Finds connected components in an undirected graph. Each component is represented as an array of
 * node indices.
 *
...
/**
 * Finds strongly connected components in a directed graph using Kosaraju's algorithm. Each SCC is
 * represented as an array of node indices.
 *
 * **Gotchas**
 *
 * Throws a `GraphError` when used with an undirected graph.
 ...
 * **When to use**
 *
 * Use to read the successful source-to-target shortest path returned by path-finding algorithms,
 * including the ordered node indices, total distance, and traversed edge data.
 *
 * **Details**
 *
 * Contains the node-index path, the total numeric distance, and the edge data encountered along the
 * path.
 *
 * **Gotchas**
 *
 * `costs` contains original edge data, not the numeric output of the cost function unless the edge
 * data is numeric.
 *
 * @since 3.18.0
 * @category Models
...
const minHeapLessThan = (self: MinHeapEntry, that: MinHeapEntry): boolean =>
  ...
  (self.priority === that.priority && self.sequence < that.sequence);
 ...
 * **When to use**
 *
 * Use when configuring `dijkstra` to find a shortest path between two existing node indices with
 * non-negative edge costs.
 *
 * **Details**
 *
 * Specifies the source and target node indices, plus a cost function that maps each edge's data to
 * a non-negative numeric weight. `Infinity` is allowed and behaves like an impassable edge.
 *
 * **Gotchas**
 *
 * `dijkstra` throws a `GraphError` when either endpoint does not exist or when the cost function
 * returns a negative weight or `NaN`.
 *
 * @since 3.18.0
 * @category Models
...
/**
 * Finds the shortest path from the configured source node to the target node using Dijkstra's
 * algorithm.
 *
 * **Details**
 *
 * Edge costs must be non-negative and not `NaN`. `Infinity` is allowed and behaves like an
 * impassable edge. Returns `Option.none()` when the target is not reachable, and throws a
 * `GraphError` when either endpoint is missing or an edge cost is negative or `NaN`.
 *
 ...
 * **When to use**
 *
 * Use when storing or passing around the complete output of `floydWarshall` so callers can look up
 * shortest distances, node paths, and edge data for any source and target node pair.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * Computes distances, reconstructed node paths, and edge-data paths for every source and target
 * pair in O(V^3) time. Negative edge weights are allowed, and `Infinity` behaves like an impassable
 * edge. A `GraphError` is thrown if any edge weight is `NaN` or `-Infinity`, or if any negative
 * cycle is detected.
 *
 ...
 * **When to use**
 *
 * Use when configuring `astar` for point-to-point shortest-path searches where node data can
 * provide a heuristic estimate toward the target.
 *
 * **Details**
 *
 * Specifies the source and target node indices, an edge-cost function that maps edge data to
 * non-negative weights, and a heuristic that estimates the remaining cost from a node to the
 * target.
 *
 * @since 3.18.0
 * @category Models
...
/**
 * Finds the shortest path from the configured source node to the target node using the A*
 * pathfinding algorithm.
 *
 * **Details**
 *
 * The edge-cost function must return non-negative weights and not `NaN`. `Infinity` is allowed and
 * behaves like an impassable edge. The heuristic should be consistent to preserve shortest-path
 * guarantees. Returns `Option.none()` when the target is not reachable, and throws a `GraphError`
...
/**
 * Finds the shortest path from the configured source node to the target node using the Bellman-Ford
 * algorithm.
 *
 * **Details**
 *
 * Negative edge weights are allowed, and `Infinity` behaves like an impassable edge. Returns
 * `Option.none()` when the target is unreachable or when a negative cycle affects the path to the
 * target. Throws a `GraphError` when either endpoint is missing or an edge weight is `NaN` or
 * `-Infinity`.
 *
 ...
 * **Details**
 *
 * A `Walker` yields `[index, data]` pairs lazily and can be viewed as just the indices, just the
 * values, or mapped entries with `indices`, `values`, `entries`, and `visit`.
 *
...
export class Walker<T, N> implements Iterable<[T, N]> {
   ...
   * **Details**
   *
   * Takes a function that receives the index and data, and returns an iterable of the mapped
   * values. Skips elements that no longer exist in the graph.
   *
  ...
  constructor(
    ...
    this[Symbol.iterator] = () =>
      visit((index, data) => [index, data] as [T, N])[Symbol.iterator]();
  }
}
...
/**
 * Type alias for edge iteration using Walker. EdgeWalker is represented as Walker<EdgeIndex,
 * Edge<E>>.
 *
 * **When to use**
 *
 * Use to type helpers or parameters that consume edge iterators returned by `Graph` APIs, where
 * each item is keyed by an `EdgeIndex` and carries the full `Edge`.
 *
 * @since 3.18.0
 * @category Models
 * @see {@link Walker} for the generic lazy iterator wrapper
 ...
 * **When to use**
 *
 * Use to configure the starting node indices and edge-following direction for lazy graph
 * traversals.
 *
 * **Details**
 *
 * `start` supplies the node indices where traversal begins. If it is omitted, the iterator is
 * empty. `direction` chooses whether traversal follows outgoing edges, incoming edges, or ignores
 * edge direction. `radius` limits traversal by edge distance from the nearest start node.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * If no start nodes are supplied, the iterator is empty. The `direction` option chooses whether to
 * follow outgoing or incoming edges. The `radius` option limits traversal by edge distance from the
 * start nodes. Throws a `GraphError` if any configured start node does not exist.
 *
 ...
 * **Details**
 *
 * `initials` optionally supplies zero in-degree node indices used as prioritized initial queue
 * entries. Topological sorting still includes the other zero in-degree nodes and produces a
 ...
 * **Details**
 *
 * The iterator uses Kahn's algorithm to lazily produce nodes in topological order. Throws an error
 ...
 * **Details**
 *
 * Nodes are emitted after their reachable descendants have been processed. If no start nodes are
 * supplied, the iterator is empty. The `direction` option chooses whether to follow outgoing or
 * incoming edges. The `radius` option limits traversal by edge distance from the start nodes.
 *
 * **Gotchas**
 *
 * With a finite `radius`, iteration first performs a bounded breadth-first traversal to determine
 * shortest-distance membership before emitting nodes in postorder.
 *
 ...
 * **Details**
 *
 * The iterator produces node indices in the order they were added to the graph. This provides
 ...
 * **Details**
 *
 * The iterator produces edge indices in the order they were added to the graph. This provides
 ...
 * **When to use**
 *
 * Use to configure how `externals` identifies graph boundary nodes when you need sinks with no
 * outgoing edges or sources with no incoming edges.
 *
 * **Details**
 *
 * `direction` chooses which missing edge direction makes a node external: `"outgoing"` selects
 * nodes with no outgoing edges, and `"incoming"` selects nodes with no incoming edges. If omitted,
 ...
 * **Details**
 *
 * External nodes have no outgoing edges (`direction: "outgoing"`) or no incoming edges (`direction:
 * "incoming"`). These are useful for finding sources, sinks, or isolated nodes.
 *
