 ...
 *
 * A `Cause<E>` can contain typed failures, unexpected defects, interruptions, and annotations.
 * Keeping those details together lets code inspect or format failures without first collapsing them
 * to a single error value. This module includes the `Cause` and `Reason` data types, helpers for
 * building and checking causes, and small error types used by several Effect APIs.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use to preserve the full structured failure information for an effect instead of collapsing it to
 * a single error value.
 *
 * **Details**
 *
 * Access the individual failure entries through the `reasons` array, then narrow each entry with
 * {@link isFailReason}, {@link isDieReason}, or {@link isInterruptReason}.
 *
 * - Use {@link hasFails} / {@link hasDies} / {@link hasInterrupts} to test for the presence of
 *   specific reason kinds without iterating.
 * - Use {@link findError} / {@link findDefect} to extract the first value of a given kind.
 ...
 * **Details**
 *
 * Narrow to a concrete type with {@link isFailReason}, {@link isDieReason}, or
 ...
 *
 * Every reason carries an `annotations` map and an `annotate` method for attaching tracing
 * metadata.
 *
 ...
 * **When to use**
 *
 * Use as a predicate for `Array.filter` to pick out typed `Fail` reasons when iterating over
 * `cause.reasons`.
 *
 ...
 * **When to use**
 *
 * Use as a predicate for `Array.filter` to pick out `Die` (defect) reasons when iterating over
 * `cause.reasons`.
 *
 ...
 * **When to use**
 *
 * Use as a predicate for `Array.filter` to pick out `Interrupt` reasons when iterating over
 * `cause.reasons`.
 *
 ...
 * **When to use**
 *
 * Use when inspecting `Cause` reasons that represent defects instead of typed failures or
 * interruptions.
 *
 * **Details**
 *
 * The `defect` property is `unknown` because defects are not part of the typed error channel. Use
 * {@link isDieReason} to narrow a `Reason` to this type.
 *
 ...
 * **When to use**
 *
 * Use when inspecting `Cause` reasons that represent expected failures from the typed error
 * channel.
 *
 * **Details**
 *
 * The `error` property carries the typed value `E`. Use {@link isFailReason} to narrow a `Reason`
 * to this type.
 *
...
/**
 * A fiber interruption signal, optionally carrying the ID of the fiber that initiated the
 * interruption.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you already have individual reasons (e.g. from filtering or transforming another cause's
 * `reasons` array) and need to wrap them back into a `Cause`.
 *
 * **Details**
 *
 * - Returns a new `Cause`.
 ...
 * **Gotchas**
 *
 * The `reasons` array is stored as provided. Treat the array as immutable after passing it to this
 * function.
 *
 ...
 * **Details**
 *
 * Represents the absence of failure. Combining any cause with `empty` via {@link combine} returns
...
/**
 * Creates a `Cause` containing a single `Interrupt` reason, optionally carrying the interrupting
 * fiber's ID.
 *
 ...
 * **When to use**
 *
 * Use when constructing a standalone typed failure reason for {@link fromReasons} or direct
 * comparison.
 *
 ...
 * **When to use**
 *
 * Use when constructing a standalone defect reason for {@link fromReasons} or direct comparison.
 *
...
/**
 * Creates a standalone `Interrupt` reason (not wrapped in a `Cause`), optionally carrying the
 * interrupting fiber's ID.
 *
 * **When to use**
 *
 * Use when constructing a standalone interrupt reason for {@link fromReasons} or direct comparison.
 *
 ...
 * @since 4.0.0
 * @category Predicates
 * @see {@link hasInterrupts} — `true` if the cause contains _any_ interrupts
 */
export const hasInterruptsOnly: <E>(self: Cause<E>) => boolean = effect.hasInterruptsOnly;

/**
 * Transforms the typed error values inside a `Cause` using the provided function. Only `Fail`
 * reasons are affected; `Die` and `Interrupt` reasons pass through unchanged.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * If at least one `Fail` reason exists, this returns a new `Cause` containing the mapped failures.
 * If the cause has no `Fail` reasons, the original cause is returned unchanged.
 *
...
/**
 * Merges two causes into a single cause whose `reasons` array is the union of both inputs
 * (de-duplicated by value equality).
 *
 * **When to use**
 *
...
/**
 * Collapses a `Cause` into a single `unknown` value, picking the "most important" failure in this
 * order:
 *
 * **When to use**
 *
 * Use to collapse a structured cause to the single value that synchronous and promise runners would
 * throw.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * This function is lossy. Use {@link prettyErrors} or iterate `cause.reasons` when you need all
 * failures.
 *
 ...
 * **When to use**
 *
 * Use to check whether a cause includes typed failures before extracting, mapping, or rendering
 * them.
 *
...
/**
 * Returns a `Result` whose success value is the first `Fail` reason in the cause, including its
 * annotations. If the cause has no `Fail` reason, the failure value is the original cause narrowed
 * to `Cause<never>`, because it contains no typed error reasons.
 *
 * **When to use**
 *
 ...
 */
export const findFail: <E>(self: Cause<E>) => Result.Result<Fail<E>, Cause<never>> =
  effect.findFail;

/**
 * Returns a `Result` whose success value is the first typed error value `E` from a `Fail` reason in
 * the cause. If the cause has no `Fail` reason, the failure value is the original cause narrowed to
 * `Cause<never>`, because it contains no typed error reasons.
 *
 * **When to use**
 *
 * Use when you need the first typed error value from a `Cause` as a `Result` that preserves the
 * original cause when no match is found.
 *
...
/**
 * Returns the first typed error value `E` from a cause wrapped in `Option.some`, or `Option.none`
 * if no `Fail` reason exists.
 *
 * **When to use**
 *
 * Use when you need the first typed error value from a `Cause` as an `Option`, discarding the
 * original cause.
 *
...
/**
 * Returns a `Result` whose success value is the first `Die` reason in the cause, including its
 * annotations. If the cause has no `Die` reason, the failure value is the original cause.
 *
 * **When to use**
 *
...
/**
 * Returns a `Result` whose success value is the first defect value from a `Die` reason in the
 * cause. If the cause has no `Die` reason, the failure value is the original cause.
 *
 * **When to use**
 *
 * Use when you need the first defect value from a `Cause` as a `Result`, without the full `Die`
 * reason.
 *
 ...
 */
export const findDefect: <E>(self: Cause<E>) => Result.Result<unknown, Cause<E>> =
  effect.findDefect;
 ...
 * @since 4.0.0
 * @category Predicates
 * @see {@link hasInterruptsOnly} — `true` only when _all_ reasons are interrupts
 * @see {@link hasFails} — check for typed errors
 * @see {@link hasDies} — check for defects
 */
...
/**
 * Returns a `Result` whose success value is the first `Interrupt` reason in the cause, including
 * its annotations. If the cause has no `Interrupt` reason, the failure value is the original
 * cause.
 *
 * **When to use**
 *
 * Use when you need the first `Interrupt` reason from a `Cause`, including the fiber ID and
 * annotations.
 *
 ...
 */
export const findInterrupt: <E>(self: Cause<E>) => Result.Result<Interrupt, Cause<E>> =
  effect.findInterrupt;

/**
 * Collects the defined fiber IDs from all `Interrupt` reasons in the cause into a `ReadonlySet`.
 * Interrupt reasons without a `fiberId` are ignored. Returns an empty set when the cause has no
...
/**
 * Returns a `Result` whose success value is the set of defined fiber IDs from the cause's
 * `Interrupt` reasons. If the cause has no `Interrupt` reason, the failure value is the original
 * cause.
 *
 * **When to use**
 *
 ...
 * **Gotchas**
 *
 * Interrupt reasons without a `fiberId` still count as interrupts, so the function succeeds with an
 * empty `Set` when every interrupt reason has an undefined fiber ID.
 *
 ...
 * **When to use**
 *
 * Use to convert every renderable failure in a cause into individual `Error` values before logging
 * or rethrowing.
 *
 * **Details**
 *
 ...
 *
 * - **Objects / Error instances** — `message`, `name`, `stack`, and `cause` are preserved. Extra
 *   enumerable properties are copied. Stack traces are cleaned up and enriched with span
 *   annotations when available.
 * - **Strings** — used directly as the `Error` message.
 * - **Other primitives** (`null`, `undefined`, numbers, …) — wrapped in an `Error` with message
 *   `"Unknown error: <value>"`.
 *
 * `Interrupt` reasons are collected separately. If the cause contains **only** interrupts (no
 * `Fail` or `Die`), a single `InterruptError` is returned whose `cause` lists the interrupting
 * fiber IDs.
 *
 ...
 * **Details**
 *
 * Delegates to {@link prettyErrors} to convert each reason to an `Error`, then joins their stack
 * traces with newlines. Nested `Error.cause` chains are rendered inline with indentation:
 *
 * ```text
 * ErrorName: message
 *     at ...
...
/**
 * Base interface for error classes that can be yielded directly inside `Effect.gen`. Yielding one
 * of these errors fails the generator with that error as the typed failure value.
 *
 * **Details**
 *
 * All built-in error classes in this module (`NoSuchElementError`, `TimeoutError`,
 * `IllegalArgumentError`, `ExceededCapacityError`, `AsyncFiberError`, and `UnknownError`) implement
 * this interface.
 *
 ...
 * **Details**
 *
 * Used by APIs that convert absence into an exception or effect failure, such as
 * `Option.getOrThrow`. Implements `YieldableError` so it can be yielded directly in `Effect.gen`.
 *
 * **Gotchas**
 *
 * Prefer APIs that return `Option` or a typed failure when absence is an expected case. This error
 * is mainly for APIs that intentionally turn absence into a thrown value or failed effect.
 *
 ...
 * **When to use**
 *
 * Use to create the error value for APIs that intentionally fail when an expected element is
 * absent.
 *
 ...
 * **Details**
 *
 * `Done` indicates that a producer has finished normally — no more elements will arrive. It is
 * distinct from an error or interruption; it represents successful completion. The optional `value`
 * field can carry a final leftover payload.
 *
...
/**
 * An error indicating that a function received an argument that violates its contract (e.g.
 * negative where positive was expected).
 *
 * **Details**
 *
 ...
 * @category Constructors
 */
export const IllegalArgumentError: new (message?: string) => IllegalArgumentError =
  effect.IllegalArgumentError;
 ...
 * @category Guards
 */
export const isExceededCapacityError: (u: unknown) => u is ExceededCapacityError =
  effect.isExceededCapacityError;
...
/**
 * An error indicating that a bounded resource (queue, pool, semaphore, etc.) has exceeded its
 * capacity.
 *
 * **When to use**
 *
 * Use to model bounded-resource failures where an operation cannot proceed because capacity has
 * been exhausted.
 *
 * **Details**
 *
 ...
 */
export const ExceededCapacityError: new (message?: string) => ExceededCapacityError =
  effect.ExceededCapacityError;

/**
 * Unique brand present on `AsyncFiberError` values and used by `isAsyncFiberError` for runtime
 * checks.
 *
 * @since 4.0.0
 * @category Type IDs
 ...
 * **When to use**
 *
 * Use to inspect failures produced when synchronous runners encounter an effect that cannot
 * complete synchronously.
 *
 * **Details**
 *
 * The `fiber` property stores the fiber that could not be synchronously resolved. This error
 * implements `YieldableError`.
 *
 ...
 */
export const AsyncFiberError: new (fiber: Fiber<unknown, unknown>) => AsyncFiberError =
  effect.AsyncFiberError;
 ...
 * **Details**
 *
 * Used when a thrown or rejected value is not represented by a more specific typed error. The
 * original value is stored in the `cause` property inherited from `Error`. Implements
 * `YieldableError`.
 *
...
/**
 * Constructs an `UnknownError`. The first argument is the original cause (stored in `Error.cause`);
 ...
 * @category Constructors
 */
export const UnknownError: new (cause: unknown, message?: string) => UnknownError =
  effect.UnknownError;
 ...
 * **Details**
 *
 * Annotations are stored as a `Context` on each reason and can be retrieved later via
 * {@link reasonAnnotations} or {@link annotations}. The runtime uses this to attach stack traces
 * and spans.
 *
 * - Returns a new `Cause`.
 ...
 * **When to use**
 *
 * Use when you need tracing metadata (e.g. `StackTrace`) from a specific reason rather than the
 * whole cause.
 *
 ...
 * **When to use**
 *
 * Use to read the failure stack-frame annotation from a `Reason` when building diagnostics,
 * logging, or custom cause renderers.
 *
 * **Details**
 *
 * The runtime annotates every reason with this when a stack frame is available. Retrieve it via
