 ...
 *
 * An `Equivalence<A>` returns `true` when two `A` values should be treated as the same for a
 * particular purpose. This module includes strict equality instances for primitive types,
 * constructors for custom comparisons, and helpers for tuples, arrays, structs, records, dates, and
 * values compared through a derived field.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need an equality rule that the built-in instances and input mapping helpers cannot
 * express, and you can provide a law-abiding comparison.
 *
 * **Details**
 *
 * The returned equivalence first checks reference equality (`===`) for performance. If the values
 * are not the same reference, it falls back to the provided equivalence function, which must
 * satisfy reflexive, symmetric, and transitive properties.
 *
 ...
 * **Details**
 *
 * Uses JavaScript's strict equality operator (`===`). Primitives compare by value. Objects compare
 * by reference, so only the same object instance is equivalent. Use this as a building block for
 * more complex equivalences via `mapInput` or `combine`.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * Returns `true` only if both equivalences return `true`. The comparison short-circuits when the
 * first equivalence returns `false`. The result is also an equivalence that satisfies reflexive,
 ...
 * **Details**
 *
 * Returns `true` only if all equivalences in the collection return `true`. The comparison stops at
 * the first equivalence that returns `false`. Empty collections return an equivalence that always
 * returns `true`. The result is also an equivalence that satisfies reflexive, symmetric, and
 ...
 * - Useful for comparing by one property or normalizing values before comparison, such as
 ...
 * **Details**
 *
 * Tuples must have the same length; different lengths are never equivalent. Each equivalence is
 * applied to the corresponding element position. The result returns `true` only if all elements are
 * equivalent according to their respective equivalences, and it also satisfies reflexive,
 * symmetric, and transitive properties.
 *
...
export {
   ...
   * - The result is also an equivalence that satisfies reflexive, symmetric, and transitive
   *   properties
   *
 ...
 * **Details**
 *
 * Compares only the properties specified in the struct definition; other properties are ignored.
 * String and symbol keys are supported via `Reflect.ownKeys`. The result returns `true` only if all
 * specified properties are equivalent according to their equivalences, and it also satisfies
...
/**
 * Creates a `Reducer` for combining `Equivalence` instances, useful for aggregating equivalences in
 * collections.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Returns a reducer that combines equivalences using `combine`. The identity element for empty
 * collections is an equivalence that always returns `true`. The reducer uses `combineAll` for
 * collections of equivalences and can be used with fold operations.
 *
...
/**
 * Equivalence instance for `Date` objects that compares their `getTime()` values using
 * `Equivalence.Number`.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Different `Date` instances that represent the same millisecond timestamp are equivalent. Because
 * `Equivalence.Number` treats `NaN` as equal to `NaN`, two invalid `Date` values are also
