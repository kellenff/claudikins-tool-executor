 ...
 *
 * This module helps create plain classes, tagged classes, tagged unions, and typed errors with
 * readonly fields. Tagged values carry a `_tag` field, which makes them easy to narrow with pattern
 * matching or simple checks. These helpers are commonly used for domain values and errors in Effect
 * programs.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * Extend `Class` with a type parameter to declare fields. The constructor accepts those fields as a
 * single object argument. When there are no fields the argument is optional. Instances are
 * `Readonly` and `Pipeable`.
 *
 ...
 * **Details**
 *
 * Like {@link Class}, but the resulting instances also carry a `readonly _tag: Tag` property. The
 * `_tag` is excluded from the constructor argument.
 *
 ...
 * **Details**
 *
 * Each key in the record becomes a variant with `readonly _tag` set to that key. Use with
 * {@link taggedEnum} to get constructors and matchers.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use to reference utility types for constructing, extracting, and matching `TaggedEnum` variants.
 *
 * **Details**
 *
 ...
 * - Extracting constructor arguments ({@link TaggedEnum.Args}) and variant values
 *   ({@link TaggedEnum.Value})
...
export declare namespace TaggedEnum {
   ...
   * **Details**
   *
   * Extend this interface and set `taggedEnum` to your union type, using `this["A"]`, `this["B"]`,
   * etc. as placeholders for the generics. The `Count` parameter declares how many generics are
   * used (up to 4).
   *
  ...
  /**
   * Applies concrete type arguments to a `WithGenerics` definition, producing the resulting tagged
   * union type.
   *
   * **When to use**
   *
   ...
   * **When to use**
   *
   * Use when you want to annotate an exported constructor bundle so downstream code keeps exact
   * variant constructors and exhaustive matching.
   *
   * **Details**
   *
   * Includes:
   ...
   * - `$is(tag)` — returns a type-guard that checks only the `_tag` field; safe when the tag is
   *   globally unique and the value was produced by your constructors. For untrusted input,
   *   validate with the `Schema` module first.
  ...
  /**
   * Function type that constructs a tagged-union variant from its fields, excluding the keys listed
   * in `Tag`.
   *
   * **When to use**
   *
   ...
   * **Details**
   *
   * The constructor returns the full variant type `A`. If no fields remain after excluding `Tag`
   * keys, the constructor argument type becomes `void`.
   *
   * @since 4.0.0
   * @category Utility types
   */
   ...
   * **Details**
   *
   * This is the `$is` / `$match` portion of the object returned by {@link taggedEnum} when used
   * with a {@link WithGenerics} definition.
   *
   * @since 3.2.0
   * @category Models
 ...
 * **When to use**
 *
 * Use when you model a closed union with plain data objects and want construction, tag checks, and
 * exhaustive matching from the same definition.
 *
 * **Details**
 *
 ...
 * - `$is(tag)` only checks the `_tag` field, not the full structure. It relies on the tag being
 *   globally unique and the value being produced by your constructors. For untrusted input,
 *   validate with the `Schema` module first.
 *
 ...
 * **Details**
 *
 * Extends `Cause.YieldableError`, so instances can be yielded inside `Effect.gen` to fail the
 * enclosing effect. Fields are passed as a single object; when there are no fields the argument is
 * optional. If a `message` field is provided, it becomes the error's `.message`.
 *
 ...
 * **Details**
 *
 * Like {@link Error}, but instances also carry a `readonly _tag` property, enabling
 * `Effect.catchTag` and `Effect.catchTags` for tag-based recovery. The `_tag` is excluded from the
 * constructor argument. Yielding an instance inside `Effect.gen` fails the effect with this error.
 *
