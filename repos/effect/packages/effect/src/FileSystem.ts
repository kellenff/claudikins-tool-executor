 ...
 *
 * `FileSystem` is the boundary between Effect code and the host file system. Platform packages
 * provide concrete layers, while this module defines the operations for reading, writing,
 * inspecting, streaming, and watching files. Operations return `Effect`, `Stream`, or `Sink` values
 * and fail with `PlatformError`. The module also includes file handles, size helpers, open flags,
 * watch events, and the watch backend service.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The FileSystem interface provides a comprehensive set of file and directory operations that work
 * cross-platform. All operations return Effect values that can be composed, transformed, and
 * executed safely with proper error handling.
 *
...
export interface FileSystem {
  ...
  /**
   * Checks whether a file can be accessed. You can optionally specify the level of access to check
   * for.
   */
  readonly access: (
    path: string,
 ...
 * **Details**
 *
 * This type ensures type safety when working with file sizes, preventing accidental mixing of
 * regular numbers with size values. The underlying bigint allows for handling very large file sizes
 * beyond JavaScript's number precision limits.
 *
 ...
 * **Details**
 *
 * This union type allows file system operations to accept size values in different formats for
 * convenience, which are then normalized to the branded `Size` type internally.
 *
 ...
 * **Details**
 *
 * Converts numbers, bigints, or existing Size values into a properly branded Size type. This
 * function handles the conversion and ensures type safety for file size operations.
 *
 ...
 * **Details**
 *
 * Converts a number of kilobytes to the equivalent size in bytes. Uses binary kilobytes (1024
 * bytes) rather than decimal (1000 bytes).
 *
 ...
 * **Details**
 *
 * Converts a number of mebibytes to the equivalent size in bytes. Uses binary mebibytes (1,048,576
 * bytes) rather than decimal megabytes.
 *
 ...
 * **Details**
 *
 * Converts a number of gibibytes to the equivalent size in bytes. Uses binary gibibytes
 * (1,073,741,824 bytes) rather than decimal gigabytes.
 *
 ...
 * **Details**
 *
 * Converts a number of tebibytes to the equivalent size in bytes. Uses binary tebibytes
 * (1,099,511,627,776 bytes) rather than decimal terabytes.
 *
 ...
 * **Details**
 *
 * Converts a number of pebibytes to the equivalent size in bytes. Uses binary pebibytes
 * (1,125,899,906,842,624 bytes) rather than decimal petabytes. This function uses BigInt arithmetic
 * to handle the very large numbers involved.
 *
 ...
 * **Details**
 *
 * These flags correspond to standard POSIX file open modes and control the file access permissions
 * and behavior when opening files.
 *
 ...
 * **When to use**
 *
 * Use to access or provide operations for files, directories, permissions, streams, and sinks
 ...
 * **When to use**
 *
 * Use to build a concrete `FileSystem` service from platform-specific core operations while
 * deriving the convenience methods that can be implemented from them.
 *
 * **Details**
 *
 * This function takes a partial FileSystem implementation and automatically provides default
 * implementations for `exists`, `readFileString`, `stream`, `sink`, and `writeFileString` methods
 * based on the provided core methods.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * By default, `exists` returns `false`, `remove` succeeds, many file operations fail with
 * `PlatformError` `NotFound`, and temporary-directory/file operations die as not implemented. Pass
 * method overrides to provide the behavior needed by a specific test without touching the real file
 * system.
 *
 ...
 * **Details**
 *
 * This is a convenience function that wraps `makeNoop` in a Layer, making it easy to provide the
 * test filesystem to your Effect programs.
 *
...
/**
 * Runtime type identifier attached to `FileSystem.File` handles and used by `isFile` to recognize
 * them.
 *
 * **Details**
 *
 * This marker is part of the runtime representation of file handles. Prefer `isFile` when narrowing
 * unknown values.
 *
 * @since 4.0.0
 * @category Type IDs
 ...
 * **When to use**
 *
 * Use when accepting an unknown value and you need to narrow it to a `File` before calling
 * file-handle operations.
 *
 * **Details**
 *
 * This is a structural marker check. It does not validate the marker value or the shape of the file
 * handle.
 *
 * @since 4.0.0
 * @category File
 ...
 * **Details**
 *
 * Provides low-level file operations including reading, writing, seeking, and retrieving file
 * information. File handles are automatically managed within scoped operations to ensure proper
 * cleanup.
 *
...
/**
 * Namespace containing types associated with open file handles, including file descriptors, entry
 * kinds, and stat information.
 *
 * @since 4.0.0
 */
export declare namespace File {
   ...
   * **Details**
   *
   * File descriptors are numeric handles used by the operating system to identify open files. The
   ...
   * **Details**
   *
   * Represents the different types of entries that can exist in a file system, from regular files
   * to special device files and symbolic links.
   *
   * @since 4.0.0
   * @category File
   */
   ...
   * **Details**
   *
   * Contains metadata about a file or directory including type, timestamps, permissions, and size
   * information. This structure is returned by file stat operations.
   *
 ...
 * **When to use**
 *
 * Use to brand an operating-system file descriptor number when implementing a `FileSystem` that
 * returns custom `File` handles.
 *
 * **Details**
 *
 * `File.Descriptor` is a branded integer handle used by operating systems to identify open files.
 *
 * **Gotchas**
 *
 * This constructor is nominal and does not check that the number is an integer or that it refers to
 ...
 * **When to use**
 *
 * Use with `File` handles when positioning the cursor before a read or write and the offset must be
 * interpreted from either the start of the file or the current cursor.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when consuming file system watch streams and pattern matching on `_tag` to handle created,
 * updated, or removed paths.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * This service provides the low-level file watching capabilities that can be implemented
 * differently on various platforms (e.g., inotify on Linux, FSEvents on macOS, etc.).
 *
