 ...
 *
 * Publishers add messages with `publish` or `publishAll`, and each active `Subscription` receives
 * its own copy of every accepted message. Unlike a queue, subscribers do not compete for messages.
 * This module includes bounded, dropping, sliding, and unbounded hubs, optional replay buffers for
 * late subscribers, message-taking helpers, capacity and shutdown operations, and low-level types
 * for custom hub strategies.
 *
 * @since 2.0.0
 */
...
/**
 * A `PubSub<A>` is an asynchronous message hub into which publishers can publish messages of type
 * `A` and subscribers can subscribe to take messages of type `A`.
 *
...
/**
 * Companion namespace containing the low-level building blocks used by `PubSub`, including atomic
 * implementations, backing subscriptions, replay windows, and delivery strategies.
 *
 * @since 2.0.0
 */
export declare namespace PubSub {
   ...
   * **Details**
   *
   * This type is part of the low-level `PubSub.Strategy` contract. Most application code should use
   * `subscribe`, `take`, and the other `PubSub` operations instead of manipulating subscriber maps
   * directly.
   *
   * @since 4.0.0
   * @category Models
  ...
  export interface Strategy<in out A> {
    ...
    /**
     * Describes how publishers should signal to subscribers that they are waiting for space to
     * become available in the `PubSub`.
     */
    handleSurplus(
      pubsub: Atomic<A>,
    ...
    /**
     * Describes how subscribers should signal to publishers waiting for space to become available
     * in the `PubSub` that space may be available.
     */
    onPubSubEmptySpaceUnsafe(pubsub: Atomic<A>, subscribers: Subscribers<A>): void;

    /**
     * Describes how subscribers waiting for additional values from the `PubSub` should take those
     * values and signal to publishers that they are no longer waiting for additional values.
     */
    ...
    /**
     * Describes how publishers should signal to subscribers waiting for additional values from the
     * `PubSub` that new values are available.
     */
 ...
 * **Details**
 *
 * Published messages are retained until all current subscribers have taken them. When the capacity
 * is full, publishers suspend until space is available. Pass an options object to configure both
 * `capacity` and an optional replay buffer for late subscribers.
 *
...
/**
 * Creates a bounded `PubSub` with the dropping strategy. The `PubSub` will drop new messages if the
 * `PubSub` is at capacity.
 *
 * **Details**
 *
...
/**
 * Creates a bounded `PubSub` with the sliding strategy. The `PubSub` will add new messages and drop
 * old messages if the `PubSub` is at capacity.
 *
 * **Details**
 *
 * For best performance use capacities that are powers of two.
 ...
 * **When to use**
 *
 * Use to provide bounded message storage when building a custom `PubSub` with `make` and an
 * explicit delivery strategy.
 *
 * **Details**
 *
 * Pass either a capacity number or an options object with `capacity` and optional `replay`. A
 * positive `replay` value enables a replay buffer for late subscribers, and fractional replay sizes
 * are rounded up.
 *
 * **Gotchas**
 *
 * The capacity must be greater than zero; invalid capacities throw synchronously before an atomic
 ...
 * **When to use**
 *
 * Use to create the low-level storage layer for a custom `PubSub` whose active subscribers may
 * retain an unbounded number of pending messages.
 *
 * **Gotchas**
 *
 * Messages published while subscribers are active can be retained without a capacity limit until
 * those subscribers take them or unsubscribe.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * If the `PubSub` has been shut down, the returned effect succeeds with `0`. The size is not a
 * count of waiting subscribers or suspended publishers.
 *
 ...
 * **When to use**
 *
 * Use when an immediate `PubSub` size snapshot is needed outside effectful code and concurrent
 * changes between the check and later use are acceptable.
 *
 * **Details**
 *
 * Returns `0` after shutdown. Because this is an unsafe synchronous snapshot, prefer `size` in
 * effectful code.
 *
...
/**
 * Shuts down the `PubSub`, interrupting suspended publishers and subscribers and finalizing active
 * subscriptions.
 *
 * **Details**
 *
 * After shutdown, `publish` and `publishAll` succeed with `false`, `publishUnsafe` returns `false`,
 * and subscription operations such as `take` interrupt.
 *
...
/**
 * Checks effectfully whether `shutdown` has been called, returning `true` after shutdown and
 * `false` otherwise.
 *
...
/**
 * Checks synchronously whether `shutdown` has been called, returning `true` after shutdown and
 * `false` otherwise.
 *
 * **When to use**
 *
 * Use when an immediate `PubSub` shutdown-state snapshot is needed outside effectful code and
 * racing shutdown changes are acceptable.
 *
...
/**
 * Waits until the queue is shutdown. The `Effect` returned by this method will not resume until the
 * queue has been shutdown. If the queue is already shutdown, the `Effect` will resume right away.
 *
 ...
 * **When to use**
 *
 * Use when you need to publish from effectful code and let the configured PubSub strategy handle
 * surplus messages.
 *
 * **Details**
 *
 * The effect succeeds with `false` if the `PubSub` is shut down. If the message cannot be accepted
 * immediately, the configured strategy decides how surplus messages are handled.
 *
...
/**
 * Attempts to publish a message synchronously without applying the PubSub strategy's effectful
 * surplus handling.
 *
 * **When to use**
 *
 * Use when you need a non-blocking synchronous publish attempt where `false` is an acceptable
 * result when the message cannot be accepted immediately.
 *
 * **Details**
 *
 * Returns `false` if the `PubSub` is shut down or the message cannot be accepted immediately, for
 * example when a bounded PubSub is full. Prefer `publish` when backpressure or sliding behavior
 * should be honored.
 *
...
/**
 * Publishes all of the specified messages to the `PubSub`, returning whether they were published to
 * the `PubSub`.
 *
...
/**
 * Subscribes to receive messages from the `PubSub`. The resulting subscription can be evaluated
 * multiple times within the scope to take a message from the `PubSub` each time.
 *
...
/**
 * Takes a single message from the subscription. If no messages are available, this will suspend
...
/**
 * Takes between the specified minimum and maximum number of messages from the subscription. Will
 * suspend if the minimum number is not immediately available.
 *
 ...
 * **Details**
 *
 * The count includes replay-buffered messages. If the subscription has been shut down, the effect
 * interrupts.
 *
...
/**
 * Synchronously returns the number of messages currently available in the subscription, or
 * `Option.none()` when it is shut down.
 *
 * **When to use**
 *
 * Use when you need synchronous polling outside a managed workflow and want shutdown observed as
 * data instead of interruption.
 *
 ...
 * **When to use**
 *
 * Use to preserve every message for current subscribers when a bounded custom `PubSub` should make
 * publishers wait for capacity instead of dropping or evicting messages.
 *
 * **Details**
 *
 * Publishers wait when the `PubSub` is at capacity, so all current subscribers can receive every
 * published message.
 *
 * **Gotchas**
 *
