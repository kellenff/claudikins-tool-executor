/**
 * The `NonEmptyIterable` module provides a type-level representation of any JavaScript `Iterable`
 * that is known to contain at least one element. A `NonEmptyIterable<A>` can be consumed anywhere
 * an `Iterable<A>` is expected, while also carrying the guarantee that reading the first element is
 * safe.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * `NonEmptyIterable<A>` includes `readonly [nonEmpty]: A`, which makes it distinct from a plain
 * `Iterable<A>` at compile time while preserving the normal iteration shape.
 *
 * @since 2.0.0
 * @category Symbols
 ...
 * **Details**
 *
 * `NonEmptyIterable<A>` extends the standard `Iterable<A>` interface with a type-level guarantee of
 * non-emptiness. This allows for safe operations that would otherwise require runtime checks or
 * could throw exceptions.
 *
 * The type is branded with a unique symbol to ensure type safety while maintaining full
 * compatibility with JavaScript's iteration protocol.
 *
 ...
 *   Chunk.fromIterable(countdown()) as unknown as NonEmptyIterable.NonEmptyIterable<number>,
 * ); // 3
 * ```
 *
 * @since 2.0.0
 ...
 * **When to use**
 *
 * Use to split a non-empty iterable into its first element and an iterator for the remaining
 * elements.
 *
 * **Details**
 *
 * This function provides a safe way to deconstruct a `NonEmptyIterable` into its head (first
 * element) and tail (remaining elements as an iterator). Since the iterable is guaranteed to be
 * non-empty, the first element is always available.
 *
