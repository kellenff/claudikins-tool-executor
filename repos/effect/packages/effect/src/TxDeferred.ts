 ...
 *
 * A `TxDeferred<A, E>` is a write-once cell whose completion is a `Result<A, E>` stored in
 * transactional state. Readers can wait for the value from inside a transaction: while the cell is
 * empty the transaction retries, and when another transaction completes the deferred the waiting
 * transaction can resume with either the success value or the typed failure.
 *
 * @since 4.0.0
 */
...
/**
 * A transactional deferred is a write-once cell readable within transactions. Readers block (retry
 * the transaction) until a value is committed, and writers succeed only on the first call;
 * subsequent writes return `false`.
 *
 * **When to use**
 *
 * Use to coordinate transaction-local readers and one-time completion with a success or failure
 * result.
 *
 ...
 * @category Getters
 */
export const poll = <A, E>(self: TxDeferred<A, E>): Effect.Effect<Option<Result<A, E>>> =>
  TxRef.get(self.ref);

/**
 * Completes the deferred with a `Result`. Returns `true` if this was the first completion, `false`
...
/**
 * Completes the deferred with a success value. Returns `true` if this was the first completion,
...
/**
 * Completes the deferred with a failure. Returns `true` if this was the first completion, `false`
 * if already completed.
 *
 * **When to use**
 *
