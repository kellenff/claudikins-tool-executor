 ...
 *
 * A `ScopedRef<A>` is useful for resource-backed values such as clients, connections,
 * subscriptions, or handles. Replacing the value acquires the replacement in a new scope and
 * releases the resources owned by the previous value. Reads can be effectful or synchronous, and
 * updates are synchronized so only one replacement happens at a time.
 *
 * @since 2.0.0
 */
...
/**
 * A `ScopedRef` is a reference whose value is associated with resources, which must be released
 * properly. You can both get the current value of any `ScopedRef`, as well as set it to a new value
 * (which may require new resources). The reference itself takes care of properly releasing
 * resources for the old value whenever a new value is obtained.
 *
 * **When to use**
 *
 * Use when an application needs to keep a current resource-backed value and later replace it with
 * another acquired value while ensuring the previous value is released.
 *
 * @since 2.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when creating a `ScopedRef` whose initial value requires acquiring resources that must be
 * released.
 *
 * @since 2.0.0
 ...
 * **When to use**
 *
 * Use when you need immediate synchronous access to the current `ScopedRef` value and can guarantee
 * that reading outside the `Effect` API is safe.
 *
 * @since 4.0.0
 * @category Getters
 ...
 * **When to use**
 *
 * Use to create a `ScopedRef` when the initial value is already available or can be produced
 * without acquiring resources.
 *
 * **Details**
 *
 * The `evaluate` function runs when the returned effect runs. The returned effect requires a
 * `Scope`, and the reference closes the currently stored value's scope when that outer scope
 * closes.
 *
 * **Gotchas**
 *
 * Do not use `make` for an initial value whose creation acquires resources; use `fromAcquire` so
 * acquisition and finalization are tracked.
 *
 * @since 2.0.0
 * @category Constructors
...
/**
 * Sets the value of this reference to a newly acquired scoped value, releasing any resources
 * associated with the old value.
 *
 * **When to use**
 *
 * Use to replace the current value of an existing `ScopedRef` with a newly acquired scoped value
 * while releasing resources for the previous value.
 *
 * **Details**
 *
 * This method will not return until either the reference is successfully changed to the new value,
 * with old resources released, or until the attempt to acquire a new value fails.
 *
 * @since 2.0.0
 * @category Setters
 */
