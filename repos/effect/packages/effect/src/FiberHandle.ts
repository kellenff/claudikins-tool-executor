 ...
 *
 * A `FiberHandle<A, E>` can hold one `Fiber<A, E>`. Installing a new fiber interrupts the previous
 * one unless the operation is configured with `onlyIfMissing`, and closing the owning scope
 * interrupts the current fiber. This module includes constructors for handles and scoped runtimes,
 * helpers for setting, reading, clearing, and running fibers, and operations for joining the
 * current fiber or waiting until the handle is empty.
 *
 * @since 2.0.0
 */
...
/**
 * Scoped handle that manages at most one fiber, interrupts the current fiber when the handle's
 * scope closes, and removes managed fibers from the handle when they complete.
 *
 ...
 * **Details**
 *
 * When the associated `Scope` is closed, the contained fiber will be interrupted. You can add a
 * fiber to the handle using `FiberHandle.run`, and the fiber will be automatically removed from the
 * `FiberHandle` when it completes.
 *
 ...
 * **Details**
 *
 * Each call returns the forked fiber, stores it in the handle, and interrupts the previous fiber
 * unless `onlyIfMissing` is set. The managed fiber is interrupted when the handle's scope closes.
 *
...
/**
 * Creates a scoped run function that forks effects into a new `FiberHandle` and returns a `Promise`
 * for each effect result.
 *
 * **When to use**
 *
 * Use when integrating a scoped `FiberHandle` runner with Promise-based APIs and Promise rejection
 * from squashed failures is the desired boundary.
 *
 * **Details**
 *
 * Each call stores the fiber in the handle and interrupts the previous fiber unless `onlyIfMissing`
 * is set. The returned Promise resolves with the effect's success value or rejects with the
 * squashed failure cause.
 *
...
/**
 * Sets the fiber in a FiberHandle. When the fiber completes, it will be removed from the
 * FiberHandle. If a fiber is already running, it will be interrupted unless `options.onlyIfMissing`
 * is set.
 *
 * **When to use**
 *
 * Use when an existing forked fiber must be installed synchronously into a handle and immediate
 * interruption of replaced or closed fibers is acceptable.
 *
 ...
 * **Details**
 *
 * When the fiber completes, it will be removed from the `FiberHandle`. If a fiber already exists in
 * the `FiberHandle`, it will be interrupted unless `options.onlyIfMissing` is set.
 *
 ...
 * **When to use**
 *
 * Use when synchronous inspection of the current fiber is needed and an `Option` result is enough
 * outside the Effect workflow.
 *
 ...
 * **Details**
 *
 * The handle manages only one fiber: running a new effect interrupts the previous fiber unless
 * `onlyIfMissing` is set. When the managed fiber completes, it is removed from the handle.
 *
...
/**
 * Captures the current runtime and returns a function for forking effects into an existing
 * `FiberHandle`.
 *
 * **Details**
 *
...
/**
 * Captures the current runtime and returns a function for running effects in an existing
 * `FiberHandle` as Promises.
 *
 * **Details**
 *
 * Each call stores the forked fiber in the handle and interrupts the previous fiber unless
 * `onlyIfMissing` is set. The Promise resolves with the effect's success value or rejects with the
 * squashed failure cause.
 *
 ...
 * **Details**
 *
 * The returned Effect fails with the first managed fiber failure that is not ignored by the
 * handle's interruption rules. Normal successful completion of a managed fiber only removes it from
 * the handle; use `awaitEmpty` to wait for the current fiber to finish.
 *
