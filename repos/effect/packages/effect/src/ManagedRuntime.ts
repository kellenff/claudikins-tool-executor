 ...
 *
 * A `ManagedRuntime` builds the services from a layer, keeps those services available for repeated
 * effect runs, and releases acquired resources when it is disposed. This module includes the
 * runtime type, a constructor, a guard, and runners for connecting Effect programs to JavaScript
 * entry points such as promises, callbacks, and synchronous code.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * The guard checks the internal `ManagedRuntime` marker property. It does not build the layer or
 * inspect the runtime's services.
 *
 * **Gotchas**
 *
 * Disposed runtimes still carry the marker, so this guard does not prove the runtime is still
 * usable.
 *
 * @since 3.9.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use as the reusable runtime value returned by `make` when application entry points or integration
 * code need to run many effects against the same layer-built services.
 *
 * **Details**
 *
 * The runtime builds and caches its service context and owns the scope for resources acquired by
 * the layer.
 *
 * **Gotchas**
 *
...
export interface ManagedRuntime<in R, out ER> {
   ...
   * **When to use**
   *
   * Use to fork an effect against this runtime's services and get the running fiber.
   */
   ...
   * **When to use**
   *
   * Use when invoking this effectful method at the edges of your program.
   */
  ...
  /**
   * Executes the effect asynchronously, eventually passing the exit value to the specified
   * callback.
   *
   * **When to use**
   *
  ...
  /**
   * Runs the `Effect`, returning a JavaScript `Promise` that will be resolved with the value of the
   * effect once the effect has been executed, or will be rejected with the first error or exception
   * throw by the effect.
   *
   * **When to use**
   *
  ...
  /**
   * Runs the `Effect`, returning a JavaScript `Promise` that will be resolved with the `Exit` state
   * of the effect once the effect has been executed.
   *
   * **When to use**
   *
   * Use when invoking this effectful method at the edges of your program.
 ...
 * **When to use**
 *
 * Use to create a reusable runtime from a `Layer` for application entry points or integration code
 * that runs many effects without rebuilding services.
 *
 * **Details**
 *
 * The layer is built lazily on first use and its context is cached for subsequent runs. Resources
 * acquired by the layer are owned by the runtime and are released when `dispose` or `disposeEffect`
 * is run. `options.memoMap` can be used to share layer memoization with other layer builds.
 *
 * **Gotchas**
 *
