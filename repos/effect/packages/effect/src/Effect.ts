 ...
 *
 * An `Effect<A, E, R>` can succeed with an `A`, fail with an `E`, and require services `R`.
 * Creating an effect does not perform the work; it builds a value that can be composed, provided
 * with services, retried, interrupted, run concurrently, or inspected by the runtime. This module
 * is the main API for creating effects, combining them, handling failures, managing resources, and
...
/**
 * The `Effect` interface defines a value that lazily describes a workflow or job. The workflow
 * requires some context `R`, and may fail with an error of type `E`, or succeed with a value of
 * type `A`.
 *
 * **When to use**
 *
 * Use when you need to represent a lazy, composable workflow that can require services, fail with a
 * typed error, or succeed with a typed value.
 *
 * **Details**
 *
 * `Effect` values model resourceful interaction with the outside world, including synchronous,
 * asynchronous, concurrent, and parallel interaction. They use a fiber-based concurrency model,
 * with built-in support for scheduling, fine-grained interruption, structured concurrency, and high
 * scalability.
 *
 * To run an `Effect` value, you need a `Runtime`, which is a type that is capable of executing
 * `Effect` values.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use to derive the value produced by an existing effect when declaring reusable type aliases,
 * service interfaces, or function signatures.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use to derive the error type from an existing `Effect` type when declaring helper types,
 * wrappers, or APIs that preserve the effect's failure channel.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to derive the context requirements of a generic or inferred `Effect` without restating its
 * `R` type parameter.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Iterator interface for Effect generators, enabling Effect values to work with generator
 * functions.
 *
 * **When to use**
 *
 * Use when defining or typing `[Symbol.iterator]()` for values typed as `Effect`s so `yield*` can
 * pass their success type back into `Effect.gen`.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Namespace containing type utilities for the `Effect.all` function, which handles collecting
 * multiple effects into various output structures.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use to run a known collection of effects and collect results in the same tuple, iterable, or
 * record shape.
 *
 * **Details**
 *
 * Tuple and iterable inputs collect results in order. Record inputs collect results under the same
 * keys. By default, the combined effect fails on the first failure; with concurrent execution,
 * effects that have already started may be interrupted, while effects not yet started are skipped.
 *
 * Options:
 *
 * Use `concurrency` to control sequential or concurrent execution. Use `mode: "result"` to run
 * every effect and collect each success or failure as a `Result` in the same output shape. Use
 * `discard: true` to ignore successful values and return `void`.
 *
 ...
 * **Details**
 *
 * The returned tuple is `[excluded, satisfying]`, where `excluded` contains all failures and
 ...
 * **Details**
 *
 * The accumulator function receives the current accumulator, the current element, and its
 * zero-based index. The `zero` function is evaluated each time the effect runs. An empty iterable
 * succeeds with its result. If a step fails, remaining elements are not processed.
 *
 ...
 *   () => 0,
 *   (total, value, index) =>
 *     Console.log(`Adding ${value} at index ${index}`).pipe(Effect.as(total + value)),
 * );
 *
 * Effect.runPromise(program).then(console.log);
 * // Output:
 ...
 * **Details**
 *
 * This function always evaluates every element. If at least one effect fails, all failures are
 * returned as a non-empty array and successes are discarded. If all effects succeed, it returns all
 * collected successes.
 *
 ...
 * **Details**
 *
 * The predicate receives the element and its index. Evaluation short-circuits as soon as an element
 * matches.
 *
 ...
 * **When to use**
 *
 * Use when you need to find the first element that satisfies an effectful filter returning a
 * `Result`, which also transforms the matching element.
 *
 * **Details**
 *
 * The filter receives the element and index. Evaluation short-circuits on the first
 * `Result.succeed` and returns the transformed value in `Option.some`.
 *
 * @since 4.0.0
 * @category Collecting
 ...
 * **When to use**
 *
 * Use to traverse an iterable with an effectful function while preserving element order in the
 * collected results.
 *
 * **Details**
 *
 * The `forEach` function applies a provided operation to each element in the iterable, producing a
 * new effect that returns an array of results.
 *
 * If any effect fails, the iteration stops immediately (short-circuiting), and the error is
 * propagated.
 *
 * Concurrency:
 *
 * The `concurrency` option controls how many operations are performed concurrently. By default, the
 * operations are performed sequentially.
 *
 * Discarding Results:
 *
 * If the `discard` option is set to `true`, the intermediate results are not collected, and the
 * final result of the operation is `void`.
 *
 ...
 * **When to use**
 *
 * Use to convert a `Promise` into an `Effect` when the async operation is guaranteed to succeed and
 * will not reject.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * The `Promise` must not reject. If it rejects, the rejection is treated as a defect, not as a
 * typed failure. Use `tryPromise` when rejection is expected.
 *
 * Interruption aborts the provided `AbortSignal`, but the underlying asynchronous operation only
 * stops if it observes that signal.
 *
...
/**
 * Creates an `Effect` from an asynchronous computation that may throw or reject, mapping failures
 * into the error channel.
 *
 * **When to use**
 *
 * Use when you need to perform asynchronous operations that might fail, such as fetching data from
 * an API, and want thrown exceptions or rejected promises captured as Effect errors.
 *
 * **Details**
 *
 * The promise thunk is evaluated when the effect runs. If it returns a promise that resolves, the
 * resolved value becomes the success value. If the thunk throws before returning a promise, or if
 * the returned promise rejects, the thrown or rejected value is mapped into the error channel.
 *
 * Passing the thunk directly maps failures to {@link Cause.UnknownError}. Passing `{ try, catch }`
 * uses `catch` to map failures to an error of type `E`.
 *
 * The thunk receives an `AbortSignal` that is aborted if the effect is interrupted. The underlying
 * asynchronous operation only stops if it observes that signal.
 *
 * **Gotchas**
 *
 * If `catch` throws while mapping the error, that thrown value is treated as a defect. Return the
 * error value you want in the error channel instead of throwing it.
 *
 ...
 *
 * class TodoFetchError extends Data.TaggedError("TodoFetchError")<{
 ...
 * **When to use**
 *
 * Use when an effect should complete successfully with a specific value without any errors or
 ...
 * **Details**
 *
 * `suspend` takes a thunk that represents an effect and delays creating it until the suspended
 * effect is evaluated. This is useful for optimizing expensive computations, managing circular
 * dependencies such as recursive functions, and helping TypeScript unify return types when branches
 * construct different effects. Any side effects or scoped captures inside the thunk are re-executed
 * on each invocation.
 *
 ...
 * **Gotchas**
 *
 * The function must not throw. If it throws, the thrown value is treated as a defect, not as a
 * typed failure. Use `try` when throwing is expected.
 *
 ...
 * **When to use**
 *
 * Use when you need to integrate APIs that complete through callbacks instead of returning a
 * `Promise`.
 *
 * **Details**
 *
 * The registration function receives a `resume` callback and, when requested, an `AbortSignal`.
 * Call `resume` at most once with the effect that should complete the fiber; later calls are
 * ignored. Return an optional cleanup effect from the registration function to run if the fiber is
 * interrupted.
 *
...
/**
 * Effect that succeeds with an empty record `{}`, used as the starting point for do notation
 * chains.
 *
...
/**
 * Gives a name to the success value of an `Effect`, creating a single-key record used in do
 * notation pipelines.
 *
 * **When to use**
 *
 * Use to start a do-notation pipeline from an existing `Effect` when its success value should
 * become the first named field in the accumulated record.
 *
 * @since 2.0.0
 * @category Do notation
...
export {
   ...
   * **When to use**
   *
   * Use to add a derived, synchronous value to a do-notation pipeline when it depends on fields
   * already accumulated in the record and does not need to run another `Effect`.
   *
   * **Details**
   *
   * The new field is added with object spreading. If the name already exists in the record, the
   * computed value replaces it in the returned type.
   *
   * @since 2.0.0
   * @category Do notation
   * @see {@link bind} for adding fields produced by effects
 ...
 * **When to use**
 *
 * Use to sequence an effectful step in a do-notation pipeline when that step depends on fields
 * already accumulated in the record and its success value should be stored under a name.
 *
 * **Details**
 *
 * The function receives the current record, runs the returned effect after the input effect
 * succeeds, and inserts its success value under `name`. The resulting effect combines the error and
 * service requirements of both steps.
 *
 * **Gotchas**
 *
...
/**
 * Provides a way to write effectful code using generator functions, simplifying control flow and
 * error handling.
 *
 * **When to use**
 *
 * Use when you want to write effectful code that looks and behaves like synchronous code, while
 * still handling asynchronous tasks, errors, and complex control flow such as loops and
 * conditions.
 *
 * Generator functions work similarly to `async/await` but keep errors, requirements, and
 * interruption in the Effect type. You can `yield*` values from effects and return the final result
 * at the end.
 *
 ...
 * **Details**
 *
 * The error keeps propagating unless it is handled. You can handle tagged errors with functions
 * like {@link catchTag} or {@link catchTags}.
 *
 ...
 * **When to use**
 *
 * Use when you already have a full `Cause` and need to preserve defects, interruptions,
 * annotations, or combined failures in the effect's failure channel.
 *
 * **Details**
 *
 * This function allows you to create effects that fail with complex error structures, including
 * multiple errors, defects, interruptions, and more.
 *
 ...
 * **Details**
 *
 * The `die` function is used to signal a defect, which represents a critical and unexpected error
 * in the code. When invoked, it produces an effect that does not handle the error and instead
 * terminates the fiber.
 *
 * The error channel of the resulting effect is of type `never`, indicating that it cannot recover
 * from this failure.
 *
...
export {
  /**
   * Creates an `Effect` from a synchronous computation that may throw, mapping thrown values into
   * the error channel.
   *
   * **When to use**
   *
   * Use when you need to perform synchronous operations that might throw, such as parsing JSON, and
   * want thrown exceptions captured as Effect errors.
   *
   * **Details**
   *
   * The thunk is evaluated when the effect runs. If it returns normally, the returned value becomes
   * the success value. If it throws, the thrown value is mapped into the error channel.
   *
   * Passing the thunk directly maps failures to {@link Cause.UnknownError}. Passing `{ try, catch
   * }` uses `catch` to map failures to an error of type `E`.
   *
   ...
   * import { Data, Effect } from "effect";
   *
   * class JsonParsingError extends Data.TaggedError("JsonParsingError")<{
   *   readonly cause: unknown;
   * }> {}
   *
...
/**
 * Yields control back to the Effect runtime with a specified priority, allowing other fibers to
 * execute.
 *
 ...
 * **Details**
 *
 * `Option.some` becomes a successful effect with the contained value, while `Option.none` becomes a
 * failed effect. By default the failure is a `NoSuchElementError`, but you can provide an `onNone`
 * callback to customize the error value.
 *
 ...
 * **When to use**
 *
 * Use when an effect should run only when an optional value is present, while preserving absence as
 * a successful `None`.
 *
 * **Details**
 *
...
/**
 * Converts a nullable value to an `Effect`, failing with a `NoSuchElementError` when the value is
 * `null` or `undefined`.
 *
...
/**
 * Chains effects to produce new `Effect` instances, useful for combining operations that depend on
 * previous results.
 *
 * **When to use**
 *
 * Use when you need to chain multiple effects, ensuring that each step produces a new `Effect`
 * while flattening any nested effects that may occur.
 *
 * **Details**
 *
 * `flatMap` lets you sequence effects so that the result of one effect can be used in the next
 * step. It is similar to `flatMap` used with arrays but works specifically with `Effect` instances,
 * allowing you to avoid deeply nested effect structures.
 *
 * Since effects are immutable, `flatMap` always returns a new effect instead of changing the
 * original one.
 *
...
/**
 * Runs this effect and then runs another effect, optionally using the first effect's success value
 * to choose the next effect.
 *
 * **When to use**
 *
 * Use when you need one effect to run after another and the second effect may depend on the first
 * effect's success value.
 *
 * **Details**
 *
 * When the second argument is an `Effect`, the first success value is discarded and the returned
 * effect produces the second effect's value. When the second argument is a function, it receives
 * the first success value and must return the next `Effect`.
 *
 ...
 * **When to use**
 *
 * Use when you need to run an effectful observation, such as logging or tracking, while passing the
 * original success value to the next step.
 *
 * **Details**
 *
 * `tap` works similarly to `flatMap`, but it ignores the result of the function passed to it. The
 * value from the previous effect remains available for the next part of the chain. Note that if the
 * side effect fails, the entire chain will fail too.
 *
 ...
 * **When to use**
 *
 * Use when you want an `Effect`'s typed failures to be handled as `Result` data while preserving
 * the original error value.
 *
 * **Details**
 *
 * This function converts an effect that may fail into an effect that always succeeds, wrapping the
 * outcome in a `Result` type. The result will be `Result.Failure` if the effect fails, containing
 * the recoverable error, or `Result.Success` if it succeeds, containing the result.
 *
 * Using this function, you can handle recoverable errors explicitly without causing the effect to
 * fail. This is particularly useful in scenarios where you want to chain effects and manage both
 * success and failure in the same logical flow.
 *
 * The resulting effect cannot fail directly because all recoverable failures are represented inside
 * the `Result` type.
 *
 * **Gotchas**
 *
 * `result` only captures typed, recoverable failures. Defects and interruptions are not captured
 * inside the `Result` and still fail the effect.
 *
 ...
 * **When to use**
 *
 * Use when you only care whether an effect succeeds and want recoverable failures represented as
 * `Option.none`.
 *
 * **Details**
 *
 * Success values become `Option.some`, recoverable failures become `Option.none`, and defects still
 * fail the effect.
 *
 * **Gotchas**
 *
 * `option` only captures typed, recoverable failures as `Option.none`. Defects and interruptions
 * are not captured inside the `Option` and still fail the effect.
 *
 ...
 * **When to use**
 *
 * Use when you need to inspect the full outcome, including typed failures, defects, and
 * interruptions.
 *
 * **Details**
 *
 * `exit` wraps an effect's success or failure inside an `Exit` type, allowing you to handle both
 * cases explicitly.
 *
 * The resulting effect cannot fail because the failure is encapsulated within the `Exit.Failure`
 * type. The error type is set to `never`, indicating that the effect is structured to never fail
 * directly.
 *
 ...
 * **When to use**
 *
 * Use to transform an effect's success value with a function that returns a plain value, producing
 * a new effect without changing the original effect's typed error or context requirements.
 *
 * **Details**
 *
 * `map` takes a function and applies it to the value contained within an effect, creating a new
 * effect with the transformed value.
 *
 ...
 * **Details**
 *
 * `as` allows you to ignore the original value inside an effect and replace it with a new constant
 * value.
 *
 ...
 * **Details**
 *
 * The `zip` function executes the first effect (left) and then the second effect (right). Once both
 * effects succeed, their results are combined into a tuple.
 *
 * Concurrency:
 *
 * By default, `zip` processes the effects sequentially. To execute the effects concurrently, use
...
/**
 * Combines two effects sequentially and applies a function to their results to produce a single
 * value.
 *
 * **When to use**
 *
 * Use when you need to run two effects sequentially and combine their results with a function
 * instead of keeping the results as a tuple.
 *
 * **Details**
 *
 * Concurrency:
 *
 * By default, the effects are run sequentially. To execute them concurrently, use the `{
 * concurrent: true }` option.
 *
...
export {
   ...
   * **Details**
   *
   * The `catch` function catches any errors that may occur during the execution of an effect and
   * allows you to handle them by specifying a fallback effect. This ensures that the program
   * continues without failing by recovering from errors using the provided fallback logic.
   *
   * **Gotchas**
   *
   * `catch` only handles recoverable errors. It will not recover from unrecoverable defects.
   *
   * @since 4.0.0
 ...
 * **Details**
 *
 * The error type must have a readonly `_tag` field. `catchTag` matches that field and only handles
 * errors with the requested tag.
 *
 ...
 * **When to use**
 *
 * Use when one recovery step should handle several tagged error types by matching their readonly
 * `_tag` fields.
 *
 * **Details**
 *
 ...
 *
 * The error type must have a readonly `_tag` field to use `catchTags`. This field is used to
 * identify and match errors.
 *
 ...
 * **When to use**
 *
 * Use to handle one nested reason inside an `Effect`'s tagged error while preserving the parent
 * error shape for unmatched reasons.
 *
 * **Details**
 *
 * Use this to handle nested error causes without removing the parent error from the error channel.
 ...
 * **When to use**
 *
 * Use to constrain custom helpers or overloads to parent error tags whose error contains a tagged
 * reason.
 *
 * **Details**
 *
 * The mapped type keeps each parent error tag whose extracted tagged error has at least one reason
 * tag, and removes tags that do not carry tagged reasons.
 *
 * @since 4.0.0
 * @category Error handling
 ...
 * **When to use**
 *
 * Use when you need to recover from an `Effect` by inspecting the full `Cause`, including
 * recoverable failures, defects, and interruptions, instead of only the typed error value.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * `catchDefect` handles unexpected defects, such as thrown exceptions or values passed to `die`,
 * without catching typed failures or interruptions.
 *
 * When to Recover from Defects:
 *
 * Defects are unexpected errors that typically should not be recovered from, as they often indicate
 * serious issues. In some cases, such as dynamically loaded plugins, controlled recovery may be
 * needed.
 *
 ...
 * **Details**
 *
 * Use a `Refinement` for type narrowing or a `Predicate` for simple boolean matching. Non-matching
 * errors re-fail with the original cause. Defects and interrupts are not caught.
 *
 ...
 *   Effect.catchFilter(Filter.tagged("NotFound"), (error) =>
 ...
 * **When to use**
 *
 * Use to recover from typed `Effect` errors with a reusable `Filter` when matching can also narrow
 * or transform the error before choosing the recovery effect.
 *
 * **Details**
 *
 * The filter runs on typed failures extracted from the `Cause`. Successful filter results are
 * passed to `f`; failed filter results are passed to `orElse` when provided. Without `orElse`, the
 ...
 * **When to use**
 *
 * Use when you expect missing-value failures and want them to become an optional success while all
 * other failures keep failing.
 *
 * **Details**
 *
 * Success values become `Option.some`, `NoSuchElementError` becomes `Option.none`, and all other
 ...
 * **Details**
 *
 * This function allows you to conditionally catch and recover from failures that match a specific
 * predicate. This is useful when you want to handle only certain types of errors while letting
 * others propagate.
 *
 ...
 * **When to use**
 *
 * Use when you need to recover an `Effect` only from causes selected by a `Filter`, while giving
 * the recovery both the selected value and the original `Cause`.
 *
 * **Details**
 *
 * The filter is applied to the full `Cause`. When it succeeds, the handler receives the selected
 * value and the original cause. When it fails, the effect re-fails with the residual cause returned
 * by the filter.
 *
 * @since 4.0.0
 * @category Error handling
 ...
 * **When to use**
 *
 * Use to transform both success and failure channels of an `Effect` without changing whether it
 * succeeds or fails.
 *
 * **Details**
 *
 * This function takes two map functions as arguments: one for the error channel and one for the
 * success channel. You can use it when you want to modify both the error and the success values
 * without altering the overall success or failure status of the effect.
 *
...
/**
 * Converts typed failures from the error channel into defects, removing the error type from the
 * returned effect.
 *
 * **When to use**
 *
 * Use when you need to turn an `Effect` typed failure that represents an unrecoverable bug or
 * invalid state into a defect.
 *
...
/**
 * Runs an effectful operation when the source effect fails, while preserving the original failure
 * when the operation succeeds.
 *
 * **Details**
 *
 * Use this for logging, metrics, or other failure-side observations. If the operation passed to
 * `tapError` fails, that error is also represented in the returned effect's error channel.
 *
 ...
 * **Details**
 *
 * Use this with tagged-union errors to perform side effects for one tag or a list of tags. When the
 * handler succeeds, the original failure is preserved; if the handler fails, its error is also
 * included in the returned effect.
 *
 ...
 * **When to use**
 *
 * Use when failure observation needs typed failures, defects, and interruptions rather than only
 * the typed error value.
 *
 * **Details**
 *
 * Use this to log or inspect typed failures, defects, and interruptions. When the operation
 * succeeds, the original cause is preserved. If the operation fails, its error is also represented
 ...
 * **Details**
 *
 * This function allows you to tap into the cause of an effect's failure only when the cause matches
 * a specific predicate. This is useful for conditional logging, monitoring, or other side effects
 ...
 * **When to use**
 *
 * Use when you need to observe only failure causes selected by a `Filter`, while giving the side
 * effect both the selected value and the original `Cause`.
 *
 * **Details**
 *
 * A successful filter result runs the side effect with the selected value and original cause. A
 * failed filter result skips the side effect and preserves the original cause.
 *
 * @since 4.0.0
 * @category Sequencing
 ...
 * **Details**
 *
 * Use this for diagnostics such as logging unexpected thrown exceptions or values passed to `die`.
 * Recoverable failures are not handled. When the operation succeeds, the original defect is
 * preserved; if the operation fails, its error is also represented in the returned effect.
 *
 ...
 * **When to use**
 *
 * Use when you need to rerun an effect after transient typed failures, such as network issues or
 * temporary resource unavailability.
 *
 * **Details**
 *
 * The policy can be a `Schedule`, a schedule builder, or a `Retry.Options` object using `schedule`,
 * `times`, `while`, or `until`. If a retry eventually succeeds, the returned effect succeeds with
 * that value. If the policy stops while the effect is still failing, the last failure is
 * propagated.
 *
 * **Gotchas**
 *
 * The source effect is always evaluated once before any retry policy is applied. For example,
 * `Schedule.recurs(3)` allows up to three retries after the initial attempt.
 *
 ...
 * **When to use**
 *
 * Use when you want to handle failures gracefully by specifying an alternative action after
 * repeated failures.
 *
 * **Details**
 *
 * The `Effect.retryOrElse` function attempts to retry a failing effect multiple times according to
 * a defined {@link Schedule} policy.
 *
 ...
 * **Details**
 *
 * Use `sandbox` when downstream error handling needs to distinguish typed failures, defects, and
 * interruptions. Use `unsandbox` to restore the original typed error channel after cause-level
 * handling.
 *
 ...
 * **When to use**
 *
 * Use when an effect should run for its side effects while both success and failure values are
 * discarded.
 *
 * **Details**
 *
 * Use the `log` option to emit the full {@link Cause} when the effect fails, and `message` to
 * prepend a custom log message.
 *
 ...
 * const programWarn = task.pipe(
 *   Effect.ignore({ log: "Warn", message: "Ignoring task failure" }),
 * );
 * ```
 *
 * @since 2.0.0
 ...
 * **When to use**
 *
 * Use when a best-effort effect should never fail, even from defects or interruption, and optional
 ...
 * const programLog = task.pipe(
 *   Effect.ignoreCause({ log: true, message: "Ignoring failure cause" }),
 * );
 * ```
 *
 * @since 4.0.0
...
/**
 * Applies an `ExecutionPlan` to an effect, retrying with step-provided resources until it succeeds
 * or the plan is exhausted.
 *
 * **Details**
 *
 * Each attempt updates `ExecutionPlan.CurrentMetadata` (attempt and step index), and retry timing
 * is derived per step (the first attempt uses the remaining attempts schedule; later retries apply
 * the step schedule at least once).
 *
 ...
 * **Details**
 *
 * If the `defectsOnly` option is set to `true`, only defects (unrecoverable errors) will be
 * reported, while regular failures will be ignored.
 *
 * @since 4.0.0
 * @category Error handling
 */
 ...
 * **Details**
 *
 * If the source effect succeeds, its value is preserved. If it fails in the error channel,
 * `orElseSucceed` evaluates the fallback and succeeds with that value, removing the typed error
 * from the returned effect.
 *
 ...
 * **When to use**
 *
 * Use when you have prioritized fallback `Effect`s, such as attempting multiple APIs, reading
 * configuration from several sources, or trying alternative resource locations in order.
 *
 * **Details**
 *
 * This function executes the provided effects in sequence, stopping at the first success. If an
 * effect succeeds, its result is returned immediately and no further effects in the sequence are
 * executed.
 *
 * If all effects fail, the returned effect fails with the error from the last effect. If the
 * collection is empty, the returned effect defects with an `Error` whose message is `"Received an
 * empty collection of effects"`.
 *
 ...
 * **Details**
 *
 * The `timeout` function allows you to specify a time limit for an effect's execution. If the
 * effect does not complete within the given time, a `TimeoutException` is raised. This can be
 * useful for controlling how long your program waits for a task to finish, ensuring that it doesn't
 * hang indefinitely if the task takes too long.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * If the source effect succeeds before the timeout, the returned effect succeeds with
 * `Option.some(value)`. If the timeout wins, the source effect is interrupted and the returned
 * effect succeeds with `Option.none`. If the source effect fails before the timeout, that failure
 * is preserved.
 *
 ...
 * **Details**
 *
 * The fallback effect is created lazily by `orElse` and may introduce its own success, failure, and
 * requirement types.
 *
 * **Gotchas**
 *
...
/**
 * Returns an effect that suspends the current fiber for the specified duration without blocking a
 * JavaScript thread.
 *
 ...
 * **Details**
 *
 * The original success, failure, or interruption is preserved; only the success value is paired
 * with the duration.
 *
 ...
 * **Details**
 *
 * Early failures do not finish the race; `raceAll` keeps waiting until one effect succeeds or every
 * effect has failed. When one effect succeeds, the remaining effects are interrupted. If every
 * effect fails, the returned effect fails with a cause containing the collected failure reasons.
 *
...
/**
 * Runs multiple effects concurrently and completes with the first effect to finish, whether it
 * succeeds or fails.
 *
 * **Details**
 *
 * After the first effect completes, all remaining effects are interrupted. Use `raceAll` when early
 * failures should be ignored until a success occurs or all effects fail.
 *
 ...
 * **Details**
 *
 * If one effect succeeds, the other is interrupted and `onWinner` can observe the winning fiber. If
 * both fail, the race fails.
 *
...
/**
 * Races two effects and returns the result of the first one to complete, whether it succeeds or
 * fails.
 *
 * **When to use**
 *
 * Use when any completion, including failure, should decide the race and interrupt the losing
 * effect.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to filter an iterable with a `Filter` inside an `Effect`, collecting each
 * filter success value.
 *
 * **Details**
 *
 * `Result.succeed` values are collected in the returned array, and `Result.fail` values are
 * skipped.
 *
 * @since 2.0.0
 * @category Filtering
 ...
 * **When to use**
 *
 * Use when you need to filter each iterable element effectfully and transform accepted elements
 * into successful output values.
 *
 * **Details**
 *
 * `Result.succeed` values are collected in the returned array, and `Result.fail` values are
 ...
 * **When to use**
 *
 * Use when a successful value that fails a predicate should continue with an effectful fallback
 * instead of failing the effect.
 *
 * **Details**
 *
 * This function applies a predicate to the result of an effect. If the predicate evaluates to
 * `false`, it executes the `orElse` effect instead. The `orElse` effect can produce an alternative
 * value or perform additional computations.
 *
 ...
 * **When to use**
 *
 * Use when a successful effect value should be accepted and transformed by a `Filter`, while
 * rejected values should continue with an alternative effect built from the filter failure.
 *
 * **Details**
 *
 ...
 *
 * This function applies a predicate to the result of an effect. If the predicate evaluates to
 * `false`, the effect fails with either a custom error (if `orFailWith` is provided) or a
 * `NoSuchElementError`.
 *
 ...
 * **When to use**
 *
 * Use when validating and transforming one effect success with a synchronous `Filter`, while
 * rejected values should fail the effect.
 *
 * **Details**
 *
 * `Result.succeed` becomes the returned success value. `Result.fail` is mapped with `orFailWith`
 * when provided, or fails with `NoSuchElementError`.
 *
 * @since 4.0.0
 * @category Filtering
 ...
 * **When to use**
 *
 * Use when you need an effectful check to decide whether another effect should run while
 * representing the skipped case explicitly.
 *
 * **Details**
 *
 * The condition effect is evaluated first. If it succeeds with `true`, the source effect is run and
 * its success value is wrapped in `Option.some`. If it succeeds with `false`, the source effect is
 * skipped and the result is `Option.none`. If the condition effect fails, that failure is
 * preserved.
 *
 ...
 * **When to use**
 *
 * Use when you need to fold an `Effect` into a value by handling success and failure differently
 * without triggering side effects.
 *
 * **Details**
 *
 * `match` lets you define custom handlers for both success and failure scenarios. You provide
 * separate functions to handle each case, allowing you to process the result if the effect
 * succeeds, or handle the error if the effect fails.
 *
...
/**
 * Handles both success and failure cases of an effect without performing side effects, with eager
 * evaluation for resolved effects.
 *
 * **When to use**
 *
 * Use when you need to handle both success and failure cases of an already-resolved `Effect` with
 * optimized handling.
 *
 * **Details**
 *
 * `matchEager` works like `match` but provides better performance for resolved effects (Success or
 * Failure). When the effect is already resolved, it applies the handlers immediately without fiber
 * scheduling. For unresolved effects, it falls back to the regular `match` behavior.
 *
 ...
 * **Details**
 *
 * The `matchCause` function allows you to handle failures with access to the full cause of the
 * failure within a fiber.
 *
 ...
 * **When to use**
 *
 * Use when you expect an `Effect` to already be resolved and want to match the `Cause` without
 * regular effect pipeline overhead.
 *
 * **Details**
 *
 * `matchCauseEager` works like `matchCause` but provides better performance for resolved effects by
 * immediately applying the matching function instead of deferring it through the effect pipeline.
 *
 ...
 * **When to use**
 *
 * Use when you need effectful success and cause-aware failure handlers for `Effect` inputs that may
 * already be resolved.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to fold an `Effect` with effectful success handlers and `Cause`-aware failure
 * handlers.
 *
 * **Details**
 *
 * The `matchCauseEffect` function works similarly to {@link matchCause}, but it also allows you to
 * perform additional side effects based on the failure cause. This function provides access to the
 * complete cause of the failure, making it possible to differentiate between various failure types,
 * and allows you to respond accordingly while performing side effects (like logging or other
 * operations).
 *
 ...
 * **Details**
 *
 * Use `matchEffect` when either branch needs to return an `Effect`, such as performing logging,
 * recovery, notification, or other effectful work. The returned effect succeeds or fails according
 * to the handler that is run.
 *
 ...
 * **Details**
 *
 * This function allows you to access all services that are currently available in the effect's
 * environment. This can be useful for debugging, introspection, or when you need to pass the entire
 * context to another function.
 *
 ...
 *   Context.add(Database, { query: () => "result" }),
 * );
 *
 ...
 * **Details**
 *
 * This function allows you to access the complete context and perform computations based on all
 * available services. This is useful when you need to conditionally execute logic based on what
...
/**
 * Provides dependencies to an effect using layers or a context. Use `options.local` to build the
 * layer every time; by default, layers are shared between provide calls.
 *
 ...
 * **Details**
 *
 * This function provides multiple services at once by supplying a context that contains all the
 * required services. It removes the provided services from the effect's requirements, making them
 ...
 * **When to use**
 *
 * Use when you already have a `Context` containing every service required by the effect and want
 * the wrapped effect to run with exactly that context.
 *
 * **Gotchas**
 *
 * `setContext` replaces the current context for the wrapped effect. Services from an outer context
 * are not inherited unless they are also present in the context passed to `setContext`.
 *
 ...
 * **When to use**
 *
 * Use to read an optional dependency from the current context without making that dependency part
 * of the effect's required environment.
 *
 * **Details**
 *
 * This function attempts to access a service from the environment. If the service is available, it
 * returns `Some(service)`. If the service is not available, it returns `None`. Unlike `service`,
 * this function does not require the service to be present in the environment.
 *
 ...
 * **Details**
 *
 * This function allows you to transform the context required by an effect, providing part of the
 * context and leaving the rest to be fulfilled later.
 *
 ...
 * **Details**
 *
 * The service must be available in the effect's context; `updateService` replaces it for the
 * wrapped effect with the value returned by the updater.
 *
 ...
 * **Details**
 *
 * The service requirement identified by the `Context.Key` is removed from the effect requirements
 * after the implementation is provided.
 *
 ...
 * **When to use**
 *
 * Use when the service implementation must be created by an effect and its acquisition failure
 * should remain in the returned effect.
 *
 * **Details**
 *
 * `provideServiceEffect` runs the acquisition effect to produce the service implementation, removes
 * that service from the wrapped effect's requirements, and leaves any other requirements to be
 * provided later. Acquisition failures are included in the returned effect's error channel.
 *
 ...
 * **Details**
 *
 * Finalizers for resources acquired inside the workflow run as soon as the workflow completes,
 ...
 * **When to use**
 *
 * Use when resource acquisition needs direct access to the scope being created, for example to
 * register finalizers manually.
 *
 ...
 * **Details**
 *
 * If acquisition succeeds, the release finalizer is added to the current scope and is guaranteed to
 * run when that scope closes. The finalizer receives the `Exit` value used to close the scope.
 *
 * By default, acquisition is protected by an uninterruptible region. Pass `{ interruptible: true }`
 * to allow the acquisition effect to be interrupted.
 *
 ...
 *   Console.log(
 *     `Closing file ${handle.path} with exit: ${Exit.isSuccess(exit) ? "success" : "failure"}`,
 *   );
 *
 * // Create a scoped resource
 ...
 * **When to use**
 *
 * Use when you work with JavaScript `Disposable` or `AsyncDisposable` resources that should be
 * closed with the surrounding scope.
 *
 * **Details**
 *
 * The resource is automatically disposed when the surrounding {@link Scope} is closed, using
 * {@link Symbol.dispose} for synchronous disposables or {@link Symbol.asyncDispose} for
 * asynchronous disposables.
 *
 * This is similar to {@link acquireRelease}, but uses the standard JavaScript disposal protocol
 * instead of requiring an explicit release function. It works with JavaScript `Disposable` and
 * `AsyncDisposable` resources.
 *
 ...
 *
 * 1. Ensures that the `Effect` value that acquires the resource will not be interrupted. Note that
 *    acquisition may still fail due to internal reasons (such as an uncaught exception).
 * 2. Ensures that the `release` `Effect` value will not be interrupted, and will be executed as long
 *    as the acquisition `Effect` value successfully acquires the resource.
 *
 * During the time period between the acquisition and release of the resource, the `use` `Effect`
 * value will be executed.
 *
 * If the `release` `Effect` value fails, then the entire `Effect` value will fail, even if the
 * `use` `Effect` value succeeds. If this fail-fast behavior is not desired, errors produced by the
 * `release` `Effect` value can be caught and ignored.
 *
 ...
 * **Details**
 *
 * The finalizer runs when the surrounding scope is closed and receives the `Exit` value used to
 * close the scope.
 *
...
/**
 * Returns an effect that, if this effect _starts_ execution, then the specified `finalizer` is
 * guaranteed to be executed, whether this effect succeeds, fails, or is interrupted.
 *
 * **Details**
 *
 * For use cases that need access to the effect's result, see `onExit`.
 *
 * Finalizers offer very powerful guarantees, but they are low-level, and should generally not be
 * used for releasing resources. For higher-level logic built on `ensuring`, see the
 * `acquireRelease` family of methods.
 *
 ...
 * The provided effect will not be interrupted.
 *
 ...
 * **When to use**
 *
 * Use when cleanup or diagnostics should run only for failures whose full `Cause` is accepted or
 * transformed by a `Filter`, and the finalizer needs the filter's pass value plus the original
 * cause.
 *
 * @since 4.0.0
 * @category Resource management
 ...
 * **When to use**
 *
 * Use when you are building a low-level `Effect` operator that must inspect the source effect's
 * `Exit`, may skip finalization by returning `undefined`, or must choose whether finalization is
 * forced into an uninterruptible region.
 *
 * **Details**
 *
 * This low-level operator preserves the source effect's result unless the finalizer fails. Prefer
 * `onExit` for normal cleanup logic.
 *
 * @since 4.0.0
 * @category Resource management
 ...
 * **When to use**
 *
 * Use when cleanup should run only for `Exit` values selected by a `Filter`, and the cleanup needs
 * the extracted pass value together with the original `Exit`.
 *
 * **Details**
 *
 * `Result.fail` skips cleanup, and `Result.succeed` runs cleanup with the selected value and the
 * original `Exit`.
 *
 * @since 4.0.0
 * @category Resource management
 ...
 * **When to use**
 *
 * Use when you need an expensive or time-consuming operation to be evaluated once and reused by
 * later callers.
 *
 * **Details**
 *
 * This function wraps an effect and ensures that its result is computed only once. Once the result
 * is computed, it is cached, meaning that subsequent evaluations of the same effect will return the
 * cached result without re-executing the logic.
 *
 ...
 * **When to use**
 *
 * Use when you need a costly effect result to be reused for a bounded duration before being
 * recomputed.
 *
 * **Details**
 *
 * This function is used to cache the result of an effect for a specified amount of time. This means
 * that the first time the effect is evaluated, its result is computed and stored.
 *
 * If the effect is evaluated again within the specified `timeToLive`, the cached result will be
 * used, avoiding recomputation.
 *
 * After the specified duration has passed, the cache expires, and the effect will be recomputed
...
/**
 * Creates a cached effect result for a specified duration and allows manual invalidation before
 * expiration.
 *
 * **When to use**
 *
 * Use when an effect result should be cached for a bounded time and callers also need a manual
 * invalidation effect to force recomputation before expiration.
 *
 * **Details**
 *
 * This function behaves similarly to {@link cachedWithTTL} by caching the result of an effect for a
 * specified period of time. However, it introduces an additional feature: it provides an effect
 * that allows you to manually invalidate the cached result before it naturally expires.
 *
 * This gives you more control over the cache, allowing you to refresh the result when needed, even
 * if the original cache has not yet expired.
 *
 * Once the cache is invalidated, the next time the effect is evaluated, the result will be
 * recomputed, and the cache will be refreshed.
 *
...
/**
 * Disables interruption and provides a restore function to restore the interruptible state within
 * the effect.
 *
...
/**
 * Runs an effect in an interruptible region while providing `restore` for locally restoring the
 ...
 * **Details**
 *
 * Each acquisition creates a fresh `AbortController`. Closing the owning scope runs a finalizer
 * that aborts the controller and the effect succeeds with the controller's signal.
 *
 * **Gotchas**
 *
 * The signal is aborted when its owning scope closes, so avoid keeping it for work that outlives
 * that scope.
 *
 * @since 4.0.0
 * @category Interruption
 ...
 * **Details**
 *
 * This function executes an effect repeatedly according to the given schedule. Each repetition
 * occurs after the initial execution of the effect, meaning that the schedule determines the number
 * of additional repetitions. For example, using `Schedule.once` will result in the effect being
 * executed twice (once initially and once as part of the repetition).
 *
 * If the effect succeeds, it is repeated according to the schedule. If it fails, the repetition
 * stops immediately, and the failure is returned.
 *
 * The schedule can also specify delays between repetitions, making it useful for tasks like
 * retrying operations with backoff, periodic execution, or performing a series of dependent
 * actions.
 *
 * You can combine schedules for more advanced repetition logic, such as adding delays, limiting
 * recursions, or dynamically adjusting based on the outcome of each execution.
 *
 * **Gotchas**
 *
 * The source effect is always evaluated once before the schedule is stepped. The schedule controls
 * additional repetitions, not the initial execution.
 *
...
/**
 * Repeats an effect according to a schedule and runs a fallback effect if repetition fails before
 * the schedule completes.
 *
 * **When to use**
 *
 * Use when successful repetitions should follow a schedule, but failures from the repeated effect
 * or schedule need an effectful fallback.
 *
 * **Details**
 *
 * If the repeated effect or schedule step fails, `orElse` receives the failure and the latest
 * schedule metadata when at least one schedule step has run; otherwise it receives `None`. If the
 * schedule completes normally, the returned effect succeeds with the schedule's output.
 *
 ...
 * **When to use**
 *
 * Use when you want to run the repeated effects immediately, with optional concurrency control or
 * result discarding.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to rerun a successful effect according to a `Schedule` when the schedule does not need a
 * custom initial input.
 *
 * **Details**
 *
 * The schedule is first stepped with `undefined`. After each successful execution, the effect's
 * success value is fed to the schedule to decide whether to run again. The returned effect fails if
 * the effect or schedule fails, and otherwise succeeds with the schedule output when the schedule
 * completes.
 *
...
/**
 * Runs an effect repeatedly according to a schedule that is initialized with a specific schedule
 * input.
 *
 * **Details**
 *
 * `initial` is passed to the schedule before the first execution, not to the effect itself. After
 * each successful execution, the effect's success value is fed back into the schedule to decide
 * whether to continue. The returned effect succeeds with the schedule output when the schedule
 * completes and fails if the effect or schedule fails.
 *
 ...
 * **Details**
 *
 * When `enabled` is `false`, spans created inside the effect are not registered with the current
 * tracer and do not propagate as normal trace parents.
 *
 ...
 * **Details**
 *
 * The effect succeeds with either a local span or external span when one is present, and fails with
 * `NoSuchElementError` when no parent span is available.
 *
 ...
 * **Details**
 *
 * These annotations are applied to spans created inside the context, such as spans created by
 * `withSpan`, `useSpan`, or `makeSpan`.
 *
 ...
 * **Details**
 *
 * These links are attached to spans created inside the context. Span links connect related spans
 * without making one span the parent of another.
 *
 ...
 * **Details**
 *
 * This is useful for connecting spans that are related but not in a direct parent-child
 * relationship. For example, you might want to link spans from parallel operations or connect spans
 ...
 * **Details**
 *
 * The span is not added to the current span stack and is not ended automatically. Use `withSpan`,
 * `useSpan`, or `makeSpanScoped` when the span should be installed as context or closed
 * automatically.
 *
 ...
 * **When to use**
 *
 * Use when you already have a `Context` and need to enqueue a request outside an `Effect` while
 * receiving completion through `onExit`.
 *
 * **Details**
 *
...
/**
 * Returns an effect that forks this effect into its own separate fiber, returning the fiber
 * immediately, without waiting for it to begin executing the effect.
 *
 * **Details**
 *
 * You can use the `forkChild` method whenever you want to execute an effect in a new fiber,
 * concurrently and without "blocking" the fiber executing other effects. Using fibers can be
 * tricky, so instead of using this method directly, consider other higher-level methods, such as
 * `raceWith`, `zipPar`, and so forth.
 *
 * The fiber returned by this method has methods to interrupt the fiber and to wait for it to finish
 * executing the effect. See `Fiber` for more information.
 *
 * Whenever you use this method to launch a new fiber, the new fiber is attached to the parent
 * fiber's scope. This means when the parent fiber terminates, the child fiber will be terminated as
 * well, ensuring that no fibers leak. This behavior is called "auto supervision", and if this
...
/**
 * Forks the effect into a new fiber attached to the global scope. Because the new fiber is attached
 * to the global scope, when the fiber executing the returned effect terminates, the forked fiber
 ...
 * **When to use**
 *
 * Use to let an effect start child work concurrently while still delaying its own completion until
 * that child work is done.
 *
 * **Gotchas**
 *
...
/**
 * Configuration options for running Effect programs, providing control over interruption and
 * scheduling behavior.
 *
 * **When to use**
 *
 * Use to pass cancellation, scheduler, interruptibility, and fiber-start hooks when running an
 * `Effect` at a program boundary.
 *
 * **Details**
 *
 * `signal` interrupts the fiber, `scheduler` provides the scheduler service, `uninterruptible`
 * starts the fiber uninterruptibly, and `onFiberStart` receives the created fiber.
 *
 * @since 4.0.0
 * @category Running
 ...
 * **When to use**
 *
 * Use when an effect still requires services, you already have a `Context`, and you want a
 * background fiber.
 *
...
/**
 * Forks an effect with the provided services, registers `onExit` as a fiber observer, and returns
 * an interruptor.
 *
 * **When to use**
 *
 * Use when embedding an effect into callback-style code with explicit services and a synchronous
 * interruptor.
 *
 * **Details**
 *
...
/**
 * Runs an effect asynchronously, registering `onExit` as a fiber observer and returning an
 * interruptor.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to execute an effect and work with the result using `Promise` syntax, typically
 * for compatibility with other promise-based code.
 *
 * If the effect succeeds, the promise will resolve with the result. If the effect fails, the
...
/**
 * Runs an effect and returns a `Promise` that resolves to an `Exit`, which represents the outcome
 * (success or failure) of the effect.
 *
 * **When to use**
 *
 * Use when you need to determine if an effect succeeded or failed, including any defects, and you
 * want to work with a `Promise`.
 *
 * **Details**
 *
 * The `Exit` type represents the result of the effect. Successful effects are wrapped in `Success`,
 * and failed effects are wrapped in `Failure` with a `Cause`.
 *
 ...
 * **When to use**
 *
 * Use when you already have a `Context` and need Promise interop that preserves success and failure
 * as an `Exit`.
 *
 ...
 * **Details**
 *
 * If the effect fails, dies, is interrupted, or performs asynchronous work, `runSync` throws a
 * `FiberFailure` instead of returning a value. Use `runSyncExit` when you want the failure captured
 * as an `Exit`.
 *
 ...
 * **When to use**
 *
 * Use when you already have a `Context`, the effect is known to complete synchronously, and
...
/**
 * Runs an effect synchronously and captures the outcome safely as an `Exit` type, which represents
 * the outcome (success or failure) of the effect.
 *
 * **When to use**
 *
 * Use to find out whether an effect succeeded or failed, including any defects, without dealing
 ...
 * and failed effects are wrapped in `Failure` with a `Cause`.
 *
 * If the effect contains asynchronous operations, `runSyncExit` will return an `Failure` with a
 * `Die` cause, indicating that the effect cannot be resolved synchronously.
 *
 ...
 * **When to use**
 *
 * Use when you already have a `Context` and need a synchronous `Exit` instead of throwing on
 * failure.
 *
...
export declare namespace fn {
   ...
   * **When to use**
   *
   * Use when you need to annotate the return type of a generator body while keeping the produced
   * function's `Effect` return type inferred.
   *
 ...
 * **When to use**
 *
 * Use when you are defining a reusable Effect function whose implementation would otherwise be a
 * normal function returning {@link gen}, especially when tracing spans or stack-frame capture are
 * not needed.
 *
 * **Details**
 *
 * Compared to a plain function that returns {@link gen}, `Effect.fnUntraced` reuses the generator
 * body instead of allocating a fresh generator closure around the arguments on every call. It does
 * not record an Effect stack-frame boundary and does not create tracing spans. Use {@link fn} when
 * you need those stack frames or spans. Additional arguments after the generator body act like
 * `pipe` transforms: each transform receives the previous result and the original function
 * arguments. Annotate the generator return type with `Effect.fn.Return<A, E, R>` when the produced
 * `Effect` type needs to be stated explicitly.
 *
 ...
 *
 * Use when you are defining a reusable Effect function whose implementation would otherwise be a
 * normal function returning {@link gen}, and you want tracing spans or stack-frame capture.
 *
 * **Details**
 *
 * Compared to a plain function that returns {@link gen}, `Effect.fn` reuses the generator body
 * instead of allocating a fresh generator closure around the arguments on every call. Call
 * `Effect.fn(body, ...)` for a generic stack-frame boundary without creating a span. Call
 * `Effect.fn("operationName", options?)(body, ...)` when that boundary should have a readable
 * operation name and the returned `Effect` should create a tracing span when run.
 * {@link SpanOptionsNoTrace} configures span metadata such as attributes, links, parent or root
 * selection, kind, sampling, and log level. Additional arguments after the generator body act like
 * `pipe` transforms: each transform receives the previous result and the original function
 * arguments. When those transforms return an `Effect`, the returned effect includes stack-frame
 * metadata and, for the named form, a tracing span. Generator bodies may declare a `this`
 * parameter; pass `{ self }` before the body to bind `this` when the function is created.
 *
...
/**
 * Retrieves the `Clock` service from the context and provides it to the specified effectful
 * function.
 *
 ...
 * **Details**
 *
 * If no level is provided, the logger uses the fiber's current log level and extracts any `Cause`
 * values from the message list.
 *
 ...
 * **Details**
 *
 * This differs from `annotateLogs`, which only annotates a specific effect. `annotateLogsScoped`
 * updates annotations for the entire current `Scope` and restores the previous annotations when the
 * scope closes.
 *
 ...
 * **Details**
 *
 * Also accepts an optional function which can be used to map the `Exit` value of the `Effect` into
 * a valid `Input` for the `Metric`.
 *
 ...
 * **Details**
 *
 * Also accepts an optional function which can be used to map the success value of the `Effect` into
 * a valid `Input` for the `Metric`.
 *
 ...
 * **Details**
 *
 * Also accepts an optional function which can be used to map the error value of the `Effect` into a
 * valid `Input` for the `Metric`.
 *
...
/**
 * Updates the provided `Metric` every time the wrapped `Effect` fails with an **unexpected** error
 * (i.e. a defect).
 *
 * **Details**
 *
 * Also accepts an optional function which can be used to map the defect value of the `Effect` into
 * a valid `Input` for the `Metric`.
 *
...
/**
 * Updates the provided `Metric` with the `Duration` of time (in nanoseconds) that the wrapped
 * `Effect` took to complete.
 *
 * **Details**
 *
 * Also accepts an optional function which can be used to map the `Duration` that the wrapped
 * `Effect` took to complete into a valid `Input` for the `Metric`.
 *
 ...
 * **Details**
 *
 * It includes a journal that stores non-committed changes to `TxRef` values and a retry flag that
 * records whether the transaction should be retried.
 *
...
/**
 * Defines a transaction boundary. Transactions are "all or nothing" with respect to changes made to
 * transactional values (i.e. TxRef) that occur within the transaction body.
 *
 * **Details**
 *
 * If called inside an active transaction, `tx` composes with the current transaction and reuses its
 * journal and retry state instead of creating a nested boundary.
 *
 * Effect transactions are optimistic with retry. A transaction is retried when its body explicitly
 * calls `Effect.txRetry` and any accessed transactional value changes, or when any accessed
 * transactional value changes because a different transaction commits before the current one.
 *
 ...
 * **Details**
 *
 * NOTE: the transaction retries on any change to transactional values (i.e. TxRef) accessed in its
 * body.
 *
 ...
 * **Details**
 *
 * The original function is called with the supplied arguments plus a final callback. A non-null
 * callback error fails the returned effect, while a successful callback value becomes the effect
 * success. Use `onError` to map callback errors and `onSyncError` to turn synchronous throws into
 * typed failures; otherwise synchronous throws become defects.
 *
 ...
 * **When to use**
 *
 * Use when an already-resolved effect should apply a success transformation immediately while
 * pending effects still use regular mapping.
 *
 * **Details**
 *
 * Success effects apply the mapping function immediately. Failure effects pass through unchanged,
 * and pending effects fall back to regular `map` behavior.
 *
 ...
 * **When to use**
 *
 * Use when an already-resolved failed effect should apply an error transformation immediately while
 * pending effects still use regular error mapping.
 *
 * **Details**
 *
 * Success effects pass through unchanged because there is no error to transform. Failure effects
 * apply the mapping function immediately, and pending effects fall back to regular `mapError`
 * behavior.
 *
 ...
 * **When to use**
 *
 * Use when an already-resolved effect should transform either success or failure immediately while
 * pending effects still use regular channel mapping.
 *
 * **Details**
 *
 * Success effects apply `onSuccess` immediately, and failure effects apply `onFailure` immediately.
 * Pending effects fall back to regular `mapBoth` behavior.
 *
 ...
 * **When to use**
 *
 * Use when an already-resolved successful effect should bind immediately to the next effect while
 * pending effects still use regular flat mapping.
 *
 * **Details**
 *
 * Success effects apply the flatMap function immediately. Failure effects pass through unchanged,
 * and pending effects fall back to regular `flatMap` behavior.
 *
 ...
 * **When to use**
 *
 * Use when an already-resolved failed effect should recover immediately while pending effects still
 * use regular error recovery.
 *
 * **Details**
 *
 * Success effects pass through unchanged because there is no error to catch. Failure effects apply
 * the catch function immediately, and pending effects fall back to regular `catch` behavior.
 *
 ...
 * **Details**
 *
 * Executes generator functions eagerly when all yielded effects are synchronous, stopping at the
 * first async effect and deferring to normal execution.
 *
