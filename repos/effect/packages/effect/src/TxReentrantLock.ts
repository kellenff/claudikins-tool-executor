 ...
 *
 * A `TxReentrantLock` lets many fibers hold read locks at the same time, or one fiber hold a write
 * lock for exclusive access. Lock ownership is tracked by fiber, so a fiber that already holds the
 * lock can acquire it again and later release each acquisition. Attempts that cannot proceed retry
 * transactionally until the lock becomes available. This module includes manual, scoped, and
...
/**
 * A TxReentrantLock provides a transactional read/write lock with reentrant semantics. Multiple
 * readers can hold the lock concurrently, or a single writer can hold exclusive access. A fiber
 * holding the write lock may acquire additional read/write locks (reentrancy).
 *
...
/**
 * Acquires a read lock. Blocks if another fiber holds the write lock. If the current fiber already
 * holds the write lock, the read lock is granted (reentrancy). Returns the current number of read
 * locks held by this fiber.
 *
 ...
 * **Details**
 *
 * Blocks if any other fiber holds a read or write lock. If the current fiber already holds the
 * write lock, the count is incremented. If the current fiber holds a read lock, the write lock is
 * granted as an upgrade.
 *
...
/**
 * Acquires a read lock for the duration of the scope. The lock is automatically released when the
 * scope closes.
 *
...
/**
 * Acquires a write lock for the duration of the scope. The lock is automatically released when the
 * scope closes.
 *
...
/**
 * Runs the provided effect while holding a read lock. The lock is automatically released after the
 * effect completes, fails, or is interrupted.
 *
...
/**
 * Runs the provided effect while holding a write lock. The lock is automatically released after the
 * effect completes, fails, or is interrupted.
 *
 ...
 * **When to use**
 *
 * Use when you need to run an effect with exclusive write access through a `TxReentrantLock` and
 * prefer the concise lock helper.
 *
