/**
 * The `Pipeable` module defines the shared interface and implementation helpers for values that
 * support Effect-style method chaining with `.pipe(...)`.
 *
 * A `Pipeable` value can pass itself through a sequence of unary functions from left to right, so
 * code can be written as `value.pipe(f, g, h)` instead of deeply nesting calls. This is the method
 * form used by many Effect data types to compose transformations, validations, and effectful
 * operations while keeping the original value as the starting point of the pipeline.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * Calling `value.pipe(f, g, h)` passes the value through each function from left to right,
 * returning the final result. Many Effect data types implement this so operations can be chained
 ...
 * **Details**
 *
 * This helper is intended for implementing `Pipeable.pipe` methods that receive JavaScript's
 * `arguments` object. With no functions it returns the original value; otherwise it feeds each
 ...
 * **When to use**
 *
 * Use when classes or object prototypes can reuse this value when they need the standard pipe
 * implementation backed by `pipeArguments`.
 *
 * @since 3.15.0
 * @category Prototypes
 */
 ...
 * **When to use**
 *
 * Use when you need to define a class that supports Effect-style method chaining through
 * `.pipe(...)`.
 *
 * @since 3.15.0
 ...
 * **When to use**
 *
 * Use as the constructor-side type when a class value should be known to create instances that
 * support Effect-style method chaining with `.pipe(...)`.
 *
 * @since 3.15.0
 * @category Models
 ...
 * **When to use**
 *
 * Use to add pipe support to an existing class without extending a base class or modifying its
 * prototype.
 *
 * **Details**
 *
 * The original constructor and instance members are preserved, and the added method delegates to
 * `pipeArguments`.
 *
 * @since 4.0.0
 * @category Constructors
