 ...
 *
 * A cache stores successful and failed lookup results, shares an in-progress lookup when multiple
 * callers request the same missing key, and limits entries by capacity and optional time-to-live
 * rules. This module includes helpers for reading, setting, refreshing, invalidating, and
...
/**
 * A cache interface that provides a mutable key-value store with automatic TTL management, capacity
 * limits, and lookup functions for cache misses.
 *
 ...
 *     capacity: 10,
 *     lookup: (key: string) =>
 *       key === "error" ? Effect.fail("Lookup failed") : Effect.succeed(key.length),
 *   });
 *
...
/**
 * Represents a low-level cache entry containing a deferred lookup result and an optional expiration
 * timestamp.
 *
 * **When to use**
 *
 * Use when inspecting a `Cache`'s low-level map and you need the stored deferred lookup result or
 ...
 * **When to use**
 *
 * Use when you need different cache entry lifetimes based on the lookup result or key
 * characteristics.
 *
 * **Details**
 *
 * The timeToLive function receives both the exit result and the key, allowing for flexible TTL
 * policies based on success/failure state and key characteristics.
 *
 ...
 * **Details**
 *
 * This is the basic cache constructor where all entries share the same TTL. The lookup function
 * will be called when a key is not found or has expired.
 *
 * **Example** (Creating a basic cache)
 *
 * ```ts
 ...
 * **Details**
 *
 * Concurrent `get` calls for the same missing key share the same pending lookup. The cache stores
 * the lookup `Exit`, so failed lookups are cached and will fail again until the entry expires, is
 * invalidated, or is refreshed.
 *
...
class EntryImpl<A, E> implements Entry<A, E> {
  ...
  constructor(parent: Fiber.Fiber<unknown, unknown>, valueEffect: Effect.Effect<A, E, any>) {
 ...
 * **Details**
 *
 * Returns `Option.none()` when the key is missing or expired, and `Option.some` when a cached
 * lookup has succeeded. If the entry is still pending, waits for it to complete. If the cached or
 * pending lookup fails, this effect fails with the same error.
 *
 ...
 * **Details**
 *
 * This checks only an existing non-expired entry. It returns `Option.some` when the entry has
 * already resolved successfully, and `Option.none` for missing, expired, failed, or still-pending
 * entries.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Sets the value associated with the specified key in the cache. This will overwrite any existing
 * value for that key, skipping the lookup function.
 *
 ...
 * **Details**
 *
 * This checks for an existing non-expired entry without invoking the cache lookup function. Expired
...
/**
 * Retrieves all key-value pairs from the cache as an iterable. This function only returns entries
 * with successfully resolved values, filtering out any failed lookups or expired entries.
 *
 * **Gotchas**
 *
