/**
 * Small helpers for schema patterns that are too specialized for the main `Schema` module. The
 * current helper builds a schema for an existing class: the encoded input is checked with a struct
 * schema, decoding calls the class constructor with the decoded properties, and the final value
 * remains an instance of that class.
 *
 * @since 4.0.0
 */
...
/**
 * Builds an experimental schema for instances of a native class using a struct schema as the
 * encoded representation.
 *
 * **When to use**
 *
 * Use when you need a schema for an existing native class while keeping a `Struct` schema as its
 * encoded representation.
 *
 * **Details**
 *
 * Decoding constructs `new constructor(props)` from the encoded fields. Encoding uses the instance
 * as the encoded shape, so the class should expose properties compatible with the provided encoding
 * schema.
 *
 * @since 4.0.0
 * @category Schemas
