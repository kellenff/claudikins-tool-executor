 ...
 *
 * A `Layer<ROut, E, RIn>` describes how to acquire one or more services, which services are
 * required to build them, and which errors can occur during acquisition. Layers can manage scoped
 * resources, memoize shared services, combine with other layers, provide services to effects or
 * streams, and attach error handling, tracing, or lifecycle hooks.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use to model construction of application services for dependency injection, especially when
 * services have dependencies, can fail during construction, or need scoped setup and release.
 *
 * **Details**
 *
 * A `Layer<ROut, E, RIn>` represents `ROut` as the services this layer provides, `E` as the
 * possible errors during layer construction, and `RIn` as the services this layer requires as
 * dependencies.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **Details**
 *
 * This is used by Effect's pipe and unification machinery to preserve the provided services, error,
 * and requirements of a `Layer`.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use to constrain generic parameters or layer collections to any `Layer` value while preserving
 * its provided, error, and required service types for inference.
 *
 * **Details**
 *
 * This interface is used to constrain generic types to `Layer` values without specifying exact type
 * parameters.
 *
 * @since 3.9.0
 ...
 * **When to use**
 *
 * Use to derive the dependency requirements of a generic or inferred `Layer` without restating its
 * `RIn` type parameter.
 *
 * @since 4.0.0
 * @category Utility types
 ...
 * **When to use**
 *
 * Use to derive a layer construction error type for helper types, wrappers, or APIs that preserve a
 * layer failure channel.
 *
 * @since 2.0.0
 * @category Utility types
 * @see {@link Success} for extracting the services provided by the same `Layer`
 ...
 * **When to use**
 *
 * Use to derive the services provided by an existing or generic `Layer` without restating its
 * `ROut` type parameter.
 *
 * @since 2.0.0
 * @category Utility types
 ...
 * **Details**
 *
 * The `MemoMap` prevents duplicate construction of the same layer instance, enabling efficient
 * resource sharing across layer dependencies.
 *
 ...
 * **Details**
 *
 * The function receives a `MemoMap` for memoization and a `Scope` for resource management. A child
 * scope is created, and if the build fails, the child scope is closed.
 *
...
/**
 * Constructs a `Layer` from a function that uses a `MemoMap` and `Scope` to build the layer, with
 * automatic memoization.
 *
 * **Details**
 *
 * This is similar to `fromBuild` but provides automatic memoization of the layer construction. The
 * layer will be memoized based on the provided `MemoMap`.
 *
...
/**
 * Constructs a child `MemoMap` synchronously, allowing it to reuse layers already memoized in the
 * parent while isolating any new layer allocations to the child map.
 *
 * **When to use**
 *
 * Use to synchronously fork a memo map for manual layer building when child builds should see
 * parent memoized layers without writing newly built layers back to the parent.
 *
 * @since 4.0.0
 * @category Memo map
...
/**
 * Constructs a child `MemoMap` effectfully, allowing it to reuse layers already memoized in the
 * parent while isolating any new layer allocations to the child map.
 *
 * **When to use**
 *
 * Use when a layer build should inherit already memoized layers from an existing `MemoMap` while
 * keeping newly memoized layers out of the parent map.
 *
 * @since 4.0.0
 * @category Memo map
 ...
 * **When to use**
 *
 * Use when building custom layer operations that need to access the current memoization map from
 * the fiber context.
 *
 * **Details**
 *
 * This service wraps a `MemoMap` as a `Context.Service`, making it available for dependency
 * injection during layer construction.
 *
 * @since 3.13.0
 * @category Models
...
/**
 * Builds a layer into an `Effect` value, using the specified `MemoMap` to memoize the layer
 * construction.
 *
 ...
 * **Details**
 *
 * Resources created by the layer are released when the supplied scope is closed, unless a resource
 * extends its own scope.
 *
...
export const buildWithScope: {
  ...
  <RIn, E, ROut>(
    ...
    core.withFiber((fiber) =>
      buildWithMemoMap(self, CurrentMemoMap.forkOrCreate(fiber.context), scope),
    ),
);
 ...
 * **When to use**
 *
 * Use when you need a `Layer` that provides a service from an already constructed implementation
 * without effectful acquisition.
 *
 ...
 * **When to use**
 *
 * Use when you need a `Layer` built from an existing `Context`, including when you need to provide
 * multiple services at once.
 *
 * **Details**
 *
 * This is a more general version of `succeed` that allows you to provide multiple services at once
 * through a `Context`.
 *
...
/**
 * An empty layer that provides no services, cannot fail, has no requirements, and performs no
 * construction or finalization work.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use when you need a `Layer` that provides one service whose value is created synchronously, but
 * creation should be deferred until the layer is built.
 *
 * **Details**
 *
 * This is a lazy version of `succeed` where the service value is computed synchronously only when
 * the layer is built.
 *
 ...
 * **When to use**
 *
 * Use when you need a `Layer` that creates multiple services synchronously but defers that work
 * until the layer is built.
 *
 * **Details**
 *
 * This is a lazy version of `succeedContext` where the `Context` is computed synchronously only
 ...
 * **When to use**
 *
 * Use when you need to construct a `Layer`-provided service with an `Effect`, dependencies, or
 * scoped resource acquisition.
 *
 * **Details**
 *
 * This allows you to create a `Layer` from an `Effect` that produces a service. The `Effect` is
 * executed in the scope of the layer, allowing for proper resource management.
 *
 ...
 * **Details**
 *
 * This allows you to create a `Layer` from an effectful computation that returns multiple services.
 * The `Effect` is executed in the scope of the layer.
 *
 ...
 * **Details**
 *
 * The factory is evaluated only when the suspended layer is first built, and the result is memoized
 * with normal layer sharing semantics.
 *
 ...
 * **Details**
 *
 * The resulting Layer will have the combined error and dependency types from both the outer Effect
 * and the inner Layer.
 *
...
/**
 * Combines all the provided layers concurrently, creating a new layer with merged input, error, and
 * output types.
 *
 * **When to use**
 *
 ...
 *
 * If multiple merged layers depend on the same layer value, that dependency is shared by default.
 * Reuse a named layer value when you want services to share the same resource, such as one database
 * pool.
 *
...
/**
 * Merges this layer with another layer concurrently, producing a new layer with combined input,
 * error, and output types.
 *
 * **When to use**
 *
 * Use to combine an existing `Layer` with another `Layer` or an array of layers while preserving
 * pipeline style.
 *
 * **Details**
 *
 * This is a binary version of `mergeAll` that merges exactly two layers or one layer with an array
 * of layers. The layers are built concurrently and their outputs are combined.
 *
...
export const merge: {
  ...
  2,
  (self: Layer<any, any, any>, that: Layer<any, any, any> | ReadonlyArray<Layer<any, any, any>>) =>
...
/**
 * Feeds the output services of the dependency layer into the requirements of this layer, returning
 * a layer that only provides the services from this layer.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * In `serviceLayer.pipe(Layer.provide(dependencyLayer))`, the dependency layer is built first and
 * is used to satisfy the requirements of `serviceLayer`.
 *
...
/**
 * Feeds the output services of the dependency layer into the requirements of this layer, returning
 * a layer that provides both sets of services.
 *
 * **When to use**
 *
 * Use when you need to compose `Layer`s while keeping both the constructed service and the
 * dependency used to build it available.
 *
 * **Details**
 *
 ...
 *     const dbLayer = Layer.succeed(Database, {
 *       query: Effect.fn("Database.query")((sql: string) =>
 *         Effect.succeed(`Querying ${config.dbUrl}: ${sql}`),
 *       ),
 *     });
 *
 ...
 * **When to use**
 *
 * Use to run an effectful observation after a layer has been built successfully, such as logging or
 * metrics, without changing the services the layer provides.
 *
 * **Details**
 *
 * The callback receives the services produced by this layer. Its result is discarded, and the
 * original layer output is preserved.
 *
 * @since 2.0.0
 * @category Sequencing
 ...
 * **When to use**
 *
 * Use to run logging, metrics, or other effects when layer construction fails while preserving the
 * original typed error.
 *
 * **Details**
 *
 * The callback receives the typed error. If the callback succeeds, the layer still fails with the
 * original error; if the callback fails, that failure is added to the layer's error type.
 *
 * @since 2.0.0
 * @category Sequencing
 ...
 * **Details**
 *
 * The callback receives the layer's `Cause`, so it can inspect typed errors, defects, and
 * interruption information. If the callback succeeds, the layer fails again with the original
 * cause; if the callback fails, that failure is added to the layer's error type.
 *
 * @since 4.0.0
 * @category Sequencing
  ...
  <RIn, E, XE extends E, ROut, RIn2, E2, X>(
    ...
    fromBuild((memoMap, scope) =>
      internalEffect.catchCause(self.build(memoMap, scope), (cause) =>
        Scope.provide(
          internalEffect.andThen(f(cause as Cause.Cause<XE>), internalEffect.failCause(cause)),
          scope,
        ),
      ),
    ),
 ...
 * **Details**
 *
 * Use this only when failures should be treated as unrecoverable defects rather than typed errors
 * that callers can handle.
 *
...
export const catchTag: {
  ...
  >(
    ...
    k: K,
    f: (
      e: Types.ExtractTag<E, K extends NonEmptyReadonlyArray<string> ? K[number] : K>,
    ) => Layer<ROut2, E2, RIn2>,
  ): Layer<
    ROut & ROut2,
    E2 | Types.ExcludeTag<E, K extends NonEmptyReadonlyArray<string> ? K[number] : K>,
 ...
 * **When to use**
 *
 * Use when you need `Layer` recovery to inspect more than the typed error, such as defects or
 * interruption information.
 *
 * **Details**
 *
 * The handler receives the full `Cause` of the failed layer, including typed errors, unexpected
 * defects, and interruption information, and returns the fallback layer to build instead.
 * Finalizers for resources acquired by the failed layer are still run before the fallback layer is
 * acquired.
 *
 ...
 * **Details**
 *
 * This function modifies the existing implementation of a service in the context. It retrieves the
 * current service, applies the provided transformation function `f`, and replaces the old service
 ...
 * **When to use**
 *
 * Use when you need two parts of an application to receive separate instances of a resource, such
 * as two independent client sessions.
 *
 * **Gotchas**
 *
 * Do not use it just to work around confusing composition. By default, sharing the same layer value
 ...
 * **Details**
 *
 * When the returned effect is interrupted, the layer scope is closed and all finalizers registered
 * during layer acquisition are run.
 *
 ...
 * **When to use**
 *
 * Use to type partial test service implementations where only exercised effectful members are
 * stubbed.
 *
 * **Details**
 *
 * This type makes `Effect`, `Stream`, and `Channel` values and functions returning them optional,
 * while keeping non-effectful properties required. This allows you to provide only the methods you
 * need to test while leaving others unimplemented.
 *
 * @since 3.17.0
 * @category Testing
...
/**
 * Creates a mock layer for testing purposes. You can provide a partial implementation of the
 * service. Any missing members that are `Effect`s, `Stream`s, `Channel`s, or functions returning
 * them will fail with an unimplemented defect when used.
 *
 * **Details**
 *
 * Missing members are represented by a value that can be used as an `Effect`, `Stream`, `Channel`,
 * or as a function returning an `Effect`. This lets the mock preserve the shape of common service
 * methods while still failing loudly when an unimplemented member is exercised.
 *
 ...
 * **Details**
 *
 * This function provides compile-time type checking to ensure that the success value of a layer
 * conforms to a specific type constraint.
 *
 ...
 * **Details**
 *
 * This function provides compile-time type checking to ensure that the error type of a layer
 * conforms to a specific type constraint.
 *
 ...
 * **Details**
 *
 * This function provides compile-time type checking to ensure that the requirements type of a layer
 * conforms to a specific type constraint.
 *
 ...
 * **When to use**
 *
 * Use to configure tracing metadata, stack trace capture, and `onEnd` finalization for spans
 * created by `Layer.span` and `Layer.withSpan` during layer construction.
 *
 * **Details**
 *
 * Extends `Tracer.SpanOptions` with `onEnd`, which runs when the layer span ends as the layer scope
 * closes.
 *
 * @since 4.0.0
 * @category Options
...
export interface SpanOptions extends Tracer.SpanOptions {
  /**
   * Runs when the span associated with the layer ends, which happens when the layer scope is
   * closed.
   */
  readonly onEnd?:
 ...
 * **Details**
 *
 * This allows you to create a traced scope for layer construction, making all operations within the
 * layer constructor part of the same trace span. The span is automatically ended when the layer's
 * scope is closed. If `onEnd` is provided, it receives the span and the layer scope's exit value
 * when the span ends.
 *
 ...
 * **Details**
 *
 * The supplied span is made available through `Tracer.ParentSpan` for layers that are built with
 * this layer. This API does not create, end, or close the span; the caller remains responsible for
 * the span's lifetime.
 *
...
/**
 * Wraps a `Layer` with a new tracing span, making all operations in the layer constructor part of
 * the named trace span.
 *
 * **Details**
 *
 * This creates a new span for the layer's construction and execution. The span is automatically
 * ended when the layer's scope is closed. This is useful for tracking the lifecycle and performance
 * of layer initialization.
 *
 ...
 * **Details**
 *
 * Use this to attach layer construction to an existing trace hierarchy. This API does not create or
 * end the supplied parent span.
 *
 * When the supplied span is a native `Span`, layer construction also receives diagnostic
 * information that helps associate failures with the layer call site. External spans are only
 * installed as the parent span and do not add this diagnostic call-site information.
 *
