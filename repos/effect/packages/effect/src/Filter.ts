 ...
 *
 * A `Filter<Input, Pass, Fail>` receives an input and returns a `Result`. Success means the value
 * passed the filter, and failure means the value was filtered out. Filters may also narrow or
 * transform the passing value. This module includes constructors from predicates, options, and
 * effects, built-in filters for common JavaScript values and tags, helpers for combining filters,
 ...
 * **Details**
 *
 * A filter takes an input value and either returns a boxed pass value or the special `fail` type to
 * indicate the value should be filtered out.
 *
 ...
 * **Details**
 *
 * Similar to a regular `Filter`, but the filtering operation itself can be effectful, allowing for
 * asynchronous operations, error handling, and dependency injection.
 *
 ...
 * **Details**
 *
 * This is the primary constructor for creating custom filters. The function should return either
 * `Result.succeed(value)` or `Result.fail(value)`.
 *
 ...
 * **Details**
 *
 * This constructor is used when the filtering operation needs to perform effectful computations,
 * such as async operations, error handling, or accessing services from the environment.
 *
 ...
 * **Details**
 *
 * This is a convenient way to create filters from boolean-returning functions. When the predicate
 * returns true, the input value is passed through unchanged. When it returns false, the `fail` type
 * is returned.
 *
...
/**
 * Creates a `Filter` from a function that returns an `Option`; `Some(value)` passes with `value`,
 * and `None` fails with the original input.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **When to use**
 *
 * Use to reuse a `Filter` with APIs that accept only boolean predicates when the pass and fail
...
/**
 * Creates a `Filter` that passes only values strictly equal to the specified value using JavaScript
 * `===` comparison.
 *
 * **When to use**
 *
 * Use when you need a `Filter` that accepts only the exact primitive value or object reference
 * using JavaScript strict equality in a `Filter` / `Result` pipeline.
 *
 * **Gotchas**
 *
 * `NaN` never passes, even when the expected value is `NaN`, and objects pass only when they are
 * the same reference.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Creates a `Filter` that passes inputs whose `has(key)` method returns `true` for the specified
 * key.
 *
 * **When to use**
 *
 * Use to keep inputs that expose a `has` method, such as `Set` or `Map`, when they contain a
 * required key.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to narrow unknown input to values created by a specific JavaScript constructor while keeping
 * the result in the `Filter` / `Result` pipeline.
 *
 * **Details**
 *
 * The filter succeeds when the input satisfies `instanceof constructor`. Otherwise it fails with
 * the original input.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when accepting an unknown input only if it is already a boolean and you want a `Filter`
 * result rather than a plain predicate result.
 *
 * **Details**
 *
 * Implemented with `fromPredicate(Predicate.isBoolean)`, so `true` and `false` succeed and
 * non-booleans fail with the original input.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to keep primitive big integer values from unknown input while staying in the composable
 * `Filter` / `Result` pipeline.
 *
 * **Details**
 *
 * Implemented with `fromPredicate(Predicate.isBigInt)`, so values where `typeof input === "bigint"`
 * succeed and all other inputs fail with the original input.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when you need to narrow unknown input to JavaScript `Date` instances with a reusable
 * `Filter`.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * The check uses `instanceof Date`, so invalid `Date` objects still pass; the filter does not
 ...
 * **When to use**
 *
 * Use to keep only the matching member of a `_tag`-discriminated union while staying in a
 * composable `Filter` / `Result` pipeline.
 *
 * **Details**
 *
 * The filter succeeds when `Predicate.isTagged(input, tag)` returns `true`. Otherwise it fails with
 * the original input.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use to accept inputs that are structurally equal to a known expected value while staying in a
 * composable `Filter` / `Result` pipeline.
 *
 * **Details**
 *
 * Delegates to `Equal.equals`. On success it returns `Result.succeed(value)`; on failure it returns
 * `Result.fail(input)`.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * Both filters must succeed (not return `fail`) for the combination to succeed. If both filters
 * pass, their outputs are combined using the provided function.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * Both filters must succeed for the combination to succeed. If both pass, their outputs are
...
/**
 * Composes two filters sequentially, passing the successful output of the first filter to the
 * second.
 *
 * **Details**
 *
 * If either filter fails, the returned filter fails with the original input instead of the
 * intermediate failure value.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Converts a `Filter` into a function that returns `Some` for passed values and `None` for
 * filtered-out values.
 *
 * **When to use**
 *
 * Use when adapting a `Filter` to `Option`-based code where passed values become `Some` and
 * filtered-out inputs become `None`.
 *
 * @since 4.0.0
 * @category Converting
 ...
 * **When to use**
 *
 * Use to adapt a `Filter` to APIs that expect a plain function returning `Result`, while preserving
 * both the pass value and the failure value.
 *
 * @since 4.0.0
 * @category Converting
