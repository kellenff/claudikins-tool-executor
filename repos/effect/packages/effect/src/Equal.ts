 ...
 *
 * `equals` compares primitives, arrays, plain objects, maps, sets, dates, regular expressions, and
 * values that implement the `Equal` interface. This module also defines the equality symbol,
 * guards, adapters, map and set comparison builders, and helpers for marking objects that should
 ...
 * **When to use**
 *
 * Use when you implement custom equality and need the computed property key for the equality
 * method.
 *
 * **Details**
 *
 ...
 * - When the default structural comparison is too broad or too narrow for your type.
 *
 * **Details**
 *
 * Any object that implements both `[Equal.symbol]` (equality) and `[Hash.symbol]` (hashing) is
 * recognized by {@link equals} and by hash-based collections such as `HashMap` and `HashSet`.
 *
 * - Extends `Hash.Hash`, so implementors **must** also provide `[Hash.symbol]`.
 * - The hash contract: if `a[Equal.symbol](b)` returns `true`, then `Hash.hash(a)` must equal
 *   `Hash.hash(b)`.
 * - {@link equals} delegates to this method when both operands implement it. If only one operand
 *   implements `Equal`, they are considered unequal.
 *
 ...
 * **Details**
 *
 * Returns a `boolean` and never throws. Primitives are compared by value, and `NaN` equals `NaN`.
 * Objects implementing `Equal` delegate to their `[Equal.symbol]` method; if only one operand
 * implements `Equal`, the result is `false`.
 *
 * Dates compare by ISO string, RegExps compare by string representation, arrays compare
 * element-by-element, Maps and Sets compare entries order-independently, and plain objects compare
 * enumerable keys recursively. Functions without an `Equal` implementation compare by reference.
 * Circular references are handled when both structures are circular at the same depth.
 *
 * Hash values are checked first as a fast-path rejection. The function also supports dual data-last
 * usage: call it with one argument to get a curried predicate.
 *
 * **Gotchas**
 *
 * - Results are cached per object pair in a WeakMap. **Objects must not be mutated after their first
 *   comparison.**
 ...
 * **When to use**
 *
 * Use when you need generic utility code to distinguish `Equal` implementors from plain values
 * before calling `[Equal.symbol]` directly.
 *
 * **Details**
 *
 * - Pure function, no side effects.
 * - Returns `true` if and only if `u` has a property keyed by {@link symbol}.
 ...
 * **Details**
 *
 * - Returns a function `(a: A, b: A) => boolean` that delegates to {@link equals}.
 ...
 * **When to use**
 *
 * Use when you need to compare a plain object or array by identity without mutating the original
 * value.
 *
 * **Details**
 *
 * - Returns a `Proxy` wrapping `obj`. The proxy reads through to the original, so property access is
 *   unchanged.
 * - The proxy is registered in an internal WeakSet; {@link equals} returns `false` for any pair where
 *   at least one operand is in that set (unless they are the same reference).
 ...
 * **When to use**
 *
 * Use when you need reference equality without proxy allocation and accept permanently marking the
 * original object for reference-only equality.
 *
 * **Details**
 *
 * - Adds `obj` to an internal WeakSet. From that point on, {@link equals} treats it as
 *   reference-only.
