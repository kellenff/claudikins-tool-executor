 ...
 *
 * A span records the lifetime of an operation, including its name, parent, attributes, links,
 * annotations, sampling decision, kind, and completion status. The module also defines the tracer
 * service, parent-span context, external span support, trace propagation settings, and the default
...
/**
 * A tracing backend used by Effect to create spans. Custom tracers implement `span` to allocate a
 * span from the supplied name, parent, annotations, links, start time, kind, root flag, and
 * sampling decision.
 *
 * @since 2.0.0
 * @category Models
...
/**
 * A low-level Effect primitive that can be evaluated by a tracer-specific context for the current
 * fiber.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Lifecycle state of a span, where `Started` records the start time and `Ended` records the start
 * time, end time, and exit value with which the span completed.
 *
...
/**
 * A span value that can participate in tracing, either an Effect-managed `Span` or an
 * `ExternalSpan` propagated from another tracing system.
 *
...
/**
 * Context service containing the `Span` or `ExternalSpan` to use as the parent of newly-created
 * child spans.
 *
...
/**
 * Represents a span created outside Effect's tracer, carrying trace and span identifiers, sampling
 * state, and annotations so it can be used as a parent or link in Effect tracing.
 *
...
/**
 * Options accepted by span-creating APIs, combining span metadata such as attributes, links,
 * parent/root selection, kind, sampling, and trace level with stack trace capture settings.
 *
...
/**
 * Span creation options that do not control stack trace capture, including attributes, links,
 * parent or root selection, annotations, span kind, sampling, and the trace level used for
 * filtering.
 *
 * @since 4.0.0
 * @category Options
...
/**
 * Options that control stack trace capture for tracing wrappers. `captureStackTrace` can disable
 * capture or provide a lazy stack string.
 *
 * @since 4.0.0
 * @category Options
 */
...
/**
 * OpenTelemetry-style role describing the kind of operation represented by a span: internal work,
 * server handling, client calls, producing, or consuming.
 *
...
/**
 * A span created by an Effect tracer. It carries trace identity, parent, annotations, attributes,
 * links, sampling and kind information, lifecycle status, and methods to end the span or add
 ...
 *     console.log(
 *       `${name} at ${startTime} with ${Object.keys(eventAttributes).length} attributes`,
 *     );
 *   },
 *   addLinks(newLinks) {
 *     links.push(...newLinks);
 ...
 * **Details**
 *
 * `make` returns the supplied implementation object unchanged. The object must satisfy the `Tracer`
 * contract, including a `span` method that returns a `Span`.
 *
 * @since 2.0.0
...
/**
 * Creates an `ExternalSpan` from trace and span identifiers, defaulting `sampled` to `true` and
 * annotations to an empty context when they are not provided.
 *
 ...
 * **Details**
 *
 * When enabled on fiber or span annotations, new spans are created as non-propagating no-op spans
 * and disabled spans are skipped when deriving a parent span.
 *
 ...
 * **Details**
 *
 * The default value is `"Info"`. Span creation uses `options.level ?? CurrentTraceLevel` before
 * applying `MinimumTraceLevel`.
 *
 * @since 4.0.0
 * @category References
...
/**
 * Context reference for setting the minimum trace level threshold. Spans and their descendants
 * below this level will have their sampling decision forced to false, preventing them from being
 * exported.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The default value is `"All"`. Span creation compares the span level from `options.level ??
 * CurrentTraceLevel` against this threshold.
 *
 * **Gotchas**
 *
...
/**
 * Context reference for the active tracer service. By default it uses the native tracer, which
 * creates `NativeSpan` instances.
 *
...
/**
 * Default in-memory `Span` implementation used by the native tracer. It generates span and trace
 * identifiers, stores attributes, events, and links, and records `Started` or `Ended` status.
 *
 * **Details**
 *
 * The constructor initializes the span with `Started` status, inherits the parent trace id or
 * generates a new one, and always generates a new span id. Attributes, events, links, and status
 * are then mutated through `Span` methods.
 *
 * @since 4.0.0
