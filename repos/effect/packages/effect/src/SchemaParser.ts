 ...
 * encoded input, and encode decoded values back to their external form. This module exposes those
 * operations through several result styles, including `Effect`, `Promise`, `Exit`, `Option`,
 * `Result`, and synchronous functions that throw. It also contains the lower-level runner that
    ...
    case "Arrays":
      return ast.recur((ast) => {
        ...
        if (defaultValue) {
          ...
          return SchemaAST.replaceEncoding(
            out,
            out.encoding ? [...out.encoding, ...defaultValue] : defaultValue,
          );
        }
 ...
 * **When to use**
 *
 * Use to construct decoded schema values in `Effect` while preserving construction failures as
 * `SchemaIssue.Issue` values in the error channel.
 *
 * **Details**
 *
 * The returned function accepts constructor input, applies constructor defaults, runs type-side
 * validation unless checks are disabled, and fails with a `SchemaIssue.Issue` when construction
 * fails.
 *
 * @since 4.0.0
...
/**
 * Creates a synchronous maker that returns `Option.some` with the constructed value on success, or
 * `Option.none` when construction fails with schema issues.
 *
 * **When to use**
 *
 * Use when you need to validate schema constructor input and only care whether construction
 * succeeds, without exposing `SchemaIssue.Issue` details.
 *
 * **Gotchas**
 *
 * Only causes made entirely of schema issues are converted to `Option.none`. Causes that contain
 * defects, interruptions, or asynchronous work at this synchronous boundary throw an `Error` whose
 ...
 * **When to use**
 *
 * Use to construct decoded schema values synchronously when invalid input should throw an `Error`
 * whose cause is `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function constructs a value from constructor input and throws an `Error` with the
 * `SchemaIssue.Issue` in its `cause` when construction fails.
 *
 * **Gotchas**
 *
 * Causes that contain defects, interruptions, or asynchronous work at this synchronous boundary
 * throw an `Error` whose cause is the underlying `Cause`, instead of being converted to a schema
 * validation error.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to build a type guard for checking the decoded side of a schema without exposing issue
 * details.
 *
 * **Details**
 *
 * The guard returns `true` on successful validation and `false` when validation fails only with
 * schema issues, without exposing issue details.
 *
 * **Gotchas**
 *
 * Only causes made entirely of schema issues are converted to `false`. Causes that contain defects,
 * interruptions, or asynchronous work at this synchronous boundary throw an `Error` whose cause is
 ...
 * **When to use**
 *
 * Use to assert that an input satisfies the decoded side of a schema when schema validation
 * failures should throw an `Error` whose cause is `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The assertion returns normally when validation succeeds. When the input does not satisfy the
 * schema with a schema-only failure, it throws an `Error` with the `SchemaIssue.Issue` in its
 * `cause`.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when you need to decode untyped boundary input in an `Effect` whose failure channel is
 * `SchemaIssue.Issue`, while preserving transformations and service requirements.
 *
 * **Details**
 *
 * The returned function succeeds with the schema's decoded `Type` or fails with a
 * `SchemaIssue.Issue`. Decoding service requirements are preserved in the returned `Effect`. Parse
 * options may be provided when creating the decoder and overridden when applying it.
 *
 * @since 4.0.0
 * @category Decoding
 ...
 * **When to use**
 *
 * Use when you already have input typed as the schema's `Encoded` type and need an `Effect` whose
 * failure channel is `SchemaIssue.Issue`, while preserving decoding service requirements.
 *
 * **Details**
 *
 * The returned function succeeds with the decoded `Type` or fails with a `SchemaIssue.Issue`,
 * preserving any decoding service requirements in the returned `Effect`.
 *
 * @since 4.0.0
 * @category Decoding
 ...
 * **When to use**
 *
 * Use when you need to decode untyped input with a service-free schema and return a JavaScript
 * `Promise`.
 *
 * **Details**
 *
 * The returned function resolves with the decoded `Type` on success and rejects with an `Error`
 * whose cause is a `SchemaIssue.Issue` on decoding failure.
 *
 * **Gotchas**
 *
 * Causes that contain defects, interruptions, or other non-schema reasons reject with an `Error`
 * whose cause is the underlying `Cause`.
 *
 * @since 3.10.0
 * @category Decoding
 ...
 * **When to use**
 *
 * Use when you need to decode unknown input synchronously into an `Exit` whose failure contains
 * `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function produces `Exit.Success` with the decoded `Type`. Schema issues are
 * represented by an `Exit.Failure` cause containing a `SchemaIssue.Issue`.
 *
 * **Gotchas**
 *
 * Because this adapter runs synchronously, async decoding work can produce an `Exit.Failure` with a
 * defect cause. When the cause contains both schema issues and non-schema reasons, all reasons
 * remain in the returned `Cause`.
 *
 * @since 4.0.0
 * @category Decoding
...
/**
 * Creates a synchronous decoder for input already typed as the schema's `Encoded` type, reporting
 * failure safely as an `Exit`.
 *
 * **When to use**
 *
 * Use when you need synchronous decoding of already typed `Encoded` input into an `Exit` whose
 * failure contains `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function produces `Exit.Success` with the decoded `Type` or `Exit.Failure` with a
 * `SchemaIssue.Issue`.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when decoding untyped boundary input and you want `SchemaIssue.Issue` failures returned as
 * data in a `Result`.
 *
 * **Details**
 *
 * The returned function produces `Result.succeed` with the decoded `Type` on success or
 * `Result.fail` with a `SchemaIssue.Issue` on decoding failure.
 *
 * **Gotchas**
 *
 * This adapter runs synchronously. Causes made entirely of schema issues become `Result.fail`, but
 * causes that contain defects, interruptions, or asynchronous work at this synchronous boundary
 * throw instead.
 *
 * @since 4.0.0
...
/**
 * Creates a decoder for input already typed as the schema's `Encoded` type, reporting failure
 * safely as a `Result`.
 *
 * **When to use**
 *
 * Use when you already have input typed as the schema's `Encoded` type and want schema decoding
 * failures represented as `Result.fail` with `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function produces `Result.succeed` with the decoded `Type` on success or
 ...
 * **Gotchas**
 *
 * This synchronous adapter returns `Result.fail` for causes made entirely of schema issues, but
 * causes that contain defects, interruptions, or other non-schema reasons throw instead.
 *
 * @since 4.0.0
 * @category Decoding
 ...
 * **When to use**
 *
 * Use to decode untrusted or dynamically typed input at a synchronous boundary where invalid data
 * should throw an `Error` whose cause is `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function returns the decoded `Type` on success and throws an `Error` with the
 * `SchemaIssue.Issue` in its `cause` on decoding failure.
 *
 * **Gotchas**
 *
 * Causes that contain defects, interruptions, or asynchronous work at this synchronous boundary
 ...
 * **When to use**
 *
 * Use to decode values already typed as the schema's `Encoded` input when decoding failure should
 * throw an `Error` whose cause is `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function returns the decoded `Type` on success and throws an `Error` with the
 ...
 * **When to use**
 *
 * Use when you need to encode untyped boundary input in an `Effect` whose failure channel is
 * `SchemaIssue.Issue`, while preserving service requirements.
 *
 * **Details**
 *
 * The returned function succeeds with the schema's `Encoded` value or fails with a
 * `SchemaIssue.Issue`. Encoding service requirements are preserved in the returned `Effect`. Parse
 * options may be provided when creating the encoder and overridden when applying it.
 *
 * @since 4.0.0
 * @category Encoding
 ...
 * **When to use**
 *
 * Use when you need to encode values already typed as the schema's decoded `Type` in an `Effect`
 * whose failure channel is `SchemaIssue.Issue`, while preserving service requirements.
 *
 * **Details**
 *
 * The returned function succeeds with the schema's `Encoded` value or fails with a
 * `SchemaIssue.Issue`, preserving any encoding service requirements in the returned `Effect`.
 *
 * @since 4.0.0
 * @category Encoding
 ...
 * **When to use**
 *
 * Use when you need to encode untrusted or dynamically typed values with a service-free schema and
 * return a JavaScript `Promise`.
 *
 * **Details**
 *
 * The returned function resolves with the schema's `Encoded` value on success and rejects with an
 * `Error` whose cause is a `SchemaIssue.Issue` on encoding failure.
 *
 * **Gotchas**
 *
 * Causes that contain defects, interruptions, or other non-schema reasons reject with an `Error`
 ...
 * **When to use**
 *
 * Use when you already have values typed as the schema's decoded `Type` and need encoding to return
 * a JavaScript `Promise`.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need synchronous encoding of unknown input into an `Exit` whose failure contains
 * `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function produces `Exit.Success` with the schema's `Encoded` value or `Exit.Failure`
 * with a `SchemaIssue.Issue`.
 *
 * **Gotchas**
 *
 * Because this adapter runs synchronously, async encoding work can produce an `Exit.Failure` with a
 * defect cause. When the cause contains both schema issues and non-schema reasons, all reasons
 * remain in the returned `Cause`.
 *
 * @since 4.0.0
...
/**
 * Creates a synchronous encoder for input already typed as the schema's decoded `Type`, reporting
 * failure safely as an `Exit`.
 *
 * **When to use**
 *
 * Use when you need synchronous encoding of already typed schema values into an `Exit` whose
 * failure contains `SchemaIssue.Issue`.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when encoding values from an unknown or dynamically typed boundary synchronously, and you
 * want `SchemaIssue.Issue` failures returned as `Result` data.
 *
 * **Details**
 *
 * The returned function produces `Result.succeed` with the schema's `Encoded` value on success or
 * `Result.fail` with a `SchemaIssue.Issue` on encoding failure.
 *
 * **Gotchas**
 *
 * This adapter runs synchronously. Causes made entirely of schema issues become `Result.fail`, but
...
/**
 * Creates an encoder for input already typed as the schema's decoded `Type`, reporting failure
 * safely as a `Result`.
 *
 * **When to use**
 *
 * Use when you already have input typed as the schema's decoded `Type` and want encoding failures
 * returned as `Result.fail` with `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function produces `Result.succeed` with the schema's `Encoded` value on success or
 ...
 * **When to use**
 *
 * Use when you need to encode values from untyped input in synchronous code and want encoding
 * failures to throw an `Error` whose cause is `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function returns the schema's `Encoded` value on success and throws an `Error` with
 * the `SchemaIssue.Issue` in its `cause` on encoding failure.
 *
 * **Gotchas**
 *
 * Causes that contain defects, interruptions, or asynchronous work at this synchronous boundary
 ...
 * **When to use**
 *
 * Use to encode already typed schema values synchronously when encoding failure should throw an
 * `Error` whose cause is `SchemaIssue.Issue`.
 *
 * **Details**
 *
 * The returned function returns the schema's `Encoded` value on success and throws an `Error` with
