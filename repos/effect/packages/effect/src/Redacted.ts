 ...
 *
 * A `Redacted<A>` shows a redacted placeholder in string, JSON, and inspection output, while still
 * storing the original value for trusted code that needs to recover it. This helps reduce
 * accidental leaks in logs and diagnostics. This module includes constructors, runtime checks,
 * value recovery, wiping of stored values, and comparison helpers that avoid exposing the wrapped
 ...
 * **When to use**
 *
 * Use to carry sensitive values while reducing accidental exposure in string, JSON, and inspection
 * output.
 *
 * **Gotchas**
 *
 * The underlying value is still stored in memory and can be recovered with `Redacted.value` until
 * the wrapper is wiped or becomes unreachable. Use `Redacted` to reduce accidental disclosure in
 * logs and diagnostics, not as a cryptographic protection mechanism.
 *
...
export declare namespace Redacted {
   ...
   * **When to use**
   *
   * Use when defining internals that need to preserve the covariant value type carried by
   * `Redacted`.
   *
   * **Details**
   *
   * This interface records the covariant value type carried by a `Redacted` value and is not
   * normally referenced directly by users.
   *
   * @since 3.3.0
   * @category Models
   */
 ...
 * **Details**
 *
 * The wrapper redacts string, JSON, and inspection output to reduce accidental disclosure. The
 * original value remains retrievable with `Redacted.value` until the wrapper is wiped or becomes
 * unreachable.
 *
...
const Proto = {
  ...
  [Equal.symbol]<T>(this: Redacted<T>, that: unknown): boolean {
    return (
      isRedacted(that) &&
      Equal.equals(redacted.redactedRegistry.get(this), redacted.redactedRegistry.get(that))
    );
  },
};

/**
 * Retrieves the original value from a `Redacted` instance. Use this function with caution, as it
 * exposes the sensitive data.
 *
 * **When to use**
 *
...
/**
 * Deletes the stored value for a `Redacted` wrapper, making future `Redacted.value` calls on that
 * wrapper fail.
 *
 * **When to use**
 *
 ...
 * **Gotchas**
 *
 * This unsafe operation does not zero memory and does not affect other references to the original
 * value. It only removes the value from the internal redacted registry.
 *
...
/**
 * Generates an equivalence relation for `Redacted<A>` values based on an equivalence relation for
 * the underlying values `A`. This function is useful for comparing `Redacted` instances without
 * exposing their contents.
 *
 * **When to use**
 *
 * Use when you need to compare wrapped secrets through an approved equality rule without exposing
 * the underlying values at each comparison site.
 *
