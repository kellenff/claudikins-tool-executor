 ...
 *
 * A `TxSemaphore` has a fixed capacity and stores its available permit count in a `TxRef`.
 * Acquiring or releasing permits can therefore commit atomically with other transactional state
 * changes. This module includes operations for creating semaphores, checking capacity and
 * availability, acquiring or releasing permits, and running effects while permits are held.
 *
 * @since 4.0.0
 */
...
/**
 * A transactional semaphore that manages permits using Software Transactional Memory (STM)
 * semantics, providing atomic permit acquisition and release operations within Effect transactions
 * for concurrency control over limited resources.
 *
 * **When to use**
 *
...
/**
 * Acquires a single permit from the semaphore. If no permits are available, the effect will block
 ...
 * **When to use**
 *
 * Use to manually acquire multiple permits transactionally, waiting until all requested permits are
 * available.
 *
 * **Details**
 *
 * If fewer than `n` permits are available, the transaction retries until enough permits are
 * released.
 *
 * **Gotchas**
 *
 * Passing a non-positive `n` dies with a defect. Passing a value greater than the semaphore
 * capacity can wait forever because the capacity is fixed.
 *
...
/**
 * Tries to acquire a single permit from the semaphore without blocking, returning `true` if
 * successful or `false` if no permits are available.
 *
 * **When to use**
 *
...
/**
 * Tries to acquire the specified number of permits from the semaphore without blocking, returning
 * `true` if successful or `false` if not enough permits are available.
 *
 * **When to use**
 *
...
/**
 * Executes an effect with a single permit from the semaphore. The permit is automatically acquired
 * before execution and released afterwards, even if the effect fails or is interrupted.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The permit acquisition and release operations use atomic semantics to ensure proper resource
 * management with Effect's scoped operations.
 *
 ...
 * **Details**
 *
 * The permits are acquired before the effect starts and released after it completes, fails, or is
 * interrupted.
 *
 * **Gotchas**
 *
 * Passing a non-positive `n` dies with a defect. Passing a value greater than the semaphore
 * capacity can wait forever.
 *
...
/**
 * Acquires a single permit from the semaphore in a scoped manner. The permit will be automatically
 * released when the scope is closed, even if effects within the scope fail or are interrupted.
 *
 * **When to use**
 *
