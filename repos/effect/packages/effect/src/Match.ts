 ...
 *
 * `Match` lets you add ordered cases and then finish them with a result, fallback, `Option`, or
 * exhaustive check. Use `Match.type` to define a reusable matcher for a type, or `Match.value` to
 * match one value immediately. Cases can match literal values, predicates, object shapes, tags,
 * negated patterns, and common checks such as strings, numbers, records, and class instances.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * A `Matcher` carries the input type, accumulated filters, remaining cases, result type, and, for
 * value matchers, the provided value being matched.
 *
 ...
 * **Details**
 *
 * A `TypeMatcher` is created when using `Match.type<T>()` and allows you to define patterns that
 * will be applied to values of the specified type. It maintains type-level information about the
 * input type, applied filters, remaining cases, and expected results.
 *
 ...
 * **Details**
 *
 * A `ValueMatcher` is created when using `Match.value(someValue)` and contains the actual value to
 * be matched against. It tracks both the provided value and the result of applying patterns to
 * determine matches.
 *
 ...
 * **When to use**
 *
 * Use as the common public type for code that needs to inspect, store, or pass either positive or
 * negative pattern matching cases.
 *
 * **Details**
 *
 * A `Case` can be either a positive match (`When`) or a negative match (`Not`). Cases are the
 * building blocks of pattern matching logic and determine how values are tested and transformed.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * A `When` case contains the logic to test if a value matches a specific pattern and the function
 * to evaluate when the pattern matches. It's the primary building block for pattern matching
 * conditions.
 *
 ...
 * **Details**
 *
 * A `Not` case contains the logic to test if a value does NOT match a specific pattern and the
 * function to evaluate when the pattern doesn't match. It's used for exclusion-based pattern
 * matching.
 *
 ...
 * **Details**
 *
 * This function defines a `Matcher` that operates on a given type, allowing you to specify
 * conditions for handling different cases. Once the matcher is created, you can use
 * pattern-matching functions like {@link when} to define how different values should be processed.
 *
 ...
 * **Details**
 *
 * This function allows you to define a `Matcher` directly from a given value, rather than from a
 * type. This is useful when working with known values, enabling structured pattern matching on
 * objects, primitives, or any data structure.
 *
 * Once the matcher is created, you can use pattern-matching functions like {@link when} to define
 * how different cases should be handled.
 *
 ...
 * **Details**
 *
 * This function provides a convenient way to pattern match on discriminated unions by providing an
 * object that maps each `_tag` value to its corresponding handler. It's similar to a switch
 * statement but with better type safety and exhaustiveness checking.
 *
 ...
 * **Details**
 *
 * This function allows you to define exhaustive pattern matching for discriminated unions by
 * providing handlers for each possible `_tag` value. It ensures type safety and can optionally
 * enforce a specific return type across all branches.
 *
 ...
 * **Details**
 *
 * This function enforces a consistent return type across all pattern-matching branches. By
 * specifying a return type, TypeScript will check that every matching condition produces a value of
 * the expected type.
 *
 * **Important:** This function must be the first step in the matcher pipeline. If used later,
 * TypeScript will not enforce type consistency correctly.
 *
 ...
 * **When to use**
 *
 * Use to add one positive pattern case to a `Match.type` or `Match.value` pipeline when a direct
 * value, predicate, or structured object pattern should run a handler for matching input.
 *
 * **Details**
 *
 * Supports both direct value comparisons and predicate functions. If the pattern matches, the
 * associated function is executed and the matched input is removed from the remaining cases tracked
 * by the matcher.
 *
 ...
 * **Details**
 *
 * This function allows defining a condition where a value matches any of the provided patterns. If
 * a match is found, the associated function is executed. It simplifies cases where multiple
 * patterns share the same handling logic.
 *
 * Unlike {@link when}, which requires separate conditions for each pattern, this function enables
 * combining them into a single statement, making the matcher more concise.
 *
 ...
 * **Details**
 *
 * This function allows defining a condition where a value must match all the given patterns
 * simultaneously. If the value satisfies every pattern, the associated function is executed.
 *
 * Unlike {@link when}, which matches a single pattern at a time, this function ensures that
 * multiple conditions are met before executing the callback. It is useful when checking for values
 * that need to fulfill multiple criteria at once.
 *
 ...
 * **Details**
 *
 * This function is used to define pattern matching on objects that follow a **discriminated union**
 * structure, where a specific field (e.g., `type`, `kind`, `_tag`) determines the variant of the
 * object. It allows matching multiple values of the discriminant and provides a function to handle
 * the matched cases.
 *
 ...
 * **Details**
 *
 * Instead of checking for exact matches, this helper matches values that share a common prefix. For
 * example, if the discriminant field contains hierarchical names like `"A"`, `"A.A"`, and `"B"`, a
 * single `"A"` rule can match both `"A"` and `"A.A"`.
 *
...
/**
 * Matches values based on a field that serves as a discriminator, mapping each possible value to a
 ...
 * **Details**
 *
 * This function simplifies working with discriminated unions by letting you define a set of
 * handlers for each possible value of a given field. Instead of chaining multiple calls to
 * {@link discriminator}, this function allows defining all possible cases at once using an object
 * where the keys are the possible values of the field, and the values are the corresponding handler
 * functions.
 *
 ...
 * **Details**
 *
 * This is the exhaustive variant of {@link discriminators}. Each possible discriminator value must
 * have a corresponding handler, so the matcher is finalized directly and does not require
 * `Match.exhaustive` at the end of the pipeline.
 *
 ...
 * **Details**
 *
 * This helper follows the Effect convention that discriminated unions use `"_tag"` as their
 * discriminator field. Use {@link discriminator} for a different discriminator field.
 *
 ...
 * **Details**
 *
 * This function allows you to match on values in a **discriminated union** based on whether the
 * `_tag` field starts with a specified prefix. It is useful for handling hierarchical or namespaced
 * tags, where multiple related cases share a common prefix.
 *
 ...
 * **Details**
 *
 * This function provides a way to handle discriminated unions by mapping `_tag` values to specific
 * functions. Each handler receives the matched value and returns a transformed result. If all
 * possible tags are handled, you can enforce exhaustiveness using `Match.exhaustive` to ensure no
 * case is missed.
 *
 ...
 * **Details**
 *
 * This function is designed for **discriminated unions** where every possible `_tag` value must
 * have a corresponding handler. Unlike {@link tags}, this function ensures **exhaustiveness**,
 * meaning all cases must be explicitly handled. If a `_tag` value is missing from the mapping,
 ...
 * **Details**
 *
 * This predicate matches any string that contains at least one character, effectively filtering out
 * empty strings ("").
 *
 ...
 * **Details**
 *
 * This function creates a predicate that matches any of the provided literal values. It's useful
 * for matching against multiple specific values in a single pattern.
 *
 ...
 * **Details**
 *
 * This predicate refines unknown values to strings, allowing pattern matching on string types. It's
 * commonly used in type-based matchers to handle string cases.
 *
 ...
 * **Details**
 *
 * This predicate refines unknown values to numbers, allowing pattern matching on numeric types. It
 * matches all number values including integers, floats, `Infinity`, `-Infinity`, and `NaN`.
 *
 ...
 * **Details**
 *
 * This predicate matches every input, including `undefined`, `null`, objects, primitives, and
 * functions.
 *
 * **Gotchas**
 *
 * `Match.any` should usually be last because cases are checked in order and the first matching case
 * wins.
 *
 ...
 * **Details**
 *
 * This predicate matches values that are neither `null` nor `undefined`, effectively filtering out
 * nullish values while preserving all other types.
 *
 ...
 * **Details**
 *
 * This predicate refines unknown values to booleans, allowing pattern matching on boolean types. It
 * only matches the primitive boolean values `true` and `false`.
 *
 ...
 * **Details**
 *
 * This predicate refines unknown values to bigints, allowing pattern matching on bigint types.
 * BigInts are used for representing integers with arbitrary precision.
 *
 ...
 * **Details**
 *
 * This predicate refines unknown values to symbols, allowing pattern matching on symbol types.
 * Symbols are unique identifiers that are often used as object keys or for creating private
 * properties.
 *
 ...
 * **Details**
 *
 * This predicate refines unknown values to Date instances, allowing pattern matching on Date
 * objects. It only matches actual Date instances, not date strings or timestamps.
 *
 ...
 * **Details**
 *
 * This predicate uses `Predicate.isObject`: it returns `true` for values whose runtime type is
 * `"object"`, are not `null`, and are not arrays. It can match `Date`, `RegExp`, and class
 * instances; use `instanceOf` or a more specific pattern when those cases need to be
 * distinguished.
 *
 ...
 * **Details**
 *
 * This predicate checks if a value is an instance of the specified constructor, providing type-safe
 * matching for class instances and built-in objects.
 *
 ...
 * **Details**
 *
 * This predicate checks if a value is an instance of the specified constructor but doesn't provide
 * the same type safety guarantees as the regular `instanceOf`. Use this when you need more
 * flexibility but understand the type safety implications.
 *
 ...
 * **Details**
 *
 * This function ensures that a matcher always returns a valid result, even if no defined patterns
 * match. It acts as a default case, similar to the `default` clause in a `switch` statement or the
 ...
 * **Details**
 *
 * This function finalizes a matcher by ensuring that if no patterns match, an error is thrown. It
 * is useful when all cases should be covered, and any unexpected input should trigger an error
 * instead of returning a default value.
 *
 * When used, this function removes the need for an explicit fallback case and ensures that an
 * unmatched value is never silently ignored.
 *
 ...
 * **Details**
 *
 * This function ensures that the result of a matcher is always wrapped in an `Result`, allowing
 * clear differentiation between successful matches (`Ok(value)`) and cases where no pattern matched
 * (`Err(unmatched value)`).
 *
 * This approach is particularly useful when handling optional values or when an unmatched case
 * should be explicitly handled rather than returning a default value or throwing an error.
 *
 ...
 * **Details**
 *
 * This function ensures that the result of a matcher is wrapped in an `Option`, making it easy to
 * handle cases where no pattern matches. If a match is found, it returns `Some(value)`, otherwise,
 * it returns `None`.
 *
 * This is useful in cases where a missing match is expected and should be handled explicitly rather
 * than throwing an error or returning a default value.
 *
 ...
 * **When to use**
 *
 * Use to require TypeScript to reject incomplete matcher definitions before the matcher is turned
 * into a function.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * `SafeRefinement` provides a way to refine types in pattern matching while maintaining type
 * safety. Unlike regular predicates, safe refinements can transform the matched value's type
 ...
 * **Details**
 *
 * This namespace provides advanced type-level utilities used internally by the Match module to
 * perform complex pattern matching, type narrowing, and filter application. These types enable the
 * sophisticated type inference that makes pattern matching both type-safe and ergonomic.
 *
 * @since 4.0.0
 */
export declare namespace Types {
   ...
   * **Details**
   *
   * This utility type determines what type a value will have after successfully matching against a
   * pattern. It handles refinements, predicates, and complex object patterns to provide accurate
   * type narrowing.
   *
   ...
   * **Details**
   *
   * This utility type determines what type remains after a `Match.not` pattern excludes certain
   * values. It's the complement of `WhenMatch`, calculating what's left after removing the matched
   * portion.
   *
   ...
   * **Details**
   *
   * This utility type processes patterns (predicates, refinements, objects) and resolves them to
   * their corresponding matched types. It's used internally to compute type transformations during
   * pattern matching.
   *
   ...
   * **Details**
   *
   * This utility type determines what should be excluded from a union type when a pattern is used
   * in filtering operations. It transforms patterns into their exclusion-safe representations.
   *
   ...
   * **Details**
   *
   * This type represents patterns that can match against complex data structures like objects and
   * arrays. It supports nested pattern matching and partial object matching, enabling sophisticated
   ...
   * **Details**
   *
   * This type represents the building blocks of pattern matching: predicates, literal values, and
   * safe refinements. These are the atomic patterns that can be composed into more complex matching
   * logic.
   *
   * @since 4.0.0
   * @category Types
   ...
   * **Details**
   *
   * `Without` is used internally to track which types should be excluded from consideration during
   * pattern matching. It helps implement the type-level logic for `Match.not` and other exclusion
   * operations.
   *
   ...
   * **Details**
   *
   * `Only` is used internally to track which types should be exclusively considered during pattern
   * matching. It helps implement the type-level logic for positive matches and type narrowing.
   *
   ...
   * **Details**
   *
   * This utility type manages the accumulation of excluded types during pattern matching. When
   * multiple exclusions are applied, it combines them into a single filter representation.
   *
   ...
   * **Details**
   *
   * This utility type manages the refinement of included types during pattern matching. It ensures
   * that only the most specific type constraints are maintained when multiple positive matches are
   * applied.
   *
   ...
   * **Details**
   *
   * This utility type takes the collected inclusion/exclusion filters and applies them to the input
   * type to compute the final narrowed result. It's the culmination of the type-level filtering
   * process.
   *
   ...
   * **Details**
   *
   * This utility type extracts the possible values of a discriminant field from a union type. It's
   * used internally to implement tag-based pattern matching for discriminated unions.
   *
   ...
   * **Details**
   *
   * This utility type takes an array of types and converts them into a single intersection type.
   * It's used internally when multiple patterns need to be satisfied simultaneously (like in
   * `Match.whenAnd`).
   *
   ...
   * **Details**
   *
   * This is the core type utility that performs the actual type extraction and narrowing logic. It
   * handles the complex type-level computation that determines what type results from applying a
   * pattern to an input type.
   *
