 ...
 *
 * A `LayerRef<I, E>` builds one `Layer` into a cached `Context<I>`, exposes it back as a layer or
 * scoped effect, and supports invalidation so later users can acquire a fresh context.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you want to share one scoped layer resource across many users while retaining the
 * ability to invalidate it and rebuild it later.
 *
 * **Details**
 *
 * A `LayerRef` is the unkeyed counterpart to a layer cache: it lazily builds the layer on first
 * use, reuses the resulting context while it is borrowed or kept idle, and can invalidate the
 * cached context so the next use rebuilds it.
 *
 * @since 4.0.0
 * @category Models
...
export interface LayerRef<in out I, in out E = never> {
  ...
  /** Invalidates the cached context so the next use rebuilds the layer, and reacquires it. */
 ...
 * **When to use**
 *
 * Use when you have one layer-built resource that should be shared, optionally kept alive while
 * idle, and refreshed on demand.
 *
 * **Details**
 *
 * The layer is built lazily on first use unless `preload` is `true`. `idleTimeToLive` keeps the
 * context cached after it stops being used, and `invalidationSchedule` can periodically invalidate
 * it. When `preload` is `true`, scheduled invalidation also reacquires the context.
 *
 * **Gotchas**
 *
 * Invalidation does not revoke contexts already borrowed by active scopes; those contexts remain
 * usable until their scopes close.
 *
...
>(
  ...
  options?:
    | {
        ...
        /**
         * Schedule used to invalidate the cached resource. When `preload` is `true`, each scheduled
         * invalidation also reacquires the resource.
         */
        readonly invalidationSchedule?: Schedule.Schedule<X, unknown, SE, SR> | undefined;
      }
    | undefined,
): Effect.fn.Return<LayerRef<I, E>, [Preload] extends [true] ? E : never, Scope.Scope | R | SR> {
 ...
 * **When to use**
 *
 * Use as the public type for classes returned by `LayerRef.Service` when an API needs to accept,
 * return, or alias the generated service class and its static helpers.
 *
 * **Details**
 *
 * It combines a `Context.Service` tag for the `LayerRef` with default layers and helper accessors
 * for retrieving, using, and invalidating the cached resource.
 *
 * @since 4.0.0
 * @category Services
 ...
 * **When to use**
 *
 * Use when you want to name a shared layer reference as an application service and expose static
 * helpers for providing, retrieving, and invalidating it.
 *
 * **Details**
 *
 * The returned class is a `Context.Service` whose value is a `LayerRef`. It also includes `.layer`,
 * `.layerNoDeps`, `.get`, `.contextEffect`, and `.invalidate` helpers so callers do not need to
 * access the `LayerRef` value directly.
 *
 ...
 *   const database = yield* Database;
 *   return yield* database.query;
 * }).pipe(Effect.provide(DatabaseRef.get), Effect.provide(DatabaseRef.layer));
 * ```
 *
 * @since 4.0.0
 * @category Services
