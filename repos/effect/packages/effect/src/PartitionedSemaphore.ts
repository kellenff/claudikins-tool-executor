/**
 * The `PartitionedSemaphore` module provides a semaphore for limiting concurrency across a shared
 * permit pool while keeping waiters grouped by partition key. A `PartitionedSemaphore<K>` is useful
 * when many independent groups of work compete for the same bounded resource and each group should
...
/**
 * A `PartitionedSemaphore` controls access to a shared permit pool while tracking waiters by
 * partition key.
 *
 * **When to use**
 *
 * Use to coordinate shared permits across partition keys so waiting groups make progress without
 * one group monopolizing the pool.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * This interface does not add members beyond `PartitionedSemaphore`; it provides an alternate
 * exported name for APIs that refer to a partitioned permit pool.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when you need to construct a partitioned semaphore synchronously outside an `Effect`
 * workflow.
 *
 * **Details**
 *
 * Negative permit counts are clamped to `0`. Non-finite permit counts create an unbounded semaphore
 * whose acquire and release operations complete immediately.
 *
 * @since 3.19.4
 ...
 * **Details**
 *
 * The `permits` option sets the shared permit capacity. The resulting semaphore tracks waiters by
 * partition key and distributes released permits across waiting partitions in round-robin order.
 *
 * **Gotchas**
 *
 * Negative permit counts are clamped to `0`. Non-finite permit counts create an unbounded
 * semaphore.
 *
 * @since 3.19.4
 * @category Constructors
 ...
 * **Details**
 *
 * Running the returned effect reads the semaphore's current availability. Taking permits decreases
 * availability, and releasing permits can increase it up to the semaphore capacity.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * Capacity is stored when the semaphore is created and does not change as permits are acquired or
 * released.
 *
 * @since 4.0.0
 * @category Getters
 ...
 * **When to use**
 *
 * Use when you need manual permit acquisition for a partition and want to control acquisition and
 * release as separate effects.
 *
 * **Details**
 *
 * If enough permits are available, the effect completes immediately. Otherwise it waits until
 * released permits are assigned to this partition.
 *
 * **Gotchas**
 *
 * Requests for more permits than the semaphore capacity never complete. Requests for zero or a
 * negative number of permits complete without acquiring anything.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Returns an effect that releases permits back to the shared pool and returns the current available
 * permit count.
 *
 * **When to use**
 *
 * Use when you need to return permits acquired with `take` in a lower-level partitioned permit
 * protocol with explicit release control.
 *
 * **Details**
 *
 * Released permits are first assigned to waiting partitions in round-robin order. Only permits not
 * needed by waiters increase the available count, which is capped at the semaphore capacity.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Runs an effect after acquiring permits for a partition, then releases those permits when the
 * effect exits.
 *
 * **When to use**
 *
 * Use to guard weighted partitioned work with automatic permit acquisition and release around an
 * effect.
 *
 * **Details**
 *
 * Permit acquisition may wait according to `take` semantics. Once acquired, the permits are
 * released even if the wrapped effect fails or is interrupted.
 *
 * **Gotchas**
 *
 * Requests for more permits than the semaphore capacity never complete. Requests for zero or a
 * negative number of permits run the effect without acquiring anything.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Runs an effect after acquiring one permit for a partition, then releases the permit when the
 * effect exits.
 *
 * **When to use**
 *
 * Use to guard partitioned work with exactly one permit and automatic release when the effect
 * exits.
 *
 * **Details**
 *
 * This is the single-permit variant of `withPermits`. The permit is released even if the wrapped
...
/**
 * Runs an effect only when the requested permits can be acquired immediately, returning the result
 * in `Some`.
 *
 * **When to use**
 *
 * Use when guarded work should run only if the shared permit pool can provide the requested permits
 * immediately.
 *
 * **Details**
 *
 * If the permits are not available, the effect is not run and the result is `None`. When permits
 * are acquired, they are released after the wrapped effect completes, fails, or is interrupted.
 * Requests for zero or a negative number of permits run the effect and return `Some`.
 *
 * @since 4.0.0
 * @category Combinators
