/**
 * The `References` module exposes the built-in `Context.Reference` keys that the Effect runtime
 * consults for execution settings and diagnostic metadata. These references cover concurrency,
 * scheduling, logging, tracing, and low-level diagnostic state.
 *
 * A `Context.Reference<A>` is a service key with a default value. Reading one of these references
 * returns the value from the current fiber context, and providing a new value changes behavior for
 * the provided effect and the fibers it starts.
 *
 * @since 4.0.0
 */
...
export {
   ...
   * **When to use**
   *
   * Use to set the default trace level for spans created in a scope when span options do not
   * provide `level`.
   *
   * @since 4.0.0
   * @category References
   ...
   * **Details**
   *
   * Annotated spans become non-propagating no-op spans, and parent selection skips spans marked
   * with disabled propagation.
   *
   * @since 4.0.0
   * @category References
   ...
   * **When to use**
   *
   * Use to configure the runtime reference for the fiber operation budget that triggers a scheduler
   * yield.
   *
   * **Details**
   *
   ...
   * **When to use**
   *
   * Use to set the trace-level threshold that decides whether newly created spans are sampled and
   * exported.
   *
   * @since 4.0.0
   * @category References
   ...
   * **When to use**
   *
   * Use to bypass automatic scheduler yield checks in a controlled runtime scope where throughput
   * is preferred over scheduler fairness.
   *
   * **Details**
   *
   * When set to `true`, the fiber run loop skips `Scheduler.shouldYield`. The default value is
   * `false`.
   *
   * **Gotchas**
   *
   ...
   * **When to use**
   *
   * Use to access or override the active tracer service through the references module when working
   * directly with Effect runtime references.
   *
   * @since 4.0.0
   * @category References
   */
...
/**
 * Context reference for controlling the current concurrency limit. Can be set to "unbounded" for
 * unlimited concurrency or a specific number to limit concurrent operations.
 *
 * **When to use**
 *
 * Use to configure the default concurrency limit for operations that read concurrency from the
 * current context.
 *
...
/**
 * Context reference for the current log severity used by `Effect.log` when no explicit level is
 * provided.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use to carry the active log span stack that should be included with log entries in the current
 * context.
 *
 ...
 * **When to use**
 *
 * Use when writing low-level tracing or diagnostic integrations that need direct access to the
 * stack-frame chain carried by the current fiber.
 *
 * **Details**
 *
 * Effect and Layer tracing use this reference to attach stack-frame information to failures and
 * interruption causes. It is normally managed by tracing APIs rather than provided directly by
 * application code.
 *
 * @since 4.0.0
 * @category References
...
/**
 * Context reference for setting the minimum log level threshold. Log entries below this level will
...
/**
 * Context reference for controlling whether tracing is enabled globally. When set to false, spans
 * will not be registered with the tracer and tracing overhead is minimized.
 *
 * **When to use**
 *
...
/**
 * Context reference for managing span links that are automatically added to all new spans. Span
 * links connect related spans that are not in a parent-child relationship.
 *
 * **When to use**
 *
...
/**
 * Context reference for controlling whether trace timing is enabled globally. When set to false,
 * spans will not contain timing information (trace time will always be set to zero).
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use when reading or supplying the stack-frame chain that Effect tracing uses to attach diagnostic
 * call-site information to failures and interruptions.
 *
 * **Details**
 *
 * Each frame has a span or operation `name`, a lazy `stack` supplier, and an optional `parent`
 * frame that links it to the previous captured frame.
 *
 * @since 4.0.0
 * @category References
 ...
 * **When to use**
 *
 * Use to inspect or provide the complete set of loggers used by Effect logging in the current
 * context.
 *
 * **Details**
 *
 * The default set contains the built-in default logger and tracer logger. Providing this reference
 * changes which `Logger` instances receive log entries in the current context.
 *
 * @since 4.0.0
 * @category References
 */
 ...
 * **When to use**
 *
 * Use to configure the runtime reference that controls whether built-in console loggers write to
 * stderr.
 *
 * **Details**
 *
 * The default value is `false`. When set to `true`, the built-in default logger and TTY pretty
 * console logger call `console.error` instead of `console.log`.
 *
 * @since 4.0.0
 * @category References
 */
...
export {
  /**
   * Context reference for the current scheduler implementation used by the Effect runtime. Controls
   * how Effects are scheduled and executed.
   *
   * **When to use**
   *
