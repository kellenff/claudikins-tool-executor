 ...
 *
 * A newtype lets TypeScript distinguish values with the same runtime shape, such as two different
 * ids that are both strings. The tag exists only in the type system, so wrapping does not allocate
 * a runtime object. This module includes the base `Newtype` interface, wrapping and unwrapping
 * helpers, optics, and helpers for reusing carrier instances such as `Equivalence`, `Order`,
...
/**
 * A tagged interface that wraps a carrier type under a unique key, preventing accidental
 * interchange of structurally identical values.
 *
 * **When to use**
 *
 * Use to define a newtype as an `interface` extending `Newtype<"MyKey", CarrierType>` when
 * structurally identical carrier types should remain distinct in TypeScript.
 *
 * **Details**
 *
 * The tag is compile-time only, so no runtime wrapper is allocated. Use {@link makeIso} to create a
 * two-way conversion, or {@link value} to unwrap.
 *
...
/**
 * Namespace containing type-level helpers for `Newtype` values, including constraints and utilities
 * for extracting a newtype's key and carrier type.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use when you need the carrier value from an existing newtype without constructing a new newtype
 ...
 * **Details**
 *
 * The returned iso composes with other optics via the standard `Optic` API. Both directions have
 * zero runtime cost because they are identity casts.
 *
 ...
 * **When to use**
 *
 * Use when you need equality for newtype-wrapped values to behave like equality for the wrapped
 * carrier value, without manually unwrapping.
 *
 * **Details**
 *
 * The returned equivalence delegates to the provided carrier equivalence and has zero runtime cost
 * beyond the underlying equivalence check.
 *
 ...
 * **When to use**
 *
 * Use when you need to sort newtype-wrapped values according to the ordering of the wrapped carrier
 * value, without manually unwrapping.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to combine newtype-wrapped values with the carrier's combining logic, without
 * manually unwrapping.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to reduce a collection of newtype-wrapped values with the carrier's reducer,
 * without manually unwrapping.
 *
 * **Details**
 *
