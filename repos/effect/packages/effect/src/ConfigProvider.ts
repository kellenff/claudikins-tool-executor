/**
 * Data sources used by `Config` to load raw configuration values. A `ConfigProvider` reads paths
 * from places such as environment variables, JavaScript objects, `.env` contents, or directories,
 * and returns a uniform `Node` shape that config schemas can decode. The module also includes
 * helpers for composing providers, changing paths, and installing providers through layers.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when implementing a custom `ConfigProvider` by returning raw nodes from the `get` callback
 * passed to {@link make}, or when inspecting raw provider output before schema parsing.
 *
 * **Details**
 *
 * `Value` is a terminal string leaf. `Record` is an object-like container whose immediate child
 * keys are known and may carry an optional co-located `value`. `Array` is an indexed container with
 * a known `length` and may also carry an optional co-located `value`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * The optional `value` allows a node to be both a container and a leaf at the same time (for
 * example, an env var `A=x` that also has children `A_FOO` and `A_BAR`).
 *
 ...
 * **When to use**
 *
 * Use when you need to describe a JSON array or numerically indexed env vars inside a custom
 * provider.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to report that a custom provider's underlying store is unreachable or produced
 * an I/O error while reading configuration data.
 *
 * **Gotchas**
 *
 * Do not use `SourceError` for "key not found". That case is represented by returning `undefined`
 * from `load`.
 *
...
/**
 * An ordered sequence of string or numeric segments that addresses a node in the configuration
 * tree. String segments name object keys; numeric segments index into arrays.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use to type-annotate variables that hold a provider or to implement a custom provider via
 * {@link make}.
 *
 * **Details**
 *
 * `load(path)` is the semantic lookup operation used by the `Config` module. It applies provider
 * transformations and composition before consulting the underlying source. `undefined` means "not
 * found" and `SourceError` means the source itself failed.
 *
 * @since 2.0.0
 * @category Models
...
export interface ConfigProvider extends Pipeable {
  /**
   * Returns the node found at `path`, or `undefined` if it does not exist. Fails with `SourceError`
   * when the underlying source cannot be read.
   *
   * **When to use**
   *
   * Use to resolve a path through this provider's path transformations before reading the backing
   * source.
   */
 ...
 * default value of `fromEnv()`. Because it is a `Context.Reference`, it is available without
 * explicit provision; `Config` schemas automatically resolve it.
 *
 * **When to use**
 *
 * Use to override the active raw configuration provider for an entire program, or retrieve the
 * current provider inside an Effect.
 *
...
function makeOrElse(first: ConfigProvider, second: ConfigProvider): ConfigProvider {
  ...
  return makeProvider(state, (path) =>
    Effect.flatMap(state.first.load(path), (node) =>
      node ? Effect.succeed(node) : state.second.load(path),
    ),
  );
}
 ...
 * **When to use**
 *
 * Use when implementing a provider backed by a custom store, such as a database, remote API, or
 * in-memory map.
 *
 * **Details**
 *
 * The `get` callback receives a `Path` and must return `Effect<Node | undefined, SourceError>`.
 * Return `undefined` when the path does not exist; fail with `SourceError` only for actual I/O
 * errors.
 *
 ...
 * **When to use**
 *
 * Use to layer multiple config sources, such as env vars plus a defaults file, or provide partial
 * overrides on top of a base config.
 *
 * **Details**
 *
 * Each provider keeps its own path transformations. If the combined provider is later transformed
 * with {@link mapInput} or {@link nested}, the transformation is applied to both sides.
 *
 * **Gotchas**
 *
 * The fallback only runs when the path is not found (`undefined`). A `SourceError` from `self` is
 * not caught; it propagates immediately.
 *
 ...
 * **Details**
 *
 * The function `f` receives the whole path produced by earlier provider transformations and must
 * return a new path. Lookup path transformations compose in application order: the existing
 * transformation runs first, then `f` runs. For providers composed with {@link orElse}, the
 * transformation is applied to each operand.
 *
 ...
 * **Details**
 *
 * Numeric segments are left unchanged. String segments use `String.configCase` so numeric word
 * groups such as `v2` are preserved for environment variable names. This is a specialization of
 * {@link mapInput}.
 *
 ...
 * **When to use**
 *
 * Use to namespace config under a prefix like `"app"` or `"database"`, or reuse the same provider
 * shape for multiple sub-configs.
 *
 * **Details**
 *
 * Accepts a single string or a full `Path` array. For providers composed with {@link orElse}, the
 * prefix is applied to each operand. Supports both data-last and data-first calling conventions.
 *
 * **Gotchas**
 *
 * Ordering matters when composing with {@link mapInput} or {@link constantCase}. Later provider
 * transformations run after earlier ones: a later `nested` becomes the outer prefix, and a later
 * `mapInput` sees the whole path produced by previous transformations.
 *
...
export const nested: {
  ...
  switch (state._tag) {
    case "Source":
      return makeSource(
        state.get,
        flow(state.transform, (input) => [...path, ...input]),
      );
    case "OrElse":
...
/**
 * Provides a layer that installs a `ConfigProvider` as the active provider for all downstream
 ...
 * **Details**
 *
 * Accepts either a plain `ConfigProvider` or an `Effect` that produces one. When given an Effect,
 * it is evaluated once when the layer is built.
 *
...
/**
 * Creates a Layer that composes a new `ConfigProvider` with the currently active one, rather than
 * replacing it.
 *
 * **When to use**
 *
 * Use to add defaults that should only apply when the primary provider has no value for a path, or
 * override specific keys while keeping the rest from the existing provider by setting `asPrimary:
 * true`.
 *
 * **Details**
 *
 * By default, the new provider acts as a fallback and is consulted only when the current provider
 * returns `undefined`. Set `asPrimary: true` to make the new provider the primary source, with the
 * existing one as fallback.
 *
...
/**
 * Creates a `ConfigProvider` backed by an in-memory JavaScript value (typically a parsed JSON
 * object).
 *
 * **When to use**
 *
 * Use when you need deterministic config from an in-memory JavaScript value, such as in tests,
 * embedded config, or parsed JSON.
 *
 * **Details**
 *
 * Path traversal follows standard JS rules: string segments index into object keys, numeric
 * segments index into arrays. Returns `undefined` for any path that cannot be resolved. Never fails
 * with `SourceError`.
 *
 * Primitive values (`number`, `boolean`, `bigint`) are stringified via `String(...)`.
 *
 * Literal empty strings are treated as missing values when loaded as values by default. Pass `{
 * preserveEmptyStrings: true }` to keep empty strings as explicit values.
 *
 * **Gotchas**
 *
 * Object keys and array lengths reflect the original input shape. A leaf value of `""` is treated
 * as missing when that leaf is loaded, but the parent container still reports its original keys or
 * length.
 *
 ...
 * **When to use**
 *
 * Use to read configuration from `process.env`, which is the default when no provider is explicitly
 * set, or pass a custom env record for testing or non-Node runtimes.
 *
 * **Details**
 *
 * Path segments are joined with `_` for direct lookup, and env var names are also split on `_` to
 * build a trie for child key discovery. This means `DATABASE_HOST=localhost` is accessible at both
 * path `["DATABASE_HOST"]` and `["DATABASE", "HOST"]`. If all immediate children of a trie node
 * have purely numeric names, the node is reported as an `Array`; otherwise as a `Record`.
 *
 * The default environment merges `process.env` and `import.meta.env` (when available). Override by
 * passing `{ env: { ... } }`.
 *
 * Literal empty strings are treated as missing values when loaded as values by default. Pass `{
 * preserveEmptyStrings: true }` to keep empty strings as explicit values. Child discovery still
 * reflects the environment variable names present in the source.
 *
 ...
 * **When to use**
 *
 * Use when you already have the `.env` contents as a string, such as contents fetched from a remote
 * store or embedded in a test.
 *
 * **Details**
 *
 * Supports `export` prefixes, single/double/backtick quoting, inline comments, and escaped
 * newlines. Variable expansion (for example, `${VAR}`) is disabled by default; enable with `{
 * expandVariables: true }`.
 *
 * Literal empty strings are treated as missing values when loaded as values by default. Pass `{
 * preserveEmptyStrings: true }` to keep empty strings as explicit values. Child discovery still
 * reflects the keys present in the parsed `.env` source.
 *
...
function interpolate(envValue: string, parsed: Record<string, string>): string {
  ...
  /**
   * This finds the inner most variable/group divided by variable name and default value (if
   * present) ( (?!(?<=\))$ // only match dollar signs that are not escaped {? // optional opening
   * curly brace ([\w]+) // match the variable name (?::-([^}\]*))? // match an optional default
   * value }? // optional closing curly brace )
   */
  const matchGroup = /((?!(?<=\\))\${?([\w]+)(?::-([^}\\]*))?}?)/;
 ...
 * **Details**
 *
 * Requires `FileSystem` in the Effect context. Defaults to reading `".env"` in the current
 * directory; override with `{ path: "/custom/.env" }`. Variable expansion (for example, `${VAR}`)
 * is disabled by default; enable with `{ expandVariables: true }`.
 *
 * Literal empty strings are treated as missing values when loaded as values by default. Pass `{
 * preserveEmptyStrings: true }` to keep empty strings as explicit values. Child discovery still
 * reflects the keys present in the parsed `.env` source.
 *
 * Returns an `Effect` that resolves to a `ConfigProvider`. Fails with a `PlatformError` if the file
 * cannot be read.
 *
...
/**
 * Creates a `ConfigProvider` that reads configuration from a directory tree on disk, where each
 * file is a leaf value and each directory is a container.
 *
 * **When to use**
 *
 * Use when you expose each config key as a file under a directory, such as Kubernetes ConfigMap or
 * Secret volume mounts.
 *
 * **Details**
 *
 * Resolution tries a regular file first and returns a `Value` node for non-empty trimmed file
 * contents. If the file read fails, it tries a directory and returns a `Record` node with immediate
 * child names as keys. If both fail with `NotFound`, it returns `undefined`. Other platform
 * failures return `SourceError`.
 *
 * Requires `Path` and `FileSystem` in the Effect context. Defaults to root path `/`; override with
 * `{ rootPath: "/etc/config" }`.
 *
 * Literal empty strings are treated as missing values by default after file contents are trimmed.
 * Pass `{ preserveEmptyStrings: true }` to keep empty strings as explicit values. Directory
 * listings still reflect the file names present on disk.
 *
