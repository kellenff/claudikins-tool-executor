 ...
 *
 * A request describes one logical piece of work without performing it. It records the success type,
 * typed error, service requirements, and fields a resolver needs to complete the request. Requests
 * are paired with `RequestResolver`, which performs backend-specific loading and completes each
...
/**
 * A `Request<A, E, R>` is a request from a data source for a value of type `A` that may fail with
 * an `E` and have requirements of type `R`.
 *
 ...
 * **When to use**
 *
 * Use as a generic constraint for APIs that accept any request while preserving each concrete
 * request's success, error, and service types.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * This marker preserves the success, error, and service requirement types for Effect's type-level
 * machinery. Users normally get it by extending `Request`.
 *
 * @since 2.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The constructor accepts the request's data fields, excluding request variance fields and any
 * fields already supplied by the constructor such as `_tag`, and returns a value of the request
 * type.
 *
 ...
 * **Details**
 *
 * This low-level value provides the structural request marker for values created by `Request.of`,
 * `Request.tagged`, `Request.Class`, and `Request.TaggedClass`. Most users should use those
 * constructors instead of interacting with the prototype directly.
 *
 * @since 4.0.0
 * @category Prototypes
 */
...
/**
 * Creates a constructor function for a tagged Request type. The tag is automatically added to the
 * request, making it useful for discriminated unions.
 *
 ...
 * **Details**
 *
 * Subclasses pass their data fields to `super`, and instances are marked as `Request` values while
 * retaining the provided readonly fields.
 *
 ...
 * **Details**
 *
 * Use this when defining class-based request types that should participate in tagged unions or
 * tag-based request resolvers.
 *
...
export const TaggedClass = <Tag extends string>(
  ...
  args: Types.Equals<Omit<A, keyof Request<unknown, unknown>>, {}> extends true
    ? void
    : {
        readonly [P in keyof A as P extends "_tag" | keyof Request<any, any, any>
          ? never
          : P]: A[P];
      },
 ...
 * **When to use**
 *
 * Use to finish a `Request.Entry` by running an effect whose success or typed failure should become
 * the request result.
 *
 * **Details**
 *
 * If the effect succeeds, the entry is completed successfully with its value. If the effect fails,
 * the entry is completed with that failure.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when you need a `RequestResolver` to complete an entry with structured cause information
 ...
 * **Details**
 *
 * An entry contains the original request, the fiber context needed to run it, an `uninterruptible`
 * flag used by batching and caching internals, and the `completeUnsafe` callback used by resolvers
 * to supply the final `Exit`.
 *
 * @since 2.0.0
 * @category Entry
 */
 ...
 * **Details**
 *
 * This is a low-level helper for request runtime and resolver infrastructure; most application code
 * receives entries from a `RequestResolver` instead of constructing them directly.
 *
 * @since 2.0.0
 * @category Entry
 */
