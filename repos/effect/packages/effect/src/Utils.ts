/**
 * Internal and advanced utilities used by Effect's generator-based syntax and higher-kinded type
 * support. This is not a general-purpose utility module for application code.
 *
 * `SingleShotGen` makes an Effect-style value work with `yield*` inside generator helpers.
 * `Variance` and `Gen` provide the type-level signatures used by modules such as `Effect`,
 * `Option`, and `Result` to type their `gen` APIs.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use to implement `[Symbol.iterator]()` on Effect-like types so they can be `yield*`-ed inside
 * generator functions, such as `Effect.gen` and `Option.gen`.
 *
 * **Details**
 *
 * The first call to `next()` returns `{ value: self, done: false }`. Every subsequent call returns
 * `{ value: a, done: true }` where `a` is the argument passed to `next()`. `[Symbol.iterator]()`
 * returns a **new** `SingleShotGen` wrapping the same value, so the outer type can be iterated
 * multiple times.
 *
...
export class SingleShotGen<T, A> implements IterableIterator<T, A> {
   ...
   * **When to use**
   *
   * Use to advance a `SingleShotGen` through its single yield and completion step.
   *
   * @since 2.0.0
   */
   ...
   * **When to use**
   *
   * Use to iterate the wrapped value again without reusing the consumed iterator state.
   *
   * @since 2.0.0
   */
 ...
 * **When to use**
 *
 * Use to define variance constraints for a higher-kinded type so that {@link Gen} can correctly
 * infer `R`, `O`, and `E` from yielded values.
 *
 * **Details**
 *
 * `F` is invariant and must match exactly. `R` is contravariant in the input or environment
 * position. `O` and `E` are covariant in the output and error positions. This is a pure type-level
 * construct with no runtime representation.
 *
 ...
 * **When to use**
 *
 * Use to type the `gen` function of a module that supports generator syntax, such as `Option.gen`,
 * `Result.gen`, and `Effect.gen`.
 *
 * **Details**
 *
 * This is a pure type alias with no runtime behavior. It infers `R`, `O`, and `E` from the yielded
 * values via {@link Variance} or `Kind` constraints. The generator's return type `A` becomes the
 * output's `A` parameter.
 *
