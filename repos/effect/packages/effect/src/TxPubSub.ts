 ...
 *
 * A `TxPubSub<A>` is a transactional publish/subscribe hub. Each subscriber owns a `TxQueue`, and
 * each published value is offered to the subscriber queues that are registered at the time of
 * publication. This module includes bounded, dropping, sliding, and unbounded hubs, publishing
 * helpers, scoped subscriptions, shutdown operations, and a guard.
 *
 * @since 4.0.0
 */
...
/**
 * A TxPubSub represents a transactional publish/subscribe hub that broadcasts messages to all
 * current subscribers using Software Transactional Memory (STM) semantics.
 *
...
/**
 * Creates a bounded TxPubSub with the specified capacity. When a subscriber's queue is full, the
 * publisher will retry the transaction until space is available.
 *
...
/**
 * Creates a dropping TxPubSub with the specified capacity. When a subscriber's queue is full, the
...
/**
 * Creates a sliding TxPubSub with the specified capacity. When a subscriber's queue is full, the
 * oldest message in that subscriber's queue is dropped.
 *
 ...
 * **Details**
 *
 * Returns `true` if the message was delivered to all subscribers, or `false` if the hub is shut
 * down or the message was dropped for any subscriber. For the bounded strategy, the transaction
 * retries if any subscriber queue is full. For the sliding strategy, full subscriber queues drop
 * their oldest messages. For the dropping strategy, full subscriber queues drop the new message and
...
/**
 * Subscribes to the TxPubSub, returning a scoped `TxQueue` for messages published after
 * subscription.
 *
 * **Details**
 *
 * The returned queue uses the hub's capacity strategy: bounded subscriptions backpressure
 * publishers when full, dropping subscriptions may miss new messages when full, and sliding
 * subscriptions may evict older queued messages. The subscription is automatically removed when the
 * scope is closed.
 *
 ...
 * **When to use**
 *
 * Use to create and register a subscriber queue inside a larger transaction when registration must
 * be atomic with other Tx operations.
 *
 * **Details**
 *
 * This is the transactional acquire step of `subscribe`, exposed so that callers can compose it
 * with other Tx operations in a single transaction, such as `TxSubscriptionRef.changes`.
 *
 * @since 4.0.0
 * @category Mutations
 ...
 * **When to use**
 *
 * Use to release a manually acquired subscriber queue inside a larger transaction, removing it from
 * the pub/sub and shutting it down together with related transactional cleanup.
 *
 * **Details**
 *
 * This is the transactional release step of `subscribe`, exposed so that callers can compose it
 * with other Tx operations in a single transaction.
 *
 * **Gotchas**
 *
 * The supplied queue is shut down after being removed, so callers should pass a queue acquired for
 * this pub/sub.
 *
 * @since 4.0.0
 * @category Mutations
 ...
 * **Details**
 *
 * After shutdown, `publish` and `publishAll` return `false`, and `awaitShutdown` completes. The
