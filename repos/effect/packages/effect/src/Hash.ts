/**
 * Computes Effect hash values and defines the interface for objects that want to provide their own
 * hash implementation. Hashes are small numeric fingerprints used by Effect data structures to
 * bucket values quickly; they are not cryptographic digests and they are not proof that two values
 * are equal. The module also includes helpers for primitive, structure, array, and reference-based
 * hashes, plus functions for combining and optimizing numeric hash values.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use as the computed property key for the method that supplies a custom hash value on a `Hash`
 * implementor.
 *
 * @since 2.0.0
 * @category Symbols
 ...
 * **Details**
 *
 * Objects implementing this interface provide a method to compute their hash value, which is used
 * for efficient comparison and storage operations.
 *
 ...
 * **Details**
 *
 * This function can hash primitives (numbers, strings, booleans, etc.) as well as objects, arrays,
 * and other complex data structures. It automatically handles different types and provides a
 * consistent hash value for equivalent inputs.
 *
 * **Gotchas**
 *
 * Objects being hashed must be treated as immutable after their first hash computation. Hash
 * results are cached, so mutating an object after hashing will lead to stale cached values and
 * broken hash-based operations. For mutable objects, implement a custom `Hash` interface that
 * hashes the object reference rather than its content.
 *
 ...
 * **Details**
 *
 * This function creates a random hash value for objects that don't have their own hash
 * implementation. The hash value is cached using a WeakMap, so the same object will always return
 * the same hash value during its lifetime.
 *
 ...
 * **Details**
 *
 * Supports both direct and pipeable usage. The implementation combines two hash values with `(self
 * * 53) ^ b`.
 *
 ...
 * **Details**
 *
 * This function takes a hash value and applies bitwise operations to improve the distribution of
 * hash values, reducing the likelihood of collisions.
 *
 ...
 * **Details**
 *
 * This function determines whether a given value has the Hash symbol property, indicating that it
 * can provide its own hash value implementation.
 *
 ...
 * **Details**
 *
 * This function creates a hash value for numeric inputs, handling special cases like NaN, Infinity,
 * and -Infinity with distinct hash values. It uses bitwise operations to ensure good distribution
 ...
 * **Details**
 *
 * This function implements a variation of the djb2 hash algorithm, which is known for its good
 * distribution properties and speed. It processes each character of the string to produce a
 ...
 * **Details**
 *
 * This function allows you to hash an object by considering only specific keys, which is useful
 * when you want to create a hash based on a subset of an object's properties.
 *
 ...
 * **Details**
 *
 * The hash is based on the object's structural keys and their values, including symbol keys and
 * relevant prototype keys for non-plain objects.
 *
 ...
 * **Details**
 *
 * The implementation folds element hashes from the seed `6151` with XOR and then optimizes the
 * final hash.
 *
 * **Gotchas**
 *
 * A hash is not an equality proof. Because this implementation uses XOR, reordered inputs can
