/**
 * Low-level helpers for making custom values behave like Effects. The module exposes a prototype
 * builder and an abstract base class that let domain-specific values, such as service keys or
 * configuration descriptions, be evaluated by Effect and yielded inside `Effect.gen`.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need to create a custom Effect-like value without extending a class, by providing a
 * label and an evaluate function that receives the current fiber.
 *
 * **Details**
 *
