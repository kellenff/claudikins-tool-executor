 ...
 *
 * A `Metric<Input, State>` accepts typed update values and stores an aggregated state that can be
 * read directly or included in a snapshot. Metrics are used for counters, gauges, frequencies,
 * histograms, summaries, and timers. This module includes metric constructors, update and read
 * helpers, attributes, histogram boundaries, registry snapshots, text dumps, and controls for
...
/**
 * A `Metric<Input, State>` represents a concurrent metric which accepts update values of type
 * `Input` and are aggregated to a value of type `State`.
 *
 * **Details**
 *
 * For example, a counter metric would have type `Metric<number, number>`, representing the fact
 * that the metric can be updated with numbers (the amount to increment or decrement the counter
 * by), and the state of the counter is a number.
 *
 ...
 * **When to use**
 *
 * Use when frequency metrics are ideal for tracking categorical data where you want to count how
 * many times specific string values occur, such as HTTP status codes, user actions, error types, or
 * any discrete string-based events.
 *
 ...
 * **When to use**
 *
 * Use when gauges are useful for tracking current state values like memory usage, CPU load, active
 * connections, queue sizes, or any value that represents a current level.
 *
...
/**
 * A Histogram metric that records observations in configurable buckets to analyze value
 * distributions.
 *
 * **When to use**
 *
 * Use when histograms are ideal for measuring request durations, response sizes, and other
 * continuous values where you need to understand the distribution of values rather than just
 * aggregates.
 *
 ...
 *   const responseTimeState: Metric.HistogramState =
 *     yield* Metric.value(responseTimeHistogram);
 ...
 * **When to use**
 *
 * Use when summaries provide statistical insights into value distributions by tracking specific
 * quantiles (percentiles) such as median (50th), 95th percentile, 99th percentile, etc. They're
 * ideal for understanding performance characteristics like response time distributions.
 *
 ...
 *       performance: {
 *         fast:
 *           quantileValues.p50 !== null && quantileValues.p50 < 200
 *             ? "Good"
 *             : "Needs improvement",
 *         reliability:
 *           quantileValues.p95 !== null && quantileValues.p95 < 500
 *             ? "Reliable"
 *             : "Concerning",
 *       },
 *     },
 ...
 * **When to use**
 *
 * Use to provide default attributes that should be merged into metric updates and reads in a scoped
 * part of a program.
 *
 * **Details**
 *
 * The default value is an empty attribute set. Metric reads and updates merge these contextual
 * attributes with the metric's own attributes to select the metric series being accessed.
 *
 ...
 * **When to use**
 *
 * Use when you need a custom metric registry for an isolated program or test instead of the default
 * registry.
 *
 * **Details**
 *
 * By default, the reference creates an empty `Map` the first time it is resolved. Metrics register
 * their metadata and hooks lazily in this map when they are read or updated.
 *
 * **Gotchas**
 *
 * Because `Context.Reference` caches default values, the default `Map` is shared by contexts that
 * do not provide an override. Provide `MetricRegistry` with a fresh `Map` when isolation matters.
 *
 * @since 4.0.0
 * @category References
...
class SummaryMetric extends Metric$<readonly [value: number, timestamp: number], SummaryState> {
  ...
  createHooks(): Metric.Hooks<readonly [value: number, timestamp: number], SummaryState> {
    ...
    const update = ([value, timestamp]: readonly [value: number, timestamp: number]) =>
 ...
 * **When to use**
 *
 * Use when you need runtime type checking and ensuring that a value conforms to the Metric
 * interface before performing metric operations.
 *
...
/**
 * Represents a Counter metric that tracks cumulative numerical values over time. Counters can be
 * incremented and decremented and provide a running total of changes.
 *
 * **Details**
 *
 * The optional `description` describes the counter, and `attributes` attach dimensions to it. Set
 * `bigint` to create a counter that accepts `bigint` inputs. Set `incremental` to `true` to create
 * a counter that can only ever be incremented.
 *
...
/**
 * Represents a `Gauge` metric that tracks and reports a single numerical value at a specific
 * moment.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The optional `description` describes the gauge, and `attributes` attach dimensions to it. Set
 * `bigint` to create a gauge that accepts `bigint` inputs.
 *
 ...
 * **Details**
 *
 * The optional `description` describes the frequency, and `attributes` attach dimensions to it. Use
 * `preregisteredWords` to initialize occurrence counts for known string values before updates
 * arrive.
 *
 ...
 * **Details**
 *
 * The optional `description` describes the histogram, and `attributes` attach dimensions to it. The
 * required `boundaries` option defines the histogram bucket boundaries.
 *
...
/**
 * Creates a `Summary` metric that records observations and calculates quantiles which takes a value
 * as input and uses the current time.
 *
 * **When to use**
 *
 * Use when you need a metric that records statistical information about a set of values, including
 * quantiles.
 *
 * **Details**
 *
 * The optional `description` describes the summary, and `attributes` attach dimensions to it.
 * `maxAge` controls how long observations are retained, `maxSize` controls how many observations
 * are kept, and `quantiles` lists the quantiles to calculate, such as `[0.5, 0.9]`.
 *
...
export const summary = (
  ...
  mapInput(
    ...
    (input, context) =>
      [input, Context.get(context, InternalEffect.ClockRef).currentTimeMillisUnsafe()] as [
        number,
        number,
      ],
  );
...
/**
 * Creates a `Summary` metric that records observations with explicit timestamps and calculates
 * quantiles.
 *
 * **When to use**
 *
 * Use when you need a metric that records statistical information about a set of values together
 * with timestamps.
 *
 * **Details**
 *
 * Inputs to this metric are `[value, timestamp]` pairs; the current clock is used when reading
 * quantiles against the configured `maxAge`.
 *
 * The optional `description` describes the summary, and `attributes` attach dimensions to it.
 * `maxAge` controls how long observations are retained, `maxSize` controls how many observations
 * are kept, and `quantiles` lists the quantiles to calculate, such as `[0.5, 0.9]`.
 ...
 * **Details**
 *
 * The unit of time will automatically be added to the metric as a tag (i.e. `"time_unit:
 * milliseconds"`).
 *
 * If `options.boundaries` is not provided, the boundaries will be computed using
 ...
 * **Details**
 *
 * The returned state depends on the metric type. Counters return `CounterState<number | bigint>`
 * with `count` and `incremental`, gauges return `GaugeState<number | bigint>` with `value`,
 * frequencies return `FrequencyState` with `occurrences`, histograms return `HistogramState` with
 * buckets, count, min, max, and sum, and summaries return `SummaryState` with quantiles, count,
 * min, max, and sum.
 *
 ...
 * **Details**
 *
 * The behavior of `modify` depends on the metric type. Counters add the input value to the current
 * count, gauges add the input value to the current gauge value, frequencies increment the
 * occurrence count for the input string, histograms record the input value in the appropriate
 * bucket, and summaries record the input observation.
 *
 ...
 * **Details**
 *
 * The behavior of `update` depends on the metric type. Counters add the input value to the current
 * count, gauges replace the current value with the input value, frequencies increment the
 * occurrence count for the input string, histograms record the input value in the appropriate
 * bucket, and summaries record the input value as a new observation.
 *
...
/**
 * Returns a new metric that is powered by this one, but which accepts updates of the specified new
 * type, which must be transformable to the input type of this metric.
 *
...
/**
 * Returns a new metric that is powered by this one, but which accepts updates of any type, and
 * translates them to updates with the specified constant update value.
 *
 ...
 * **Details**
 *
 * Attributes are key-value pairs that provide additional context for metrics, enabling filtering,
 ...
 * **Details**
 *
 * Returns an array of metric snapshots, each containing the metric's metadata (name, description,
 * type) and current state (values, counts, etc.).
 *
...
/**
 * Returns a human-readable string representation of all currently registered metrics in a tabular
 * format.
 *
 * **Details**
 *
 * This debugging utility captures a snapshot of all metrics and formats them in an easy-to-read
 * table showing names, descriptions, types, attributes, and current state values.
 *
 ...
 * **When to use**
 *
 * Use to read metric snapshots from an explicit `Context` in low-level integrations, exporters, or
 * debugging tools that already have the context.
 *
 * **Details**
 *
 * This is the "unsafe" version that bypasses Effect's safety guarantees and requires manual
 * handling of the services context. Use the safe `snapshot` function for normal application code.
 *
 ...
 * **Details**
 *
 * Processes any iterable of numbers by removing duplicates, filtering out non-positive values, and
 * automatically appending positive infinity as the final boundary.
 *
 ...
 * **Details**
 *
 * Generates `count - 1` finite boundaries using `start + width + index` for each zero-based index,
 * then applies the same normalization as `boundariesFromIterable`: non-positive values are removed,
 * duplicates are collapsed, and `Infinity` is appended.
 *
 ...
 * **Details**
 *
 * Creates boundaries that grow exponentially, useful for metrics that span multiple orders of
 * magnitude. Each boundary is calculated as start * factor^i.
 *
 ...
 * **When to use**
 *
 * Use to provide or inspect the service that receives fiber start and end notifications for
 * automatic runtime metrics.
 *
 * **Details**
 *
 * When provided, the runtime can notify the service about child-fiber start and end events. When
 * the reference is `undefined`, automatic fiber runtime metric collection is disabled.
 *
 ...
 *     yield* Effect.all(
 *       [Effect.sleep("100 millis"), Effect.sleep("200 millis"), Effect.sleep("300 millis")],
...
/**
 * Layer that enables automatic collection of fiber runtime metrics across an entire Effect
 * application.
 *
 * **When to use**
 *
 * Use when you need runtime metrics collection for all Effects in the application context rather
 * than wrapping individual Effects.
 *
 ...
 *   const [users, analytics] = yield* Effect.all([userService, analyticsService], {
 *     concurrency: 2,
 *   });
 *
 *   yield* Console.log(
 ...
 * @category Metrics
 */
export const enableRuntimeMetricsLayer =
  Layer.succeed(FiberRuntimeMetrics)(FiberRuntimeMetricsImpl);
 ...
 * **Details**
 *
 * When enabled, automatically tracks fiber lifecycle metrics including active fibers, started
 * fibers, successful completions, and failures. These metrics provide valuable insights into the
 * concurrency patterns and health of your Effect application.
 *
 ...
 *       }).pipe(
 *         Effect.catchTag("RuntimeMetricsError", () => Effect.succeed(`Task ${i} failed`)),
 *       ),
 *     );
 *
 ...
 *   const runtimeMetrics = allMetrics.filter(
 *     (m) => m.id.startsWith("child_fiber") || m.id.includes("fiber"),
 *   );
 *
...
export const enableRuntimeMetrics: <A, E, R>(self: Effect<A, E, R>) => Effect<A, E, R> =
  InternalEffect.provideService(FiberRuntimeMetrics, FiberRuntimeMetricsImpl);
 ...
 * **When to use**
 *
 * Use when you need to disable runtime metrics for a specific effect while keeping them enabled
 * elsewhere.
 *
 ...
 * @category Metrics
 */
export const disableRuntimeMetrics: <A, E, R>(self: Effect<A, E, R>) => Effect<A, E, R> =
  InternalEffect.provideService(FiberRuntimeMetrics, undefined);
