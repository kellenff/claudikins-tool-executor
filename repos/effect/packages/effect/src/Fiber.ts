/**
 * Operations for handles returned by forking effects. A `Fiber<A, E>` is a lightweight execution of
 * an `Effect` that may still be running or may already have completed. This module lets callers
 * await or join fiber results, interrupt one or many fibers, check unknown values, access the
 * current fiber, and attach manually managed fibers to a `Scope` for cleanup.
 *
 * @since 2.0.0
 */
...
/**
 * A runtime fiber is a lightweight thread that executes Effects. Fibers are the unit of concurrency
 * in Effect. They provide a way to run multiple Effects concurrently while maintaining structured
 ...
 * **Details**
 *
 * A fiber exposes both safe Effect-based operations, such as {@link await_ await}, {@link join},
 * and {@link interrupt}, and low-level runtime fields used by the scheduler and runtime internals.
 *
 * **Gotchas**
 *
 * Prefer the exported functions in this module over calling `interruptUnsafe` or `pollUnsafe`
 * directly. The unsafe methods are immediate runtime hooks and do not provide the same Effect-based
...
/**
 * The Fiber namespace contains utility types and functions for working with fibers. It provides
 * type-level utilities for fiber operations and variance encoding.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The namespace currently exposes type-level support used by the `Fiber` interface. Runtime
 * operations are exported as module-level functions.
 *
...
export declare namespace Fiber {
  /**
   * Variance encoding for the Fiber type, specifying covariance in both the success type `A` and
   * the error type `E`.
   *
   * **When to use**
   *
   * Use to carry the success and error type parameters for `Fiber` in Effect's type machinery.
   *
...
export {
   ...
   * **When to use**
   *
   * Use when you need to inspect whether the fiber succeeded, failed, died, or was interrupted
   * without propagating the failure.
   *
   * **Details**
   *
   * The returned Effect always succeeds with an `Exit` describing the fiber's outcome.
   *
   * **Gotchas**
   *
   * This does not flatten the fiber result into the current Effect. Use {@link join} when you want
   * fiber failures to fail the current Effect.
   *
...
/**
 * Waits for all fibers in the provided iterable to complete and returns an array of their exit
 * values.
 *
 * **When to use**
 *
 ...
 * **Gotchas**
 *
 * Failures are captured as `Exit.Failure` values. Use {@link joinAll} when you want the first
 * failed fiber to fail the returned Effect.
 *
...
/**
 * Joins a fiber, blocking until it completes. If the fiber succeeds, returns its value. If it
 * fails, the error is propagated.
 *
 * **When to use**
 *
 * Use when you need a forked fiber's failure to fail the current Effect because that fiber is part
 * of the current workflow.
 *
 * **Gotchas**
 *
 * Joining a failed fiber propagates the fiber's Cause. Use {@link await_ await} when you need to
 * inspect the `Exit` instead of failing.
 *
 ...
 * **When to use**
 *
 * Use when you need every fiber to succeed and want the successful values rather than the `Exit`
 * values.
 *
 * **Details**
 *
 * If any fiber fails, the returned `Effect` fails with that fiber's cause and stops waiting for
 * additional results. This does not interrupt the remaining fibers.
 *
 * **Gotchas**
 *
 * A failure stops waiting, but it does not interrupt any other fibers. Use {@link interruptAll}
 ...
 * **When to use**
 *
 * Use when you need to cancel a forked fiber and wait for its cleanup to complete.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * Interruption is cooperative. A fiber can continue running while it is inside uninterruptible work
 * or finalizers.
 *
...
/**
 * Interrupts a fiber with a specific fiber ID as the interruptor. This allows tracking which fiber
 ...
 * **Gotchas**
 *
 * The supplied ID affects the recorded interruptor. It does not make interruption synchronous or
 * force uninterruptible regions to stop early.
 *
...
/**
 * Interrupts all fibers in the provided iterable, causing them to stop executing and clean up any
 * acquired resources.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The current fiber is recorded as the interruptor. The returned Effect completes only after all
 * interrupted fibers have completed.
 *
 * **Gotchas**
 *
 * Interruption is cooperative for each fiber. The returned Effect can wait for uninterruptible work
 * and finalizers in any interrupted fiber.
 *
...
/**
 * Interrupts all fibers in the provided iterable using the specified fiber ID as the interrupting
 * fiber. This allows you to control which fiber is considered the source of the interruption, which
 * can be useful for debugging and tracing.
 *
 * **When to use**
 *
...
/**
 * Checks whether a value is a Fiber. This is a type guard that can be used to determine if an
 * unknown value is a Fiber instance.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The check looks for the internal Fiber type ID marker and does not inspect the fiber's current
 * state.
 *
 ...
 * **When to use**
 *
 * Use when you need low-level runtime integrations that need access to the currently executing
 * fiber.
 *
 * **Gotchas**
 *
 * This is a synchronous accessor, not an Effect. It returns `undefined` outside an active fiber
 * runtime context.
 *
 ...
 * **Details**
 *
 * When the scope is closed, the fiber is interrupted. If the scope is already closed, the fiber is
 * interrupted immediately.
 *
 * **Gotchas**
 *
 * This does not wait for the fiber to complete. It only registers the interruption finalizer and
 * returns the same fiber.
 *
 * @since 4.0.0
