 ...
 *
 * A `FiberMap<K, A, E>` owns a map of running fibers, interrupts them when its scope closes, and
 * automatically removes each entry when the corresponding fiber completes. Use it when a program
 * needs to start, replace, join, or interrupt background work by a stable key while keeping all
 * fibers tied to one scope.
 *
 * @since 2.0.0
 */
...
/**
 * A FiberMap is a collection of fibers, indexed by a key. When the associated Scope is closed, all
 * fibers in the map will be interrupted. Fibers are automatically removed from the map when they
 * complete.
 *
 ...
 * **Details**
 *
 * When the associated Scope is closed, all fibers in the map will be interrupted. You can add
 * fibers to the map using `FiberMap.set` or `FiberMap.run`, and the fibers will be automatically
 * removed from the `FiberMap` when they complete.
 *
 ...
 * **Details**
 *
 * Each call stores the forked fiber under the supplied key and returns that fiber. If the key
 * already has a fiber, the previous fiber is interrupted unless `onlyIfMissing` is set. All managed
 * fibers are interrupted when the map's scope closes.
 *
...
/**
 * Creates a scoped run function that forks effects into a new `FiberMap` and returns a `Promise`
 * for each effect result.
 *
 * **When to use**
 *
 * Use when keyed fibers must be managed in a scoped map while exposing their results through
 * Promise-based APIs.
 *
 * **Details**
 *
 * Each call stores the fiber under the supplied key, interrupting any previous fiber for that key
 * unless `onlyIfMissing` is set. The returned Promise resolves with the effect's success value or
 ...
 * **When to use**
 *
 * Use when an existing forked fiber must be installed under a key immediately and synchronous
 * interruption of the replaced fiber is acceptable.
 *
 * **Details**
 *
 * When the fiber completes, it is removed from the map. If the key already has a fiber, that
 * previous fiber is interrupted unless `onlyIfMissing` is set; in that case the new fiber is
 * interrupted and the existing entry is kept.
 *
 ...
 * **When to use**
 *
 * Use when synchronous keyed lookup of a fiber in a `FiberMap` is needed and an `Option` result is
 * enough outside the Effect workflow.
 *
 ...
 * **Details**
 *
 * When the fiber completes, it is removed from the map. If the key already has a fiber, the
 * previous fiber is interrupted unless `onlyIfMissing` is set.
 *
...
/**
 * Captures the current runtime and returns a function for running effects in an existing `FiberMap`
 * as Promises.
 *
 * **Details**
 *
 * Each call stores the forked fiber under the supplied key, interrupting any previous fiber for
 * that key unless `onlyIfMissing` is set. The Promise resolves with the effect's success value or
 * rejects with the squashed failure cause.
 *
 ...
 * **Details**
 *
 * The returned Effect fails with the first managed fiber failure that is not ignored by the map's
 * interruption rules. Normal successful completion removes fibers from the map; use `awaitEmpty` to
 * wait until the map has no fibers.
 *
