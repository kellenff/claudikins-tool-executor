 ...
 *
 * The `Redactable` module provides a protocol for objects that need to present alternative
 * representations of themselves depending on the runtime context. Typical use cases include masking
 * secrets, tokens, or personal data in logs, traces, and serialized output.
 *
 * @since 4.0.0
 */
...
/**
 * Defines the symbol used to identify objects that implement the {@link Redactable} protocol.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Add a method under this key to make an object redactable. The method receives the current
 * `Context` and must return the replacement value. The symbol is registered globally via
 * `Symbol.for("~effect/Redactable")`, so it is identical across multiple copies of the library at
 * runtime.
 *
 ...
 * **When to use**
 *
 * Use to define classes or objects that hold sensitive data and should present a sanitized form
 * when inspected or logged.
 *
 * **Details**
 *
 * The `[symbolRedactable]` method receives the current fiber's `Context`. If no fiber is active, an
 * empty `Context` is provided.
 *
 * **Example** (Masking an API key)
 *
 * ```ts
...
/**
 * Type guard that checks whether a value implements the {@link Redactable} interface.
 *
 * **When to use**
 *
...
/**
 * Returns a redacted value if it implements {@link Redactable}, otherwise returns it unchanged.
 *
 * **When to use**
 *
 * Use as the general-purpose entry point for redaction when the input may or may not implement the
 * redaction protocol.
 *
 * **Details**
 *
 * This function calls {@link isRedactable} and, when it returns `true`, delegates to
 * {@link getRedacted}.
 *
 * **Gotchas**
 *
 * Redaction is not recursive. Nested redactable values inside the returned object are not
...
/**
 * Returns the result of calling `[symbolRedactable]` on a value that is already known to be
 * {@link Redactable}.
 *
 * **When to use**
 *
 * Use when you need to read the redacted representation from a value already verified as
 * `Redactable`.
 *
 * **Details**
 *
 * This function reads the current fiber's `Context` from the global fiber reference and passes it
 * to the redaction method.
 *
 * **Gotchas**
 *
