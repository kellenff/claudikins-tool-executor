 ...
 *
 * A `HashMap<Key, Value>` hashes keys and resolves matches with Effect's structural equality rules.
 * Lookup, insertion, removal, and transformation operations return new maps, while temporary
 * mutation helpers support efficient batch updates. This module also includes constructors,
 * iteration, conversion, mapping, filtering, and reducing helpers.
 *
 * @since 2.0.0
 */
...
/**
 * A HashMap is an immutable key-value data structure that provides efficient lookup, insertion, and
 * deletion operations. It uses a Hash Array Mapped Trie (HAMT) internally for structural sharing
...
/**
 * The HashMap namespace contains type-level utilities and helper types for working with HashMap
 * instances.
 *
...
export declare namespace HashMap {
  /**
   * A function that updates a value based on its current state. Takes an Option representing the
   * current value and returns an Option representing the new value.
   *
   ...
   */
  export type Key<T extends HashMap<any, any>> = [T] extends [HashMap<infer _K, infer _V>]
    ? _K
    : never;
   ...
   * @category Utility types
   */
  export type Value<T extends HashMap<any, any>> = [T] extends [HashMap<infer _K, infer _V>]
    ? _V
    : never;
...
/**
 * Looks up the value for the specified key in the `HashMap` safely using the internal hashing
 * function.
 *
...
/**
 * Looks up the value for the specified key in the `HashMap` unsafely using the internal hashing
 * function.
 *
 * **When to use**
 *
 * Use when reading from a `HashMap` by a key known to exist, and throwing is an acceptable
 * programming error for a missing key.
 *
 * **Gotchas**
 *
 * This function throws an error if the key is not found. Use `HashMap.get` for safe access that
 * returns `Option`.
 *
 ...
 * **Details**
 *
 * Apply updates to the returned map, then call `endMutation` to finish the mutation window and use
 * the result as an immutable `HashMap`.
 *
...
/**
 * Runs a batch of updates against a transient mutable copy of the `HashMap` and returns the
 * finalized immutable result.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * The update function receives `Some(value)` when the key exists or `None` when it does not.
 * Returning `Some(newValue)` stores the value, and returning `None` removes the key or leaves it
 * absent.
 *
 ...
 * **Details**
 *
 * Entries from `that` are inserted into `self`; when both maps contain an equal key, the value from
 * `that` replaces the value from `self`.
 *
...
/**
 * Maps over the entries of the `HashMap` using the specified filter and keeps only successful
 * results.
 *
