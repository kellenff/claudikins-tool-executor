 ...
 *
 * A `HashSet<A>` contains at most one value for each equality class according to Effect's `Equal`
 * and `Hash` rules. Membership checks, additions, removals, and set operations return new sets.
 * This module also includes constructors, union, intersection, difference, subset checks, mapping,
 * filtering, and reducing helpers.
 *
 * @since 2.0.0
 */
...
/**
 * A HashSet is an immutable set data structure that provides efficient storage and retrieval of
 * unique values. It uses a HashMap internally for optimal performance.
 *
...
/**
 * The HashSet namespace contains type-level utilities and helper types for working with HashSet
 * instances.
 *
...
/**
 * Reduces the HashSet to a single value by iterating through the values and applying an accumulator
 * function.
 *
