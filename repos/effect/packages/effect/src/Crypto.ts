 ...
 *
 * Runtime packages provide concrete implementations backed by the host platform's cryptography
 * APIs. This module defines the service interface and a constructor from random-byte and digest
 * primitives. The service provides secure random bytes and numbers, UUIDv4 and UUIDv7 generation,
 * shuffling, and SHA message digests.
 *
 * @since 4.0.0
 */
 ...
 * **Gotchas**
 *
 * SHA-1 is included for interoperability with existing protocols. Do not use SHA-1 for new
 ...
 * **Details**
 *
 * `Crypto` implementations must use cryptographically secure platform APIs. The random generator
 * helpers are derived by the `make` constructor from the random methods on this service.
 *
...
export interface Crypto {
  ...
  /**
   * Generates a random integer in the range Number.MIN_SAFE_INTEGER to Number.MAX_SAFE_INTEGER
   * (both inclusive).
   */
  ...
  /**
   * Generates a cryptographically secure random integer between `Number.MIN_SAFE_INTEGER` and
   * `Number.MAX_SAFE_INTEGER` (both inclusive).
   */
  readonly randomInt: Effect.Effect<number>;

  /**
   * Generates a cryptographically secure random number between `min` (inclusive) and `max`
   * (exclusive).
   */
   ...
   * **Details**
   *
   * The lower bound is rounded up with `Math.ceil` and the upper bound is rounded down with
   * `Math.floor`. By default the range is inclusive; set `options.halfOpen: true` to exclude the
   * upper bound.
   */
  randomIntBetween(
    min: number,
 ...
 * **When to use**
 *
 * Use when you need to provide or retrieve the full platform cryptography service from an effect's
 * context.
 *
 * **Details**
 *
 * Providing this service supplies platform-agnostic cryptographic operations such as hashing, UUID
 * generation, and secure random values.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Creates a `Crypto` service from the primitive implementation, deriving the random generator
 * helpers and UUID generation from those primitives.
 *
 * **When to use**
 *
 * Use to build a Crypto service for a platform integration, test layer, or custom runtime from
 * primitive random-byte and digest operations.
 *
 * **Details**
 *
 * The constructor derives random numbers, booleans, integer ranges, shuffling, and UUID generation
 * from `impl.randomBytes`. Digest operations delegate to `impl.digest`.
 *
 * **Gotchas**
 *
 * `impl.randomBytes` must return cryptographically secure bytes of the requested length. UUID
 * formatting mutates the byte array returned for UUID generation, so the implementation should
 * return a fresh array for each call.
 *
