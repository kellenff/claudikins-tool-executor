 ...
 *
 * A `Pool<A, E>` acquires resource-backed values with a scoped effect, lets fibers borrow them with
 * `get`, can invalidate broken values, and releases all acquired values when the pool scope closes.
 * This module includes fixed-size pools, pools that resize with a time-to-live policy, custom
 * strategy pools, per-item concurrency limits, and runtime state types used by pool strategies.
 *
 * @since 2.0.0
 */
...
/**
 * A `Pool<A, E>` is a pool of items of type `A`, each of which may be associated with the
 * acquisition and release of resources. An attempt to get an item `A` from a pool may fail with an
 * error of type `E`.
 *
 * **When to use**
 *
 * Use when you need to share a bounded set of scoped resources across fibers while the pool manages
 * acquisition, reuse, and release.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use as the normalized, read-only description of how a pool acquires, sizes, shares, and resizes
 * its items after construction.
 *
 * **Details**
 *
 * The config stores the acquire effect, size bounds, per-item concurrency, target utilization, and
 * resizing strategy used by the pool implementation.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when you need to inspect or support the runtime state backing a `Pool`, including its scope,
 * item sets, semaphores, waiters, invalidation tracking, and shutdown flag.
 *
 * **Details**
 *
 * This state tracks the pool scope, active and available items, invalidated items, semaphores,
 * waiters, and shutdown status. It is exposed for inspection and implementation support; user code
 ...
 * **When to use**
 *
 * Use when implementing a custom pool `Strategy` that needs to inspect acquired items, track
 * reference counts, or return reclaimable items to the pool.
 *
 * **Details**
 *
 * Each item stores the acquisition `Exit`, its finalizer, the current reference count, and whether
 * automatic reclaiming has been disabled because the item was invalidated.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when defining a custom pool lifecycle policy that needs to run background work, observe
 * acquired items, or choose items for reclamation.
 *
 * **Details**
 *
 * `run` starts any strategy-specific background work, `onAcquire` is invoked when an item is
 * acquired, and `reclaim` selects an item that can be removed or replaced.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * The pool is returned in a `Scope`, which governs the lifetime of the pool. When the pool is
 * shutdown because the `Scope` is closed, the individual items allocated by the pool will be
 * released in some unspecified order.
 *
 * By setting the `concurrency` parameter, you can control the level of concurrent access per pool
 * item. By default, the number of permits is set to `1`.
 *
 * `targetUtilization` determines when to create new pool items. It is a value between 0 and 1,
 * where 1 means only create new pool items when all the existing items are fully utilized.
 *
...
/**
 * Creates a scoped pool with minimum and maximum sizes and a time-to-live policy for shrinking
 * unused excess items.
 *
 * **When to use**
 *
 * Use to create an elastic scoped pool that can grow up to a maximum size and later reclaim unused
 * excess items.
 *
 * **Details**
 *
 * The returned pool requires `Scope`; when that scope is closed, allocated items are released in an
 * unspecified order. `concurrency` controls how many fibers may use each pool item at once and
 * defaults to `1`.
 *
 * `targetUtilization` controls when new items are created and is clamped by the pool
 * implementation. A value of `1` waits until existing items are fully utilized before creating more
 * items.
 *
 * `timeToLiveStrategy` controls when excess items expire: `"creation"` measures from item creation,
 * while `"usage"` measures from pool usage. The default is `"usage"`.
 *
 ...
 * **When to use**
 *
 * Use to build a pool whose item lifecycle is controlled by an explicit `Strategy`, such as custom
 * background resizing, replacement, or reclamation.
 *
 * **Details**
 *
 * The returned pool requires `Scope`; closing the scope shuts down the pool and releases allocated
 * items.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to borrow a pooled resource for the lifetime of the current scope so it is automatically
 * returned when that scope closes.
 *
 * **Details**
 *
 * The returned effect waits for an available item when the pool is at capacity. If acquiring a new
 * item fails, the effect fails with the acquisition error.
 *
 * **Gotchas**
 *
...
/**
 * Invalidates the specified item so the pool can remove it and reallocate the item, lazily if
 * needed.
 *
 * **When to use**
 *
 * Use to prevent a pooled item from being reused after it becomes unsuitable, such as a stale
 * connection or a resource that failed a health check.
 *
 * **Gotchas**
 *
 * The item is matched with strict equality. Passing an equivalent but different object instance
 * does nothing.
 *
 * @since 2.0.0
  ...
  Effect.suspend(() => {
    ...
    return self.config.strategy.reclaim(self).pipe(
      ...
      Effect.flatMap((items) =>
        items.some((_) => _.exit._tag === "Failure") ? Effect.void : resizeLoop(self),
      ),
    );
  });
