 ...
 *
 * A `TxSubscriptionRef<A>` combines a `TxRef<A>` for the current value with a transactional pub/sub
 * channel for updates. Subscribers first receive the current value and then every later value that
 * is published by committed updates. This module includes constructors, reads, writes, update and
 * modify helpers, transactional-queue subscriptions, stream subscriptions, and a guard.
 *
 * @since 4.0.0
 */
...
/**
 * A TxSubscriptionRef is a transactional reference that allows subscribing to all committed
 * changes. Subscribers receive the current value followed by every subsequent update via a
...
/**
 * Modifies the value of the TxSubscriptionRef using a function that returns both a result and the
 * new value. The new value is published to all subscribers atomically.
 *
 * **When to use**
 *
 * Use to compute a separate return value and next `TxSubscriptionRef` state in one transactional
 * update.
 *
...
/**
 * Updates the value of the TxSubscriptionRef using a function and publishes the new value to all
 * subscribers.
 *
 * **When to use**
 *
...
/**
 * Gets the current value and sets a new value atomically. Publishes the new value to all
 * subscribers.
 *
 * **When to use**
 *
 * Use to replace a `TxSubscriptionRef` value while returning the previous value and publishing the
...
/**
 * Gets the current value and updates it using a function atomically. Publishes the new value to all
 * subscribers.
 *
 * **When to use**
 *
...
/**
 * Updates the value using a function and returns the new value. Publishes the new value to all
 * subscribers.
 *
 * **When to use**
 *
...
/**
 * Subscribes to all changes of the TxSubscriptionRef. Returns a scoped TxDequeue that first yields
 * the current value, then every subsequent update.
 *
 * **When to use**
 *
...
/**
 * Returns a Stream of all changes to the TxSubscriptionRef, starting with the current value
...
export const changesStream = <A>(self: TxSubscriptionRef<A>): Stream.Stream<A, never, never> =>
  Stream.unwrap(
    Effect.map(changes(self), (sub) => Stream.fromEffectRepeat(Effect.tx(TxQueue.take(sub)))),
  );
 ...
 */
export const isTxSubscriptionRef = (u: unknown): u is TxSubscriptionRef<unknown> =>
