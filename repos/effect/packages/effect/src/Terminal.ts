/**
 * Service definition for interactive terminal capabilities. Programs can query terminal dimensions,
 * read a line of input, receive low-level key events, and display text without depending directly
 * on a specific platform implementation.
 *
 * This module defines the `Terminal` service, input event shapes, key metadata, the `QuitError`
 * used when a user cancels input, a guard for that error, and a constructor for custom terminal
...
/**
 * A `Terminal` represents a command-line interface which can read input from a user and display
 * messages to a user.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when consuming low-level terminal input events from `Terminal.readInput` and you need both
 * raw character input and parsed key metadata.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Represents an error that occurs when a user attempts to quit out of a `Terminal` prompt for input
 * (usually by entering `ctrl`+`c`).
 *
 * **When to use**
 *
 * Use when implementing terminal input or prompts that need to signal user-requested cancellation
 ...
 * **Details**
 *
 * Returns `true` when the value carries the `QuitError` runtime marker and narrows it to
 * `QuitError`.
 *
 * @since 4.0.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use to access or provide platform terminal capabilities such as reading input, writing output,
 * and inspecting terminal dimensions.
 *
 * @since 4.0.0
 * @category Services
 */
 ...
 * **When to use**
 *
 * Use to construct a custom `Terminal` service implementation from concrete terminal capabilities
 * when writing a platform adapter, test implementation, or custom runtime service.
 *
 * **Details**
 *
 * The implementation object supplies `columns`, `rows`, `readInput`, `readLine`, and `display`;
 * `make` attaches the `Terminal` service marker so the result can be provided through the
