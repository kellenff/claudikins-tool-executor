/**
 * Reference-counted handles for sharing one scoped resource across many scoped users. An `RcRef<A,
 * E>` acquires the resource lazily the first time `get` needs it, reuses that value while it is
 * borrowed or kept idle, and finalizes it when the final borrowing scope closes unless an idle
 * timeout keeps it available. The module also provides `invalidate` for forcing the next `get` to
 * acquire a fresh resource.
 *
 * @since 3.5.0
 */
 ...
 * **When to use**
 *
 * Use to share a scoped resource across active users with reference-counted acquisition and
 * release.
 *
 * **Details**
 *
 * An RcRef wraps a resource that can be acquired and released multiple times. The resource is
 * lazily acquired on the first call to `get` and automatically released when the last reference is
 * released.
 *
...
export declare namespace RcRef {
   ...
   * **When to use**
   *
   * Use to carry the value and error type parameters for `RcRef` in Effect's type machinery.
   *
   * **Details**
   *
   * This interface records the covariant value and error types carried by an `RcRef`. It is used by
   * Effect's type machinery and is not normally referenced directly by users.
   *
   * @since 3.5.0
   * @category Models
   */
 ...
 * **Details**
 *
 * The resource is acquired lazily on the first `get` and shared by subsequent gets while it remains
 * cached. Each `get` adds a reference to the current `Scope`. When the last reference is released,
 * the resource is closed immediately by default, or after `idleTimeToLive` when that option is
 * provided.
 *
 ...
 * **Details**
 *
 * The reference count is incremented for the current `Scope`, and a release finalizer is added to
 * that scope. When the current scope closes, the reference is released; the resource is closed when
 * the final reference is released, subject to any configured idle time-to-live.
 *
 ...
 *   const ref = yield* RcRef.make({
 *     acquire: Effect.acquireRelease(Effect.succeed("shared resource"), (resource) =>
 ...
 * **When to use**
 *
 * Use to force future `RcRef.get` calls to acquire a fresh resource when the currently cached
 * resource should no longer be reused.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * Invalidation does not revoke resources already borrowed by active scopes; those remain usable
 * until their scopes close.
 *
 * @since 3.19.6
 * @category Combinators
