 ...
 *
 * A `TxHashMap` stores an immutable `HashMap` in a `TxRef`, so map reads and writes can commit
 * atomically with other transactional operations. Use it for shared registries, counters, indexes,
 * and other maps that need safe read-modify-write sequences alongside related transactional state.
 *
 * @since 2.0.0
 */
...
/**
 * A TxHashMap is a transactional hash map data structure that provides atomic operations on
 * key-value pairs within Effect transactions. It uses an immutable HashMap internally with TxRef
 * for transactional semantics, ensuring all operations are performed atomically.
 *
...
/**
 * The TxHashMap namespace contains type-level utilities and helper types for working with TxHashMap
 * instances.
 *
 ...
 * **Details**
 *
 * This function mutates the original TxHashMap by updating its internal state. It does not return a
 * new TxHashMap reference.
 *
 ...
 * **Details**
 *
 * This function mutates the original TxHashMap by removing the specified key-value pair. It does
 * not return a new TxHashMap reference.
 *
 ...
 * **Details**
 *
 * This function mutates the original TxHashMap by clearing all key-value pairs. It does not return
 * a new TxHashMap reference.
 *
...
/**
 * Updates the value for the specified key if it exists, returning the previous value in `Some`;
 * returns `None` and leaves the map unchanged when the key is absent.
 *
 * **Details**
 *
 * This function mutates the original TxHashMap by updating the value at the specified key. It does
 * not return a new TxHashMap reference.
 *
 ...
 * **Details**
 *
 * This function mutates the original TxHashMap by updating, adding, or removing the key-value pair
 * based on the function result. It does not return a new TxHashMap reference.
 *
...
/**
 * Merges another HashMap into this TxHashMap. If both maps contain the same key, the value from the
 * other map will be used.
 *
 * **Details**
 *
 * This function mutates the original TxHashMap by merging the provided HashMap into it. It does not
 ...
 * **Details**
 *
 * This function mutates the original TxHashMap by removing all specified keys. It does not return a
 * new TxHashMap reference.
 *
 ...
 * **Details**
 *
 * This function mutates the original TxHashMap by setting all provided key-value pairs. It does not
 * return a new TxHashMap reference.
 *
 ...
 * **Gotchas**
 *
 * The supplied hash must be the hash for the same key, such as a precomputed `Hash.hash(key)`
 * value. If the hash does not match the key, an existing entry may not be found.
 *
 ...
 * **Details**
 *
 * This function returns a new TxHashMap reference with the transformed values. The original
 * TxHashMap is not modified.
 *
 ...
 * **Details**
 *
 * This function returns a new TxHashMap reference containing only the entries that match the
 * condition. The original TxHashMap is not modified.
 *
...
/**
 * Reduces the TxHashMap entries to a single value by applying a reducer function. Iterates over all
 * key-value pairs and accumulates them into a final result.
 *
...
/**
 * Combines filtering and mapping in a single operation. Applies a filter to each entry, keeping
 * only successful results and transforming them.
 *
 * **Details**
 *
 * This function returns a new TxHashMap reference containing only the transformed entries that
 * succeeded. The original TxHashMap is not modified.
 *
 ...
 *   const hasAdminUser = yield* TxHashMap.hasBy(userStatuses, (user, username) =>
...
/**
 * Finds the first entry in the TxHashMap that matches the given predicate. Returns the key-value
 * pair as a tuple wrapped in an Option.
 *
...
/**
 * Executes a side-effect function for each entry in the TxHashMap. The function receives the value
 * and key as parameters and can perform effects.
 *
 ...
 * **Details**
 *
 * This function returns a new TxHashMap reference with the flattened results. The original
 * TxHashMap is not modified.
 *
 ...
 * **Details**
 *
 * This function returns a new TxHashMap reference with only the Some values unwrapped. The original
 * TxHashMap is not modified.
 *
 ...
 *   const sortedAgeEntries = ageEntries.toSorted(([left], [right]) =>
...
/**
 * Returns an array of all key-value pairs in the TxHashMap. This is an alias for the `entries`
 * function, providing API consistency with HashMap.
 *
...
/**
 * Returns an array of all values in the TxHashMap. This is an alias for the `values` function,
 * providing API consistency with HashMap.
 *
