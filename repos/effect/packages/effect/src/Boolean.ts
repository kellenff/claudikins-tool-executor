 ...
 *
 * This module exposes the native `Boolean` constructor together with helpers for checking values,
 * choosing between lazy branches, combining booleans with logical operations, checking collections
 * with `every` or `some`, ordering booleans, and reducing boolean values.
 *
 * @since 2.0.0
 */
 ...
 * **Gotchas**
 *
 * This follows native truthiness rules. For example, non-empty strings such as `"false"` coerce to
 * `true`.
 *
...
/**
 * Provides an `Order` instance for `boolean` that allows comparing and sorting boolean values. In
 * this ordering, `false` is considered less than `true`.
 *
 * **When to use**
 *
 * Use when you need to sort or compare boolean values through APIs that accept an ordering instance
 * where `false` comes before `true`.
 *
 ...
 * **Gotchas**
 *
 * `combineAll` uses the default left-to-right `Reducer.make` fold and does not short-circuit on
 * `false`.
 *
 * @since 4.0.0
 * @category Math
