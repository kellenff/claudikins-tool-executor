 ...
 *
 * A `Resource<A, E>` keeps the latest successful or failed acquisition result. It can be read
 * repeatedly, refreshed manually, or refreshed automatically on a schedule. Resource acquisition
 * runs in a scope, so replacements and final cleanup release the resources owned by previous
 * values.
 *
 * @since 2.0.0
 */
...
/**
 * A `Resource` is a value loaded into memory that can be refreshed manually or automatically
 * according to a schedule.
 *
 * **When to use**
 *
 * Use to model a scoped value whose latest acquisition result is kept available for repeated reads
 * and can be refreshed manually or on a schedule.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when a resource should refresh in the background according to a schedule for the lifetime of
 * its scope.
 *
 * @since 2.0.0
 * @category Constructors
 ...
 * **Gotchas**
 *
 * If the resource currently stores a failed acquisition result, the returned effect fails with the
 * stored error.
 *
 * @since 2.0.0
 * @category Getters
 ...
 * **Details**
 *
 * When acquisition succeeds, refreshing replaces the value stored in the resource's scoped
 * reference and releases resources associated with the previous value.
 *
 * **Gotchas**
 *
 * If acquisition fails, the returned effect fails and the previously stored result is left as what
 * `get` reads.
 *
 * @since 2.0.0
