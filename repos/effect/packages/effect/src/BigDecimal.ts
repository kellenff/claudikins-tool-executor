/**
 * Decimal numbers and arithmetic for cases where JavaScript `number` rounding is not precise
 * enough. A `BigDecimal` stores digits as a `bigint` plus a decimal scale, which lets the module
 * parse, compare, add, subtract, multiply, divide, round, and format decimal values such as money,
 ...
 * **Details**
 *
 * Increasing the scale appends decimal zeros. Decreasing the scale discards digits beyond the
 * target scale by `bigint` division, which truncates toward zero.
 *
 ...
 * **When to use**
 *
 * Use when you need a decimal addition function for piping or higher-order APIs while preserving
 * decimal precision.
 *
 ...
 * **When to use**
 *
 * Use when you need to aggregate decimal quantities with decimal precision instead of converting
 ...
 * **Details**
 *
 * If the dividend is not a multiple of the divisor, the result will be a `BigDecimal` value with up
 * to the default division precision. If the divisor is `0`, the result will be `Option.none()`.
 *
 ...
 * console.log(
 *   Option.getOrThrow(
 *     BigDecimal.divide(BigDecimal.fromStringUnsafe("6"), BigDecimal.fromStringUnsafe("3")),
 *   ),
 * ); // BigDecimal(2)
 * console.log(
 *   Option.getOrThrow(
 *     BigDecimal.divide(BigDecimal.fromStringUnsafe("6"), BigDecimal.fromStringUnsafe("4")),
 *   ),
 * ); // BigDecimal(1.5)
 * console.log(
 *   Option.isNone(
 *     BigDecimal.divide(BigDecimal.fromStringUnsafe("6"), BigDecimal.fromStringUnsafe("0")),
 *   ),
 * ); // true
 * ```
 *
 ...
 * **When to use**
 *
 * Use when you need to divide `BigDecimal` values where the divisor is known to be non-zero, so
 * division by zero should be a thrown exception.
 *
 * **Details**
 *
 * If the dividend is not a multiple of the divisor, the result will be a `BigDecimal` value with up
 * to the default division precision.
 *
 * **Gotchas**
 *
...
/**
 * Provides an `Order` instance for `BigDecimal` that allows comparing and sorting BigDecimal
 * values.
 *
 * **When to use**
 *
 * Use when you need to sort or compare decimal values through APIs that accept an ordering
 * instance.
 *
...
/**
 * Restricts the given `BigDecimal` to be within the range specified by the `minimum` and `maximum`
 * values.
 *
 * **When to use**
 *
 ...
 * If it is greater than the `maximum` value, the function returns the `maximum` value. Otherwise,
 * it returns the original `BigDecimal`.
 *
 ...
 *
 * assert.deepStrictEqual(
 *   BigDecimal.abs(BigDecimal.fromStringUnsafe("-5")),
 *   BigDecimal.fromStringUnsafe("5"),
 * );
 * assert.deepStrictEqual(
 *   BigDecimal.abs(BigDecimal.fromStringUnsafe("0")),
 ...
 *
 * assert.deepStrictEqual(
 *   BigDecimal.negate(BigDecimal.fromStringUnsafe("3")),
 *   BigDecimal.fromStringUnsafe("-3"),
 * );
 * assert.deepStrictEqual(
 *   BigDecimal.negate(BigDecimal.fromStringUnsafe("-6")),
 *   BigDecimal.fromStringUnsafe("6"),
 * );
 * ```
 *
 ...
 * **When to use**
 *
 * Use when you need to compute a `BigDecimal` remainder with a divisor known to be non-zero and
 * want a plain `BigDecimal` result instead of an `Option`.
 *
 * **Gotchas**
 *
 * Throws a `RangeError` if the divisor is `0`.
...
/**
 * Provides an `Equivalence` instance for `BigDecimal` that determines equality between BigDecimal
 * values.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use when you need to convert a trusted finite JavaScript number to a `BigDecimal` and want a
 * plain result instead of an `Option`.
 *
 * **Gotchas**
 *
 * It is not recommended to convert a floating point number to a decimal directly, as the floating
 * point representation may be unexpected. Throws a `RangeError` if the number is not finite (`NaN`,
 ...
 *
 * It is not recommended to convert a floating point number to a decimal directly, as the floating
 * point representation may be unexpected.
 *
 ...
 * **Details**
 *
 * Returns `Option.some` for valid decimal or exponent notation and `Option.none` when the string
 * cannot be parsed or would produce an unsafe scale. The empty string parses as zero.
 *
 ...
 * **Details**
 *
 * Accepts the same syntax as `fromString`. Use `fromString` when invalid input should be
 * represented as `Option.none` instead of throwing.
 *
 ...
 * **Details**
 *
 * The value is normalized before formatting. Scientific notation is used when the absolute value of
 * the normalized scale is at least `16`; otherwise plain decimal notation is used.
 *
 ...
 * assert.deepStrictEqual(
 *   BigDecimal.format(BigDecimal.fromStringUnsafe("-0.00000123")),
 *   "-0.00000123",
 * );
 * ```
 *
 ...
 *
 * assert.deepStrictEqual(
 *   BigDecimal.toExponential(BigDecimal.make(123456n, -5)),
 *   "1.23456e+10",
 * );
 * ```
 *
 ...
 * **Gotchas**
 *
 * This conversion is unsafe because the result can lose integer or fractional precision, round to a
 * nearby representable value, or become `Infinity` when the decimal cannot be represented as a
 ...
 *
 * assert.deepStrictEqual(
 *   BigDecimal.toNumberUnsafe(BigDecimal.fromStringUnsafe("123.456")),
 *   123.456,
 * );
 * ```
 *
 ...
 * - `half-even`: round to the nearest neighbor; if equidistant round to the neighbor with an even
 *   digit
...
/**
 * Computes a truncated `BigDecimal` at the given scale. This removes fractional digits beyond the
 * scale, rounding toward zero.
 *
 * **When to use**
 *
 * Use when you need to discard fractional digits beyond a scale rather than round half up, half
 * down, or toward an infinity.
 *
 ...
 * **Details**
 *
 * The default scale is `0`. Positive scales keep digits to the right of the decimal point, and
 * negative scales round positions to the left of the decimal point.
 *
 * @since 3.16.0
 * @category Math
 * @see {@link floor} for rounding toward negative infinity
