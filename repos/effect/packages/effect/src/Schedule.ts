 ...
 *
 * A `Schedule<Output, Input, Error, Env>` is stepped with an input value. Each step either stops or
 * produces an output together with the delay before the next step. Schedules are used by retry,
 * repeat, stream, and channel APIs to decide when work should continue, how long to wait, and when
 * to stop.
 *
 * @since 2.0.0
 */
 ...
 * const retrySchedule = Schedule.max([Schedule.exponential("100 millis"), Schedule.recurs(3)]);
 *
 ...
 * **Details**
 *
 * Repeat, retry, stream, and channel scheduling operations provide this service to effects run
 * between schedule steps. The default value contains undefined input and output values, zero
 * duration, and zeroed timing fields before any schedule step has produced metadata.
 *
 * @since 4.0.0
 * @category Metadata
 */
...
export declare namespace Schedule {
   ...
   * const errorSchedule = Schedule.exponential("100 millis").pipe(Schedule.upTo({ times: 5 }));
   *
  ...
  /**
   * Type-level marker used by `Schedule.Variance` to record the variance of `Schedule` type
   * parameters.
   *
   * **Details**
   *
   * This interface exists for TypeScript inference and assignability. Users normally do not
 ...
 * @category Guards
 */
export const isSchedule = (u: unknown): u is Schedule<unknown, never, unknown, unknown> =>
...
/**
 * Extracts a step function from a `Schedule` that sleeps for each computed delay and returns
 * metadata for the completed step.
 *
 * **When to use**
 *
 * Use to drive a schedule manually while preserving the computed output, delay, input, attempt, and
 * elapsed timing metadata for each step.
 *
 * **Details**
 *
 * The returned step reads the current time from `Clock` when invoked, calls the schedule step with
 * that timestamp and input, sleeps for the returned duration, and then yields `Metadata`.
 *
 * @since 4.0.0
 * @category Destructors
...
/**
 * Returns a new `Schedule` that adds the delay computed by the specified effectful function to the
 ...
 *
 * class RetryAttemptError extends Data.TaggedError("RetryAttemptError")<{
 ...
 *   ({ output }) => Effect.succeed(Duration.millis(Duration.toMillis(output) * 0.25)),
 * );
 *
 ...
 *       Schedule.tap(({ output: delay }) => Console.log(`Base delay: ${delay}`)),
 *     ),
 *   );
 * });
 *
 ...
 *           new RetryAttemptError({ message: `Attempt ${attempt} failed` }),
 *         );
 *       }
...
export const addDelay: {
  ...
  <Output, Input, Error, Env, Error2 = never, Env2 = never>(
    ...
    modifyDelay(self, (metadata) =>
      effect.map(f(metadata), (d) => Duration.sum(Duration.fromInputUnsafe(d), metadata.duration)),
    ),
);
 ...
 * const quickRetries = Schedule.exponential("100 millis").pipe(Schedule.upTo({ times: 3 }));
 * const slowRetries = Schedule.exponential("1 second").pipe(Schedule.upTo({ times: 2 }));
 *
...
/**
 * Returns a schedule that runs `self` to completion, then runs `other`, and preserves which
 * schedule produced each output.
 *
 * **Details**
 *
 * The resulting schedule emits a `Result` to indicate which phase produced each output: outputs
 * from `self` are emitted as `Failure`, and outputs from `other` are emitted as `Success`.
 *
    ...
    fromStep(
      effect.sync(() => {
        ...
        > {
          ...
          return toStep<Result.Result<Output2, Output>, Input & Input2, Error | Error2, Env | Env2>(
...
/**
 * Combines schedules by recurring while all schedules want to recur, using the maximum delay
 * between recurrences and outputting that maximum delay.
 *
 * **When to use**
 *
 * Use when a combined policy should continue only while every schedule still recurs, and should
 * wait for the slowest schedule between recurrences.
 *
 ...
 * @category Combining
 */
export const max = <const Schedules extends NonEmptyReadonlyArray<Schedule<any, any, any, any>>>(
  schedules: Schedules,
): Schedule<
  Duration.Duration,
...
/**
 * Returns a new `Schedule` that recurs on the specified `Cron` schedule and outputs the duration
 * between recurrences.
 *
 ...
 * import { Console, Data, Effect, Schedule } from "effect";
 *
 * class ScheduledTaskError extends Data.TaggedError("ScheduledTaskError")<{
 *   readonly message: string;
 * }> {}
 *
 * // Run every minute
 ...
 *       Schedule.tap(({ output: duration }) => Console.log(`Next execution in: ${duration}`)),
 *     ),
 *   );
 * });
 *
 ...
 *       if (attempt === 1) {
 *         return yield* Effect.fail(
 *           new ScheduledTaskError({ message: "Scheduled task failed" }),
 *         );
 *       }
 *       return "success";
 *     }),
...
export const cron: {
  ...
  return fromStep(
    effect.map(effect.fromResult(parsed), (cron) => (now, _) => {
...
export const duration = (durationInput: Duration.Input): Schedule<Duration.Duration> => {
  ...
  return fromStepWithMetadata(
    effect.succeed((meta) =>
      meta.attempt === 1 ? effect.succeed([duration, duration]) : Cause.done(Duration.zero),
    ),
  );
};

/**
 * Returns a new `Schedule` that will always recur, but only during the specified `duration` of
 * time.
 *
 * **When to use**
 *
...
/**
 * Combines schedules by recurring while at least one schedule wants to recur, using the minimum
 * delay between recurrences and outputting that minimum delay.
 *
 * **When to use**
 *
 * Use when a combined policy should continue while any schedule still recurs, and should wait for
 * the fastest schedule between recurrences.
 *
 ...
 * @category Combining
 */
export const min = <const Schedules extends NonEmptyReadonlyArray<Schedule<any, any, any, any>>>(
  schedules: Schedules,
): Schedule<
  Duration.Duration,
  UnionToIntersection<Input<Schedules[number]>>,
...
/**
 * Schedule that always recurs, but will wait a certain amount between repetitions, given by `base *
 * factor.pow(n)`, where `n` is the number of repetitions so far. Returns the current duration
 * between recurrences.
 *
 ...
 * const retryPolicy = Schedule.max([Schedule.exponential("50 millis"), Schedule.recurs(5)]);
 *
 * const program = Effect.gen(function* () {
 *   let attempt = 0;
 *
...
/**
 * Schedule that always recurs, increasing delays by summing the preceding two delays (similar to
 * the Fibonacci sequence). Returns the current duration between recurrences.
 *
 ...
 *     ]).pipe(Schedule.tap(({ output: delay }) => Console.log(`Next retry in ${delay}`))),
 *   );
 *
 *   yield* Console.log(`Final result: ${result}`);
 * });
...
/**
 * Returns a `Schedule` that recurs on the specified fixed `interval` and outputs the number of
 ...
 * **Gotchas**
 *
 * If the action run between recurrences takes longer than the interval, the next recurrence happens
 * immediately, but missed intervals are not replayed.
 *
 * ```text
 ...
 *   return "healthy";
 * }).pipe(Effect.repeat(Schedule.fixed("2 seconds").pipe(Schedule.upTo({ times: 5 }))));
 *
...
/**
 * Returns a new `Schedule` that maps each schedule decision to a new output using the full schedule
 * metadata.
 *
 * **Details**
 *
 * The callback receives the schedule input, output, selected delay duration, current attempt, and
 * elapsed timing information. Return either a plain value or an `Effect` that produces the new
 * output.
 *
 ...
 *   Schedule.map(({ output: recurrence }) =>
 *     Effect.succeed({
 ...
 *         Console.log(`${info.phase} phase - iteration ${info.iteration}`),
 *       ),
 *     ),
 *   );
 *
        ...
        return (now, input) =>
          Pull.matchEffect(step(now, input), {
            onSuccess: ([output, duration]) => {
              ...
              if (!isEffect(result))
                return effect.succeed([result, duration] as [Output2, Duration.Duration]);
              return effect.map(
                result,
...
/**
 * Returns a new `Schedule` that modifies the delay of the next recurrence of the schedule using the
  ...
  <Output, Input, Error, Env, Error2 = never, Env2 = never>(
    ...
    fromStep(
      effect.map(toStep(self), (step) => {
        const meta = metadataFn();
        return (now, input) =>
          effect.flatMap(step(now, input), ([output, duration]) =>
 ...
 * **When to use**
 *
 * Use to add random variation to an existing schedule's recurrence delays while preserving its
 * output and completion behavior.
 *
 * **Details**
 *
...
/**
 * Returns a `Schedule` which can only be stepped the specified number of `times` before it
 * terminates.
 *
 * **When to use**
 *
 ...
 * **Gotchas**
 *
 * `recurs(n)` counts schedule recurrences, not the first evaluation of the effect being repeated or
 * retried. For retrying, this means one initial attempt plus at most `n` retries.
 *
...
/**
 * Returns a schedule that recurs continuously, each repetition spaced the specified duration from
 * the last run.
 *
 * **When to use**
 *
 ...
 * }).pipe(Effect.repeat(Schedule.spaced("1 second").pipe(Schedule.upTo({ times: 5 }))));
 *
...
/**
 * Returns a new `Schedule` that allows execution of an effectful function for every decision of the
 * schedule, but does not alter the inputs and outputs of the schedule.
 *
 * **Details**
 *
 * The callback receives the full schedule metadata, including the input, output, computed delay
 * duration, current attempt, and elapsed timing information.
 *
...
/**
 * Returns a new `Schedule` that limits an existing schedule by elapsed duration, number of outputs,
 * or both.
 *
 * **When to use**
 *
 * Use to bound an existing schedule while preserving its output and delay behavior. When both
 * `duration` and `times` are specified, the schedule stops as soon as either limit is reached.
 *
 * **Gotchas**
 *
 * The `times` option limits schedule outputs. When used with repeat or retry, the effect is
 * evaluated once before the schedule is stepped, so the total number of evaluations can be one
 * greater than the configured number of outputs.
 *
 * The `duration` option is based on the elapsed time observed by the schedule step. Long-running
 * effects can cause the duration limit to be detected on the following schedule step.
 *
 ...
 * const limitedHeartbeat = Schedule.spaced("1 second").pipe(Schedule.upTo({ times: 5 }));
 *
 ...
 * const unchanged = Schedule.fixed("500 millis").pipe(Schedule.upTo({}));
 * ```
 *
 * @since 4.0.0
 * @category Filtering
...
export const upTo: {
  ...
  <Output, Input, Error, Env>(
    ...
    const duration =
      options.duration === undefined ? undefined : Duration.fromInputUnsafe(options.duration);
    return while_(self, ({ attempt, elapsed }) =>
      effect.succeed(
        (options.times === undefined || attempt <= options.times) &&
          (duration === undefined ||
            Duration.isLessThanOrEqualTo(Duration.millis(elapsed), duration)),
      ),
    );
  },
);
...
export {
   ...
   * **When to use**
   *
   * Use to stop an existing schedule based on its full metadata, such as the current input, output,
   * attempt, delay, or elapsed time.
   *
   * **Details**
   *
   * The predicate receives `Metadata`, may return `boolean` or an `Effect<boolean, ...>`, preserves
   * the output and delay when it returns `true`, and stops the schedule when it returns `false`.
   *
   * @since 4.0.0
   * @category Filtering
...
/**
 * Schedule that divides the timeline to `interval`-long windows, and sleeps until the nearest
 * window boundary every time it recurs.
 *
 * **Details**
 *
...
export {
   ...
   * **Details**
   *
   * This schedule runs indefinitely, returning each input value as its output without modification.
   * It effectively acts as a pass-through that simply echoes its input values at each step.
   *
   * @since 2.0.0
   * @category Constructors
