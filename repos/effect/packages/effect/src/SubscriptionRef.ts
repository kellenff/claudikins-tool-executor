 ...
 * committed update so subscribers can observe state over time. Updates are serialized so only one
 * change is applied at a time. This module includes constructors, current-value reads, the
 * `changes` stream, writes, updates, partial updates, and effectful update helpers.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use to narrow an unknown value before calling `SubscriptionRef` operations that require a
...
/**
 * The `SubscriptionRef` namespace containing type definitions associated with subscription
 * references.
 *
 * @since 2.0.0
 */
export declare namespace SubscriptionRef {
  /**
   * Type-level variance marker for the value type carried by a `SubscriptionRef`.
   *
   * @since 2.0.0
   * @category Models
   */
 ...
 * **When to use**
 *
 * Use to create a `SubscriptionRef` when consumers need to read the latest value and subscribe to
 * every update.
 *
 * **Details**
 *
 * The initial value is published during construction, so `changes` starts new subscribers with that
 * value before future updates.
 *
 * @since 2.0.0
...
/**
 * Creates a stream that emits the current value and all subsequent changes to the
 * `SubscriptionRef`.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * This function directly accesses the underlying reference without any synchronization. It should
 * only be used when you are certain there are no concurrent modifications.
 *
...
/**
 * Retrieves the current value and updates it atomically with the result of applying a function,
...
/**
 * Retrieves the current value and updates it atomically with the result of applying an effectful
 ...
 * **When to use**
 *
 * Use to read the old `SubscriptionRef` value while applying a synchronous update only when a new
 * value is available.
 *
 * **Details**
 *
 * If the function returns `Option.some`, the new value is set and published. If it returns
 * `Option.none`, the reference is left unchanged and no update is published.
 *
 ...
 * **When to use**
 *
 * Use to read the old `SubscriptionRef` value while applying an effectful update only when a new
 * value is available.
 *
 * **Details**
 *
 * If the effect succeeds with `Option.some`, the new value is set and published. If it succeeds
 * with `Option.none`, the reference is left unchanged and no update is published.
 *
...
/**
 * Modifies the `SubscriptionRef` atomically with a function that computes a return value and a new
 * value, notifying subscribers of the change.
 *
...
/**
 * Modifies the `SubscriptionRef` atomically with an effectful function that computes a return value
 * and a new value, notifying subscribers of the change.
 *
 ...
 * **When to use**
 *
 * Use to return a separate result while synchronously deciding whether to publish a new
 * `SubscriptionRef` value.
 *
 * **Details**
 *
 * If the function returns `Option.some` for the new value, the value is set and published. If it
 * returns `Option.none`, the reference is left unchanged and no update is published.
 *
 ...
 * **When to use**
 *
 * Use to return a separate result while effectfully deciding whether to publish a new
 * `SubscriptionRef` value.
 *
 * **Details**
 *
...
/**
 * Sets the value of the `SubscriptionRef` and returns the new value, notifying all subscribers of
 * the change.
 *
...
/**
 * Updates the value of the `SubscriptionRef` with the result of applying a function, notifying
...
/**
 * Updates the value of the `SubscriptionRef` with the result of applying an effectful function,
 * notifying subscribers of the change.
 *
...
/**
 * Updates the value of the `SubscriptionRef` with the result of applying a function and returns the
 * new value, notifying subscribers of the change.
 *
...
/**
 * Updates the value of the `SubscriptionRef` with the result of applying an effectful function and
 * returns the new value, notifying subscribers of the change.
 *
...
/**
 * Applies an update function to the current value. If it returns `Option.some`, sets and publishes
 * that value; if it returns `Option.none`, leaves the reference unchanged and does not publish.
 *
 ...
 * **When to use**
 *
 * Use to conditionally update a `SubscriptionRef` with an effectful function while discarding the
 * resulting value.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to conditionally update a `SubscriptionRef` and read the value that is current after the
 * update decision.
 *
 * **Details**
 *
 * If the function returns `Option.some`, the new value is set, published, and returned. If it
 * returns `Option.none`, the unchanged current value is returned without publishing.
 *
 ...
 * **When to use**
 *
 * Use to conditionally update a `SubscriptionRef` effectfully and read the value that is current
 * after the update decision.
 *
 * **Details**
 *
 * If the effect succeeds with `Option.some`, the new value is set, published, and returned. If it
 * succeeds with `Option.none`, the unchanged current value is returned without publishing.
 *
