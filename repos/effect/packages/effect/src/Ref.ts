 ...
 *
 * A `Ref<A>` holds one value and exposes reads, writes, and atomic transformations as effects, so
 * state changes compose with Effect's concurrency model. This module includes constructors, safe
 * and unsafe reads, set and get-and-set helpers, update and modify helpers, and conditional update
 * variants that leave the value unchanged when an `Option.none` result is returned.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * A `Ref` is a thread-safe mutable reference type for shared state. It supports simple read and
 * write operations as well as atomic transformations.
 *
 ...
 * **When to use**
 *
 * Use when you need immediate synchronous construction and can guarantee that creating the `Ref`
 * outside of `Effect` is safe.
 *
 * **Gotchas**
 *
...
/**
 * Gets the current value of the Ref, sets it to the specified value, and returns the previous value
 * atomically.
 *
 * **When to use**
 *
...
/**
 * Gets the current value of the Ref, updates it with the given function, and returns the previous
 * value atomically.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * If the partial function returns `Option.some`, the Ref is updated with the new value. If it
 * returns `Option.none`, the Ref is left unchanged. The effect always returns the value that was in
 * the Ref before the attempted update.
 *
 ...
 * **Details**
 *
 * The function receives the current value and returns a tuple of `[result, newValue]`. The Ref is
 * updated with `newValue`, and `result` is returned by the effect.
 *
 ...
 * **Details**
 *
 * The callback receives the current value and returns `[result, nextValue]`, where `nextValue` is
 * an `Option`. If `nextValue` is `Option.some(value)`, the `Ref` is updated to `value`; if it is
 * `Option.none()`, the `Ref` is left unchanged. The returned effect always succeeds with `result`.
 *
 ...
 *
 * If the partial function returns `Option.some`, the Ref is updated with the new value. If it
 * returns `Option.none`, the Ref is left unchanged.
 *
...
/**
 * Updates the value of the Ref atomically using the given partial function and returns the current
 * value.
 *
 * **When to use**
 *
 ...
 *
 * If the partial function returns `Option.some`, the Ref is updated with the new value. If it
 * returns `Option.none`, the Ref is left unchanged. The effect returns the current value of the Ref
 ...
 * **When to use**
 *
 * Use when you need immediate synchronous access and can guarantee that reading the `Ref` outside
 * of `Effect` is safe.
 *
 * **Gotchas**
 *
