/**
 * Helpers for normalizing and converting JSON Schema and OpenAPI schema documents. Supported inputs
 * include JSON Schema Draft-07, Draft 2020-12, OpenAPI 3.0, and OpenAPI 3.1; conversions normalize
 * through `Document<"draft-2020-12">` before emitting another dialect. The module also defines
 * document types, meta-schema constants, OpenAPI component-key helpers, and `$ref` resolution
 * utilities.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * This is an open record type (`[x: string]: unknown`) so it can hold any JSON Schema keyword. Most
 * functions in this module accept or return this type.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use as the dialect marker for `JsonSchema` documents when parsing, converting, or emitting
 * schemas across the supported formats.
 *
 * **Details**
 *
 * Supported values are `"draft-07"` for JSON Schema Draft-07, `"draft-2020-12"` for JSON Schema
 * Draft 2020-12 and the canonical internal form, `"openapi-3.1"` for OpenAPI 3.1, and
 ...
 * **When to use**
 *
 * Use as the shared lookup table for named JSON Schema nodes that are referenced from JSON Schema
 * documents.
 *
 * **Details**
 *
 * The map is dialect-neutral. Conversion APIs emit it as `$defs`, `definitions`, or
 * `components.schemas` depending on the target format.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when you need to carry a root schema together with its shared definitions, or when converting
 * between dialects with the `from*` and `to*` functions.
 *
 * **Details**
 *
 * The `schema` field holds the root schema _without_ the definitions collection. Root definitions
 * are stored separately in `definitions` and referenced via `#/$defs/<name>` for Draft-2020-12,
 * `#/definitions/<name>` for Draft-07, and `#/components/schemas/<name>` for OpenAPI 3.1 and
 * OpenAPI 3.0.
 *
...
/**
 * Like {@link Document}, but carries multiple root schemas that share a single definitions pool.
 *
 * **When to use**
 *
 * Use when generating several schemas, such as a request body and a response body, that reference
 ...
 * **When to use**
 *
 * Use when constructing a Draft-07 JSON Schema document and you need a stable value for the root
 * `$schema` field.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to populate the `$schema` field while emitting a JSON Schema document that
 ...
 * **When to use**
 *
 * Use when you have a raw JSON Schema object that follows Draft-07 conventions and need the
 * canonical Draft-2020-12 document representation.
 *
 * **Details**
 *
 * This converts Draft-07 tuple syntax (`items` as array plus `additionalItems`) to Draft-2020-12
 * form (`prefixItems` plus `items`), rewrites `#/definitions/...` refs to `#/$defs/...`, and
 ...
 * **Details**
 *
 * This separates `$defs` from the root schema into the `definitions` field. Unlike
 ...
 * **Details**
 *
 * This rewrites `#/components/schemas/...` refs to `#/$defs/...`, then delegates to
 ...
 * **Details**
 *
 * This handles OpenAPI 3.0 extensions, including `nullable`, singular `example`, and boolean
 * `exclusiveMinimum` or `exclusiveMaximum`. It normalizes the schema to Draft-07 first, then
 * converts to Draft-2020-12 via {@link fromSchemaDraft07}.
 *
 ...
 * **Details**
 *
 * This rewrites `#/$defs/...` refs to `#/definitions/...`, converts Draft-2020-12 tuple syntax
 * (`prefixItems` plus `items`) to Draft-07 form (`items` as array plus `additionalItems`), and
 ...
 * **Details**
 *
 * This rewrites `#/$defs/...` refs to `#/components/schemas/...`, sanitizes definition keys to
 * match the OpenAPI component key pattern (`^[a-zA-Z0-9.\-_]+$`) by replacing invalid characters
 * with `_`, updates all `$ref` pointers to use the sanitized keys, and converts all schemas and
...
/**
 * Returns a sanitized key for an OpenAPI component schema. Should match the `^[a-zA-Z0-9.\-_]+$`
 * regular expression.
 *
 * @internal
 */
 ...
 * **Details**
 *
 * This only resolves the final segment of the ref path, such as `"User"` from `"#/$defs/User"`. It
 ...
 * **When to use**
 *
 * Use when you need to dereference a top-level `$ref` before inspecting the root JSON Schema
