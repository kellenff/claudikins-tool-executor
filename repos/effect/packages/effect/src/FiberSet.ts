 ...
 *
 * A `FiberSet<A, E>` tracks running fibers, removes each fiber when it completes, and interrupts
 * all still-running fibers when the owning scope closes. This module includes scoped runtime
 * constructors plus helpers for adding, clearing, running, counting, joining, and waiting for
 * managed fibers.
 *
 * @since 2.0.0
 */
...
/**
 * A FiberSet is a collection of fibers that can be managed together. When the associated Scope is
 * closed, all fibers in the set will be interrupted.
 *
 ...
 * **Details**
 *
 * When the associated Scope is closed, all fibers in the set will be interrupted. You can add
 * fibers to the set using `FiberSet.add` or `FiberSet.run`, and the fibers will be automatically
 * removed from the FiberSet when they complete.
 *
 ...
 * **Details**
 *
 * Each call returns the forked fiber and adds it to the set. Managed fibers are removed when they
 * complete and are interrupted when the set's scope closes.
 *
...
/**
 * Creates a scoped run function that forks effects into a new `FiberSet` and returns a `Promise`
 * for each effect result.
 *
 * **When to use**
 *
 * Use when many scoped fibers should be tracked as a set while exposing each result through
 * Promise-based APIs.
 *
 * **Details**
 *
 * Managed fibers are removed when they complete and are interrupted when the set's scope closes.
 * Each Promise resolves with the effect's success value or rejects with the squashed failure
 * cause.
 *
 ...
 * **When to use**
 *
 * Use when an already forked fiber must be registered immediately and synchronous interruption on a
 * closed set is acceptable.
 *
 * **Details**
 *
 * When the fiber completes, it is removed from the set. If the set is already closed, the supplied
 * fiber is interrupted immediately. Non-interruption failures are recorded for `FiberSet.join`.
 *
...
/**
 * Forks an Effect and add the forked fiber to the FiberSet. When the fiber completes, it will be
 * removed from the FiberSet.
 *
 ...
 * **When to use**
 *
 * Use when you need to bridge effects to `Promise` values while still tracking their fibers in a
 * `FiberSet`.
 *
 * **Details**
 *
...
/**
 * Joins all fibers in the FiberSet. If any fiber in the set terminates with a failure, the returned
 * Effect will terminate with the first failure that occurred.
 *
