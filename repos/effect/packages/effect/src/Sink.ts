 ...
 *
 * A `Sink` may read no input, a fixed amount of input, or keep reading until a condition is met. If
 * it reads more than it needs, it can return leftovers so the stream can continue from those
 * values. Sinks are used to collect, fold, search, count, or otherwise reduce streamed input, and
 * they can be composed when a stream needs more than one consuming step.
 *
 * @since 2.0.0
 */
...
/**
 * A `Sink<A, In, L, E, R>` is used to consume elements produced by a `Stream`. You can think of a
 * sink as a function that will consume a variable amount of `In` elements (could be 0, 1, or many),
 * might fail with an error of type `E`, and will eventually yield a value of type `A` together with
 * a remainder of type `L` (i.e. any leftovers).
 *
 ...
 * **Details**
 *
 * The first element is the sink result. The optional second element contains a non-empty array of
 * leftover input that was pulled but not consumed.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * This preserves the result, input, leftover, error, and service type parameters when Effect's
 * `Unify` machinery normalizes generic values that include sinks. Users normally do not need to
 ...
 * **Details**
 *
 * It prevents the inherited `Effect` unifier from being selected when sink-specific unification
 * should preserve the `Sink` type parameters. Users normally do not need to reference this
 * interface directly.
 *
 * @since 2.0.0
 * @category Models
...
export declare namespace Sink {
   ...
   * **Details**
   *
   * The result `A`, leftovers `L`, errors `E`, and services `R` are covariant. The input type `In`
   * is contravariant because values flow into the sink.
   *
   * @since 2.0.0
   * @category Models
   */
 ...
 * @category Guards
 */
export const isSink = (u: unknown): u is Sink<unknown, never, unknown, unknown, unknown> =>
 ...
 * **Details**
 *
 * The transform receives the upstream pull of non-empty input arrays and the active scope, and
 * returns an effect that completes with the sink's `End` value.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The returned function exposes the sink input as a `Stream<In>`, applies the provided pipeline,
 * and uses the final effect's success value as the sink result.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Companion namespace containing overload types for the pipe-style sink constructor returned by
 * `Sink.make`.
 *
 * @since 4.0.0
 */
export declare namespace make {
   ...
   * **Details**
   *
   * The first pipeline function receives the sink input as a `Stream<In>`. The final pipeline step
   * must return an `Effect`, whose success value becomes the sink result.
   *
   * @since 4.0.0
   * @category Models
   */
...
/**
 * Creates a sink that ignores upstream input and completes from an effect that already returns an
 * `End`.
 *
 * **When to use**
 *
 * Use when you need to create a sink from an effect that returns both the sink result value and
 * optional leftovers.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Creates a sink that ignores upstream input and completes with the success value of the provided
 * effect.
 *
 * **Details**
 *
...
): Sink<never, unknown, never, E> => fromEffectEnd(Effect.failCauseSync(evaluate));
 ...
 * **Details**
 *
 * The sink result is preserved, but any leftover elements are discarded instead of being returned
 * to downstream sink composition. This does not continue pulling additional elements from the
 * upstream stream.
 *
 * @since 2.0.0
 * @category Filtering
 ...
 * **When to use**
 *
 * Use to consume all upstream input and complete with void when the input values and any aggregate
 ...
 * **When to use**
 *
 * Use to accumulate stream input element by element with an effectful step and stop based on the
 * accumulated state.
 *
 * **Details**
 *
 * The initial state is evaluated lazily. Each input element is folded with the effectful function,
 * and the sink continues while `contFn` returns `true`. If the sink stops in the middle of a pulled
 * array, the remaining elements from that array are returned as leftovers.
 *
 * @since 2.0.0
 * @category Folding
 ...
 * **When to use**
 *
 * Use to update state with an effectful function once per pulled non-empty input array when
 * batch-level processing is the natural unit.
 *
 * **Details**
 *
 * The initial state is evaluated lazily. After each pulled array is folded, the sink continues
 * while `contFn` returns `true`; otherwise it completes with the current state.
 *
 * @since 4.0.0
 * @category Folding
...
/**
 * Folds input elements into state until the specified maximum number of elements has been consumed
 * or the upstream stream ends.
 *
 * **Details**
 *
 * If the sink stops in the middle of a pulled array, the remaining elements from that array are
 ...
 * **When to use**
 *
 * Use to reduce a stream to a boolean that is true only when every input satisfies a pure
 * predicate.
 *
 * @since 2.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to compute a new result from the original sink result while preserving the sink's input
 * consumption behavior.
 *
 * **Details**
 *
 * The transformed sink preserves the original sink's input type, leftovers, errors, and
 * requirements.
 *
 * @since 2.0.0
 * @category Mapping
 ...
 * **When to use**
 *
 * Use to keep a sink's input consumption, errors, requirements, and leftovers while replacing only
 * its result with a known value.
 *
 * @since 2.0.0
 * @category Mapping
...
export const mapInputArray: {
  ...
  <A, In, L, E, R, In0>(
    ...
    fromTransform((upstream, scope) => self.transform(Effect.map(upstream, f), scope)),
);
...
export const mapEnd: {
  ...
  <A, In, L, E, R, A2, L2 = never>(
    ...
    fromTransform((upstream, scope) => Effect.map(self.transform(upstream, scope), f)),
);
 ...
 * **Details**
 *
 * This can change both the result value and the optional leftovers, and the transformation can fail
 * or require services.
 *
 * @since 4.0.0
 * @category Mapping
 ...
 * **Details**
 *
 * The transformed sink preserves the original sink's input consumption and leftovers while adding
 * the errors and requirements of the transformation.
 *
 * @since 2.0.0
 * @category Mapping
...
export const mapEffect: {
  ...
  <A, In, L, E, R, A2, E2, R2>(
    ...
    mapEffectEnd(self, ([a, l]) => Effect.map(f(a), (a2) => [a2, l] as End<A2, L>)),
);
 ...
 * **Details**
 *
 * If `n` is less than or equal to zero, the sink completes with an empty array. If more elements
 * are pulled than needed, the remaining elements from the same array are returned as leftovers.
 *
 * @since 2.0.0
 * @category Collecting
 */
...
/**
 * Runs this sink until it yields a result, then uses that result to create another sink from the
 * provided function which will continue to run until it yields a result.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Leftovers from the first sink are fed to the sink returned by `f` before more upstream input is
 * pulled.
 *
 * @since 2.0.0
 * @category Sequencing
...
/**
 * A sink that reduces input elements from the provided `initial` state with `f` while the specified
...
/**
 * A sink that effectfully reduces input elements from the provided `initial` state with `f` while
...
/**
 * A sink that reduces non-empty input arrays from the provided `initial` state with `f` while the
...
/**
 * A sink that effectfully reduces non-empty input arrays from the provided `initial` state with `f`
 * while the specified `predicate` returns `true`.
 *
 * @since 4.0.0
 * @category Reducing
 */
...
/**
 * A sink that reduces its inputs using the provided function `f` starting from the provided
 * `initial` state.
 *
 * @since 4.0.0
 * @category Reducing
...
/**
 * A sink that reduces its inputs using the provided function `f` starting from the specified
 * `initial` state.
 *
 * @since 4.0.0
 * @category Reducing
...
/**
 * A sink that reduces its inputs using the provided effectful function `f` starting from the
 ...
 * **Details**
 *
 * Returns `Option.some(first)` for non-empty input, or `Option.none` when the upstream ends without
 * input. The first element is consumed; later elements from the same pulled array are emitted as
 * leftovers.
 *
 * @since 2.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * Returns `Option.some(last)` with the final input value, or `Option.none` when the upstream ends
 * without input.
 *
 * **Gotchas**
 *
 * This sink produces a result only when the upstream ends, so it does not complete for a stream
 * that does not end.
 *
 * @since 2.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to scan stream input until the first matching element is found and return that element as an
 * `Option`.
 *
 * **Details**
 *
 * Returns `Option.none` if the upstream stream ends before a match is found. Refinement predicates
 * narrow the returned value type. The matching input is consumed; any later elements from the same
 * pulled array are returned as leftovers.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use when you need to run effects, fail, or use services while searching for the first matching
 * input.
 *
 * **Details**
 *
 * Returns `Option.some` with the first input whose predicate result is `true`, or `Option.none` if
 * the upstream stream ends first. If the predicate effect fails, the sink fails with the same
 * error.
 *
 * @since 2.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * The first failing input is consumed and excluded from the result. Any later elements from the
 * same pulled array are returned as leftovers.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The first input for which the filter fails is consumed and excluded from the result. Any later
 ...
 * **Details**
 *
 * The first input for which the predicate returns `false` is consumed and excluded from the result.
 * Any later elements from the same pulled array are returned as leftovers.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Applies a `FilterEffect` to input elements effectfully while it succeeds, collecting each
 * successful output.
 *
 * **Details**
 *
...
/**
 * Collects input elements until the predicate returns `true`, including the matching element in the
 * result.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Collects input elements effectfully until the predicate returns `true`, including the matching
 ...
 * **Details**
 *
 * The sink stops consuming input when the function returns `false` or when the upstream stream
 * ends, and completes with `void`.
 *
 * @since 2.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The fallback is built from the error and continues consuming from the same upstream stream. If
 * the upstream stream had already ended, the fallback sees the upstream end instead.
 *
 * @since 2.0.0
 ...
 * **When to use**
 *
 * Use to recover from a sink failure based on the full `Cause` instead of only the typed error
 * value.
 *
 * **Details**
 *
 * When this sink fails, the handler effect is run and its success value becomes the sink result. If
 * the handler fails, the returned sink fails with that error.
 *
 * @since 4.0.0
 * @category Error handling
 ...
 * **Details**
 *
 * The effect receives the sink's `Exit` for the result value. The original sink result and
 * leftovers are preserved unless the finalizer itself fails.
 *
 * @since 4.0.0
