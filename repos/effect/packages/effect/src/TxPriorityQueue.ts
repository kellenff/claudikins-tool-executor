/**
 * Transactional priority queues whose state is stored in a `TxRef`. Elements are kept in the order
 * defined by the `Order` supplied at construction time, and dequeue operations return the first
 * element according to that ordering.
 *
 * Use `TxPriorityQueue` when multiple fibers coordinate through a shared queue and queue operations
 * need to compose with other transactional state changes. The retrying `peek` and `take` operations
 * wait transactionally when the queue is empty, so they can be combined with other transactional
 * reads and writes in one atomic workflow.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Elements are stored in ascending order according to the `Order` provided at construction time.
 * `take` returns the smallest element, `peek` observes it without removing.
 *
