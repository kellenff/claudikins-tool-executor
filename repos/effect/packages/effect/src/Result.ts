 ...
 *
 * A `Result<A, E>` is `Success<A, E>` when a value is available and `Failure<A, E>` when an error
 * is available. It is plain data, so inspecting or transforming it does not run side effects. This
 * module includes helpers for creating, checking, mapping, combining, and extracting results, plus
 ...
 * **When to use**
 *
 * Use when both success and failure should remain available as data and `Option` would lose failure
 * information.
 *
 * **Details**
 *
...
/**
 * Iterator protocol used to yield a `Result` inside {@link gen}, returning the success value type
 * back to the generator.
 *
 * **When to use**
 *
 * Use when defining or typing `[Symbol.iterator]()` for `Result` values so `yield*` can pass the
 * success value type back into `Result.gen`.
 *
 * @since 4.0.0
 * @category Generators
 ...
 * **Details**
 *
 * This is an internal interface used by the Effect type system. You typically do not need to
 ...
 * **Details**
 *
 * Used internally to integrate `Result` with generic type-class utilities (e.g., `map`, `flatMap`
 * abstractions). You typically do not need to reference this directly.
 *
 * @since 4.0.0
...
export declare namespace Result {
   ...
   * @category Type Level
   */
  export type Failure<T extends Result<any, any>> = [T] extends [Result<infer _A, infer _E>]
    ? _E
    : never;
  /**
   ...
   * @category Type Level
   */
  export type Success<T extends Result<any, any>> = [T] extends [Result<infer _A, infer _E>]
    ? _A
    : never;
}
...
export {
   ...
   * **When to use**
   *
   * Use when you need a successful `Result` value that signals completion without carrying
   * meaningful data.
   *
   * **Details**
   *
   * This is equivalent to `Result.succeed(undefined)`, but reuses a shared `Success` wrapper
 ...
 * **When to use**
 *
 * Use when you need a failed `Result` value that acts only as a control signal without failure
 * data.
 *
 * **Details**
 *
 * This is equivalent to `Result.fail(undefined)` with type `Result<never, void>`, but reuses a
 * shared `Failure` wrapper instead of allocating one each time.
 *
 ...
 * **When to use**
 *
 * Use when you need `null` or `undefined` input to become a `Failure` while present values remain
 ...
 * **When to use**
 *
 * Use when an existing `Option` should become a `Result`, preserving `Some` as success and turning
 * `None` into a caller-provided failure.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to extract the success value from a `Result` as an `Option` and discard failure
 * information.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to extract the failure value from a `Result` as an `Option` and discard
 * successful values.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to transform both success and failure values without changing whether the result succeeds or
 * fails.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to apply a transformation to the success value of a `Result` while preserving any existing
 * failure.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when a `Result`'s success and failure branches should be collapsed into one plain output
 * type.
 *
 * **Details**
 *
...
/**
 * Validates the success value of a `Result` using a predicate, failing with a custom error if the
...
/**
 * Unwraps a `Result` into `A | E` by returning the inner value regardless of whether it is a
 * success or failure.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need the success value from a `Result`, with a fallback computed from the failure
 * value.
 *
 * **Details**
 *
...
/**
 * Returns the original `Result` if it is a `Success`, otherwise applies `that` to the error and
 ...
 * **When to use**
 *
 * Use to collect independent `Result` values into one `Result` while preserving the original
 * structure.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to swap channels when failure-focused operations are easier through success-oriented
 * combinators.
 *
 * **Details**
 *
 ...
 * - Useful when you want to apply success-oriented operations (like `map`) to the error channel, then
 *   flip back
 *
 ...
 * **When to use**
 *
 * Use when you need generator syntax to compose sequential `Result` computations instead of nested
 * `flatMap` calls.
 *
 * **Details**
 *
 * - Use `yield*` to unwrap a `Result` inside the generator; if any yielded `Result` is a `Failure`,
 *   the generator short-circuits and returns that failure
 ...
 * **When to use**
 *
 * Use to start a `Result` do-notation pipeline from an empty successful record before adding named
 * fields from `Result`-producing computations and pure computed values.
 *
 * **Details**
 *
 * Creates a `Result<{}>` (success with an empty object). Use with {@link bind} to add
 * `Result`-producing fields and {@link let_ let} to add pure computed fields.
 *
...
/**
 * Adds a named field to the do-notation accumulator by running a `Result`-producing function that
 * receives the current accumulated object.
 *
 * **When to use**
 *
 * Use when you need to add a `Result`-producing step to a `Result` do-notation pipeline and store
 * its successful value under a named field in the accumulated object.
 *
 * **Details**
 *
...
export {
  /**
   * Adds a named field to the do-notation accumulator by computing a pure (non-Result) value from
   * the current accumulated object.
   *
   * **When to use**
   *
 ...
 * **When to use**
 *
 * Use when optional absence should be treated as a successful `None`, while an inner `Result`
 * failure should still fail the whole result.
 *
 * **Details**
 *
...
/**
 * Maps an `Option` value with a `Result`-producing function, then transposes the structure from
 * `Option<Result<B, E>>` to `Result<Option<B>, E>`.
 *
 * **When to use**
 *
 * Use when an optional value should be validated only when present, preserving absence as a
 * successful `None`.
 *
 * **Details**
 *
 ...
 *   isNaN(Number(s)) ? Result.fail("not a number" as const) : Result.succeed(Number(s));
 *
 ...
 * **When to use**
 *
 * Use when an optional success should be absent, such as the `None` branch of `transposeOption` or
 * `transposeMapOption`.
 *
 * **Details**
 *
 * This is equivalent to `Result.succeed(Option.none())`, but reuses a shared `Success` wrapper
 * instead of allocating one each time.
 *
