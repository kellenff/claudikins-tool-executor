 ...
 *
 * `MutableHashMap` updates the same collection in place and supports fast lookup, insertion,
 * removal, clearing, and iteration. It combines a native `Map` for ordinary JavaScript keys with
 * hash buckets for keys that implement Effect `Equal` and `Hash`, so callers can mix
 * reference-based and structural lookup in the same collection.
 *
 * @since 2.0.0
 */
...
/**
 * A mutable hash map that stores key-value pairs and supports both referential and Effect
 * structural equality.
 *
 * **When to use**
 *
 * Use as a mutable key-value map when in-place updates are acceptable and keys may rely on Effect
 * structural equality.
 *
 * **Details**
 *
 * Operations mutate the map in place. Keys that implement `Equal` / `Hash` can be looked up
 * structurally; other keys use normal JavaScript reference or primitive equality.
 *
...
/**
 * Sets a key-value pair in the MutableHashMap, mutating the map in place. If the key already
...
/**
 * Updates the value of the specified key within the MutableHashMap if it exists. If the key doesn't
 * exist, the map remains unchanged.
 *
 * **When to use**
 *
...
/**
 * Updates or removes the specified key using a function from the current optional value to the next
 * optional value.
 *
 * **When to use**
 *
...
/**
 * Removes the specified key from the MutableHashMap, mutating the map in place. If the key doesn't
 * exist, the map remains unchanged.
 *
 * **When to use**
 *
...
/**
 * Removes all key-value pairs from the MutableHashMap, mutating the map in place. The map becomes
 ...
 * **When to use**
 *
 * Use to run a synchronous side-effecting callback for every key-value pair in an existing mutable
 * map.
 *
 * **Details**
 *
 * Iteration follows the backing map's order. The callback receives the value first and the key
 * second, matching `Map.prototype.forEach`.
 *
 * @since 2.0.0
 * @category Traversing
