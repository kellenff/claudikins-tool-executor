 ...
 *
 * A `Stream<A, E, R>` can emit many `A` values, fail with `E`, and require services `R` while it is
 * being consumed. Streams are useful for data that is pulled in steps, such as values from
 * collections, queues, pubsubs, schedules, callbacks, async iterables, or platform streams. The
 * APIs here cover the full stream lifecycle: create a stream, transform or combine it, control
 * buffering and timing, handle failures, and finally consume it.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * This marker is part of the runtime representation of `Stream` values. Prefer `isStream` when
...
/**
 * A `Stream<A, E, R>` describes a program that can emit many `A` values, fail with `E`, and require
 * `R`.
 *
 * **Details**
 *
 * Streams are pull-based with backpressure and emit chunks to amortize effect evaluation. They
 * support monadic composition and error handling similar to `Effect`, adapted for multiple values.
 *
 ...
 * @category Context
 */
export const service = <I, S>(service: Context.Key<I, S>): Stream<S, never, I> =>
  fromEffect(Effect.service(service));
 ...
 * **When to use**
 *
 * Use when you need a stream that emits an optional service from the context without requiring that
...
): Stream<A, Pull.ExcludeDone<E>, R> => fromPull(Effect.succeed(Effect.map(effect, Arr.of)));
...
/**
 * Creates a stream that emits `void` immediately once, then emits another `void` after each
 * specified interval.
 *
 ...
 * **Details**
 *
 * A pull effect yields chunks on demand and completes when the upstream stream ends. See
 * `Stream.toPull` for a matching producer.
 *
 ...
 * **Details**
 *
 * A forked scope is also provided to the transformation function, which is closed once the
 * resulting stream has finished processing.
 *
 ...
 * **When to use**
 *
 * Use when you need callback-based code to emit stream values by offering to a `Queue`, or signal
 * stream completion through the `Queue` module APIs.
 *
 * By default it uses an "unbounded" buffer size. You can customize the buffer size and strategy by
 * passing an object as the second argument with the `bufferSize` and `strategy` fields.
 *
...
export const fromIteratorSucceed = <A>(
  ...
  maxChunkSize?: number,
): Stream<A> => fromChannel(Channel.fromIteratorArray(() => iterator, maxChunkSize));
 ...
 * **When to use**
 *
 * Use when the iterable must be acquired from an Effect before the stream emits, and acquisition
 * services or failures should be part of the stream.
 *
 ...
 * **When to use**
 *
 * Use when the array must be acquired from an Effect before the stream emits, and acquisition
 * services or failures should be part of the stream.
 *
...
export const fromArrays = <Arr extends ReadonlyArray<ReadonlyArray<any>>>(
  ...
  fromChannel(Channel.fromArray(Arr.filter(arrays, Arr.isReadonlyArrayNonEmpty)));
 ...
 * **Details**
 *
 * The stream emits non-empty batches of queued values and ends when the queue fails with
 * `Cause.Done`; other queue failures are propagated.
 *
 ...
 * **Details**
 *
 * The stream reads from a `ReadableStreamDefaultReader`, maps read failures with `onError`, and
 * closes the reader when the stream finalizes. By default the reader is canceled; set
 * `releaseLockOnEnd` to release the lock instead.
 *
...
/**
 * Creates a stream that emits each output of a schedule that does not require input, for as long as
 ...
 *
 * const program = Effect.gen(function* () {
 *   const schedule = Schedule.spaced("50 millis").pipe(Schedule.upTo({ times: 3 }));
 ...
 * **When to use**
 *
 * Use when you already have a `PubSub.Subscription` and want to expose its messages as a `Stream`,
 * with `Stream.take` or cancellation controlling how many values are consumed.
 *
 ...
 * **Details**
 *
 * Each `readonly [value, nextState]` result emits `value` and continues with `nextState`; returning
 * `undefined` ends the stream.
 *
 ...
 * **When to use**
 *
 * Use to consume paginated APIs where each step returns a batch of values together with an optional
 * next state.
 *
 * **Details**
 *
 * This is similar to {@link unfold}, but each step can emit zero or more values and independently
 * decide whether another state should be requested.
 *
...
/**
 * Runs a stream that requires `Scope` in a managed scope, ensuring its finalizers are run when the
 * stream completes.
 *
 ...
 *   Stream.fromEffect(
 *     Effect.acquireRelease(Console.log("acquire").pipe(Effect.as("resource")), () =>
 *       Console.log("release"),
 *     ),
 *   ),
 * );
...
/**
 * Transforms each emitted chunk using the provided function, which receives the chunk and its
 * index.
 *
 ...
 * **When to use**
 *
 * Use when each stream element transformation needs an Effect, service dependency, failure channel,
 ...
 * **When to use**
 *
 * Use when stream elements already are effects and their successes should become stream elements
 * while their failures enter the stream error channel.
 *
 ...
 * **When to use**
 *
 * Use when transformation needs to see and replace each non-empty emitted chunk effectfully instead
 * of mapping individual stream elements.
 *
 ...
 * **Details**
 *
 * With the default sequential concurrency, inner streams are concatenated in input order. When
 * `concurrency` is greater than `1` or `"unbounded"`, multiple inner streams may run at the same
 * time and their outputs are merged as they arrive.
 *
...
/**
 * Switches to the latest stream produced by the mapping function, interrupting the previous stream
 ...
 * **Details**
 *
 * With the default sequential concurrency, inner streams are concatenated in strict order. When
 * `concurrency` is greater than `1` or `"unbounded"`, multiple inner streams may run at the same
 * time and their outputs are merged as they arrive.
 *
...
/**
 * Runs the provided stream in the background while this stream runs, interrupting it when this
 * stream completes and failing if the background stream fails or defects.
 *
 ...
 * **When to use**
 *
 * Use when a stream should continue with another stream if an upstream pull waits longer than the
 * allowed duration.
 *
 * **Details**
 *
 * The timeout is checked for each pull. A zero duration uses `orElse` immediately, while an
 * infinite duration leaves the original stream unchanged.
 *
 * **Gotchas**
 *
...
/**
 * Repeats each element of the stream according to the provided schedule, including the original
 * emission.
 *
      ...
      Channel.fromTransform((upstream, scope) =>
        Effect.map(
          ...
          (pullElement) => {
            ...
            const pull: Pull.Pull<Arr.NonEmptyReadonlyArray<A>, E, void, R | R2> = Effect.gen(
              function* () {
...
/**
 * Unwraps `Take` values, emitting elements from non-empty arrays and ending or failing when the
...
/**
 * Concatenates two streams, emitting all elements from the first stream followed by all elements
...
export const prepend: {
  ...
  2,
  <A, E, R, B>(self: Stream<A, E, R>, values: Iterable<B>): Stream<A | B, E, R> =>
    concat(fromIterable(values), self),
);
 ...
 * **Details**
 *
 * By default, the merged stream ends when both streams end. Use `haltStrategy` to change the
 ...
 * **When to use**
 *
 * Use when an effect should run concurrently for the lifetime of a stream while only the stream's
 * elements remain in the output.
 *
 * **Details**
 *
 * The effect runs concurrently, fails the stream if it fails, and is interrupted when the stream
 * completes.
 *
...
export const mergeEffect: {
  ...
  <A, E, R, A2, E2, R2>(
    self: Stream<A, E, R>,
    effect: Effect.Effect<A2, E2, R2>,
  ): Stream<A, E | E2, R | R2> => self.channel.pipe(Channel.mergeEffect(effect), fromChannel),
);

/**
 * Merges this stream and the specified stream together, tagging values from the left stream as
 * `Result.succeed` and values from the right stream as `Result.fail`.
 *
 * **When to use**
 *
 * Use when values from both streams should be emitted and downstream code needs left values wrapped
 * as successful `Result` values and right values wrapped as failed `Result` values.
 *
 ...
 * **When to use**
 *
 * Use when the right stream is needed for its effects or failures, but downstream consumers should
 * only receive values from the left stream.
 *
 * **Details**
 *
 * The right stream still runs for its effects, and any failures from the right stream are
 * propagated. The merged stream completes when the left stream completes, interrupting the right
 * stream.
 *
...
/**
 * Merges this stream and the specified stream together, emitting only the values from the right
 * stream while the left stream runs for its effects.
 *
 * **When to use**
 *
 * Use when the left stream is needed for its effects or failures, but downstream consumers should
 * only receive values from the right stream.
 *
 * **Details**
 *
 * The merged stream ends when the right stream completes, interrupting the left stream. Failures
 * from the left stream still fail the merged stream.
 *
 ...
 * **When to use**
 *
 * Use to merge an iterable of already-created streams while bounding how many inner streams may run
 * at the same time.
 *
 * **Details**
 *
 * The `concurrency` option is required and may be a number or `"unbounded"`. `bufferSize` controls
 * buffering between inner streams, and outputs are emitted as they arrive under concurrent
 * merging.
 *
...
/**
 * Creates the cartesian product of two streams, running the `right` stream for each element in the
 * `left` stream.
 *
 * **Details**
 *
...
/**
 * Zips this stream with another point-wise and emits tuples of elements from both streams. The new
 * stream ends when either stream ends.
 *
...
/**
 * Zips this stream with another point-wise, keeping only right values and ending when either stream
 * ends.
 *
 ...
 * @category Zipping
 */
export const zipWithIndex = <A, E, R>(self: Stream<A, E, R>): Stream<[A, number], E, R> =>
  map(self, (a, i) => [a, i]);
...
/**
 * Zips multiple streams so that when a value is emitted by any stream, it is combined with the
 * latest values from the other streams to produce a result.
 *
 * **When to use**
 *
 * Use when each stream should contribute its latest value after all streams have emitted at least
 * once.
 *
 * **Gotchas**
 *
 * Note: tracking the latest value is done on a per-array basis. That means that emitted elements
 * that are not the last value in arrays will never be used for zipping.
 *
 ...
 * **When to use**
 *
 * Use when two streams should start emitting combined pairs after both have produced at least one
 * value.
 *
 * **Gotchas**
 *
 ...
 *
 * const program = Effect.gen(function* () {
 *   const result = yield* Stream.zipLatest(Stream.make(1), Stream.make("a")).pipe(
 *     Stream.runCollect,
 *   );
 *
 *   yield* Console.log(result);
 ...
 * **When to use**
 *
 * Use when two streams should start emitting custom combined values after both have produced at
 * least one value.
 *
 * **Gotchas**
 *
...
/**
 * Runs all streams concurrently until one stream emits its first value, then mirrors that winning
 * stream and interrupts the rest.
 *
 * **Details**
 *
 * Failures or completion from losing streams before a winner is chosen are ignored unless every
 * stream fails or completes before emitting. After a winner is chosen, that stream's later failures
 * are propagated.
 *
...
/**
 * Runs both streams concurrently until one stream emits its first value, then mirrors that winning
 * stream and interrupts the other.
 *
 * **Details**
 *
 * A failure or completion from one side before the other side emits does not win the race unless
 * both sides fail or complete before emitting. After a winner is chosen, that stream's later
 ...
 * **When to use**
 *
 * Use to apply effectful logic that can reject stream elements or emit transformed values before
 * they continue downstream.
 *
 * **Details**
 *
 * `Result.succeed` values are emitted, `Result.fail` values are skipped, and effect failures fail
 * the stream.
 *
 * @since 2.0.0
 * @category Filtering
 ...
 * **Details**
 *
 * The queues are backed by a fiber in the current scope and should be consumed while that scope
 * remains open. Each queue fails with the stream error or `Cause.Done` when the source ends.
 *
...
export const partitionQueue: {
  ...
  > {
    ...
    const capacity =
      options?.capacity === "unbounded" ? undefined : (options?.capacity ?? DefaultChunkSize);
...
/**
 * Splits a stream with an effectful `Filter`, returning scoped streams for filter successes and
 * failures.
 *
 * **When to use**
 *
 * Use when you need to classify each stream element with an effectful `Filter` and consume both
 * passing and failing mapped values as streams.
 *
 * **Details**
 *
 * The returned streams are backed by queues in the current scope and should be consumed while that
 * scope remains open. The first stream emits success values from the filter, and the second emits
 * failure values.
 *
 * @since 4.0.0
 * @category Filtering
 ...
 *
 * The returned streams are backed by queues in the current scope and should be consumed while that
 * scope remains open. The faster stream may advance up to `bufferSize` elements ahead of the slower
 * one.
 *
...
/**
 * Returns the specified stream if the given condition is satisfied, otherwise returns an empty
 * stream.
 *
...
/**
 * Runs a sink to peel off enough elements to produce a value and returns that value with the
 * remaining stream in a scope.
 *
 * **Details**
 *
...
/**
 * Buffers up to `capacity` elements so a faster producer can progress independently of a slower
 * consumer.
 *
 * **Details**
 *
 * Finite buffers use the configured queue strategy: `"suspend"` applies backpressure, while
 * `"dropping"` and `"sliding"` may discard elements when the buffer is full. This combinator
 * destroys chunking; use `Stream.rechunk` afterward if you need fixed chunk sizes.
 *
 ...
 * `capacity` chunks in a queue.
 *
 * **Details**
 *
 * Finite buffers use the configured queue strategy: `"suspend"` applies backpressure, while
 * `"dropping"` and `"sliding"` may discard chunks when the buffer is full. This combinator
 * preserves chunking and is best with power-of-2 capacities.
 *
...
/**
 * Switches over to the stream produced by the provided function in case this one fails. Allows
...
/**
 * Runs an effect when the stream fails without changing its values or error, unless the tap effect
 * itself fails.
 *
...
export const tapError: {
  ...
  <A, E, R, A2, E2, R2>(
    self: Stream<A, E, R>,
    f: (error: E) => Effect.Effect<A2, E2, R2>,
  ): Stream<A, E | E2, R | R2> => self.channel.pipe(Channel.tapError(f), fromChannel),
);
 ...
 * **Details**
 *
 * When a failure matches the filter, the stream switches to the recovery stream. Non-matching
 * failures propagate downstream, so the error type is preserved unless the filter narrows it.
 *
 ...
 * **When to use**
 *
 * Use to recover from stream errors with a reusable `Filter` when matching can also narrow or
 * transform the error before choosing the recovery stream.
 *
 * **Details**
 *
 * Successful filter results are passed to `f`. Failed filter results go to `orElse` when provided;
 * otherwise the filter failure is re-failed.
 *
 * @since 4.0.0
 * @category Error handling
...
/**
 * Recovers from failures whose `_tag` matches the provided value by switching to the stream
 * returned by `f`.
 *
 * **When to use**
 *
 * Use when you need to handle a specific error case from a stream whose error type is a tagged
 * union with a readonly `_tag` field.
 *
...
export const catchTags: {
  ...
  >(
    ...
    | {
        [K in keyof Cases]: Cases[K] extends (...args: Array<any>) => Stream<infer A, any, any>
          ? A
          : never;
      }[keyof Cases],
    | E2
    ...
    | {
        [K in keyof Cases]: Cases[K] extends (...args: Array<any>) => Stream<any, infer E, any>
          ? E
          : never;
      }[keyof Cases],
    | R
    | R2
    | {
        [K in keyof Cases]: Cases[K] extends (...args: Array<any>) => Stream<any, any, infer R>
          ? R
          : never;
      }[keyof Cases]
  >;
...
export const catchReasons: {
  ...
  >(
    ...
    | Exclude<A2, unassigned>
    | {
        [RK in keyof Cases]: Cases[RK] extends (...args: Array<any>) => Stream<infer A, any, any>
          ? A
          : never;
      }[keyof Cases],
    | ExcludeTag<E, K>
    ...
    | {
        [RK in keyof Cases]: Cases[RK] extends (...args: Array<any>) => Stream<any, infer E, any>
          ? E
          : never;
      }[keyof Cases],
    | R
    | R2
    | {
        [RK in keyof Cases]: Cases[RK] extends (...args: Array<any>) => Stream<any, any, infer R>
          ? R
          : never;
      }[keyof Cases]
  >;
  ...
  (self, errorTag, cases, orElse) => {
    const handlers: Record<
      string,
      (reason: any, error: any) => Channel.Channel<any, any, any, any, any, any, any>
    > = {};
 ...
 * **When to use**
 *
 * Use when you need to recover a stream only from causes selected by a `Filter`, while giving the
 * recovery both the selected value and the original `Cause`.
 *
 * **Details**
 *
 * The filter is applied to the full `Cause`. A successful filter result is passed to `f` together
 * with the original cause; a failed filter result re-fails with the residual cause.
 *
 * @since 4.0.0
 * @category Error handling
...
export const catchCauseFilter: {
  ...
  <A, E, R, EB, A2, E2, R2, X extends Cause.Cause<any>>(
    ...
    fromChannel(
      Channel.catchCauseFilter(self.channel, filter, (failure, cause) => f(failure, cause).channel),
    ),
);
 ...
 * **When to use**
 *
 * Use when you need to silently suppress a stream's entire failure cause, including both typed
 * errors and defects, rather than propagate it downstream.
 *
...
/**
 * Applies an `ExecutionPlan` to a stream, retrying with step-provided resources until it succeeds
 * or the plan is exhausted.
 *
 * **Details**
 *
 * By default, a failing step can fallback even after emitting elements; set
 * `preventFallbackOnPartialStream` to fail instead of mixing partial output with a later fallback.
 *
 ...
 * **When to use**
 *
 * Use when the stopping condition needs an Effect or service and predicate failure should fail the
 * stream.
 *
 ...
 * **When to use**
 *
 * Use to keep the leading stream elements that a `Filter` accepts, emit the filter's success
 * values, and stop at the first filter failure.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when the leading-prefix predicate needs an Effect or service and the stream should stop
 * before the first false result.
 *
...
/**
 * Drops elements until the specified predicate evaluates to `true`, then drops that matching
 * element.
 *
 ...
 * **When to use**
 *
 * Use when dropping the leading prefix requires an Effect or service and the first matching element
 ...
 * **When to use**
 *
 * Use when you need to remove a leading stream prefix based on a synchronous `Filter` result while
 * preserving the remaining original stream elements.
 *
 * **Details**
 *
 * `Result.succeed` drops the current element. The first `Result.fail` stops dropping, emits that
 * original element, and the rest of the source stream is emitted without further filtering.
 *
 * @since 4.0.0
 * @category Filtering
  ...
  <A, E, R, E2, R2>(
    ...
    transformPull(self, (pull, _scope) =>
      Effect.sync(() => {
        let dropping = true;
        let index = 0;
        const filtered: Pull.Pull<Arr.NonEmptyReadonlyArray<A>, E | E2, void, R2> = Effect.gen(
          function* () {
            while (true) {
...
export const sliding: {
  (
    chunkSize: number,
  ): <A, E, R>(self: Stream<A, E, R>) => Stream<Arr.NonEmptyReadonlyArray<A>, E, R>;
...
export const split: {
  ...
  ): <E, R>(self: Stream<A, E, R>) => Stream<Arr.NonEmptyReadonlyArray<A>, E, R>;
  <A, E, R, B extends A>(
    self: Stream<A, E, R>,
...
/**
 * Combines elements from this stream and the specified stream by repeatedly applying a stateful
 * function that can pull from either side.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use to coordinate pulling chunks from two streams when each emitted chunk depends on both sides
 * and local state.
 *
 * **Details**
 *
 * The combining function receives the current state and pull functions for the left and right
 * streams. It returns the next non-empty chunk together with the next state.
 *
 ...
 * **When to use**
 *
 * Use when stateful element mapping needs Effects or can fail while emitting zero or more values
 * per input element.
 *
 * **Details**
 *
 * The mapping effect receives the current state and element, then returns the next state plus the
 * values to emit. The state is threaded through the stream.
 *
...
/**
 * Maps each non-empty input chunk statefully and effectfully, emitting zero or more output values
 * per chunk.
 *
 * **When to use**
 *
 * Use when stateful mapping should process each emitted non-empty chunk with an Effect instead of
 * each element separately.
 *
 * **Details**
 *
 * The mapping effect receives the current state and chunk, then returns the next state plus the
 * values to emit. The state is threaded across chunks.
 *
...
/**
 * Drops earlier elements within the debounce window and emits only the latest element after the
 * pause.
 *
 ...
 * **Details**
 *
 * Uses a token bucket. The bucket can accumulate up to `units + burst` tokens, and each chunk
 * consumes the cost returned by the effectful `cost` function.
 *
 * If using the "enforce" strategy, arrays that do not meet the bandwidth constraints are dropped.
 * If using the "shape" strategy, arrays are delayed until they can be emitted without exceeding the
 ...
 *
 * Uses a token bucket. The bucket can accumulate up to `units + burst` tokens, and each chunk
 * consumes the cost returned by `cost`.
 *
 * If using the "enforce" strategy, arrays that do not meet the bandwidth constraints are dropped.
 * If using the "shape" strategy, arrays are delayed until they can be emitted without exceeding the
 * bandwidth constraints.
...
export const grouped: {
  ...
  2,
  <A, E, R>(self: Stream<A, E, R>, n: number): Stream<Arr.NonEmptyReadonlyArray<A>, E, R> =>
    chunks(rechunk(self, n)),
);

/**
 * Partitions the stream into arrays, emitting when the chunk size is reached or the duration
 * passes.
 *
 ...
 *     Stream.mapEffect(
 *       ([key, stream]) =>
 *         Stream.runCollect(stream).pipe(Effect.map((values) => [key, values] as const)),
 *       { concurrency: "unbounded" },
 *     ),
 *     Stream.runCollect,
 *   );
 ...
 * **When to use**
 *
 * Use when you already have a stream ordered by the grouping key and want to emit each consecutive
 * run as a non-empty array while keeping later non-adjacent runs separate.
 *
 * **Details**
 *
 * The key is computed with `f`; adjacent elements whose keys are equal by `Equal.equals` are
 * emitted as one `[key, group]`. Later non-adjacent runs with the same key are emitted separately.
 *
 * @since 2.0.0
 * @category Grouping
  ...
  <A, E, R, A2, E2, R2>(
    ...
    transformPull(self, (upstream, scope) =>
      Effect.sync(() => {
        ...
        return Effect.suspend(
          (): Pull.Pull<Arr.NonEmptyReadonlyArray<A2>, E | E2, void, R2> => (done ? done : pull),
        );
      }),
    ),
);
 ...
 * **Details**
 *
 * The stream runs the upstream and downstream in separate fibers, so the sink can keep consuming
 * input while downstream is busy processing the previous output.
 *
...
/**
 * Aggregates elements with a sink, emitting each result when the sink completes or the schedule
 * triggers.
 *
 * **Details**
 *
    ...
    fromChannel(
      Channel.fromTransformBracket(
        Effect.fnUntraced(function* (_upstream, _, scope) {
          ...
          // buffer -> sink
          const pullFromBuffer: Pull.Pull<Arr.NonEmptyReadonlyArray<A>, E> = Queue.take(
            buffer,
          ).pipe(
            Effect.flatMap((arr) => {
              if (arr === scheduleStep) {
 ...
 * **Details**
 *
 * The source stream starts after all downstream streams have been subscribed. With the default
 * suspend strategy, the source can only advance `capacity` chunks ahead of the slowest downstream
 * stream. If a downstream stream is interrupted, it unsubscribes from the broadcast so it no longer
 * contributes backpressure.
 *
 ...
 *     );
 *
 *     const values = yield* Effect.all([Stream.runCollect(left), Stream.runCollect(right)], {
 *       concurrency: "unbounded",
 *     });
 *
 *     yield* Console.log(values);
...
/**
 * Returns a new Stream that multicasts the original stream, subscribing when the first consumer
 * starts.
 *
 * **Details**
 *
 * The upstream continues running while there is at least one consumer and is finalized after the
 * last one exits. If `idleTimeToLive` is set, the upstream is kept alive for that duration so a
 * later subscriber can continue from the next element instead of restarting.
 *
 ...
 * **Details**
 *
 * The channel receives `NonEmptyReadonlyArray` chunks and can transform both the output elements
 * and error type.
 *
...
/**
 * Pipes values through the provided channel while preserving this stream's failures alongside any
 * channel failures.
 *
 * **Details**
 *
 * Upstream failures are not passed to the channel, so the resulting stream can fail with either the
 * original stream error or the channel error.
 *
 ...
 * @category Accumulation
 */
export const collect = <A, E, R>(self: Stream<A, E, R>): Stream<Array<A>, E, R> =>
  fromEffect(runCollect(self));
 ...
 *     Stream.fromArray([1, 2, 3]).pipe(Stream.rechunk(1), Stream.accumulate),
 *   );
...
/**
 * Returns a stream that only emits elements that are not equal to the previously emitted element,
 * as determined by the specified predicate.
 *
 ...
 * **Details**
 *
 * The predicate runs for each element after the first; returning `true` treats it as equal and
 * skips it.
 *
...
/**
 * Interleaves this stream with the specified stream by alternating pulls from each stream; when one
 * ends, the remaining values from the other stream are emitted.
 *
 ...
 * **Details**
 *
 * The decider controls how many elements are pulled; if one side ends, pulls for that side are
 * ignored.
 *
    ...
    fromChannel(
      Channel.fromTransform(
        Effect.fnUntraced(function* (upstream, scope) {
          const pullDecider = yield* Channel.toTransform(Channel.flattenArray(decider.channel))(
            upstream,
            scope,
          );
          const retry = Symbol();
...
/**
 * Interrupts the evaluation of this stream when the provided effect completes. The given effect
 * will be forked as part of this stream, and its success will be discarded. This combinator will
 * also interrupt any in-progress element being pulled from upstream.
 *
 * **Details**
 *
 * If the effect completes with a failure before the stream completes, the returned stream will emit
 * that failure.
 *
...
export const interruptWhen: {
  ...
  <A, E, R, X, E2, R2>(
    self: Stream<A, E, R>,
    effect: Effect.Effect<X, E2, R2>,
  ): Stream<A, E | E2, R | R2> => fromChannel(Channel.interruptWhen(self.channel, effect)),
);
 ...
 * **Gotchas**
 *
 * This does not interrupt an in-progress pull. Use {@link interruptWhen} when the stream should be
...
export const haltWhen: {
  ...
  <A, E, R, X, E2, R2>(
    self: Stream<A, E, R>,
    effect: Effect.Effect<X, E2, R2>,
  ): Stream<A, E | E2, R | R2> => fromChannel(Channel.haltWhen(self.channel, effect)),
);
 ...
 * **Gotchas**
 *
 * Note: Unlike `Effect.onError` there is no guarantee that the provided effect will not be
 * interrupted.
 *
...
export const ensuring: {
  ...
  <A, E, R, R2>(
    self: Stream<A, E, R>,
    finalizer: Effect.Effect<unknown, never, R2>,
  ): Stream<A, E, R | R2> => fromChannel(Channel.ensuring(self.channel, finalizer)),
);

/**
 * Provides a layer or context to the stream, removing the corresponding service requirements. Use
 * `options.local` to build the layer every time; by default, layers are shared between provide
 * calls.
 *
 ...
 * class Greeter extends Context.Service<Greeter, { greet: (name: string) => string }>()(
 *   "Greeter",
 * ) {}
 *
...
export const provideContext: {
  ...
  <A, E, R, R2>(
    self: Stream<A, E, R>,
    context: Context.Context<R2>,
  ): Stream<A, E, Exclude<R, R2>> => fromChannel(Channel.provideContext(self.channel, context)),
);

/**
 * Provides the stream with a single required service, eliminating that requirement from its
 * environment.
 *
 ...
 *
 * const stream = Stream.fromEffect(
 *   Effect.service(Greeter).pipe(Effect.map((greeter) => greeter.greet("Ada"))),
 * );
 *
 * const program = Effect.gen(function* () {
...
/**
 * Provides a service to the stream using an effect, removing the requirement and adding the
 * effect's error and environment.
 *
 ...
 * import { Console, Context, Effect, Stream } from "effect";
 *
 * class ApiConfig extends Context.Service<ApiConfig, { readonly baseUrl: string }>()(
 *   "ApiConfig",
 * ) {}
 *
 * const stream = Stream.fromEffect(
...
export const bind: {
  <N extends string, A, B, E2, R2>(
    ...
    self: Stream<A, E, R>,
  ) => Stream<{ [K in N | keyof A]: K extends keyof A ? A[K] : B }, E2 | E, R2 | R>;
 ...
 * **Details**
 *
 * `Option.some` contains the last emitted element. `Option.none` means the stream completed without
 * emitting.
 *
 * **Gotchas**
 *
...
/**
 * Runs the stream, applying the effectful predicate to each element and stopping when it returns
 * `false`.
 *
 ...
 * const stream = Stream.make(1, 2, 3, 4, 5);
 * const program = Effect.gen(function* () {
 *   yield* Stream.runForEachArray(stream, (chunk) =>
 ...
 * **When to use**
 *
 * Use when bridging to Web Streams and you already have the `Context` required to run the stream
 * outside an `Effect`.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when bridging to Web Streams from inside an `Effect` so the required services can be captured
 ...
 * **When to use**
 *
 * Use when converting outside an Effect and you already have the `Context` needed to run the
 * stream.
 *
  ...
  <A, E, XR, R extends XR>(
    ...
    [Symbol.asyncIterator]() {
      ...
      return {
        ...
        return(_) {
          return runPromise(
            Effect.as(Scope.close(scope, Exit.void), { done: true, value: undefined }),
          );
        },
      };
    },
 ...
 * **When to use**
 *
 * Use when the `AsyncIterable` should be created inside Effect with the current context supplying
 * the stream's services.
 *
 ...
 * **Details**
 *
 * `shutdownOnEnd` controls whether the PubSub is shut down when the stream ends. It only shuts down
 * when set to `true`.
 *
 ...
 * **Details**
 *
 * `shutdownOnEnd` indicates whether the PubSub should be shut down when the stream ends. By default
 * this is `true`.
 *
 ...
 * **Details**
 *
 * Elements are offered to the queue as the stream runs. Stream completion is signaled with
 * `Cause.Done`, stream failures fail the queue, and the queue is shut down when the surrounding
 * scope closes.
 *
...
/**
 * Runs the stream, offering each element to the provided queue and ending it with `Cause.Done` when
