/**
 * Utilities for recurring calendar schedules written as cron expressions or explicit field
 * constraints. A `Cron` value stores allowed seconds, minutes, hours, days of month, months,
 * weekdays, and an optional time zone. The module can create or parse schedules, compare them, test
 * whether a date matches, and find previous or next scheduled occurrences.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use to represent a recurring calendar schedule that can be matched against dates or used to
 * compute scheduled occurrences.
 *
 * **Details**
 *
 * A `Cron` instance defines when a scheduled task should run, supporting seconds, minutes, hours,
 * days, months, and weekday constraints. It also supports timezone-aware scheduling.
 *
 ...
 * **Details**
 *
 * This function is a type guard that determines whether the provided value is a valid Cron instance
 * by checking for the presence of the Cron type identifier.
 *
 ...
 * **Details**
 *
 * Constructs a cron schedule by specifying which seconds, minutes, hours, days, months, and
 * weekdays the schedule should match. Empty arrays leave a time unit unrestricted. If only days or
 * weekdays are restricted, that field must match. When both are restricted, the default matches
 * either field; set `and: true` to require both fields to match. Weekdays range from `0` (Sunday)
 * to `7` (also Sunday). The constructor throws a `RangeError` when a field contains a non-integer
 ...
 * **Details**
 *
 * This error provides information about what went wrong during parsing, including the error message
 * and optionally the input that caused the error.
 *
 ...
 * **Details**
 *
 * This function is a type guard that determines whether the provided value is a CronParseError by
 * checking for the presence of the CronParseError type identifier.
 *
 ...
 * **When to use**
 *
 * Use to parse cron expressions from configuration or user input while handling invalid input as a
 * `Result`.
 *
 * **Details**
 *
 * The expression may contain five fields, where seconds default to `0`, or six fields including
 * seconds. Fields support `*`, comma-separated values, ranges, steps, and month or weekday aliases.
 * Invalid expressions fail with `CronParseError`. When both the day-of-month and weekday fields are
 * restricted, a date matches if either field matches. When either field starts with `*`, both
 * fields must match; an unrestricted field always matches.
 *
...
export const parse = (
  ...
  if (segments.length !== 5 && segments.length !== 6) {
    return Result.fail(
      new CronParseError({ message: `Invalid number of segments in cron expression`, input: cron }),
    );
  }
  ...
  return Result.all({
    ...
    Result.map(({ tz, seconds, minutes, hours, days, months, weekdays }) =>
      make({
        tz,
 ...
 * @category Constructors
 */
export const parseUnsafe = (cron: string, tz?: DateTime.TimeZone | string): Cron =>
  Result.getOrThrow(parse(cron, tz));
 ...
 * **Details**
 *
 * The schedule's timezone determines which calendar fields are read from the input; the host
 * system's timezone is used when the schedule has no timezone. Seconds, minutes, hours, and months
 * are checked against their restrictions; an empty set leaves that field unrestricted. If only
 * `days` or `weekdays` is restricted, that field must match. If both are restricted, either may
 * match unless the schedule was created with `and: true`, which requires both to match.
 *
 ...
 * **When to use**
 *
 * Use to find the next occurrence of a cron schedule after a specific date/time or after the
 * current time.
 *
 * **Details**
 *
 * Searches for the next date and time when the cron schedule should trigger, starting after the
 * specified date/time or after the current time when no date is provided.
 *
 ...
 * **When to use**
 *
 * Use to find the most recent occurrence of a cron schedule before a specific date/time or before
 * the current time.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * The search is strict: if the supplied date/time already matches the schedule, the result is the
 * earlier occurrence.
 *
 * @since 3.20.0
 * @category Getters
      ...
      if (cron.months.size !== 0) {
        ...
        const clampBoundaryDay = (targetMonthIndex: number): number => {
          const maxDayInMonth = daysInMonth(
            new Date(Date.UTC(current.getUTCFullYear(), targetMonthIndex + 1, 0)),
          );
          if (cron.days.size !== 0 && cron.weekdays.size === 0) {
 ...
 * **Details**
 *
 * The iterator generates an infinite sequence of dates when the cron schedule should trigger,
 * starting after the specified date/time or after the current time when no date is provided.
 *
...
/**
 * Equivalence instance for comparing the timezone, field restrictions, and day-matching mode of two
 * `Cron` schedules.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * This comparison checks the optional timezone, the `and` day-matching mode, seconds, minutes,
...
/**
 * Checks whether two `Cron` instances have equal timezone values, field restrictions, and
 * day-matching modes.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The comparison checks the optional timezone, the `and` day-matching mode, seconds, minutes,
 * hours, days, months, and weekdays.
 *
  ...
  for (let index = 0; index < fields.length; index++) {
    ...
    if (step !== undefined) {
      if (!Number.isInteger(step)) {
        return Result.fail(
          new CronParseError({ message: `Expected step value to be a positive integer`, input }),
        );
      }
      if (step < 1) {
        return Result.fail(
          new CronParseError({ message: `Expected step value to be greater than 0`, input }),
        );
      }
      if (step > options.max) {
        return Result.fail(
