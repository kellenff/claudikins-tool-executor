 ...
 *
 * The runtime helpers in this module create new tuples instead of mutating their inputs, and the
 * types preserve element positions where possible. The helpers cover tuple construction, indexed
 * access, selecting or removing positions, appending values, transforming elements, renaming
 * indices, mapping typed positions, and deriving comparison or combination helpers for tuple
 * shapes.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need a properly typed tuple without writing `[a, b, c] as const` or another manual
 * cast.
 *
 * **Details**
 *
...
export const get: {
  ...
  2,
  <T extends ReadonlyArray<unknown>, I extends keyof T>(self: T, index: I): T[I] => self[index],
);
 ...
 * **When to use**
 *
 * Use when you need the appended value to remain part of the tuple's type-level shape and preserve
 * literal element positions.
 *
 * **Details**
 *
...
} = dual(2, <T extends ReadonlyArray<unknown>, E>(self: T, element: E): [...T, E] => [
  ...self,
  element,
]);
 ...
 * **Details**
 *
 * The result type is `[...T1, ...T2]`, preserving all element types from both tuples. Neither input
 * tuple is mutated; a fresh tuple is returned.
 *
...
export const appendElements: {
  ...
  <const T1 extends ReadonlyArray<unknown>, const T2 extends ReadonlyArray<unknown>>(
    self: T1,
    that: T2,
  ): [...T1, ...T2];
} = dual(
...
/**
 * Transforms elements of a tuple by providing an array of transform functions. Each function
 * applies to the element at the same position. Positions beyond the array's length are copied
 * unchanged.
 *
 * **When to use**
 *
...
/**
 * Renames tuple indices by providing an array of stringified source indices. Each position in the
 * array specifies which index to read from (e.g., `["2", "1", "0"]` reverses a 3-element tuple).
 *
 * **When to use**
 *
...
export const map: {
  <L extends Lambda>(
    lambda: L,
  ): <const T extends ReadonlyArray<unknown>>(self: T) => { [K in keyof T]: Apply<L, T[K]> };
...
/**
 * Applies a `Struct.Lambda` transformation only to the elements at the specified indices; all other
 * elements are copied unchanged.
 *
 * **When to use**
 *
...
/**
 * Applies a `Struct.Lambda` transformation to all elements except those at the specified indices;
 * the excluded elements are copied unchanged.
 *
 * **When to use**
 *
...
/**
 * Creates an `Equivalence` for tuples by comparing corresponding elements using the provided
 * per-position `Equivalence`s. Two tuples are equivalent when all their corresponding elements are
 * equivalent.
 *
 * **When to use**
 *
...
/**
 * Creates an `Order` for tuples by comparing corresponding elements using the provided per-position
 * `Order`s. Elements are compared left-to-right; the first non-zero comparison determines the
 * result.
 *
 * **When to use**
 *
 * Use when you need to sort fixed-position arrays lexicographically, with each position using its
 * own ordering rule.
 *
 * **Details**
 *
...
export {
  /**
   * Checks whether an array has exactly `N` elements, narrowing the type to a fixed-length tuple.
   *
   * **When to use**
   *
   ...
   * **Details**
   *
   * This is a re-export of `Predicate.isTupleOf`. It narrows the type to `TupleOf<N, T>` in the
   * truthy branch.
   *
   * **Gotchas**
   *
  ...
  isTupleOf,
  /**
   * Checks whether an array has at least `N` elements, narrowing the type to a tuple with a minimum
   * length.
   *
   * **When to use**
   *
   ...
   * **Details**
   *
   * This is a re-export of `Predicate.isTupleOfAtLeast`. It narrows the type to `TupleOfAtLeast<N,
   * T>` in the truthy branch.
   *
   * **Gotchas**
   *
   * This only checks `.length`; it does not validate element types.
...
/**
 * Creates a `Combiner` for a tuple shape by providing a `Combiner` for each position. When two
 * tuples are combined, each element is merged using its corresponding combiner.
 *
 * **When to use**
 *
 * Use when you need to merge two same-shape tuples by combining each position independently, such
...
/**
 * Creates a `Reducer` for a tuple shape by providing a `Reducer` for each position. The initial
 * value is derived from each position's `Reducer.initialValue`. When reducing a collection of
 * tuples, each element is combined independently.
 *
 * **When to use**
 *
 * Use when you need to fold same-shape tuples by accumulating each position independently into one
 * summary tuple.
 *
