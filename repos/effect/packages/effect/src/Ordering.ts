/**
 * The standard result of comparing two values. An `Ordering` is `-1` when the first value is less
 * than the second, `0` when both values compare as equal, and `1` when the first value is greater
 * than the second. This module also provides helpers for reversing an ordering, matching on the
 * three cases, and combining ordered comparison results with a reducer.
 *
 * @since 2.0.0
 */
...
/**
 * Reverses the ordering of the input Ordering. This is useful for creating descending sort orders
 * from ascending ones.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use to combine multiple comparison results in priority order, such as checking secondary criteria
 * only when earlier criteria compare as equal.
 *
 * **Details**
 *
 * If any of the `Ordering`s is non-zero, the result is the first non-zero `Ordering`. If all the
 * `Ordering`s are zero, the result is zero.
 *
 * **Gotchas**
 *
