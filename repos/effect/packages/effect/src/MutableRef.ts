 ...
 *
 * A `MutableRef<A>` stores one current value and exposes it through `.current`. Unlike `Ref`, its
 * operations are synchronous and update the same object in place. This module includes pipeable
 * helpers for reading, setting, comparing, and updating the value, plus numeric increment/decrement
 * helpers and a boolean toggle helper.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * Read or write the value directly through `.current`, or use the `MutableRef` helpers for pipeable
 * updates such as `get`, `set`, `update`, and `compareAndSet`. All operations mutate the same
 * reference in place.
 *
...
/**
 * Sets the value to newValue atomically if the current value equals oldValue. Returns true if the
 * value was updated, false otherwise. Uses Effect's Equal interface for value comparison.
 *
 * **When to use**
 *
...
/**
 * Updates the MutableRef with the result of applying a function to its current value, and returns
 * the previous value.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use when you need an in-place `MutableRef` value transformation that returns the same
 * `MutableRef`.
 *
