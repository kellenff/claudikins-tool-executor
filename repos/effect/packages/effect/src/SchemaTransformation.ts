 ...
 *
 * A `Transformation<T, E>` describes how to decode an encoded value into a decoded value and how to
 * encode it back again. Schema APIs use transformations to connect two representations, such as a
 * string and a number, a JSON value and a richer TypeScript value, or a form field and an
 * application value. This module includes transformation and middleware types, constructors for
 * pure or effectful conversions, and common conversions used by the Schema module.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need a schema middleware to catch or recover from parsing errors (e.g.
 * `Schema.catchDecoding`), run side effects around the parsing pipeline, or access the full
 * `Effect` rather than a single decoded value.
 *
 * **Details**
 *
 * Unlike `Transformation`, which operates on individual values via `Getter`, `Middleware` receives
 * the full `Effect` produced by the inner schema and can intercept, modify, retry, or replace it.
 *
 ...
 *
 * Typically constructed indirectly via `Schema.middlewareDecoding` or `Schema.middlewareEncoding`
...
/**
 * Represents a bidirectional transformation between a decoded type `T` and an encoded type `E`,
 * built from a pair of `Getter`s.
 *
 * **When to use**
 *
 * Use when you need a schema transformation that defines how a schema converts between two
 * representations.
 *
 ...
 * **Details**
 *
 * This is the primary building block for `Schema.decodeTo`, `Schema.encodeTo`, `Schema.decode`,
 * `Schema.encode`, and `Schema.link`. Each direction is a `SchemaGetter.Getter` that handles
 * optionality, failure, and Effect services.
 *
 * - Immutable — `flip()` and `compose()` return new instances.
 * - `flip()` swaps the decode and encode getters.
 * - `compose(other)` chains: `this.decode` then `other.decode` for decoding, `other.encode` then
 *   `this.encode` for encoding.
 *
...
/**
 * Constructs a `Transformation` from an object with `decode` and `encode` `Getter`s. If the input
 * is already a `Transformation`, returns it as-is.
 *
 * **When to use**
 *
 * Use when you already have schema getter instances and want to pair them into a schema
 * transformation.
 *
 ...
 *           ? Effect.fail(
 *               new SchemaIssue.InvalidValue(Option.some(s), { message: "Invalid date" }),
 *             )
 *           : Effect.succeed(d);
 *       },
...
/**
 * Creates a `Transformation` where decode and encode operate on `Option` values, giving full
 * control over missing-key handling.
 *
 * **When to use**
 *
 ...
 * - `Option.None` input means the key is absent; returning `Option.None` omits the key from the
 *   output.
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to normalize user input by stripping leading/trailing
 * whitespace.
 *
 * **Details**
 *
 * Decoding applies `String.prototype.trim()`. Encoding is passthrough and returns the string
 * unchanged. This is not round-trippable if the original had whitespace.
 *
...
/**
 * Transforms strings by converting snake_case to camelCase on decode and camelCase to snake_case on
 * encode.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to convert API field names between snake_case and
 * camelCase conventions.
 *
 * **Details**
 *
 * Decoding converts values such as `"my_field_name"` to `"myFieldName"`. Encoding converts values
 * such as `"myFieldName"` back to `"my_field_name"`. The transformation is round-trippable for
 * standard snake_case and camelCase.
 *
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to normalize strings to lowercase (e.g. email
 * addresses).
 *
 * **Details**
 *
 * Decoding applies `String.prototype.toLowerCase()`. Encoding is passthrough. This is not
 * round-trippable if the original had uppercase characters.
 *
 ...
 * **Details**
 *
 * Decoding applies `String.prototype.toUpperCase()`. Encoding is passthrough. This is not
 * round-trippable if the original had lowercase characters.
 *
...
/**
 * Transforms a string into a record of key-value pairs and encodes a record of key-value pairs into
 * a string.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to parse query-string-like or config-file-like strings
 * into records.
 *
 * **Details**
 *
 * Decoding splits the string by `separator` (default `","`) into pairs, then splits each pair by
 * `keyValueSeparator` (default `"="`). Encoding joins the record back into a string using the same
 * separators. The transformation is round-trippable when keys and values do not contain the
 * separators.
 *
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to connect two schemas that share the same type with no
 * actual conversion.
 *
 * **Details**
 *
 ...
 * - By default, `T` and `E` must be the same type. Pass `{ strict: false }` to bypass the type
 *   constraint.
 *
...
/**
 * Transforms values without changing them, typed so that `T extends E`, where the decoded type `T`
 * is a subtype of the encoded type `E`.
 *
 * **When to use**
 *
 * Use when you need a no-op schema transformation whose decoded side is narrower than the encoded
 * side.
 *
 * **Details**
 *
...
/**
 * Transforms values without changing them, typed so that `E extends T` — the encoded type is a
 * subtype of the decoded type.
 *
 * **When to use**
 *
 * Use when you need a no-op schema transformation whose encoded side is more specific than its
 * decoded side.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to parse numeric strings from APIs, form data, or URL
 * parameters.
 *
 * **Details**
 *
 * Decoding coerces the string to a number like `Number(s)`. Encoding coerces the number to a string
 * like `String(n)`. This does not validate that the result is finite; combine with `Schema.Finite`
 * or `Schema.Int` for stricter checks.
 *
 ...
 * @see {@link transform}
 */
export const numberFromString = new Transformation(SchemaGetter.Number(), SchemaGetter.String());
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to parse large integer strings (e.g. database IDs,
 * blockchain values).
 *
 * **Details**
 *
 * Decoding coerces the string to a bigint like `BigInt(s)`. Encoding coerces the bigint to a string
 * like `String(n)`. Decoding fails if the string is not a valid bigint representation.
 *
 ...
 * @see {@link transform}
 */
export const bigintFromString = new Transformation(SchemaGetter.BigInt(), SchemaGetter.String());
 ...
 * **Details**
 *
 * Decoding creates a `Date` from the string like `new Date(s)`. Encoding converts the `Date` to an
 * ISO string like `date.toISOString()`, returning `"Invalid Date"` for invalid dates.
 *
 ...
 * **When to use**
 *
 * Use when you need a schema transformation for numeric timestamps represented as milliseconds
 * since the Unix epoch.
 *
 * **Details**
 *
 * Decoding creates a `Date` from the number like `new Date(ms)`. Encoding returns the `Date`
 * timestamp like `date.getTime()`.
 *
 * **Gotchas**
 *
 * This transformation does not validate date validity. `NaN`, `Infinity`, and `-Infinity` decode to
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to parse human-readable duration strings from APIs,
 * config, or user input.
 *
 * **Details**
 *
 * Decoding accepts any string that `Duration.fromInput` can parse, including `"Infinity"` and
 * `"-Infinity"`. Encoding returns `String(duration)`, producing strings such as `"2000 millis"` or
 * `"10 nanos"` that round-trip through the parser.
 *
...
/**
 * Decodes a `bigint` (nanoseconds) into a `Duration` and encodes a `Duration` back to `bigint`
 * nanoseconds.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Decoding always succeeds and creates a `Duration` from nanoseconds. Encoding fails with
 * `InvalidValue` if the `Duration` cannot be represented as a `bigint`, such as
 * `Duration.infinity`.
 *
...
/**
 * Decodes a `number` of milliseconds into a `Duration` and encodes a `Duration` back to
 * milliseconds.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to decode timeouts, delays, elapsed intervals, or other
 * duration values stored as millisecond counts.
 *
 * **Details**
 *
 * Decode creates a duration from the number, and encode returns the duration length in
 * milliseconds.
 *
...
const decodeDefect = (input: Json): unknown =>
  isJsonError(input) ? decodeJsonError(input) : input;
 ...
 * **Details**
 *
 * Decoding maps `null` to `Option.none()` and non-null values to `Option.some(value)`. Encoding
 * maps `Option.none()` to `null` and `Option.some(value)` to `value`. The transformation is pure
 * and synchronous.
 *
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to convert API fields that use `undefined` for absence
 * to `Option`.
 *
 * **Details**
 *
 * Decoding maps `undefined` to `Option.none()` and non-undefined values to `Option.some(value)`.
 * Encoding maps `Option.none()` to `undefined` and `Option.some(value)` to `value`. The
 * transformation is pure and synchronous.
 *
...
/**
 * Decodes `T | null | undefined` into `Option<T>` and encodes `Option<T>` back to `T | null` or `T
 * | undefined` depending on the provided `options.onNoneEncoding` (defaults to `undefined`).
 *
 * **When to use**
 *
 * Use when you need a schema transformation to convert nullish API fields to `Option` when both
 * `null` and `undefined` represent absence.
 *
 * **Details**
 *
 * Decoding maps `null` and `undefined` to `Option.none()` and all other values to
 * `Option.some(value)`. Encoding maps `Option.none()` to `null` or `undefined` according to
 * `options.onNoneEncoding`, and maps `Option.some(value)` to `value`. The transformation is pure
 * and synchronous.
 *
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to convert optional struct keys (declared with
 * `Schema.optionalKey`) to `Option` values.
 *
 * **Details**
 *
 * Decoding maps an absent key (`None`) to `Some(None)` and a present key (`Some(v)`) to
 * `Some(Some(v))`. Encoding maps `Some(None)` to `None` to omit the key, and maps `Some(Some(v))`
 * to `Some(v)`. This uses `transformOptional` under the hood.
 *
...
/**
 * Decodes optional values into `Option<T>` and encodes `Option.none()` back to an omitted optional
 * value.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to convert optional (possibly `undefined`) values to
 * `Option`.
 *
 * **Details**
 *
 * Decoding maps an absent or `undefined` value to `Some(None)` and a present value to
 * `Some(Some(v))`. Encoding maps `Some(None)` to `None` to omit the value, and maps `Some(Some(v))`
 * to `Some(v)`. This uses `transformOptional` under the hood and filters out `undefined` on
 * decode.
 *
 ...
 * **Details**
 *
 * Decoding checks `URL.canParse(s)` and fails with `InvalidValue` if the string is not a valid URL.
 * Encoding returns `url.href`.
 *
...
/**
 * Decodes a `string` into a `BigDecimal` and encodes a `BigDecimal` back to its string
 * representation.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to parse decimal number strings from APIs or user
 * input.
 *
 * **Details**
 *
 * Decoding calls `BigDecimal.fromString(s)` and fails with `InvalidValue` if the string is not a
 * valid `BigDecimal` representation. Encoding returns `BigDecimal.format(bd)`.
 *
 * @since 4.0.0
 * @category Transforming
 */
...
/**
 * Decodes a Base64-encoded `string` into a `Uint8Array` and encodes a `Uint8Array` back to a Base64
 * string.
 *
 * **When to use**
 *
 * Use when you need a schema transformation for binary data transmitted as Base64 strings (e.g.
...
export const uint8ArrayFromBase64String: Transformation<
  ...
  string
> = new Transformation(SchemaGetter.decodeBase64(), SchemaGetter.encodeBase64());

/**
 * Decodes a Base64-encoded `string` into a UTF-8 `string` and encodes a UTF-8 `string` back to a
 * Base64 string.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Decoding parses the Base64 string into a UTF-8 string. Encoding writes the string as a Base64
 * string.
 *
 ...
 * **Details**
 *
 * Decoding parses the Base64 URL string into a UTF-8 string. Encoding writes the string as a Base64
 * URL string.
 *
...
/**
 * Decodes a URI component encoded string into a UTF-8 string and encodes a UTF-8 string into a URI
 * component encoded string.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to store structured data in URL query parameters or
 * fragments, such as composing with `Schema.parseJson` to round-trip JSON through a URL.
 *
 * **Details**
 *
 * Decoding calls `decodeURIComponent` and fails if the input contains malformed percent-encoding
 * sequences. Encoding calls `encodeURIComponent`.
 *
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to decode JSON stored or transmitted as a string,
 * usually before composing with another schema that validates the parsed structure.
 *
 * **Details**
 *
 * Decode fails with `InvalidValue` for invalid JSON, and encode can fail with `InvalidValue` when
...
/**
 * Decodes a `FormData` instance into a nested record using bracket-path keys and encodes
 * object-like values back into `FormData`.
 *
 * **When to use**
 *
 * Use when you need a schema transformation for form or multipart payloads whose keys, such as
 * `user[name]` or `items[0]`, should become nested data.
 *
 * **Details**
 *
 * Decode preserves string and `Blob` leaves. Encode flattens nested objects and arrays into
 * bracket-path entries and returns an empty `FormData` for non-object inputs.
 *
...
/**
 * Decodes `URLSearchParams` into a nested record using bracket-path keys and encodes object-like
 * values back into `URLSearchParams`.
 *
 * **When to use**
 *
 * Use when you need a schema transformation for query strings whose keys, such as `filter[name]` or
 * `items[0]`, should become nested data.
 *
 * **Details**
 *
 * Decode produces string leaves. Encode flattens nested objects and arrays into bracket-path
 * entries and returns empty `URLSearchParams` for non-object inputs.
 *
...
/**
 * Decodes a numeric time-zone offset in milliseconds into a `DateTime.TimeZone.Offset` and encodes
 * it back to the offset number.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to represent fixed-offset time zones with numeric
 * millisecond offsets.
 *
 * **Details**
 *
...
/**
 * Decodes an IANA time-zone identifier string into a `DateTime.TimeZone.Named` and encodes a named
 * time zone back to its `id`.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to accept only IANA time-zone identifier strings and
 * produce `DateTime.TimeZone.Named` values.
 *
 * **Details**
 *
 ...
 */
export const timeZoneNamedFromString: Transformation<DateTime.TimeZone.Named, string> =
...
/**
 * Decodes a string into a `DateTime.TimeZone` and encodes a time zone back to its string
 * representation.
 *
 * **When to use**
 *
 * Use when you need a schema transformation to accept either an IANA time-zone identifier or an
 * offset string and produce a general `DateTime.TimeZone`.
 *
 * **Details**
 *
 * Accepted decode inputs include valid IANA identifiers and offset strings such as `"+03:00"`.
 * Decode fails with `InvalidValue` when the string cannot be parsed as a time zone.
 *
 * @since 4.0.0
 * @category Transforming
...
>({
  decode: (s) => {
    return Option.match(DateTime.zoneFromString(s), {
      onNone: () =>
        Effect.fail(
          new SchemaIssue.InvalidValue(Option.some(s), { message: `Invalid time zone: ${s}` }),
        ),
      onSome: Effect.succeed,
    });
  },
 ...
 * **When to use**
 *
 * Use when you need a schema transformation to decode date-time strings to a normalized
 * `DateTime.Utc` and encode back as a UTC ISO string.
 *
 * **Details**
 *
 * Decode accepts strings supported by `DateTime.make`, converts the result to UTC, and fails with
 * `InvalidValue` when parsing fails. Encode uses `DateTime.formatIso`.
 *
 * @since 4.0.0
 * @category Transforming
...
/**
 * Decodes a zoned date-time string into a `DateTime.Zoned` and encodes it back to an ISO zoned
 * string.
 *
 * **When to use**
 *
 * Use when you need a schema transformation for ISO zoned date-time strings that decode to
 * `DateTime.Zoned` and encode with `DateTime.formatIsoZoned`.
 *
 * **Details**
 *
 * Decode uses `DateTime.makeZonedFromString` and fails with `InvalidValue` when the input is not a
 * valid zoned date-time. Encode uses `DateTime.formatIsoZoned`.
 *
 * @since 4.0.0
 * @category Transforming
