 ...
 *
 * The `Console` service exposes common console methods such as logging, warnings, errors, groups,
 * counters, tables, and timers. Because console access goes through a service, programs can use
 * custom console implementations in tests or other environments. This module also includes scoped
...
/**
 * Context reference for the current console service in the Effect system, allowing access to the
 * active console implementation from within the Effect context.
 *
 * **When to use**
 *
 * Use when you need an effect to run against a provided console implementation, such as tests or
 * alternate runtimes, rather than the default console.
 *
 * **Details**
 *
...
/**
 * Creates an Effect that provides access to the current console service and lets you perform
 * operations with it within an Effect context.
 *
...
): Effect.Effect<A, E, R> => core.withFiber((fiber) => f(fiber.getRef(Console)));

/**
 * Writes the supplied assertion message to the console as an error when `condition` is false; when
 * `condition` is true, no console output is produced.
 *
...
/**
 * Logs and increments the counter associated with `label`, using the console's default counter when
 ...
 * **Details**
 *
 * The arguments are passed to the service's `debug` method when the returned Effect is executed.
 * Any filtering behavior depends on the active console implementation.
 *
...
/**
 * Displays an interactive list of the properties of the specified object, optionally using
 * console-specific inspection options for debugging complex data structures.
 *
...
/**
 * Displays an interactive tree of descendant XML or HTML elements, which is particularly useful for
 * inspecting DOM elements in browser environments.
 *
...
/**
 * Writes an error-level message to the console, typically displayed with error styling by the
...
/**
 * Creates a scoped console group, optionally collapsed and labeled, and closes it automatically
 * when the Effect scope is finalized.
 *
...
/**
 * Writes an informational message to the console, typically displayed with info styling by the
 * active console implementation.
 *
...
/**
 * Displays tabular data as a formatted table in the console, optionally limited to selected
 * properties.
 *
...
/**
 * Logs the elapsed time for an existing timer without stopping it, allowing progress reports for
...
/**
 * Writes the current stack trace to the console to show how the current point in the code was
 * reached.
 *
...
/**
 * Writes a warning-level message to the console, typically displayed with warning styling by the
 * active console implementation.
 *
...
/**
 * Runs an Effect inside an optionally labeled or collapsed console group, starting the group before
 * execution and ending it after the Effect completes.
 *
...
/**
 * Runs an Effect with a console timer, starting the timer before execution and ending it after the
 * Effect completes.
 *
