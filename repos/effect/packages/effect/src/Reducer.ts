/**
 * Reusable strategies for reducing many values into one value. A `Reducer<A>` extends
 * `Combiner.Combiner` with an `initialValue` for empty collections and a `combineAll` method for
 * folding an entire iterable. This module provides a constructor for reducers and a helper for
 * reversing the order in which values are combined.
 *
 * @since 4.0.0
 */
 ...
 *
 * - You want a reusable reducing strategy that can be passed to library functions like
 *   `Struct.makeReducer`, `Option.makeReducer`, or `Record.makeReducerUnion`.
 ...
 *
 * - You want to provide an optimized `combineAll` (e.g. short-circuiting on a known absorbing element
 *   like `0` for multiplication).
 *
 * **Details**
 *
 ...
 * non-commutative reducer such as string concatenation.
 *
 * **Details**
 *
 * - Returns a new `Reducer` where `combine(self, that)` calls the original reducer as `combine(that,
 *   self)`.
 * - The `initialValue` is preserved from the original reducer.
 * - The `combineAll` is re-derived from the flipped `combine` (using the default left-to-right fold),
