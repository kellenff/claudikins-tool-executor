 ...
 *
 * A `Request` describes what a fiber needs, while a `RequestResolver` describes how to collect
 * request entries, group them into batches, run backend work, and complete each waiting entry. This
 * module includes constructors for common resolver shapes and tools for controlling batching,
 * grouping, delays, tracing, caching, racing, hooks around resolver execution, and persistence.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * A resolver controls how requests are grouped, delayed, optionally pre-checked, and finally run.
 * Its `runAll` method receives a non-empty batch of `Request.Entry` values for a single batch key
 * and must complete every received entry, usually by calling `completeUnsafe` or one of the
 * `Request` completion helpers.
 *
 * **Gotchas**
 *
 * If a resolver finishes without completing an entry, the waiting request fails because the
 * resolver did not supply a result.
 *
...
export interface RequestResolver<in A extends Request.Any>
  ...
  /**
   * An optional pre-check function that can be used to filter requests before they are added to a
   * batch. If the function returns `false`, the request will not be processed.
   */
  readonly preCheck: ((entry: Request.Entry<A>) => boolean) | undefined;

  /**
   * Should the resolver continue collecting requests? Otherwise, it will immediately execute the
   * collected requests cutting the delay short.
   */
...
export declare namespace RequestResolver {
   ...
   * **Details**
   *
   * This marker preserves the request type accepted by the resolver for Effect's type-level
   * machinery. Users normally do not implement it directly.
   *
   * @since 2.0.0
   * @category Models
   */
 ...
 * **When to use**
 *
 * Use when you need to supply the resolver batching primitives directly, including the batch key,
 * optional pre-check, delay effect, collection cutoff, and batch runner.
 *
 * **Details**
 *
 * `batchKey` groups request entries, `delay` schedules batch execution, `collectWhile` can end
 * collection early, and `runAll` receives a non-empty batch for one key.
 *
 * **Gotchas**
 *
 * Accepted entries must be completed. If `runAll` succeeds with incomplete entries, waiting
 * requests fail. If `preCheck` returns `false`, the entry is not batched, so it must be completed
 * or linked to another completion path.
 *
 * @since 4.0.0
...
/**
 * Constructs a request resolver from a pure function that takes a list of requests and returns a
 * list of results of the same size. Each item in the result list must correspond to the item at the
 * same index in the request list.
 *
...
/**
 * Constructs a request resolver from a list of tags paired to functions, that takes a list of
 * requests and returns a list of results of the same size. Each item in the result list must
 * correspond to the item at the same index in the request list.
 *
  ...
  >(
    ...
    make<A>((entries): Effect.Effect<void> => {
      ...
      return Effect.forEach(
        ...
        ([tag, requests]) =>
          Effect.matchCause(
            (fns[tag] as any)(requests) as Effect.Effect<Array<any>, unknown, unknown>,
            {
              onFailure: (cause) => {
 ...
 * **When to use**
 *
 * Use as a resolver value for request types that are statically impossible and should never be
 * issued.
 *
 * **Gotchas**
 *
 * If this resolver is used for an actual request, the request waits forever unless the fiber is
 * interrupted.
 *
 * @since 2.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * When more than `n` requests are waiting for the same resolver and batch key, the current batch is
 * run and additional requests are collected into later batches.
 *
...
/**
 * Returns a request resolver that sends each batch to both resolvers and completes with the first
 * resolver to finish.
 *
 * **Details**
 *
...
/**
 * Adds a tracing span to the request resolver, which will also add any span links from the
 * request's.
 *
...
/**
 * Wraps a request resolver in a cache, allowing it to cache results up to a specified capacity and
 * optional time-to-live.
 *
 * **When to use**
 *
 * Use to turn a request resolver into a first-class `Cache` when callers need cache lookup,
 * refresh, invalidation, or inspection around request results.
 *
 * **Details**
 *
 * The request value is the cache key. Cache misses run the resolver via `Effect.request`,
 * `timeToLive` receives the request `Exit` and the request, and `requireServicesAt` controls
 * whether services are required at lookup time or construction time.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use to reuse completed results for repeated equal request values while still passing a
 * `RequestResolver` to `Effect.request`.
 *
 * **Details**
 *
 * Running the returned effect creates the cache and returns a wrapped resolver. The cache stores
 * completed success or failure results by request equality up to `capacity`. The `strategy` option
 * controls eviction order and defaults to `"lru"`; `"fifo"` keeps insertion order.
 *
 * **Gotchas**
 *
 * Entries do not expire by time, and completed failures are cached the same as successes. Request
 * equality controls cache hits.
 *
 * @since 4.0.0
 * @category Caching
 ...
 * **When to use**
 *
 * Use to keep a `RequestResolver` interface while reusing completed `Persistable` request results
 * through a `Persistence` store.
 *
 * **Details**
 *
 * Cached results are loaded from the configured persistence store before running the underlying
 * resolver. Missing entries are resolved normally and written back to the store. Entries marked
 * stale by `staleWhileRevalidate` receive the stored result and are also resolved again so the
 * refreshed result can be written back to the store. Creating the persisted resolver requires
...
export const persisted: {
  <
    A extends Request.Request<any, Persistence.PersistenceError | Schema.SchemaError, any> &
      Persistable.Any,
  >(options: {
  ...
  >(
    ...
    return makeWith<A>({
      ...
      runAll: Effect.fnUntraced(function* (entries, key) {
        ...
        yield* store
          .setMany(toPersist)
          .pipe(Effect.provideContext(entries[0].context)) as Effect.Effect<void, Request.Error<A>>;
      }),
    });
  }),
);
