/**
 * The `PrimaryKey` module defines a small protocol for values that can expose a stable,
 * string-based identifier. A value participates by implementing a method at {@link symbol};
 * consumers can check unknown values with {@link isPrimaryKey} and read the key with
 * {@link value}.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use to implement the `PrimaryKey` protocol as a computed property key on classes or object
 * literals that expose a stable string identifier.
 *
 * @since 2.0.0
 * @category Symbols
 ...
 * **When to use**
 *
 * Use to define values that expose a stable string identifier for equality, hashing, caching, or
 * persistence.
 *
 * **Details**
 *
 * Objects implementing this interface must provide a method that returns a unique string
 * identifier.
 *
