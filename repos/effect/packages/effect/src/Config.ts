/**
 * Descriptions of configuration values that can be read from a `ConfigProvider`. A `Config<T>`
 * explains which keys to read, how to decode and validate them, and how to combine defaults,
 * fallbacks, nested paths, and multiple settings. Configs are also Effects, so they can be yielded
 * in `Effect.gen` after a provider has been supplied.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need to distinguish a `Config` from an unknown value before calling `.parse` or
 * {@link unwrap}.
 *
 ...
 * - A `SchemaError` — the data was found but did not match the schema (wrong type, out of range,
 *   missing key, etc.).
 *
 * @since 4.0.0
 * @category Errors
 ...
 * **When to use**
 *
 * Use to describe typed configuration that can be parsed from a provider or yielded inside
 * `Effect.gen`.
 *
 * **Details**
 *
 * Key members:
 *
 * - `parse(provider, pathPrefix?)` – runs the config against a specific provider. The optional path
 *   prefix is the logical scope accumulated from outer `Config.nested` calls.
 * - Yieldable – can be yielded inside `Effect.gen`, which automatically resolves the current
 *   `ConfigProvider` from the context.
...
function make<T>(
  ...
  self.parse = (provider: ConfigProvider.ConfigProvider, pathPrefix: Path = []) =>
    parse(provider, pathPrefix);
  return self;
}
 ...
 * **When to use**
 *
 * Use when you need to transform a parsed config value with a function that can produce a
 * `ConfigError` (e.g. parsing a URL, checking a range).
 *
 ...
 * **Details**
 *
 * Unlike {@link withDefault}, this catches **all** `ConfigError`s (not just missing data). The
 * fallback function receives the error and returns a new `Config`.
 *
...
export const orElse: {
  ...
  return make((provider, pathPrefix) =>
    Effect.catch(self.parse(provider, pathPrefix), (error) =>
      that(error).parse(provider, pathPrefix),
    ),
  );
});
 ...
 * **Details**
 *
 * Accepts a tuple (preserves positions), an iterable, or a record of configs. Returns a config
 * whose parsed value mirrors the input shape.
 *
 ...
 * **Gotchas**
 *
 * Only applies when the error is a `SchemaError` caused exclusively by missing data (missing keys,
 * undefined values). Validation errors (wrong type, out of range) still propagate.
 *
 ...
 * **Gotchas**
 *
 * Like {@link withDefault}, only missing-data errors produce `None`. Validation errors still
 * propagate.
 *
 ...
 * **When to use**
 *
 * Use to derive the parsed value type from an existing `Config` value when declaring reusable
 * config-driven types.
 *
 * @since 2.5.0
 ...
 * **When to use**
 *
 * Use when typing the input of {@link unwrap} so callers can pass either a `Config` or a record of
 * `Config`s.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when accepting config from callers who may pass either a single `Config` or a record of
 * individual `Config`s.
 *
 * **Details**
 *
 * If the input is already a `Config`, it is returned as-is. Otherwise, each key is recursively
 * unwrapped and combined.
 *
...
const dump: (
  provider: ConfigProvider.ConfigProvider,
  path: Path,
) => Effect.Effect<Schema.StringTree, SourceError> = Effect.fnUntraced(function* (provider, path) {
 ...
 * **Details**
 *
 * The optional `path` sets the local path segment(s) for the config lookup. It is appended to the
 * logical path prefix accumulated from outer {@link nested} calls. Pass a single string for a flat
 * key or an array for nested paths.
 *
 * Convenience constructors such as `string`, `number`, and `boolean` delegate to this API.
 *
 * The codec is used to decode the raw `StringTree` produced by the provider into `T`. Schema
 * validation errors are wrapped in `ConfigError`.
 *
 ...
 * **Details**
 *
 * Accepted string values: `true`, `false`, `yes`, `no`, `on`, `off`, `1`, `0`, `y`, `n`
 * (case-sensitive).
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * `"key1=val1,key2=val2"`. The `separator` (default `","`) and `keyValueSeparator` (default `"="`)
 * can be customized.
 *
...
export {
   ...
   * **Details**
   *
   * Accepts either a JSON-like array from the provider or a flat string like `"a,b,c"`. The
   * `separator` defaults to `","` and can be customized.
   *
   * @since 4.0.0
   * @category Schemas
   */
 ...
 * **When to use**
 *
 * Use when you need to re-raise a specific config error, such as inside {@link orElse}.
 *
 * @since 2.0.0
 ...
 * **When to use**
 *
 * Use when you need a hardcoded config value, such as inside {@link orElse} or tests.
 *
 ...
 * **When to use**
 *
 * Use when you need config input to accept JavaScript's full number domain, including NaN and
 * infinities, rather than reject non-finite values.
 *
 * **Details**
 *
 ...
 *
 * Accepts any string that `Duration.fromInput` can parse (e.g. `"10 seconds"`, `"500 millis"`,
 * `"Infinity"`, `"-Infinity"`).
 *
...
/**
 * Creates a config for a redacted string value. The parsed result is wrapped in a `Redacted`
 * container that hides the value from logs and `toString`.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The prefix is prepended to every key the inner config reads. With `fromUnknown` this means an
 * extra object level; with `fromEnv` it means a `_`-separated prefix on env var names.
 *
