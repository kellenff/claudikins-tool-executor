 ...
 *
 * A `Trie<Value>` is similar to a map whose keys are strings, but it is built for looking up keys
 * by prefix. It is useful for autocomplete, route tables, dictionaries, and command lookup. Updates
 * return new tries, and the module includes exact lookup, prefix lookup, longest-prefix lookup,
 * iteration, mapping, filtering, reducing, and traversal helpers.
 *
 * @since 2.0.0
 */
...
/**
 * An immutable string-keyed map optimized for prefix lookup. Iteration yields `[key, value]` pairs
 * in key order, and update operations such as insert and remove return new `Trie` values.
 *
 ...
 * ]);
 * assert.equal(
 *   Equal.equals(Trie.make(["call", 0], ["me", 1], ["mind", 2], ["mid", 3]), trie),
 *   true,
 * );
 * ```
 *
...
/**
 * Returns an `IterableIterator` of the keys within the `Trie` that have `prefix` as prefix
 * (`prefix` included if it exists).
 *
...
/**
 * Returns an `IterableIterator` of the values within the `Trie` that have `prefix` as prefix
 * (`prefix` included if it exists).
 *
...
/**
 * Returns an `IterableIterator` of the entries within the `Trie` that have `prefix` as prefix
 * (`prefix` included if it exists).
 *
...
/**
 * Returns an `Array<[string, V]>` of the entries within the `Trie` whose keys start with `prefix`,
 * including the entry for `prefix` itself when it exists.
 *
...
/**
 * Returns the longest key/value in the `Trie` that is a prefix of that `key` if it exists, `None`
 * otherwise.
 *
 ...
 * **When to use**
 *
 * Use when the trie key is known to exist and a missing key should be treated as a programming
 * error.
 *
 * **Gotchas**
 *
 * `getUnsafe` throws if the key is not found. Use `get` instead to safely get a value from the
 * `Trie`.
 *
