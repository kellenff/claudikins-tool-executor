 ...
 *
 * Use this module for plain TypeScript values of type `A | undefined` when `undefined` is the only
 * absence marker. It is a small alternative to wrapping values in `Option` when your data already
 * uses `undefined` to mean "no value". The module includes helpers for mapping defined values,
 * matching both cases, throwing when a value is missing, adapting throwing functions, and building
 * reducers or combiners.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to apply a pure transformation to an `A | undefined` value while preserving `undefined` as
 * absence.
 *
 * @since 4.0.0
 * @category Mapping
...
/**
 * Pattern matches on an `A | undefined` value, running `onDefined` when the value is present or
 * evaluating `onUndefined` when the value is `undefined`.
 *
 * **When to use**
 *
 * Use when you need to turn an `A | undefined` into a non-optional result by handling both the
 * defined and undefined branches in one expression.
 *
 * @since 4.0.0
...
/**
 * Returns the defined value, or throws the value produced by `onUndefined` when the input is
 * `undefined`.
 *
 * **When to use**
 *
 * Use when you need fail-fast unwrapping of an `A | undefined` value and want to provide the thrown
 * error for the undefined case.
 *
 * **Details**
 *
 * Defined values are returned unchanged. When the input is `undefined`, `onUndefined` is called and
 ...
 * **When to use**
 *
 * Use when you need to unwrap a value that should already be defined and a generic missing-value
 * `Error` is acceptable.
 *
 * **Details**
 *
 * Defined inputs are returned unchanged. `undefined` throws `new Error("getOrThrow called on a
 * undefined")`.
 *
 * @since 4.0.0
 * @category Getters
...
/**
 * Converts a throwing function into one that returns successful results unchanged and returns
 * `undefined` when the function throws.
 *
 * **When to use**
 *
 * Use to adapt exception-throwing functions when `undefined` is the absence value you want to
 * return for failures.
 *
 * **Gotchas**
 *
 * Thrown values are discarded. If the wrapped function can successfully return `undefined`, that
 * success is indistinguishable from a thrown failure.
 *
 * @since 4.0.0
 * @category Converting
 */
...
/**
 * Creates a `Reducer` for `UndefinedOr<A>` that prioritizes the first non-`undefined` value and
 * combines values when both operands are present.
 *
 * **When to use**
 *
 * Use when you need to reduce values that may be `undefined`, keeping the first defined value as a
 * fallback and combining only when both operands are defined.
 *
 * **Details**
 *
 * Combining `undefined` with `undefined` returns `undefined`. Combining a defined value with
 * `undefined` keeps the defined value, so the first defined value wins when only one side is
 * present. When both values are defined, they are combined with `combiner.combine`. The reducer's
...
/**
 * Creates a `Combiner` for `A | undefined` that combines values only when both operands are
 * defined.
 *
 * **When to use**
 *
