 ...
 *
 * A scope is a lifetime boundary. Code can register cleanup effects on it, and closing the scope
 * runs those cleanups with the `Exit` value that ended the work. Most application code uses
 * higher-level APIs such as `Effect.scoped` and `Layer`, while this module is useful when code
 * needs to create, provide, fork, close, or inspect scopes directly.
 *
 * @since 2.0.0
 */
...
/**
 * A `Scope` represents a context where resources can be acquired and automatically cleaned up when
 * the scope is closed. Scopes can use either sequential or parallel finalization strategies.
 *
...
/**
 * A `Closeable` scope extends the base `Scope` interface with the ability to be closed, executing
 * all registered finalizers.
 *
...
/**
 * The `State` namespace contains the concrete states of a scope: `Empty` before any finalizers are
 * registered, `Open` with registered finalizers, and `Closed` with the exit value used to close the
 * scope.
 *
...
export declare namespace State {
   ...
   * **Details**
   *
   * Adding the first finalizer transitions the scope to `Open`; closing an empty scope transitions
  ...
  /**
   * Represents an open scope state where finalizers can be added and the scope is still accepting
   * new resources.
   *
  ...
  /**
   * Represents a closed scope state where finalizers have been executed and the scope is no longer
 ...
 * **When to use**
 *
 * Use to access the active lifetime when registering finalizers or sharing resources with the
 * surrounding scope.
 *
 ...
 */
export const make: (finalizerStrategy?: "sequential" | "parallel") => Effect<Closeable> =
  effect.scopeMake;

/**
 * Creates a new `Scope` synchronously without wrapping it in an `Effect`. This is useful when you
 * need a scope immediately but should be used with caution as it doesn't provide the same safety
 * guarantees as the `Effect`-wrapped version.
 *
 * **When to use**
 *
 ...
 * @category Constructors
 */
export const makeUnsafe: (finalizerStrategy?: "sequential" | "parallel") => Closeable =
  effect.scopeMakeUnsafe;
 ...
 * **Details**
 *
 * If the scope is open, the finalizer runs when the scope closes and receives the scope's exit
 * value. If the scope is already closed, the finalizer runs immediately with the stored exit
 * value.
 *
 ...
 *     Console.log(
 *       `Cleaning up resource. Exit: ${Exit.isSuccess(exit) ? "Success" : "Failure"}`,
 *     ),
 *   );
 *
 *   // Use the resource
 ...
 * **Details**
 *
 * If the scope is open, the finalizer runs when the scope closes, regardless of whether the scope
 * closes successfully or with an error. If the scope is already closed, the finalizer runs
 * immediately.
 *
 ...
 * @category Combinators
 */
export const addFinalizer: (scope: Scope, finalizer: Effect<unknown>) => Effect<void> =
 ...
 * **Details**
 *
 * Closing the parent closes the child with the same exit value, and closing the child detaches it
 * from the parent. The optional finalizer strategy configures the child scope and defaults to
 * `"sequential"` when omitted.
 *
 ...
 * **When to use**
 *
 * Use when a child scope must be created synchronously and the caller controls both parent and
 * child scope lifetimes.
 *
 * **Details**
 *
 * Closing the parent closes the child with the same exit value, and closing the child detaches it
 ...
 * **When to use**
 *
 * Use when implementing lower-level scope machinery that must transition a scope to `Closed`
 * immediately and can run the returned finalizer effect when one is produced.
 *
 * **Details**
 *
 * Returns an effect that runs registered finalizers, or `undefined` when the scope was already
 * closed or no finalizers need to run.
 *
 * **Gotchas**
 *
...
/**
 * Runs an effect with the provided closeable scope in its context and closes that scope when the
 * effect exits.
 *
 * **When to use**
 *
 * Use when you already have a `Closeable` scope and want to run an effect that requires `Scope`
 * while automatically closing that scope when the effect exits.
 *
 * **Details**
 *
 * The scope is closed with the same exit value as the effect, so registered finalizers can observe
 * whether the effect succeeded, failed, or was interrupted.
 *
 * @since 2.0.0
 * @category Combinators
