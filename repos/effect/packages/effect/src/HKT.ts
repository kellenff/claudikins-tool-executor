 ...
 *
 * TypeScript cannot directly abstract over shapes such as `Option<A>`, `ReadonlyArray<A>`, or
 * `Effect<A, E, R>`. This module represents those shapes with `TypeLambda` and applies concrete
 * type arguments with `Kind`. It is mostly useful when defining generic helpers or type classes
 * that should work across several data types.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * This symbol links a type class shape with its compile-time type lambda. It is intended for
 * type-class definitions and has no runtime behavior.
 *
 ...
 *
 * const value: HKT.Kind<
 *   NonNullable<LinkedTypeLambda>,
 *   never,
 *   never,
 *   never,
 *   string
 ...
 * **Details**
 *
 * A `TypeClass` defines operations that can be performed on any type constructor that matches the
 * given `TypeLambda`. This enables writing generic code that works across different container types
 * like Array, Option, Effect, etc.
 *
 ...
 * **Details**
 *
 * A `TypeLambda` encodes the "shape" of a type constructor, specifying how many type parameters it
 * takes and their variance (contravariant, covariant, or invariant). The four parameters are `In`
 * for contravariant input, `Out2` for covariant output often used for errors, `Out1` for covariant
 * output often used for context or environment, and `Target` for the invariant main type.
 *
 ...
 * **Details**
 *
 * This type-level function takes a `TypeLambda` and four type parameters, then "applies" them to
 * get the actual type. It handles variance correctly, ensuring contravariant parameters are used as
 * inputs and covariant parameters as outputs. This is the core mechanism that allows HKT to
 * transform abstract type constructors into concrete types by applying arguments.
 *
