 ...
 *
 * A record is an object whose keys are strings or symbols. This module includes helpers for
 * construction, lookup, updates, mapping, filtering, folding, set-like combination, and typed
 * conversions between records and iterable entries. Helpers that change values return new records
...
/**
 * Represents a readonly record with keys of type `K` and values of type `A`. This is the
 * foundational type for immutable key-value mappings in Effect.
 *
...
/**
 * Namespace containing utility types for working with readonly records. These types help with
 * type-level operations on record keys and values.
 *
...
export declare namespace ReadonlyRecord {
  ...
  /**
   * Represents a type that converts literal string keys to generic string type and symbol keys to
   * generic symbol type. This is useful for maintaining type safety while allowing flexible key
   * types in record operations.
   *
  ...
  /**
   * Represents the intersection of two key types, handling both literal and non-literal string
   * keys. This type is used in record operations that need to compute overlapping keys.
   *
...
/**
 * Type lambda for readonly records, used in higher-kinded type operations. This enables records to
 * work with generic type constructors and functors.
 *
...
/**
 * Takes an iterable and a projection function and returns a record. The projection function maps
 * each value of the iterable to a tuple of a key and a value, which is then added to the resulting
 * record.
 *
...
/**
 * Creates a new record from an iterable, utilizing the provided function to determine the key for
 * each element.
 *
 ...
 * **Details**
 *
 * If there are conflicting keys when using `fromEntries`, the last occurrence of the key/value pair
 * will overwrite the previous ones. So the resulting record will only have the value of the last
 ...
 * @category Converting
 */
export const toEntries: <K extends string, A>(self: ReadonlyRecord<K, A>) => Array<[K, A]> =
...
/**
 * Applies a function to the element at the specified key safely, creating a new record, or return
 * `Option.none()` if the key doesn't exist.
 *
...
/**
 * Retrieves the value of the property with the given `key` from a record safely and returns an
 * `Option` of a tuple with the value and the record with the removed property. If the key is not
 * present, returns `Option.none()`.
 *
...
/**
 * Maps entries of a `ReadonlyRecord` using the provided function, allowing modification of both
...
/**
 * Transforms a record by applying the function `f` to each key and value in the original record. If
 * the function succeeds, the key-value pair is included in the output record.
 *
...
/**
 * Returns a new record containing only the `Some` values from a record of `Option` values,
...
/**
 * Returns a new record containing only the `Err` values from a record of `Result` values,
 * preserving the original keys.
 *
...
/**
 * Returns a new record containing only the `Ok` values from a record of `Result` values, preserving
 * the original keys.
 *
...
/**
 * Applies a function to each record entry and partitions the returned `Result` values into two
 * records.
 *
 * **Details**
 *
 * Failure values are collected in the left record, and success values are collected in the right
 * record, preserving the original keys.
 *
...
/**
 * Partitions a record of `Result` values into two separate records, one with the `Err` values and
 * one with the `Ok` values.
 *
 ...
 * import * as assert from "node:assert";
 *
 * assert.deepStrictEqual(Record.separate({ a: Result.fail("e"), b: Result.succeed(1) }), [
 *   { a: "e" },
 *   { b: 1 },
 * ]);
 * ```
 ...
 * @category Getters
 */
export const values = <K extends string, A>(self: ReadonlyRecord<K, A>): Array<A> =>
  collect(self, (_, a) => a);
...
/**
 * Checks whether all the keys and values in one record are also found in another record. Uses the
 * provided equivalence function to compare values.
 *
 ...
 * **Details**
 *
 * Returns `true` when every key and value in `self` is also present in `that`. Values are compared
 * with Effect equality via `Equal.asEquivalence()`.
 *
...
/**
 * Merges two records, preserving entries that exist in either of the records. For keys that exist
 * in both records, the provided combine function is used to merge the values.
 *
...
/**
 * Merges two records, retaining only the entries that exist in both records. For intersecting keys,
 * the provided combine function is used to merge the values.
 *
...
/**
 * Merges two records, preserving only the entries that are unique to each record. Keys that exist
 * in both records are excluded from the result.
 *
...
/**
 * Create an `Equivalence` for records using the provided `Equivalence` for values. Two records are
 * considered equivalent if they have the same keys and their corresponding values are equivalent.
 *
...
/**
 * Creates a `Reducer` for combining `Record`s using union, with values for keys that exist in both
 * records combined using the provided `Combiner`.
 *
 * **When to use**
 *
 * Use to build a reusable reducer for accumulating many records into one union-shaped record,
 * preserving keys from every input and combining overlapping values with the supplied combiner.
 *
 * **Details**
 *
 * The returned reducer uses `Record.union` for combine and an empty record as `initialValue`, so
 * the default `combineAll` folds from `{}` and accumulates keys from each input record.
 *
 * @since 4.0.0
 * @category Combining
...
/**
 * Creates a `Reducer` whose `combine` operation intersects two records and combines values for keys
 * present in both records.
 *
 * **When to use**
 *
 * Use to build a `Reducer` that combines records by retaining only keys shared by both inputs and
 * combining matching values with a `Combiner`.
 *
 * **Gotchas**
 *
 * The reducer's `initialValue` is an empty record. Because intersection with an empty record is
 * empty, the default `combineAll` folds from `{}` and therefore produces `{}` for ordinary
 * non-empty inputs.
 *
 * @since 4.0.0
 * @category Combining
...
/**
 * Returns the first entry that satisfies the specified predicate, or `None` if no such entry
 * exists.
 *
