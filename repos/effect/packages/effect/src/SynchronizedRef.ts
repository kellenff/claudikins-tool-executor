 ...
 *
 * A `SynchronizedRef<A>` behaves like a `Ref<A>` for reading and simple state storage, but update
 * and modify operations are serialized so each change sees a consistent current value. This is
 * especially useful when the next value is computed by an effect, because the effectful transition
 * is still protected from concurrent updates. This module includes constructors, reads, writes,
...
/**
 * A mutable reference whose update and modify operations are serialized with an internal semaphore,
 * including effectful transformations.
 *
 * **When to use**
 *
 * Use when shared state may be updated by multiple fibers and each update, including effectful
 * state transitions, must observe one current value and run one at a time.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use to create a `SynchronizedRef` inside an Effect program when later updates may run effects and
 * must be serialized.
 *
 * **Details**
 *
 * The returned effect constructs a fresh `SynchronizedRef` by delegating to `makeUnsafe` when the
 * effect is evaluated.
 *
 * @since 2.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use when you need immediate synchronous access to a `SynchronizedRef` value in low-level code
 * that can safely read outside an `Effect`.
 *
 * @since 4.0.0
 * @category Getters
 ...
 * **When to use**
 *
 * Use to read the current value of a `SynchronizedRef` inside an `Effect` program without changing
 * it.
 *
 * @since 2.0.0
 * @category Getters
...
/**
 * Updates the current value atomically with a function and returns the previous value, serialized
 * by the ref's semaphore.
 *
 * **When to use**
 *
...
/**
 * Runs an effectful update atomically while holding the ref's semaphore, sets the new value if the
 * effect succeeds, and returns the previous value.
 *
 * **When to use**
 *
 * Use when you need an effectful `SynchronizedRef` state transition to return the previous stored
 * value.
 *
 * @since 2.0.0
 * @category Mutations
...
/**
 * Applies a partial update atomically and returns the previous value. If the function returns
 * `Option.some`, the ref is updated; if it returns `Option.none`, the ref is left unchanged.
 *
 * **When to use**
 *
...
/**
 * Runs an effectful partial update atomically while holding the ref's semaphore and returns the
 * previous value. `Option.some` updates the ref; `Option.none` leaves it unchanged.
 *
 * **When to use**
 *
...
/**
 * Computes a return value and a new ref value atomically, stores the new value, and returns the
 * computed result.
 *
 * **When to use**
 *
 * Use to derive a separate result and the next stored `SynchronizedRef` value from the same current
 * value in one serialized pure update.
 *
 * @since 2.0.0
 * @category Mutations
...
/**
 * Runs an effectful modification atomically while holding the ref's semaphore, stores the new value
 * if the effect succeeds, and returns the computed result.
 *
 * **When to use**
 *
 * Use to effectfully compute both a separate return value and the next stored `SynchronizedRef`
...
/**
 * Computes a return value and an optional new ref value atomically. `Option.some` updates the ref;
 * `Option.none` leaves it unchanged.
 *
 * **When to use**
 *
...
/**
 * Runs an effectful modification atomically while holding the ref's semaphore. The effect computes
 * a return value and an optional new ref value; `Option.some` updates the ref and `Option.none`
 * leaves it unchanged.
 *
 * **When to use**
 *
 * Use to effectfully compute a return value while optionally updating the stored `SynchronizedRef`
 * value.
 *
 * @since 2.0.0
 * @category Mutations
 ...
 * **When to use**
 *
 * Use to replace the current value of a `SynchronizedRef` with a known value while keeping the
 * write serialized with other synchronized updates.
 *
 * @since 2.0.0
 * @category Mutations
...
/**
 * Runs an effectful update while holding the ref's semaphore and stores the new value if the effect
 * succeeds.
 *
 * **When to use**
 *
 * Use to run an effectful state transition on a `SynchronizedRef` when storing the new value is the
...
/**
 * Runs an effectful update while holding the ref's semaphore, stores the new value if the effect
 * succeeds, and returns that new value.
 *
 * **When to use**
 *
...
/**
 * Applies a partial update to the current value. `Option.some` stores the new value; `Option.none`
...
/**
 * Runs an effectful partial update while holding the ref's semaphore. `Option.some` stores the new
...
/**
 * Applies a partial update and returns the resulting current value. `Option.some` stores and
 * returns the new value; `Option.none` returns the unchanged value.
 *
 * **When to use**
 *
...
/**
 * Runs an effectful partial update while holding the ref's semaphore and returns the resulting
 * current value. `Option.some` stores and returns the new value; `Option.none` returns the
 * unchanged value.
 *
 * **When to use**
 *
 * Use to run an effectful conditional `SynchronizedRef` update and return the resulting current
 * value.
 *
 * @since 2.0.0
 * @category Mutations
