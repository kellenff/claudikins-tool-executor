 ...
 * idle time, or removed by capacity limits or explicit invalidation. It is meant for resource
 * lifecycles such as clients, sessions, and connections, not as a general mutable cache.
 *
 * @since 3.5.0
 */
...
/**
 * An `RcMap` is a reference-counted map data structure that manages the lifecycle of resources
 * indexed by keys. Resources are lazily acquired and automatically released when no longer in use.
 *
 * **When to use**
 *
 * Use to share scoped resources by key while automatically releasing them after their last active
 * reference is gone.
 *
...
/**
 * Represents the internal state of an RcMap, which can be either Open (active) or Closed (shutdown
 * and no longer accepting operations).
 *
 * **When to use**
 *
 * Use when typing code that inspects an `RcMap`'s `state` field and narrows between open and closed
 * lifecycle states.
 *
 * @since 4.0.0
 * @category Models
...
export declare namespace State {
  /**
   * Represents the open/active state of an RcMap, containing the actual resource map that stores
   * entries.
   *
   * **When to use**
   *
  ...
  /**
   * Represents the closed state of an RcMap, indicating that the map has been shut down and will no
   * longer accept new operations.
   *
   * **When to use**
   *
  ...
  /**
   * Represents an individual entry in the RcMap, containing the resource's metadata including
   * reference count, expiration time, and lifecycle management.
   *
   * **When to use**
   *
   * Use when inspecting the stored resource, reference count, and idle lifecycle metadata for a
   * single key.
   *
   * @since 4.0.0
   * @category Models
...
/**
 * Creates an `RcMap` that can contain multiple reference counted resources that can be indexed by a
 * key. The resources are lazily acquired on the first call to `get` and released when the last
 * reference is released.
 *
 * **When to use**
 *
 * Use to create a scoped reference-counted map for resources that should be acquired once per key
 * and shared while in use.
 *
 * **Details**
 *
 ...
 * - `idleTimeToLive`: When the reference count reaches zero, the resource will be released after this
 *   duration.
 *
...
/**
 * Gets the resource for a key, acquiring it with the map's lookup function when the key is not
 * already cached.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The resource's reference count is incremented for the current `Scope`, and a release finalizer is
 * added to that scope. When the current scope closes, the reference is released; the resource is
 * closed when the last reference is released, subject to the map's idle time-to-live setting.
 *
 ...
 *   const map = yield* RcMap.make({
 *     lookup: (key: string) =>
 *       Effect.acquireRelease(Effect.succeed(`Resource: ${key}`), () =>
...
/**
 * Invalidates and removes a specific key from the RcMap. If the resource is not currently in use
 * (reference count is 0), it will be immediately released.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use to check whether a key is already present in an `RcMap` without running the lookup function
 * or acquiring a missing resource.
 *
 * **Details**
 *
...
/**
 * Extends the idle time for a resource in the RcMap. If the RcMap has an `idleTimeToLive`
 * configured, calling `touch` will reset the expiration timer for the specified key.
 *
 * **When to use**
 *
