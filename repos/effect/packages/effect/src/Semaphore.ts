 ...
 *
 * A `Semaphore` owns a number of permits. Work can run only after acquiring the permits it needs,
 * and those permits are returned when the work finishes. This module includes constructors,
 * automatic wrappers that acquire and release permits around an effect, manual permit operations, a
 * non-waiting variant for work that should only run immediately, and resizing support for an
 * existing semaphore.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Effects can acquire permits, wait until enough permits are available, release permits, or run
 * with permits that are automatically released when the effect exits.
 *
...
export interface Semaphore {
  ...
  /**
   * Runs an effect with the given number of permits and releases the permits when the effect
   * completes.
   *
   * **When to use**
   *
   ...
   * **Details**
   *
   * This function acquires the specified number of permits before executing the provided effect.
   * Once the effect finishes, the permits are released. If insufficient permits are available, the
   * function will wait until they are released by other tasks.
   */
  withPermits(
    this: Semaphore,
    permits: number,
   ...
   * **Details**
   *
   * This function attempts to acquire the specified number of permits. If they are available, it
   * runs the effect and releases the permits after the effect completes. If permits are not
   * available, the effect does not execute, and the result is `Option.none`.
   */
  ...
  /**
   * Acquires the specified number of permits and returns the resulting available permits,
   * suspending the task if they are not yet available. Concurrent pending `take` calls are
   ...
   * **When to use**
   *
   * Use to manually return permits acquired by a lower-level coordination protocol.
   */
 ...
 * **When to use**
 *
 * Use to construct a semaphore synchronously when an immediate value is required outside an Effect
 * workflow.
 *
 ...
 * const program = Effect.all([task(1), task(2), task(3), task(4), task(5)], {
 ...
 * **When to use**
 *
 * Use to create a semaphore inside Effect code for bounding concurrency with automatic or manual
 * permit management.
 *
 ...
 * **When to use**
 *
 * Use to change the concurrency limit of an existing semaphore while keeping current acquisitions
 * in place.
 *
 * **Details**
 *
 * Existing acquisitions remain taken after resizing. If the new total is less than the currently
 * taken permit count, new acquisitions wait until enough permits are released.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use to run an effect while holding a specified number of semaphore permits for the duration of
 * that effect.
 *
 * **Details**
 *
 * The effect waits until enough permits are available. Acquired permits are released when the
 * wrapped effect exits.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **When to use**
 *
 * Use to guard an effect with exactly one semaphore permit while automatically releasing that
 ...
 * **Details**
 *
 * When the permits are unavailable, the effect is not run and the result is `Option.none`. When
 * permits are available, the effect is run, its result is wrapped in `Option.some`, and the
 * acquired permits are released when the effect exits.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **When to use**
 *
 * Use when you need manual permit acquisition for a lower-level protocol with explicit acquisition
 * and release control.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to return permits acquired with `take` in a lower-level permit protocol with
 * explicit release control.
 *
 * **Details**
 *
 * Running the effect releases the requested permits, wakes waiting acquirers when permits become
 * available, and returns the current available permit count.
 *
 * **Gotchas**
 *
 * Manual `take` / `release` usage must keep permit counts balanced. Prefer `withPermit` or
 * `withPermits` when the acquisition can be scoped to one effect.
 *
 * @since 4.0.0
 * @category Combinators
 * @see {@link take} for manually acquiring permits
 ...
 * **When to use**
 *
 * Use to return every currently taken permit to a semaphore at once, typically during cleanup of
 * manual `take` / `release` protocols.
 *
 * @since 4.0.0
 * @category Combinators
