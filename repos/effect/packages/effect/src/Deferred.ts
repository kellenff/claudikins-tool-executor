/**
 * One-time coordination cells for Effect programs. A `Deferred<A, E>` starts empty, can be
 * completed exactly once with a success, failure, defect, or interruption, and lets any number of
 * fibers wait for that result. Awaiting a `Deferred` suspends the fiber instead of blocking an
 * operating-system thread, and every waiter observes the same completion.
 *
 * @since 2.0.0
 */
...
/**
 * A `Deferred` represents an asynchronous variable that can be set exactly once, with the ability
 * for an arbitrary number of fibers to suspend (by calling `Deferred.await`) and automatically
 * resume when the variable is set.
 *
 * **When to use**
 *
...
export declare namespace Deferred {
   ...
   * **When to use**
   *
   * Use to carry the value and error type parameters for `Deferred` in Effect's type machinery.
   *
   * **Details**
   *
   * This interface is part of the public type structure and is not intended to be constructed
   * directly.
   *
   * @since 2.0.0
   * @category Models
...
export {
  /**
   * Retrieves the value of the `Deferred`, suspending the fiber running the workflow until the
   * result is available.
   *
   * **When to use**
   *
   * Use to wait for a `Deferred` to be completed and resume with its success, failure, defect, or
   * interruption.
   *
   * **Details**
   *
 ...
 * **When to use**
 *
 * Use when completing a `Deferred` should run an effect once and share its result with all
 * awaiters.
 *
 * **Details**
 *
 * The returned effect succeeds with `true` when this call completed the `Deferred`, or `false` if
 ...
 * **When to use**
 *
 * Use to store an already environment-free effect as the completion without running it during
 * completion.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * The supplied effect is not memoized by `completeWith`; each awaiter may run the stored effect
 * independently.
 *
...
/**
 * Completes the `Deferred` with the specified `Exit` value, which will be propagated to all fibers
 * waiting on the value of the `Deferred`.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` fail with that error only if this call completes it. The
 * returned effect succeeds with `true` when this call completed the `Deferred`, or `false` if it
...
export const fail: {
  ...
  2,
  <A, E>(self: Deferred<A, E>, error: E): Effect<boolean> => done(self, core.exitFail(error)),
);

/**
 * Computes an error when the returned effect is run, then attempts to complete the `Deferred` with
 * that error.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` fail with the computed error only if this call completes it. The
 * returned effect succeeds with `true` when this call completed the `Deferred`, or `false` if it
 * was already completed.
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` observe that cause only if this call completes it. The returned
 * effect succeeds with `true` when this call completed the `Deferred`, or `false` if it was already
 * completed.
 *
...
/**
 * Computes a `Cause` when the returned effect is run, then attempts to complete the `Deferred` with
 * that cause.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` observe the computed cause only if this call completes it. The
 * returned effect succeeds with `true` when this call completed the `Deferred`, or `false` if it
 * was already completed.
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` die with that defect only if this call completes it. The
 * returned effect succeeds with `true` when this call completed the `Deferred`, or `false` if it
 * was already completed.
 *
...
/**
 * Computes a defect when the returned effect is run, then attempts to complete the `Deferred` with
 * that defect.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` die with the computed defect only if this call completes it. The
 * returned effect succeeds with `true` when this call completed the `Deferred`, or `false` if it
 * was already completed.
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` are interrupted with the current fiber id only if this call
 * completes it. The returned effect succeeds with `true` when this call completed the `Deferred`,
 * or `false` if it was already completed.
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` are interrupted with that fiber id only if this call completes
 * it. The returned effect succeeds with `true` when this call completed the `Deferred`, or `false`
...
/**
 * Returns `true` if this `Deferred` has already been completed with a value or an error, `false`
 * otherwise.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use to check `Deferred` completion synchronously in code that cannot return an `Effect`, such as
...
/**
 * Returns the current completion effect as an `Option`. This returns `Option.some(effect)` when the
 * `Deferred` is completed, `Option.none()` otherwise.
 *
 * **When to use**
 *
 * Use to inspect whether a `Deferred` is already completed and retrieve its stored completion
 * effect when available.
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` receive the value only if this call completes it. The returned
 * effect succeeds with `true` when this call completed the `Deferred`, or `false` if it was already
 * completed.
 *
...
export const succeed: {
  ...
  2,
  <A, E>(self: Deferred<A, E>, value: A): Effect<boolean> => done(self, core.exitSucceed(value)),
);

/**
 * Computes a value when the returned effect is run, then attempts to complete the `Deferred` with
 * that value.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Fibers waiting on the `Deferred` receive the computed value only if this call completes it. The
 * returned effect succeeds with `true` when this call completed the `Deferred`, or `false` if it
 * was already completed.
 *
 ...
 * **When to use**
 *
 * Use to complete a `Deferred` synchronously in low-level code that already has the completion
 * effect.
 *
 * **Details**
 *
 * This mutates the `Deferred` directly and should be reserved for low-level code; prefer the
 * effectful completion APIs when possible. Returns `true` if this call completed the `Deferred`, or
 ...
 * **When to use**
 *
 * Use to pipe an effect result into a `Deferred` while preserving success, failure, defects, and
 * interruption.
 *
 * **Details**
 *
 * If the effect succeeds, fails, dies, or is interrupted, that result is used as the attempted
 * completion. The returned effect cannot fail; it succeeds with `true` if it completed the
 * `Deferred`, or `false` if the `Deferred` was already completed.
 *
