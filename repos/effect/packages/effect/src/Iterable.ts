 ...
 *
 * Iterables include arrays, strings, generators, sets, and custom lazy sequences. The helpers in
 * this module let code transform, search, group, and fold iterable values while preserving the
 * input as an iterable instead of forcing an array first.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * The function is called with each index starting from `0`. If no length is specified, the iterable
 * is infinite. This is useful for generating sequences, patterns, or any indexed data.
 *
 ...
 * **Details**
 *
 * When `end` is provided and `start <= end`, both endpoints are included. When `end` is omitted,
 * the iterable is unbounded. When `start > end`, the iterable contains only `start`.
 *
...
export const repeat: {
  ...
  2,
  <A>(self: Iterable<A>, n: number): Iterable<A> => flatten(makeBy(() => self, { length: n })),
);
 ...
 * **When to use**
 *
 * Use to cycle a reusable iterable without an upper bound when a downstream consumer controls how
 * many values are taken.
 *
 * **Gotchas**
 *
 * The returned iterable is lazy and should usually be bounded with `take` or another terminating
 * consumer before materializing it.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * import * as assert from "node:assert";
 *
 * assert.deepStrictEqual(Array.from(Iterable.prependAll([1, 2], ["a", "b"])), [
 *   "a",
 *   "b",
 *   1,
 *   2,
 ...
 * **When to use**
 *
 * Use to lazily concatenate two iterables while preserving order, yielding all elements from `self`
 * before `that`.
 *
 * **Details**
 *
...
/**
 * Reduces an `Iterable` from the left, keeping all intermediate results instead of only the final
 * result.
 *
 ...
 * **When to use**
 *
 * Use when the `Iterable` is known to be non-empty and direct access to the first element is
 * preferred over handling `Option.none`.
 *
 * **Gotchas**
 *
...
/**
 * Takes the longest initial `Iterable` prefix for which all elements satisfy the specified
 * predicate.
 *
...
/**
 * Returns the first element that satisfies the specified predicate, or `None` if no such element
 * exists.
 *
...
/**
 * Applies a function to pairs of elements at the same index in two `Iterable`s, collecting the
 * results. If one input `Iterable` is short, excess elements of the longer `Iterable` are
 * discarded.
 *
...
/**
 * Returns a function that checks if an `Iterable` contains a given value using a provided
...
/**
 * Splits an `Iterable` into length-`n` pieces. The last piece will be shorter if `n` does not
 * evenly divide the length of the `Iterable`.
 *
...
/**
 * Groups equal, consecutive elements of an `Iterable` into `NonEmptyArray`s using the provided
 * `isEquivalent` function.
 *
 ...
 * **Details**
 *
 * Each property in the returned record contains a non-empty array of elements that produced that
 * key. Unlike `group`, matching elements do not need to be consecutive.
 *
 ...
 * **When to use**
 *
 * Use when you need an empty iterable as a typed "no data" value or a base case for iterable
 * operations.
 *
 ...
 * **When to use**
 *
 * Use to wrap a single value in an iterable context so it can be combined with other iterable
 * operations.
 *
 ...
 * **Details**
 *
 * This is one of the most fundamental operations for working with iterables. It applies a
 * transformation function to each element, creating a new iterable with the transformed values. The
 * operation is lazy, so elements are only transformed when the iterable is consumed.
 *
...
/**
 * Applies a function to each element in an Iterable and returns a new Iterable containing the
 * concatenated mapped elements.
 *
...
/**
 * Transforms elements of an iterable using a function that returns a `Result`, keeping only
 * successful values.
 *
 * **Details**
 *
 * This combines mapping and filtering in a single operation. The function is applied to each
 * element, and only elements that result in `Result.succeed` are included in the result.
 *
...
/**
 * Returns a lazy iterable containing the failure values from an iterable of `Result`s, skipping
 * successful results.
 *
...
/**
 * Returns a lazy iterable containing the success values from an iterable of `Result`s, skipping
 * failed results.
 *
 ...
 * **Details**
 *
 * This function creates a new iterable containing only the elements for which the predicate
 * function returns true. Like map, this operation is lazy and elements are only tested when the
...
/**
 * Transforms elements using a function that may return null or undefined, filtering out the
 * null/undefined results.
 *
 * **When to use**
 *
 * Use when working with APIs or functions that return nullable values, providing a clean way to
 * filter out null or undefined while transforming.
 *
 ...
 * **Details**
 *
 * This is useful for creating iterables from a generating function that maintains state. The
 * function should return `Option.some([value, nextState])` to continue or `Option.none()` to stop.
 *
 ...
 *   n <= 1000 ? Option.some([n, n * 2]) : Option.none(),
 * );
...
/**
 * Reduces an iterable to a single value by applying a function to each element and accumulating the
 * result.
 *
 * **Details**
 *
 * This function applies a reducing function against an accumulator and each element of the iterable
 ...
 * @category Filtering
 */
export const dedupeAdjacent: <A>(self: Iterable<A>) => Iterable<A> = dedupeAdjacentWith(
  Equal.asEquivalence(),
);
