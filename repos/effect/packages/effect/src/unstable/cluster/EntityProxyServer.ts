 ...
 *
 * Proxy handlers read the target `entityId`, call the entity client, and forward the payload to the
 * matching entity RPC method. This module provides handlers for HTTP API groups created by
 * `EntityProxy.toHttpApiGroup` and RPC handler services for RPC groups created by
 * `EntityProxy.toRpcGroup`. Both normal requests and discard requests are forwarded to the
 ...
 * **Details**
 *
 * Each generated endpoint reads the `entityId` path parameter and forwards the request payload to
 * the corresponding entity client method, including discard endpoints.
 *
 * @since 4.0.0
 * @category Layers
 */
        ...
        handlers = handlers
          .handle(
            ...
            ({ params, payload }: { params: { entityId: string }; payload: any }) =>
              (client(params.entityId) as any as Record<string, (p: any) => Effect.Effect<any>>)
 ...
 * **Details**
 *
 * The handlers forward each prefixed proxy RPC to the target entity client using the `entityId`
 * embedded in the proxy payload.
 *
 * @since 4.0.0
 * @category Layers
 */
