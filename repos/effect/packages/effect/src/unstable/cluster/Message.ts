 ...
 *
 * Messages carry entity requests and control envelopes between callers, durable storage,
 * transports, and runner handlers. This module includes incoming and outgoing variants for encoded
 * stored requests, decoded local requests, acknowledgements, and interrupts. It also provides
 * helpers for local delivery and for encoding or decoding request payloads with matching RPC
 * schemas.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * An incoming message is either a persisted request with an encoded payload or an incoming control
 * envelope.
 *
 * @since 4.0.0
 * @category Incoming
 ...
 * **Details**
 *
 * Request messages keep their decoded payload and response callback, while control envelopes are
 * wrapped as incoming envelopes.
 *
 * @since 4.0.0
 * @category Incoming
 */
 ...
 * **Details**
 *
 * It includes dynamic annotations, the last sent reply, and a callback for replying with decoded
 * replies.
 *
 * @since 4.0.0
 * @category Incoming
 ...
 * **Details**
 *
 * It carries the service context used for serialization, the last received reply, the reply
 * callback, dynamic annotations, and an optional encoded request cache.
 *
 * @since 4.0.0
 * @category Outgoing
 */
 ...
 * **Details**
 *
 * Control envelopes pass through unchanged. Requests are encoded with their RPC payload schema,
 * reusing the cached encoded request when available.
 *
 * @since 4.0.0
...
export const serialize = <Rpc extends Rpc.Any>(
  ...
  return Effect.suspend(() =>
    message.encodedCache ? Effect.succeed(message.encodedCache) : serializeRequest(message),
  );
};
...
/**
 * Encodes the payload of an `OutgoingRequest` with the request's RPC payload schema and service
 * context.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * Control envelopes pass through directly. Request envelopes require the original `OutgoingRequest`
 * so the payload can be decoded with the correct RPC schema and context.
 *
 * @since 4.0.0
 * @category Serialization
 */
