 ...
 *
 * `RunnerStorage` records which runners are registered, whether they are healthy, which machine id
 * a runner receives, and which shard locks are held by each runner. This module includes the typed
 * storage service, a string-encoded backend interface, an adapter from encoded storage to the typed
...
/**
 * String-encoded runner storage interface used by adapters that persist runner addresses, runners,
 * machine ids, and shard ids outside the in-memory model.
 *
 * @since 4.0.0
 * @category Encoded
 */
export interface Encoded {
  ...
  /** Acquire the lock on the given shards, returning the shards that were successfully locked. */
  readonly acquire: (
    address: string,
    shardIds: NonEmptyArray<string>,
  ) => Effect.Effect<Array<string>, PersistenceError>;

  /** Refresh the lock on the given shards, returning the shards that were successfully locked. */
  readonly refresh: (
    address: string,
...
/**
 * Adapts an encoded runner storage implementation into `RunnerStorage`, converting runner
 * addresses, runners, machine ids, and shard ids between typed values and their string or numeric
 * storage forms.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **Details**
 *
 * Registered runners are treated as healthy and shard acquisition is kept only in process memory.
 *
 * @since 4.0.0
