 ...
 *
 * The SQL-backed `RunnerStorage` records runners, health flags, machine ids, and shard locks so
 * multiple processes can coordinate which runner owns each shard. This module creates the required
 * runner and lock tables, supports an optional table prefix, uses advisory locks for PostgreSQL and
 * MySQL when enabled, and provides constructors and layers for the storage service.
 *
 * @since 4.0.0
 */
...
/**
 * Creates a SQL-backed `RunnerStorage` implementation for registered runners and shard locks, using
 * the configured table prefix and advisory locks where supported and enabled.
 *
 * **When to use**
 *
 * Use to create a SQL-backed `RunnerStorage` value directly when building custom service or layer
 * composition around the storage implementation.
 *
 * **Details**
 *
 * When `prefix` is omitted, `make` uses the `cluster` prefix, creating `cluster_runners` and
 * `cluster_locks`. PostgreSQL and MySQL use advisory locks unless
 * `ShardingConfig.shardLockDisableAdvisory` is enabled; other dialects use rows in the locks
 * table.
 *
 * **Gotchas**
 *
 * Changing `prefix` changes both generated table names, so runners using different prefixes do not
 * share registrations or shard locks.
 *
 * @since 4.0.0
...
export const make = Effect.fnUntraced(function* (options: {
  ...
  const lockConn =
    acquireLockConn && (yield* ResourceRef.from(yield* Effect.scope, acquireLockConn));
...
`.unprepared.pipe(Effect.andThen(acquiredLocks(address, shardIds)));
  ...
  const mysqlLocks = (shardIds: ReadonlyArray<string>) =>
    shardIds
      .map((shardId) => `GET_LOCK('${lockNames.get(shardId)!}', 0) AS "${shardId}"`)
      .join(", ");
  ...
  return RunnerStorage.makeEncoded({
    ...
    acquire: (address, shardIds) =>
      acquireLock(address, shardIds).pipe(PersistenceError.refail, withTracerDisabled),
