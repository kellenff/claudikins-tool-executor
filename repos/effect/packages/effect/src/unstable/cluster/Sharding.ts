 ...
 *
 * `Sharding` decides which shard owns an entity id, tracks which shards belong to the local runner,
 * and sends cluster messages to local handlers or remote runners. It also registers entities and
 * singletons, creates clients for entity requests, polls stored messages, and tracks shutdown
 * state. The main layer connects these responsibilities to runner communication, storage, health
 * checks, configuration, and local resources.
 *
 * @since 4.0.0
 */
...
/**
 * Service that registers entities and singletons, routes messages to owned shards, generates
 * runner-local snowflake ids, and polls storage for persisted work.
 *
 * **When to use**
 *
 * Use to access or provide cluster routing, shard ownership, entity registration, singleton
 * registration, and persisted-work polling.
 *
 * @since 4.0.0
 * @category Services
 */
export class Sharding extends Context.Service<
  Sharding,
  {
    /** Returns a stream of events that occur when the runner registers entities or singletons. */
    readonly getRegistrationEvents: Stream.Stream<ShardingRegistrationEvent>;

    /** Returns the `ShardId` of the shard to which the entity at the specified `address` is assigned. */
    ...
    /** Constructs a `RpcClient` which can be used to send messages to the specified `Entity`. */
...
const make = Effect.gen(function* () {
  ...
  if (initialRunnerAddress) {
    ...
    yield* Effect.gen(function* () {
      ...
      while (true) {
        ...
        yield* storage
          ...
          .pipe(Effect.ignore, Effect.timeoutOption(config.shardLockRefreshInterval));
    ...
    yield* Effect.suspend(() =>
      runnerStorage.refresh(selfAddress, [...acquiredShards, ...releasingShards]),
    ).pipe(
    ...
    > {
      ...
      if (message._tag === "IncomingRequest" || message._tag === "IncomingEnvelope") {
        if (!isLocal) {
          ...
          return Effect.fail(
            new AlreadyProcessingMessage({ address, envelopeId: message.envelope.requestId }),
          );
  ...
  const makeClient = <Type extends string, Rpcs extends Rpc.Any>(
    ...
    (entityId: string) => RpcClient.RpcClient.From<Rpcs, MailboxFull | AlreadyProcessingMessage>
  ...
  const registerSingleton: Sharding["Service"]["registerSingleton"] = Effect.fnUntraced(function* (
    name,
    run,
    options,
  ) {
...
/**
 * Layer that constructs the `Sharding` service from sharding configuration, runner communication,
 * message storage, runner storage, runner health, the snowflake generator, and the entity reaper.
 *
 * **When to use**
 *
 * Use when you need to assemble a cluster sharding runtime from explicit sharding configuration,
 * runner communication, message storage, runner storage, and runner health layers.
 *
 * **Details**
 *
 * The layer provides the `Sharding` service and installs its own snowflake generator and entity
 * reaper. Callers still provide `ShardingConfig`, `Runners`, `MessageStorage`, `RunnerStorage`, and
 * `RunnerHealth`.
 *
 * **Gotchas**
 *
 * Persisted messages require a non-no-op `MessageStorage`; if this layer is provided with
 * `MessageStorage.layerNoop`, persisted sends defect.
 *
 * @since 4.0.0
 * @category Layers
