 ...
 *
 * An entity gives a stable name and RPC protocol to a group of values that are addressed by id. The
 * cluster uses that information to choose a shard and route each request to the runner responsible
 * for that id. This module includes constructors for entity definitions, helpers for creating
 * sharded clients, layer builders for registering handlers, and services that expose the current
 * entity address while a request is being handled.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * An entity defines how ids map to shard groups, exposes a sharded client, and can be registered as
 * a layer using RPC handlers or a mailbox queue.
 *
 * @since 4.0.0
 * @category Models
 */
export interface Entity<in out Type extends string, in out Rpcs extends Rpc.Any>
  ...
  /**
   * A RpcGroup definition for messages which represents the messaging protocol that the entity is
 ...
 * **Details**
 *
 * Each handler receives the entity request envelope for that RPC and returns the RPC result or a
 * supported RPC wrapper.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a new `Entity` of the specified `type` which will accept messages that adhere to the
 * provided `RpcGroup`.
 *
 * @since 4.0.0
...
export const fromRpcGroup = <const Type extends string, Rpcs extends Rpc.Any>(
  ...
  type: Type,
  /** The schema definition for messages that the entity is capable of processing. */
 ...
 * **When to use**
 *
 * Use to define a cluster entity from individual `Rpc` definitions, giving the cluster runtime a
 * typed protocol for handlers and per-entity clients.
 *
 * **Details**
 *
 * The `type` argument is stored as the entity `EntityType`, and the RPC array is grouped into the
 * entity's `protocol`.
 *
 * **Gotchas**
 *
 * RPC tags should be unique within the array. If multiple definitions use the same tag, the
 * resulting protocol keeps the later definition for that tag.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to read the current entity identity and shard address from entity handlers and keep-alive
 * logic.
 *
 * @since 4.0.0
 * @category Context
 ...
 * **When to use**
 *
 * Use when you use it to complete an entity request by succeeding, failing, failing with a cause,
 * or supplying an explicit `Exit`.
 *
 * @since 4.0.0
 * @category Replier
 */
...
export declare namespace Replier {
   ...
   * **Details**
   *
   * For streaming RPCs this may be either a stream of success chunks or a dequeue of success
   * chunks. For non-streaming RPCs it is the RPC success value.
   *
   * @since 4.0.0
   * @category Replier
   */
 ...
 * **Details**
 *
 * It includes the underlying request envelope plus the last stream reply chunk that was sent,
 * allowing handlers to resume chunk sequencing after a restart.
 *
 * @since 4.0.0
 * @category Request
 */
 ...
 * **Details**
 *
 * The returned function creates a no-serialization RPC client for each entity ID, using a test
 * sharding service instead of the cluster transport.
 *
 * @since 4.0.0
 * @category Testing
 */
 ...
 * **Details**
 *
 * When enabled it sends the internal keep-alive RPC for the current address; when disabled it
 * releases the keep-alive latch if one is present.
 *
 * @since 4.0.0
 * @category Keep alive
 */
 ...
 * **Details**
 *
 * The RPC is marked as persisted and uninterruptible so the keep-alive signal survives normal
 * entity restarts.
 *
 * @since 4.0.0
 * @category Keep alive
 ...
 * **Details**
 *
 * `keepAlive` closes the latch when keep-alive is active and opens it again when the resource no
 * longer needs to keep the entity alive.
 *
 * @since 4.0.0
 * @category Keep alive
 */
