 ...
 *
 * `ShardingConfig` describes the runner address, shard group membership, shard counts and weights,
 * lock timing, entity mailbox and lifecycle limits, polling intervals, health checks, and local
 * serialization simulation. This module includes the service, default values, programmatic and
 * environment-based layers, a `Config` description for loading values, and helpers for normalizing
...
export class ShardingConfig extends Context.Service<
  ...
  {
    /**
     * The address for the current runner that other runners can use to communicate with it.
     *
     * If `None`, the runner is not part of the cluster and will be in a client-only mode.
     */
    ...
    /**
     * A number that determines how many shards this runner will be assigned relative to other
     * runners.
     *
     * Defaults to `1`.
     *
     * A value of `2` means that this runner should be assigned twice as many shards as a runner with
     * a weight of `1`.
     */
    ...
    /**
     * The maximum duration of inactivity (i.e. without receiving a message) after which an entity
     * will be interrupted.
     */
    readonly entityMaxIdleTime: Duration.Input;
    /**
     * If an entity does not register itself within this time after a message is sent to it, the
    ...
    /** The interval at which to poll for new runners and refresh shard assignments. */
...
/**
 * Default values for `ShardingConfig`, including the default local runner address, shard group,
 * shard count, mailbox settings, polling intervals, and remote serialization simulation.
 *
 * @since 4.0.0
 * @category Defaults
 */
 ...
 * **When to use**
 *
 * Use when you need to wire a cluster runner with explicit `ShardingConfig` values, especially in
 * tests, local development, or code paths where configuration should be provided programmatically
 * instead of loaded from environment variables.
 *
 * **Details**
 *
 * The merge is shallow: omitted fields use `defaults`, and provided fields replace the
 * corresponding default value.
 *
 * **Gotchas**
 *
 * This layer only merges and provides configuration; it does not check that cluster-wide settings
 * are consistent across runners. Keep values such as `shardsPerGroup` and `availableShardGroups`
 * aligned for runners that should share shard assignments.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Describes how to load `ShardingConfig` values, applying the same defaults used by the in-memory
 * `defaults` object.
 *
 * @since 4.0.0
...
/**
 * Effect that loads `ShardingConfig` from environment variables using the constant-case config
 * provider.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Layer that loads `ShardingConfig` from environment variables and, when options are provided,
 * overlays those options on top of the loaded values.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Normalizes the provided `ShardingConfig` to calculate the `available` and `assigned` shard
 * groups.
 *
 * @since 4.0.0
