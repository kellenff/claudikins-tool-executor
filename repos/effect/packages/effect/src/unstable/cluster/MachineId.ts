/**
 * Branded integer identifiers for cluster runners. A `MachineId` marks the machine component used
 * by cluster services, especially snowflake id generation, while keeping the value distinct from an
 * ordinary `number` in TypeScript APIs.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to turn a trusted numeric machine id into the branded type when implementing runner storage
 * adapters or configuring snowflake generation.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * `make` does not validate integer input or enforce the snowflake machine-id range. Snowflake ids
 * encode the machine component modulo 1024.
 *
 * @since 4.0.0
 * @category Constructors
