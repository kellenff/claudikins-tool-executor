 ...
 *
 * Runner protocol handlers receive ping, notification, request, stream, and envelope messages from
 * other runners. They forward those messages into `Sharding` and coordinate persisted replies
 * through `MessageStorage`. This module includes the handler layer, a transport-independent RPC
 * server layer, a full server layer that also provides runner clients, and a client-only layer for
 * applications that do not serve runner RPCs.
 *
 * @since 4.0.0
 */
...
/**
 * Layer that handles runner protocol RPCs by forwarding requests to `Sharding` and
 * `MessageStorage`.
 *
 * @since 4.0.0
 * @category Layers
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      Effect: ({ persisted, request }) => {
        ...
        > = Option.none();
        let resume = (
          reply: Effect.Effect<Reply.Encoded, ClusterError.EntityNotAssignedToRunner>,
        ) => {
        ...
        if (persisted) {
          return Effect.callback<Reply.Encoded, ClusterError.EntityNotAssignedToRunner>(
            (resume_) => {
              resume = resume_;
...
/**
 * Creates the runner RPC server layer, which receives messages from other runners, forwards them to
 * the `Sharding` layer, and responds to `Ping` requests.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use to embed a cluster client inside another Effect application without registering with the
 * ShardManager or receiving shard assignments.
 *
 * @since 4.0.0
 * @category Layers
 */
