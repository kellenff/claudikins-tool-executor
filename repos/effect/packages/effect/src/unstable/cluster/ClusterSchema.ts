/**
 * The `ClusterSchema` module collects the annotations that add cluster behavior to RPC protocols
 * and entity definitions. These annotations describe how requests are persisted, handled in
 * transactions, interrupted, traced, and routed to shard groups without changing the request or
 * response schema.
 *
 * @since 4.0.0
 */
...
/**
 * Annotation that marks whether request handling should be wrapped in the configured message
 * storage transaction.
 *
 * **When to use**
 *
 * Use when you need server-side request handling or storage work wrapped in the storage
 * transaction.
 *
 * **Details**
 *
 * The default value is `false`. When `true`, entity handling wraps server writes with the
 * configured storage transaction.
 *
 * **Gotchas**
 *
 * This annotation has transactional behavior only when the configured `MessageStorage` implements
 * it.
 *
 * @since 4.0.0
 * @category Annotations
 ...
 * **Details**
 *
 * Use `true` for both client and server handling, `"client"` for client-side handling only,
 * `"server"` for server-side handling only, or `false` to allow interruption.
 *
 * @since 4.0.0
 * @category Annotations
 */
...
/**
 * Returns whether the `Uninterruptible` annotation applies to server-side request handling for the
 * provided context.
 *
 * **Details**
 *
...
/**
 * Returns whether the `Uninterruptible` annotation applies to client-side request handling for the
 * provided context.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Returns `true` when `Uninterruptible` is `true` or `"client"`, and `false` for `"server"` or the
 * default `false`.
 *
 * @since 4.0.0
 * @category Annotations
