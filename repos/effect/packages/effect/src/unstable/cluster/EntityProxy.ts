 ...
 *
 * The generated APIs let callers use ordinary RPC clients or HTTP routes while the cluster runtime
 * still locates and delivers messages to the entity instance identified by `entityId`. Each
 * generated operation keeps the original payload and success schema, adds cluster client errors,
 * and creates a discard variant for fire-and-forget delivery.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * For each entity RPC, this creates a prefixed request RPC and a discard RPC whose payload includes
 * `entityId`, and whose errors include cluster client errors.
 *
 * @since 4.0.0
 * @category Converting
 */
 ...
 * // Counter entity
 * export class MyApi extends HttpApi.make("api").add(
 ...
 * **Details**
 *
 * For each entity RPC, this creates a POST endpoint at `/<tag>/:entityId` and a discard endpoint at
 * `/<tag>/:entityId/discard`, including cluster client errors.
 *
 * @since 4.0.0
 * @category Converting
 */
