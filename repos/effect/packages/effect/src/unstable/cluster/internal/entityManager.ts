...
>(
  ...
  const entities: ResourceMap<EntityAddress, EntityState, EntityNotAssignedToRunner> =
...
const makeMessageDecode = <Type extends string, Rpcs extends Rpc.Any>(
  ...
  const decodeRequest = Effect.fnUntracedEager(function* (
    ...
    const payload = yield* Schema.decodeEffect(Schema.toCodecJson(rpc.payloadSchema))(
