/**
 * The `ShardId` module models the address of a shard inside an Effect Cluster shard group. A shard
 * id is made from a string `group` and numeric `id`, and the module gives that pair stable
 * equality, hashing, primary-key behavior, schema support, and conversion to and from the
 * `group:id` string form used by routing and storage boundaries.
 *
 * @since 4.0.0
 */
...
/**
 * Identifier for a shard within a shard group, with equality, hashing, and primary key behavior
 * based on the `group:id` string form.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use to create a `ShardId` when the shard group and numeric id are already known, such as after a
 * routing decision or after decoding stored shard-id parts.
 *
 * **Details**
 *
 * Repeated calls with the same `group` and `id` return the same cached `ShardId` instance. The
 * returned value stores those fields, compares by `group` and `id`, formats as `group:id`, and uses
 * that string form for hashing and primary keys.
 *
 * **Gotchas**
 *
 * `make` does not compute a shard from an entity id or check whether the shard belongs to the
 * current sharding configuration. Pass the shard group and numeric id produced by the routing or
 * storage layer.
 *
 * @since 4.0.0
