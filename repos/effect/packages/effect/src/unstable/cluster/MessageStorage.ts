 ...
 *
 * `MessageStorage` is the boundary between cluster runner logic and the storage system that keeps
 * mailbox state recoverable. It saves requests, control envelopes, and replies; finds unprocessed
 * messages for assigned shards; tracks duplicate requests; and manages reply handlers waiting for
 * responses. This module also includes the encoded storage-driver contract and no-op or in-memory
 * implementations for local use and tests.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It stores outgoing requests, control envelopes, and replies; reads unprocessed messages; manages
 * reply handlers; and provides transaction wrapping for storage operations.
 *
 * @since 4.0.0
 * @category Context
 */
 ...
 * **Details**
 *
 * A duplicate result carries the original request ID and the last reply already received for the
 * duplicated request.
 *
 * @since 4.0.0
 * @category SaveResult
...
export declare namespace SaveResult {
   ...
   * **Details**
   *
   * Duplicate results contain an encoded last received reply instead of a decoded reply.
   *
   * @since 4.0.0
   * @category SaveResult
   */
   ...
   * **Details**
   *
   * It carries the original request ID and the latest decoded reply, when one is available.
   *
   * @since 4.0.0
   * @category SaveResult
   */
   ...
   * **Details**
   *
   * It carries the original request ID and the latest encoded reply, when one is available.
   *
   * @since 4.0.0
   * @category SaveResult
   */
 ...
 * **Details**
 *
 * Implementations persist encoded messages, track primary keys and delayed delivery, read
 * unprocessed messages, and provide transaction wrapping.
 *
 * @since 4.0.0
 * @category Encoded
 */
 ...
 * **Details**
 *
 * The fields distinguish existing shards from newly assigned shards and carry the driver-specific
 * pagination cursor.
 *
 * @since 4.0.0
 * @category Encoded
 ...
 * **Details**
 *
 * The fields distinguish existing requests from new requests and carry the driver-specific
 * pagination cursor.
 *
 * @since 4.0.0
 * @category Encoded
 ...
 * **Details**
 *
 * The returned service can register waiting reply handlers, notify them when replies are saved, and
 * fail them when a request or shard is unregistered.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The adapter handles envelope and reply encoding and decoding, primary-key generation, delayed
 * delivery checks, duplicate decoding, and malformed-message defect replies.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * It stores the encoded envelope, last acknowledged chunk, accumulated replies, and optional
 * delivery time.
 *
 * @since 4.0.0
 * @category Memory
 ...
 * **Details**
 *
 * It provides a `MessageStorage` service, the encoded driver implementation, and maps used to track
 * requests, primary keys, unprocessed envelopes, reply IDs, and the journal.
 *
 * @since 4.0.0
 * @category Memory
 */
