 ...
 *
 * `EntityResource` is useful for long-lived resources tied to an entity address, such as external
 * processes, network clients, Kubernetes Pods, or other handles that should survive routine shard
 * movement. This module includes the resource wrapper, a close scope that survives normal entity
 * restarts, a generic resource constructor, and a Kubernetes Pod resource helper built on
 * `K8sHttpClient`.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * `get` acquires or reuses the resource in the caller's scope, while `close` invalidates it so its
 * close scope can be released.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when a cluster entity resource needs a scope that survives restarts and closes only through
 ...
 * **When to use**
 *
 * Use when a cluster entity should lazily share an acquired resource across messages and release it
 * only on idle timeout or explicit close.
 *
 * **Details**
 *
 * The resource will only be fully released when the idle time to live is reached, or when the
 * `close` effect is called.
 *
 * **Gotchas**
 *
 * By default, the `idleTimeToLive` is infinite, meaning the resource will only be released when
 * `close` is called.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The pod is created and waited on through `K8sHttpClient`, and is kept alive until the resource is
 * closed or its idle time to live expires.
 *
 * @since 4.0.0
 * @category Kubernetes
 */
