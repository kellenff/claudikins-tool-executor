 ...
 *
 * `Runners` sits between sharding decisions and runner execution. It can ping a runner, send
 * requests or control envelopes, notify a runner that persisted work is available, and record that
 * a runner address is unavailable. This module defines the runner communication service, its RPC
 * protocol, no-op and RPC-backed implementations, local persistence support, reply recovery, and
...
/**
 * Service for communicating with cluster runners, including pinging runners, sending and notifying
 * messages, coordinating persisted replies, and marking runners unavailable.
 *
 * @since 4.0.0
 * @category Context
 */
export class Runners extends Context.Service<
  Runners,
  {
     ...
     *
     * This ensures that the message hits storage before being sent to the local entity.
     */
    ...
    /**
     * Notify the current Runner that a message is available, then read replies from storage.
     *
     * This ensures that the message hits storage before being sent to the local entity.
     */
...
/**
 * Builds the `Runners` service from remote runner callbacks and adds local message persistence,
 * duplicate request handling, optional local serialization simulation, and polling for persisted
 * replies.
 *
 * **When to use**
 *
 * Use when you need a custom `Runners` service around remote `ping`, `send`, `notify`, and
 * `onRunnerUnavailable` callbacks, with standard local persistence and reply recovery behavior.
 *
 * **Details**
 *
 * `make` uses the supplied remote callbacks for runner communication and derives `sendLocal` and
 * `notifyLocal`. Local sends can optionally simulate remote serialization, persisted notifications
 * are saved through `MessageStorage`, duplicate requests are resumed from stored replies when
 * possible, and pending replies are polled according to `ShardingConfig.entityReplyPollInterval`.
 *
 * **Gotchas**
 *
 * `notify` and `notifyLocal` only support RPCs annotated as persisted; calling either path with a
...
/**
 * Creates a no-op `Runners` service that rejects sends with `EntityNotAssignedToRunner` and ignores
 * notifications, pings, and unavailable runner reports.
 *
 * @since 4.0.0
 * @category No-op
 */
...
const rpcErrors: Schema.Union<
  [typeof EntityNotAssignedToRunner, typeof MailboxFull, typeof AlreadyProcessingMessage]
> = Schema.Union([EntityNotAssignedToRunner, MailboxFull, AlreadyProcessingMessage]);

/**
 * RPC group used for runner-to-runner communication, including ping, notify, effect, stream, and
 * envelope messages.
 *
 * @since 4.0.0
 * @category Rpcs
...
/**
 * Builds a runner RPC client from the current `RpcClient.Protocol`, using the `Runners` span prefix
 * with tracing disabled.
 *
 * @since 4.0.0
 * @category Rpcs
 */
export const makeRpcClient: Effect.Effect<RpcClient, never, RpcClient_.Protocol | Scope> =
  RpcClient_.make(Rpcs, { spanPrefix: "Runners", disableTracing: true });

/**
 * Builds a `Runners` service backed by RPC clients, caching a client per runner address and
 * dispatching ping, notify, effect, stream, and envelope messages over the runner protocol.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Layer that provides an RPC-backed `Runners` service using `RpcClientProtocol`, message storage,
 * sharding configuration, and the default snowflake generator.
 *
 * @since 4.0.0
 * @category Layers
 */
