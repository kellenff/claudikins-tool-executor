/**
 * HTTP client support for talking to the Kubernetes API from code running inside a cluster. The
 * module builds a `K8sHttpClient` service on top of the shared HTTP client, points requests at the
 * in-cluster API endpoint, and uses the mounted service-account token when one is available.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It targets `https://kubernetes.default.svc/api`, adds the service-account bearer token when
 * available, requires successful HTTP statuses, and retries transient failures.
 *
 * @since 4.0.0
 * @category Layers
 */
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return (yield* HttpClient.HttpClient).pipe(
      ...
      token._tag === "Some"
        ? HttpClient.mapRequest(HttpClientRequest.bearerToken(token.value.trim()))
        : identity,
 ...
 * **Details**
 *
 * The request can be limited by namespace and label selector, and the result is a map keyed by pod
 * IP address.
 *
 * @since 4.0.0
