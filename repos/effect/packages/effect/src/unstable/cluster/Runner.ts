 ...
 *
 * A `Runner` combines the stable `RunnerAddress` used to contact a process, the shard groups that
 * process participates in, and the relative weight used when the sharding service distributes
 * shards across healthy runners.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Each runner has a unique network `address`, the shard `groups` it participates in, and a relative
 * `weight` used when assigning shards across runners.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Constructs a `Runner` from its network address, shard groups, and relative shard-assignment
 * weight.
 *
 * **When to use**
 *
 * Use to build runner metadata from an existing `RunnerAddress`, shard groups, and relative weight
 * when registering or exchanging a cluster runner.
 *
 * **Details**
 *
 * The `groups` array lists the shard groups the runner can host. During shard assignment, the
 * runner's address is added to each group's hash ring with `weight` as its relative weight.
 *
 * **Gotchas**
 *
 * This helper constructs the value without runtime schema validation, so only pass trusted
 * `RunnerAddress`, `groups`, and `weight` values.
 *
 * @since 4.0.0
