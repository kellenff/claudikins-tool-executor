 ...
 *
 * Runner nodes communicate through the `Runners.Rpcs` protocol. This module provides client
 * protocol layers for dialing runner addresses over HTTP or WebSocket, HTTP effects that serve
 * runner RPC handlers, route layers for installing runner endpoints into an `HttpRouter`, and
 * ready-made layers for HTTP or WebSocket runner communication.
 *
 * @since 4.0.0
 */
...
/**
 * Provides a runner RPC client protocol that connects to runner addresses over HTTP.
 *
 * **Details**
 *
 * The configured path is appended to each runner address, and `https` switches the generated URL
 * from `http` to `https`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Provides a runner RPC client protocol that connects to runner addresses over WebSocket.
 *
 * **Details**
 *
 * The configured path is appended to each runner address, and `https` switches the generated URL
 ...
 * **Details**
 *
 * The returned effect is produced from `RunnerServer.layerHandlers` and the cluster runner RPC
 * group.
 *
 * @since 4.0.0
 * @category Http app
...
/**
 * Layer that provides `Sharding` and `Runners` using the configured runner RPC client protocol and
 * storage services.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **Details**
 *
 * It serves runner routes at `/` and configures runner clients to communicate over HTTP.
 *
 * @since 4.0.0
 * @category Layers
 */
 ...
 * **Details**
 *
 * It serves runner routes at `/` and configures runner clients to communicate over WebSocket.
 *
 * @since 4.0.0
 * @category Layers
 */
 ...
 * **Details**
 *
 * It configures runner clients to communicate over WebSocket without serving runner WebSocket
 * routes.
 *
 * @since 4.0.0
 * @category Layers
