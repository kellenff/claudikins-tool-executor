 ...
 *
 * `EventLogEncryption` turns local journal entries into encrypted remote payloads and decrypts
 * encrypted changes received from a server. It also hashes byte data and creates event-log
 * identities, so remote replication can use storage or transport that should not see plaintext
 * event data.
 *
 * @since 4.0.0
 */
...
/**
 * Type of an encrypted remote entry, including its remote sequence number, initialization vector,
 * entry id, and encrypted entry bytes.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Service that provides identity generation, entry encryption and decryption, and SHA-256 hashing
 * for event-log replication.
 *
 * **When to use**
 *
...
/**
 * Creates an `EventLogEncryption` service backed by the Web Crypto `SubtleCrypto` APIs from the
 * supplied `Crypto` implementation.
 *
 * @since 4.0.0
 * @category Encryption
 */
  ...
  Effect.sync(() => {
    ...
    return EventLogEncryption.of({
      ...
      decrypt: Effect.fnUntraced(function* (identity, entries) {
        ...
        return decoded.map(
          (entry, index) => new RemoteEntry({ remoteSequence: entries[index].sequence, entry }),
        );
      }),
      sha256: (data) =>
