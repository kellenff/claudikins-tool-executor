 ...
 *
 * This module implements `EventJournal` on top of a `SqlClient`. It stores event entries as encoded
 * bytes and stores per-remote sequence metadata in separate tables, giving event-log programs a
 * durable journal that can be replayed after restart and synchronized with remote journals.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The constructor creates the entry and remote metadata tables when needed, persists local and
 * remote entries, and uses the configured `SqlClient`.
 *
 * @since 4.0.0
...
export const make = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const toEntries = (rows: ReadonlyArray<EntryRow>): ReadonlyArray<EventJournal.Entry> =>
      rows.map(toEntry);
    ...
    return EventJournal.EventJournal.of({
      entries: sql`SELECT * FROM ${entryTableSql} ORDER BY timestamp ASC`.pipe(
        ...
        Effect.mapError(
          (cause) => new EventJournal.EventJournalError({ cause, method: "entries" }),
        ),
      ),
      ...
      writeFromRemote: (options) =>
        writeFromRemote(options).pipe(
          ...
          Effect.catchIf(
            ...
            (cause) =>
              Effect.fail(new EventJournal.EventJournalError({ cause, method: "writeFromRemote" })),
          ),
        ),
      withRemoteUncommited: Effect.fnUntraced(
        ...
        withTracerDisabled,
        Effect.mapError(
          (cause) => new EventJournal.EventJournalError({ cause, method: "withRemoteUncommited" }),
        ),
      ),
      nextRemoteSequence: (remoteId) =>
        sql<{
          ...
          withTracerDisabled,
          Effect.mapError(
            (cause) => new EventJournal.EventJournalError({ cause, method: "nextRemoteSequence" }),
          ),
        ),
      ...
      destroy: Effect.gen(function* () {
        ...
        withTracerDisabled,
        Effect.mapError(
          (cause) => new EventJournal.EventJournalError({ cause, method: "destroy" }),
        ),
      ),
      withLock(_storeId) {
        return (effect) =>
          sql.withTransaction(effect).pipe(Effect.catchIf(SqlError.isSqlError, Effect.die));
      },
    });
  });
 ...
 * **When to use**
 *
 * Use when composing a Layer graph that should provide a persistent SQL-backed `EventJournal` from
 * an existing `SqlClient` service.
 *
 * **Details**
 *
 * The layer delegates to `make(options)`, so the same optional `entryTable` and `remotesTable`
 * settings are used and construction requires `SqlClient` and may fail with `SqlError`.
 *
 * **Gotchas**
 *
 * Layer construction performs the same minimal `CREATE TABLE IF NOT EXISTS` setup as `make`; manage
 * indexes and schema migrations outside this layer when your SQL schema needs more than the
 * built-in tables.
 *
 * @since 4.0.0
 * @category Layers
