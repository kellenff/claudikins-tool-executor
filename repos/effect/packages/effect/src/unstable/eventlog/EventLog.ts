 ...
 *
 * `EventLog` combines event groups, handlers, a journal, local identity, optional remote replicas,
 * and reactivity hooks. Writers send typed payloads through a client; the matching handler runs
 * first, and the journal entry is committed only after the handler succeeds. This module also
 * contains the layers and helpers needed to assemble that runtime.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * `write` encodes the event payload, runs the matching handler, commits the entry only when the
 * handler succeeds, and exposes access to the underlying journal entries and destroy operation.
 *
 * @since 4.0.0
 * @category Services
 */
...
/**
 * Service that collects event handlers, compaction handlers, remote replicas, and reactivity
 * invalidation keys.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Provides an in-memory `Registry` for event handlers, compactors, remote replicas, and reactivity
 * keys.
 *
 * @since 4.0.0
 * @category Layers
...
export const layerRegistry = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    let remoteHandler: (remote: EventLogRemote["Service"]) => Effect.Effect<void> = (_) =>
      Effect.void;
...
/**
 * Context service for an event-log identity containing a public key and redacted private key
 * material.
 *
 * **Details**
 *
 * The identity is used by remote replication for authentication and by the encryption service to
 ...
 * **Details**
 *
 * The `Events` type parameter tracks the event tags that still need handlers, and each call to
 * `handle` records a handler while accumulating any required services.
 *
 * @since 4.0.0
 * @category Handlers
 */
...
export declare namespace Handlers {
  ...
  /**
   * Runtime representation of one registered event handler, including its event metadata, captured
   * context, and handler function.
   *
   * @since 4.0.0
   * @category Handlers
   */
   ...
   * **Details**
   *
   * If any event tag remains unhandled, the type evaluates to an explanatory compile-time error
   * string.
   *
   * @since 4.0.0
   * @category Handlers
   ...
   */
  export type Error<A> =
    A extends Effect.Effect<Handlers<infer _R, infer _Events>, infer _EX, infer _RX> ? _EX : never;

  /**
   * Computes the services required by a `Handlers` value or by an effect that produces one,
   * including event schema services.
   *
   * @since 4.0.0
   * @category Handlers
   */
...
/**
 * Schema for an event-log identity with a string public key and redacted base64-encoded private key
 * bytes.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Encodes an event-log identity as a base64url string containing the public key and private key
 * bytes.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * The callback receives a `Handlers` builder; its return type is checked so every event in the
 * group is handled.
 *
 * @since 4.0.0
 * @category Handlers
...
> =>
  Layer.effectDiscard(
    Effect.gen(function* () {
      ...
      const context =
        yield* Effect.context<Exclude<Handlers.Services<Return>, Scope.Scope | Identity>>();
      const result = f(
        makeHandlers({
 ...
 * **Details**
 *
 * During remote replay, matching entries are decoded, grouped by primary key, and passed to the
 * compaction effect, which may write replacement entries.
 *
 * @since 4.0.0
 * @category Compaction
 */
 ...
 * **Details**
 *
 * The returned handler decodes the entry and conflicts with the registered event schema, runs the
 * matching handler with the supplied identity and store id, logs failures, and invalidates
 * configured reactivity keys.
 *
 * @since 4.0.0
 * @category Handlers
 */
export const makeReplayFromRemote = (options: {
  ...
  Effect.fnUntraced(
    function* ({
      ...
      const decodedConflicts: Array<{ entry: Entry; payload: unknown }> = new Array(
        conflicts.length,
      );
...
const make = Effect.gen(function* () {
  ...
  const runRemote = Effect.fnUntraced(
    function* (remote: EventLogRemote["Service"]) {
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.catchCause(Effect.logError),
        Effect.repeat(
          Schedule.min([Schedule.exponential(200, 1.5), Schedule.spaced({ seconds: 10 })]),
        ),
        Effect.annotateLogs({
          service: "EventLog",
  ...
  const writeHandler = Effect.fnUntraced(function* (
    ...
    const payload = yield* Schema.encodeUnknownEffect(handler.event.payloadMsgPack)(
      options.payload,
    ).pipe(Effect.orDie);
 ...
 * @category Layers
 */
export const layerEventLog: Layer.Layer<EventLog | Registry, never, EventJournal | Identity> =
 ...
 * **When to use**
 *
 * Use when you need one layer that installs the shared `EventLog` runtime for an `EventLogSchema`
 * and registers an event-group handler layer for typed writes.
 *
 * **Details**
 *
 * The supplied handler layer is provided with `layerEventLog`. The returned layer provides
 * `EventLog | Registry`, preserves the handler layer's error type, and still requires its remaining
 * services plus `EventJournal` and `Identity`.
 *
 * **Gotchas**
 *
 * The schema argument does not register handlers by itself. Handler registration comes from the
 * supplied layer, and writing an event without a registered handler dies with `Event handler not
 * found for: "<tag>"`.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **Details**
 *
 * The returned function delegates to the `EventLog` service and preserves each event's success and
 * error types.
 *
 * @since 4.0.0
 * @category Client
