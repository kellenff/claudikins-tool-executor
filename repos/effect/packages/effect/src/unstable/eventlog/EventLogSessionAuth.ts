 ...
 *
 * Remote peers use this challenge-response flow to prove that they control a session signing key
 * before sending session traffic. The signed payload includes the remote id, a short-lived
 * challenge, the event-log public key, and the signing public key in a stable byte format.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need the domain-separation string used to build canonical event-log session
 ...
 * **When to use**
 *
 * Use when implementing session-auth serialization or validation that must reject public keys with
 * a non-canonical raw byte length.
 *
 * @since 4.0.0
 * @category Constants
 */
 ...
 * **When to use**
 *
 * Use when implementing session-auth verification that must reject signatures with a non-canonical
 * byte length before cryptographic checking.
 *
 * @since 4.0.0
 * @category Constants
 */
...
/**
 * Error raised while encoding, decoding, signing, verifying, or generating session authentication
 * challenges.
 *
 * @since 4.0.0
 * @category Errors
 ...
 * **Details**
 *
 * The decoder validates the context field, UTF-8 fields, signing public key length, and rejects
 ...
 * **Details**
 *
 * The payload, signing public key, and signature lengths are validated before calling
 * `SubtleCrypto.verify`.
 *
 * @since 4.0.0
...
/**
 * Encodes a session authentication payload in canonical form and signs it with an Ed25519 private
 * key.
 *
 * @since 4.0.0
 * @category Signing
...
/**
 * Verifies an authentication request by requiring the `Ed25519` algorithm and checking the
 * signature over the canonical session authentication payload.
 *
 * @since 4.0.0
 * @category Verification
 */
