 ...
 *
 * This module provides the durable `Storage` implementation used by `EventLogServerEncrypted` when
 * entries should be stored without exposing plaintext event data to the database. It persists the
 * server remote id, session authentication bindings, and encrypted entry tables, assigns stable
 * sequence numbers, and streams changes. Clients remain responsible for encrypting writes and
 * decrypting reads.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It persists the server remote id, session authentication bindings, and encrypted entries in
 * dialect-specific tables, creating per-identity/store entry tables as needed.
 *
 * @since 4.0.0
...
/**
 * Provides SQL-backed encrypted server `Storage` and supplies the default Web Crypto
 * `EventLogEncryption` layer.
 *
 * @since 4.0.0
 * @category Layers
 */
