 ...
 *
 * Transport modules use these handlers to expose an event journal to remote replicas. The handlers
 * run the hello/authenticate challenge flow, attach the authenticated `EventLog.Identity` to later
 * requests, accept single or chunked writes, and stream changes back as single or chunked
 * messages.
 *
 * @since 4.0.0
 */
...
/**
 * Provides RPC authentication middleware that reads the authenticated `EventLog.Identity` from
 * client annotations.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * The layer manages hello challenges, verifies session authentication, reassembles chunked writes,
 * delegates write and change handling to the supplied callbacks, and frames large change payloads
 * into chunks.
 *
 * @since 4.0.0
 * @category Layers
