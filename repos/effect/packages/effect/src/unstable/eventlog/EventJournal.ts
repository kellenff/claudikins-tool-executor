 ...
 *
 * `EventJournal` records committed entries, exposes them for replay, publishes local changes, and
 * tracks the remote metadata needed to exchange entries with other journals. Higher-level event-log
 * schemas and handlers use this service to rebuild projections, sync offline clients, import remote
 * changes, and coordinate writes per store. This module also defines journal errors, entry and
 * remote identifiers, schemas, and in-memory or IndexedDB-backed journal layers.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The service writes local entries, imports entries from remote journals, exposes a stream of local
 * changes, and provides per-store locking.
 *
 * @since 4.0.0
 * @category Context
 */
export class EventJournal extends Context.Service<
  EventJournal,
  {
    ...
    /** Write an event to the journal, performing an effect before committing the event. */
 ...
 * **When to use**
 *
 * Use when generating a fresh event-log remote id internally and the UUID bytes are trusted to
 * satisfy the brand.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when generating an event-log entry id internally and the UUID v7 bytes are trusted to satisfy
 * the brand.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * An entry records its ID, event tag, primary key, and MessagePack-encoded payload, with helpers
 * for array MessagePack encoding and creation timestamps.
 *
 * @since 4.0.0
 * @category Entry
 */
 ...
 * **Gotchas**
 *
 * Entries, remote tracking state, and locks live only in the current process and are lost when the
 ...
 * **Details**
 *
 * The journal stores entries and remote replication metadata in the configured browser database,
 * publishes local changes, and requires `Scope` so the database connection can be closed when the
 * scope ends.
 *
 * @since 4.0.0
 * @category Indexed db
