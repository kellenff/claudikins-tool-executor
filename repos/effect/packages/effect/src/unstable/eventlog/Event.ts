 ...
 *
 * An `Event` is the durable contract shared by writers, handlers, journals, and replicas. It gives
 * an event a stable tag, derives the aggregate or entity primary key from the decoded payload, and
 * records the schemas used for the payload, handler result, and handler errors. The payload schema
 * is also used to derive the MessagePack encoding for journal entries and remote replication.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * An event definition contains its tag, primary-key function, payload schema, MessagePack payload
 * schema, success schema, and error schema.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * `ToService` derives this service from an `Event` so handler layers can expose which events they
 * implement.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * It preserves the runtime tag, primary-key function, payload schema, success schema, and error
 * schema without retaining the original type parameters.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * All schema services required to encode and decode the payload, success, and error schemas for an
 * event definition.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * If omitted, the payload and success schemas default to `Schema.Void`, the error schema defaults
 * to `Schema.Never`, and the MessagePack payload schema is derived from the payload schema.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The returned event keeps the same tag, primary key, payload, and success schema while replacing
 * the error schema with a union of the existing and new errors.
 *
 * @since 4.0.0
 * @category Constructors
 */
