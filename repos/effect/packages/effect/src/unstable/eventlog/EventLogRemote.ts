 ...
 *
 * `EventLogRemote` writes local entries to another journal, receives remote change streams from a
 * sequence number, and can wait until the current event-log identity has completed remote
 * authentication. The encrypted constructor is the default for browser, edge, or service replicas
 * crossing an untrusted network. The unencrypted constructor is intended for trusted transports or
 * tests.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to access or provide a remote event-log replica that can write local entries and stream
 * remote changes.
 *
 * **Details**
 *
 * It can write local entries to the remote, stream remote changes from a sequence number, and run
 * effects only after the supplied identity has authenticated.
 *
 * @since 4.0.0
 * @category Services
 */
 ...
 * **When to use**
 *
 * Use to provide the RPC client used by remote event-log replicas to authenticate, write entries,
 ...
 * **Details**
 *
 * The remote performs the hello/authentication handshake, retries after forbidden responses by
 * re-authenticating, chunks large writes, and registers itself with the `Registry` for the current
 * scope.
 *
 * @since 4.0.0
...
/**
 * Creates an `EventLogRemote` that encrypts outgoing entries and decrypts incoming changes with
 * `EventLogEncryption`.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Provides an encrypted `EventLogRemote` using the remote RPC client and the default Web Crypto
 * encryption layer.
 *
 * @since 4.0.0
 * @category Layers
