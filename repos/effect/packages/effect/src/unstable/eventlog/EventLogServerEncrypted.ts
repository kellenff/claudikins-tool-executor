 ...
 *
 * Encrypted `EventLogRemote` clients use this module when they need a remote synchronization
 * endpoint that never sees plaintext events. The server stores encrypted entries and replication
 * metadata keyed by client public key and store id, then streams encrypted changes back to clients
 * for local decryption. This module defines the RPC handlers, server layer, storage contract, and
 * in-memory storage layer for that encrypted server path.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Incoming encrypted write payloads are decoded and persisted through `Storage`; change streams
 * read encrypted entries from storage and encode them for the remote protocol.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Provides an encrypted event-log RPC server using `EventLogRemoteRpcs` and the encrypted server
 * RPC handlers.
 *
 * **When to use**
 *
 * Use when you need an encrypted event-log RPC server for encrypted `EventLogRemote` replication
 * over an existing `RpcServer.Protocol`.
 *
 * **Details**
 *
 * This layer installs `EventLogRemoteRpcs` on the provided RPC server protocol and wires those RPCs
 * to `layerRpcHandlers`. Encrypted entries, session authentication bindings, remote ids, and change
 * streams are delegated to `Storage`.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **Details**
 *
 * It provides the server remote id, stores session authentication bindings, persists encrypted
 * entries, and streams encrypted changes for a public key and store id.
 *
 * @since 4.0.0
 * @category Storage
 */
 ...
 * **Details**
 *
 * Data, session authentication bindings, and streams are process-local and are released with the
 * surrounding scope.
 *
 * @since 4.0.0
 * @category Storage
