 ...
 *
 * An `EventGroup` collects event definitions that belong together. Start with `empty`, add events
 * with their payload, success, and error schemas, then use the group to build clients with
 * `EventLog.schema` and server handlers with `EventLog.group`. The group only describes events; it
 * does not write or run them.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when build groups from `empty.add(...)`, then provide implementations for the events with
 * `EventLog.group`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when you need the starting `EventGroup` value before adding event definitions with
 * `.add(...)`.
 *
 * @since 4.0.0
