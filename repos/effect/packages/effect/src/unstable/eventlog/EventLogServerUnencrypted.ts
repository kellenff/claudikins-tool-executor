 ...
 *
 * This module accepts unencrypted event batches from remote clients, runs the registered event
 * handlers, stores journal entries, and streams backlog plus live changes through the shared
 * `EventLogServer` RPC protocol. It is intended for trusted deployments, local development, and
 * tests where event data does not need a server-side encryption layer. The module also provides the
 * services and layers needed to authorize requests, map stores, persist entries, and install the
 * plaintext server.
 *
 * @since 4.0.0
 */
...
/**
 * Service that writes plaintext event-log entries directly to unencrypted storage through
 ...
 * **Details**
 *
 * Incoming plaintext entries are authorized, mapped to a server store, checked for conflicts, run
 * through registered handlers, and persisted; change streams include compacted backlog entries when
 * compactors are registered.
 *
 * @since 4.0.0
 * @category Layers
 */
 ...
 * **When to use**
 *
 * Use to provide authorization checks for plaintext event-log writes, reads, and identity
 * authentication.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Service that resolves client-requested store ids to server store ids and checks whether a store
 * exists.
 *
 * **When to use**
 *
 * Use to map client-visible store identifiers to server storage identifiers before authorizing or
 * serving unencrypted event-log requests.
 *
 * @since 4.0.0
 * @category Services
 */
...
/**
 * Provides a `StoreMapping` that accepts only one configured store id and fails all other store ids
 * as not found.
 *
 * @since 4.0.0
 * @category Store
 ...
 * **Details**
 *
 * It provides the server remote id, stores session authentication bindings, allocates remote
 * sequence numbers, persists entries, streams changes, and exposes a transaction boundary.
 *
 * @since 4.0.0
 * @category Storage
 */
...
): ReadonlyArray<RemoteEntry> => journal.filter((entry) => entry.remoteSequence >= startSequence);
 ...
 * **Details**
 *
 * Contiguous entries handled by the same compactor may be replaced with compacted entries when the
 * replacement count can be mapped back to increasing remote sequence numbers; otherwise the
 * original entries are kept.
 *
 * @since 4.0.0
 * @category Compaction
 */
 ...
 * **Details**
 *
 * The implementation keeps per-store journals and session authentication bindings in memory,
 * publishes live changes, and serializes transactions with a semaphore.
 *
 * @since 4.0.0
 * @category Storage
 */
  ...
  function* () {
    ...
    return Storage.of({
      ...
      changes: Effect.fnUntraced(function* ({ storeId, startSequence, compactors }) {
        ...
        const replayedUpTo =
          backlog.length > 0 ? backlog[backlog.length - 1].remoteSequence : startSequence - 1;
...
/**
 * Creates the `EventLogServerUnencrypted` service from the configured storage and registered event
 * handlers.
 *
 * **When to use**
 *
 * Use when you need the unencrypted event-log server service from provided `Storage` and an
 * event-log `Registry`.
 *
 * **Details**
 *
 * The constructed service exposes `makeWrite`, which builds a typed server-side write function from
 * an `EventLogSchema`. Each write encodes the payload with the event schema, runs the registered
 * handler, and persists the generated entry inside `Storage.withTransaction`.
 *
 * **Gotchas**
 *
 * The write function dies if the requested event tag is not present in the schema passed to
 * `makeWrite`; it does not report that case as a typed failure.
 *
 * @since 4.0.0
        ...
        const entry = new EventJournal.Entry(
          {
            ...
            payload: yield* Schema.encodeUnknownEffect(schemaEvent.payloadMsgPack)(
              options.payload,
            ).pipe(
              Effect.mapError(
                (_) =>
...
/**
 * Provides `EventLogServerUnencrypted` and an event-log `Registry` using the configured unencrypted
 * server `Storage`.
 *
 * **When to use**
 *
 * Use to provide the unencrypted event-log server service together with the registry needed by
 * event handlers.
 *
 * @since 4.0.0
 * @category Layers
...
export const layerServer: Layer.Layer<
  ...
  never,
  Storage
> = Layer.effect(EventLogServerUnencrypted, make).pipe(Layer.provideMerge(EventLog.layerRegistry));

/**
 * Builds a full unencrypted event-log RPC server for the supplied schema and event-group handler
 * layer.
 *
 * **When to use**
 *
 * Use when you need the full unencrypted event-log RPC server layer with storage, authorization,
 * RPC protocol, and event-group handler dependencies supplied externally.
 *
 * **Details**
 *
 * The layer installs `EventLogRemoteRpcs`, wires `layerRpcHandlers`, registers the supplied
 * event-group handler layer, and provides `layerServer`, leaving only the required infrastructure
 * services in the environment.
 *
 * **Gotchas**
 *
 * Entries are persisted and streamed in plaintext. Protect the backing `Storage` with the
 * surrounding infrastructure, and use durable storage that preserves session authentication
 * bindings when the server must survive restarts.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Builds the unencrypted event-log server handlers without installing an `RpcServer.Protocol`
 * implementation.
 *
 * @since 4.0.0
 * @category Layers
...
const makeServerHandler = Effect.gen(function* () {
  ...
  return Effect.fnUntraced(function* (options: {
    ...
    const payloadEffect =
      "payload" in options ? Effect.succeed(options.payload) : decodePayload(options.entry.payload);
