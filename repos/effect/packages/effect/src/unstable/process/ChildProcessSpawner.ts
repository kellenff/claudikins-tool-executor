 ...
 *
 * `ChildProcessSpawner` is the service used by `ChildProcess` commands to start operating-system
 * processes. A spawner turns a command description into a handle that can write to stdin, read
 * stdout and stderr, wait for exit, kill the process, and manage whether the process keeps its
 * parent alive. Platform backends implement this service, while most application code uses the
 * higher level `ChildProcess` module.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * This value is returned by `ChildProcessHandle.unref` and can be run later to restore the default
 * behavior where the child process keeps the parent process alive.
 *
 * @since 4.0.0
 * @category Models
 */
...
export interface ChildProcessHandle {
  ...
  /** Waits for the child process to exit and returns the `ExitCode` of the command that was run. */
  readonly exitCode: Effect.Effect<ExitCode, PlatformError.PlatformError>;
  /** Returns `true` if the child process is still running, otherwise returns `false`. */
   ...
   * **Gotchas**
   *
   * Using this stream alongside `all` may cause interleaving of output and unexpected results.
   */
  ...
  /**
   * A stream which combines and interleaves all messages output by the child process `stdout` and
   * `stderr` streams.
   */
   ...
   * **Details**
   *
   * If a file descriptor is accessed that was not configured, returns a drain `Sink`.
   */
  readonly getInputFd: (
    fd: number,
   ...
   * **Details**
   *
   * If a file descriptor is accessed that was not configured, returns an empty `Stream`.
   */
   ...
   * **Details**
   *
   * Running this `Effect` removes this child process from the parent process's reference count, so
   * the parent process is allowed to exit without waiting for the child process to finish.
   *
   * The returned `Reref` effect adds the child process back into the parent process's reference
   * count when run, restoring the default behavior.
   *
   * **Gotchas**
   *
   * This is the only supported way to re-reference a child process after it has been unrefed.
   *
...
/**
 * Creates a `ChildProcessSpawner` service from a `spawn` function, deriving helpers for exit codes
 * and output collection from that implementation.
 *
 * @since 4.0.0
 * @category Models
 */
...
export class ChildProcessSpawner extends Context.Service<
  ChildProcessSpawner,
  {
    ...
    /**
     * Stream the output of a command as strings. Optionally include stderr output interleaved with
     * stdout.
     */
    streamString(
      command: Command,
    ...
    /**
     * Stream the output of a command as lines. Optionally include stderr output interleaved with
     * stdout.
     */
    streamLines(
      command: Command,
