 ...
 *
 * A `Command` stores the executable, arguments, environment, standard streams, working directory,
 * and other process options. Commands can also be piped together. A command is an `Effect`; running
 * it asks the `ChildProcessSpawner` service to start the process and returns a
 * `ChildProcessHandle`.
 *
 * @since 4.0.0
 */
...
/**
 * A command that can be built using `make`, combined using `pipeTo`, and executed using `exec` or
 * `spawn`.
 *
 * @since 4.0.0
 * @category Models
...
export interface KillOptions {
  ...
  /**
   * The duration of time to wait after the child process has been terminated before forcefully
   * killing the child process by sending it the `"SIGKILL"` signal. Defaults to `undefined`, which
   * means that no timeout will be enforced by default.
   */
...
export interface StdinConfig {
   ...
   * **Details**
   *
   * Can be a string indicating how the operating system should configure the pipe established
   * between the child process `stdin` and the parent process.
   *
   * Can also be a `Stream`, which will pipe all elements produced into the `stdin` of the child
   * process.
   *
   * Defaults to "pipe".
   */
  ...
  /**
   * Whether or not the child process `stdin` should be closed after the input stream is finished.
   * Defaults to `true`.
   */
...
export interface StdoutConfig {
   ...
   *
   * Can be a string indicating how the operating system should configure the pipe established
   * between the child process `stdout` and the parent process.
   *
   * A `Sink` can also be passed, which will receive all elements produced by the `stdout` of the
   * child process.
   *
   * Defaults to "pipe".
   */
...
export interface StderrConfig {
   ...
   *
   * Can be a string indicating how the operating system should configure the pipe established
   * between the child process `stderr` and the parent process.
   *
   * A `Sink` can also be passed, which will receive all elements produced by the `stderr` of the
   * child process.
   *
   * Defaults to "pipe".
   */
...
export type AdditionalFdConfig =
  | {
      /**
       * The direction of data flow for this file descriptor. - "input": Data flows from parent to
       * child (writable by parent) - "output": Data flows from child to parent (readable by parent)
       */
      readonly type: "input";
      /** For input file descriptors, an optional stream to pipe into the file descriptor.. */
  ...
  | {
      ...
      /** For output file descriptors, an optional sink which receives data from the file descriptor. */
      readonly sink?:
        | Sink.Sink<Uint8Array, Uint8Array, never, PlatformError.PlatformError>
        | undefined;
    };
...
export interface CommandOptions extends KillOptions {
   ...
   * **Details**
   *
   * If `extendEnv` is set to `true`, the value of `env` will be merged with the value of
   * `globalThis.process.env`, prioritizing the values in `env` when conflicts exist.
   */
  readonly env?: Record<string, string | undefined> | undefined;
  /**
   * If set to `true`, the child process uses both the values in `env` as well as the values in
   * `globalThis.process.env`, prioritizing the values in `env` when conflicts exist.
   *
   * **Details**
   *
  ...
  /**
   * If set to `true`, runs the command inside of a shell, defaulting to `/bin/sh` on UNIX systems
   * and `cmd.exe` on Windows.
   *
   * **Details**
   *
   * Can also be set to a string representing the absolute path to a shell to use on the system.
   *
   * **Gotchas**
   *
  ...
  /**
   * If set to `true`, the child process will run independently of the parent process.
   *
   * **Details**
   *
   * The specific behavior of this option depends upon the platform. For example, the NodeJS
   * documentation outlines the differences between Windows and non-Windows platforms.
   *
  ...
  /**
   * Additional file descriptors to expose to the child process beyond `stdin` / `stdout` /
   * `stderr`.
   *
   * **Details**
   *
   * Keys must be in the format `"fd3"`, `"fd4"`, etc. with a file descriptor index >= 3.
   *
   * The file descriptor index is determined by the numeric suffix (i.e. `fd3` has a file descriptor
   * index of 3).
   *
 ...
 * @category Guards
 */
export const isStandardCommand = (command: Command): command is StandardCommand =>
 ...
 *
 * Template literals are not parsed until execution time, allowing parsing errors to flow through
 ...
 * **Details**
 *
 * By default, pipes `stdout` from the source to `stdin` of the destination. Use the `options`
 * parameter to customize which streams are connected.
 *
...
/**
 * Adds environment variables to a command, merging them with any existing command environment and
...
const parseTemplate = (
  ...
  const { hasLeadingWhitespace, hasTrailingWhitespace, tokens } = splitByWhitespaces(
    template,
    rawTemplate,
  );
...
/**
 * Convert valid expressions defined in a template string command (i.e. using `${expression}` into
 * strings.
 */
...
/**
 * Number of characters in backslash escape sequences: \0 \xXX or \uXXXX \cX is allowed in RegExps
 * but not in strings Octal sequences are not allowed in strict mode
 */
const ESCAPE_LENGTH: Record<string, number> = { x: 3, u: 5 };

/**
 * Splits a template string by whitespace while also properly handling escape sequences.
 *
 ...
 * ```
 *
 * The problem is that when we evaluate the template string for both of the above commands, we will
 * end up with the same string "echo foo\n bar".
 *
 * What we really want is to include the escaped character in the arguments for the first command,
 * since it was written explicitly by the user.
 *
 * This is why also having access to the raw template string is useful - in a template string, there
 * are two representations of the same string:
 *
 ...
 *
 * If either set is empty or `isSeparated=false`, the last element of `prevTokens` and the first
 * element of `nextTokens` will be joined into a single token.
 */
const concatTokens = (
  prevTokens: ReadonlyArray<string>,
