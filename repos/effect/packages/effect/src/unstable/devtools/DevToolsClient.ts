 ...
 *
 * `DevToolsClient` speaks the devtools NDJSON protocol used by the unstable devtools integration.
 * It sends span starts, span events, span completions, ping messages, and metric snapshots through
 * a socket, then exposes tracer layers that forward telemetry while preserving the current tracer's
 * behavior.
 *
 * @since 4.0.0
 */
...
/**
 * Creates a devtools client over the current `Socket`, speaking the devtools NDJSON protocol,
 * sending periodic pings, and responding to metrics snapshot requests.
 *
 * **When to use**
 *
 * Use when you already have a `Socket` and need the low-level `DevToolsClient` service to exchange
 * devtools telemetry directly.
 *
 * **Details**
 *
 * The effect requires `Scope` because it starts background fibers for the socket stream and
 * heartbeat.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use to provide the low-level `DevToolsClient` service over an existing `Socket` for custom
 * devtools integrations that send telemetry through the client directly.
 *
 * **Details**
 *
 * It delegates to `make`, so it speaks the devtools NDJSON protocol over the provided `Socket`,
 * sends periodic pings, responds to metrics snapshot requests, and finalizes its background fibers
 * when the layer scope closes.
 *
 * **Gotchas**
 *
...
/**
 * Creates a tracer that delegates to the current tracer while sending span starts, span events, and
 * span ends to `DevToolsClient`.
 *
 * @since 4.0.0
 * @category Constructors
 */
