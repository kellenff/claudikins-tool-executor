 ...
 *
 * Use this module when an integration needs to accept devtools clients over a `SocketServer`,
 * decode newline-delimited JSON messages, and handle each connection with application-specific
 * logic. It does not interpret spans or metrics itself; it gives handlers a typed surface for the
 * telemetry described by `DevToolsSchema`.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It exposes a queue of non-ping requests received from the socket and a `send` function for
 * non-pong responses.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Each connection is decoded as NDJSON devtools protocol messages, `Ping` requests are answered
 * with `Pong`, and all other requests are delivered through the `Client` passed to the handler.
 *
 * @since 4.0.0
...
export const run: <_, E, R>(
  ...
  Effect.fnUntraced(function* <R, E, _>(handle: (client: Client) => Effect.Effect<_, E, R>) {
