/**
 * Marks encoded worker message fields that should move through `postMessage` as transfer-list
 * entries.
 *
 * Worker messages still pass through schema encoding and structured clone, but schemas wrapped with
 * `schema` can also report backing resources such as `ArrayBuffer`, `ImageData.data.buffer`, or
 * `MessagePort` to a `Collector`. Worker platforms then pass the collected values as the transfer
 * list for the same `postMessage` call, avoiding copies for large payloads and ports.
 *
 * @since 4.0.0
 */
...
/**
 * Service for collecting `Transferable` objects while encoding worker messages so they can be
 * passed to `postMessage` transfer lists.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a mutable `Collector` service directly, exposing unsafe synchronous methods for reading,
 * adding, and clearing collected transferables.
 *
 * @since 4.0.0
...
/**
 * Adds transferables to the current `Collector` when one is present in the context, and does
 * nothing otherwise.
 *
 * @since 4.0.0
 * @category Accessors
...
/**
 * Creates a schema getter that records transferables derived from a value in the current
 * `Collector` while passing the value through unchanged.
 *
 * @since 4.0.0
 * @category Getters
 */
...
/**
 * Schema wrapper whose encode path can record transferables with a `Collector` while preserving the
 * wrapped schema's decoded type.
 *
 * @since 4.0.0
 * @category Schemas
 */
...
/**
 * Wraps a schema so encoding records transferables selected from the encoded value, enabling worker
 * messages to populate a `postMessage` transfer list.
 *
 * @since 4.0.0
 * @category Schemas
 */
