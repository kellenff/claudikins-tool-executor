 ...
 *
 * This module defines the platform-neutral `Worker` client, the `WorkerPlatform` service that
 * creates workers by numeric id, and the `Spawner` service used to find platform-specific worker
 * instances. `makePlatform` wraps platform setup and listen hooks into a `WorkerPlatform`, buffers
 * outgoing messages until the worker is ready, runs incoming messages with Effect handlers, and
 * ties worker cleanup to scope lifetime.
 *
 * @since 4.0.0
 */
...
/**
 * Service that spawns effect `Worker` instances for numeric worker ids using the configured
 * `Spawner`.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Effect-based worker abstraction that can send input messages and run a long-lived handler for
 * output messages, failing with `WorkerError` or handler errors.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Wraps platform-specific send and run functions into a `Worker`, translating platform ready/data
 * messages and running the optional `onSpawn` effect when the worker reports readiness.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Internal worker platform protocol message: `[0]` signals readiness and `[1, payload]` carries
 * data.
 *
 * @since 4.0.0
 * @category Models
 ...
 * @category Services
 */
export const Spawner: Context.Service<Spawner, SpawnerFn<unknown>> = Context.Service(
 ...
 * @category Layers
 */
export const layerSpawner: <W = unknown>(spawner: SpawnerFn<W>) => Layer.Layer<Spawner> =
  Layer.succeed(Spawner);

/**
 * Creates a `WorkerPlatform` from platform-specific setup and listen hooks, buffering sent messages
 * until the worker is ready and scoping port cleanup to the worker run.
 *
 * @since 4.0.0
