 ...
 *
 * A `WorkerRunner` receives messages tagged by a numeric port id, sends replies back through the
 * same transport, and can expose disconnect notifications. The module also defines the small
 * platform message shape and the `WorkerRunnerPlatform` service that starts a platform-specific
 * runner.
 *
 * @since 4.0.0
 */
...
/**
 * Platform-neutral worker runner that receives inbound messages by port ID, sends outbound
 * messages, and optionally exposes disconnect notifications.
 *
 * @since 4.0.0
 * @category Models
 */
