 ...
 *
 * A `Primitive<A>` receives one string and returns an `Effect` that either produces an `A` or fails
 * with a parser message. `Argument` and `Flag` build on these primitives to add names, aliases,
 * defaults, prompts, configuration fallbacks, repetition, and help metadata. Primitive parsers
 * cover common scalar values, paths, files, structured config files, schema-decoded input, redacted
 ...
 * **Details**
 *
 * The wrapped value is hidden when formatted or inspected, while the original string remains
 * available through the `Redacted` API when explicitly needed.
 *
 ...
 * **Details**
 *
 * The parser is selected from `options.format` when provided, otherwise from the file extension.
 * Supported formats include INI, JSON, TOML, YAML, and YML.
 *
...
/**
 * Represents options which can be provided to methods that deal with parsing file content and
 * decoding the file content with a `Schema`.
 *
 * @since 4.0.0
 * @category Options
 */
 ...
 * **When to use**
 *
 * Use when you need the display type name for a `Primitive`, such as when generating CLI help
 * documentation.
 *
