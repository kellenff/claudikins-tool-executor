/**
 * Global flags for Effect CLI command trees. Global flags are parsed outside a single command's
 * local flags and can apply to a command and its descendants.
 *
 * This module defines two kinds of global flags: action flags, which run an effect and stop normal
 * command execution, and setting flags, which provide a parsed value to the command handler through
 * the Effect context. It also defines the built-in help, version, wizard, shell-completion, and
 * log-level flags used by `Command.run` and `Command.runWith`.
 *
 * @since 4.0.0
 */
...
export declare namespace Setting {
  /**
   * Type-level service identifier used by `Setting` global flags for the parsed value associated
   * with a setting id.
   *
   * @since 4.0.0
   * @category Models
...
/**
 * Defines the `--help` / `-h` global flag, which shows help documentation for the active command
 * path.
 *
 * @since 4.0.0
 * @category References
 ...
 * **Details**
 *
 * `Command.run` and `Command.runWith` handle this action specially so the generated arguments can
 * be passed back through the command parser.
 *
 * @since 4.0.0
 * @category References
 */
export const Wizard: Action<boolean> = action({
  flag: Flag.boolean("wizard").pipe(Flag.withDescription("Start wizard mode for a command")),
  run: () => Effect.void,
});

/**
 * Defines the `--completions` global flag, which prints a shell completion script for the given
 * shell.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when extending or inspecting the default global-flag set that `Command.runWith` prepends
 * before user-defined global flags.
 *
 * **Details**
 *
 * The built-ins are `Help`, `Version`, `Wizard`, `Completions`, and `LogLevel`. `Command.runWith`
 * prepends these built-ins when collecting and parsing global flags.
 *
 * **Gotchas**
 *
 * Action flags are processed in active flag order and the first present action exits, so this array
 * controls built-in action precedence.
 *
 * @since 4.0.0
 * @category References
