 ...
 *
 * CLI errors describe problems such as unknown or duplicate flags, missing flags or arguments,
 * invalid values, unknown subcommands, user handler failures, and requests to show command help.
 * This module includes the `CliError` union, the `isCliError` guard, schema-backed error classes
 * with display messages, and the `NonShowHelpErrors` union used when parse or validation errors
 * should be shown with help output.
 *
 * @since 4.0.0
 */
 ...
 * @category Models
 */
export class InvalidValue extends Schema.TaggedErrorClass<InvalidValue>(`${TypeId}/InvalidValue`)(
  "InvalidValue",
  {
    option: Schema.String,
    value: Schema.String,
  ...
  override get message() {
    const expectation =
      this.expected.startsWith("Expected ") || this.expected.startsWith("Expected:")
        ? this.expected
 ...
 * @category Models
 */
export class UserError extends Schema.TaggedErrorClass<UserError>(`${TypeId}/UserError`)(
  "UserError",
  {
    cause: Schema.Defect(),
  },
 ...
 * **Details**
 *
 * This excludes `ShowHelp` itself, allowing parse and validation errors to be stored in
 * `ShowHelp.errors` without nesting another help-control value.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * These errors can be accumulated and attached to `ShowHelp.errors` when the runner should display
 * help along with the underlying parse or validation failures.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * It is used for explicit help requests and for parse or validation failures that should be shown
 * with help text. When `errors` is non-empty, the runtime exit code is `1`; otherwise it is `0`.
 *
 * @since 4.0.0
 * @category Models
 */
export class ShowHelp extends Schema.TaggedErrorClass<ShowHelp>(`${TypeId}/ShowHelp`)("ShowHelp", {
