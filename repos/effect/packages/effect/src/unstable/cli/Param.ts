 ...
 *
 * A `Param<Kind, A>` describes how to consume parsed command-line input and return a typed value.
 * The `Kind` decides whether the parameter reads positional arguments or named flags. `Argument`
 * and `Flag` build on this module to share parsing structure, primitive constructors, help
 * metadata, aliases, defaults, prompts, configuration fallbacks, validation, schema decoding,
 * fallback parameters, and traversal helpers.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * A parameter knows whether it consumes positional arguments or flags and parses a `ParsedArgs`
 * value into its typed result.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * It returns the remaining positional arguments together with the parsed value, or fails with a
 * `CliError` while requiring the CLI parsing environment.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Map of flag names to their provided string values. Multiple occurrences of a flag produce
 * multiple values.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Input context passed to `Param.parse` implementations. - `flags`: already-collected flag values
 * by canonical flag name - `arguments`: remaining positional arguments to be consumed
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Represents a fallback prompt that can either be provided directly or computed effectfully when
 ...
 * **Details**
 *
 * Single parameters carry the user-facing name, aliases, description, primitive type, and optional
 * metavar/type name used in help output.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Parameter node that rewrites another parameter's parser, allowing effectful validation, fallback
 * behavior, or error translation while preserving the same parameter kind.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Parameter node that turns a missing argument or flag into `Option.none()` and a present parsed
 * value into `Option.some(value)`.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Parameter node that parses another parameter zero or more times and returns all parsed values as
 * an array, respecting optional minimum and maximum occurrence bounds.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Constructs a leaf `Single` parameter from its kind, name, primitive parser, and optional help
 * metadata.
 *
 * **Details**
 *
...
/**
 * Constructs command-line params that represent a choice between several inputs. The input will be
 * mapped to it's associated value during parsing.
 *
 ...
 * **Details**
 *
 * This is a convenience function that creates a path parameter with the `pathType` set to
 * `"directory"` and a default type name of `"directory"`.
 *
 ...
 * **Details**
 *
 * This is a convenience function that creates a path parameter with a `pathType` set to `"file"`
 * and a default type name of `"file"`.
 *
...
/**
 * Creates a redacted parameter for sensitive data like passwords. The value is masked in help
 * output and logging.
 *
 ...
 * **Details**
 *
 * The parser that is utilized will depend on the specified `format`, or the extension of the file
 * passed on the command-line if no `format` is specified.
 *
 ...
 * **When to use**
 *
 * Use when you need command-line options or arguments that collect `key=value` configuration
 * entries.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need an empty CLI parameter sentinel for optional parameter construction or internal
 * combinators.
 *
 ...
 * **Details**
 *
 * This works on any param structure by recursively finding the underlying `Single` node and
 * applying the alias there.
 *
 ...
 * **Details**
 *
 * Descriptions provide users with information about what the option does when they view help
 * documentation.
 *
...
/**
 * Hides a parameter from generated help output and completions while keeping it parseable on the
 * command line.
 *
 * **When to use**
 *
...
/**
 * Transforms the parsed value of an option using a function that may throw, converting any thrown
 * errors into failure messages.
 *
 ...
 * **Details**
 *
 * When the parameter is absent, parsing succeeds with `Option.none()` instead of failing with a
 * missing option or missing argument error. When present, the parsed value is wrapped in
 * `Option.some()`.
 *
 ...
 * **Details**
 *
 * The fallback may be a pure value or an effect. It is used only when the parameter is absent;
 * provided values are parsed normally.
 *
 ...
 * **When to use**
 *
 * Use when you need config to provide a fallback source for required flags or arguments that are
 * absent from CLI input.
 *
 * **Details**
 *
 ...
 * **Gotchas**
 *
 * Missing config preserves the original missing-parameter error. Config parse failure becomes
 ...
 * **Details**
 *
 * `FallbackPrompt` accepts either a `Prompt` or an effect that builds one. Effectful prompt
 * creation is lazy and runs only when the fallback is needed.
 *
 * **Gotchas**
 *
 * This only handles missing options and missing arguments. Invalid values do not prompt, and prompt
 * cancellation re-fails with the original missing error.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * This is the base combinator for creating parameters that accept multiple values. The `min` and
 * `max` parameters are optional. When they are not provided, the parameter can be specified any
 * number of times, from 0 to infinity.
 *
 ...
 * **Details**
 *
 * This combinator transforms an option to accept between `min` and `max` occurrences on the command
 * line, returning an array of all provided values.
 *
...
export const between: {
  <A>(
    min: number,
    max: number,
  ): <Kind extends ParamKind>(self: Param<Kind, A>) => Param<Kind, ReadonlyArray<A>>;
 ...
 * **Details**
 *
 * This combinator transforms an option to accept between 0 and `max` occurrences on the command
 * line, returning an array of all provided values.
 *
 ...
 * **Details**
 *
 * This combinator transforms an option to accept at least `min` occurrences on the command line,
 * returning an array of all provided values.
 *
...
/**
 * Filters and transforms parsed values, failing with a custom error message if the filter function
 * returns `Option.none()`.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The metavar is displayed in usage text to indicate what value the user should provide. For
 * example, `--output FILE` shows `FILE` as the metavar.
 *
 ...
 * **Details**
 *
 * The original param's value is returned as `Result.succeed`, while the fallback param's value is
...
/**
 * Type-safe param matcher that handles the unsafe casting internally. This provides a clean API for
 * pattern matching on param types while maintaining type safety at the call site.
 */
...
/**
 * Recursively transforms a param by applying a function to any `Single` nodes. This is used
 * internally by combinators like `withAlias` to traverse the param tree.
 */
...
/**
 * Extracts all Single params from a potentially nested param structure. This handles all param
 * combinators including Map, Transform, Optional, and Variadic.
 *
 * @internal
 */
...
/**
 * Gets the underlying Single param from a potentially nested param structure. Throws an error if
 * there are no singles or multiple singles found.
 *
 * @internal
 */
