 ...
 *
 * A `Command` combines a name, typed flags and positional arguments, optional subcommands, help
 * metadata, and an effectful handler. The module includes builders for command trees and the
 * runners that parse command-line input, handle built-in help and version behavior, render help
 * through `CliOutput`, and execute the selected handler.
 *
 * @since 4.0.0
 */
...
> extends Effect.Effect<ContextInput, never, CommandContext<Name>> {
  ...
  /**
   * Whether this command is hidden from parent help output, shell completions, and
   * unknown-subcommand suggestions. Hidden commands still parse and execute normally when invoked
   * by exact name.
   */
...
export declare namespace Command {
   ...
   * **Details**
   *
   * The parsed input type is contravariant, while the command error and service requirement types
   * are covariant.
   *
   * @since 4.0.0
   * @category Models
   ...
   * **Details**
   *
   * `Command.Config` can contain individual flags and arguments using `Param` types, nested
   * configuration objects for organization, and arrays of parameters for repeated elements.
   *
 ...
 * **Details**
 *
 * This includes file-system and path services for arguments, terminal and stdio services for
 * running commands, and child-process spawning for process-related CLI features.
 *
 * @since 4.0.0
...
/**
 * Service context for a specific command, enabling subcommands to access their parent's parsed
 * configuration.
 *
 * **Details**
 *
 * When a subcommand handler needs access to flags or arguments from a parent command, it can yield
 * the parent command directly to retrieve its config. This is powered by Effect's service system -
 * each command automatically creates a service that provides its parsed input to child commands.
 *
 ...
 * **Details**
 *
 * Use `withDescription` and related metadata combinators to add help text. The overloads support
 * simple commands, configured commands, and commands with effectful handlers.
 *
 ...
 * **Details**
 *
 * Subcommands can access their parent's parsed configuration by yielding the parent command within
 * their handler. This enables shared parent flags that affect all subcommands.
 *
 ...
 * **Details**
 *
 * Shared flags are available to this command's handler and to descendant handlers via `yield*
 * parentCommand`. Shared flags are accepted both before and after a selected subcommand name
 * (npm-style).
 *
 * @since 4.0.0
 * @category Combinators
...
type ExtractGlobalFlagContext<T extends ReadonlyArray<GlobalFlag.GlobalFlag<any>>> =
...
type ExtractSubcommand<T> =
  T extends Command<infer _Name, infer _Input, infer _CI, infer _E, infer _R>
    ...
    : T extends Command.SubcommandGroup<infer Commands>
      ? Commands[number]
      : never;
type ExtractSubcommandErrors<T extends ReadonlyArray<Command.SubcommandEntry>> = Error<
  ExtractSubcommand<T[number]>
>;
type ExtractSubcommandContext<T extends ReadonlyArray<Command.SubcommandEntry>> = Services<
  ExtractSubcommand<T[number]>
>;
 ...
 * **Details**
 *
 * Descriptions provide users with information about what the command does when they view help
 * documentation.
 *
 ...
 * **Details**
 *
 * Short descriptions are used when listing subcommands in help output and shell completions. If no
 * short description is provided, the full `description` is used as a fallback.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **Details**
 *
 * Aliases are accepted as alternate subcommand names during parsing and are shown in help output as
 * `name, alias`.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Hides a subcommand from parent help output, shell completions, and "did you mean?" suggestions
 * while keeping it fully invocable by exact name.
 *
 * **When to use**
 *
 * Use when experimental or internal subcommands should be accepted but not advertised on the public
 * CLI surface.
 *
 ...
 * **When to use**
 *
 * Use to attach one command-scoped metadata value under a `Context.Key`, especially for consumers
 * such as custom help formatters.
 *
 * **Details**
 *
 * Annotations are stored on the command's annotation context and flow into generated help document
 * annotations.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * Merged annotations are stored on the command and exposed through generated help document
 * annotations.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * Examples are exposed in structured `HelpDoc` data and rendered by the default formatter in an
 * `EXAMPLES` section.
 *
...
/**
 * Provides the handler of a command with the services produced by a layer that optionally depends
 * on the command-line input to be created.
 *
...
/**
 * Provides the handler of a command with the implementation of a service that optionally depends on
 * the command-line input to be constructed.
 *
 * **When to use**
 *
 * Use when a command handler needs a pure service implementation, optionally derived from the
...
/**
 * Provides the handler of a command with the service produced by an effect that optionally depends
 * on the command-line input to be created.
 *
 * **When to use**
 *
...
/**
 * Allows for execution of an effect, which optionally depends on command-line input to be created,
 * prior to executing the handler of a command.
 *
 * @since 4.0.0
 * @category Providing services
 */
 ...
 * **Details**
 *
 * The returned arguments include the command name and can be inspected, modified, or passed to
 * another command runner by the caller.
 *
 ...
 * **When to use**
 *
 * Use when you need to test CLI applications or programmatically execute commands with specific
 * arguments.
 *
      ...
      for (const flag of activeFlags) {
        ...
        if (flag === GlobalFlag.Wizard) {
          return yield* Effect.gen(function* () {
            yield* Console.log(
              Wizard.renderIntroduction(command.name, config.version, command.description),
            );
            const prefix = [
              command.name,
