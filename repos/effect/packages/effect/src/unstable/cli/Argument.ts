 ...
 *
 * Arguments consume ordered values after a command name and its flags, then parse them into the
 * types a command handler expects. This module includes constructors for common argument shapes,
 * plus helpers for optional or variadic arguments, schema validation, transformations, defaults,
 * config fallbacks, and prompts for missing values.
 *
 * @since 4.0.0
 */
 ...
 * **Gotchas**
 *
 * `boolean` is intentionally omitted from Argument constructors. Positional boolean arguments are
 * ambiguous in CLI design since there is no flag name to negate (for example, `--no-verbose`). Use
 * Flag.boolean instead, or use Argument.choice with explicit "true" / "false" strings if needed.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The parser is chosen from the explicit `format` option or, when omitted, the file extension. The
 * parsed value is `unknown`; use `fileSchema` when the parsed content should also be decoded with a
 * Schema.
 *
 ...
 * example, `<FILE>` shows `FILE` as the metavar.
 *
