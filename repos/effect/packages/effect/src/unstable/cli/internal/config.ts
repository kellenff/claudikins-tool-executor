 ...
 * # Config Internal
 *
 * The processed internal representation of a Command.Config declaration. Separates the user's
 * declared config shape from the flat parsing representation.
 *
 * Key concepts:
 *
 ...
 *
 * Created by parsing the user's config. Separates parameters by type while preserving the original
 * nested structure via the tree.
 */
...
/**
 * Namespace containing the tree and node types used to reconstruct parsed command configuration
 * values.
 *
 * @since 4.0.0
 */
export declare namespace ConfigInternal {
  /**
   * Maps declaration keys to their node representations. Preserves the shape of the user's config
   * object.
   */
 ...
 *
 * Walks the config structure and: 1. Extracts all Params into flat arrays (flags, arguments,
 * orderedParams) 2. Builds a tree that remembers the original nested structure 3. Assigns each
 * Param an index to link parsed values back
 *
 * @internal
 */
 ...
 *
 * Uses the tree as a blueprint to place each parsed value back into its original position in the
 * nested structure.
 *
 * @internal
 */
