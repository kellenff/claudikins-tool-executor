 ...
 *
 * The parser transforms raw argv tokens into structured command input through three main phases:
 *
 ...
 * 2. **Built-in Extraction**: Peels off built-in flags (help/version/completions) before
 *    command-specific parsing begins.
...
/**
 * Mutable map of flag names to their collected string values. Used internally during parsing;
 * converted to readonly at boundaries.
 */
type FlagMap = Record<string, Array<string>>;

/**
 * Registry of known flags for a command level. Enables O(1) lookup of flag definitions by name or
 * alias.
 */
type FlagRegistry = {
...
/**
 * Mutable accumulator for collecting flag values during parsing. Provides methods to add values and
 * merge results from sub-scans.
 */
...
/** Parsing mode during command-level scanning. Determines how value tokens are interpreted. */
...
/**
 * Stateful cursor for navigating a token stream. Provides peek/take semantics for single-pass
 * parsing.
 */
...
/**
 * Creates a mutable accumulator for collecting flag values. Pre-initializes empty arrays for all
 * known flags.
 */
...
/** Checks whether a token is a boolean literal value. Recognizes: true/false, yes/no, on/off, 1/0 */
 ...
 *
 * Value resolution order: 1. Inline value: --flag=value or -f=value 2. Boolean special case:
 * implicit "true" or explicit boolean literal 3. Next token: consume following Value token if
 * present
 */
...
/**
 * Consumes known flags from a token stream. Unrecognized tokens are passed through to remainder.
 ...
 *
 * Command-local flags take precedence over global flags at the selected command level. This lets
 * commands reuse a global flag name, for example a subcommand with its own `--version <value>` flag
 * overriding the built-in global `--version` action.
 *
 * @internal
 */
    ...
    for (let token = cursor.take(); token; token = cursor.take()) {
      ...
      if (token._tag === "Value" && awaitingFirstValue) {
        ...
        if (sub !== undefined) {
          remainder.push(token);
          remainder.push(
            ...consumeLevel(cursor.rest(), sub, [...ignoredRegistries, inheritedRegistry]),
          );
          return remainder;
        }
 ...
 *
 * If it matches a known subcommand: - Collect inherited parent flags from remaining tokens
 * (npm-style) - Return SubcommandResult with child tokens
 *
 * Otherwise: - Return Argument to treat it as a positional argument - Report error if command
 * expects subcommand but got unknown value
 */
...
/**
 * Processes a flag token: looks up in registry, consumes value, records it. Reports unrecognized
 * flags as errors.
 */
const processFlag = (
  token: FlagToken,
 ...
 *
 * In AwaitingFirstValue mode: - Check if value is a subcommand - If so, return SubcommandResult to
 * exit scanning - If not, switch to CollectingArguments mode
 *
 * In CollectingArguments mode: - Simply add value to arguments list
 */
const processValue = (
  value: string,
  cursor: TokenCursor,
 ...
 *
 * For each token: - Flags: Look up, consume value, record in accumulator - Values: Check for
 * subcommand (first value only), then collect as arguments
 *
