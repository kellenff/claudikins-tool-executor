 ...
 *
 * Internal implementation details for CLI commands. Public API is in ../Command.ts
 */
...
/**
 * Internal implementation interface with all the machinery. Use toImpl() to access from internal
 * code.
 */
...
/**
 * Casts a Command to its internal implementation. For use by internal modules that need access to
 * config, parse, handle, etc.
 */
...
export const makeCommand = <const Name extends string, Input, E, R, ContextInput = {}>(options: {
  ...
  readonly parse?:
    | ((input: ParsedTokens) => Effect.Effect<Input, CliError.CliError, Environment>)
    | undefined;
  readonly parseContext?:
  ...
  const service =
    options.service ??
    Context.Service<CommandContext<Name>, ContextInput>(`${TypeId}/${options.name}`);
...
/**
 * Creates a parser for a given config. Used as the default for both `parse` and `parseContext`, and
 * also by `withSharedFlags` to avoid constructing a full throwaway command.
 */
...
/**
 * Checks that inherited parent context flags do not reuse names declared by child command flags.
 *
 * When `contextConfig` is supplied, it is used as the inherited flag set; otherwise the parent's
 * current context config is checked.
 */
