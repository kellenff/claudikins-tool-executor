 ...
 * Help Documentation ================
 *
 * Internal helpers for generating help documentation. Extracted from command.ts to avoid circular
 * dependencies.
 */
...
/**
 * Returns the resolved command lineage for the provided path. Includes the root command as the
 * first element.
 */
...
/**
 * Returns active global flags for a command path. Built-ins are prepended and declarations are
 * collected root -> leaf.
 */
...
/**
 * Returns all global flags declared in a command tree. Built-ins are prepended and command
 * declarations are deduplicated by identity.
 */
...
/**
 * Helper function to get help documentation for a specific command path. Navigates through the
 * command hierarchy to find the right command. Reads active global flags for the path and includes
 * them in the help doc.
 */
