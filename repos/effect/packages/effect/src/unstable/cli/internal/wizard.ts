...
export const run: (
  ...
  options?: Options,
) => Effect.Effect<Array<string>, CliError.CliError | Terminal.QuitError, Command.Environment> =
  Effect.fnUntraced(function* (command, options) {
...
const promptCommand: (
  ...
  sectionName: string,
) => Effect.Effect<void, CliError.CliError | Terminal.QuitError, Command.Environment> =
  Effect.fnUntraced(function* (command, commandLine, sectionName) {
    const impl = toImpl(command);
    const visibleSubcommands = command.subcommands.flatMap((group) =>
      group.commands.filter((child) => !child.hidden),
    );
...
const hasWizardSteps = (command: Command.Command.Any): boolean => {
  const hasVisibleSubcommands = command.subcommands.some((group) =>
    group.commands.some((child) => !child.hidden),
  );
...
const renderParamLabel = (single: Param.Single<Param.ParamKind, unknown>): string => {
  ...
  const label =
    single.kind === Param.flagKind && description !== undefined && description.length <= 32
      ? description
...
const renderSection = (commandName: string, section: string): string =>
  `${Ansi.annotate(commandName.toUpperCase(), Ansi.bold, Ansi.cyanBright)} ${Ansi.annotate("·", Ansi.blackBright)} ${Ansi.annotate(
    section,
    Ansi.bold,
    Ansi.white,
  )}`;
...
export const renderIntroduction = (
  ...
  const title = `${Ansi.annotate(name, Ansi.bold, Ansi.cyanBright)} ${Ansi.annotate(`v${version}`, Ansi.white)} ${Ansi.annotate(
    "· Command wizard",
    Ansi.bold,
    Ansi.white,
  )}`;
