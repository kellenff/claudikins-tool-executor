 ...
 *
 * A `Prompt<A>` describes a small terminal UI that renders frames, reads keyboard input, validates
 * responses, and eventually produces an `A`. Prompts can ask for simple values, selections, lists,
 * files, hidden text, or custom interactions. This module includes prompt constructors, tools for
 * combining and transforming prompt output, and support for running prompts through the `Terminal`
 * service.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * A `Prompt` is an `Effect` that may fail with `Terminal.QuitError` and requires the prompt
 * environment needed to render frames, read input, and access files or paths when a prompt uses
 * them.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Represents the action that should be taken by a `Prompt` based upon user input or an external
 * event received during the current frame.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * It connects the action state and output type parameters to the `Beep`, `NextFrame`, and `Submit`
 * action cases.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Represents the input that should be processed by a `Prompt` based upon user input or an external
 * event received during the current frame.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * The handlers render the current frame, process user input into the next `Prompt.Action`, and
...
export interface Handlers<State, Output, Input = Terminal.UserInput> {
  ...
  /**
   * A function that is called to process user input and determine the next `Prompt.Action` that
   * should be taken.
   */
  readonly process: (
    input: Input,
  ...
  /**
   * A function that is called to clear the terminal screen before rendering the next frame of the
   * `Prompt`.
   */
  readonly clear: (
    state: State,
...
export interface ConfirmOptions {
  ...
  readonly placeholder?: {
    /**
     * The placeholder to use if the `initial` value of the prompt is `true` (defaults to
     * `"(Y/n)"`).
     */
    readonly defaultConfirm?: string;
    /**
     * The placeholder to use if the `initial` value of the prompt is `false` (defaults to
     * `"(y/N)"`).
     */
...
/**
 * Options for a date prompt, including the displayed message, initial value, format mask,
 * validation, and locale labels.
 *
 * @since 4.0.0
 * @category Options
 */
export interface DateOptions {
  /** The message to display in the prompt. */
  readonly message: string;
  /** The initial date value to display in the prompt (defaults to the current date). */
  readonly initial?: globalThis.Date;
  /** The format mask of the date (defaults to `YYYY-MM-DD HH:mm:ss`). */
  readonly dateMask?: string;
  /**
   * An effectful function that can be used to validate the value entered into the prompt before
   * final submission.
   */
...
export interface IntegerOptions {
  ...
  readonly max?: number;
  /**
   * The value that will be used to increment the prompt value when using the up arrow key (defaults
   * to `1`).
   */
  readonly incrementBy?: number;
  /**
   * The value that will be used to decrement the prompt value when using the down arrow key
   * (defaults to `1`).
   */
 ...
 * **Details**
 *
 * In addition to the numeric bounds and step settings from `IntegerOptions`, the prompt can be
 * configured with a display precision.
 *
 * @since 4.0.0
 * @category Options
 */
 ...
 * **Details**
 *
 * They control which path type can be selected, the starting directory, paging, and filtering of
 * displayed entries.
 *
 * @since 4.0.0
 * @category Options
...
export interface FileOptions {
  ...
  readonly message?: string;
  /**
   * Where the user will initially be prompted to select files from, defaulting to the current
   * working directory.
   */
  ...
  /**
   * A predicate or effect that keeps a file in the prompt when it returns `true`, defaulting to
   * returning all files.
   */
...
/**
 * Options for a multi-select prompt, including bulk-selection labels and minimum or maximum
 * selection counts.
 *
 * @since 4.0.0
 * @category Options
...
export interface SelectChoice<A> {
  ...
  readonly value: A;
  /** An optional description for the select option which will be displayed to the user. */
...
/**
 * Options for text-entry prompts, including the displayed message, default text, and effectful
...
export interface ToggleOptions {
  ...
  readonly initial?: boolean;
  /** The text to display when the toggle is in the active state (defaults to `on`). */
  readonly active?: string;
  /** The text to display when the toggle is in the inactive state (defaults to `off`). */
...
export declare namespace All {
  ...
  /**
   * Computes the prompt returned by `Prompt.all` for a readonly tuple or array of prompts,
   * preserving tuple positions in the output type.
   *
   * @since 4.0.0
   * @category Utility types
   */
  ...
  /**
   * Computes the prompt returned by `Prompt.all` for a record of prompts, preserving the record
   * keys and replacing each prompt with its output type.
   *
   * @since 4.0.0
   * @category Utility types
   */
  ...
  /**
   * Computes the return prompt type for `Prompt.all` based on the input structure.
   *
   * @since 4.0.0
   * @category Constructors
   */
  export type Return<Arg extends Iterable<Any> | Record<string, Any>> = [Arg] extends [
    ReadonlyArray<Any>,
  ]
    ? ReturnTuple<Arg>
 ...
 */
export const all: <const Arg extends Iterable<Prompt<any>> | Record<string, Prompt<any>>>(
  arg: Arg,
 ...
 * **Details**
 *
 * `initial` defaults to `false`. Enter submits the current default, yes-style input submits `true`,
 * no-style input submits `false`, and other input beeps.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * The initial state can either be a pure value or an `Effect`. This is particularly useful when the
 * initial state of the `Prompt` must be computed by performing an effectful computation, such as
 * reading data from the file system. A `Prompt` runs as a render loop: `render` returns ANSI output
 * for the current frame, the `Terminal` obtains user input, `process` returns the next prompt
 * action, and `clear` returns ANSI output used to clear the previous frame.
 *
 * Optionally, an external `events` dequeue can be provided as the third argument. When present, the
 * render loop will race user input against events from the dequeue, allowing background events to
 * trigger re-renders without waiting for a keypress. When an event is received from the dequeue,
 * the `receive` handler is called instead of `process`.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Creates a date prompt that lets the user edit a formatted date value and validates the final
 * `Date` before submission.
 *
 * **Details**
 *
 * `initial` defaults to the current `Date`, `dateMask` defaults to `YYYY-MM-DD HH:mm:ss`, mask
 * parsing creates editable date parts plus literal tokens, `locales` customizes month and weekday
 * labels, and `validate` runs on submission.
 *
 * **Gotchas**
 *
 * A supplied `initial` `Date` is edited in place during prompt interaction. Date edits use
 * JavaScript `Date` setters, so out-of-range typed values can normalize before validation. If the
 * prompt is meant to be editable, `dateMask` should contain at least one editable date token.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The prompt supports minimum and maximum bounds, keyboard step sizes, display precision, and
...
/**
 * Creates a text prompt that does not echo typed input and returns the submitted value wrapped in
 * `Redacted`.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * The prompt supports minimum and maximum bounds, keyboard step sizes, and additional validation
 * before submission.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Creates a text prompt that returns an array of strings by splitting the submitted input on the
...
/**
 * Creates a password prompt that masks typed input and returns the submitted value wrapped in
 * `Redacted`.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Runs a prompt by reading terminal input and rendering prompt frames until the prompt submits a
 * value.
 *
 * **Gotchas**
 *
 * The returned effect may fail with `Terminal.QuitError` if terminal input ends or the prompt is
 * quit.
 *
 * @since 4.0.0
 * @category Execution
...
export const autoComplete = <const A>(options: AutoCompleteOptions<A>): Prompt<A> => {
  ...
  const index =
    filtered.length === 0 ? 0 : filtered.includes(initialIndex) ? initialIndex : filtered[0];
 ...
 * **Details**
 *
 * The prompt supports default selected choices, bulk-selection commands, and minimum or maximum
 * selection counts.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Creates a toggle prompt that lets the user switch between active and inactive states and returns
    ...
    return Effect.flatMap(FileSystem.FileSystem, (fs) =>
      ...
      Effect.orDie(fs.exists(startingPath)).pipe(
        Effect.flatMap((exists) =>
          exists
            ? Effect.void
            : Effect.die(`The provided starting path '${startingPath}' does not exist`),
        ),
...
const handleFileClear = (options: FileOptionsReq) => {
  return Effect.fnUntraced(function* (state: FileState, _: Action<FileState, string>) {
    ...
    const resolvedPath =
      selectedPath === undefined ? currentPath : path.resolve(currentPath, selectedPath);
    ...
    switch (input.key.name) {
      ...
      case "return": {
        ...
        if (options.min !== undefined && selectedCount < options.min) {
          return Effect.succeed(
            Action.NextFrame({
              state: { ...state, error: Option.some(`At least ${options.min} are required`) },
            }),
          );
        }
        if (options.max !== undefined && selectedCount > options.max) {
          return Effect.succeed(
            Action.NextFrame({
              state: { ...state, error: Option.some(`At most ${options.max} choices are allowed`) },
            }),
          );
        }
