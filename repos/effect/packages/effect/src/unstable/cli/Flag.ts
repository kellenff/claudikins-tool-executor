 ...
 *
 * A `Flag<A>` describes how to read one named value from parsed command-line input, validate it,
 * and produce an `A`. Flags are useful for inputs such as ports, verbosity switches, configuration
 * files, output directories, choices, secrets, and repeated values. The helpers here build flags
 * with aliases, defaults, optional values, prompts, configuration fallbacks, validation, and value
 * transformations.
 *
 * @since 4.0.0
 */
...
/**
 * Constructs option parameters that represent a choice between several inputs. Each tuple maps a
 * string flag value to an associated typed value.
 *
 ...
 * **When to use**
 *
 * Use when you need to define a named CLI flag with fixed string choices and no custom value
 * mapping.
 *
 * **Gotchas**
 *
...
/**
 * Creates a string flag whose parsed value is wrapped in `Redacted.Redacted` so stringification and
 * logging redact the value.
 *
 * **Gotchas**
 *
 * Values supplied on the command line may still be visible to the operating system or shell
 * history.
 *
...
/**
 * Creates an empty sentinel flag that always fails to parse. This is useful for creating
 * placeholder flags or for combinators.
 *
...
/**
 * Hides a flag from generated help output and shell completions while keeping it fully parseable on
 * the command line.
 *
 * **When to use**
 *
 * Use when experimental or internal flags should be accepted but not advertised, such as
 * `--experimental-foo`, debug toggles, or escape hatches that are not yet committed to the public
 * CLI surface.
 *
...
/**
 * Transforms and filters a flag value, failing with a custom error if the transformation returns
 * None.
 *
...
/**
 * Filters a flag value based on a predicate, failing with a custom error if the predicate returns
 * false.
 *
...
/**
 * Tries to parse with the first flag, then the second, returning a Result that indicates which
 * succeeded.
 *
