 ...
 *
 * An `AtomRegistry` evaluates atoms, caches their current values, tracks dependencies, applies
 * writes and refreshes, manages subscriptions, and disposes unused nodes. Each registry is
 * independent, so the same atom can hold different values in different registries. Serializable
 * atom values can also be preloaded before the first read.
 *
 * @since 4.0.0
 */
...
/**
 * The runtime registry that stores atom nodes and coordinates reads, writes, refreshes,
 * subscriptions, and disposal.
 *
 * **Details**
 *
 * It also manages scheduler configuration, serializable preloaded values, and node addition/removal
 * callbacks.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Nodes expose the current value, parent and child dependency links, listener set, and current
 * lifecycle state.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Options can preload initial atom values, provide a custom task scheduler, configure timeout
 * bucket resolution, and set a default idle time-to-live for unused atoms.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use to access or provide the registry that stores atom values, dependencies, subscriptions, and
 * disposal state for a reactive lifetime.
 *
 * @since 4.0.0
 * @category Services
 */
 ...
 * **Details**
 *
 * The stream emits the current value immediately, emits subsequent changes, and unsubscribes from
 * the registry when the stream scope closes.
 *
 * @since 4.0.0
 * @category Converting
 */
 ...
 * **Details**
 *
 * Initial results are skipped, failures fail the stream with their cause, and duplicate stream
 * values are dropped with `Stream.changes`.
 *
 * @since 4.0.0
 * @category Converting
 */
 ...
 * **Details**
 *
 * The effect waits for the result to leave `Initial`, and also waits through waiting results when
 * `suspendOnWaiting` is enabled.
 *
 * @since 4.0.0
 * @category Converting
 */
 ...
 * **Details**
 *
 * The atom is subscribed with a no-op listener and the subscription is released when the scope
 * finalizer runs.
 *
 * @since 4.0.0
 * @category Converting
