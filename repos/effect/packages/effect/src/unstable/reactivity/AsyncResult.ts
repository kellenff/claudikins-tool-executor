 ...
 *
 * `AsyncResult<A, E>` records whether asynchronous work has no value yet, succeeded with an `A`, or
 * failed with an `E`. Every state also carries a `waiting` flag, so callers can keep showing the
 * current value while newer work is loading, refreshing, retrying, or recovering. This module
 * includes constructors, checks, accessors, mapping and matching helpers, ways to combine several
 * results, and schemas for encoding or decoding results.
 *
 * @since 4.0.0
 */
...
/**
 * Represents the state of an asynchronous value as `Initial`, `Success`, or `Failure`, with a
 * `waiting` flag for in-flight refreshes.
 *
 * @since 4.0.0
 * @category Models
 */
...
export declare namespace AsyncResult {
  /**
   * Common prototype fields implemented by every `AsyncResult` variant, including pipeability, the
   * type marker, phantom type members, and the `waiting` flag.
   *
   * @since 4.0.0
   * @category Models
   */
...
/**
 * Rebuilds an `AsyncResult` with new success and failure types while preserving the variant of
 * another result.
 *
 * @since 4.0.0
 * @category Utility types
...
/**
 * Returns whether an `AsyncResult` is currently waiting for an asynchronous computation or refresh
 * to finish.
 *
 * @since 4.0.0
...
/**
 * Converts an `Exit` into a `Success` when it succeeds or a `Failure` carrying the exit cause when
 * it fails.
 *
 * @since 4.0.0
...
/**
 * Converts an `Exit` to a result, preserving the latest previous success when the exit is a
 * failure.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Creates a waiting result from an optional previous result, using `Initial(true)` when no previous
 * result exists.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * @category Refinements
 */
export const isInitial = <A, E>(result: AsyncResult<A, E>): result is Initial<A, E> =>
  result._tag === "Initial";
...
/**
 * Successful `AsyncResult` containing the current value, its timestamp, and the shared waiting
 * flag.
 *
 * @since 4.0.0
 * @category Models
 ...
 * @category Refinements
 */
export const isSuccess = <A, E>(result: AsyncResult<A, E>): result is Success<A, E> =>
  result._tag === "Success";
...
/**
 * Failed `AsyncResult` containing a failure cause and the latest previous success when one is
 * available.
 *
 * @since 4.0.0
 * @category Models
 ...
 * @category Refinements
 */
export const isFailure = <A, E>(result: AsyncResult<A, E>): result is Failure<A, E> =>
  result._tag === "Failure";
...
/**
 * Creates a `Failure` result from a `Cause`, optionally preserving a previous success and marking
 * the result as waiting.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Creates a `Failure` result from a `Cause`, carrying forward the latest success stored in a
 * previous result.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Creates a `Failure` result from a typed error while carrying forward the latest success stored in
 * a previous result.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Marks an `AsyncResult` as waiting, optionally touching the timestamp when the result is a
 * `Success`.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Refreshes the timestamp of a `Success` result while preserving its value and waiting flag;
...
/**
 * Replaces a `Failure` value's stored previous success with the latest success found in another
 * result.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Returns the current success value, or the previous success value stored in a failure, as an
 * `Option`.
 *
 * @since 4.0.0
 * @category Accessors
...
/**
 * Returns the available value from `value`, or evaluates the fallback when no current or previous
 * success exists.
 *
 * @since 4.0.0
 * @category Accessors
...
/**
 * Returns the available value from `value`, or throws `NoSuchElementError` when no current or
...
/**
 * Returns the first typed error from a failure cause, or `None` for successes, initial results,
 * defects, and interrupt-only causes.
 *
 * @since 4.0.0
 * @category Accessors
 */
...
/**
 * Converts a result to an `Exit`, succeeding with a success value, failing with a failure cause, or
 * failing with `NoSuchElementError` for `Initial`.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Maps the success value of an `AsyncResult`, also mapping any previous success stored in a failure
 * while leaving initial results unchanged.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **When to use**
 *
 * Use to sequence computations that may return another `AsyncResult` while preserving initial and
 * failure states.
 *
 * **Details**
 *
 * Initial results are left unchanged. Failures preserve their cause and remap the stored previous
 * success when the mapping function returns a success.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Pattern matches a result, handling successes and initials directly while splitting failures into
 * typed errors or squashed non-error causes passed to `onDefect`.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Pattern matches a result by calling `onWaiting` for waiting or initial states, otherwise handling
 * successes and splitting failures into typed errors or squashed non-error causes.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Combines an iterable or record of `AsyncResult` and plain values into one `AsyncResult`,
 * returning the first non-success result or a success of the collected values marked waiting when
 * any input success is waiting.
 *
 * @since 4.0.0
 * @category Combinators
 */
export const all = <const Arg extends Iterable<any> | Record<string, any>>(
  ...
  [Arg] extends [ReadonlyArray<any>]
    ? {
        -readonly [K in keyof Arg]: [Arg[K]] extends [AsyncResult<infer _A, infer _E>]
          ? _A
          : Arg[K];
      }
...
/**
 * Creates a typed builder for rendering an `AsyncResult` by handling waiting, initial, success,
 * error, defect, interrupt, and failure cases.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Fluent renderer for `AsyncResult` values that tracks unhandled cases at the type level and
 * exposes `exhaustive` only after all possible cases are handled.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Schema interface for `AsyncResult` values, retaining the schemas used for success values and
 * failure errors.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Creates a schema for `AsyncResult` values using optional schemas for success values and failure
 * errors.
 *
 * @since 4.0.0
 * @category Schemas
