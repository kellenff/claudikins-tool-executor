 ...
 * helpers. Query atoms call RPCs and track asynchronous or streaming results, while mutations run
 * RPC calls that can invalidate reactivity keys after success. Query atoms can also use request
 * headers, time-to-live settings, and serialization keys for hydration.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It exposes the RPC client, an atom runtime, mutation helpers that return `AtomResultFn`s, and
 * query helpers that return atoms or pull atoms for RPC results.
 *
 * @since 4.0.0
 * @category Models
 */
...
> extends Context.Service<Self, RpcClient.RpcClient.Flat<Rpcs, RpcClientError>> {
 ...
 * **Details**
 *
 * The options provide the RPC group, protocol layer, tracing options, request id generation,
 * optional custom client effect, and runtime factory used by the query and mutation helpers.
 *
 * @since 4.0.0
...
export const Service =
  ...
  >(
    ...
    options: {
      ...
      readonly makeEffect?:
        | Effect.Effect<RpcClient.RpcClient.Flat<Rpcs, RpcClientError>, never, RM>
        | undefined;
    ...
    const queryFamily = Atom.family((key: QueryKey) => {
      ...
      let atom = isStream
        ? self.runtime.pull(
            Stream.unwrap(
              self.use((client) => Effect.succeed(client(tag, payload, { headers } as any) as any)),
            ),
          )
