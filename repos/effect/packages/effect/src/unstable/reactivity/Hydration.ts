 ...
 *
 * `dehydrate` reads atoms marked with `Atom.serializable` from an `AtomRegistry` and returns
 * encoded entries keyed by their serialization keys. `hydrate` preloads those entries into another
 * registry before the atoms are read. Initial `AsyncResult` values can be ignored, encoded as
 * values, or represented by promises that update the target registry once the result is no longer
 * initial.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It stores the atom serialization key, encoded value, dehydration timestamp, and an optional
 * promise used when an `AsyncResult.Initial` value is encoded as a future non-initial value.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * Only atoms marked with `Atom.serializable` are included. `encodeInitialAs` controls whether
 * `AsyncResult.Initial` values are ignored, encoded as values, or represented by promises that
 * resolve when the atom leaves the initial state.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * Encoded values are preloaded by serialization key. Entries with a `resultPromise` update the
 * matching registry node, or preload the resolved value, when the promise resolves.
 *
 * @since 4.0.0
 * @category Hydration
 */
