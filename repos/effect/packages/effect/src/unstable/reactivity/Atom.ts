 ...
 *
 * An `Atom` describes how to produce or update one piece of reactive state. The registry runs atom
 * reads, remembers current values, tracks dependencies between atoms, starts effects and streams,
 * and cleans up atoms that are no longer used. This module includes the atom constructors and
 * update helpers used for cached values, effect-backed values, streams, browser state, stored
...
/**
 * Reactive value read by an `AtomRegistry`, with metadata controlling caching, laziness, refresh
 * behavior, and initial value targeting.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Returns an atom type without serializable metadata, preserving `Writable` read and write types
 * when the input atom is writable.
 *
 * @since 4.0.0
 * @category Utility types
 */
...
/**
 * Atom that can also be written to, using a `WriteContext` and an input value to update reactive
 * state.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Context passed to atom read functions for reading dependencies, awaiting `AsyncResult` or
 * `Option` values, managing subscriptions and finalizers, refreshing atoms, and updating writable
 * atoms.
 *
 * @since 4.0.0
 * @category Context
...
/**
 * Context passed to writable atom write functions for reading atoms, refreshing or setting the
 * current atom, and writing to other writable atoms.
 *
 * @since 4.0.0
 * @category Context
 */
...
/**
 * Returns a copy of an atom with an idle time-to-live: finite durations dispose it after
 * inactivity, while an infinite duration keeps it alive.
 *
 * @since 4.0.0
...
/**
 * Creates a read-only atom from a read function and an optional custom refresh registration
 * callback.
 *
 * @since 4.0.0
...
/**
 * Creates a writable atom from read and write functions, with an optional custom refresh
...
/**
 * Creates an atom from a synchronous value or read function, or from an `Effect` or `Stream` whose
 * state is exposed as an `AsyncResult`; plain values create writable state atoms.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Atom that builds a `Context` from a `Layer` and exposes constructors for atoms, functions, pulls,
 * and subscription refs that run with that context.
 *
 * @since 4.0.0
 * @category Models
 */
...
export interface RuntimeFactory {
  ...
  /** Uses the `Reactivity` service from the runtime to refresh the atom whenever the keys change. */
 ...
 * @category Constructors
 */
export const context: (options: { readonly memoMap: Layer.MemoMap }) => RuntimeFactory = (
  options,
) => {
...
/**
 * Returns `Rx.runtime.withReactivity` for refreshing an atom whenever the keys change in the
 * `Reactivity` service.
 *
 * **When to use**
 *
 * Use to refresh an atom whenever one or more invalidation keys change in the default reactivity
 * runtime.
 *
 * @since 4.0.0
 * @category Reactivity
...
/**
 * Creates a writable atom backed by a `SubscriptionRef`, or by an effect that produces one,
 * updating from ref changes and writing atom updates back to the ref.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Context passed to `fn` and `fnSync` computations for reading atoms, awaiting results, registering
 * finalizers, refreshing atoms, subscribing to changes, and writing updates.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a writable atom for a synchronous function; writing an argument re-runs the function,
 * returning `Option.none` before the first call unless an initial value is supplied.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Writable async function atom whose value is an `AsyncResult` and whose writes accept function
 * arguments plus `Reset` and `Interrupt` controls.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Defines the control symbol that can be written to an `AtomResultFn` to reset it to its initial
 * state.
 *
 * **When to use**
 *
 * Use when you need an `AtomResultFn` write value that clears the current async result and returns
...
/**
 * Defines the control symbol that can be written to an `AtomResultFn` to interrupt the current
 * asynchronous computation.
 *
 * **When to use**
 *
 * Use when you need an `AtomResultFn` write value that interrupts the currently running async
 * computation.
 *
 * @since 4.0.0
 * @category Symbols
...
/**
 * Creates a writable atom for an `Effect` or `Stream` function; writing an argument starts the
 * computation and exposes its state as an `AsyncResult`.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
const makeFn = <Arg, E, A>(
  f: (
    arg: Arg,
    get: FnContext,
  ) => Stream.Stream<A, E, AtomRegistry> | Effect.Effect<A, E, Scope.Scope | AtomRegistry>,
  options?: {
    readonly initialValue?: A | undefined;
    readonly concurrent?: boolean | undefined;
  },
...
function makeResultFn<Arg, E, A>(
  f: (
    arg: Arg,
    get: FnContext,
  ) => Effect.Effect<A, E, Scope.Scope | AtomRegistry> | Stream.Stream<A, E, AtomRegistry>,
  options?: {
    readonly initialValue?: A;
    readonly concurrent?: boolean | undefined;
  },
...
/**
 * `AsyncResult` produced by `pull`, containing a non-empty batch of pulled items and a `done` flag,
 * or `NoSuchElementError` when the stream completes without items.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a writable atom that pulls an initial chunk from a stream and then pulls the next chunk
 * whenever it is written to, accumulating items unless `disableAccumulation` is enabled.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Creates a memoized atom factory that returns the same object for the same argument, using weak
 * references for cached values when the platform supports them.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Uses a fallback `AsyncResult` atom while the primary atom is `Initial`, marking the fallback
 * result as waiting until the primary atom produces a non-initial result.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **Details**
 *
 * Lazy atoms defer recomputation while they have no active listeners or active non-lazy dependents,
 * rebuilding the next time their value is observed.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **Details**
 *
 * The label is used for inspection and debugging metadata and does not change the atom's read or
 * write behavior.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * The returned tuple can be supplied to `AtomRegistry` initial values so the atom starts with the
 * provided value before it is first rebuilt.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **Details**
 *
 * If the source is writable, the derived atom keeps the source write input and forwards writes to
 * the source. `initialValueTarget` controls which atom receives preloaded initial values for the
 * derived atom.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * When the source atom is writable, the returned atom remains writable and keeps the source atom's
 * write input type.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * Initial and failure states are preserved, and writable source atoms keep their original write
 * input type.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Creates an atom that publishes source changes only after the source has stopped changing for the
 * specified duration.
 *
 * **Details**
 *
 * The current source value is used immediately, and any pending debounce timer is cleared when the
 * derived atom is disposed.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Creates a derived atom that reads the source and schedules a refresh after the specified
 * duration.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * Automatic revalidation during reads is skipped while the current value is fresh within
 * `staleTime`. Manual `refresh` calls remain forceful and always forward to the wrapped atom. Use
 * `revalidateOnMount` to control whether stale data should trigger a background refresh on first
 * mount. Use `revalidateOnFocus` to control focus behavior. `true` respects `staleTime` and
 * `"always"` forces refetch.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
const isFreshWithin = (timestamp: number, staleTime: number, now: number): boolean =>
  now - timestamp < staleTime;
 ...
 * **Details**
 *
 * Writes accept transition atoms containing `AsyncResult` values. Waiting successes are shown
 * optimistically while transitions run; when successful transitions finish, the source atom is
 * refreshed, and failures roll the value back to the latest source value.
 *
 * @since 4.0.0
 * @category Optimistic
 */
export const optimistic = <A>(
  ...
  const writeAtom = removeTtl(
    state([counter, undefined as any as Atom<AsyncResult.AsyncResult<A, unknown>>] as const),
  );
  return writable(
    (get) => {
...
/**
 * Creates an `AtomResultFn` that applies an optimistic update before running the underlying
 * mutation.
 *
 * **Details**
 *
 * The reducer computes the provisional value from the current value and mutation input. The wrapped
 * function result then completes the transition or updates the optimistic value through the
 * provided setter callback.
 *
 * @since 4.0.0
 * @category Optimistic
 */
          ...
          ? autoDispose(
              options.fn((value) =>
                get.set(
                  transition,
                  AsyncResult.success(
                    AsyncResult.isAsyncResult(value) ? AsyncResult.waiting(value) : value,
                    { waiting: true },
                  ),
                ),
              ),
 ...
 * **Details**
 *
 * Stale nodes are rebuilt and listeners are notified after the callback completes, so dependent
 * updates observe the final batched state.
 *
 * @since 4.0.0
 * @category Batching
 */
 ...
 * **Details**
 *
 * It listens for `visibilitychange` events on `window` and removes the listener when the atom is
 * disposed.
 *
 * @since 4.0.0
 * @category Focus
 ...
 * **Details**
 *
 * The derived atom also subscribes to the source atom so normal source updates are forwarded to its
 * own value.
 *
 * @since 4.0.0
 * @category Focus
 ...
 * **Details**
 *
 * Values are encoded and decoded with the supplied schema. In sync mode the atom exposes the
 * decoded value and writes the default value when the key is missing; in async mode it exposes an
 * `AsyncResult` of the decoded value.
 *
 * @since 4.0.0
 * @category KeyValueStore
 */
export const kvs = <
  S extends Schema.ConstraintCodec<any, any>,
 ...
 * **Details**
 *
 * The stream emits the atom's current value immediately and then emits subsequent changes until the
 * stream scope is closed.
 *
 * @since 4.0.0
 * @category Converting
 */
 ...
 * **Details**
 *
 * Initial results are skipped, successes are emitted as stream values, and failures fail the stream
...
/**
 * Reads a writable atom, computes a return value and next write value, writes the next value, and
...
export const modify: {
  ...
  <R, W, A>(
    self: Writable<R, W>,
    f: (_: R) => [returnValue: A, nextValue: W],
  ): Effect.Effect<A, never, AtomRegistry> => Effect.map(AtomRegistry, (_) => _.modify(self, f)),
);
...
/**
 * Updates a writable atom by reading its current value from the registry and writing the value
 ...
 * **Details**
 *
 * The effect waits while the result is `Initial`, and also while it is waiting when
 * `suspendOnWaiting` is enabled. Successes succeed with the value and failures fail with the result
 * cause.
 *
 * @since 4.0.0
 * @category Converting
 ...
 * **Details**
 *
 * The key identifies the atom in dehydrated state, and the encode/decode functions convert between
 * the atom value and the schema encoded value.
 *
 * @since 4.0.0
 ...
 * @category Serializable
 */
export const isSerializable = (self: Atom<any>): self is Atom<any> & Serializable<any> =>
 ...
 * **Details**
 *
 * The schema is converted to a JSON codec for synchronous encode/decode, and the key is also used
 * as the atom label when the atom does not already have one.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * @category ServerValue
 */
export const withServerValueInitial = <A extends Atom<AsyncResult.AsyncResult<any, any>>>(
  self: A,
