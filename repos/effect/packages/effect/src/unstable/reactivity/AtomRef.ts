 ...
 *
 * `AtomRef` provides small observable state cells that can be read, updated, mapped, and subscribed
 * to without going through an `AtomRegistry`. Mutable refs can also create refs for nested
 * properties. The module also provides a collection helper that stores item refs and notifies
 * subscribers when items are inserted, removed, or changed.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It exposes a stable key, the current value, subscriptions to value changes, and `map` for
 * creating derived read-only references. Equality and hashing are based on the current value.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * It supports replacing the whole value, updating it from the current value, and creating mutable
 * references to nested properties.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The collection can push, insert, and remove item refs, and `toArray` returns the current raw item
 * values.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Each item is wrapped in an `AtomRef`, and changes to item refs notify the collection subscribers.
 *
 * @since 4.0.0
 * @category Constructors
 */
