 ...
 *
 * This module does not cache values itself. It lets callers register handlers for keys, invalidate
 * those keys, wrap successful mutations so they invalidate keys, and expose effects as queues or
 * streams that rerun when matching keys change. The service can also batch invalidations so
 * handlers run after the batch completes.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to provide the invalidation service that refreshes queries, streams, and atoms when
 * application keys change.
 *
 * **Details**
 *
 * The service can register handlers for keys, invalidate those keys, wrap mutations so successful
 * effects invalidate keys, and turn query effects into queues or streams that rerun when keys are
 * invalidated.
 *
 * @since 4.0.0
 * @category Services
 ...
 * **Details**
 *
 * The service tracks handlers by hashed keys and runs the registered handlers when matching keys
 * are invalidated.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The returned queue receives the initial result and each later result after the keys are
 * invalidated. The registration is removed when the current scope closes.
 *
 * @since 4.0.0
 * @category Accessors
 */
 ...
 * **Details**
 *
 * Registered queries for matching keys are rerun immediately, or collected until the enclosing
 * reactivity batch completes.
 *
 * @since 4.0.0
 * @category Accessors
 */
