 ...
 *
 * Server-Sent Events, or SSE, are the text format used by `EventSource` for one-way
 * server-to-client updates. This module includes parsers, encoders, channel helpers, and
 * schema-based helpers for the `id`, `event`, and `data` fields of each event.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * SSE `retry` directives are emitted as `Retry` failures so callers can reconnect with the
 * requested delay.
 *
 * @since 4.0.0
 * @category Decoding
 ...
 * **Details**
 *
 * The output preserves the SSE `event` name and optional `id` while replacing `data` with the
 * decoded value.
 *
 * @since 4.0.0
 * @category Decoding
 ...
 * **Details**
 *
 * Call `feed` with text chunks to parse `Event` and `Retry` values through the callback, and call
 * `reset` to clear any buffered event state.
 *
 * @since 4.0.0
 * @category Decoding
 */
 ...
 * **Details**
 *
 * If the upstream channel fails with `Retry`, the retry directive is written and the encoder
 * completes.
 *
 * @since 4.0.0
 * @category Encoding
 ...
 * **Details**
 *
 * Values are schema-encoded to the untagged SSE event shape, transformed to `Event`, and then
 * written as Server-Sent Events text.
 *
 * @since 4.0.0
 * @category Encoding
 */
...
/**
 * Tagged model for a Server-Sent Events message containing the event name, optional event ID, and
 * string data payload.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Schema for the untagged Server-Sent Events payload shape containing an optional `id`, `event`,
 * and string `data` fields.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Schema for the tagged Server-Sent Events message model that adds `_tag: "Event"` to the event
 * name, optional event ID, and string data payload.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Untagged Server-Sent Events payload shape containing the event name, optional event ID, and
 * string data payload.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Decoders surface this value as a failure to request reconnection after `duration`; encoders
 * serialize an upstream `Retry` failure as a `retry:` line.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * It renders `Event` values as `id`, `event`, and `data` lines and renders `Retry` values as
 * `retry:` directives.
 *
 * @since 4.0.0
 * @category Encoding
