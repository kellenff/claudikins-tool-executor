 ...
 *
 * MessagePack is a compact binary serialization format for protocols and storage layers that expect
 * bytes instead of JSON text, such as RPC transports, socket streams, caches, or database columns.
 * This module includes raw channel helpers for values whose shape is already agreed on, and
 ...
 * **Details**
 *
 * The `kind` field identifies whether the failure happened while packing or unpacking, and `cause`
...
/**
 * Creates a channel that encodes non-empty chunks of values as MessagePack byte arrays.
 *
 * **Details**
 *
      ...
      return Effect.flatMap(upstream, (chunk) => {
        try {
          return Effect.succeed(
            Arr.map(chunk, (item) => packr.pack(item) as Uint8Array<ArrayBuffer>),
          );
        } catch (cause) {
 ...
 * **Details**
 *
 * Values are first encoded with the schema and then packed as MessagePack bytes, so the channel can
 * fail with either schema errors or `MsgPackError`.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * Incomplete frames are buffered across chunks, and invalid MessagePack data fails with
 * `MsgPackError`.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * The channel unpacks bytes into unknown values and then decodes each value with the schema.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * Outgoing values are packed as MessagePack bytes before reaching the wrapped channel, and incoming
 * bytes are unpacked into values.
 *
 * @since 4.0.0
...
/**
 * Wraps a bidirectional byte channel with schema-aware MessagePack encoding and decoding.
 *
 * **Details**
 *
 * Values sent to the wrapped channel are encoded with `inputSchema` and packed as MessagePack
 * bytes; bytes received from it are unpacked and decoded with `outputSchema`.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **Details**
 *
 * It decodes a `Uint8Array` MessagePack payload to the target schema type and encodes the target
 * type back to bytes.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Schema for decoding MessagePack bytes into values and encoding values back to MessagePack bytes.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * The resulting schema decodes `Uint8Array` payloads with MessagePack and the provided schema, and
 * encodes values back to MessagePack bytes.
 *
 * @since 4.0.0
 * @category Schemas
 */
