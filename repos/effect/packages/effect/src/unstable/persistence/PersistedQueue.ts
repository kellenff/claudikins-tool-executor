 ...
 *
 * A `PersistedQueue<A>` keeps JSON-encoded values in a named queue and lets workers take one value
 * at a time inside a scoped processing window. It is useful for durable handoffs, background jobs,
 * outbox-style integrations, and work that should retry across fibers, process restarts, or
 * multiple workers. This module includes a queue factory, store service, id-based de-duplication,
 ...
 * **Details**
 *
 * `offer` enqueues values by id, and `take` processes one value at a time, marking it complete on
 * success or retrying it until the maximum attempts is reached.
 *
 * @since 4.0.0
 * @category Models
 */
export interface PersistedQueue<in out A, out R = never> {
   ...
   * **Details**
   *
   * If an element with the same id already exists in the queue, it will not be added again.
   */
  readonly offer: (
    value: A,
    options?: {
  ...
  /**
   * Takes an element from the queue, waiting until one is available when the queue is empty.
   *
   * **Details**
   *
   * If the returned effect succeeds, the element is marked as processed; otherwise it will be
   * retried according to the provided options. By default, max attempts is set to 10.
   */
 ...
 * **Details**
 *
 * Values are encoded and decoded with the supplied schema, automatically assigned an id when
 * needed, and acknowledged or retried according to the `take` handler's exit.
 *
 * @since 4.0.0
...
export const makeFactory = Effect.gen(function* () {
  ...
  return PersistedQueueFactory.of({
    make<S extends Schema.Constraint>(options: { readonly name: string; readonly schema: S }) {
 ...
 * **When to use**
 *
 * Use to provide the persistence backend that stores queued elements, scoped takes, retry attempts,
 * and acknowledgements.
 *
 * **Details**
 *
 * The store persists offered elements and returns taken elements in a scope so the finalizer can
 * complete or retry them based on the processing exit.
 *
 * @since 4.0.0
 * @category Store
 */
 ...
 * **Details**
 *
 * The store is process-local and volatile; failed takes are requeued until the configured maximum
 * attempts is reached.
 *
 * @since 4.0.0
 * @category Store
 ...
 * **Details**
 *
 * The store uses Redis lists and hashes with worker locks, periodically refreshes locks while items
 * are being processed, and moves exhausted items to a failed queue.
 *
 * @since 4.0.0
 * @category Store
 */
  ...
  const queues = yield* RcMap.make({
    lookup: Effect.fnUntraced(function* (name: string) {
      ...
      const poll = (size: number) =>
        take(queueKey, pendingKey, prefix, workerId, size, lockExpirationMillis);
...
const requeueRedis = Redis.script(
  (...args: [keyQueue: string, keyPending: string, keyLock: string, id: string, payload: string]) =>
    args,
  {
    lua: `
local key_queue = KEYS[1]
...
const failedRedis = Redis.script(
  (
    ...args: [keyPending: string, keyLock: string, keyFailed: string, id: string, payload: string]
  ) => args,
  {
    lua: `
local key_pending = KEYS[1]
 ...
 * **Details**
 *
 * The store creates the queue table and indexes, acquires rows with per-worker locks, refreshes
 * active locks while scoped takes are running, and retries or completes rows according to the
 * processing exit.
 *
 * @since 4.0.0
 * @category Store
