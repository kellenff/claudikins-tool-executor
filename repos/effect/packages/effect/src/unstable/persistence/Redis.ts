 ...
 *
 * This module defines a `Redis` service that can send Redis commands and run Lua scripts. It does
 * not create a Redis client itself; callers provide a `send` function from their client or
 * connection pool. The module also provides helpers for describing Lua scripts, loading them once,
 * and running them later by their cached Redis id.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Lua scripts are loaded through `SCRIPT LOAD`, cached, and then invoked with `EVALSHA`.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * It defines the Lua source, parameter-to-argument mapping, Redis key count, and result type used
 * by `Redis.eval`.
 *
 * @since 4.0.0
 * @category Scripting
 ...
 * **Details**
 *
 * The result type defaults to `void` and can be refined with `withReturnType`.
 *
 * @since 4.0.0
 * @category Scripting
 */
