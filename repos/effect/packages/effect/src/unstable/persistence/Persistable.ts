 ...
 *
 * A `Persistable` request has a primary key and schemas for its success and error results.
 * `Persistence` and `PersistedCache` use that information to store the request's `Exit` value and
 * restore it later from a backing store.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to implement persistable request values by attaching success and error schemas at this
 * property key.
 *
 * @since 4.0.0
 * @category Symbols
 ...
 * @category Models
 */
export interface Persistable<A extends Schema.Constraint, E extends Schema.Constraint>
...
/**
 * All schema services required to encode and decode a persistable request result.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The generated class stores the supplied tag, derives its primary key from the payload, and
 * carries schemas for persisted success and error exits.
 *
 * @since 4.0.0
...
/**
 * Returns the cached `Exit` schema for a persistable request's success and error schemas.
 *
 * @since 4.0.0
 * @category Accessors
 */
...
/**
 * Decodes a persisted value into an `Exit` for a persistable request using its success and error
 * schemas.
 *
 * @since 4.0.0
 * @category Serialization
