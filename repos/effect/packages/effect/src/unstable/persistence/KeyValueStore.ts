 ...
 *
 * `KeyValueStore` is a service for storing string or binary values by key. It is useful for
 * lightweight durable state, browser storage, local files, SQL tables, tests, and as a storage
 * building block for higher-level persistence APIs. This module includes store operations, prefixed
 * views, schema-aware JSON storage, error values, and layers for memory, filesystem, Web Storage,
 * and SQL-backed stores.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Primitive operations are required, while helpers such as `has`, `isEmpty`, and `modify` can be
 * supplied to override the defaults.
 *
 * @since 4.0.0
 * @category Options
 */
...
/**
 * Error raised by key/value store operations, including the failed method, optional key, message,
 * and cause.
 *
 * @since 4.0.0
 * @category Errors
 ...
 * **Details**
 *
 * Default implementations are derived for `has`, `isEmpty`, `modify`, and `modifyUint8Array` unless
 * they are provided in the options.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * `Uint8Array` values are stored as base64 strings. `getUint8Array` decodes base64 values and falls
 * back to UTF-8 encoding for non-base64 strings.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Returns a view of a `KeyValueStore` that prepends the given prefix to every key.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The layer creates the configured table if it does not exist and stores both string and binary
 * values through the current `SqlClient`.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Provides a `KeyValueStore` backed by a Web `Storage` instance such as `localStorage` or
 * `sessionStorage`.
 *
 * **Details**
 *
