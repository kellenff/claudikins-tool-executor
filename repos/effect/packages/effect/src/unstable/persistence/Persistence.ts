 ...
 *
 * The `Persistence` service creates scoped stores keyed by each request's `PrimaryKey`. Stores read
 * and write schema-encoded `Exit` values with optional TTLs, letting request workflows reuse
 * expensive or idempotent results across fibers, process restarts, or workers that share a backing
 * store.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The layer serializes and deserializes `Persistable` exits, applies per-entry TTLs, and skips
 * writes whose TTL is zero or negative.
 *
 * @since 4.0.0
 * @category Layers
 */
 ...
 * **Details**
 *
 * Each table is created if needed and stores JSON-encoded values with optional expiration
 * timestamps.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **Details**
 *
 * Rows are partitioned by `store_id` and store JSON-encoded values with optional expiration
 * timestamps.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **Details**
 *
 * Each store id is used as a key prefix, values are JSON-encoded, and finite TTLs are stored with
 * Redis expiration.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **Details**
 *
 * Each store id becomes a key prefix, and values are stored as JSON with optional expiration
 * timestamps.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **Details**
 *
 * Returns `null` for no TTL and uses `clock.currentTimeMillisUnsafe`, so it is intended for
 * backing-store internals.
 *
 * @since 4.0.0
 * @category Converting
 */
