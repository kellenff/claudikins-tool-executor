 ...
 *
 * A `PersistedCache` checks a process-local `Cache`, then a named `Persistence` store, before
 * running the supplied lookup. It stores the lookup `Exit`, so expensive or idempotent results can
 * be reused across fibers, process restarts, or workers that share the same backing store.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The cache reads persisted exits before running the lookup, stores lookup exits with the
 * configured persistent TTL, and also keeps a scoped in-memory cache with its own capacity and
 * TTL.
 *
 * @since 4.0.0
