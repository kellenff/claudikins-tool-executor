 ...
 *
 * SQL drivers often expose large query results through cursors, event emitters, or driver-specific
 * streams that push rows as they arrive. This module provides the small interop layer used by SQL
 * integrations to turn those producers into `Stream` values for `Statement.stream` and
 * `Connection.executeStream`, so callers can process large result sets incrementally instead of
...
/**
 * Creates a stream from a callback-style producer with pause and resume callbacks that are
 * triggered when the internal queue applies backpressure.
 *
 * @since 4.0.0
