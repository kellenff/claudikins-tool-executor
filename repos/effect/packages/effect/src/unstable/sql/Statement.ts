 ...
 *
 * `SqlClient` uses this module to build executable, parameterized SQL from reusable fragments. A
 * statement can be executed, streamed, run without row transformation, or compiled to SQL text and
 * parameters for a specific dialect. The module also contains helpers for identifiers, parameters,
 * inserts, updates, custom dialect fragments, statement compilation, and row transformation.
 *
 * @since 4.0.0
 */
...
/**
 * Composable SQL fragment represented as low-level segments that can be interpolated into
 * statements.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Executable SQL statement that is also a `Fragment` and `Effect`, with helpers for raw execution,
...
/**
 * Hook that can rewrite or wrap a `Statement` before execution, using the current SQL constructor,
 * fiber, and tracing span.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Context reference for an optional current SQL statement transformer applied before statement
 * execution.
 *
 * @since 4.0.0
...
/**
 * Raw SQL literal segment. The literal text is inserted directly into the compiled SQL, while
 * optional `params` are appended as bind parameters.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Constructs a raw SQL literal segment. The literal text is not escaped, so use bound parameters
...
/**
 * Helper segment for compiling an array of values, commonly used to produce placeholder lists for
 * `IN` clauses.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Helper segment for compiling one or more record objects into an INSERT column/value clause, with
...
/**
 * Helper segment for compiling multi-row update values with a table alias and optional returning
 * output.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Helper segment for compiling a single record into update assignments, omitting selected columns
 * and optionally returning output.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Constructs a `RecordUpdateHelperSingle` from a record and a list of columns to omit from the
 * update.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * SQL tagged-template constructor and helper API for building parameterized statements, escaped
 * identifiers, fragments, record helpers, and dialect-specific branches. Raw helpers such as
 * `unsafe` and `literal` insert SQL text directly.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a cached SQL statement constructor from a connection acquirer, compiler, tracing
 * attributes, and optional row transformation function.
 *
 * @since 4.0.0
 * @category Constructors
 */
export const make = (
  ...
  const cache =
    transformRows === undefined ? constructorCache.noTransforms : constructorCache.transforms;
...
/**
 * Builds a `Statement` from template strings and arguments, preserving fragments and helper
 * segments while converting ordinary interpolated values into bound parameters.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Creates a helper that joins SQL clauses with a literal separator, optionally wrapping multiple
 * clauses in parentheses and using a fallback for an empty list.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Combines clauses with `AND`, parenthesizing multiple clauses and returning `1=1` when the list is
 * empty.
 *
 * @since 4.0.0
 * @category Constructors
 */
export const and: (clauses: ReadonlyArray<string | Fragment>) => Fragment = join(
  " AND ",
  true,
  "1=1",
);
...
/**
 * Combines clauses with `OR`, parenthesizing multiple clauses and returning `1=1` when the list is
 * empty.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Creates a comma-separated SQL fragment from values, optionally adding a prefix, and returns an
 * empty fragment when no values are provided.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Dialect-specific compiler that converts a SQL `Fragment` into SQL text and bind parameters, with
 * a no-transform variant.
 *
 * @since 4.0.0
 * @category Compiler
 */
...
/**
 * Callbacks used by `makeCompiler` to render dialect placeholders, identifiers, insert helpers,
 * update helpers, and custom SQL segments.
 *
 * @since 4.0.0
 * @category Compiler
 */
...
const CompilerProto = {
  compile(
    ...
    const placeholder =
      placeholderOverride ?? ((u: unknown) => opts.placeholder(++placeholderCount, u));
...
/**
 * Creates a SQLite compiler that uses `?` placeholders and quoted identifiers, optionally
 * transforming identifier names before escaping.
 *
 * @since 4.0.0
 * @category Compiler
 */
...
/**
 * Creates an identifier escaping function that wraps names in the given delimiter, doubles
 * delimiter characters, and escapes dots between identifier parts.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Classifies a JavaScript value as a SQL primitive kind, treating `undefined` as `null` and
 * defaulting unrecognized objects to `string`.
 *
 * @since 4.0.0
 * @category Predicates
 */
...
/**
 * Builds value, object, and row-array transformers that rename object keys with the supplied
 * function and optionally recurse into nested object arrays.
 *
 * @since 4.0.0
...
> = {
  ...
  get valuesUnprepared(): Effect.Effect<ReadonlyArray<ReadonlyArray<unknown>>, SqlError> {
    return this.withConnection("executeValuesUnprepared", (connection, sql, params) =>
      connection.executeValuesUnprepared(sql, params),
    );
  },
