 ...
 *
 * `SqlError` wraps the different reasons a SQL operation can fail, such as connection,
 * authentication, authorization, syntax, constraint, or transaction problems. Each reason keeps the
 * original cause, optional message and operation metadata, and whether retrying may succeed. This
 * module also includes schemas, guards, a SQLite error classifier, and the `ResultLengthMismatch`
 * error used by ordered batched SQL resolvers.
 *
 * @since 4.0.0
 */
...
/**
 * SQL error reason for a unique constraint violation, including the violated constraint identifier;
...
/**
 * Union of structured SQL error reasons, each carrying the original cause plus optional message and
 * operation metadata.
 *
 * @since 4.0.0
 * @category Errors
...
/**
 * Error wrapper for SQL failures whose `message`, `cause`, and `isRetryable` values are derived
 * from its `SqlErrorReason`.
 *
 * @since 4.0.0
 * @category Errors
 */
...
/**
 * Classifies a native SQLite error cause into a `SqlErrorReason` using its `code` or `errno`, with
...
/**
 * Error raised when an ordered batched SQL resolver receives a different number of result rows than
 * requests.
 *
 * @since 4.0.0
 * @category Errors
