 ...
 *
 * A migrator loads numbered migration effects, records completed ids in a migrations table, and
 * runs only pending migrations in a transaction. It creates the table when needed, detects
 * duplicate ids, treats concurrent runs as locked, and can dump the schema after successful
 * migrations.
 *
 * @since 4.0.0
 */
...
/**
 * Options for running SQL migrations, including the migration loader, optional schema dump
 * directory, and migrations table name.
 *
 * @since 4.0.0
 * @category Options
 */
...
/**
 * Tuple produced by a migration loader, containing the migration id, migration name, and an effect
 * that loads the migration implementation.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Metadata for a migration recorded in the migrations table, including its id, name, and creation
 * timestamp.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Creates a migrator that ensures the migrations table exists, runs pending migrations in a
 * transaction, and optionally dumps the schema after successful migrations.
 *
 * @since 4.0.0
...
/**
 * Creates a migration loader from a glob record of dynamic import functions, parsing files named
 * `<id>_<name>.js`, `<id>_<name>.ts`, `<id>_<name>.mjs`, or `<id>_<name>.mts` and sorting
 * migrations by id.
 *
 * @since 4.0.0
 * @category Loaders
...
/**
 * Creates a migration loader from a Babel-style glob record, parsing keys such as `_<id>_<name>Js`,
 * `_<id>_<name>Ts`, `_<id>_<name>Mjs`, or `_<id>_<name>Mts` and sorting migrations by id.
 *
 * @since 4.0.0
 * @category Loaders
 */
export const fromBabelGlob = (migrations: Record<string, any>): Loader =>
  pipe(
    ...
    Arr.map(
      ([key, id, name]): ResolvedMigration => [Number(id), name, Effect.succeed(migrations[key])],
    ),
    Arr.sort(migrationOrder),
    Effect.succeed,
  );
...
/**
 * Creates a migration loader from a record of migration effects keyed by `<id>_<name>`, sorted by
 * migration id.
 *
 * @since 4.0.0
 * @category Loaders
...
/**
 * Creates a migration loader that reads a directory with `FileSystem`, imports files named
 * `<id>_<name>.js`, `<id>_<name>.ts`, `<id>_<name>.mjs`, or `<id>_<name>.mts`, and sorts migrations
 * by id.
 *
 * @since 4.0.0
 * @category Loaders
 */
export const fromFileSystem: (directory: string) => Loader<FileSystem> = Effect.fnUntraced(
  function* (directory) {
