 ...
 *
 * This module represents each lookup or mutation as a `SqlRequest` and batches concurrent requests
 * into SQL operations. Request payloads are encoded with the request schema before `execute` is
 * called, and returned rows are decoded with the result schema before requests are completed. It
 * provides ordered, grouped, id-based, and side-effect-only resolver constructors, and keeps
...
/**
 * Request type used by SQL request resolvers, carrying the input payload together with the
 * resolver's result, error, and environment types.
 *
 * @since 4.0.0
 * @category Requests
 */
...
/**
 * Runs a payload as a `SqlRequest` through a request resolver, either directly with a payload and
 * resolver or curried by resolver.
 *
 * @since 4.0.0
 * @category Requests
 */
...
/**
 * Constructs a `SqlRequest` from a payload. Equality and hashing are based on the payload so equal
 * requests can be batched and deduplicated.
 *
 * @since 4.0.0
 * @category Requests
 */
 ...
 * **Details**
 *
 * The request schema is used to validate the input of the query, and the result schema is used to
 * validate the output of the query. Results are mapped to the requests in order, so the length of
 * the results must match the length of the requests.
 *
 * @since 4.0.0
 * @category Resolvers
 */
export const ordered = <
  Req extends Schema.Constraint,
...
/**
 * Creates a batched SQL request resolver that encodes requests, decodes result rows, groups decoded
 * results by matching request and result keys, and fails a request with `NoSuchElementError` when
 * no result group exists.
 *
 * @since 4.0.0
 * @category Resolvers
 */
...
/**
 * Creates a batched resolver that fetches rows for encoded ids, decodes results, completes each
 * matching request using `ResultId`, and fails missing ids with `NoSuchElementError`.
 *
 * @since 4.0.0
 * @category Resolvers
 */
...
const void_ = <Req extends Schema.Constraint, _, E, R>(options: {
  readonly Request: Req;
  readonly execute: (
    requests: Arr.NonEmptyArray<Req["Encoded"]>,
  ) => Effect.Effect<ReadonlyArray<_>, E, R>;
