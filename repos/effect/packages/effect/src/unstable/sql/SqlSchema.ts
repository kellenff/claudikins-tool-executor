 ...
 *
 * `SqlSchema` is a small adapter between Effect Schema and SQL statements. Each helper builds a
 * function that accepts the decoded request type used by application code, encodes it before
 * calling `execute`, and decodes unknown driver rows into the result schema. The helpers cover
 * returning all rows, a non-empty row list, the first row, an optional first row, or discarding the
...
/**
 * Builds a query function that encodes the request and decodes all result rows, allowing an empty
 * result set.
 *
 * **When to use**
 *
 * Use when you need to run a query that may return zero or more rows and represent an empty result
 * as an empty array.
 *
 * @since 4.0.0
...
/**
 * Builds a query function that encodes the request, decodes all result rows, and fails with
 * `NoSuchElementError` when the result set is empty.
 *
 * **When to use**
 *
 * Use when you need to run a query that must return at least one row and treat an empty result as a
 * failure.
 *
 * @since 4.0.0
 * @category Constructors
...
const void_ = <Req extends Schema.Constraint, E, R>(options: {
  readonly Request: Req;
...
/**
 * Builds a query function that encodes the request, decodes the first result row, and fails with
 * `NoSuchElementError` when no rows are returned.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Builds a query function that encodes the request, decodes the first result row as `Option.some`,
 * and returns `Option.none` when no rows are returned.
 *
 * @since 4.0.0
 * @category Constructors
 */
