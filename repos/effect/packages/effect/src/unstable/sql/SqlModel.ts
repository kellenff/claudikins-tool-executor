 ...
 *
 * Use this module when a schema `Model` represents rows in a SQL table and the usual insert,
 * update, find-by-id, delete, and batching behavior should be derived from that model. The helpers
 * encode insert and update input with the model's input schemas and decode returned rows with the
 * full model schema. Soft deletes are optional, and SQL dialect differences such as `returning`
...
/**
 * Creates a CRUD repository for a schema model backed by a SQL table, with insert, update,
 * find-by-id, and delete operations. When `softDeleteColumn` is supplied, reads ignore soft-deleted
 * rows and delete updates that column instead of removing the row.
 *
 * @since 4.0.0
 * @category Repository
 */
...
/**
 * Creates batched request resolvers for a schema model's insert, insert-void, find-by-id, and
 * delete operations, honoring the optional soft-delete column.
 *
 * @since 4.0.0
 * @category Repository
 */
