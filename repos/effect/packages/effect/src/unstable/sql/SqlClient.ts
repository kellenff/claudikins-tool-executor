 ...
 *
 * `SqlClient` combines the tagged-template statement constructor with connection acquisition,
 * dialect compilation, transactions, row transforms, tracing, and reactive query helpers. Driver
 * integrations build this service from their connection and compiler pieces.
 *
 * @since 4.0.0
 */
...
/**
 * SQL client service interface, combining the statement constructor API with connection
 * reservation, transaction handling, and reactive query helpers.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use to access or provide the SQL client used to build statements, stream rows, reserve
 * connections, and run transactions.
 *
 * @since 4.0.0
 * @category Services
 */
...
export declare namespace SqlClient {
  /**
   * Options used to construct a `SqlClient`, including connection acquirers, the SQL compiler,
   * transaction SQL, row transformation, tracing attributes, and optional reactive query
   * integration.
   *
   * @since 4.0.0
   * @category Options
...
/**
 * Constructs a `SqlClient` from connection acquirers, a compiler, transaction commands, tracing
 * attributes, optional row transforms, and reactive query integration.
 *
 * @since 4.0.0
...
export const make = Effect.fnUntraced(function* (options: SqlClient.MakeOptions) {
  ...
  const rollbackSavepoint =
    options.rollbackSavepoint ?? ((name: string) => `ROLLBACK TO SAVEPOINT ${name}`);
  ...
  const withTransaction = makeWithTransaction({
    ...
    rollbackSavepoint: (conn, id) =>
      conn.executeUnprepared(rollbackSavepoint(`effect_sql_${id}`), [], undefined),
  });
...
/**
 * Builds a transaction wrapper that begins top-level transactions, uses savepoints for nested
 * transactions, commits on success, and rolls back on failure or interruption.
 *
 * @since 4.0.0
            ...
            (id === 0 ? options.begin(conn) : options.savepoint(conn, id)).pipe(
              ...
              Effect.flatMap((exit) => {
                ...
                if (Exit.isSuccess(exit)) {
                  ...
                  effect = Effect.orDie(
                    id > 0 ? options.rollbackSavepoint(conn, id) : options.rollback(conn),
                  );
                }
                const withScope =
                  scope !== undefined ? Effect.ensuring(effect, Scope.close(scope, exit)) : effect;
...
export declare namespace TransactionConnection {
  /**
   * Service payload stored during a transaction, containing the active connection and nested
   * transaction depth.
   *
   * @since 4.0.0
   * @category Services
...
/**
 * Creates a unique context service tag for the active transaction connection of a specific SQL
 * client.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Context reference used by SQL integrations to opt in to safe integer handling; defaults to
 * `false`.
 *
 * @since 4.0.0
 * @category References
