 ...
 *
 * The `LanguageModel` service lets application code ask for generated text, streamed text, or
 * structured output without depending on a specific provider. Requests can include tools, and the
 * service can resolve tool calls while the model is generating a response. This module contains the
 * service contract, request and response types, structured-output support, and the constructor used
 * by provider packages to adapt their own generate and stream functions to the shared interface.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to access or provide text generation, streaming generation, structured output, and
 * tool-calling capabilities through the Effect context.
 *
...
/**
 * The service interface for language model operations, defining the contract that all language
 * model implementations must fulfill.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * A function that transforms a `Schema.Codec` into a provider-compatible form for structured output
 * generation.
 *
 * **Details**
 *
 * Different language model providers have varying constraints on the JSON schemas they accept. A
 * `CodecTransformer` rewrites a codec's encoded side to satisfy those constraints while preserving
 * the decoded type.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use as the codec transformer for provider implementations when the provider accepts the JSON
 * Schema generated from an `Effect` Schema codec without provider-specific rewrites.
 *
 * **Details**
 *
 * The transformer returns the original codec, resolves a top-level `$ref`, and copies schema
...
export interface GenerateTextOptions<Tools extends Record<string, Tool.Any>> {
  ...
  /**
   * A toolkit containing both the tools and the tool call handler to use to augment text
   * generation.
   */
   ...
   *
   * - `auto` (default): The model can decide whether or not to call tools, as well as which tools to
   *   call.
   ...
   * - `{ mode?: "auto" (default) | "required", "oneOf": [<tool-names>] }`: The model is restricted to
   *   the subset of tools specified by `oneOf`. When `mode` is `"auto"` or omitted, the model can
   *   decide whether or not a tool from the allowed subset of tools can be called. When `mode` is
   *   `"required"`, the model **must** call one tool from the allowed subset of tools.
   */
  readonly toolChoice?:
  ...
  /**
   * When set to `true`, tool calls requested by the large language model are not auto-resolved by
   * the framework.
   *
   * **When to use**
   *
   * Use when you want to include tool call definitions from an `AiToolkit` in requests to the large
   * language model, while controlling tool call resolver execution yourself.
   */
...
> extends GenerateTextOptions<Tools> {
  /**
   * The name of the structured output that should be generated. Used by some large language model
   * providers to provide additional guidance to the model.
   */
...
/**
 * Response class for text generation operations, with accessors for extracting text, tool calls,
 * usage information, and other response parts from generated content.
 *
 ...
 * **Details**
 *
 * Unlike `ToolkitOption`, this type does not distribute over unions. It is intended for call-site
 * assignability, while `ToolkitOption` remains the distributive helper used for extraction and
 * inference.
 *
 * @since 4.0.0
 * @category Utility types
 ...
 * **Details**
 *
 * Automatically infers the possible error types based on toolkit configuration and tool call
 * resolution settings.
 *
 * @since 4.0.0
 * @category Utility types
 ...
 * **Details**
 *
 * This interface defines the normalized options that are passed to the underlying provider
 * implementation, regardless of the specific provider being used.
 *
 * @since 4.0.0
 * @category Options
 */
export interface ProviderOptions {
  ...
  /**
   * The tools that the large language model will have available to provide additional information
   * which can be incorporated into its text generation.
   */
   ...
   * **Details**
   *
   * If `"text"` is specified, the large language model response is returned as text. If `"json"` is
   * specified, the large language model response is provided as a JSON object that conforms to the
   * shape of the specified schema. The default is `{ type: "text" }`.
   */
...
/**
 * Creates a LanguageModel service from provider-specific text generation and streaming
 * implementations.
 *
 * **When to use**
 *
 * Use when you are implementing a provider adapter and need to expose the standard language-model
 * service while keeping provider-specific request hooks behind it.
 *
 * **Details**
 *
 * The returned service implements `generateText`, `generateObject`, and `streamText`. It prepares
 * `ProviderOptions` for each request, including the normalized prompt, tools, tool choice, response
 * format, tracing span, and incremental response fields, before calling the supplied provider hook.
 * Structured object generation uses the `generateText` hook and the configured `codecTransformer`,
 * or `defaultCodecTransformer` when none is supplied.
 *
 * **Gotchas**
 *
 * Provider hooks must return encoded response parts that match the toolkit and response format
 * prepared in `ProviderOptions`; invalid parts fail decoding as `AiError.InvalidOutputError`.
 *
 * @since 4.0.0
...
export const make: (params: {
  /**
   * A method that requests text generation from the large language model provider and returns the
   * final result when generation finishes.
   */
  readonly generateText: (
  ...
  /**
   * A method that requests text generation from the large language model provider and streams
  ...
  /**
   * A function that transforms a `Schema.Codec` into a provider-compatible form for structured
   * output generation.
   */
  ...
  >(options: Options & GenerateTextOptions<Tools>, providerOptions: Mutable<ProviderOptions>) {
    ...
    yield* streamWithNonIncrementalFallback().pipe(
      ...
      // - fail (FiberSet.join)
      Effect.andThen(
        Effect.raceFirst(FiberSet.join(toolCallFibers), FiberSet.awaitEmpty(toolCallFibers)),
      ),
 ...
 * **Details**
 *
 * Returns a stream of response parts that are emitted as soon as they are available from the model,
 * enabling real-time text generation experiences.
 *
...
/**
 * Strip resolved approval artifacts from the prompt before sending to the provider. After
 * pre-resolving approvals (executing approved tools and creating denial results), the original
 * `tool-approval-request` parts in assistant messages and `tool-approval-response` parts in tool
 * messages are no longer needed. Leaving them in causes provider-specific errors (e.g. OpenAI
 * rejects `mcp_approval_response` items that reference approval requests it never issued).
 */
