 ...
 *
 * MCP clients and servers use these schemas to describe the JSON-RPC requests, notifications,
 * results, and errors that can cross the protocol boundary. This module focuses on message shapes:
 * it defines the shared protocol data model, groups related messages for the RPC layer, and
 * provides helpers for optional fields and parameter metadata. Transport and server behavior live
 * in other modules.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It represents an optional struct field that supplies a default value when the field is absent
 * during decoding or construction.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * @category Schemas
 */
export const ProgressToken: Schema.Union<[typeof Schema.String, typeof Schema.Number]> =
 ...
 * **Details**
 *
 * Request metadata may include a progress token that asks the receiver to send out-of-band progress
 ...
 * **Details**
 *
 * The `_meta` field is reserved for protocol, extension, or implementation metadata attached to a
 * result.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Details**
 *
 * It includes the base request metadata fields plus an optional cursor indicating where the server
 * should continue listing results.
 *
 * @since 4.0.0
 * @category Schemas
 */
export class PaginatedRequestMeta extends Schema.Opaque<PaginatedRequestMeta>()(
  Schema.Struct({
    ...RequestMeta.fields,
    /**
     * An opaque token representing the current pagination position. If provided, the server should
 ...
 * **Details**
 *
 * It includes the base result metadata fields plus an optional `nextCursor`, which indicates that
...
export class PaginatedResultMeta extends Schema.Opaque<PaginatedResultMeta>()(
  Schema.Struct({
    ...ResultMeta.fields,
    /**
     * An opaque token representing the pagination position after the last returned result. If
 ...
 * **When to use**
 *
 * Use to describe intended audience and priority metadata for objects shown or processed by a
 * client.
 *
 * @since 4.0.0
 * @category Schemas
...
export class Annotations extends Schema.Opaque<Annotations>()(
  Schema.Struct({
     ...
     *
     * A value of 1 means "most important," and indicates that the data is effectively required, while
     * 0 means "least important," and indicates that the data is entirely optional.
     */
 ...
 * **Details**
 *
 * Known capabilities are represented by this schema, but the capability set is open and clients may
...
export class ClientCapabilities extends Schema.Class<ClientCapabilities>(
  ...
  /**
   * Optional extensions capabilities advertised by the client. Keys are extension identifiers
   * following <vendor-prefix>/<extension-name> (e.g. "io.modelcontextprotocol/ui").
   */
  extensions: optional(
 ...
 * **Details**
 *
 * Known capabilities are represented by this schema, but the capability set is open and servers may
 * define additional capabilities.
 *
 * @since 4.0.0
 * @category Schemas
...
export class ServerCapabilities extends Schema.Opaque<ServerCapabilities>()(
  Schema.Struct({
    ...
    experimental: optional(Schema.Record(Schema.String, Schema.Struct({}))),
    /**
     * Optional extensions capabilities advertised by the server. Keys are extension identifiers
     * following <vendor-prefix>/<extension-name> (e.g. "io.modelcontextprotocol/ui").
     */
    extensions: optional(
      Schema.Record(Schema.TemplateLiteral([Schema.String, "/", Schema.String]), Schema.Json),
...
export class McpErrorBase extends Schema.Class<McpErrorBase>("@effect/ai/McpSchema/McpErrorBase")({
  ...
  /**
   * Additional information about the error. The value of this member is defined by the sender (e.g.
 ...
 * **When to use**
 *
 * Use when building an MCP/JSON-RPC error response for a syntactically parsed request object that
 * fails request-shape validation.
 *
 * @since 4.0.0
 * @category Constants
 */
 ...
 * **When to use**
 *
 * Use when building an MCP/JSON-RPC error response for a request whose `method` is unknown or
 * unavailable.
 *
 * @since 4.0.0
 * @category Constants
 ...
 * **When to use**
 *
 * Use when building an MCP/JSON-RPC error response for decoded request parameters that fail
 ...
 * **When to use**
 *
 * Use when building an MCP/JSON-RPC error response before a request object is available because the
...
/**
 * Schema for MCP protocol errors returned in JSON-RPC failure responses, including standard
 * protocol errors and custom `McpErrorBase` values.
 *
 * @since 4.0.0
 * @category Errors
 */
...
export class InitializeResult extends Schema.Opaque<InitializeResult>()(
  Schema.Struct({
    ...ResultMeta.fields,
    /**
     * The version of the Model Context Protocol that the server wants to use. This may not match the
     * version that the client requested. If the client cannot support this version, it MUST
     * disconnect.
     */
     ...
     * etc. It can be thought of like a "hint" to the model. For example, this information MAY be
...
export class ProgressNotification extends Rpc.make("notifications/progress", {
  payload: {
    ...
    /**
     * The progress thus far. This should increase every time progress is made, even if the total is
     * unknown.
     */
...
export class Resource extends Schema.Class<Resource>("@effect/ai/McpSchema/Resource")({
   ...
   *
   * This can be used by clients to improve the LLM's understanding of available resources. It can
...
export class TextResourceContents extends Schema.Opaque<TextResourceContents>()(
  Schema.Struct({
    ...
    /**
     * The text of the item. This must only be set if the item can actually be represented as text
     * (not binary data).
     */
    text: Schema.String,
  }),
...
export class ReadResource extends Rpc.make("resources/read", {
  ...
  payload: {
    ...RequestMeta.fields,
    /**
     * The URI of the resource to read. The URI can use any protocol; it is up to the server how to
     * interpret it.
     */
    uri: Schema.String,
  },
...
export class Subscribe extends Rpc.make("resources/subscribe", {
  ...
  payload: {
    ...RequestMeta.fields,
    /**
     * The URI of the resource to subscribe to. The URI can use any protocol; it is up to the server
     * how to interpret it.
     */
    uri: Schema.String,
  },
...
export class ResourceUpdatedNotification extends Rpc.make("notifications/resources/updated", {
  payload: {
    ...NotificationMeta.fields,
    /**
     * The URI of the resource that has been updated. This might be a sub-resource of the one that
 ...
 * **Details**
 *
 * It is up to the client how best to render embedded resources for the benefit of the LLM and/or
 * the user.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Gotchas**
 *
 * Resource links returned by tools are not guaranteed to appear in the results of `resources/list`
 * requests.
 *
 * @since 4.0.0
 * @category Schemas
...
export class Tool extends Schema.Class<Tool>("@effect/ai/McpSchema/Tool")({
   ...
   *
   * This can be used by clients to improve the LLM's understanding of available tools. It can be
 ...
 * not be able to see that an error occurred and self-correct. However, any errors in _finding_ the
 * tool, an error indicating that the server does not support tool calls, or any other exceptional
 ...
 * **When to use**
 *
 * Use when you need to represent a client request that already knows the tool name and asks the
 * server to execute it with argument values.
 *
 * @since 4.0.0
 * @category Tools
...
/**
 * Schema for log message severity levels, mapped to syslog message severities as specified in RFC
 * 5424 section 6.2.1: https://datatracker.ietf.org/doc/html/rfc5424#section-6.2.1.
 *
 * @since 4.0.0
 * @category Logging
 */
...
/**
 * Type represented by the MCP logging level schema, mapped to syslog message severities as
...
export class SetLevel extends Rpc.make("logging/setLevel", {
  payload: {
    ...RequestMeta.fields,
    /**
     * The level of logging that the client wants to receive from the server. The server should send
     * all logs at this level and higher (i.e., more severe) to the client as
 ...
 * **Details**
 *
 * The notification includes the severity level, optional logger name, and JSON-serializable log
 * data.
 *
 * @since 4.0.0
 * @category Logging
...
export class LoggingMessageNotification extends Rpc.make("notifications/message", {
  payload: Schema.Struct({
    ...
    /**
     * The data to be logged, such as a string message or an object. Any JSON serializable type is
     * allowed here.
     */
    data: Schema.Any,
  }),
 ...
 * **Details**
 *
 * Keys not declared here are currently left unspecified by the spec and are up to the client to
 * interpret.
 *
 * @since 4.0.0
 * @category Sampling
...
export class ModelHint extends Schema.Opaque<ModelHint>()(
  Schema.Struct({
     ...
     *
     * The client SHOULD treat this as a substring of a model name; for example: - `claude-3-5-sonnet`
     * should match `claude-3-5-sonnet-20241022` - `sonnet` should match `claude-3-5-sonnet-20241022`,
     * `claude-3-sonnet-20240229`, etc. - `claude` should match any Claude model
     *
     * The client MAY also map the string to a different provider's model name or a different model
     * family, as long as it fills a similar niche; for example: - `gemini-1.5-flash` could match
 ...
 * others are more capable but more expensive, and so on. This interface allows servers to express
 * their priorities across multiple dimensions to help clients make an appropriate selection for
 * their use case.
 *
 * **Gotchas**
 *
...
export class ModelPreferences extends Schema.Class<ModelPreferences>(
   ...
   *
   * If multiple hints are specified, the client MUST evaluate them in order (such that the first
   * match is taken).
   *
   * The client SHOULD prioritize these hints over the numeric priorities, but MAY still use the
   * priorities to select from ambiguous matches.
   */
  hints: optional(Schema.Array(ModelHint)),
  /**
   * How much to prioritize cost when selecting a model. A value of 0 means cost is not important,
  ...
  /**
   * How much to prioritize sampling speed (latency) when selecting a model. A value of 0 means
   * speed is not important, while a value of 1 means speed is the most important factor.
   */
  speedPriority: optional(Schema.Number.check(Schema.isBetween({ minimum: 0, maximum: 1 }))),
  /**
   * How much to prioritize intelligence and capabilities when selecting a model. A value of 0 means
   * intelligence is not important, while a value of 1 means intelligence is the most important
   * factor.
   */
 ...
 * **Details**
 *
 * The client chooses the model and should ask the user to approve the sampling request before it
 * begins.
 *
 * @since 4.0.0
 * @category Sampling
...
export class Root extends Schema.Class<Root>("@effect/ai/McpSchema/Root")({
  /**
   * The URI identifying the root. This _must_ start with file:// for now. This restriction may be
   * relaxed in future versions of the protocol to allow other URI schemes.
   */
  uri: Schema.String,
  /**
   * An optional name for the root. This can be used to provide a human-readable identifier for the
   * root, which may be useful for display purposes or for referencing the root in other parts of
   * the application.
   */
  name: optional(Schema.String),
}) {}
 ...
 * **Details**
 *
 * This request is typically used when the server needs to understand the file system structure or
 * access specific locations that the client has permission to read from.
 *
 * @since 4.0.0
 * @category Roots
 */
...
export class ElicitAcceptResult extends Schema.Class<ElicitAcceptResult>(
  ...
  ...ResultMeta.fields,
  /**
   * The user action in response to the elicitation. - "accept": User submitted the form/confirmed
   * the action - "decline": User explicitly declined the action - "cancel": User dismissed without
   * making an explicit choice
   */
  action: Schema.Literal("accept"),
  /**
   * The submitted form data, only present when action is "accept". Contains values matching the
   * requested schema.
   */
  content: Schema.Any,
}) {}
...
export class Elicit extends Rpc.make("elicitation/create", {
  ...
  error: McpError,
  payload: {
    /** A message to display to the user, explaining what they are being elicited for. */
    message: Schema.String,
...
/**
 * Error raised when an MCP elicitation request is declined or fails before accepted content is
 * returned.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * It exposes the current client id, the client's initialize payload, and a scoped RPC client for
...
/**
 * Encoded JSON-RPC request message for an RPC in `Group`, including the request id, method, and
 * encoded payload.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Encoded notification message for an RPC in `Group`, including the method and encoded payload
...
/**
 * Encoded success response for an RPC in `Group`, containing the original request id and encoded
 * result.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Encoded failure response for an RPC in `Group`, containing the original request id and encoded
 * error.
 *
 * @since 4.0.0
 * @category Protocols
 ...
 * **Details**
 *
 * The group includes initialization, resource, prompt, tool, logging, completion, and ping
 * requests, and installs `McpServerClientMiddleware` for handlers.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * RPC group for notifications that MCP clients send to the server, such as cancellation, progress,
 * initialization completion, and roots list changes.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * RPC group for requests that an MCP server can send to a client, including ping, sampling, roots
 * listing, and elicitation.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * RPC group for notifications that an MCP server can send to a client, including cancellation,
 * progress, logging, and list or resource update notifications.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * Encoded MCP messages accepted from a client by the server protocol: client requests and client
 * notifications.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Encoded MCP messages emitted by the server protocol to a client: server responses and server
 * notifications.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Returns `true` when a schema was created with `param` and therefore carries a resource URI
 ...
 * **Details**
 *
 * A `Param` behaves like the wrapped schema while carrying the parameter name used for template
 * compilation and completion lookup.
 *
 * @since 4.0.0
 * @category Parameters
 */
