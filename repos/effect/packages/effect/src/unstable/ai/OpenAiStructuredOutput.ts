 ...
 *
 * OpenAI structured output accepts only a subset of JSON Schema. This module converts an Effect
 * `Schema.Codec` into a provider-compatible JSON Schema and a matching codec for decoding the model
 * response back into the original application type. When possible, unsupported schema shapes are
 * rewritten into supported ones; schema kinds that cannot be represented safely fail during
 * conversion.
 *
 * @since 4.0.0
 */
...
/**
 * Converts a `Schema.Codec` to OpenAI structured-output JSON Schema and a matching codec for model
 * output.
 *
 * **When to use**
 *
 * Use when you send Effect Schema-backed structured output requests to OpenAI and need
 * provider-compatible JSON Schema without losing the decoded application type.
 *
 * **Details**
 *
 * Returns the JSON Schema to include in the request and the codec to use when decoding the model
 * response. If the input schema already fits OpenAI's supported JSON Schema subset, the original
 * codec is returned unchanged.
 *
 * **Gotchas**
 *
 * - Some schemas use a provider-safe encoded shape: tuples become objects with numeric string keys,
 *   records become arrays of `[key, value]` pairs, and optional properties become required nullable
 *   properties.
 * - `oneOf` unions are emitted as `anyOf` unions.
 * - Regex patterns from multiple filters are merged into one `pattern` because OpenAI structured
 *   output does not support `allOf`.
...
/**
 * Post-processes the JSON schema produced by `Schema.toJsonSchemaDocument`, recursively flattening
 * `allOf` arrays by merging each member's keys into the parent object. This is necessary because
            ...
            encode: (t) => {
              ...
              if (ast.rest.length === 1) {
                o[REST_PROPERTY_NAME] =
                  t.length >= ast.elements.length ? t.slice(ast.elements.length) : [];
              }
              return o;
            },
          }),
