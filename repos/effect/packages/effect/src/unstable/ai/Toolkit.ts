 ...
 *
 * A toolkit connects `Tool` schemas to the handler functions an application provides for a language
 * model workflow. It can build a handler context or layer and execute tool calls by name. Execution
 * validates parameters, runs the handler, encodes the result, supports preliminary streamed
 * results, and applies the tool's failure mode.
 *
 * @since 4.0.0
 */
...
/**
 * Represents a collection of tools which can be used to enhance the capabilities of a large
 * language model.
 *
...
export interface HandlerContext<Tool extends Tool.Any> {
   ...
   * **Details**
   *
   * Preliminary results are streamed to the caller before the handler completes, enabling real-time
   * progress updates for lengthy operations.
   */
...
/**
 * A utility type which transforms either a record or an array of tools into a record where keys are
 * tool names and values are the tool instances.
 *
 * @since 4.0.0
 * @category Utility types
 */
 ...
 * **Details**
 *
 * Handlers can return either the tool's custom failure type, an `AiErrorReason` (which will be
 * wrapped in `AiError`), or a full `AiError`.
 *
 * @since 4.0.0
 * @category Utility types
 */
...
export interface WithHandler<in out Tools extends Record<string, Tool.Any>> {
   ...
   * **Details**
   *
   * Validates the input parameters, executes the corresponding handler, and streams back both the
   * typed result and encoded result. Streaming allows handlers to emit preliminary results before
   * completion.
   */
      ...
      const handle = Effect.fnUntraced(function* (name: string, params: unknown) {
        ...
        const fiber = yield* schemas.handler(decodedParams, context).pipe(
          Effect.flatMap((result) =>
            Queue.offer(queue, { result, isFailure: false, preliminary: false }),
          ),
 ...
 * **When to use**
 *
 * Use when you need an empty starting point for building toolkits or a default toolkit value that
 * can be extended with `merge`.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * This is the primary constructor for creating toolkits. It accepts multiple tools and organizes
 * them into a toolkit that can be provided to AI language models.
 *
 ...
 * **Details**
 *
 * Combines all tools from the provided toolkits into one unified toolkit. If there are naming
 * conflicts, tools from later toolkits will override tools from earlier ones.
 *
