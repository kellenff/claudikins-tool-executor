 ...
 *
 * The main entry point returns the JSON Schema to send to Anthropic and a codec for decoding the
 * model response back into the original application type. When Anthropic cannot express the
 * original schema shape directly, the conversion rewrites supported cases such as tuples, records,
 * optional properties, and `oneOf` unions. Schema kinds that cannot be represented throw during
...
/**
 * Converts a `Schema.Codec` to Anthropic structured-output JSON Schema and a matching codec for
 * model output.
 *
 * **When to use**
 *
 * Use when you send Effect Schema-backed structured output requests to Anthropic and need
 ...
 * response. If the input schema already fits Anthropic's supported JSON Schema subset, the original
