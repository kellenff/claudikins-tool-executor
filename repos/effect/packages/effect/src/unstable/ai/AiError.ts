 ...
 *
 * `AiError` records where a failure happened and stores the detailed reason in a `reason` field.
 * Those reasons cover transport problems, provider responses, rate limits, authentication, content
 * policy failures, invalid requests, invalid output, unsupported schemas, tool failures, invalid
 * user input, and unknown failures. This module also includes metadata schemas, guards,
 * constructors, and helpers for converting HTTP response information into AI error reasons.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * This error is raised when issues arise before receiving an HTTP response, such as network
 * connectivity problems, request encoding issues, or invalid URLs.
 *
 ...
 * **Details**
 *
 * Provider-specific metadata is namespaced by provider name. Each provider value can contain
 * arbitrary mutable JSON metadata or `null`.
 *
 ...
 * **Details**
 *
 * Metadata is keyed by provider name, and each provider value is either mutable JSON metadata or
 * `null`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Schema for optional provider-reported token counts for prompt tokens, completion tokens, and
 * total tokens.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **When to use**
 *
 * Use to attach request details, optional response details, and optional body text to AI provider
 * errors.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * Rate limit errors are always retryable. When `retryAfter` is provided, callers should wait that
...
/**
 * Error indicating the LLM generated text that does not conform to the requested structured output
 * schema.
 *
 * **Details**
 *
...
/**
 * Error indicating a codec transformer rejected a schema because it contains unsupported
 * constructs.
 *
 * **Details**
 *
 * Unsupported schema errors are not retryable because they indicate a programmer error where the
 * schema is incompatible with the provider.
 *
 ...
 * **Details**
 *
 * This error is retryable because the model may self-correct when provided with the list of
 * available tools.
 *
...
/**
 * Error indicating the tool handler returned an invalid result that does not match the tool's
 * schema.
 *
 * **Details**
 *
 * This error is not retryable because invalid results indicate a bug in the tool handler
 * implementation.
 *
 ...
 * **Details**
 *
 * This error is not retryable because encoding failures indicate a bug in the tool schema
 * definitions.
 *
 ...
 * **Details**
 *
 * This error is not retryable because it indicates a programming error in the tool configuration
 ...
 * **Details**
 *
 * This error occurs when tool approval responses are present in the prompt but no toolkit was
 ...
 * **Details**
 *
 * This error is raised when the prompt contains content that is structurally valid but not
 * supported by the provider (e.g., unsupported media types, unsupported file formats, etc.).
 *
 ...
 *   description:
 *     "Unsupported media type 'video/mp4'. Supported types include images, application/pdf, text/plain",
 * });
 *
 * console.log(error.isRetryable); // false
 * console.log(error.message);
 ...
 * **Details**
 *
 * Every reason carries a semantic `_tag`, a human-readable message, and an `isRetryable` getter.
 * Provider-facing reasons may also include retry timing, provider metadata, usage information, or
 * HTTP context.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * This error stores `module` and `method` context, the semantic `reason`, and delegates
 * `isRetryable` and `retryAfter` to the underlying reason.
 *
 ...
 * **When to use**
 *
 * Use as the base mapping when provider packages translate HTTP status codes into provider-specific
 * error reasons.
 *
