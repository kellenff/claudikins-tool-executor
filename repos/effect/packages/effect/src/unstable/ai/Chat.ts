 ...
 *
 * A `Chat` keeps `Prompt` history in a `Ref` and reuses it for text generation, streaming, and
 * structured output. Each generation call combines the current history with the caller's new
 * prompt, invokes the active language model, and appends the response parts back into history.
 * Constructors create fresh sessions, seed sessions from prompts, restore exported history, or
 * connect a chat to persistence.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * This tag provides access to chat functionality throughout your application, enabling persistent
 * conversational AI interactions with full context management.
 *
 ...
 * **When to use**
 *
 * Use as the service contract for code that receives or constructs a stateful chat session and
 * needs history, export, text generation, streaming, and structured-output operations.
 *
 * @since 4.0.0
 * @category Models
...
export interface Service {
   ...
   * **Details**
   *
   * Provides direct access to the conversation history for advanced use cases like custom history
   ...
   * **Details**
   *
   * Returns the complete conversation history as a structured object that can be stored,
   * transmitted, or processed by other systems.
   *
   ...
   * **Details**
   *
   * Provides a convenient way to serialize the entire conversation for storage or transmission in
   * JSON format.
   *
   ...
   * **Details**
   *
   * If a toolkit is specified, the language model will have access to tools for function calling
   * and enhanced capabilities. Both input and output messages are automatically added to the chat
   * history.
   *
   ...
   * **Details**
   *
   * Returns a stream of response parts that are emitted as soon as they're available from the
   * model. Supports tool calling and maintains chat history.
   *
   ...
   * **Details**
   *
   * Forces the model to return data that conforms to the specified schema, enabling structured data
   * extraction and type-safe responses. The conversation history is maintained across calls.
   *
  ...
  >(
    options: Options &
      LanguageModel.GenerateObjectOptions<LanguageModel.ExtractTools<Options>, ObjectSchema>,
  ) => Effect.Effect<
 ...
 * **Details**
 *
 * This is the primary constructor for creating chat instances. It initializes a new conversation
 * with the provided prompt as the starting context.
 *
 ...
 * method. This is the most convenient way to persist and restore chat sessions to/from storage
 * systems.
 *
...
/**
 * Represents an error that occurs when attempting to retrieve a persisted `Chat` that does not
 * exist in the backing persistence store.
 *
 * **When to use**
 *
...
export declare namespace Persistence {
  /**
   * Represents the backing persistence for a persisted `Chat`. Allows for creating and retrieving
   * chats that have been saved to a persistence store.
   *
   * @since 4.0.0
   * @category Models
   */
  export interface Service {
    /**
     * Attempts to retrieve the persisted chat from the backing persistence store with the specified
     * chat identifer. If the chat does not exist in the persistence store, a `ChatNotFoundError`
     * will be returned.
     */
    readonly get: (
      chatId: string,
    ...
    /**
     * Attempts to retrieve the persisted chat from the backing persistence store with the specified
     * chat identifer. If the chat does not exist in the persistence store, an empty chat will be
 ...
 * **Details**
 *
 * When calling a text generation method (e.g. `generateText`), the previous chat history as well as
 * the relevent response parts will be saved to the backing persistence store.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when you need programmatic persisted chat creation and retrieval backed by the current
 * `BackingPersistence`.
 *
 * **Details**
 *
 * The provided store identifier will be used to indicate which "store" the backing persistence
 * should load chats from.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use to provide `Chat.Persistence` from a configured `BackingPersistence` when your application
 * needs persisted chat sessions backed by a named store.
 *
 * **Details**
 *
 * The provided store identifier will be used to indicate which "store" the backing persistence
