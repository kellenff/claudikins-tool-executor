 ...
 *
 * An `EmbeddingModel` turns text into numeric vectors. It supports single-input embedding and
 * ordered batch embedding, and represents provider failures as `AiError` values. This module also
 * includes the embedding dimensions service, request and response models, usage metadata, provider
 * contracts, and a constructor that adapts a provider batch implementation into the service. Single
 * `embed` calls can be batched together internally.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to retrieve or provide the embedding model service for an `Effect` program that embeds text
 * into vectors.
 *
 * @since 4.0.0
 * @category Services
 ...
 * **Details**
 *
 * Contains optional provider-reported `inputTokens`. The value may be `undefined` when the provider
 * does not report usage or when `embedMany([])` bypasses the provider.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when you need a typed request for one embedding input while building or calling a low-level
 * embedding request resolver.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use to adapt a provider's batch embedding implementation into an `EmbeddingModel.Service` that
 * offers single-input and batch embedding operations.
 *
 * **Details**
 *
 * The returned service builds single-input `embed` calls through a request resolver, so concurrent
 * `embed` requests can be batched into one provider `embedMany` call. Direct `embedMany` calls pass
 * the input array to the provider, while `embedMany([])` returns an empty response without calling
 * the provider.
 *
 * **Gotchas**
 *
 * Provider responses are interpreted positionally and must contain exactly one result for each
 * requested input. If the provider returns a different number of results, `embed` and `embedMany`
 * fail with `AiError.InvalidOutputError`.
 *
 * @since 4.0.0
 * @category Constructors
