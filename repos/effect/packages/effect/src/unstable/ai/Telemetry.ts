 ...
 *
 * This module models the `gen_ai.*` attributes used by language model and embedding providers. It
 * includes attribute types, helpers for writing non-null attributes onto existing spans, and a
 * `CurrentSpanTransformer` service for adding custom span annotations from provider options and
 * response parts.
 *
 * @since 4.0.0
 */
...
/**
 * The attributes used to describe telemetry in the context of Generative Artificial Intelligence
 * (GenAI) models requests and responses.
 *
 * **Details**
 *
 * These attributes follow the OpenTelemetry generative AI semantic conventions:
...
/**
 * Telemetry attributes which are part of the GenAI specification and are namespaced by
 * `gen_ai.operation`.
 *
 * @since 4.0.0
 * @category Models
...
export interface RequestAttributes {
  ...
  /**
   * The seed setting for the GenAI request. Requests with same seed value are more likely to return
   * same result.
   */
 ...
 * **Details**
 *
 * If one of them applies, then the respective value **MUST** be used; otherwise, a custom value
 * **MAY** be used.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Transforms attribute keys by adding a prefix and formatting them according to OpenTelemetry
 * conventions (camelCase to snake_case).
 *
 ...
 * **Details**
 *
 * This type recursively transforms string literal types from camelCase to snake_case, which is the
 * standard format for OpenTelemetry attributes.
 *
 ...
 * **Details**
 *
 * The returned function mutates the supplied span by adding each non-nullish attribute as
 ...
 * **Details**
 *
 * Combines base attributes with optional grouped attributes for comprehensive telemetry coverage of
 * AI operations.
 *
 ...
 * **When to use**
 *
 * Use when you need to write GenAI request, response, token, or usage attributes onto an existing
 * OpenTelemetry span.
 *
 * **Details**
 *
 * This function adds standardized GenAI attributes to a span following OpenTelemetry semantic
 * conventions.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * Span transformers receive the complete request/response context from AI operations and can add
 * custom telemetry attributes, metrics, or other observability data.
 *
 ...
 * **When to use**
 *
 * Use to retrieve or provide the current `SpanTransformer` through context for language model span
 * annotation.
 *
 * @since 4.0.0
 * @category Services
