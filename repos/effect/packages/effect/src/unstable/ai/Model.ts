 ...
 *
 * A `Model` can be used anywhere its underlying `Layer` can be used. It also provides the current
 * provider name and model name through the Effect context. This module includes the `Model`
 * interface, the `ProviderName` and `ModelName` service tags, and the `make` constructor. Models
 * can also capture their required services from the current context when they need to be used
 ...
 * **When to use**
 *
 * Use when you use a Model directly as a Layer to provide a particular model implementation to an
 * Effect program, or use it as an Effect to "lift" dependencies of the Model constructor into the
 * parent Effect when you want to use a Model from within an Effect service.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * This tag is automatically provided by Model instances and can be used to access the name of the
 * provider that is currently in use within a given Effect program.
 *
 * @since 4.0.0
 * @category Services
 */
 ...
 *
 * This tag is automatically provided by Model instances and can be used to access the name of the
 * model that is currently in use within a given Effect program.
 *
 * @since 4.0.0
 * @category Services
 */
...
export const make = <const Provider extends string, const Name extends string, Provides, Requires>(
  ...
  Object.assign(
    ...
    Layer.merge(
      layer,
      Layer.succeedContext(ProviderName.context(provider).pipe(Context.add(ModelName, modelName))),
    ),
  );
