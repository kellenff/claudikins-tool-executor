 ...
 *
 * Responses are represented as typed parts so different providers can expose text, reasoning, tool
 * calls, files, sources, metadata, finish information, and errors through one shape. The same model
 * is used for complete responses and streaming responses, where start, delta, and end parts
 * describe content as it arrives. This module also carries provider metadata and schemas used by
 * tools that need to validate response parts.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Generates a schema that includes all possible response parts, with tool call and tool result
 * parts dynamically created based on the provided toolkit.
 *
...
/**
 * Schema for provider-specific metadata attached to response parts, represented as a record from
 * provider-specific keys to JSON values or `null`.
 *
 * @since 4.0.0
 * @category Schemas
 */
...
/**
 * Type of provider-specific metadata attached to response parts, keyed by provider-specific names
 * with JSON or `null` values.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Base interface for all response content parts, including the type identifier and optional
 * metadata.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Represents provider-specific metadata that can be associated with a `TextPart` through module
 * augmentation.
 *
 * @since 4.0.0
...
/**
 * Represents provider-specific metadata that can be associated with a `TextStartPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Response part containing incremental text content to be added to the existing text chunk with the
...
/**
 * Represents provider-specific metadata that can be associated with a `TextDeltaPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Represents provider-specific metadata that can be associated with a `TextEndPart` through module
 * augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Response part carrying provider-supplied reasoning text, such as an exposed reasoning summary or
 * explanation. Do not assume it contains hidden chain-of-thought.
 *
...
/**
 * Represents provider-specific metadata that can be associated with a `ReasoningPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Response part indicating the start of streaming reasoning content with a unique reasoning chunk
 * identifier.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Represents provider-specific metadata that can be associated with a `ReasoningStartPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Response part containing incremental reasoning content to be added to the existing chunk of
 * reasoning text with the same unique identifier.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Represents provider-specific metadata that can be associated with a `ReasoningDeltaPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Represents provider-specific metadata that can be associated with a `ReasoningEndPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
> {
  ...
  readonly id: string;
  /**
   * Name of the tool being called, which corresponds to the name of the tool in the `Toolkit`
...
/**
 * Represents provider-specific metadata that can be associated with a `ToolParamsStartPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
 ...
 * **Details**
 *
 * Represents a chunk of tool parameters being streamed, containing the incremental JSON content
 * that forms the tool parameters.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Represents provider-specific metadata that can be associated with a `ToolParamsDeltaPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
 ...
 * **Details**
 *
 * Marks the completion of a tool parameter stream, indicating that all parameter data has been sent
 * and the tool call is ready to be executed.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Represents provider-specific metadata that can be associated with a `ToolParamsEndPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Represents provider-specific metadata that can be associated with a `ToolCallPart` through module
 * augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
> {
   ...
   * **Details**
   *
   * Preliminary results represent progress updates during streaming tool execution. Only the final
   * result (where `preliminary` is `false` or `undefined`) should be used as the authoritative
   * output.
   *
   * **Gotchas**
   *
...
/**
 * Represents provider-specific metadata that can be associated with a `ToolResultPart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
 ...
 * **Details**
 *
 * Emitted when a tool requires user approval before execution. The framework checks the tool's
 * `needsApproval` property and emits this part instead of executing the tool when approval is
 * required.
 *
...
/**
 * Represents provider-specific metadata that can be associated with a `ToolApprovalRequestPart`
...
/**
 * Represents provider-specific metadata that can be associated with a `FilePart` through module
 * augmentation.
 *
 * @since 4.0.0
 * @category Configuration
...
/**
 * Represents provider-specific metadata that can be associated with a `DocumentSourcePart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
 ...
 * **Details**
 *
 * Validates `type: "source"`, `sourceType: "document"`, required `id`, `mediaType`, and `title`,
 * optional `fileName`, and the metadata fields inherited from response parts.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Represents provider-specific metadata that can be associated with a `UrlSourcePart` through
 * module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
 ...
 * **Details**
 *
 * Captures comprehensive information about the HTTP request made to the AI provider, enabling
 * inspection of request metadata for debugging and observability purposes.
 *
 ...
 * **Details**
 *
 * Captures essential information about the HTTP response received from the AI provider, including
 * status codes and headers for debugging and observability purposes.
 *
...
/**
 * Represents provider-specific metadata that can be associated with a `ResponseMetadataPart`
 * through module augmentation.
 *
 * @since 4.0.0
 * @category Configuration
 ...
 * **Details**
 *
 * Values include normal stops, token-limit stops, content filtering, tool-call pauses, provider
 * errors, and unknown provider-specific finish reasons.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * If the model provider returns additional usage information than what is specified here, you can
 * generally find that information under the provider metadata of the finish part of the response.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Represents provider-specific metadata that can be associated with a `FinishPart` through module
 * augmentation.
 *
 * @since 4.0.0
 * @category Configuration
 ...
 * **Details**
 *
 * Validates `type: "finish"`, `reason` through `FinishReason`, `usage` through `Usage`, and
 * optional provider HTTP response details.
 *
 * @since 4.0.0
 * @category Schemas
 */
...
/**
 * Represents provider-specific metadata that can be associated with a `ErrorPart` through module
 * augmentation.
 *
 * @since 4.0.0
 * @category Configuration
 ...
 * **Gotchas**
 *
 * The decoded `error` value is not guaranteed to be an `Error`; narrow it before reading
