 ...
 *
 * The `McpServer` service stores the tools, resources, resource templates, prompts, completions,
 * initialized clients, and outgoing notifications exposed by a server. This module also includes
 * the server runner, custom protocol, stdio, and HTTP layers, registration helpers, and APIs that
 * let handlers ask the connected client for structured input or read its advertised capabilities.
 *
 * @since 4.0.0
 */
...
/**
 * Service that stores and serves an MCP server's registered tools, resources, prompts, completions,
...
>()("effect/ai/McpServer") {
      ...
      callTool: (request) =>
        Effect.suspend(
          (): Effect.Effect<CallToolResult, InternalError | InvalidParams, McpServerClient> => {
            ...
            if (!handle) {
              return Effect.fail(
                new InvalidParams({ message: `Tool '${request.name}' not found` }),
              );
            }
   ...
   * @since 4.0.0
   */
  static readonly layer: Layer.Layer<McpServer | McpServerClient> = Layer.effect(McpServer)(
    McpServer.make,
  ) as any;
}
 ...
 * **Details**
 *
 * The server performs initialization and session handling, serves registered tools, resources, and
 * prompts, and forwards queued server notifications to initialized clients.
 *
 * @since 4.0.0
...
}) => Effect.Effect<never, never, McpServer | RpcServer.Protocol> = Effect.fnUntraced(
  function* (options: { readonly name: string; readonly version: string }) {
    ...
    const encodeNotification = Schema.encodeUnknownEffect(
      Schema.Union(
        Array.from(ServerNotificationRpcs.requests.values(), (rpc) => rpc.payloadSchema),
      ),
    );
...
/**
 * Creates a layer that starts an MCP server over an existing `RpcServer.Protocol` and provides the
 * `McpServer` and `McpServerClient` services.
 *
 * **When to use**
 *
 * Use when you already have a custom or externally provided `RpcServer.Protocol` and want to start
 * an MCP server as part of a layer graph.
 *
 * **Details**
 *
 * The returned layer forks `run(options)` in the layer scope and merges `McpServer.layer`, so
 * registration layers can use the `McpServer` service while the server is running.
 *
 * **Gotchas**
 *
 * Unlike `layerStdio` and `layerHttp`, this layer does not install a concrete transport. The
 * surrounding layer graph must provide `RpcServer.Protocol`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Registers an HTTP POST JSON-RPC route at `options.path` on the current `HttpRouter`.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * This layer composes `layer(options)`, `RpcServer.layerProtocolHttp(options)`, and
 ...
 * **Details**
 *
 * Each schema interpolation contributes a parameter key, using an explicit `Param` name when
 * present or `paramN` otherwise, and each handler returns candidate values for that parameter.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when you are already inside an Effect program with an `McpServer` service and need to add a
 * concrete resource or URI-template resource directly.
 *
 * @since 4.0.0
 * @category Resources
 ...
 *
 * Use when you are already inside an Effect program with an `McpServer` service and need to add a
 * prompt handler directly.
 *
 * **Details**
 *
 * Parameters are decoded with the supplied schema, completion handlers encode per-parameter
 * suggestions, and string prompt content is converted into a user text message.
 *
 * @since 4.0.0
 * @category Prompts
...
> => {
  ...
  return Effect.gen(function* () {
    const registry = yield* McpServer;
    const services =
      yield* Effect.context<Exclude<R | Schema.Struct.DecodingServices<Params>, McpServerClient>>();
    const completions: Record<
      string,
      (input: string) => Effect.Effect<CompleteResult, InternalError, McpServerClient>
    > = {};
...
/**
 * Collects structured input from the current MCP client and decodes the accepted response with
 * `schema`.
 *
 * **Details**
 *
 * Accepted content is decoded with the supplied schema, declined requests fail with
 * `ElicitationDeclined`, and canceled requests interrupt the effect.
 *
 * @since 4.0.0
...
>(options: {
  ...
  >(options: { readonly message: string; readonly schema: S }) {
    ...
    const res = yield* client["elicitation/create"](request).pipe(
      Effect.catchCause((cause) =>
        Effect.fail(new ElicitationDeclined({ cause: Cause.squash(cause), request })),
      ),
    );
    switch (res.action) {
      case "accept":
 ...
 */
export const clientCapabilities: Effect.Effect<ClientCapabilities, never, McpServerClient> =
  ...
  ClientRpcs.toLayer(
    Effect.gen(function* () {
      ...
      return ClientRpcs.of({
        ...
        "prompts/get": (r) =>
          server.getPromptResult(r).pipe(Effect.provideService(CurrentLogLevel, currentLogLevel)),
        ...
        "tools/call": (r) =>
          server.callTool(r).pipe(Effect.provideService(CurrentLogLevel, currentLogLevel)),
