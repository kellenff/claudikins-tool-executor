 ...
 *
 * The `Socket` service runs handlers for binary, string, or raw frames and provides a scoped writer
 * for outgoing bytes, text, or close events. This module also includes socket errors, channel
 * adapters, WebSocket layers, and transform-stream-backed sockets.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to access or provide the socket implementation used by programs that read and write frames
...
/**
 * Effect-based socket abstraction for running string or binary read handlers and obtaining a scoped
 * writer for outgoing frames and close events.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Constructs a `Socket` from a raw read loop and scoped writer, deriving binary and string read
 * loops when they are not provided.
 *
 * @since 4.0.0
...
/**
 * Typed error for failures that occur while opening a socket, including unknown open failures and
 * open timeouts.
 *
 * @since 4.0.0
 * @category Errors
...
/**
 * Tagged error that wraps socket read, write, open, and close failures while preserving the
 * underlying reason.
 *
 * @since 4.0.0
 * @category Errors
 */
export class SocketError extends Schema.TaggedErrorClass<SocketError>(SocketErrorTypeId)(
  "SocketError",
  {
...
/**
 * Converts a `Socket` into a bidirectional `Channel`, mapping incoming string or binary frames and
 * writing outgoing frame batches to the socket.
 *
 * @since 4.0.0
...
/**
 * Converts a `Socket` into a string `Channel`, decoding binary frames with the optional text
 * encoding.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Context service for the active `WebSocket` instance available while a WebSocket-backed socket run
 * is handling events.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Creates a `Socket` backed by a `WebSocketConstructor`, acquiring the WebSocket for each run and
 * using the close-code classifier to decide which closes fail the run.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Builds a `Socket` from a scoped WebSocket acquisition effect, waiting for the socket to open,
 * dispatching message handlers in fibers, and translating open, read, and close events into
 * `SocketError` values.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Builds a `Socket` from a scoped `InputTransformStream`, reading incoming chunks through socket
 * handlers and writing outgoing chunks to the writable stream, encoding strings as UTF-8 and using
 * close-code classification for `CloseEvent` values.
 *
 * @since 4.0.0
 * @category Constructors
 */
