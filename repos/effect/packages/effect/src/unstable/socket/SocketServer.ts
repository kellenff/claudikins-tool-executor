 ...
 *
 * `SocketServer` exposes the bound server `address` and a long-running `run` loop that hands each
 * accepted connection to a handler as a `Socket.Socket`. The module also defines TCP and Unix
 * socket address models plus the server-level errors reported while opening or running a server.
...
/**
 * Context service for a socket server, exposing its bound address and a run loop that handles each
 * accepted `Socket`.
 *
 * @since 4.0.0
 * @category Services
