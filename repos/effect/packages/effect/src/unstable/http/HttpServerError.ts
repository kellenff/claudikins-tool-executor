 ...
 *
 * `HttpServerError` covers failures that happen while accepting a request, matching a route,
 * running a handler, or building and sending a response. Request-scoped failures keep the request
 * that caused them, and response failures keep the response being produced. This module also
 * includes helpers for turning failed causes or exits into HTTP responses and an annotation for
 ...
 * **Details**
 *
 * It wraps a `HttpServerErrorReason`, exposes the associated request and optional response, and can
 * be converted to an HTTP response through the `Respondable` protocol.
 *
 * @since 4.0.0
 * @category Errors
 */
 ...
 * **Details**
 *
 * When converted to a response it produces an empty `404` response, and it is ignored by the error
 * reporter.
 *
 * @since 4.0.0
 * @category Errors
 ...
 * **Details**
 *
 * It carries the request and response involved in the failure. When converted to a response it
 * produces an empty `500` response.
 *
 * @since 4.0.0
 * @category Errors
 */
 ...
 * **Details**
 *
 * `causeResponse` uses this annotation to map a pure client abort to a `499` response instead of a
...
/**
 * Converts a failed handler cause into the HTTP response that should be sent and the cause that
 * should be reported.
 *
 * **Details**
 *
 * Respondable failures and defects can choose their own response, defects that are already
 * `HttpServerResponse` values are used directly, and pure interrupts produce either `499` for
 * client aborts or `503` for server aborts.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * If the cause contains a defect that is already an `HttpServerResponse`, that response is used and
 * removed from the remaining cause. Otherwise the response defaults to `500`.
 *
 * @since 4.0.0
 * @category Error handling
 */
...
/**
 * Extracts the response from a successful handler exit, or derives a response from the failure
 * cause.
 *
 * @since 4.0.0
 * @category Error handling
