 ...
 *
 * `HttpServerRequest` is the context service used by handlers, middleware, schema decoders,
 * multipart parsers, WebSocket upgrades, and adapters. A request stores its method, URL, original
 * URL, headers, cookies, remote address, body stream, and platform source object. This module also
 * includes request conversions and schema decoders for cookies, headers, search parameters, JSON,
 * forms, URL-encoded bodies, and multipart bodies.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It extends `HttpIncomingMessage` with request metadata, parsed cookies, multipart accessors,
 * WebSocket upgrade support, and a `modify` method for creating adjusted request views.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * @category Context
 */
export const HttpServerRequest: Context.Service<HttpServerRequest, HttpServerRequest> =
 ...
 * **Details**
 *
 * The channel reads incoming socket messages and writes byte chunks to the socket, failing if the
 * request cannot be upgraded or the socket fails.
 *
 * @since 4.0.0
 * @category Accessors
 */
 ...
 * **Details**
 *
 * Multipart requests are persisted and decoded as multipart data; other form requests are decoded
 * from URL-encoded body parameters.
 *
 * @since 4.0.0
 * @category Schemas
 */
...
/**
 * Reads the current request body as URL-encoded parameters and decodes them with the supplied
 * schema.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Details**
 *
 * The effect requires the services needed to persist multipart files, including a scope, file
 * system, and path service.
 *
 * @since 4.0.0
 * @category Schemas
 */
 ...
 * **Details**
 *
 * For multipart requests, the named multipart field is decoded as JSON. For URL-encoded requests,
 * the named parameter is decoded as JSON and then decoded with the supplied schema.
 *
 * @since 4.0.0
 * @category Schemas
 */
 ...
 * **Details**
 *
 * The request's current URL is stored without the scheme and host, while the original Web URL
 * remains available as `originalUrl`.
 *
 * @since 4.0.0
 * @category Converting
 */
 ...
 * **Details**
 *
 * The converted request preserves the method, headers, body stream, and a URL derived from the
 ...
 * **Details**
 *
 * The host comes from the `host` header, defaulting to `localhost`, and the protocol is `https`
 * only when `x-forwarded-proto` is `https`; invalid URLs return `Option.none`.
 *
 * @since 4.0.0
 * @category Converting
 */
 ...
 * **Details**
 *
 * If the source is already a Web `Request`, it is returned unchanged. Otherwise an absolute URL is
 * derived from the request; invalid URLs fail with a `RequestParseError`.
 *
 * @since 4.0.0
 * @category Converting
 */
