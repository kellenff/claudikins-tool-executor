 ...
 *
 * `HttpIncomingMessage` is used by server requests and client responses to expose headers, an
 * optional remote address, and body accessors. This module provides decoders for JSON bodies,
 * URL-encoded bodies, and headers, plus the shared body-size setting and inspection helper used by
...
/**
 * Common model for incoming HTTP messages, with headers, remote address, and effectful body
 * accessors.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Creates a decoder that reads an incoming message's JSON body and decodes it with the supplied
 * schema.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Creates a decoder that reads an incoming message's URL-encoded body parameters and decodes them
...
/**
 * Creates a decoder that validates and decodes an incoming message's headers with the supplied
 * schema.
 *
 * @since 4.0.0
 * @category Schemas
...
export const schemaHeaders = <A, I extends Readonly<Record<string, string | undefined>>, RD>(
  ...
  return <E>(self: HttpIncomingMessage<E>): Effect.Effect<A, Schema.SchemaError, RD> =>
...
/**
 * Builds an inspectable object for an incoming message, redacting headers and including a
 * synchronously readable JSON or text body when available.
 *
 * @since 4.0.0
 * @category Converting
 */
