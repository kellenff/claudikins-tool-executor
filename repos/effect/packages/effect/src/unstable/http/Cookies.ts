 ...
 *
 * A `Cookie` stores a name, value, encoded value, and standard cookie attributes. A `Cookies` value
 * is an immutable collection keyed by cookie name. This module parses request `Cookie` headers,
 * builds response `Set-Cookie` headers, and provides helpers for adding, removing, merging, and
 * expiring cookies.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * JSON encoding uses `Set-Cookie` header strings, while isomorphic encoding uses a readonly record
 * of cookie values.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * HTTP cookie value with its decoded value, encoded value, and optional cookie attributes such as
 * domain, path, expiration, security, and same-site settings.
 *
 * @since 4.0.0
 * @category Cookies
 */
...
/**
 * Error reason describing why cookie construction failed, such as invalid name, value, domain,
 * path, or infinite max-age.
 *
 * @since 4.0.0
 * @category Errors
 */
 ...
 * **Details**
 *
 * The cookie fields are validated first; invalid input returns a `CookiesError` in the `Result`
 * failure channel.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * If any tuple is invalid, returns the first `CookiesError` and leaves the original collection
 * unchanged.
 *
 * @since 4.0.0
 * @category Combinators
...
export const schemaRecord = CookiesSchema.pipe(
  Schema.decodeTo(
    ...
    SchemaTransformation.transform({
      decode: toRecord,
      encode: (self) =>
        fromIterable(Object.entries(self).map(([name, value]) => makeCookieUnsafe(name, value))),
    }),
  ),
);
