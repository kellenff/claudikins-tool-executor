 ...
 *
 * Platform adapters provide `HttpServer`, and routers or applications consume it to run an
 * `HttpServerResponse` effect for each incoming request. The service exposes the listening address,
 * while this module also includes helpers for address formatting, server logging, and clients that
 * target the current server in tests.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The service can serve an HTTP response effect and exposes the address where the server is
 * listening.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * The request service is supplied by the server for each request; the returned layer still requires
 * the server, a scope, and any non-request dependencies of the response effect or middleware.
 *
 * @since 4.0.0
 * @category Accessors
 */
 ...
 * **Details**
 *
 * The request service is supplied by the server for each request; the effect requires a scope and
 * any non-request dependencies of the response effect or middleware.
 *
 * @since 4.0.0
 * @category Accessors
 */
 ...
 * **Details**
 *
 * TCP addresses are formatted as `http://host:port`; Unix socket addresses are formatted as
 * `unix://path`.
 *
 * @since 4.0.0
 * @category Address
...
/**
 * Reads the current server address, formats it with `formatAddress`, and passes the formatted
 * address to the supplied effectful function.
 *
 * @since 4.0.0
 * @category Address
 */
 ...
 * **Details**
 *
 * For TCP servers, requests are prefixed with the server URL and `0.0.0.0` is rewritten to
 * `127.0.0.1`.
 *
 * **Gotchas**
 *
