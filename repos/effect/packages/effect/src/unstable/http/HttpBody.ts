 ...
 *
 * `HttpBody` is the shared body representation used by the HTTP modules. Each variant stores the
 * payload together with metadata that can be known before sending it, such as `contentType` and
 * `contentLength`. This module includes body constructors for common payload shapes, support for
 * schema-encoded JSON bodies, streaming and file-backed bodies, and the error type used when body
 * construction fails.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Supported variants include empty bodies, raw bodies, byte arrays, `FormData`, and streams of
 * bytes.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * `JsonError` represents a `JSON.stringify` failure; `SchemaError` represents a schema encoding
 * issue.
 *
 * @since 4.0.0
 * @category Errors
...
/**
 * Creates a raw HTTP body from an arbitrary value and optional `contentType` and `contentLength`
 * metadata.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The content type defaults to `application/octet-stream`, and the content length is the byte array
 * length.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * `JSON.stringify` failures are captured as `HttpBodyError` values, and the content type defaults
 * to `application/json`.
 *
 * @since 4.0.0
 * @category Constructors
...
export const jsonSchema = <S extends Schema.Constraint>(
  ...
  return (
    ...
    encode(body, options).pipe(
      Effect.mapError(
        (issue) => new HttpBodyError({ reason: { _tag: "SchemaError", issue }, cause: issue }),
      ),
...
export const urlParams = (urlParams: UrlParams.Input, contentType?: string): Uint8Array =>
  text(
    UrlParams.toString(UrlParams.fromInput(urlParams)),
 ...
 * **Details**
 *
 * The content type and content length are left unset so the runtime can supply multipart
 * boundaries.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * `File` and `Blob` values are appended directly, primitive values are converted to strings, and
 ...
 * **Details**
 *
 * Array fields append each item under the same key; primitive values are stringified, `File` and
 * `Blob` values are appended directly, and nullish values are skipped.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The effect requires `FileSystem`, stats the file to set the content length, and can fail with
 * `PlatformError`.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * The effect requires `FileSystem`, uses the provided file size as the content length, and can fail
 * with `PlatformError`.
 *
 * @since 4.0.0
 * @category Constructors
