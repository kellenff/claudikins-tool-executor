 ...
 *
 * An `HttpClientResponse` keeps the original request together with the response status, headers,
 * cookies, and body accessors from the shared incoming-message model. This module includes
 * constructors, schema-based decoders, helpers for streaming response bodies, and utilities for
 * matching or filtering by HTTP status.
 *
 * @since 4.0.0
 */
...
export {
  ...
  schemaBodyJson,
  /**
   * Creates a decoder that reads response URL-encoded body parameters and decodes them with the
   * supplied schema.
   *
   * @since 4.0.0
   * @category Schemas
...
/**
 * Model of an HTTP client response, including the original request, status, cookies, headers, and
 * body accessors.
 *
 * @since 4.0.0
 * @category Models
...
export interface HttpClientResponse
  extends HttpIncomingMessage.HttpIncomingMessage<Error.HttpClientError>, Pipeable {
...
/**
 * Pattern matches on a response status, checking exact status handlers before status-class handlers
 * and `orElse`.
 *
 * @since 4.0.0
...
/**
 * Succeeds with the response when its status satisfies the predicate, otherwise fails with
 * `HttpClientError`.
 *
 * @since 4.0.0
 * @category Filters
...
/**
 * Succeeds with the response only when its status is in the 2xx range, otherwise fails with
 * `HttpClientError`.
 *
 * @since 4.0.0
 * @category Filters
