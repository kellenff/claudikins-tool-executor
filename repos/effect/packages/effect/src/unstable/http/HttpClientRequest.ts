 ...
 *
 * `HttpClientRequest` is the request model shared by Effect HTTP clients and platform adapters. A
 * request stores its method, URL, query parameters, hash, headers, and body as structured data.
 * This module includes constructors, helpers for updating requests, body encoders for common
 * payloads, and conversions to and from Web `Request` values.
 *
 * @since 4.0.0
 */
...
/**
 * Immutable model of an outgoing HTTP client request, including its method, URL components,
 * headers, and body.
 *
 * @since 4.0.0
 * @category Models
...
export declare namespace Options {
  /**
   * Request options that omit the method and URL for helpers that already receive those values
   * separately.
   *
   * @since 4.0.0
   * @category Options
...
/**
 * Sets multiple request headers from an input collection, replacing existing values with matching
 * names.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Sets the request URL. When given a `URL`, its search parameters and hash are extracted into the
...
/**
 * Appends query parameters from an input collection without removing existing values for matching
 * names.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Sets the request body and updates `Content-Type` and `Content-Length` headers from the body
...
/**
 * Encodes a value as a JSON request body and sets it on the request, failing with `HttpBodyError`
 * if encoding fails.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **When to use**
 *
 * Use when the request body is known to be JSON-serializable and a synchronous `HttpClientRequest`
 * result is needed.
 *
 * **Gotchas**
 *
...
/**
 * Sets a streaming `Uint8Array` request body with optional content type and content length
 * metadata.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Builds a `URL` from the request URL, query parameters, and hash, returning `Option.none()` if the
 * URL is invalid.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Converts a Web `Request` into an `HttpClientRequest`, preserving method, URL, headers, and
...
/**
 * Converts an `HttpClientRequest` safely to a Web `Request` as a `Result`, failing when the request
 * URL is invalid.
 *
 * @since 4.0.0
 * @category Converting
...
/**
 * Converts an `HttpClientRequest` to a Web `Request`, failing with `UrlError` when the request URL
 * is invalid.
 *
 * @since 4.0.0
 * @category Converting
