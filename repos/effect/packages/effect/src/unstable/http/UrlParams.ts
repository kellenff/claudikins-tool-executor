 ...
 *
 * `UrlParams` is used for HTTP client query strings, URL-encoded form bodies, and server-side
 * decoding. Values can be built from records, iterables, or native `URLSearchParams`, then updated,
 * serialized, converted to a `URL`, or decoded with schemas.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Parameters are stored as ordered string key-value pairs and can contain multiple values for the
 * same key.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Values can be provided as a coercible record, an iterable of key-value pairs, or a native
 ...
 * **Details**
 *
 * Primitive values are converted to strings, arrays produce repeated parameters, nested records use
 * bracket notation, and `undefined` values are omitted.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * Keys present in the input replace existing values for those keys, while unmentioned existing
...
/**
 * Builds a `Record` containing all the key-value pairs in the given `UrlParams` as `string` (if
 * only one value for a key) or a `NonEmptyArray<string>` (when more than one value for a key)
 *
 ...
 * **Details**
 *
 * Keys with one value map to a string, and keys with multiple values map to a non-empty readonly
 * array of strings.
 *
 * @since 4.0.0
 * @category Converting
 ...
 * **Details**
 *
 * Keys with one value decode to a string, and keys with multiple values decode to a non-empty
