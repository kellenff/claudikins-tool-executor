 ...
 *
 * ETags are validators for a specific representation of a resource. Servers put them in `ETag`
 * response headers so clients and intermediaries can revalidate cached content with
 * `If-None-Match`, or protect writes with preconditions such as `If-Match`.
 *
 * @since 4.0.0
 */
...
/**
 * Layer that provides a `Generator` which produces strong ETags from file size and modification
 * time metadata.
 *
 * **When to use**
 *
 * Use when you need the `Generator` service to produce strong ETags and file size plus modification
 * time reliably change for every byte-level change.
 *
 * **Gotchas**
 *
 * This layer marks metadata-derived tags as strong. If the underlying storage can update file
 * contents without changing the recorded size or modification time, those tags can stop
 * representing byte-for-byte identity.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Layer that provides a `Generator` which produces weak ETags from file size and modification time
 * metadata.
 *
 * @since 4.0.0
 * @category Layers
