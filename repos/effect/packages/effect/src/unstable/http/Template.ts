 ...
 *
 * Template interpolations can be plain values, optional values, effects, or streams. The resulting
 * effect or stream keeps the errors and service requirements from any effectful interpolations,
 * which lets response helpers assemble dynamic text without losing type information.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Interpolations can be primitive values, optional primitive values, or effects that produce
 * primitive values.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Primitive and `Option` interpolations are rendered immediately. Effect interpolations are
 * evaluated and rendered before the final string is produced.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * Static text is emitted with interpolated values. Effect interpolations are evaluated as stream
 * chunks, and stream interpolations are flattened into the output.
 *
 * @since 4.0.0
 * @category Constructors
 */
