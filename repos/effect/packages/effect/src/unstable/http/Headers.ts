 ...
 *
 * `Headers` values are immutable maps keyed by lowercase header name. This module converts common
 * header inputs into that shape, provides helpers for reading and updating header values, and
 * redacts configured sensitive headers when values are inspected.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Records may contain string values, string arrays, or `undefined`; arrays are joined with `", "`,
 * and `undefined` values are omitted.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * Header names are normalized to lowercase. Array values in record input are joined with `", "`,
 * and `undefined` values are omitted.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Gotchas**
 *
 * This mutates the record's prototype and does not normalize header names; callers must provide the
 * expected lowercase keys.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * Input headers are normalized with `fromInput` and override existing headers with the same
 * lowercase name.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * String keys are normalized to lowercase before matching; regular expressions are tested against
 * the stored header names.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Context reference listing header names or patterns that should be redacted when `Headers` are
