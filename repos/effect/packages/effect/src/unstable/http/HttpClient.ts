 ...
 *
 * `HttpClient` executes immutable `HttpClientRequest` values and returns `HttpClientResponse`
 * values. Keeping HTTP behind this service lets programs, tests, and generated API clients use the
 * same request model without depending on one concrete platform transport. This module includes
 * request accessors, constructors and layers, request and response transformations, status
 * filtering, retries, rate limiting, cookies, redirect handling, scoped request abortion, and
 * tracing support.
 *
 * @since 4.0.0
 */
...
/**
 * HTTP client whose requests produce `HttpClientResponse` values and can fail with
 * `HttpClientError`.
 *
 * @since 4.0.0
 * @category Models
...
export declare namespace HttpClient {
   ...
   * **Details**
   *
   * It exposes preprocessing, postprocessing, direct request execution, and method-specific
   * helpers.
   *
   * @since 4.0.0
   * @category Models
  ...
  /**
   * Function that turns a preprocessed request effect into the response effect executed by the
   * client.
   *
   * @since 4.0.0
   * @category Models
 ...
 * **When to use**
 *
 * Use to provide the default outgoing HTTP client service used by request accessors such as
 * `execute`, `get`, and `post`.
 *
 * @since 4.0.0
 * @category Services
 */
export const HttpClient: Context.Service<HttpClient, HttpClient> = Context.Service<
  HttpClient,
  HttpClient
>("effect/HttpClient");
 ...
 * **Details**
 *
 * The transformation receives both the response effect and the original request, allowing it to
 * change success, error, and environment behavior.
 *
 * @since 4.0.0
...
export {
  /**
   * Handles all client failures with an effectful recovery function and returns a transformed
   * client.
   *
   * @since 4.0.0
...
/**
 * Filters successful responses, or fails with the error produced by `orFailWith` when the predicate
 * does not match.
 *
 * @since 4.0.0
 * @category Filters
...
export const filterStatus: {
  ...
  ): <E, R>(self: HttpClient.With<E, R>) => HttpClient.With<E | Error.HttpClientError, R>;
  <E, R>(
    self: HttpClient.With<E, R>,
    f: (status: number) => boolean,
 ...
 * **Details**
 *
 * `execute` applies preprocessing to the request and then passes the resulting request effect to
 * postprocessing.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The runner receives the request, resolved URL, abort signal, and current fiber. The client
 * wrapper handles URL construction failures, tracing and propagation, header redaction, and
 * aborting non-scoped requests on interruption.
 *
 * @since 4.0.0
 * @category Constructors
 */
      ...
      Effect.flatMap(effect, (request) =>
        Effect.withFiber((fiber) => {
          ...
          const tracerDisabled =
            fiber.getRef(Tracer.DisablePropagation) || fiber.getRef(TracerDisabledWhen)(request);
          if (tracerDisabled) {
...
export const mapRequest: {
  ...
  <E, R>(
    ...
    makeWith(self.postprocess, (request) => Effect.map(self.preprocess(request), f)),
);
...
export declare namespace Retry {
   ...
   * **Details**
   *
   * The result includes errors and requirements introduced by schedules and effectful retry
   * predicates.
   *
   * @since 4.0.0
   * @category Error handling
      ...
      | (O extends { schedule: Schedule.Schedule<infer _O, infer _I, infer _E, infer _R> }
          ...
          : O extends { times: number }
            ...
            : O extends { until: Predicate.Refinement<E, infer E2> }
              ? E2
              : E)
      | (O extends { while: (...args: Array<any>) => Effect.Effect<infer _A, infer E, infer _R> }
          ? E
          : never)
      | (O extends { until: (...args: Array<any>) => Effect.Effect<infer _A, infer E, infer _R> }
          ? E
          : never),
      | R
      | (O extends { schedule: Schedule.Schedule<infer _O, infer _I, infer _E, infer R> }
          ? R
          : never)
      | (O extends { while: (...args: Array<any>) => Effect.Effect<infer _A, infer _E, infer R> }
          ? R
          : never)
      | (O extends { until: (...args: Array<any>) => Effect.Effect<infer _A, infer _E, infer R> }
          ? R
          : never)
    > extends infer Z
      ? Z
 ...
 * **Details**
 *
 * Specifying a `while` predicate allows you to consider other errors as transient, and is ignored
...
export declare namespace WithRateLimiter {
   ...
   * **Details**
   *
   * They define the backing limiter, initial limit window, keying strategy, algorithm, token cost,
   * and whether response headers update future limits.
   *
   * @since 4.0.0
  ...
  export interface Options {
    ...
    /**
     * The key to identify the rate limit. Requests with the same key will share the same rate
     * limit. This can be used to implement per-user or per-endpoint rate limits.
     */
 ...
 * **Details**
 *
 * It can update limits by inspecting common rate limit response headers and automatically retries
 * HTTP `429` responses (or `HttpClientError` values wrapping a `429` response) by forcing the retry
 * back through the limiter.
 *
 * @since 4.0.0
 * @category Rate limiting
 */
      ...
      return Effect.flatMap(
        ...
        ({ delay }) => {
          ...
          return Duration.isZero(delay)
            ? runAdaptive()
            : Effect.flatMap(Effect.sleep(delay), runAdaptive);
        },
      );
    });
  },
 ...
 * **When to use**
 *
 * Use to add shared cookie storage to a client so response cookies are retained and sent by later
 * requests.
 *
 * @since 4.0.0
 * @category Cookies
...
/**
 * Context reference for a predicate that disables client-side tracing for matching outgoing
 * requests.
 *
 * @since 4.0.0
 * @category References
...
/**
 * Creates an `HttpClient` layer and merges the layer construction context into client response
 * effects.
 *
 * @since 4.0.0
 * @category Layers
