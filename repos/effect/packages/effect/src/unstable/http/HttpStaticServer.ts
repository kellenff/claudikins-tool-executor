 ...
 *
 * `HttpStaticServer` turns request paths into file responses under a configured root directory. It
 * can be used as an application value or mounted onto an `HttpRouter`, and it handles index files,
 * optional single-page application fallback, MIME type headers, cache-control headers, byte ranges,
 * and conditional `304 Not Modified` responses.
 *
 * @since 4.0.0
 */
...
> = Effect.fnUntraced(function* (options) {
  ...
  const serveFile: (
    ...
    filePath: string,
    fileSize?: number,
  ) => Effect.Effect<HttpServerResponse.HttpServerResponse, HttpServerError.HttpServerError> =
    Effect.fnUntraced(function* (request, filePath, fileSize) {
      ...
      const resolvedFileSize =
        rangeHeader === undefined
          ? undefined
          : (fileSize ??
            Number((yield* handlePlatformError(request, fileSystem.stat(filePath))).size));
      const parsedRange =
