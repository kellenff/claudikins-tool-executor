 ...
 *
 * `Multipart` turns incoming byte streams into typed form parts. Text parts become decoded fields,
 * while upload parts stay as streamed files until they are collected or written to scoped temporary
 * files. The persisted representation can then be decoded with schemas for handlers that receive
 * fields and uploaded files together. This module also includes multipart error types, schema
 * helpers for persisted files, and parser limit settings.
 *
 * @since 4.0.0
 */
...
export declare namespace Part {
   ...
   * **Details**
   *
   * It provides the multipart type identifier, tag, and inspectable behavior shared by fields,
   * files, and persisted files.
   *
   * @since 4.0.0
   * @category Models
   */
 ...
 * **Details**
 *
 * The `key` is the field name, `contentType` is the part media type, and `value` is the decoded
 * field content.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Gotchas**
 *
 * The file content is exposed as a byte stream. `contentEffect` collects the full file into memory
 * and should be used only when the file size is acceptable.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The `path` points to the persisted file while the scope used to persist the multipart data
 * remains open.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * It identifies parser and limit failures such as oversized files or fields, too many parts, total
 * body size limits, parse errors, and internal errors.
 *
 * @since 4.0.0
 * @category Errors
 */
 ...
 * **Details**
 *
 * The `reason` field contains the concrete `MultipartErrorReason`. When used as a server response,
 * parse errors render as `400`, limit errors as `413`, and internal errors as `500`. Multipart
 * errors are ignored by the error reporter.
 *
 * @since 4.0.0
 * @category Errors
 */
  ...
  implements HttpServerRespondable.Respondable
{
   ...
   * **Details**
   *
   * Parse errors produce `400`, size and part-count limits produce `413`, and internal errors
   * produce `500`.
   *
   * @since 4.0.0
   */
 ...
 * **Details**
 *
 * The encoded form is a one-element file array, while the decoded value is the single
 * `PersistedFile`.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Details**
 *
 * The returned function decodes an unknown input into the schema output and fails with
...
/**
 * Builds the low-level multipart parser configuration from request headers and the current fiber
 * context.
 *
 * **Details**
 *
 * Parser limits are read from the multipart references, including maximum parts, field size, file
 * size, total body size, and field MIME type overrides.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The channel consumes non-empty batches of `Uint8Array` chunks and emits non-empty batches of
 * parsed `Part` values, failing with `MultipartError` for parser and limit failures.
 *
 * @since 4.0.0
 * @category Parsers
 */
 ...
 * **Details**
 *
 * Text fields are collected as strings, and file parts are written to files in a scoped temporary
 * directory.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * The context can provide maximum part count, field size, file size, total body size, and MIME
 * types that should be parsed as fields.
 *
 * @since 4.0.0
 * @category References
 */
...
export declare namespace withLimits {
   ...
   * **Details**
   *
   * These settings control maximum part count, field size, file size, total body size, and MIME
   * types that should be treated as fields instead of files.
   *
   * @since 4.0.0
   * @category Fiber refs
   */
...
/**
 * Context reference for MIME type fragments that should be parsed as multipart fields instead of
 * files.
 *
 * **Details**
 *
