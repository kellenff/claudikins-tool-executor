 ...
 *
 * This module converts Effect `Tracer.Span` values into outbound trace headers and decodes inbound
 * propagation headers into `Tracer.ExternalSpan` parents. HTTP clients use it to continue the
 * current span across outgoing requests, and server middleware uses it to parent request spans from
 * upstream services.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Returns `Option.none` when the headers do not contain a supported or valid trace context.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The generated headers include both compact B3 (`b3`) and W3C `traceparent` formats.
 *
 * @since 4.0.0
 * @category Encoding
 */
 ...
 * **Details**
 *
 * W3C `traceparent` is tried first, followed by compact B3 (`b3`) and then multi-header B3
 * (`x-b3-*`).
 *
 * @since 4.0.0
 * @category Decoding
 ...
 * **Details**
 *
 * Returns `Option.none` when the header is missing or does not contain trace and span identifiers.
 *
 * @since 4.0.0
 * @category Decoding
 */
 ...
 * **Details**
 *
 * The decoder reads `x-b3-traceid`, `x-b3-spanid`, and optional `x-b3-sampled` headers.
 *
 * @since 4.0.0
 * @category Decoding
 */
