 ...
 *
 * `HttpRouter` collects routes and middleware while an application layer is being built. Once the
 * router is complete, it handles each `HttpServerRequest` by finding a matching route and producing
 * an `HttpServerResponse`. The module also includes helpers for route definitions, prefixes,
 * parameters, request decoding, CORS, and running the router.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * An `HttpRouter` can add routes, apply path prefixes, install global middleware, and expose the
 * registered routes as an Effect that handles the current server request.
 *
 * @since 4.0.0
 * @category HttpRouter
 */
...
/**
 * Service tag for the HTTP router used while constructing an HTTP application. Route and middleware
 * layers require this service to register themselves with the router.
 *
 * @since 4.0.0
 * @category HttpRouter
 */
 ...
 * **Details**
 *
 * The returned router accepts route and middleware registrations and later routes the current
 * `HttpServerRequest` to the matching `HttpServerResponse`.
 *
 * @since 4.0.0
 * @category HttpRouter
 */
          ...
          middleware.length === 0
            ...
            : makeRoute({
                ...
                handler: applyMiddleware(
                  routes[i].handler as Effect.Effect<HttpServerResponse.HttpServerResponse>,
                ),
              });
        if (route.method === "*") {
 ...
 * **Details**
 *
 * The value is passed to the route matcher when an `HttpRouter` is created and defaults to an empty
 * configuration.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use to read captured path parameters and route metadata while handling a request matched by the
 * router.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * The input passed to the schema includes the request method, URL, headers, cookies, path
 * parameters, search parameters, and parsed JSON body. The effect fails if the body cannot be
 * parsed or the schema decode fails.
 *
 * @since 4.0.0
 * @category Schemas
 */
...
> => {
  const parse = Schema.decodeUnknownEffect(schema);
  return Effect.contextWith(
    (context: Context.Context<HttpServerRequest.ParsedSearchParams | RouteContext>) => {
      const searchParams = Context.get(context, HttpServerRequest.ParsedSearchParams);
      const routeContext = Context.get(context, RouteContext);
...
export const add = <E = never, R = never>(
  ...
  never,
  never,
  HttpRouter | Request.From<"Requires", Exclude<R, Provided>> | Request.From<"Error", E>
 ...
 * **Details**
 *
 * The returned effect handles the current `HttpServerRequest` in the current `Scope`; route request
 * markers are converted into the ordinary requirements of the returned handler.
 *
 * @since 4.0.0
 * @category HttpRouter
 */
 ...
 * **Details**
 *
 * A route pairs an HTTP method and path pattern with a response handler, plus metadata used for
 * prefix handling and interruptibility.
 *
 * @since 4.0.0
 * @category Route
 */
 ...
 * **Details**
 *
 * The handler may be a static response, an effect that produces a response, or a function from the
 * current request to a response effect. Set `uninterruptible` to prevent the route handler from
 * being made interruptible while it runs.
 *
 * @since 4.0.0
 * @category Route
 */
...
/**
 * Path pattern accepted by the router. Routes must use an absolute path beginning with `/` or the
 * wildcard `*`.
 *
 * @since 4.0.0
 * @category PathInput
 ...
 * **Details**
 *
 * Trailing slashes are removed from the prefix; `/` becomes the prefix itself and `*` becomes a
 * wildcard route under the prefix.
 *
 * @since 4.0.0
 * @category PathInput
 */
 ...
 * **Details**
 *
 * The prefix is also tracked on the route so that, when the route handles a request, the matched
 * prefix can be removed from the request URL seen by the handler.
 *
 * @since 4.0.0
 * @category Route
 */
...
export declare namespace Request {
  ...
  /**
   * Extracts the payload types from request-level markers that have the supplied kind.
   *
   * @since 4.0.0
   * @category Request types
   */
  ...
  /**
   * Removes request-level markers from a union, leaving only ordinary requirement or error types.
   *
   * @since 4.0.0
   * @category Request types
   */
 ...
 * **Details**
 *
 * Its `layer` can be provided to route layers, and `combine` composes middleware while tracking
 * provided services, handled errors, and remaining requirements at the type level.
 *
 * @since 4.0.0
 * @category Middleware
 */
 ...
 *
 * If you want to create a middleware that applies globally to all routes, pass the `global` option
 * as `true`.
 *
  ...
  options?.global
    ...
    : new MiddlewareImpl(
        Effect.isEffect(middleware)
          ? Layer.effectContext(
              Effect.map(middleware, (fn) => Context.makeUnsafe(new Map([[fnContextKey, fn]]))),
            )
...
export declare namespace middleware {
   ...
   * **Details**
   *
   * It builds either a route-scoped `Middleware` or, when `global` is `true`, a layer that installs
   * middleware for all routes. The type tracks provided services, handled errors, middleware
  ...
  /**
   * Function that transforms an HTTP response effect into another HTTP response effect.
   *
   * @since 4.0.0
   * @category Middleware
   */
...
>(
  ...
  options?: {
     ...
     * **Gotchas**
     *
     * This middleware is applied to the entire HTTP server chain, including the sending of the
     * response. Changes to the response are not reflected in the final response sent to the client.
     * Use `HttpRouter.middleware` when middleware must modify the response.
     */
    readonly middleware?: (
      effect: Effect.Effect<
        HttpServerResponse.HttpServerResponse,
 ...
 * **Details**
 *
 * The result contains a `handler` function that converts Web `Request` values to Web `Response`
 * values and a `dispose` function for releasing the layer resources.
 *
 * @since 4.0.0
 * @category Server
 */
