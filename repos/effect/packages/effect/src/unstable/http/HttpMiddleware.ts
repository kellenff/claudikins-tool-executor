 ...
 *
 * A middleware is a function from one HTTP server app effect to another. The app runs with the
 * current `HttpServerRequest` in its context, so middleware can inspect or rewrite the request,
 * provide request-scoped services, attach hooks before the response is sent, or observe the app
 * exit. This module includes middleware for response logging, server tracing, forwarded proxy
 * headers, parsed search parameters, and CORS response headers.
 *
 * @since 4.0.0
 */
...
/**
 * Creates a layer that disables server-side tracing for requests whose URL exactly matches one of
 * the supplied URLs.
 *
 * @since 4.0.0
 * @category Tracer
...
/**
 * Middleware that logs sent HTTP responses with request method, request URL, and response status
 * annotations.
 *
 * @since 4.0.0
 * @category Logger
...
/**
 * Middleware that creates a server trace span for each request and records request and response
 * HTTP attributes.
 *
 * @since 4.0.0
 * @category Tracer
...
/**
 * Middleware that trusts `X-Forwarded-Host` and `X-Forwarded-For`, updating the request host header
 * and remote address.
 *
 * @since 4.0.0
 * @category Proxying
...
/**
 * Middleware that parses the current request URL's search parameters and provides them as
 * `ParsedSearchParams`.
 *
 * @since 4.0.0
...
/**
 * Middleware that handles CORS preflight requests and adds configured CORS headers to HTTP
 * responses.
 *
 * @since 4.0.0
 * @category CORS
