 ...
 *
 * `HttpPlatform` is the boundary between the portable HTTP response model and the runtime that
 * knows how to stream bytes from the host platform. Server code uses this service when it needs to
 * return local files, static assets, downloads, byte ranges, or Web `File`-like values without
...
/**
 * Creates an `HttpPlatform` service from platform-specific file response constructors, using
 * `FileSystem` and `Etag.Generator`.
 *
 * @since 4.0.0
...
/**
 * Provides the default `HttpPlatform` implementation for serving file paths and `File`-like values
 * as streamed HTTP responses.
 *
 * **Details**
 *
 * The layer uses the `FileSystem` and weak ETag services to add file metadata headers such as
