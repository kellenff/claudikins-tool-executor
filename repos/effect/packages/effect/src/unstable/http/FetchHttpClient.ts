 ...
 *
 * This module provides an `HttpClient` layer that executes requests through a Web Fetch API
 * implementation. It is the transport to use in browsers, edge runtimes, and Node.js environments
 * where `globalThis.fetch` is available, or anywhere a compatible fetch function can be supplied.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to provide default credentials, cache, redirect, integrity, or other fetch options for
 * outgoing HTTP requests.
 *
 * **Details**
 *
 * Request-specific method, headers, body, and abort signal are supplied by the client when a
 * request is executed.
 *
 * @since 4.0.0
 * @category Services
 ...
 * **When to use**
 *
 * Use when an Effect program should execute `HttpClient` requests through the platform `fetch`
 * implementation, especially in browser, edge, or Node.js runtimes with `globalThis.fetch`.
 *
 * **Details**
 *
 * The layer uses the current `Fetch` reference and optional `RequestInit` service for each request.
 * Request-specific method, headers, body, and abort signal are supplied by the client and override
 * matching `RequestInit` fields.
 *
 * **Gotchas**
 *
 * Fetch behavior comes from the runtime's implementation, so CORS, cookies, redirects, abort
 * handling, and streaming support can vary by platform. Stream request bodies are sent as Web
 * streams with `duplex: "half"`, and any `content-length` header is removed before calling
 * `fetch`.
 *
 * @since 4.0.0
 * @category Layers
