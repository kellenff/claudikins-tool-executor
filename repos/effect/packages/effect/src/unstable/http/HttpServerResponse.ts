 ...
 *
 * An `HttpServerResponse` stores the status, optional status text, headers, cookies, and body that
 * the server runtime later turns into a platform response such as a Web `Response`. This module
 * includes constructors for common response bodies, helpers for updating response data, file
 * response support through `HttpPlatform`, and conversions to or from Web and Effect HTTP client
 * responses.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * A response contains a status, optional status text, headers, cookies, and an HTTP body that can
 * later be converted to platform-specific response types.
 *
 * @since 4.0.0
 * @category Models
 */
...
export declare namespace Options {
  /**
   * Response options for constructors whose body determines its own content type and content
   * length.
   *
   * @since 4.0.0
   * @category Options
  ...
  /**
   * Response options for constructors that allow overriding the content type while deriving the
 ...
 * **Details**
 *
 * Passing a string returns a response directly. Using it as a template tag returns an effect so
 * interpolated values can be rendered with their required services and errors.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The template is encoded as a byte stream and can use streaming interpolated values from the
 * current context.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * The body is serialized with `JSON.stringify`; serialization errors are captured as
 * `HttpBodyError` failures.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The returned function encodes the value with the supplied schema before serializing it as JSON,
 * and can fail with `HttpBodyError` if schema encoding or JSON serialization fails.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **When to use**
 *
 * Use when the response body is known to be JSON-serializable and you need a synchronous
 * `HttpServerResponse`.
 *
 * **Gotchas**
 *
...
/**
 * Creates a response from URL parameters using the `application/x-www-form-urlencoded` content type
 * by default.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use when you want to pass through a body value already understood by the underlying runtime, such
 * as a Web `Response`, `Blob`, or `ReadableStream`, for later platform conversion.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The effect requires `HttpPlatform`, can fail with a platform error, and supports options for
 * status, headers, offset, and byte range.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * The effect requires `HttpPlatform` and supports options for status, headers, offset, and byte
 * range.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * Returns an effect because cookie encoding can fail. The original response is not mutated; the
 * effect succeeds with a response containing the updated cookie set.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **When to use**
 *
 * Use when you need to set one trusted cookie and want encoding failures to throw instead of being
 * represented as `CookiesError` failures.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Sets an expired cookie on an `HttpServerResponse`, throwing if the expiration cookie cannot be
 * encoded.
 *
 * **When to use**
 *
 * Use when you need to expire one trusted cookie and want encoding failures to throw instead of
 * being represented as `CookiesError` failures.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **Details**
 *
 * The original response is not mutated; a new response is returned with the callback result as its
 * cookie collection.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * The original response is not mutated; a new response is returned with the merged cookie
 * collection.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * Each input entry contains a cookie name, value, and optional cookie options. The returned effect
 * fails with `CookiesError` if any cookie cannot be encoded.
 *
 * @since 4.0.0
 * @category Combinators
 */
 ...
 * **When to use**
 *
 * Use when you need to set multiple trusted cookies and want encoding failures to throw instead of
 * being represented as `CookiesError` failures.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * When the body carries a content type or content length, the returned response includes the
 ...
 * **Details**
 *
 * Cookies are appended as `Set-Cookie` headers. Stream bodies are converted using the supplied
 * context, and `withoutBody` can be used for responses such as HEAD responses.
 *
 * @since 4.0.0
 * @category Converting
 */
export const toWeb = (
  ...
  switch (body._tag) {
    ...
    case "Stream": {
      return new Response(
        Stream.toReadableStreamWith(body.stream, options?.context ?? Context.empty()),
        {
          status: response.status,
          statusText: response.statusText!,
          headers,
  ...
  private get bytes(): Effect.Effect<Uint8Array, HttpClientError.HttpClientError> {
    ...
    switch (body._tag) {
      ...
      case "Raw": {
        ...
        return Effect.tryPromise({
          try: () =>
            new Response(rawBody as any).arrayBuffer().then((buffer) => new Uint8Array(buffer)),
          catch: (cause) => this.decodeError(cause),
        });
      }
      case "FormData": {
        return Effect.tryPromise({
          try: () =>
            new Response(body.formData).arrayBuffer().then((buffer) => new Uint8Array(buffer)),
          catch: (cause) => this.decodeError(cause),
        });
      }
    }
 ...
 * **Details**
 *
 * The response body is streamed from the client response. `Set-Cookie` headers are removed from the
 * header map and represented in the response cookie collection.
 *
 * @since 4.0.0
 * @category Converting
 */
 ...
 * **Details**
 *
 * `Set-Cookie` headers are parsed into the response cookie collection and removed from the header
 * map. A present Web body is exposed as a stream body.
 *
 * @since 4.0.0
 * @category Converting
 */
