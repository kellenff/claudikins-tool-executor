 ...
 *
 * This module turns effects that produce `HttpServerResponse` values into Web `Request` handlers
 * and other server adapters. It also applies middleware, converts failures into responses, runs
 * hooks before a response is sent, and manages request scopes for streamed responses.
 *
 * @since 4.0.0
 */
...
/**
 * Runs an HTTP server effect, sends the produced response with the supplied handler, and converts
 ...
 * **Gotchas**
 *
 * Use only when another owner will close the scope; otherwise resources attached to the request
 * scope can leak.
 *
 * @since 4.0.0
...
/**
 * Function run with the current request and response just before the response is sent, allowing the
 * response to be replaced or failing with `HttpServerError`.
 *
 * @since 4.0.0
...
/**
 * Converts an HTTP server effect into a Web `Request` handler using the supplied base context and
 * optional middleware.
 *
 * @since 4.0.0
 * @category Converting
...
/**
 * Builds a Web `Request` handler from a layer and handler factory, returning the handler with a
 * `dispose` function for the layer scope.
 *
 * @since 4.0.0
 * @category Converting
 */
...
/**
 * Builds a Web `Request` handler for an HTTP server effect using a layer to provide its services,
 * returning the handler with a `dispose` function.
 *
 * @since 4.0.0
 * @category Converting
 */
