 ...
 *
 * A `DurableDeferred` has a stable name and schemas for the value that will be recorded later.
 * Workflows can await it, suspend when no result exists yet, and resume after its result is
 * recorded. Tokens identify the workflow name, execution id, and deferred name so external code can
 * complete the correct wait point later.
 *
 * @since 4.0.0
 */
...
/**
 * Named durable deferred value whose completion is persisted by the workflow engine and encoded
 * with success and error schemas.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a named durable deferred with optional success and error schemas for persisted
 * completion.
 *
 * @since 4.0.0
...
export {
  /**
   * Waits for the durable deferred, suspending the current workflow when no persisted completion is
   * available.
   *
   * @since 4.0.0
...
/**
 * Runs an effect and records its exit into the durable deferred, resuming workflows that are
 * waiting on that deferred.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Runs effects as a durable race, returning a previously persisted result when present or
 * completing a named deferred with the first result.
 *
 * @since 4.0.0
 * @category Racing
 */
...
/**
 * Schema for a decoded durable deferred token containing the workflow name, execution ID, and
 * deferred name.
 *
 * @since 4.0.0
 * @category Token
  ...
  static readonly FromString = Schema.String.pipe(
    ...
    Schema.decodeTo(TokenParsed, {
      ...
      encode: SchemaGetter.transform(
        (parsed) => [parsed.workflowName, parsed.executionId, parsed.deferredName] as const,
      ),
    }),
  );
...
/**
 * Creates a token for a durable deferred using the current workflow instance's workflow name and
 * execution ID.
 *
 * @since 4.0.0
 * @category Token
...
/**
 * Creates a durable deferred token by deriving the workflow execution ID from the supplied workflow
 * payload.
 *
 * @since 4.0.0
 * @category Token
...
/**
 * Completes the durable deferred identified by a token with the supplied exit, encoding the result
 * through the deferred schemas.
 *
 * @since 4.0.0
 * @category Combinators
 */
export const done: {
  ...
  2,
  Effect.fnUntraced(function* <Success extends Schema.Constraint, Error extends Schema.Constraint>(
    self: DurableDeferred<Success, Error>,
    options: {
      readonly token: Token;
      readonly exit: Exit.Exit<Success["Type"], Error["Type"]>;
