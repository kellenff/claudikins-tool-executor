 ...
 *
 * `make` creates a `DurableClock` with a name, duration, and deferred wake-up signal. `sleep`
 * ignores zero durations, runs short sleeps through an in-memory activity, and schedules longer
 * sleeps through the `WorkflowEngine` before awaiting the durable deferred tied to the clock.
 *
 * @since 4.0.0
 */
...
/**
 * Represents a durable workflow timer with a name, duration, and deferred completed when the timer
 * wakes.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Waits inside a workflow, using an in-memory activity for durations at or below the threshold and
 * scheduling a durable clock for longer durations.
 *
 * @since 4.0.0
 * @category Sleeping
 */
export const sleep: (options: {
  ...
  readonly duration: Duration.Input;
  /**
   * If the duration is less than or equal to this threshold, the clock will be executed in
   * memory.
   *
   * @default 60 seconds
   */
  readonly inMemoryThreshold?: Duration.Input | undefined;
}) => Effect.Effect<void, never, WorkflowEngine | WorkflowInstance> = Effect.fnUntraced(
  function* (options: {
    readonly name: string;
    readonly duration: Duration.Input;
    readonly inMemoryThreshold?: Duration.Input | undefined;
