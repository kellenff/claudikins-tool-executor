 ...
 *
 * `WorkflowEngine` registers workflow handlers, runs executions, polls results, resumes suspended
 * runs, executes activities, stores durable deferred results, and schedules durable clocks.
 * `WorkflowInstance` holds the runtime state for one workflow run. The in-memory layer is useful
...
/**
 * Service that represents workflow runtimes, responsible for registering and executing workflows
 * and coordinating activities, durable deferreds, interrupts, resumes, and clocks.
 *
 * @since 4.0.0
 * @category Services
 */
export class WorkflowEngine extends Context.Service<
  WorkflowEngine,
  {
    ...
    /**
     * Interrupts a registered workflow unsafely, potentially ignoring compensation finalizers and
     * orphaning child workflows.
     */
    ...
    /** Set the result of a DurableDeferred, and then resume any waiting workflows. */
 ...
 * **When to use**
 *
 * Use to read or update workflow execution, suspension, interruption, lifetime, failure, and
 * activity coordination state inside workflow engine internals.
 *
 * **Details**
 *
 * The service stores the execution ID, workflow definition, long-lived scope, suspension and
 * interruption flags, the stored failure cause, and activity coordination state for a single
 * workflow run.
 *
 * @since 4.0.0
 * @category Services
...
export class WorkflowInstance extends Context.Service<
  WorkflowInstance,
  {
    ...
    /** When SuspendOnFailure is triggered, the cause of the failure is stored here. */
...
/**
 * Low-level workflow engine contract that works with encoded payloads and results before
 * `makeUnsafe` adds typed schema decoding and encoding.
 *
 * @since 4.0.0
 * @category Encoded
 */
 ...
 * **When to use**
 *
 * Use when wiring a trusted low-level workflow engine implementation into the typed
 ...
 * **Gotchas**
 *
 * This layer keeps state only in memory and is not suitable for production workflows that require
 * durability.
 *
 * @since 4.0.0
 * @category Layers
