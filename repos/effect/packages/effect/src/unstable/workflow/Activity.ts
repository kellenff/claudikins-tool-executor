 ...
 *
 * An `Activity` is an `Effect` with a stable name and schemas for its success and error values.
 * `make` wraps an effect so the `WorkflowEngine` can execute it, store its result, or replay that
 * result during a workflow run. This module also includes helpers for retry attempts, idempotency
 * keys, and durable races.
 *
 * @since 4.0.0
 */
...
/**
 * Durable workflow activity that behaves as an `Effect` and records its name, result schemas,
 * annotations, and encoded execution form for the workflow engine.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Type-erased activity shape for APIs that only need the activity identity, name, annotations, and
 * encoded execution.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Creates a workflow activity from an effect, using the provided schemas to encode successes and
 * failures for durable execution.
 *
 * @since 4.0.0
...
/**
 * Computes a deterministic activity idempotency key from the current workflow execution ID, the
 * supplied name, and optionally the current attempt.
 *
 * @since 4.0.0
...
/**
 * Runs a non-empty collection of activities as a durable race and returns the first completed
 * success or failure using unioned success and error schemas.
 *
 * @since 4.0.0
 * @category Racing
 */
