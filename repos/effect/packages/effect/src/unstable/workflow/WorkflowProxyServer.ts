 ...
 *
 * `layerHttpApi` connects the HTTP API group created by `WorkflowProxy` to the supplied workflows.
 * `layerRpcHandlers` does the same for the generated RPC definitions. Both layers route execute,
 * discard, and resume requests to the matching workflow operation, while the `WorkflowEngine` and
 * workflow handler services stay on the server side.
 *
 * @since 4.0.0
 */
...
/**
 * Creates handlers for a workflow HTTP API group, wiring execute, discard, and resume endpoints to
  ...
  HttpApiBuilder.group(
    ...
    Effect.fnUntraced(function* (handlers: any) {
      for (const workflow_ of workflows) {
        ...
        handlers = handlers
          .handle(workflow._tag, ({ payload }: { payload: any }) =>
          ...
          .handle(workflow._tag + "Discard", ({ payload }: { payload: any }) =>
          ...
          .handle(workflow._tag + "Resume", ({ payload }: { payload: any }) =>
...
/**
 * Creates RPC handlers for the supplied workflows, wiring execute, discard, and resume RPCs to
 * workflow operations.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Union of RPC handler services required to serve the generated workflow execute, discard, and
 * resume RPCs.
 *
 * @since 4.0.0
 * @category Services
