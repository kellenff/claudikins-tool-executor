/**
 * Durable workflow queues delegate work to persisted background workers and resume the waiting
 * workflow with the worker result.
 *
 * A workflow calls `process` to encode a payload, offer it to a named `PersistedQueue`, attach a
 * `DurableDeferred` token, and suspend. A worker created with `makeWorker` or `worker` takes the
 * item, runs the handler, and records the handler's `Exit` through that token so the original
 * workflow can continue with the typed success or error.
 *
 * @since 4.0.0
 */
...
/**
 * Durable workflow queue definition containing a payload schema, idempotency key, and deferred used
 * to await worker results.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a `DurableQueue` that waits for persisted items to finish processing using a
 * `DurableDeferred`.
 *
