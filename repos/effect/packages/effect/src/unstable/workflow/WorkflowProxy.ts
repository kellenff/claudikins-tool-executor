 ...
 *
 * Given one or more `Workflow` values, `toRpcGroup` creates the RPC definitions for clients and
 * servers, while `toHttpApiGroup` creates HTTP POST endpoints that can be mounted in an API. Each
 * workflow gets execute, discard, and resume operations, so callers can start a workflow or resume
 * a suspended run by `executionId` without importing the workflow handler directly.
 *
 * @since 4.0.0
 */
...
>(
  ...
  for (const workflow_ of workflows) {
    ...
    rpcs.push(
      ...
      Rpc.make(`${prefix}${workflow._tag}Resume`, { payload: ResumePayload }).annotateMerge(
...
/**
 * Maps each workflow to the RPC definitions generated for execute, discard, and resume operations.
 *
 * @since 4.0.0
 * @category Converting
 */
...
/**
 * Maps each workflow to the HTTP API endpoints generated for execute, discard, and resume
 * operations.
 *
 * @since 4.0.0
 * @category Converting
