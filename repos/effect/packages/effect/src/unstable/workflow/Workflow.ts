 ...
 *
 * A `Workflow` has a stable tag, schemas for payload, success, and failure, and an idempotency key
 * used to derive execution ids. Workflow definitions can be executed, discarded, polled,
 * interrupted, resumed, and registered with a handler layer. This module also includes workflow
 * result types, compensation and cleanup helpers, suspension support, and settings for defect
 * capture or failure suspension.
 *
 * @since 4.0.0
 */
...
/**
 * Durable workflow definition with typed payload, success, and error schemas plus operations for
 * execution, polling, interruption, resumption, and registration.
 *
 * @since 4.0.0
 * @category Models
 */
...
> {
   ...
   * **Details**
   *
   * The compensation finalizer is called if the entire workflow fails, allowing you to perform
   * cleanup or other actions based on the success value and the cause of the workflow failure.
   *
   * **Gotchas**
   *
   * Compensation finalizers are only registered for top-level effects in the workflow and do not
   * work for nested activities.
   */
...
/**
 * Type-erased workflow shape for APIs that operate on workflows without preserving their specific
 * payload, success, or error types.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Type-erased workflow shape that also exposes executable operations needed by workflow proxy and
 * engine helpers.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Computes the schema services required by handlers that decode workflow payloads and encode
 * workflow results.
 *
 * @since 4.0.0
 * @category Models
...
const Proto = {
  ...
  execute<const Discard extends boolean = false>(
    ...
    return Effect.suspend(() => {
      ...
      return Effect.flatMap(EngineTag, (engine) =>
    ...
    }).pipe(Effect.withSpan(`${this._tag}.execute`, {}, { captureStackTrace: false })) as any;
  },
  ...
  executionId(this: AnyWithProps, payload: any) {
    return Effect.flatMap(Effect.orDie(this.payloadSchema.makeEffect(payload)), (payload) =>
      makeExecutionIdFromPayload(this, payload),
    );
  },
...
/**
 * Creates a durable workflow definition with schemas, annotations, and deterministic execution IDs
 * derived from the workflow tag and idempotency key.
 *
 * @since 4.0.0
...
> =>
  ...
  >({
    ...
    payloadSchema: (Schema.isSchema(options.payload)
      ? options.payload
      : Schema.Struct(options.payload as any)) as Payload extends Schema.Struct.Fields
...
/**
 * Represents a suspended workflow execution, optionally carrying the cause that triggered
 * suspension.
 *
 * @since 4.0.0
 * @category Results
...
/**
 * Runs an effect as a workflow execution and converts its outcome into a `Result`, handling
 * suspension, defect capture, interruption, and workflow scope finalization.
 *
 * @since 4.0.0
 * @category Results
 */
...
/**
 * Wraps an activity-like effect so workflow suspension waits for currently running activities to
 * finish or suspend.
 *
 * @since 4.0.0
 * @category Results
...
/**
 * Provides the workflow scope to the given effect, and closes the scope only when the workflow
 * execution fully completes.
 *
 * @since 4.0.0
 * @category Resource management
 */
...
/**
 * Adds an exit finalizer to the current workflow scope, preserving the services available when the
 ...
 * **When to use**
 *
 * Use when a top-level workflow step needs compensating cleanup if the overall workflow later fails
 ...
 * **Gotchas**
 *
 * Compensation finalizers are only registered for top-level effects in the workflow and do not work
 * for nested activities.
 *
 * @since 4.0.0
...
/**
 * Marks a workflow instance as suspended and interrupts the current fiber to stop execution until
 * it is resumed.
 *
 * @since 4.0.0
 * @category Results
...
/**
 * Captures defects for a workflow and includes them in the result of the workflow or its
 * activities.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * The suspended execution can later be resumed with the workflow's `resume` method, for example
 * `MyWorkflow.resume(executionId)`.
 *
 * @since 4.0.0
 * @category Annotations
 */
