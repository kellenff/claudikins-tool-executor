 ...
 *
 * `make` fixes the variant names and default variant, then lets callers define fields that are
 * shared by all variants or specific to some variants. From those definitions it can create schema
 * classes, unions, extracted struct schemas, and helpers for changing fields across variants.
 *
 * @since 4.0.0
 */
...
/**
 * Pipeable container of schema fields that can be extracted into per-variant `Schema.Struct`
 * schemas.
 *
 * @since 4.0.0
 * @category Models
...
export declare namespace Struct {
  ...
  /**
   * Field map accepted by a variant struct, where each property may be a schema, a variant field, a
   * nested struct, or `undefined`.
   *
   * @since 4.0.0
   * @category Models
   */
  ...
  /**
   * Type-level validation that every variant field in a struct only uses variants from the
   * configured variant set.
   *
   * @since 4.0.0
   * @category Models
   */
...
/**
 * Computes the `Schema.Struct` field map for a variant by selecting matching field schemas and
 * recursively extracting nested structs.
 *
 * @since 4.0.0
 * @category Extractors
 */
...
/**
 * Schema class type returned by variant class constructors, combining the default variant schema
 * with access to the original variant fields.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Schema type whose constructor can use an effectful default unless a value is explicitly branded
 * with `Override`.
 *
 * @since 4.0.0
 * @category Overrideable
...
/**
 * Wraps a schema with an effectful constructor default while allowing explicit values to be marked
 * with `Override`.
 *
 * @since 4.0.0
 * @category Overrideable
