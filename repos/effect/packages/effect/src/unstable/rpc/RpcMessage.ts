 ...
 *
 * `RpcMessage` is the protocol vocabulary below `RpcClient` and `RpcServer`. It defines decoded
 * messages for in-process channels and encoded messages for transport boundaries, so custom
 * protocols can move the same request, streaming, acknowledgement, interrupt, keepalive, and defect
 * signals as the built-in HTTP, socket, worker, and test transports.
 *
 * @since 4.0.0
 */
...
/**
 * A branded request identifier used to correlate RPC requests, responses, chunks, acknowledgements,
 * and interrupts.
 *
 * @since 4.0.0
 * @category Request
...
/**
 * The transport-encoded RPC request envelope, including the string request id, RPC tag, encoded
 * payload, headers, and optional trace context.
 *
 * @since 4.0.0
 * @category Request
 */
...
/**
 * The decoded RPC request envelope for an RPC union, carrying a branded request id, typed RPC tag,
 * decoded payload, headers, and optional trace context.
 *
 * @since 4.0.0
 * @category Request
 */
...
/**
 * A decoded request to interrupt an in-flight RPC, carrying the request id and interrupting fiber
 * ids.
 *
 * @since 4.0.0
 * @category Request
...
/**
 * A client-to-server message indicating that the client has finished sending input for the current
 * connection or request batch.
 *
 * @since 4.0.0
 * @category Request
 */
...
/**
 * The transport-encoded response message containing a non-empty batch of stream chunk values for a
 * request.
 *
 * @since 4.0.0
 * @category Response
...
/**
 * The decoded response message containing a non-empty batch of stream chunk values for a specific
 * client and request.
 *
 * @since 4.0.0
 * @category Response
...
/**
 * The transport representation of an RPC `Exit`, encoding success values or a failure cause made of
...
/**
 * A server-to-client protocol message reporting a client protocol error to all affected in-flight
 * requests.
 *
 * @since 4.0.0
 * @category Response
...
/**
 * The transport-encoded server defect message used for protocol-level defects that affect the
 * client connection.
 *
 * @since 4.0.0
 * @category Response
...
/**
 * Creates an encoded terminal response for a request whose exit is a defect encoded with
 * `Schema.Defect()`.
 *
 * @since 4.0.0
 * @category Response
