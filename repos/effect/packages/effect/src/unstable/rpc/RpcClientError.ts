 ...
 *
 * `RpcClientError` is the error type generated clients use when a call fails before a remote
 * handler can return its declared typed error. Its `reason` covers built-in transport failures from
 * HTTP, sockets, and workers, plus `RpcClientDefect` values for malformed or incompatible protocol
 * data.
 *
 * @since 4.0.0
 */
...
/**
 * Represents a client-side RPC defect, such as a protocol violation or decoding failure, with a
...
/**
 * Error wrapper for RPC client failures, including worker, socket, HTTP client, and client protocol
 * defect failures.
 *
 * @since 4.0.0
 * @category Errors
