 ...
 *
 * This module connects typed handlers for an `RpcGroup` to a server `Protocol`. It receives client
 * messages, decodes request payloads, runs matching handlers and middleware, tracks in-flight
 * requests, handles acknowledgements and interrupts, and sends responses back to clients. It also
 * provides constructors and layers for decoded messages, HTTP, WebSocket, sockets, stdio, and
 * worker runner protocols.
 *
 * @since 4.0.0
 */
...
/**
 * The decoded RPC server boundary, accepting client messages for a client id and allowing that
...
/**
 * Creates an RPC server for an already-decoded message channel, running handlers for a group and
 * sending decoded server responses through `onFromServer`.
 *
 * @since 4.0.0
 * @category Server
 */
...
> = Effect.fnUntraced(function* <Rpcs extends Rpc.Any>(
  ...
  const handleRequest = (
    ...
    if (enableTracing) {
      const parentSpan = requestFiber.context.mapUnsafe.get(Tracer.ParentSpan.key) as
        | Tracer.AnySpan
        | undefined;
...
/**
 * Runs an RPC server for a group using the current server `Protocol`, decoding requests, invoking
 * handlers, encoding responses, and managing in-flight request lifetime.
 *
 * @since 4.0.0
 * @category Server
 */
    ...
    switch (request._tag) {
      case "Request": {
        ...
        return Effect.matchEffect(
          ...
          {
            onFailure: (error) =>
              sendRequestDefect(client, requestId, schemas.encodeDefect, error.issue.toString()),
            onSuccess: (payload) => {
              client.schemas.set(
                requestId,
                supportsTransferables
...
/**
 * Provides a scoped layer that starts an RPC server for a group using the current server
 * `Protocol`.
 *
 * @since 4.0.0
 * @category Server
...
/**
 * Defines the service interface for an RPC server transport, responsible for receiving encoded
 * client messages, sending encoded responses, tracking clients, and declaring transport
 * capabilities.
 *
 * **When to use**
 *
 * Use to provide the transport boundary for RPC servers over HTTP, WebSocket, workers, sockets, or
 * custom protocols.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Creates a server `Protocol` backed by the current `SocketServer`, accepting socket connections
 * and routing decoded RPC messages.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * Creates a websocket server `Protocol` together with an HTTP effect that upgrades the current
 * request to a websocket and attaches it to the protocol.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * Creates a websocket server `Protocol` and registers its upgrade handler as a GET route on the
 * current `HttpRouter`.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Creates an HTTP request/response server `Protocol` together with an HTTP effect that decodes the
 * current request and streams or returns encoded RPC responses.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * Creates an HTTP server `Protocol` and registers its request handler as a POST route on the
 * current `HttpRouter`.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Starts an RPC server for a group and returns the HTTP request/response effect that serves the
 * non-websocket HTTP RPC protocol.
 *
 * @since 4.0.0
 * @category Http app
 */
...
/**
 * Starts an RPC server for a group and returns the HTTP effect that upgrades requests to the
...
/**
 * Creates a server `Protocol` that reads RPC messages from `Stdio.stdin` and writes encoded
 * responses to `Stdio.stdout`.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * Provides a server `Protocol` that reads RPC messages from `Stdio.stdin` and writes encoded
 * responses to `Stdio.stdout`.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Creates a server `Protocol` backed by `WorkerRunnerPlatform`, routing worker messages to the RPC
 * server and server responses back to workers.
 *
 * @since 4.0.0
 * @category Protocols
 */
          ...
          return Effect.whileLoop({
            ...
            body() {
              ...
              if (message._tag === "Request" && headers) {
                (message as Types.Mutable<RequestEncoded>).headers = headers.concat(
                  message.headers,
                );
              }
