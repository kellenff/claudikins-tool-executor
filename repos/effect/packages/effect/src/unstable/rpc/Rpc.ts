 ...
 *
 * An `Rpc` describes one remote procedure by recording its tag, payload schema, success schema,
 * error schema, defect schema, middleware, and annotations. Clients and servers read the same
 * declaration, so the procedure contract is independent of the transport used to call it. This
 * module includes constructors, type helpers for deriving client and handler shapes, exit schemas,
 * and handler wrappers for special execution modes.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Defect schemas decode and encode without services and can be constructed from `null`,
 * `undefined`, or an object value.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * An RPC is identified by a tag and carries payload, success, error, defect, middleware, and
 * annotation metadata used by RPC clients and servers.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The handler receives the decoded request plus client, request id, headers, and RPC metadata, and
 * returns either an effectful result or a stream result.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * For streaming RPCs, the immediate exit success is `void` because stream elements are delivered
 * separately.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * For streaming RPCs, this includes both the stream error schema and the RPC error schema;
 * otherwise it is the RPC error schema.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Extracts all schema services required to encode or decode an RPC's payload, success, error, and
 * middleware error schemas.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * This includes payload encoding services and success, error, and middleware error decoding
 * services.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * This includes payload decoding services and success, error, and middleware error encoding
 * services.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Extracts client-side middleware service requirements for middleware marked as required on the
 * client.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Returns an RPC type with additional middleware and the corresponding middleware service
 ...
 * **Details**
 *
 * The function receives the decoded payload and request metadata, and returns the RPC result shape,
 * optionally wrapped with `Wrapper` options.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Returns `true` when the RPC with the specified tag has a streaming success schema, or `never`
 * otherwise.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Streaming RPCs may return a stream or an effect that produces a queue. Other RPCs return an
 * effect that succeeds with the success value or a deferred success value.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Returns an RPC type with the specified string prefix added to its tag while preserving its
 * payload, success, error, middleware, and requirements.
 *
 * @since 4.0.0
 * @category Models
 */
...
const Proto = {
  ...
  setPayload(
    this: AnyWithProps,
    payloadSchema: Schema.Struct<Schema.Struct.Fields> | Schema.Struct.Fields,
  ) {
    return makeProto({
      _tag: this._tag,
 ...
 * **Details**
 *
 * Payload options can be either a schema or struct fields. `stream: true` wraps the success and
 * error schemas in a stream schema and sets the normal error schema to `Schema.Never`. `primaryKey`
 * creates a payload class with a primary key derived from the payload value.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * A custom constructor receives the original success, error, and defect schemas and returns
 * transformed output schemas through `out`.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
export declare namespace Custom {
  ...
  /**
   * Applies a custom constructor definition to concrete success and error schemas and returns its
   * transformed output schema type.
   *
   * @since 4.0.0
   * @category Constructors
   */
 ...
 * **Details**
 *
 * The failure side includes the RPC error schema, middleware error schemas, and stream error schema
 * for streaming RPCs. Streaming RPCs use `Schema.Void` for the exit success value. The schema is
 * cached per RPC definition.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Returns the wrapped response value when the input is an RPC `Wrapper`, or the input itself when
...
/**
 * Maps the value inside an RPC wrapper, preserving wrapper options such as `fork` and
 * `uninterruptible`; unwrapped values are mapped directly.
 *
 * @since 4.0.0
 * @category Wrapping
 */
...
/**
 * Wraps a response Effect or Stream so the RPC server executes it concurrently regardless of the
 * server concurrency setting.
 *
 * @since 4.0.0
 * @category Wrapping
 */
