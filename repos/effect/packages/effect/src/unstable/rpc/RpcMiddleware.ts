 ...
 *
 * A middleware service wraps server handler execution and can also install a client-side wrapper
 * for generated clients. Its metadata records the services provided to downstream handlers, the
 * services required by the middleware implementation, the schema for server-visible failures, the
 * client-only error type, and whether generated clients must require the matching client layer.
 *
 * @since 4.0.0
 */
...
/**
 * The server-side RPC middleware function shape, wrapping a handler effect with access to request
 * metadata and translating provided services into required services.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Marker success type used by RPC middleware to represent successful completion without exposing
 * the handler's concrete success value.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * The client-side RPC middleware function shape, allowing outgoing requests to be inspected or
 * modified before calling `next`.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Marker service requirement indicating that a middleware has a client-side implementation
 * available for an RPC client.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * An erased server-side RPC middleware function, useful when the concrete provided services,
 * errors, and requirements are not needed.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * A type-level carrier for RPC middleware metadata, including provided services, required services,
 * error schema, and client error type.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * The `Context.Service` class shape created for an RPC middleware, including its error schema,
 * service metadata, and client-side requirement marker.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Applies a middleware's service transformation to an RPC environment by removing services the
 * middleware provides and adding services it requires.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * An erased RPC middleware context key whose service value is a server-side middleware function.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Creates a typed RPC middleware service class, with optional service requirements, provided
 * services, error schema, and client-side requirement metadata.
 *
 * @since 4.0.0
...
/**
 * Provides the client-side implementation for an RPC middleware service, capturing the layer's
 * environment and merging it into each middleware invocation.
 *
 * @since 4.0.0
 * @category Client
 */
