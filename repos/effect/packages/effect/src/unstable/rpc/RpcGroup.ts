 ...
 *
 * An `RpcGroup` stores RPC definitions by tag and keeps annotations shared by the group. This
 * module provides helpers for composing groups, applying middleware or annotations, deriving
 * handler types, and turning handler objects into `Context` or `Layer` values used by RPC servers.
 *
 * @since 4.0.0
 */
...
/**
 * A collection of RPC definitions that can be composed, annotated, and converted into server
 * handlers or layers.
 *
 * @since 4.0.0
 * @category Groups
...
export interface RpcGroup<in out R extends Rpc.Any> extends Pipeable {
  ...
  /** Implement the handlers for the procedures in this group, returning a context object. */
...
/**
 * An erased `RpcGroup` type for APIs that only need to know that a value is an RPC group.
 *
 * @since 4.0.0
 * @category Groups
 */
...
/**
 * Computes the services required by a single RPC handler, excluding services provided by middleware
 * and `Scope` where the server supplies it.
 *
 * @since 4.0.0
 * @category Groups
 */
