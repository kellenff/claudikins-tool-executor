 ...
 *
 * `RpcTest` connects a generated client directly to `RpcServer` handlers for the same `RpcGroup`.
 * It uses the no-serialization path, so requests, responses, stream chunks, acknowledgements,
 * interrupts, headers, and middleware metadata travel through the normal client/server machinery
 * without opening HTTP, socket, worker, or serializer infrastructure.
 *
 * @since 4.0.0
 */
...
/**
 * Creates an in-memory RPC client for a group, backed by the group's handlers from the environment
 * and using the no-serialization test transport.
 *
 * @since 4.0.0
