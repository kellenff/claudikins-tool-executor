 ...
 *
 * `RpcSerialization` is the boundary between `RpcMessage` envelopes and the bytes or strings
 * carried by a transport. This module provides built-in serializers for JSON, newline-delimited
 * JSON, JSON-RPC 2.0, and MessagePack, including framed formats that can decode multiple messages
...
/**
 * Service that describes how RPC protocol messages are encoded and decoded, including the content
 * type and whether the serialization format provides message framing.
 *
 * **When to use**
 *
 * Use to provide the serialization boundary shared by RPC clients and servers for a chosen wire
 * format.
 *
 * @since 4.0.0
...
/**
 * A stateful parser for an RPC serialization format, able to decode input chunks into protocol
 * messages and encode messages for transport.
 *
 * @since 4.0.0
 * @category Serialization
 */
...
/**
 * JSON RPC serialization for whole message payloads. It does not include message framing, so it is
 * intended for transports that frame responses themselves.
 *
 * @since 4.0.0
 * @category Serialization
 */
...
/**
 * Serializes RPC protocol messages as newline-delimited JSON, framing each message with a trailing
 * newline.
 *
 * @since 4.0.0
 * @category Serialization
...
/**
 * Creates a JSON-RPC 2.0 serialization for RPC protocol messages without additional message
 * framing.
 *
 * @since 4.0.0
 * @category Serialization
 ...
 * **Details**
 *
 * MessagePack has a more compact binary format compared to JSON and NDJSON. It also has better
