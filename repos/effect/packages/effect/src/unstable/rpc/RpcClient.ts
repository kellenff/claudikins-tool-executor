 ...
 *
 * This module turns RPC definitions from an `RpcGroup` into callable client methods. Each call
 * encodes its payload, sends a message through the active `Protocol`, decodes exits or stream
 * chunks from the server, and routes the response back to the waiting `Effect`, `Stream`, or queue.
 * It also defines the protocol service and includes protocol layers for HTTP, sockets, and
 * workers.
 *
 * @since 4.0.0
 */
...
export declare namespace RpcClient {
  /**
   * Builds an object client type from an RPC union, mapping each RPC tag to a method that accepts
   * the RPC payload and returns either an `Effect` or `Stream` based on the RPC success schema.
   *
   * @since 4.0.0
   * @category Client
   */
  ...
  /**
   * Builds a flattened RPC client function that accepts an RPC tag and payload, returning the
   * corresponding `Effect` or `Stream` for that RPC.
   *
   * @since 4.0.0
   * @category Client
   */
...
/**
 * Creates an RPC client for an already-decoded message channel, returning the client API together
 * with a `write` function for delivering server messages back to the client.
 *
 * @since 4.0.0
 * @category Client
 */
...
/**
 * Creates a schema-aware RPC client for a group using the current client `Protocol`, encoding
 * requests and decoding server responses.
 *
 * @since 4.0.0
 * @category Client
 */
      ...
      case "Exit": {
        ...
        return entry.schemas.decodeExit(message.exit).pipe(
          ...
          Effect.matchCauseEffect({
            ...
            onFailure: (cause) =>
              write({ _tag: "Exit", clientId: 0, requestId, exit: Exit.failCause(cause) }),
          }),
        ) as Effect.Effect<void>;
      }
      case "Defect": {
...
/**
 * Runs an effect with additional RPC client headers, merging them with the current `CurrentHeaders`
 * value for outgoing requests.
 *
 * @since 4.0.0
 * @category Headers
 */
...
/**
 * Defines the service interface for an RPC client transport, responsible for running the receive
 * loop and sending encoded client messages.
 *
 * **When to use**
 *
 * Use to provide the transport boundary for RPC clients over HTTP, WebSocket, workers, sockets, or
 * custom protocols.
 *
 * @since 4.0.0
 * @category Protocols
...
/**
 * Creates a client `Protocol` that sends each RPC request through the supplied `HttpClient` and
 * decodes responses with the current `RpcSerialization`.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * Provides a client `Protocol` backed by `HttpClient`, targeting the configured URL and optionally
 * transforming the client before use.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * Creates a client `Protocol` over the current `Socket`, using the current `RpcSerialization`,
 * connection hooks, ping timeouts, and the configured retry policy.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
const defaultRetryPolicy = Schedule.min([Schedule.exponential(500, 1.5), Schedule.spaced(5000)]);
...
/**
 * Creates a client `Protocol` backed by a pool of workers, routing RPC requests to workers and
 * supporting transferable values when the platform does.
 *
 * @since 4.0.0
 * @category Protocols
 */
...
/**
 * Provides a client `Protocol` backed by a worker pool using the current worker platform and
 * spawner services.
 *
 * @since 4.0.0
 * @category Protocols
