 ...
 *
 * Client and server protocol constructors use these helpers to expose a stable service before the
 * active receiver is installed. Writes made before `run` starts are buffered with their current
 * `Context`, then replayed once the receiver is ready.
 *
 * @since 4.0.0
 */
...
/**
 * Builds a service with a `run` method that buffers writes until `run` installs a writer, replays
 * buffered writes with their original contexts, and restores the previous writer when the run
 * ends.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Builds an RPC client protocol service that tracks active client IDs and buffers server responses
 * per client until that client's `run` handler is installed.
 *
 * @since 4.0.0
 * @category Services
 */
