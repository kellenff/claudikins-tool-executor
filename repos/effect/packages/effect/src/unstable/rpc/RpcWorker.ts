 ...
 *
 * A worker-backed RPC client can send one schema-encoded value before normal RPC requests are
 * handled. This module defines the `InitialMessage` service, a helper for encoding that value while
 * collecting transferables, a layer for providing it to the client protocol, and a decoder for
 * reading it from the worker server protocol.
 *
 * @since 4.0.0
 */
...
/**
 * Context service that supplies the initial RPC worker message as encoded data paired with any
 * transferables that should be posted with it.
 *
 * @since 4.0.0
...
export declare namespace InitialMessage {
  /**
   * Tagged wire representation of an RPC worker initial message after schema encoding.
   *
   * @since 4.0.0
   * @category Initial message
   */
...
/**
 * Runs an effect, encodes its result with the schema's JSON codec, and returns the encoded value
 * together with collected transferables.
 *
 * @since 4.0.0
 * @category Initial message
 */
...
/**
 * Provides the `InitialMessage` service from a schema and build effect, capturing the layer context
 * and dying if schema encoding fails.
 *
 * @since 4.0.0
 * @category Initial message
 */
...
/**
 * Reads the protocol initial message and decodes it with the supplied schema, failing if no initial
 * message is available or decoding fails.
 *
 * @since 4.0.0
 * @category Initial message
 */
