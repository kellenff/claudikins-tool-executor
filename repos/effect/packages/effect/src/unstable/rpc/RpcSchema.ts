 ...
 *
 * This module contains the small pieces of schema metadata that the RPC declaration, client,
 * server, cluster, and reactivity layers share. It marks streamed responses and annotates
 * interruptions that came from a remote client closing or cancelling a request.
 *
 * @since 4.0.0
 */
...
/**
 * A schema marker for RPC streaming responses, storing the success element schema and stream error
 * schema used for encoding and decoding stream chunks.
 *
 * @since 4.0.0
 * @category Streams
 */
...
> extends Schema.BottomLazy<SchemaAST.Declaration, Stream<A, E>> {
 ...
 * @category Streams
 */
export function Stream<A extends Schema.Constraint, E extends Schema.Constraint>(
  success: A,
  error: E,
): Stream<A, E> {
