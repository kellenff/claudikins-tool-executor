 ...
 *
 * Use this module to mount a browser-based API reference on an `HttpRouter` without writing or
 * storing a separate OpenAPI file. The route renders an HTML page containing the OpenAPI document
 * produced from the supplied `HttpApi` and boots Scalar in the browser.
 *
 * @since 4.0.0
 */
...
export type ScalarConfig = {
  ...
  customCss?: string;
  /**
   * Origin used when the OpenAPI document contains relative server URLs and is rendered during SSR.
   *
   * **Details**
   *
   * Browsers can derive the origin from `window.location.origin`; server rendering needs this value
   * supplied explicitly.
   *
  ...
  /**
   * Whether all tags are open by default instead of only the tag matching the current URL.
   *
   * @default false
   */
 ...
 * **Details**
 *
 * The route serves the OpenAPI specification generated from the API at the configured path,
 * defaulting to `/docs`.
 *
 * @since 4.0.0
 * @category Layers
 ...
 *
 * The route serves the OpenAPI specification generated from the API at the configured path,
 * defaulting to `/docs`; `version` selects the Scalar package version loaded from the CDN.
 *
 * @since 4.0.0
 * @category Layers
 */
