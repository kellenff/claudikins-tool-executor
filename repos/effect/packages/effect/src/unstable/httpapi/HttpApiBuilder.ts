 ...
 *
 * This module turns an `HttpApi` description plus group handlers into `HttpRouter` routes. At
 * runtime it decodes request parts with schemas, runs middleware and security handlers, invokes the
 * registered endpoint handler, and encodes successes or declared errors into `HttpServerResponse`
 * values.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The `build` function receives an unimplemented `Handlers` instance that can be used to add
 * handlers to the group. Implement endpoints with `handlers.handle`.
 *
 * @since 4.0.0
 * @category Handlers
 */
...
type HandleAllEntryHandler<Entry> = Entry extends { readonly handler: infer Handler }
  ? Handler
  : Entry;
...
> = {
  readonly [Identifier in keyof HandlersByIdentifier &
    keyof EndpointsByIdentifier]: HandleAllEntryHandler<
 ...
 * **Details**
 *
 * Each call to `handle` or `handleRaw` registers an endpoint implementation and adds that endpoint
 * identifier to the type-level set of implemented endpoints. Endpoint identifiers that were already
 * handled are rejected at the type level.
 *
 * @since 4.0.0
 * @category Handlers
 */
...
> extends Pipeable {
  ...
  /**
   * Add the implementation for an unhandled `HttpApiEndpoint` to a `Handlers` group. This version
   * opts out of automatic payload decoding and provides the raw request.
   */
...
export declare namespace Handlers {
  /**
   * Creates a handler collection for a group where every endpoint in the group is still awaiting an
   * implementation.
   *
   * @since 4.0.0
   * @category Handlers
  ...
  /**
   * Validates the return value of a group handler builder, preserving successful handler
   * collections and producing a descriptive type error when endpoints remain unhandled.
   *
   * @since 4.0.0
   * @category Handlers
   */
  ...
  /**
   * Extracts the error channel from an effect that produces a `Handlers` collection, returning
   * `never` for non-effectful handler collections.
   *
   * @since 4.0.0
   * @category Handlers
   */
  ...
  /**
   * Extracts the services required by a handler collection, including both handler requirements and
   * the environment required to construct the handlers.
   *
   * @since 4.0.0
   * @category Handlers
   */
...
/**
 * Builds the server-side HTTP effect for a single endpoint in an API group using the endpoint
 * metadata, middleware, codecs, and supplied handler.
 *
 * @since 4.0.0
 * @category Handlers
 */
...
/**
 * Decodes credentials for an HTTP API security scheme from the current request, supporting bearer,
 * API key, and basic authentication inputs.
 *
 * @since 4.0.0
 * @category Security
 */
...
/**
 * Registers a pre-response handler that sets an API-key cookie on the outgoing response, defaulting
 * the cookie to `secure` and `httpOnly` unless overridden.
 *
 * @since 4.0.0
 * @category Security
 */
...
function getAuthorizationCredential(
  ...
  return credentialStart === authorization.length
    ? undefined
    : authorization.slice(credentialStart);
}
...
const registerHandler = (
  ...
  if (!Object.hasOwn(self.group.endpoints, identifier)) {
    throw new Error(
      `HttpApiEndpoint "${identifier}" not found in HttpApiGroup "${self.group.identifier}"`,
    );
  }
...
const isHandlerError = (value: unknown): value is HandlerError =>
  hasProperty(value, HandlerErrorTypeId);
    ...
    case "Json": {
      return (e) => {
        ...
        try {
          ...
          return Effect.fail(
            new SchemaIssue.InvalidValue(Option.some(e), { message: globalThis.String(error) }),
          );
        }
      };
    }
