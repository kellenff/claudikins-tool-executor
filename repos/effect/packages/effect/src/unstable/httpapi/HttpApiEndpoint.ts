 ...
 *
 * An endpoint records a stable identifier, HTTP method, router path, request schemas, response
 * schemas, declared errors, middleware, and annotations. Endpoint values are declarations, not
 * handlers: builders use them to decode requests, type handler input, encode responses, generate
 * OpenAPI metadata, and derive generated-client call signatures. This module also includes HTTP
 * method constructors, payload and response schema helpers, and type utilities used by builders and
 * generated clients.
 *
 * @since 4.0.0
 */
...
/**
 * Returns `true` when a value is an `HttpApiEndpoint`, narrowing the value to the endpoint
 * interface.
 *
 * @since 4.0.0
 * @category Guards
...
type SuccessEncodingServices<S> =
  S extends HttpApiSchema.StreamSse<infer _Events, infer _Error, infer _Value>
...
type RequestRawFromParts<Endpoint, ParamsType, QueryType, HeadersType> = ([ParamsType] extends [
  never,
]
  ? {}
  : { readonly params: Simplify<ParamsType> }) &
...
/**
 * Maps normalized media types to a payload encoding strategy and one or more schemas. Each schema
 * retains its declared content type in its encoding annotation.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Represents an API endpoint. An API endpoint is mapped to a single route on the underlying
 * `HttpRouter`.
 *
 * @since 4.0.0
 * @category Models
...
> extends Pipeable {
  ...
  readonly "~Request": RequestFromParts<
    this,
    Params["Type"],
    Query["Type"],
    Payload["Type"],
...
/**
 * A widened `HttpApiEndpoint` type used when the concrete method, path, schemas, and middleware
...
/**
 * A widened endpoint type that preserves concrete runtime properties such as method, path, schemas,
 * annotations, and middleware sets.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * @category Models
 */
export type Middleware<Endpoint> = Endpoint extends { readonly "~Middleware": infer M } ? M : never;
...
/**
 * Computes the full error value union for an endpoint, including the endpoint error schema's type
 * and errors introduced by middleware.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Computes the services required to encode an endpoint's error responses, including services
 * required by middleware error encoders.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Builds the decoded request shape passed to a normal endpoint handler, including available params,
 * query, payload, headers, the raw request, endpoint, and group. Multipart stream payloads are
 * exposed as streams of parts.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Builds the request shape passed to a raw endpoint handler, including decoded params, query, and
 * headers plus the raw request, endpoint, and group, while leaving payload handling to the raw
 * request.
 *
 * @since 4.0.0
 * @category Models
 */
export type RequestRaw<Endpoint> = Endpoint extends ConstraintRequest
  ? Endpoint["~RequestRaw"]
  : {};

/**
 * Builds the request object accepted by a generated client method, including only the params,
 * query, headers, payload, and response mode fields required by the endpoint. Multipart payloads
 * are supplied as `FormData`.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Controls what a generated client method returns: the decoded success value, the decoded value
 * paired with the raw response, or only the raw response.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Computes the services required on the server to decode endpoint inputs and encode endpoint
 * success, error, and middleware error responses.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Computes the services required on the client to encode endpoint requests and decode endpoint
...
/**
 * Computes the services required to decode an endpoint's error responses, including services
 * required by middleware error decoders.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * The normal server handler for an endpoint, accepting the decoded request shape and returning
 * either the endpoint success value or a custom `HttpServerResponse`.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * The raw server handler for an endpoint, receiving a request shape without a decoded payload so
 * the handler can read the raw `HttpServerRequest` directly.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Derives the normal handler type for the endpoint with the specified identifier in an endpoint
 * union.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Extracts the decoded success value type for the endpoint with the specified identifier in an
 * endpoint union.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Computes the full error value union for the endpoint with the specified identifier in an endpoint
 * union.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Computes the server-side service requirements for the endpoint with the specified identifier in
 * an endpoint union.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Extracts the middleware identifiers for the endpoint with the specified identifier in an endpoint
 * union.
 *
 * @since 4.0.0
 * @category Models
 */
export type MiddlewareWithIdentifier<
  Endpoints extends Constraint,
  Identifier extends string,
> = Middleware<WithIdentifier<Endpoints, Identifier>>;

/**
 * Extracts the middleware service requirements for the endpoint with the specified identifier in an
 * endpoint union.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Removes services provided by the HTTP router and the selected endpoint's middleware from a
...
/**
 * Removes services provided by the HTTP router and endpoint middleware from a service requirement
 * union.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Returns an endpoint type with the supplied path prefix prepended while preserving the endpoint's
 * schemas, method, errors, and middleware.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Returns an endpoint type with additional middleware applied and the endpoint's middleware service
...
/**
 * Constraint for path parameter schemas: each parameter must encode to `string | undefined`, or the
 * schema must encode to a record of those values.
 *
 * @since 4.0.0
...
/**
 * Constraint for header schemas: each header must encode to `string | undefined`, or the schema
 * must encode to a record of those values.
 *
 * @since 4.0.0
 * @category Constraints
 */
...
/**
 * Constraint for query schemas: each field must encode to `string`, an array of strings, or
 * `undefined`, or the schema must encode to a record of those values.
 *
 * @since 4.0.0
 * @category Constraints
 */
 ...
 *
 * - For no-body methods, payload is modeled as query params, so each field must encode to `string |
 *   ReadonlyArray<string> | undefined` and OpenAPI can expand it into `in: query` parameters
 * - For body methods, payload may be any `Schema.Top` (or content-type keyed schemas) and OpenAPI
 *   uses `requestBody` instead of `parameters`
 *
 * @since 4.0.0
 * @category Constraints
 */
...
/**
 * Payload constraint used when automatic codecs are enabled: no-body methods accept field records
 * for query-style encoding, while body methods accept one or more schemas.
 *
 * @since 4.0.0
 * @category Constraints
 */
...
/**
 * Constraint for success response schemas, allowing either a single schema or a readonly array of
 * schemas.
 *
 * @since 4.0.0
 * @category Constraints
...
/**
 * Constraint for error response schemas, allowing either a single schema or a readonly array of
 * schemas.
 *
 * @since 4.0.0
 * @category Constraints
...
/**
 * Creates endpoint constructors for a specific HTTP method. The resulting constructor builds an
 * `HttpApiEndpoint` from an identifier, path, and optional request and response schemas, applying
 * automatic JSON or string-tree codecs unless `disableCodecs` is enabled.
 *
 * @since 4.0.0
...
export const make =
  ...
  >(
    ...
    Payload extends Schema.Struct.Fields
      ...
      : Payload extends ReadonlyArray<Schema.Constraint>
        ? Payload[number]
        : Payload,
...
function getSuccessResponse(
  ...
  return new Set(
    disableCodecs
      ? schemas
      : schemas.map((schema) =>
          HttpApiSchema.isStreamSchema(schema) ? schema : transformResponse(schema),
        ),
  );
}
  ...
  for (const schema of schemas) {
    if (HttpApiSchema.isStreamSchema(schema)) {
      ...
      if (entry.stream !== undefined) {
        ...
        if (
          MediaType.normalize(encoding.contentType) ===
          MediaType.normalize(entry.stream.contentType)
        ) {
          throw new Error(
