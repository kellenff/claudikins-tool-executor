 ...
 *
 * HTTP API middleware wraps endpoint execution on the server and, when the API requires it, can
 * also wrap requests made by generated clients. It is used for cross-cutting behavior that belongs
 * to the API contract, such as authentication, authorization, logging, tracing, rate limiting,
 * request-scoped services, schema-error handling, and client request decoration. This module
 * defines the middleware service keys and helpers used by `HttpApi` declarations.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * It receives the endpoint response effect and endpoint/group metadata, and returns a new response
 * effect that may require additional services and fail with the middleware's declared error
 * schema.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Each property handles the credential decoded for that scheme and wraps the endpoint response
 * effect with the middleware's declared requirements and errors.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * It receives endpoint/group metadata, the outgoing request, and a `next` function for continuing
...
/**
 * Base service key shape for HTTP API middleware services, including provided services, declared
 * error schemas, and client requirements.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Middleware service key shape for security middleware, including the security schemes handled by
 * the service.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Type-level identifier carried by middleware services to track provided services, required
 * services, errors, client errors, and client requirements.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Applies a middleware's service changes to an existing requirement type by removing services it
 * provides and adding services it requires.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Computes the client-side service marker required for middleware that must also run in generated
 * clients.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * It combines a `Context.Service` class with the middleware metadata used by endpoints, builders,
 ...
 * **When to use**
 *
 * Use when you need an HTTP API middleware service whose configuration declares required services,
 * provided services, typed error schemas, security schemes, client errors, or a matching client
 ...
 * **Details**
 *
 * The middleware catches schema errors produced while running an endpoint and uses the supplied
 * `transform` function to convert them into the middleware's declared error schema.
 *
...
/**
 * Provides a client-side middleware implementation for a middleware that is required by generated
 * clients.
 *
 * **Details**
 *
 * The layer captures the surrounding services and makes the middleware available through the
 * `ForClient` service marker used by HTTP API clients.
 *
 * @since 4.0.0
 * @category Client
 */
