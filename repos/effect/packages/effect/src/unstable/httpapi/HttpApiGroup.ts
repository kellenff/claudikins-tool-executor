 ...
 * Builders, generated clients, URL builders, and OpenAPI generation read the same group value,
 * including its identifier, endpoints, annotations, and `topLevel` flag. This module includes
 * helpers for creating groups, adding endpoints, prefixing paths, applying middleware, annotating
...
/**
 * An `HttpApiGroup` is a named collection of `HttpApiEndpoint`s that represents a portion of your
 * domain.
 *
 * **Details**
 *
...
> extends Pipeable {
  ...
  /**
   * Stable group identifier. This field intentionally is not named `name` because `HttpApiGroup`
   * values can be extended as classes, where `name` would collide with JavaScript's built-in
   * `Function.name`.
   */
  ...
  /**
   * Add a path prefix to all endpoints in an `HttpApiGroup`. Note that this will only add the
   * prefix to the endpoints before this api is called.
   */
   ...
   * **Gotchas**
   *
   * Endpoints added after this method is called do not have the middleware applied.
   */
 ...
 * **Details**
 *
 * `HttpApiBuilder.group` provides this service, and `HttpApiBuilder.layer` requires one service for
 * each group in the API. The type carries both the API id and the group identifier so the
 * relationship between an API and its implemented groups is checked at compile time.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Derives the group implementation service required for each group in an HTTP API.
 *
 * **Details**
 *
 * When given an API id and a group or union of groups, this type maps each group to the `Service`
 * identity that must be provided by `HttpApiBuilder.group`.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * A widened `HttpApiGroup` type used when the concrete group identifier, endpoints, and top-level
 * flag are not needed.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * A widened group type that preserves concrete runtime properties such as identifier, key,
 * top-level status, endpoints, and annotations.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Extracts the runtime services required by middleware attached to the endpoints in an
 * `HttpApiGroup`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * @category Models
 */
export type EndpointsWithIdentifier<
  Group extends Constraint,
  Identifier extends string,
> = Endpoints<WithIdentifier<Group, Identifier>>;

/**
 * Computes the schema encoding and decoding services required by clients for all endpoints in a
 * group.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Returns the type of a group after applying a middleware identifier to every endpoint in the
 * group.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Add endpoints with `add`, provide implementations with `HttpApiBuilder.group`, and set `topLevel`
 * when the generated client should expose endpoint methods directly instead of nesting them under
