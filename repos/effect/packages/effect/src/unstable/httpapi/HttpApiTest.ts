 ...
 *
 * The helpers use the same request encoding, routing, response encoding, and client decoding as the
 * normal `HttpApiBuilder` and `HttpApiClient` pipeline, but they do not start an HTTP server. This
 * is useful for focused handler tests, schema round trips, middleware behavior, and typed client
 * calls.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Handlers for the selected groups are taken from the environment; unselected groups are wired with
 * placeholder handlers that fail if called.
 *
 * @since 4.0.0
 * @category Testing
 */
