 ...
 *
 * The client methods are derived from the groups and endpoints in an `HttpApi` and run through an
 * `HttpClient`. They use the same schema-driven contract as the server: request parts are encoded
 * from endpoint schemas, client middleware is applied, the HTTP request is executed, and declared
 * success or error responses are decoded. This module also includes helpers for building a client
 * for only one group, one endpoint, or only the encoded URL.
 *
 * @since 4.0.0
 */
...
/**
 * The type-safe client shape generated from HTTP API groups, with non-top-level groups exposed as
 * nested objects and top-level endpoints exposed as methods.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Derives the typed client interface for an `HttpApi`, preserving any additional client error and
 * service requirements supplied by the caller.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Helper types used to describe generated HTTP API clients, including endpoint methods, response
 * modes, and grouped client shapes.
 *
 * @since 4.0.0
 */
export declare namespace Client {
  /**
   * The response mode accepted by generated client methods, controlling whether a call returns the
   * decoded success value, the raw response, or both.
   *
   * @since 4.0.0
   * @category Models
   */
  ...
  /**
   * The client object for one API group, mapping each endpoint identifier in that group to its
   * typed client method.
   *
   * @since 4.0.0
   * @category Models
  ...
  /**
   * The typed function generated for an endpoint, accepting the endpoint request shape and
   * returning an effect whose success, error, and service channels reflect the endpoint schemas,
   * middleware, and selected response mode.
   *
   * @since 4.0.0
   * @category Models
   */
  ...
  /**
   * Extracts client methods for endpoints in top-level groups so they can be exposed directly on
   * the generated client object.
   *
   * @since 4.0.0
   * @category Models
   */
...
/**
 * The type-safe URL builder shape for an HTTP API, mirroring the generated client layout while
 * returning URL strings instead of executing requests.
 *
 * @since 4.0.0
 * @category Models
 */
...
type UrlBuilderTopLevelMethods<Groups extends HttpApiGroup.Constraint> = {
  readonly [Endpoint in HttpApiGroup.Endpoints<
...
/**
 * Constructs a type-safe client for an HTTP API using the `HttpClient` service, endpoint schemas,
 * middleware, and optional client or response transformations.
 *
 * @since 4.0.0
...
/**
 * Constructs a type-safe client for an HTTP API from the supplied `HttpClient`, using the API
 * metadata to encode requests, execute middleware, and decode responses.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Builds a typed client object for a single API group from the supplied `HttpClient`, filtering the
 * API to that group.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Builds the typed client method for one endpoint in one API group, using the supplied `HttpClient`
  ...
  const out = $HttpBody.pipe(
    Schema.decodeTo(
      ...
      SchemaTransformation.transformOrFail<unknown, HttpBody.HttpBody>({
        decode(httpBody) {
          return Effect.fail(
            new SchemaIssue.Forbidden(Option.some(httpBody), { message: "Encode only schema" }),
          );
        },
        encode(t) {
          switch (encoding._tag) {
            ...
            case "FormUrlEncoded": {
              if (!Predicate.isObject(t)) {
                return Effect.fail(
                  new SchemaIssue.InvalidValue(Option.some(t), { message: "Expected a record" }),
                );
              }
