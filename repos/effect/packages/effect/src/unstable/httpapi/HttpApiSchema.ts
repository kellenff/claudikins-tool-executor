 ...
 *
 * This module is the schema-side bridge for HttpApi endpoint builders, generated clients, and
 * OpenAPI support. It does not define routes or perform IO. Instead, the helpers annotate schemas
 * so the surrounding HTTP API tooling can choose response status codes, content types, body codecs,
 * multipart handling, and no-body response behavior.
 *
 * @since 4.0.0
 */
...
declare module "../../Schema.ts" {
  namespace Annotations {
    interface Augment {
      ...
      /**
       * The encoding of the payload or response. This is kept internal because encodings are only
       * exposed through the `as*` functions.
       *
       * @internal
       */
 ...
 * **Details**
 *
 * This is equivalent to calling `.annotate({ httpApiStatus: code })` on the schema. You can pass
 * either a numeric status code (for example, `201`) or a common literal name (for example,
 * `"Created"`).
 *
 * @since 4.0.0
 * @category Status
...
/**
 * Creates a void schema with the given HTTP status code. This is used to represent empty responses
...
/**
 * Schema type returned by `asNoContent`, encoding as `void` while decoding to the original schema
 * type.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Details**
 *
 * The server encodes the response as `void`; generated clients call `decode` to produce the
 * schema's decoded value when the response has no body.
 *
 * @since 4.0.0
 * @category Encoding
 ...
 * **Details**
 *
 * `events` describes successful application events emitted by the stream, and `error` describes
 * typed stream failures that will be encoded by later endpoint/server/client integrations using the
 * reserved failure event. If `error` is omitted, it defaults to `Schema.Never`. When `StreamSse` is
 * constructed from `data`, handlers and clients expose raw data values while the server and client
 * still use an SSE event schema internally.
 *
 * **Gotchas**
 *
 * The client treats `effect/httpapi/stream/failure` as a stream failure only when its decoded
 * `data` is a `Cause`. If an event schema accepts that name dynamically but decodes `data` to
 * another value, the client emits it as an application event. Endpoint construction rejects event
 * schemas that declare the reserved name statically.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * This declaration stores the response content type for later endpoint, server, client, and OpenAPI
 * integrations. It is intentionally separate from the buffered `asUint8Array` response encoding.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The check succeeds for direct `void` schemas and schemas whose encoded or transformation target
 * is `void`.
 *
 * @since 4.0.0
 * @category Predicates
