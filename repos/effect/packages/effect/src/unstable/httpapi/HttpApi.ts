 ...
 *
 * An `HttpApi` value is data: it has an identifier, annotations, and groups of endpoints that
 * describe request inputs, responses, middleware, and route metadata. The same description can be
 * used by server builders, generated clients, URL builders, OpenAPI generation, and reflection
 * tools.
 *
 * @since 4.0.0
 */
...
/**
 * An `HttpApi` is a collection of HTTP API groups and endpoints that represents a portion of your
 * domain.
 *
 * **When to use**
 *
 * Use when endpoint implementations can be provided with `HttpApiBuilder.group`, and the completed
 * API can be registered with `HttpApiBuilder.layer`.
 *
 * @since 4.0.0
 * @category Models
 */
...
> extends Pipeable {
  ...
  add<const A extends NonEmptyReadonlyArray<HttpApiGroup.Constraint>>(
    ...groups: A
   ...
   * **Details**
   *
   * The added API is flattened into this API rather than retained as a nested value. Each added
   * group is copied with the added API's annotations, leaving the added API unchanged. Annotation
   * precedence from least to most specific is this API, the added API, the group, and then the
   * endpoint.
   *
   * **Gotchas**
   *
   * Annotations from the added API do not become top-level annotations of the result and do not
   * affect groups already present in this API. They remain scoped to the groups and endpoints being
   * added.
   */
...
/**
 * An `HttpApi` with broad identifier and group types while retaining the concrete runtime
 * properties used by implementation helpers.
 *
 * @since 4.0.0
 * @category Models
 */
...
const Proto = {
  ...
  add(this: Top, ...toAdd: NonEmptyReadonlyArray<HttpApiGroup.Top>) {
...
const makeProto = <Id extends string, Groups extends HttpApiGroup.Constraint>(options: {
  readonly identifier: Id;
 ...
 * **When to use**
 *
 * Use when you need to start defining an HTTP API, add groups with `add` or `addHttpApi`, provide
 * endpoint implementations with `HttpApiBuilder.group`, and register the API with
 ...
 * **Details**
 *
 * The callbacks receive each group or endpoint with merged annotations, endpoint middleware, and
 * response schemas grouped by HTTP status.
 *
 * @since 4.0.0
 * @category Reflection
 */
...
/**
 * Adds additional schemas to components/schemas. The provided schemas must have a `identifier`
 * annotation.
 *
 * @since 4.0.0
 * @category Services
