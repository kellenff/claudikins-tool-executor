 ...
 *
 * This module creates a `Tracer.Tracer` backed by the shared OTLP batch exporter, so spans created
 * by Effect tracing APIs can be sent to an OpenTelemetry Collector, vendor endpoint, or local
 * collector. Exported spans include identifiers, parent links, attributes, events, timing, kind,
 * and status information. Use the constructor directly or install it through the provided layer.
 *
 * @since 4.0.0
 */
 ...
 * `OtlpSerialization`, and flushed when the surrounding `Scope` closes.
 *
 * @since 4.0.0
...
export const layer: (options: {
  ...
  readonly context?:
    ...
    | undefined;
  readonly shutdownTimeout?: Duration.Input | undefined;
}) => Layer.Layer<never, never, OtlpSerialization | HttpClient.HttpClient> = flow(
  make,
