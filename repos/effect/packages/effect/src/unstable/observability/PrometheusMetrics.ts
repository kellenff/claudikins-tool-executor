 ...
 *
 * This module reads metrics from the current Effect context and renders them in the Prometheus text
 * format. It can also register a pull-based HTTP endpoint, such as `/metrics`, for Prometheus to
 * scrape.
 *
 * @since 4.0.0
 */
...
export interface FormatOptions {
  /**
   * Optional prefix to prepend to all metric names. The prefix will be sanitized and joined with an
   * underscore.
   */
 ...
 * @category Formatting
 */
export const format: (options?: FormatOptions | undefined) => Effect.Effect<string> =
 ...
 * **Details**
 *
 * This layer automatically adds a GET route to your HTTP router that serves metrics in Prometheus
 * exposition format. By default, the endpoint is registered at `/metrics`, but this can be
 * customized via the `path` option.
 *
...
/**
 * Sanitize a metric name to conform to Prometheus naming rules. Valid characters:
 * [a-zA-Z_:][a-zA-Z0-9_:]*
 */
...
/**
 * Sanitize a label name to conform to Prometheus naming rules. Valid characters:
 * [a-zA-Z_][a-zA-Z0-9_]*
 */
...
/** Escape special characters in label values. Backslash, double-quote, and newline must be escaped. */
const escapeLabelValue = (value: string): string => {
  return value.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/\n/g, "\\n");
};

/** Escape special characters in HELP text. Backslash and newline must be escaped. */
const escapeHelp = (text: string): string => {
  return text.replace(/\\/g, "\\\\").replace(/\n/g, "\\n");
};

/** Format a numeric value for Prometheus output. Handles special values like NaN and Infinity. */
...
/**
 * Format labels as a Prometheus label string. Returns empty string if no labels, otherwise returns
...
/** Format a Histogram metric. Produces _bucket, _sum, and _count lines. */
...
/** Format a Summary metric. Produces quantile lines, _sum, and _count lines. */
const formatSummary = (
  name: string,
