 ...
 *
 * The logger turns Effect log entries into OTLP log records and sends them to a logs endpoint, such
 * as an OpenTelemetry Collector or vendor OTLP endpoint. It includes log levels, messages,
 * annotations, causes, fiber ids, optional log spans, and current trace/span ids when they are
 * present.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The logger serializes records with the configured resource, sends them through the OTLP exporter,
 * and requires `Scope` so pending records can be flushed on shutdown.
 *
 * @since 4.0.0
...
export const layerFromConfig = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const { baseTimeout, logsTimeout, exportTimeout, scheduleDelay, maxBatchSize } =
      yield* Config.all({
