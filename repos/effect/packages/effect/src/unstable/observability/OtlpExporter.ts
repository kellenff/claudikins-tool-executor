 ...
 *
 * Signal modules use this exporter to buffer already-encoded telemetry and post it to a configured
 * OTLP endpoint. It owns the scoped transport loop, batching, retry behavior, temporary disabling
 * after repeated failures, and final flush during shutdown.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * The exporter buffers pushed data, periodically posts encoded batches to the configured URL,
 * retries transient failures, temporarily disables exporting after unhandled failures, and flushes
 * during scope finalization up to `shutdownTimeout`.
 *
 * @since 4.0.0
