 ...
 *
 * A `Context` holds service implementations under `Context.Service` or `Context.Reference` keys,
 * and its type records which keys are present. Effects use contexts as their environment, so
 * services can be provided once instead of passed through every function call. This module includes
 * helpers for creating keys, building contexts, adding and reading services, merging contexts, and
 * selecting or removing services.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * `Identifier` tracks the requirement in Effect types, while `Shape` is the service implementation
 * retrieved by the key. A key is also an Effect value, so yielding it inside `Effect.gen` retrieves
 * the service from the current fiber context.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * `context` creates a one-service `Context`, `use` and `useSync` retrieve the service from the
 * current Effect context before applying a function, and `of` is a type-level helper for service
 * values.
 *
...
export declare namespace ServiceClass {
  /**
   * Runtime and type-level metadata carried by a class-style service key, including its service
   * type identifier, string key, and service shape.
   *
   * @since 4.0.0
   * @category Models
   */
 ...
 * **When to use**
 *
 * Use when you need to define a context service key for a dependency that must be provided by the
 * surrounding context.
 *
 * **Details**
 *
 * Call `Context.Service("Key")` for a function-style key, or use the two-stage form
 * `Context.Service<Self, Shape>()("Key")` for class-style service declarations. The returned key
 * can be yielded as an Effect and passed to `Context.make`, `Context.add`, and the Context getter
 * functions.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * When a `Reference` is requested from a `Context` that does not contain an override, Context
 * getters that resolve references return the cached default value instead of failing.
 *
 ...
 * const LoggerRef: Context.Reference<{ log: (msg: string) => void }> = Context.Reference(
 *   "Logger",
 *   {
 ...
 * **Details**
 *
 * The type parameter tracks the service identifiers available in the context. At runtime, services
 * are stored by each key's string `key`.
 *
 ...
 * **Gotchas**
 *
 * This is unsafe because later mutation of the provided map can affect the created `Context`.
 * Prefer `empty`, `make`, `add`, or `merge` for normal Context construction.
 *
 ...
 * **Details**
 *
 * This checks the runtime `Context` marker and does not inspect which services the context
 * contains.
 *
 * **Gotchas**
 *
 * This guard only proves that the value is a `Context`; it does not prove that any specific service
 * is present.
 *
 ...
 * **Details**
 *
 * When `service` is `Option.some`, the value is stored for the key. When it is `Option.none`, the
 * key is removed from the returned `Context`.
 *
 ...
 * **When to use**
 *
 * Use when you need a fallback for a missing `Context.Service` key while still resolving
 * `Context.Reference` defaults.
 *
 * **Details**
 *
 * If the key is a `Context.Reference` and no override is stored in the context, its cached default
 * value is returned instead of the fallback.
 *
 * **Gotchas**
 *
 * The fallback is not evaluated for missing `Context.Reference` keys because references resolve to
 * their default value.
 *
...
} = dual(2, <Services, S, I>(self: Context<Services>, key: Key<I, S>): S | undefined =>
 ...
 * **When to use**
 *
 * Use when you need to read a service from a context whose type does not prove the service is
 * present.
 *
 * **Details**
 *
 * If the key is a `Context.Reference` and no override is stored in the context, its cached default
 * value is returned. For absent non-reference keys, this function throws a runtime error.
 *
...
/**
 * Gets the value for a `Context.Reference`, returning its cached default when the context does not
 * contain an override.
 *
 * **When to use**
 *
 * Use when you need a `Context.Reference` value resolved from either a stored override or the
 * reference's default value.
 *
 * **Details**
 *
 * Stored overrides take precedence. If no override is present, the reference's default value is
 * computed lazily and cached on the reference itself.
 *
 * **Gotchas**
 *
 * Mutable default values can be shared across contexts unless an override is provided, because the
 * default is cached on the `Context.Reference`.
 *
 ...
 * **Details**
 *
 * Returns `Option.some` when the service is stored in the context. If the key is a
 * `Context.Reference` and no override is stored, returns `Option.some` of the cached default value.
 * Missing non-reference keys return `Option.none`.
 *
 ...
 * **Details**
 *
 * When both contexts contain the same service key, the service from `that` overrides the service
 * from `self`.
 *
 ...
 * **Details**
 *
 * When multiple contexts contain the same service key, the service from the last context with that
 * key is kept.
 *
...
/**
 * Performs a series of mutations on a `Context`. Prevents unnecessary copying of the underlying map
 * when multiple mutations are needed.
 *
 * **When to use**
 *
 * Use to apply several `Context` transformations in one callback while copying the underlying
 * service map only once.
 *
 * @since 4.0.0
 * @category Mutations
 ...
 * **Details**
 *
 * `Context.Reference` allows you to create a key that can hold a value. You can provide a default
 * value for the service, which will automatically be used when the context is accessed, or override
 * it with a custom implementation when needed. The default value is computed lazily and cached on
 * the reference.
 *
