 ...
 *
 * A scheduler decides how tasks are queued, when queued tasks run, and when a fiber should pause so
 * other work can continue. This module includes the scheduler service reference, the default
 * `MixedScheduler`, dispatcher types for queued tasks, and references for tuning or disabling
 * automatic scheduler yields.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * A scheduler determines the execution mode, schedules tasks with different priorities, and decides
 * when fibers should yield control after consuming their operation budget.
 *
 * @since 2.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when implementing or testing scheduler-created dispatchers that enqueue prioritized runtime
 * tasks and flush queued work deterministically.
 *
 * **Details**
 *
 * `scheduleTask` queues a task with a priority. `flush` drains pending work synchronously, which is
 * useful when callers need deterministic completion of already scheduled tasks. Lower priority
 * numbers run first, and equal priorities run in FIFO order.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when you need to replace scheduling behavior globally in tests or runtime setup, such as
 * forcing deterministic task dispatch.
 *
 * **Details**
 *
 * The default value creates a `MixedScheduler`. Provide this service to customize execution mode,
 * task dispatching, or yield behavior.
 *
 * @since 2.0.0
 * @category References
 */
 ...
 * **When to use**
 *
 * Use when you need the default runtime scheduler directly, including a scheduler that batches
 * queued work by priority and preserves FIFO order within each priority.
 *
 * **Details**
 *
 * `MixedScheduler` supports synchronous and asynchronous execution modes, uses operation counts to
 * decide when fibers should yield, and is the default scheduler implementation.
 *
 * @since 2.0.0
 * @category Schedulers
 */
export class MixedScheduler implements Scheduler {
   ...
   * **When to use**
   *
   * Use when you need a standalone dispatcher from a scheduler instance, for example in tests that
   * enqueue tasks and then flush them deterministically.
   *
   * @since 4.0.0
   */
  makeDispatcher() {
...
/**
 * Context reference that controls the maximum number of operations a fiber can perform before
 * yielding control back to the scheduler.
 *
 * **When to use**
 *
 * Use to tune scheduler fairness for CPU-bound fibers by changing the scheduler operation budget
 * that triggers a yield.
 *
 * **Details**
 *
 * The default value is `2048` operations, which balances performance and fairness by helping
...
/**
 * Context reference that controls whether the runtime should bypass scheduler yield checks. When
 * set to `true`, the fiber run loop won't call `Scheduler.shouldYield`.
 *
 * **When to use**
 *
 * Use to bypass scheduler yield checks for controlled runtime workloads where cooperative yielding
 * should be disabled.
 *
 * **Gotchas**
 *
