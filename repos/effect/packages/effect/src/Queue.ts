 ...
 *
 * A `Queue<A, E>` accepts values, hands each value to one consumer in offer order, and can
 * complete, fail, interrupt, or shut down. Queues can be bounded or unbounded, and bounded queues
 * can suspend, drop, or slide values when producers are faster than consumers.
 *
 * @since 3.8.0
 */
 ...
 * **When to use**
 *
 * Use to narrow an unknown value to a full `Queue` before passing it to APIs that need both
 * offering and taking capabilities.
 *
 * @since 2.0.0
 * @category Guards
 ...
 * **When to use**
 *
 * Use to expose only the producer side of a `Queue` to code that should offer values or signal
 * queue lifecycle.
 *
 * **Gotchas**
 *
 * This is a type-level capability restriction. It returns the same queue object, so it does not
 * hide read operations at runtime.
 *
 * @since 4.0.0
 * @category Converting
 ...
 * **When to use**
 *
 * Use to pass a queue to code that should consume values while keeping producer-side operations out
 * of that code's TypeScript type.
 *
 * **Gotchas**
 *
 * This is a type-level narrowing operation. It returns the same queue object and does not create a
 * runtime wrapper.
 *
 * @since 4.0.0
 * @category Converting
 ...
 * **Details**
 *
 * This interface represents the write-only part of a Queue, allowing you to offer elements to the
 * queue but not take elements from it.
 *
...
export declare namespace Enqueue {
   ...
   * **Details**
   *
   * `Enqueue` is contravariant in both its offered value type `A` and failure type `E`, because
   * values and failures flow into the queue through this handle.
   *
   * @since 4.0.0
   * @category Models
   */
 ...
 * **Details**
 *
 * This interface represents the read-only part of a Queue, allowing you to take elements from the
 * queue but not offer elements to it.
 *
...
export declare namespace Dequeue {
   ...
   * **Details**
   *
   * `Dequeue` is covariant in both the taken value type `A` and failure type `E`, because values
   * and failures are observed through this handle.
   *
   * @since 4.0.0
   * @category Models
   */
...
export declare namespace Queue {
   ...
   * **Details**
   *
   * A full `Queue` is invariant in both `A` and `E` because the same handle can both produce and
   * consume values and failures.
   *
   * @since 4.0.0
   * @category Models
   */
   ...
   * **Details**
   *
   * `Open` queues can accept offers and takers, `Closing` queues are completing with a stored
   * failure exit, and `Done` queues have finished. This is low-level metadata exposed by the queue
   * model; most users should inspect queues through the public operations.
   *
   * @since 4.0.0
   * @category Models
   */
   ...
   * **Details**
   *
   * An entry is either a single message or a batch with an offset into its remaining messages, plus
   * a resume callback that completes the suspended offer when the queue can accept more input.
   *
   * @since 4.0.0
   * @category Models
   */
 ...
 * **Details**
 *
 * By default the queue is unbounded and uses the `"suspend"` strategy. Provide `capacity` for a
 * bounded queue and choose `"suspend"`, `"dropping"`, or `"sliding"` to control what happens when
 * the queue is full. The returned queue can be offered to, taken from, failed, ended, interrupted,
 * or shut down.
 *
 ...
 * **Details**
 *
 * When the queue reaches capacity, producers will be suspended until space becomes available. This
 * ensures all messages are processed but may slow down producers.
 *
...
/**
 * Creates a bounded queue with sliding strategy. When the queue reaches capacity, new elements are
 * added and the oldest elements are dropped.
 *
 * **When to use**
 *
 * Use when you need producer offers not to block and can accept dropping the oldest messages, such
 * as when maintaining a rolling window of recent values.
 *
...
/**
 * Creates a bounded queue with dropping strategy. When the queue reaches capacity, new elements are
 * dropped and the offer operation returns false.
 *
 * **When to use**
 *
 * Use when you need producer offers not to block while preserving existing queued messages, even if
 * new messages may be dropped when the queue is full.
 *
 ...
 * **When to use**
 *
 * Use when you need producers to add messages without backpressure and accept unbounded memory
 * growth.
 *
 ...
 * **Details**
 *
 * For bounded queues, this operation may suspend if the queue is at capacity, depending on the
 * backpressure strategy. For dropping/sliding queues, it may return false or succeed immediately by
 * dropping/sliding existing messages.
 *
 ...
 * **When to use**
 *
 * Use when you are already in synchronous queue internals or a performance boundary where wrapping
 * the mutation in `Effect` is intentionally avoided.
 *
 * **Gotchas**
 *
 * This is an unsafe operation that directly modifies the queue without Effect wrapping. Use this
 * only when you're certain about the synchronous nature of the operation.
 *
 ...
 * **When to use**
 *
 * Use when producers can submit a batch at once and need to know which messages did not fit under
 * the queue's capacity strategy.
 *
 * **Details**
 *
 * For bounded queues, this operation may suspend if the queue doesn't have enough capacity. The
 * operation returns an array of messages that couldn't be added (empty array means all messages
...
/**
 * Adds multiple messages to the queue synchronously. Returns the remaining messages that were not
 * added.
 *
 * **When to use**
 *
 * Use when queue internals or a performance boundary need a synchronous batch offer and can handle
 * any messages that do not fit.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when queue completion must be driven from synchronous internals while preserving the full
 * failure `Cause`.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use when implementing low-level queue integrations that must complete a queue without wrapping
 * the operation in `Effect`.
 *
 * **Details**
 *
 * Returns `false` if the queue is already done.
 ...
 * **Details**
 *
 * This operation stops accepting new offers but allows existing messages to be consumed. Once all
 * messages are drained, the queue transitions to the Done state with an interrupt cause.
 *
 ...
 * **Details**
 *
 * The operation is idempotent and returns `true`, including when the queue has already been shut
 * down or completed.
 *
 ...
 * **Details**
 *
 * Returns an empty array when the queue is empty or has completed normally. If the queue has
 * failed, the effect fails with the queue's error.
 *
...
/**
 * Takes all currently available messages, waiting until at least one message is available when the
 * queue is empty.
 *
 * **When to use**
 *
 * Use when consumers should process the next non-empty batch of buffered messages instead of
 * repeatedly taking one message at a time.
 *
 * **Details**
 *
 * Returns a non-empty array. If the queue completes or fails before a message can be taken, the
 * effect fails with the queue's terminal error.
 *
 ...
 * **Details**
 *
 * The operation may wait until enough messages are available to satisfy the queue's batching rules.
 * If `n` is less than or equal to zero, it succeeds with an empty array. If the queue completes or
 * fails before messages can be taken, the effect fails with the queue's terminal error.
 *
 ...
 * **Details**
 *
 * The operation waits when fewer than the required minimum messages are available. It returns at
 * most `max` messages. If the queue completes or fails before the minimum can be satisfied, the
 * effect fails with the queue's terminal error.
 *
 ...
 * **Details**
 *
 * If the queue is done, it will fail with `Done`. If the queue fails, the Effect will fail with the
 * error.
 *
 ...
 * **Details**
 *
 * Returns `Option.some` when an item is immediately available. Returns `Option.none` when no item
 * is available, when the queue is done, or when the immediate take observes a queue failure.
 *
 ...
 * **When to use**
 *
 * Use when polling queue internals must not suspend or register a waiting taker, and `undefined` is
 * an acceptable result for an empty queue.
 *
 * **Details**
 *
 * Returns an `Exit` for an immediately available message or for the queue's terminal state. Returns
 * `undefined` when no message is immediately available. This operation does not wait or register a
 * taker.
 *
...
export {
   ...
   * **When to use**
   *
   * Use to suspend a fiber until no further values can be taken from the queue and its terminal
   * outcome is known.
   *
   * **Details**
   *
   * The effect succeeds with `void` for normal `Done` completion. Other terminal causes are
   * preserved, so failures and interruptions complete this effect with the same terminal outcome.
   *
   * **Gotchas**
   *
   * A queue can be closing before it is done. `await` resumes at `Done`, not at the first
   * completion signal, so buffered messages may need to be drained first.
   *
   * @since 4.0.0
   * @category Completion
 ...
 * **When to use**
 *
 * Use when you need an immediate `Queue` size snapshot for diagnostics or internals and do not need
 * the read wrapped in `Effect`.
 *
 * **Details**
 *
 * Completed queues report a size of `0`. This unsafe operation reads the queue state directly
 ...
 * **When to use**
 *
 * Use when an immediate `Queue` capacity snapshot is needed outside effectful code and racing queue
