 ...
 *
 * Each cached entry owns its own `Scope`, so resources opened while creating a value stay alive
 * while that entry is cached and are released when the entry is removed. A `ScopedCache` also
 * belongs to an outer scope, which closes all remaining entries when the cache is closed. Lookups
 * for the same missing key share one in-progress effect, and entries can expire, be refreshed, be
 ...
 * **When to use**
 *
 * Use to cache values that acquire scoped resources and must release those resources when entries
 * expire, are evicted, or are invalidated.
 *
 * **Details**
 *
 * Concurrent requests for the same key share the same in-flight lookup. Entries can expire based on
 * the lookup exit, are evicted when capacity is exceeded, and release their entry scopes when
 * invalidated, evicted, expired, or when the cache's owning scope closes.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **Details**
 *
 * `Open` stores cached entries in access order for reuse and eviction. `Closed` means the owning
 * scope has closed and the cache can no longer perform lookup operations.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **When to use**
 *
 * Use when inspecting the open state of a `ScopedCache` and you need the stored deferred result,
 * entry scope, or expiration timestamp for a key.
 *
 * **Details**
 *
 * The entry contains the deferred lookup result shared by readers, the scope that owns resources
 * acquired while computing the value, and an optional expiration time in milliseconds. Removing the
 * entry closes its scope.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Creates a `ScopedCache` from a lookup function, maximum capacity, and a time-to-live function
 * computed from each lookup exit and key.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The cache must be constructed in a `Scope`. Each lookup runs in its own entry scope, and that
 * scope is closed when the entry expires, is invalidated, is evicted by capacity, or when the
 * cache's owning scope closes. `requireServicesAt` controls whether lookup services are captured at
 ...
 * **Details**
 *
 * This is the constant-TTL variant of `makeWith`: values are acquired by the lookup effect in
 * per-entry scopes, capacity can evict older entries, and entry scopes are closed when entries
 * expire, are invalidated, are evicted, or when the cache's owning scope closes.
 *
 * @since 2.0.0
 * @category Constructors
...
const defaultTimeToLive = <A, E>(_: Exit.Exit<A, E>, _key: unknown): Duration.Duration =>
  Duration.infinity;
 ...
 * **When to use**
 *
 * Use to retrieve a scoped cached value by key when a missing or expired entry should run the cache
 * lookup and share the in-flight lookup with concurrent callers.
 *
 * **Details**
 *
 * Concurrent `get` calls for the same key share the same in-flight lookup. Successful and failed
 * lookup exits are cached according to the configured TTL. If the cache is closed, the effect is
 * interrupted.
 *
 * @since 4.0.0
      ...
      core.withFiber((fiber) => {
        ...
        return checkCapacity(fiber, state.map, self.capacity).pipe(
          Option.isSome(oentry)
            ? effect.flatMap(() => Scope.close(oentry.value.scope, effect.exitVoid))
            : identity,
 ...
 * **When to use**
 *
 * Use to read a scoped value only when it is already cached, without starting the lookup for
 * missing or expired keys.
 *
 * **Details**
 *
 * Returns `Option.none` when the key is absent or expired. If an entry exists, the effect waits for
 * its cached result and returns `Option.some(value)` on success, or fails with the cached lookup
 * error.
 *
 * @since 4.0.0
 * @category Combinators
...
/**
 * Retrieves the value associated with the specified key from the cache, only if it contains a
 * resolved successful value.
 *
 * **When to use**
 *
 * Use to inspect an already-completed successful scoped cache entry without running or awaiting the
 * lookup effect.
 *
 * **Details**
 *
 * Returns `Option.some` for a resolved successful entry. Returns `Option.none` for missing,
 ...
 * **Details**
 *
 * This replaces and closes any existing entry scope for the key, applies the cache's TTL using a
 * successful exit for the value, and may evict older entries if the cache capacity is exceeded.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **Details**
 *
 * This does not start lookups and does not refresh access order. Expired entries are treated as
 * absent and their scopes are closed while checking. If the cache is closed, the effect is
 * interrupted.
 *
 * @since 4.0.0
 * @category Combinators
 ...
 * **When to use**
 *
 * Use to remove a single key from a scoped cache and release any resources owned by that entry
 * before a later lookup computes it again.
 *
 * **Details**
 *
...
/**
 * Invalidates the entry associated with the specified key in the cache when the predicate returns
 * true for the cached value.
 *
 * **When to use**
 *
 * Use to remove an already-cached scoped value only when the successful cached value satisfies a
 * predicate.
 *
 * **Details**
 *
 * Returns `true` only when a successful cached value matches and is removed. It returns `false` for
 * absent, expired, failed, or non-matching entries.
 *
 * **Gotchas**
 *
 ...
 * **When to use**
 *
 * Use to recompute a scoped cache entry immediately, even when an unexpired value is already
 * cached.
 *
 * **Details**
 *
 * It will always invoke the lookup function to construct a new value, overwriting any existing
 * value for that key.
 *
 * @since 4.0.0
 * @category Combinators
...
const invalidateAllImpl = <Key, A, E>(
  ...
  for (const [, entry] of map) {
    fibers.push(
      effect.forkUnsafe(parent as any, Scope.close(entry.scope, effect.exitVoid), true, true),
    );
  }
 ...
 * **Gotchas**
 *
 * Note that expired entries are counted until they are accessed and removed. The size reflects the
 * current number of entries stored, not the number of valid entries.
 *
 * @since 4.0.0
 * @category Combinators
 */
  ...
  core.withFiber((fiber) => {
    ...
    for (const [key, entry] of state.map) {
      if (entry.expiresAt === undefined || entry.expiresAt > now) {
        ...
        MutableHashMap.remove(state.map, key);
        fibers.push(
          effect.forkUnsafe(fiber, Scope.close(entry.scope, effect.exitVoid), true, true),
        );
      }
    }
...
/**
 * Retrieves all successfully cached values from the cache, excluding failed lookups and expired
 * entries.
 *
 * **When to use**
 *
...
/**
 * Retrieves all key-value pairs from the cache as an array. This function only returns entries with
 * successfully resolved values, filtering out any failed lookups or expired entries.
 *
 * **When to use**
 *
