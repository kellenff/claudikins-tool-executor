 ...
 *
 * A `TxQueue<A, E>` stores values of type `A`, exposes write-only `TxEnqueue` and read-only
 * `TxDequeue` handles, and can complete, fail, or shut down with causes observed by consumers.
 * Queue operations can retry transactionally when they cannot proceed, such as taking from an empty
 * open queue or offering to a full bounded queue. This makes the queue useful for coordinating
 * producers and consumers alongside other transactional state changes.
 *
 * @since 4.0.0
 */
...
/**
 * Represents the shared state of a transactional queue that can be inspected. This interface
 * contains the core properties needed for queue state inspection operations like size, capacity,
...
/**
 * A TxEnqueue represents the write-only interface of a transactional queue, providing operations
 * for adding elements (enqueue operations) and inspecting queue state.
 *
...
/**
 * A TxDequeue represents the read-only interface of a transactional queue, providing operations for
 * consuming elements (dequeue operations) and inspecting queue state.
 *
...
/**
 * A TxQueue represents a transactional queue data structure that provides both enqueue and dequeue
 * operations with Software Transactional Memory (STM) semantics.
 *
 ...
 * @category Guards
 */
export const isTxEnqueue = <A = unknown, E = unknown>(u: unknown): u is TxEnqueue<A, E> =>
 ...
 * @category Guards
 */
export const isTxDequeue = <A = unknown, E = unknown>(u: unknown): u is TxDequeue<A, E> =>
 ...
 * @category Guards
 */
export const isTxQueue = <A = unknown, E = unknown>(u: unknown): u is TxQueue<A, E> =>
 ...
 * **Details**
 *
 * This function returns a new TxQueue reference with the specified capacity. No existing TxQueue
 ...
 * **Details**
 *
 * This function returns a new TxQueue reference with unlimited capacity. No existing TxQueue
 * instances are modified.
 *
 ...
 * **Details**
 *
 * This function returns a new TxQueue reference with dropping strategy. No existing TxQueue
 * instances are modified.
 *
 ...
 * **Details**
 *
 * This function returns a new TxQueue reference with sliding strategy. No existing TxQueue
 * instances are modified.
 *
 ...
 * **Details**
 *
 * Open unbounded queues always accept; open bounded queues retry while full; dropping queues return
 * `false` when full; sliding queues evict the oldest item when full. Closing or done queues return
 * `false`. This function mutates the original TxQueue by adding the item according to the queue's
 * strategy. It does not return a new TxQueue reference.
 *
 ...
 * items when full, sliding queues evict old items to accept new items, and closing or done queues
 * reject all items. This function mutates the original TxQueue by adding items according to the
 * queue's strategy. It does not return a new TxQueue reference.
 *
 ...
 * **Details**
 *
 * If the queue is done, the effect fails with the queue's completion cause. This function mutates
 * the original TxQueue by removing the first item. It does not return a new TxQueue reference.
 *
 ...
 * **Details**
 *
 * If the queue is already in a failed state, the error is propagated through the E-channel. This
 * follows the same patterns as `take` and waits when there are no elements. It returns a non-empty
 * array because it blocks until at least one item is available. This function mutates the original
 * TxQueue by removing all items. It does not return a new TxQueue reference.
 *
 ...
 * **Details**
 *
 * For an open queue, waits until `min(n, capacity)` items are available, then removes that many
 * items. If `n` is less than or equal to zero, returns an empty array without modifying the queue.
 * If the queue is closing, drains the currently available items and transitions to `Done`. If the
 * queue is already done, the effect fails with the queue's completion cause. This function mutates
 * the original TxQueue by removing the taken items. It does not return a new TxQueue reference.
 *
 ...
 * **Details**
 *
 * If the queue is closing, drains the currently available items even when fewer than `min` are
 * available and transitions to `Done`. Invalid ranges (`min <= 0`, `max <= 0`, or `min > max`)
 * return an empty array. If the queue is already done, the effect fails with the queue's completion
 * cause.
 *
 ...
 * **Details**
 *
 * If the queue is open but empty, the transaction retries until an item is available or the queue
 * completes. If the queue is done, the queue's completion cause is propagated through the error
 * channel.
 *
 ...
 * **Details**
 *
 * If the queue still contains items, it enters the closing state so buffered items can be drained
 * before consumers observe the interruption. If it is empty, it transitions directly to done.
 * Returns `false` if the queue was already closing or done.
 *
 ...
 * **Details**
 *
 * The queue transitions directly to done with `Cause.fail(error)`. Returns `false` if the queue was
 ...
 * **Details**
 *
 * If the queue is empty, it transitions directly to done. If it still contains items, it enters the
 * closing state so buffered items can be drained before the cause is observed. Returns `false` if
 * the queue was already closing or done.
 *
 ...
 * **Details**
 *
 * This is a convenience wrapper around `failCause` for queues whose error channel can contain
 * `Cause.Done`. If buffered items remain, the queue enters the closing state and those items may
 * still be consumed before later `take` or `peek` operations fail with `Cause.Done`.
 *
 ...
 * **Details**
 *
 * If the queue is already done with a `Cause.Done` error, returns an empty array. If the queue is
 * done for any other cause, including interruption or failure, that cause is propagated.
 *
...
/**
 * Shuts down the queue immediately by clearing all items and interrupting it (legacy
 * compatibility).
 *
 * **Details**
 *
 * This operation clears all items from the queue using `clear`, then interrupts the queue using
 * `interrupt`. This function mutates the original TxQueue by clearing its contents and marking it
 * as shutdown. It does not return a new TxQueue reference.
 *
