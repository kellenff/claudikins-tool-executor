 ...
 *
 * When the test layer is provided, calls made through the Effect console APIs are captured in
 * memory instead of being written to the host console. Tests can then assert on logged values
 * deterministically. This module includes the console layer, helpers for reading captured
 * `Console.log` and `Console.error` arguments, and access to the provided test console service.
 *
 * @since 4.0.0
 */
...
/**
 * A `TestConsole` provides a testable implementation of the Console interface. It captures all
 * console output for testing purposes while maintaining full compatibility with the standard
 * Console API.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * This interface extends the standard Console interface and adds methods to retrieve logged
...
export interface TestConsole extends Console.Console {
  /**
   * Returns an array of all items that have been logged by the program using `Console.log` thus
   * far.
   *
   * **When to use**
   *
   * Use to inspect captured `Console.log` calls through a `TestConsole` instance.
   */
  readonly logLines: Effect.Effect<ReadonlyArray<unknown>>;
  /**
   * Returns an array of all items that have been logged by the program using `Console.error` thus
   * far.
   *
   * **When to use**
   *
   * Use to inspect captured `Console.error` calls through a `TestConsole` instance.
   */
  readonly errorLines: Effect.Effect<ReadonlyArray<unknown>>;
}

/**
 * The `TestConsole` namespace provides types and utilities for working with test console
 * implementations.
 *
 * **When to use**
 *
...
export declare namespace TestConsole {
  /**
   * Represents a console method name that can be invoked on the TestConsole. This type includes all
   * methods available on the Console interface.
   *
   * **When to use**
   *
  ...
  /**
   * Represents a single console method invocation captured by the TestConsole. Each entry contains
   * the method name and the parameters passed to it.
   *
   * **When to use**
   *
...
/**
 * Creates a new TestConsole instance that captures all console output. The returned TestConsole
 * implements the Console interface and provides additional methods to retrieve logged messages.
 *
 * **When to use**
 *
...
): Effect.Effect<A, E, R> => Console.consoleWith((console) => f(console as TestConsole));

/**
 * Creates a `Layer` which constructs a `TestConsole`. This layer can be used to provide a
 * TestConsole implementation for testing purposes.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use to assert on captured `Console.error` output from a program provided with
 * `TestConsole.layer`.
 *
