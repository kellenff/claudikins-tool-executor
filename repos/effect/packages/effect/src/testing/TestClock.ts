 ...
 *
 * Instead of waiting for real time to pass, effects that use `Effect.sleep`, timeouts, schedules,
 * retries, and other time-based operators can be driven by advancing the test clock. This makes
 * time-based tests deterministic and fast. The module also includes helpers for moving test time,
 * temporarily using the live clock, and warning when a test appears to be waiting on time without
 * advancing it.
 *
 * @since 2.0.0
 */
...
/**
 * A `TestClock` simplifies deterministic and efficient testing of effects that involve the passage
 * of time.
 *
 * **Details**
 *
 * Instead of waiting for actual time to pass, `sleep` and methods implemented in terms of it
 * schedule effects to take place at a given clock time. Use `adjust` and `setTime` to move clock
 * time, and all effects scheduled to take place on or before that time will automatically run in
 * order.
 *
 * **Gotchas**
 *
 * Calls to `sleep` and methods derived from it will semantically block until the time is set to on
 * or after the time they are scheduled to run. Fork the effect being tested, then adjust the clock
 * time, and finally verify that the expected effects have been performed.
 *
...
export interface TestClock extends Clock.Clock {
  /**
   * Increments the current clock time by the specified duration. Any effects that were scheduled to
   * occur on or before the new time will be run in order.
   */
  adjust(duration: Duration.Input): Effect.Effect<void>;
  /**
   * Sets the current clock time to the specified `timestamp`. Any effects that were scheduled to
   * occur on or before the new time will be run in order.
   */
...
export declare namespace TestClock {
  /**
   * Options used when constructing a `TestClock`. `warningDelay` controls how long the live clock
   * waits before logging a warning when a test uses time without advancing the test clock.
   *
  ...
  export interface Options {
    /**
     * The amount of time to wait before displaying a warning message when a test is using time but
  ...
  /**
   * Represents the state tracked by a `TestClock`, including the current millisecond timestamp and
   * the sleeps scheduled to resume when the clock reaches their target time.
   *
   * @since 4.0.0
   * @category Models
   */
...
/**
 * The warning message that will be displayed if a test is using time but is not advancing the
 * `TestClock`.
 */
...
export const make = Effect.fnUntraced(function* (options?: TestClock.Options) {
  ...
  /**
   * Forks a fiber that will display a warning message if a test is using time but is not advancing
   * the `TestClock`.
   */
  ...
  /**
   * Cancels the warning message that is displayed if a test is using time but is not advancing the
   * `TestClock`.
   */
...
/**
 * Accesses a `TestClock` instance in the context and increments the time by the specified duration,
...
/**
 * `WarningState` describes the state of the warning message that is displayed if a test is using
 * time but is not advancing the `TestClock`.
 *
 * The possible states are: - `Start` if a test has not used time yet - `Pending` if a test has used
 * time but has not adjusted the `TestClock` - `Done` if a test has adjusted the `TestClock` or the
...
type WarningState = Data.TaggedEnum<{
  ...
  readonly Start: {};
  /**
   * The `WarningState` which indicates that a test has used time but has not adjusted the
   * `TestClock`.
   *
   * The `Pending` state also includes a reference to the fiber that will display the warning
   * message.
   */
  readonly Pending: {
  ...
  /**
   * The `WarningState` which indicates that a test has used time, or that the warning message has
