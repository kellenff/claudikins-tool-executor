 ...
 *
 * Property-based tests describe a rule that should hold for many inputs, and `fast-check` generates
 * random examples to look for counterexamples. This module exposes the library from
 * `effect/testing` so tests can use it alongside the other Effect testing helpers.
 *
 * @since 3.10.0
 */

/**
 * Re-exports all functionality from the fast-check library, providing access to property-based
 * testing utilities including arbitraries, property testing, and random data generation.
 *
 * Fast-check allows you to write tests by specifying properties that should hold true for your
 * functions, rather than writing specific test cases. The library then generates many random inputs
 ...
 *     const result = a + b;
 *     return (
 *       result.length === a.length + b.length && result.startsWith(a) && result.endsWith(b)
 *     );
 *   },
 * );
 *
