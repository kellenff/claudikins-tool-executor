 ...
 *
 * These utilities assert how schemas construct values, decode input, encode output, generate
 * arbitrary values, and round-trip between encoded and decoded forms. The `Asserts` class groups
 * the common checks for one schema, while `Decoding` and `Encoding` can be used directly when a
...
/**
 * Provides schema test assertions for decoding, encoding, make, arbitrary generation, and
 * round-trip verification.
 *
 * **When to use**
 *
 * Use when writing schema unit tests for decoding, encoding, construction, property-based
 * round-trip, or generation behavior.
 *
...
export class Asserts<S extends Schema.Constraint> {
   ...
   * **When to use**
   *
   * Use to assert that two schema field or tuple element definitions produce the same AST
   * structure.
   *
   * **Details**
   *
   * `ast.fields.equals(a, b)` compares struct field ASTs via `assert.deepStrictEqual`.
   * `ast.elements.equals(a, b)` compares tuple element ASTs via `assert.deepStrictEqual`.
   *
   ...
   * **When to use**
   *
   * Use to assert how `Schema.make` accepts, transforms, or rejects construction input for this
   * schema.
   *
   * **Details**
   *
   * `succeed(input)` asserts make returns the input unchanged. `succeed(input, expected)` asserts
   * make returns `expected`. `fail(input, message)` asserts make fails with `message`.
   *
  ...
  /**
   * Runs a property-based test that encodes arbitrary values and then decodes them, asserting the
   ...
   * **Details**
   *
   * FastCheck generates arbitrary values matching the schema's `Type`. The assertion fails if any
   * generated value does not round-trip. Pass `options.params` to control FastCheck parameters such
   * as `numRuns`.
   *
   ...
   * **Details**
   *
   * `verifyGeneration()` generates arbitrary values and asserts each value satisfies the schema's
   * `is` predicate. It defaults to 20 runs. Pass `options.params` to override FastCheck
   * parameters.
   *
...
/**
 * Provides decoding test assertions through `succeed` and `fail` methods that run the schema's
 * decoder and compare the result.
 *
 * **When to use**
 *
 * Use when you want to assert that specific inputs decode to expected values, invalid inputs
 * produce specific error messages, or schemas receive required decoding services.
 *
 * **Details**
 *
 * All assertions are async and use `assert.deepStrictEqual` internally. `succeed(input)` asserts
 * the decoded output equals `input`; `succeed(input, expected)` asserts it equals `expected`;
 * `fail(input, message)` asserts decoding fails and the stringified issue equals `message`.
 * `provide(key, impl)` returns a new `Decoding` with the service injected into the decoding
 * context.
 *
...
export class Decoding<S extends Schema.Constraint> {
  ...
  /**
   * Asserts that decoding `input` succeeds. With one argument, asserts the output equals the input.
   * With two arguments, asserts the output equals `expected`.
   *
   * **When to use**
   *
  ...
  /**
   * Returns a new {@link Decoding} instance with the given service injected into the decoding
   * effect context.
   *
   * **When to use**
   *
...
/**
 * Provides encoding test assertions through `succeed` and `fail` methods that run the schema's
 * encoder and compare the result.
 *
 * **When to use**
 *
 * Use when you want to assert that specific values encode to expected outputs, invalid inputs
 * produce specific error messages, or schemas receive required encoding services.
 *
 * **Details**
 *
 * All assertions are async and use `assert.deepStrictEqual` internally. `succeed(input)` asserts
 * the encoded output equals `input`; `succeed(input, expected)` asserts it equals `expected`;
 * `fail(input, message)` asserts encoding fails and the stringified issue equals `message`.
 * `provide(key, impl)` returns a new `Encoding` with the service injected into the encoding
 * context.
 *
 * **Example** (Encoding assertions)
 *
...
export class Encoding<S extends Schema.Constraint> {
  ...
  /**
   * Asserts that encoding `input` succeeds. With one argument, asserts the output equals the input.
   * With two arguments, asserts the output equals `expected`.
   *
   * **When to use**
   *
  ...
  /**
   * Returns a new {@link Encoding} instance with the given service injected into the encoding
   * effect context.
   *
   * **When to use**
   *
