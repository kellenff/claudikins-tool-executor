 ...
 *
 * An `Order<A>` compares two `A` values and returns whether the first is less than, equal to, or
 * greater than the second. Orders are used for sorting, choosing minimum or maximum values,
 * checking ranges, and building ordered data structures. This module includes built-in orders,
 * constructors for custom orders, tools for reversing and combining comparisons, tuple and struct
 ...
 * **Details**
 *
 * An order returns `-1` when the first value is less than the second, `0` when the values are equal
 * according to this ordering, and `1` when the first value is greater than the second. It must
 * satisfy total ordering laws: totality, antisymmetry, and transitivity.
 *
 ...
 * **Details**
 *
 * This is type-level only, has no runtime representation, and is used internally by the Effect type
 * system.
 *
 * @since 2.0.0
 ...
 * **When to use**
 *
 * Use when you need a sorting rule not covered by the built-in orders or input mapping helpers, and
 * you can provide a total comparison.
 *
 * **Details**
 *
 * Uses reference equality (`===`) as a shortcut: if `self === that`, it returns `0` without calling
 * the comparison function. The comparison function should return `-1`, `0`, or `1`, and the
 * returned order satisfies total ordering laws when the comparison function does.
 *
 ...
 * **Details**
 *
 * Uses lexicographic dictionary ordering. The empty string is less than any non-empty string, and
 ...
 * **Details**
 *
 * `0` is considered equal to `-0`. All `NaN` values are considered equal to each other, and any
 * `NaN` is considered less than any non-`NaN` number. All other values use standard numeric
 * comparison.
 *
 ...
 * **Details**
 *
 * Returns a new order that swaps the arguments before comparison. If the original order returns
 * `-1`, the flipped order returns `1`, and vice versa. Equal comparisons remain `0`.
 *
...
/**
 * Combines two `Order` instances to create a new `Order` that first compares using the first
 * `Order`, and if the values are equal, then compares using the second `Order`.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * First applies the first order. If the result is non-zero, that result is returned; otherwise, the
 * second order is applied. The result is the first non-zero comparison result, or `0` if both
 * orders return `0`.
 *
 ...
 * **Details**
 *
 * Always returns `0` regardless of input values, making it useful as a neutral element in order
 * composition.
 *
...
/**
 * Combines all `Order` instances in the provided collection into a single `Order`. The resulting
 * `Order` compares using each `Order` in sequence until a non-zero result is found.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Applies orders in iteration order and short-circuits on the first non-zero result. It returns `0`
 * only if all orders return `0`.
 *
...
/**
 * Transforms an `Order` on type `A` into an `Order` on type `B` by providing a function that maps
 * values of type `B` to values of type `A`.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Applies the mapping function to both values before comparison. The mapping function should be
 * pure and not have side effects so the ordering properties of the original order are preserved.
 *
 ...
 * **Details**
 *
 * Compares dates by their underlying timestamp in milliseconds since the epoch. Earlier dates are
 * less than later dates. Invalid dates are compared through their `getTime()` result.
 *
 ...
 * **Details**
 *
 * Compares tuples element-by-element using the corresponding order and stops at the first non-zero
 * comparison result. Tuples must have the same length as the order collection, and the result is
 * `0` only if all elements are equal.
 *
...
export {
  /**
   * Creates an `Order` for arrays by applying the given `Order` to each element, then comparing by
   * length if all elements are equal.
   *
   * **When to use**
   *
   ...
   * **Details**
   *
   * Compares arrays element-by-element using the provided order and stops at the first non-zero
   * comparison result. If all elements are equal, shorter arrays are less than longer arrays. The
   * result is `0` only if arrays have the same length and all elements are equal.
   *
 ...
 * **Details**
 *
 * Compares structs field-by-field in the key order of the fields object and stops at the first
 * non-zero comparison result. Field order matters: earlier fields take precedence. The result is
 * `0` only if all fields are equal.
 *
 ...
 * **Details**
 *
 * Returns `true` if the order returns `-1`, meaning the first value is less than the second. Equal
 * or greater values return `false`.
 *
 ...
 * **Details**
 *
 * Returns `true` if the order returns `1`, meaning the first value is greater than the second.
 * Equal or lesser values return `false`.
 *
 ...
 * **Details**
 *
 * Returns `true` if the order returns `-1` or `0`, and returns `false` only if the order returns
 * `1`.
 *
...
/**
 * Returns the minimum of two values according to the given order. If they are equal, returns the
 * first argument.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Returns the value that compares as less than or equal to the other value. If values are equal,
...
/**
 * Returns the maximum of two values according to the given order. If they are equal, returns the
 * first argument.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Returns the value that compares as greater than or equal to the other value. If values are equal,
 * the first argument is returned.
 *
 ...
 * **Details**
 *
 * Returns the value itself when it is between minimum and maximum, inclusive. Values below the
 * range return minimum, and values above the range return maximum. The minimum must be less than or
 * equal to the maximum according to the order.
 *
...
/**
 * Checks whether a value is between a minimum and a maximum (inclusive) according to the given
 * order.
 *
 * **When to use**
 *
 * Use when you need range checks that respect domain-specific ordering, such as dates, versions, or
 * custom priorities, instead of JavaScript numeric comparison.
 *
 * **Details**
 *
 * Returns `true` when the value is greater than or equal to minimum and less than or equal to
 * maximum. Values outside the range return `false`. Both bounds are inclusive.
 *
...
/**
 * Creates a `Reducer` for combining `Order` instances, useful for aggregating orders in
 * collections.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Returns a reducer that combines orders using `combine`, uses `alwaysEqual` as the identity
 * element for empty collections, and uses `combineAll` for combining collections of orders. The
 * reducer can be used with fold operations on collections.
 *
