 ...
 *
 * Unification collapses unions of protocol-enabled values into their public data types. It is
 * mostly for maintainers of Effect data types and advanced library authors; application code
 * usually benefits from it through APIs such as `Effect`, `Option`, `Result`, `Stream`, `Layer`,
 * and `Match`. This module exports the protocol symbols, the `Unify` type that performs
 * normalization, and `unify`, an identity function that changes only the inferred type.
 *
 * @since 2.0.0
 */
 ...
 * **When to use**
 *
 * Use to define the widened type produced by the `Unify` protocol for a custom protocol-enabled
 * data type.
 *
 * **Details**
 *
 * This symbol is a type-level protocol key. It describes how a protocol-enabled type widens during
 * unification and has no runtime behavior.
 *
 * @since 2.0.0
 * @category Symbols
 ...
 * **Details**
 *
 * This type represents the unique symbol used for identifying unification behavior in Effect types.
 * It's typically used in type-level operations to enable automatic type unification.
 *
 * @since 2.0.0
 * @category Symbols
 */
 ...
 * **When to use**
 *
 * Use when you need a type-level protocol key that exposes the source type read by `Unify` from a
 * protocol-enabled data type.
 *
 * **Details**
 *
 * This symbol is a type-level protocol key. It stores the source type that unification reads when
 * widening protocol-enabled values.
 *
 * @since 2.0.0
 * @category Symbols
 ...
 * **Details**
 *
 * This type represents the unique symbol used for storing type information in types that support
 * unification. It's used in type-level operations to access and manipulate type information.
 *
 * @since 2.0.0
 * @category Symbols
 */
 ...
 * **When to use**
 *
 * Use to hide helper protocol entries from `Unify` when they should not contribute to the widened
 * type.
 *
 * **Details**
 *
 * This symbol is a type-level protocol key. It lists protocol entries that unification should
 * ignore when computing the widened type.
 *
 * @since 2.0.0
 * @category Symbols
 ...
 * **Details**
 *
 * This type represents the unique symbol used for marking types that should be ignored during
 * unification operations. It's used in type-level operations to exclude specific types from the
 * unification process.
 *
 * @since 2.0.0
 * @category Symbols
 ...
 * **Details**
 *
 * This type performs automatic type unification for types that contain the unification symbols
 * (`unifySymbol`, `typeSymbol`, `ignoreSymbol`). It's primarily used internally by the Effect type
 * system to handle complex type unions and provide better type inference.
 *
 ...
 * **When to use**
 *
 * Use to keep a value or function unchanged at runtime while normalizing its inferred type with
 * Effect's unification protocol.
 *
 * **Details**
 *
 * This is an identity function at runtime. For functions, the returned function has the same
 * runtime behavior while its return type is normalized with the Effect unification protocol.
 *
