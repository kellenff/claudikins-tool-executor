 ...
 *
 * A `Logger<Message, Output>` receives each log event as `Options` and turns it into output such as
 * a formatted string, structured object, console write, file write, JSON line, or trace span event.
 * This module also includes active logger references, console routing helpers, built-in formatters,
 * batching, file logging, and layers for installing loggers.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * The runtime calls `log` with the message, level, cause, fiber, and timestamp for each log event.
 * Use `Logger.layer` to install one or more loggers for an effect.
 *
 ...
 * **Details**
 *
 * By default this set includes the default logger and the tracer logger. Providing `Logger.layer`
 * replaces or merges with this set depending on its options.
 *
...
/**
 * Context reference that routes the built-in default logger and TTY pretty console logger to
 * stderr.
 *
 * **When to use**
 *
 * Use to route built-in logger output to stderr while keeping stdout reserved for protocol messages
 * or data output.
 *
 * **Details**
 *
 * The reference defaults to `false`. Providing `true` makes the affected loggers call
...
/**
 * Returns a new `Logger` that writes all output of the specified `Logger` to the console using
 * `console.log`.
 *
 * **When to use**
 *
 * Use when a logger's string or object output should be routed to `console.log` for development or
 * debugging.
 *
 ...
 * **When to use**
 *
 * Use when logger output should be routed to `console.error`, such as error logs that should appear
 ...
 * const errorFormatter = Logger.make(
 *   (options) => `ERROR [${options.date.toISOString()}]: ${options.message}`,
 * );
 *
 ...
 * **Details**
 *
 * Will use the appropriate console method (i.e. `console.log`, `console.error`, etc.) based upon
 * the current `LogLevel`.
 *
 * `Debug` uses `console.debug`, `Info` uses `console.info`, `Trace` uses `console.trace`, `Warn`
 * uses `console.warn`, `Error` and `Fatal` use `console.error`, and all other levels use
 * `console.log`.
 *
...
/** Match strings that do not contain any whitespace characters, double quotes, or equal signs. */
const textOnly = /^[^\s"=]*$/;
...
/**
 * Used by both {@link formatSimple} and {@link formatLogFmt} to render a log message.
 *
 * @internal
 */
const format =
 ...
 * **Details**
 *
 * The log function receives an options object containing the message, log level, cause, fiber
 * information, and timestamp, and should return the desired output.
 *
 ...
 * **Details**
 *
 * For example, a simple log entry is rendered as `timestamp=2025-01-03T14:22:47.570Z level=INFO
 * fiber=#1 message=hello`.
 *
 ...
 * **Details**
 *
 * For example, a logfmt entry is rendered as `timestamp=2025-01-03T14:22:47.570Z level=INFO
 * fiber=#1 message=hello`.
 *
 ...
 * **Details**
 *
 * For example, a structured entry can contain `message: [ "hello" ]`, `level: "INFO"`, `timestamp:
 * "2025-01-03T14:25:39.666Z"`, `annotations: { key: "value" }`, `spans: { label: 0 }`, and
 * `fiberId: "#1"`.
 *
 ...
 * **Details**
 *
 * The returned effect starts a scoped background process that periodically passes buffered outputs
 * to `flush`. When the scope closes, the background process is interrupted and any remaining
      ...
      return effect.uninterruptibleMask((restore) =>
        restore(effect.sleep(options.window).pipe(effect.andThen(flush), effect.forever)).pipe(
          ...
          effect.flatMap((fiber) =>
            effect.scopeAddFinalizerExit(scope, () => effect.fiberInterrupt(fiber)),
          ),
 ...
 * **Details**
 *
 * For example, pretty output can render as `[09:37:17.579] INFO (#1) label=0ms: hello` followed by
 * an annotation line such as `key: value`.
 *
...
/**
 * A `Logger` which outputs logs using the [logfmt](https://brandur.org/logfmt) style and writes
 * them to the console.
 *
 * **Details**
 *
 * For example, a console logfmt entry is rendered as `timestamp=2025-01-03T14:22:47.570Z level=INFO
 * fiber=#1 message=info`.
 *
 ...
 * **Details**
 *
 * For example, console structured output can contain `message: [ "info", "message" ]`, `level:
 * "INFO"`, `timestamp: "2025-01-03T14:25:39.666Z"`, `annotations: { key: "value" }`, `spans: {
 * label: 0 }`, and `fiberId: "#1"`.
 *
...
/**
 * A `Logger` which outputs logs using a structured format serialized as JSON on a single line and
 ...
 * **Details**
 *
 * This logger integrates logging with distributed tracing by recording all log messages as events
 * on the current trace span, making them visible in tracing tools like OpenTelemetry, Jaeger, or
 * Zipkin.
 *
 * This logger is included in the default set of loggers for all Effect programs, so log messages
 * automatically appear as span events unless you override the default loggers.
 *
...
/**
 * Creates a `Layer` which will overwrite the current set of loggers with the specified array of
 * `loggers`.
 *
 * **Details**
 *
 * If the specified array of `loggers` should be _merged_ with the current set of loggers (instead
 * of overwriting them), set `mergeWithExisting` to `true`.
 *
    ...
    withFiber(
      effect.fnUntraced(function* (fiber) {
        const currentLoggers = new Set(
          options?.mergeWithExisting === true ? fiber.getRef(effect.CurrentLoggers) : [],
        );
 ...
 * **Details**
 *
 * The returned effect requires `FileSystem` and `Scope`. The file logger batches string output,
 * writes each batch to the specified path, and flushes remaining entries when the scope closes.
 *
