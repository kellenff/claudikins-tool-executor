/**
 * Reusable synchronization primitives for coordinating fibers. A `Latch` is either open or closed:
 * when it is closed, `await` and `whenOpen` suspend until the latch opens or the current waiters
 * are released. The module includes effectful and synchronous constructors plus helpers to open,
 * release, close, wait, and gate effects behind the latch.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * A closed latch causes `await` and `whenOpen` to suspend. `open` opens the latch and releases
 * current and future waiters, `release` releases only current waiters without opening it, and
 * `close` makes future waiters suspend again.
 *
...
export interface Latch {
  ...
  /**
   * Runs the given effect only after the latch allows waiting fibers to continue.
   *
   * **When to use**
   *
 ...
 * **Details**
 *
 * The returned effect succeeds with `true` when this call changed the latch from closed to open, or
 ...
 * **When to use**
 *
 * Use when you need synchronous code to open a latch immediately and release the fibers waiting on
 * it.
 *
 * **Details**
 *
 * Returns `true` when this call changed the latch from closed to open, or `false` if it was already
 * open. This unsafe variant performs the state change immediately instead of returning an
 * `Effect`.
 *
 * @since 4.0.0
 * @category Unsafe
 ...
 * **When to use**
 *
 * Use to let the fibers currently waiting on a latch proceed while keeping the latch closed for
 * future waiters.
 *
 * **Details**
 *
 * The returned effect succeeds with `true` when release was requested while the latch was closed,
 * or `false` if the latch was already open. Future waiters still suspend until the latch is opened
 * or released again.
 *
 * @since 4.0.0
 * @category Combinators
...
export {
   ...
   * **When to use**
   *
   * Use to suspend the current fiber until the latch is opened or the current set of waiters is
   * released.
   *
   * **Details**
   *
   * Awaiting an already open latch completes immediately. Awaiting a closed latch suspends until
   * `open` or `release` resumes the waiters.
   *
   * **Gotchas**
   *
   * `release` can resume current waiters without opening the latch, so later waiters may still
   * suspend.
   *
   * @since 4.0.0
   * @category Getters
 ...
 * **When to use**
 *
 * Use to re-enable waiting on a latch after it was opened, so later `await` and `whenOpen` calls
 * suspend again.
 *
 * **Details**
 *
 * The returned effect succeeds with `true` when this call changed the latch from open to closed, or
 ...
 * **Details**
 *
 * Returns `true` when this call changed the latch from open to closed, or `false` if it was already
 * closed. This unsafe variant performs the state change immediately instead of returning an
 * `Effect`.
 *
 * @since 4.0.0
 * @category Unsafe
 ...
 * **When to use**
 *
 * Use to gate another effect so it starts only after the latch is opened or the current waiters are
 * released.
 *
 * **Details**
 *
 * If the latch is open, the effect runs immediately. If it is closed, the returned effect suspends
 * until the latch is opened or the current waiters are released. The provided effect's success,
 * failure, and requirements are preserved.
 *
 * @since 4.0.0
 * @category Combinators
