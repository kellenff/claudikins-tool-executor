 ...
 *
 * The runtime helpers in this module create new objects instead of mutating their inputs. They
 * cover common object workflows such as reading properties, listing typed keys, picking or omitting
 * fields, assigning and renaming keys, transforming values, deriving comparison helpers, and
 * creating records from a list of keys. The module also includes type-level helpers for simplifying
 * and merging object shapes.
 *
 * @since 2.0.0
 */
 ...
 * **Details**
 *
 * This helper is purely cosmetic at the type level and has no runtime effect. It preserves
 * `readonly` modifiers; use {@link Mutable} to strip them.
 *
 ...
 * **Details**
 *
 * This helper is purely cosmetic at the type level and has no runtime effect. It also flattens
 * intersections like {@link Simplify}.
 *
...
/**
 * Merges two object types with properties from `U` taking precedence over `T` on overlapping keys
 ...
 * **Details**
 *
 * When no keys overlap, this returns a simple intersection for efficiency. When keys overlap, the
 * type from `U` wins.
 *
 ...
 * **When to use**
 *
 * Use when you want a typed replacement for `Object.keys` that narrows the result to the known
 * string keys of the struct.
 *
 * **Gotchas**
 *
...
/**
 * Merges two structs into a new struct. When both structs share a key, the value from `that` (the
 * second struct) wins.
 *
 * **When to use**
 *
...
/**
 * Transforms values of a struct selectively using per-key functions. Keys without a corresponding
 * function are copied unchanged.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Each transform function receives the current value and returns the new value; the return type can
...
/**
 * Transforms keys of a struct selectively using per-key functions. Keys without a corresponding
 * function are copied unchanged.
 *
 * **When to use**
 *
...
/**
 * Transforms both keys and values of a struct selectively. Each per-key function receives `(key,
 * value)` and must return a `[newKey, newValue]` tuple. Keys without a corresponding function are
 * copied unchanged.
 *
 * **When to use**
 *
...
/**
 * Renames keys in a struct using a static `{ oldKey: newKey }` mapping. Keys not mentioned in the
 * mapping are copied unchanged.
 *
 * **When to use**
 *
...
/**
 * Creates an `Equivalence` for a struct by providing an `Equivalence` for each property. Two
 * structs are equivalent when all their corresponding properties are equivalent.
 *
 * **When to use**
 *
 * Use when you need equality for a record-like object to be decided field by field, with a custom
 * equality rule for each property.
 *
 * **Details**
 *
 * This is an alias of `Equivalence.Struct`. Each property's equivalence is checked independently;
 * all must return `true` for the overall result to be `true`.
 *
...
/**
 * Creates an `Order` for a struct by providing an `Order` for each property. Properties are
 * compared in the order they appear in the fields object; the first non-zero comparison determines
 * the result.
 *
 * **When to use**
 *
 * Use when you need to sort record-like objects lexicographically by several fields, with each
 * field using its own ordering rule.
 *
 * **Details**
 *
 * This is an alias of `Order.Struct`. The order of keys in the `fields` object determines
 * comparison priority.
 *
 ...
 * **When to use**
 *
 * Use when defining a typed function for {@link map}, {@link mapPick}, or {@link mapOmit}.
 *
 * **Details**
 *
 * Extend this interface with concrete `~lambda.in` and `~lambda.out` types to describe how a
 * function transforms values at the type level. At runtime, create lambda values with
 * {@link lambda}.
 *
...
/**
 * Wraps a plain function as a {@link Lambda} value so it can be used with {@link map},
 * {@link mapPick}, and {@link mapOmit}.
 *
 * **When to use**
 *
 * Use to create a typed lambda for struct mapping APIs that need type-level input and output
 * tracking.
 *
 * **Details**
 *
 * The type parameter `L` encodes both the input and output types at the type level, allowing the
 * compiler to track how struct value types change. At runtime, the returned value is the same
 * function; `lambda` only adjusts the type.
 *
 ...
 */
export const lambda = <L extends (a: any) => any>(f: (a: Parameters<L>[0]) => ReturnType<L>): L =>
  f as any;
 ...
 * **Details**
 *
 * The lambda must be created with {@link lambda} so the compiler can track the output types.
 *
...
/**
 * Applies a {@link Lambda} transformation only to the specified keys; all other keys are copied
 * unchanged.
 *
 * **When to use**
 *
...
/**
 * Applies a {@link Lambda} transformation to all keys except the specified ones; the excluded keys
 ...
 *
 * The callback returns either • `undefined` → nothing is copied, or • `[newKey, newVal]`
 *
...
/**
 * Creates a `Combiner` for a struct shape by providing a `Combiner` for each property. When two
 * structs are combined, each property is merged using its corresponding combiner.
 *
 * **When to use**
 *
 * Use when you need to merge two same-shape records by combining each property independently, such
 * as summing counters or concatenating strings.
 *
 * **Details**
 *
 * Pass `omitKeyWhen` to drop properties whose merged value matches a predicate, such as omitting
 * zero counters.
 *
...
/**
 * Creates a `Reducer` for a struct shape by providing a `Reducer` for each property. The initial
 * value is derived from each property's `Reducer.initialValue`. When reducing a collection of
 * structs, each property is combined independently.
 *
 * **When to use**
 *
 * Use when you need to fold same-shape records by accumulating each property independently into one
 * summary record.
 *
 * **Details**
 *
