/**
 * Service and helpers for reading time and sleeping inside Effect programs. The active `Clock`
 * provides current time in milliseconds or nanoseconds and a `sleep` operation for delaying work.
 * Because time is accessed through a service, tests can replace the clock with a controlled
 * implementation.
 *
 * @since 2.0.0
 */
...
export interface Clock {
   ...
   * **When to use**
   *
   * Use to read millisecond time synchronously when you already have a `Clock` service and can
   * accept non-effectful access.
   */
   ...
   * **When to use**
   *
   * Use to read nanosecond time synchronously when you already have a `Clock` service and can
   * accept non-effectful access.
   */
 ...
 * **When to use**
 *
 * Use when you need to access or provide the full time service, including sleep operations, rather
 * than a single timestamp accessor.
 *
 ...
 * **When to use**
 *
 * Use when you need the full Clock service interface to perform multiple time operations or call
 * unsafe variants within a single effect.
 *
