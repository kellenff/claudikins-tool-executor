 ...
 *
 * Platform services such as file systems, terminals, and sockets use `PlatformError` to report
 * host-level failures in a consistent shape. The wrapper records whether the problem came from an
 * invalid argument or from the operating system, while preserving useful details such as the
 * module, method, path, descriptor, description, and original cause when available.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need to model caller input rejected before a platform operation runs, including
 * invalid-argument reason data.
 *
 * **Details**
 *
 * The error records the module and method that rejected the argument, with an optional description
 * and cause. It is usually wrapped in `PlatformError`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use to type or match the normalized `_tag` on `SystemError` values reported by platform
 * operations.
 *
 * **Details**
 *
 * The tags group lower-level platform errors into a stable set such as `NotFound`,
 * `PermissionDenied`, `TimedOut`, and `Unknown`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when you need normalized reason data for a platform or system operation failure, including
 * the operation details.
 *
 * **Details**
 *
 * The error records a normalized `_tag`, the module and method that failed, and optional details
 * such as the syscall, path or descriptor, description, and original cause. It is usually wrapped
 * in `PlatformError`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use as the shared error type for platform APIs that expose invalid arguments and host or
 * operating-system failures through a single `Effect` error channel.
 *
 * **Details**
 *
 * The `reason` field contains the underlying `BadArgument` or `SystemError`. When that reason has a
 * cause, the cause is preserved on the wrapper.
 *
 * @since 4.0.0
 * @category Models
