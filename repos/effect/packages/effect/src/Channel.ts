 ...
 *
 * A `Channel` can read input elements, write output elements, fail with a typed error, and finish
 * with a typed result while managing resources safely. Streams and sinks are built on channels, so
 * most application code uses those higher-level modules instead. This module is useful when
 * implementing stream operators or specialized streaming workflows.
 *
 * @since 2.0.0
 */
...
/**
 * A `Channel` is a nexus of I/O operations, which supports both reading and writing. A channel may
 * read values of type `InElem` and write values of type `OutElem`. When the channel finishes, it
 * yields a value of type `OutDone`. A channel may fail with a value of type `OutErr`.
 *
 * **Details**
 *
 * Channels are the foundation of Streams: both streams and sinks are built on channels. Most users
 * shouldn't have to use channels directly, as streams and sinks are much more convenient and cover
 * all common use cases. However, when adding new stream and sink operators, or doing something
 * highly specialized, it may be useful to use channels directly.
 *
 * Channels compose in a variety of ways:
 *
 * - **Piping**: One channel can be piped to another channel, assuming the input type of the second is
 *   the same as the output type of the first.
 * - **Sequencing**: The terminal value of one channel can be used to create another channel, and both
 *   the first channel and the function that makes the second channel can be composed into a
 *   channel.
 * - **Concatenating**: The output of one channel can be used to create other channels, which are all
 *   concatenated together. The first channel and the function that makes the other channels can be
 *   composed into a channel.
 *
 ...
 * **Details**
 *
 * This preserves all `Channel` type parameters when `Unify` normalizes unions or generic return
 * types that include channels. Users normally do not need to reference this interface directly.
 *
 * @since 2.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * It prevents the inherited `Effect` unifier from being selected when the channel-specific unifier
 * should preserve `Channel` input, output, and environment type parameters. Users normally do not
 * need to reference this interface directly.
 *
 * @since 2.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * Output element, output error, output done, and environment types are covariant. Input element,
 * input error, and input done types are contravariant. This is type-level machinery and is not used
 * directly at runtime.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **Details**
 *
 * The `_OutElem`, `_OutErr`, `_OutDone`, and `_Env` fields are covariant; the `_InElem`, `_InErr`,
 * and `_InDone` fields are contravariant. Users normally do not need to reference this interface
 * directly.
 *
 * @since 2.0.0
 * @category Models
...
/**
 * Creates a `Channel` from a transformation function that operates on upstream pulls, but also
 * provides a forked scope that closes when the resulting Channel completes.
 *
 * **When to use**
 *
 * Use when building channels that require scoped resource lifecycle management, providing both the
 * channel scope and a forked scope that automatically closes when the channel completes.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Acquires a resource, uses it to build a `Channel`, and guarantees that `release` runs with the
 * channel's `Exit` when the channel completes, fails, or is interrupted.
 *
 * **Details**
 *
 * Acquisition is uninterruptible. If acquisition fails, `use` is not run and `release` is not
 * registered.
 *
...
/**
 * Acquires a resource, emits the acquired value as a single channel element, and registers
 * `release` in the channel scope.
 *
 * **Details**
 *
 * The release action runs when the channel scope closes and receives the scope exit. If acquisition
 * fails, no element is emitted and `release` is not registered.
 *
 ...
 * const channelWithFallback = Channel.concatWith(failedChannel, () =>
 ...
 * **When to use**
 *
 * Use when the error value should be computed each time the channel runs instead of when the
 ...
 * **When to use**
 *
 * Use when the channel failure must preserve a full `Cause`, such as defects, interruptions, or
 * combined failures.
 *
...
/**
 * Creates a channel that evaluates an effect and uses its successful value as the channel's done
 * value without emitting any output elements.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * A successful `Take` emits a non-empty array of output elements. A failed `Take` fails the
 * channel. A done `Take` completes the channel with its done value.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Creates a channel that forwards upstream input elements, input errors, and the upstream done
 * value unchanged.
 *
 * @since 2.0.0
 * @category Constructors
 ...
 * **Details**
 *
 * This constructor creates a channel that reads from a PubSub subscription and outputs arrays of
 * values in chunks. It's useful when you want to process multiple values at once for better
 * performance.
 *
 ...
 * **Details**
 *
 * This constructor creates a channel that reads from a PubSub by automatically subscribing to it.
 * The channel outputs individual values as they are published to the PubSub, making it ideal for
 ...
 *
 * This constructor creates a channel that reads from a PubSub by automatically subscribing to it
 * and collecting values into arrays. The channel outputs arrays of values in chunks, making it
 ...
 * **Details**
 *
 * Output `Take` values are emitted as non-empty arrays. Failed `Take` values fail the channel. Done
 * `Take` values complete the channel.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **Details**
 *
 * Each yielded value is emitted as an output element. The iterator's return value becomes the
 * channel's done value. Thrown or rejected iterator errors are converted with `onError`. If the
 * channel scope closes early and the iterator has a `return` method, that method is called.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Creates a channel from an `AsyncIterable`, emitting each yielded value as a single-element
 * non-empty array.
 *
 * **Details**
 *
 * The iterator's return value becomes the channel's done value. Thrown or rejected iterator errors
 * are converted with `onError`. If the channel scope closes early and the iterator has a `return`
 ...
 * **When to use**
 *
 * Use when the terminal done value transformation needs services or can fail, while emitted
 * elements should pass through unchanged.
 *
 * @since 4.0.0
 * @category Sequencing
 */
export const mapDoneEffect: {
  ...
  <OutElem, OutErr, OutDone, InElem, InErr, InDone, Env, OutDone2, E, R>(
    ...
    transformPull(self, (pull) =>
      Effect.succeed(
        Pull.catchDone(pull, (done) => Effect.flatMap(f(done as OutDone), Cause.done)),
      ),
    ),
);
 ...
 * **When to use**
 *
 * Use when transforming each channel output needs an Effect, service dependency, failure channel,
 * or configured concurrency.
 *
 * **Details**
 *
 * The mapping function receives the output element and its zero-based index. By default elements
 * are mapped sequentially. Use `options.concurrency` to map multiple elements concurrently, and
 * `options.unordered` to allow concurrently mapped outputs to be emitted as soon as they complete.
 *
...
>(
  ...
  fromTransformBracket(
    Effect.fnUntraced(function* (upstream, scope, forkedScope) {
      ...
      const concurrencyN =
        options.concurrency === "unbounded" ? Number.MAX_SAFE_INTEGER : options.concurrency;
      ...
      if (options.unordered) {
        ...
        yield* semaphore.take(1).pipe(
          ...
          Effect.catchCause((cause) =>
            semaphore.withPermits(concurrencyN - 1)(Queue.failCause(queue, cause)),
          ),
...
/**
 * Returns a new channel which is the same as this one but applies the given function to the input
 * channel’s input elements.
 *
 * @since 2.0.0
 * @category Sequencing
 */
...
/**
 * Applies a side effect function to each output element of the channel, returning a new channel
 * that emits the same elements.
 *
 * **Details**
 *
 * The `tap` function allows you to perform side effects (like logging or debugging) on each element
 * emitted by a channel without modifying the elements themselves.
 *
 ...
 * **Details**
 *
 * The source channel's done value is preserved. Child channel done values are used only for
 * child-channel completion. By default child channels are run sequentially. Use
 * `options.concurrency` and `options.bufferSize` to run child channels concurrently.
 *
...
> =>
  fromTransform((upstream, scope) =>
    Effect.map(toTransform(self)(upstream, scope), (pull) => {
      ...
      const makePull: Pull.Pull<OutElem1, OutErr | OutErr1, OutDone, Env1> = Effect.flatMap(
        pull,
        (value) => {
...
/**
 * Concatenates this channel with another channel created from the terminal value of this channel.
 * The new channel is created using the provided function.
 *
...
export const concatWith: {
  ...
  > =>
    fromTransform((upstream, scope) =>
      Effect.sync(() => {
        let currentPull:
          | Pull.Pull<OutElem | OutElem1, OutErr1 | OutErr, OutDone1, Env1 | Env>
          | undefined;
...
/**
 * Concatenates this channel with another channel, so that the second channel starts emitting values
 * after the first channel has completed.
 *
 ...
 * **When to use**
 *
 * Use to coordinate pulling from two channels when each output element depends on both sides and
 * local state.
 *
 * **Details**
 *
 * The combining function receives the current state and pull functions for the left and right
 * channels. It returns the next output element together with the next state.
 *
 * @since 4.0.0
 * @category Sequencing
 */
 ...
 * **Details**
 *
 * If the source emits at least one element, the source is used unchanged. If the source completes
 * before emitting an element, the fallback function receives the source done value and returns the
 * replacement channel.
 *
 * @since 4.0.0
 * @category Sequencing
...
export const orElseIfEmpty: {
  ...
  > =>
    fromTransform((upstream, scope) =>
      Effect.sync(() => {
        let currentPull:
          | Pull.Pull<OutElem | OutElem1, OutErr1 | OutErr, OutDone | OutDone1, Env1 | Env>
          | undefined;
        const forkedScope = Scope.forkUnsafe(scope);
        const makePull = Effect.flatMap(toTransform(self)(upstream, forkedScope), (pull) => {
...
/**
 * Flattens a channel that emits `Take` values into a channel that emits the `Take` outputs
 * directly.
 *
 * **Details**
 *
 * Output `Take` values are emitted as non-empty arrays. Failed `Take` values fail the returned
 * channel. Done `Take` values complete the returned channel.
 *
 * @since 4.0.0
 * @category Transforming
 */
...
/**
 * Creates a new channel that consumes all output from the source channel but emits nothing,
 * preserving only the completion value.
 *
  ...
  <OutElem, OutErr, OutDone, InElem, InErr, InDone, Env, SO, SE, SR>(
    ...
    Schedule.toStepWithMetadata(
      ...
      Effect.map((step) => {
        ...
        const loop: Channel<OutElem, OutErr | SE, OutDone, InElem, InErr, InDone, Env | SR> =
          concatWith(
 ...
 * **Details**
 *
 * The schedule receives each output element as input. Schedule delays are applied between emitted
 * elements. If the schedule fails, the returned channel fails. If the schedule finishes, the
 * returned channel completes with the schedule output.
 *
 * @since 4.0.0
 * @category Sequencing
 */
  ...
  <OutElem, OutErr, OutDone, InElem, InErr, InDone, Env, SO, SE, SR>(
    ...
    transformPull(self, (pull, _scope) =>
      Effect.map(Schedule.toStepWithSleep(schedule), (step) => {
        const pullWithStep: Pull.Pull<OutElem, OutErr | SE, OutDone | SO, SR> = Effect.tap(
          pull,
          step,
        );
        return pullWithStep;
...
/**
 * Filters the output elements of a channel using a predicate function. Elements that don't match
 * the predicate are discarded.
 *
 ...
 * **When to use**
 *
 * Use to keep only channel output elements accepted by a `Filter` and emit each filter success
 * value.
 *
 * **Details**
 *
 * Successful filter results are emitted as mapped values. Failed filter results are discarded. The
 * source channel's errors and done value are preserved.
 *
 * @since 4.0.0
 * @category Filtering
 ...
 * **When to use**
 *
 * Use when the keep/discard decision depends on an Effect or service and predicate failures should
 * fail the returned channel.
 *
 * **Details**
 *
 * Elements for which the predicate succeeds with `true` are emitted. Elements for which the
 * predicate succeeds with `false` are discarded. Predicate failures fail the returned channel.
 *
 * @since 4.0.0
 * @category Filtering
 */
 ...
 * **When to use**
 *
 * Use to apply effectful logic that can discard channel output elements and emit transformed values
 * for the elements that pass.
 *
 * **Details**
 *
 * Successful filter results are emitted as mapped values. Failed filter results are discarded.
 * Failures from the effectful filter fail the returned channel.
 *
 * @since 4.0.0
 * @category Filtering
...
/**
 * Filters arrays of elements emitted by a channel, applying the filter to each element within the
 * arrays and only emitting non-empty filtered arrays.
 *
 ...
 * **Details**
 *
 * Successful filter results are kept as mapped values. Failed filter results are removed from the
 * array. Arrays that become empty are discarded.
 *
 * @since 4.0.0
 * @category Filtering
 */
 ...
 * **When to use**
 *
 * Use when filtering array-valued channel outputs requires Effects or services, and arrays that
 * become empty should be skipped.
 *
 * **Details**
 *
 * The predicate receives the element and its index within the array. Elements for which the
 * predicate succeeds with `true` are kept. Arrays that become empty are discarded. Predicate
 ...
 * **When to use**
 *
 * Use when array-valued channel outputs need an effectful filter-map that can fail and can discard
 * arrays that become empty.
 *
 * **Details**
 *
 * Successful filter results are kept as mapped values. Failed filter results are removed from the
 * array. Arrays that become empty are discarded. Failures from the effectful filter fail the
 * returned channel.
 *
 * @since 4.0.0
 * @category Filtering
...
/**
 * Maps over a channel statefully with an accumulator, where each element can produce multiple
 * output values.
 *
...
/**
 * Transforms a channel statefully by scanning over its output with an accumulator function. Emits
 * the intermediate results of the scan operation.
 *
...
/**
 * Transforms a channel statefully by scanning over its output with an effectful accumulator
 * function. Emits the intermediate results of the scan operation.
 *
 * **When to use**
 *
 * Use when maintaining accumulated state over channel output requires Effects or can fail, while
 * still emitting each intermediate state.
 *
...
/**
 * Catches any cause of failure from the channel and allows recovery by creating a new channel based
 * on the caught cause.
 *
  ...
  > =>
    fromTransform((upstream, scope) => {
      ...
      return Effect.map(toTransform(self)(upstream, forkedScope), (pull) => {
        let currentPull: Pull.Pull<OutElem | OutElem1, OutErr1, OutDone | OutDone1, Env | Env1> =
          pull.pipe(
...
/**
 * Runs an effect with the full failure `Cause` when the channel fails, then fails the returned
 * channel with the original cause.
 *
 * **When to use**
 *
 * Use when observing the full channel failure `Cause` is needed without changing successful output
 * or replacing the original cause.
 *
 * **Details**
 *
 * Use this for observing failures, such as logging or metrics. If the observer effect fails, that
 * failure can fail the returned channel.
 *
 * @since 4.0.0
 * @category Error handling
 */
...
/**
 * Catches causes of failure that match a specific filter, allowing conditional error recovery based
 ...
 * **Details**
 *
 * When the predicate matches, the recovery function receives the original cause. When it does not
 * match, the returned channel fails with the original cause.
 *
 * @since 4.0.0
 * @category Error handling
...
export const catchCauseIf: {
  ...
  > =>
    catchCause(
      self,
      (cause): Channel<OutElem1, OutErr | OutErr1, OutDone1, InElem1, InErr1, InDone1, Env1> => {
 ...
 * **When to use**
 *
 * Use when you need to recover a channel only from causes selected by a `Filter`, while giving the
 * recovery both the selected value and the original `Cause`.
 *
 * **Details**
 *
 * When the filter succeeds, the recovery function receives the selected value and the original
 * cause. When the filter fails, the returned channel fails with the residual cause produced by the
 * filter.
 *
 * @since 4.0.0
 * @category Error handling
...
/**
 * Runs an effect when the channel fails with a typed error, then preserves the original channel
 * failure.
 *
 * **Details**
 *
 * The effect is not run for normal channel completion. If the observer effect fails, that failure
 * can fail the returned channel.
 *
 * @since 4.0.0
 * @category Error handling
 */
export const tapError: {
  ...
  <OutElem, OutErr, OutDone, InElem, InErr, InDone, Env, A, E, R>(
    ...
    transformPull(self, (pull) =>
      Effect.succeed(
        Effect.tapError(pull, (err) => (Cause.isDone(err) ? Effect.void : Effect.asVoid(f(err)))),
      ),
    ),
);
 ...
 * **When to use**
 *
 * Use to recover from typed channel errors when a predicate or refinement selects the failures that
 * should switch to a recovery channel.
 *
 * **Details**
 *
 * Matching errors are handled by the recovery function. Non-matching errors are handled by `orElse`
 * when provided. Without `orElse`, non-matching errors are re-failed.
 *
 * @since 4.0.0
 * @category Error handling
...
export const catchIf: {
  ...
  >(
    ...
    orElse?:
      | ((failure: OutErr) => Channel<OutElem2, OutErr2, OutDone2, InElem2, InErr2, InDone2, Env2>)
      | undefined,
  ): <OutElem, OutDone, InElem, InErr, InDone, Env>(
    self: Channel<OutElem, OutErr, OutDone, InElem, InErr, InDone, Env>,
  ) => Channel<
 ...
 * **When to use**
 *
 * Use to recover from channel errors with a reusable `Filter` when matching can also narrow or
 * transform the error before choosing the recovery channel.
 *
 * **Details**
 *
 * Successful filter results are handled by the recovery function. Failed filter results are handled
 * by `orElse` when provided. Without `orElse`, failed filter results are re-failed.
 *
 * @since 4.0.0
 * @category Error handling
...
export const catchFilter: {
  ...
  >(
    ...
    f: (failure: EB) => Channel<OutElem1, OutErr1, OutDone1, InElem1, InErr1, InDone1, Env1>,
    orElse?:
      | ((failure: X) => Channel<OutElem2, OutErr2, OutDone2, InElem2, InErr2, InDone2, Env2>)
      | undefined,
  ): <OutElem, OutDone, InElem, InErr, InDone, Env>(
    self: Channel<OutElem, OutErr, OutDone, InElem, InErr, InDone, Env>,
  ) => Channel<
 ...
 * **Details**
 *
 * Matching tagged errors are handled by the recovery function. Non-matching errors are handled by
 * `orElse` when provided. Without `orElse`, non-matching errors are re-failed.
 *
 * @since 4.0.0
 * @category Error handling
 */
...
/**
 * Converts all errors in the channel to defects (unrecoverable failures). This is useful when you
 * want to treat errors as programming errors.
 *
 ...
 * **When to use**
 *
 * Use when a channel should become best-effort and all failure causes, including defects and
 * interruptions, can be converted to empty output.
 *
 * **Details**
 *
 * Use the `log` option to emit the full {@link Cause} when the channel fails.
...
/**
 * Returns a new channel that retries this channel according to the specified schedule whenever it
 * fails.
 *
 * @since 4.0.0
 * @category Error handling
...
export const retry: {
  ...
  <OutElem, OutErr, OutDone, InElem, InErr, InDone, Env, SO, SE, SR>(
    ...
    suspend(() => {
      let step:
        | ((input: OutErr) => Pull.Pull<Schedule.Metadata<SO, OutErr>, SE, SO, SR>)
        | undefined = undefined;
      let meta = Schedule.CurrentMetadata.defaultValue();
...
/**
 * Maps each output element to a channel and emits values from the most recent active child
 * channels.
 *
 * **Details**
 *
 * With the default concurrency of `1`, starting a new child channel interrupts the previous child
 * channel. Use `options.concurrency` to allow more active child channels. The source channel's done
 * value is preserved.
 *
 ...
 * **When to use**
 *
 * Use when channel outputs are themselves channels and multiple inner channels should run with
 * configured concurrency and buffering.
 *
...
export const mergeAll: {
  ...
  > =>
    fromTransformBracket(
      Effect.fnUntraced(function* (upstream, scope, forkedScope) {
        const concurrencyN =
          concurrency === "unbounded" ? Number.MAX_SAFE_INTEGER : Math.max(1, concurrency);
 ...
 * **When to use**
 *
 * Use when a side effect should run for the lifetime of a channel and only the channel's output
 * elements should be emitted.
 *
 * **Details**
 *
 * The effect's successful value is ignored. If the effect fails while the channel is running, the
 * returned channel fails with that error.
 *
 * @since 4.0.0
 * @category Combining
 */
...
/**
 * Splits upstream string chunks into lines, recognizing `\n`, `\r\n`, and standalone `\r` as line
 * terminators. The behavior matches `String.linesIterator` regardless of how the input is chunked.
 *
 * **Details**
 *
 * A line terminator at the very end of the stream does **not** produce a trailing empty line
 * (consistent with `String.linesIterator`). Conversely, if the stream ends without a terminator the
 * final partial line is still emitted.
 *
 ...
 * **Details**
 *
 * Input chunks are decoded with streaming enabled so multi-byte characters may span `Uint8Array`
 * boundaries. The optional `encoding` and `options` are passed to `TextDecoder`.
 *
 * @since 4.0.0
 * @category String manipulation
 */
...
/**
 * Returns a new channel that pipes the output of this channel into the specified channel. The
 * returned channel has the input type of this channel, and the output type of the specified
...
/**
 * Returns a new channel that pipes the output of this channel into the specified channel and
 * preserves this channel's failures without providing them to the other channel for observation.
 *
...
/**
 * Runs a channel with a scope provided for the duration of the channel execution, removing the
 * channel's `Scope` requirement.
 *
 * @since 2.0.0
...
/**
 * Runs an input handler against the upstream pull while the wrapped channel runs without receiving
 * upstream input directly.
 *
 * **Details**
 *
 * The input handler is forked in the channel scope. The wrapped channel is run with an
...
/**
 * Buffers individual output elements in a queue with the configured `capacity` so a faster producer
 * can progress independently of a slower consumer.
 *
 * **When to use**
 *
 * Use when output elements can be decoupled from downstream demand and the configured backpressure
 * or loss strategy is acceptable.
 *
 * **Details**
 *
 * Finite queues use the `strategy` option. The default `"suspend"` strategy applies backpressure,
 * while `"dropping"` and `"sliding"` can discard output elements when the queue is full.
 * `"unbounded"` capacity does not use a finite capacity strategy.
 *
 * **Gotchas**
 *
...
/**
 * Buffers array output elements in a queue with the configured `capacity` so a faster producer can
 * progress independently of a slower consumer.
 *
 * **When to use**
 *
 * Use when emitted arrays are batches of elements and it is acceptable for buffering to flatten and
 * rebuild those batches.
 *
 * **Details**
 *
 * Finite queues use the `strategy` option. The default `"suspend"` strategy applies backpressure,
 ...
 * **Gotchas**
 *
 * Input arrays are offered to the queue element-by-element and outputs are rebuilt from the
 * currently available queued elements, so upstream array boundaries are not preserved.
 *
 * @since 4.0.0
 * @category Buffering
 ...
 * **When to use**
 *
 * Use to race channel execution against an external effect whose success can become the channel's
 * done value.
 *
 * **Details**
 *
 * If the effect completes first, its success value becomes the returned channel's done value. If
 * the channel completes first, the original channel's done value is preserved.
 *
 * @since 2.0.0
 ...
 * **Details**
 *
 * If the effect completes before the channel is done, its success value becomes the returned
 * channel's done value. If the effect fails, the returned channel fails with that error. If the
 * channel completes first, the channel's done value is preserved.
 *
 * @since 4.0.0
 * @category Interruption
 */
 ...
 * **Details**
 *
 * The finalizer receives the failure `Cause`. The original channel failure is preserved. The
 * finalizer itself must not fail.
 *
 * @since 4.0.0
 * @category Error handling
 */
...
/**
 * Returns a channel with an exit-aware finalizer that is guaranteed to run once the channel begins
 * execution, whether it succeeds or fails.
 *
 ...
 * **Details**
 *
 * The effect's successful value is ignored. If the effect fails, the returned channel fails before
 * running the source channel.
 *
 * @since 4.0.0
 * @category Hooks
 */
 ...
 * **Details**
 *
 * The effect receives the first emitted element. The first element is still emitted unchanged. The
 * effect is not run if the channel completes without emitting an element.
 *
 * @since 4.0.0
 * @category Hooks
 */
 ...
 * **Details**
 *
 * The effect runs before the original done value is propagated. The effect is not run when the
 * channel fails. If the effect fails, the returned channel fails with that error.
 *
 * @since 4.0.0
 * @category Hooks
 */
...
/**
 * Returns a channel with a finalizer effect that is guaranteed to run once the channel begins
 * execution, whether it succeeds or fails.
 *
...
/**
 * Provides a `Context` to the channel, removing the corresponding service requirements from the
 * returned channel.
 *
 * @since 2.0.0
 * @category Services
...
/**
 * Provides a concrete service for a context key, removing that service requirement from the
 * returned channel.
 *
 * @since 2.0.0
 * @category Services
 ...
 * **Details**
 *
 * If the service effect fails, the returned channel fails. The provided service removes the
 * corresponding service requirement from the returned channel.
 *
 * @since 4.0.0
 * @category Services
 */
 ...
 * **Details**
 *
 * Providing a `Context` delegates to `provideContext`. Providing a `Layer` builds the layer in the
 * channel scope. Use `options.local` to build a fresh layer instance for this provision.
 *
 * @since 4.0.0
 * @category Services
 */
 ...
 * **Details**
 *
 * The function receives the surrounding context and returns the context to provide to the channel.
 * The returned channel requires the services needed to build that context.
 *
 * @since 4.0.0
 * @category Services
 */
 ...
 * **Details**
 *
 * The existing service is read from the context. The updated service is provided to the channel
 * under the same key.
 *
 * @since 2.0.0
 * @category Services
 ...
 * **Details**
 *
 * The created span is provided as the current parent span while the channel runs. The span is ended
 * with the channel's exit value.
 *
 * @since 2.0.0
 * @category Tracing
 */
...
/**
 * Adds a field to each object emitted by a channel by running another channel derived from that
 * object.
 *
 * **Details**
 *
 * The field name must not already exist on the emitted object. The derived channel's output becomes
 * the value of the new field. `options.concurrency` and `options.bufferSize` control how derived
 * channels are flattened.
 *
 * @since 4.0.0
 * @category Do notation
 */
 ...
 * **When to use**
 *
 * Use when you need to start a Channel Do-notation chain from an existing output value by assigning
 * that value to a field name.
 *
 * @since 4.0.0
 * @category Do notation
 ...
 * const completedChannel = Channel.concatWith(resultChannel, () =>
...
/**
 * Runs a channel and applies an effectful predicate to each output element until the predicate
 * returns `false`.
 *
 * **Details**
 *
 * Returning `true` continues consuming the channel. Returning `false` stops consumption early. The
 * returned effect completes with `void`.
 *
 * @since 4.0.0
 * @category Execution
 */
 ...
 * **Details**
 *
 * Returns `Option.some` with the first output element, or `Option.none` if the channel completes
 ...
 * **Details**
 *
 * Returns `Option.some` with the last emitted element, or `Option.none` if the channel completes
 * without emitting output.
 *
 * @since 4.0.0
 * @category Execution
 ...
 * **When to use**
 *
 * Use when folding channel output needs effects, services, or an additional failure channel during
 * accumulation.
 *
 * **Details**
 *
 * The initial accumulator is evaluated lazily. Each output element is passed to the effectful
 * accumulator function. The returned effect succeeds with the final accumulator value.
 *
 * @since 4.0.0
 * @category Execution
 */
 ...
 * **Details**
 *
 * The effect requires a `Scope`. The returned pull should be consumed only while that scope remains
 * open. Pulls are serialized so only one pull is evaluated at a time.
 *
 ...
 * **Details**
 *
 * When the channel completes, the queue is ended. When the channel fails, the queue is failed with
 * the channel's cause. The returned effect itself completes with `void`.
 *
 * @since 4.0.0
 * @category Destructors
 */
 ...
 * **Details**
 *
 * Output elements are offered to the queue. Channel completion and failure are signaled through the
 * queue. The queue is shut down when the surrounding scope closes.
 *
 ...
 * **Details**
 *
 * Each element inside emitted non-empty arrays is offered to the queue. Channel completion and
 * failure are signaled through the queue. The queue is shut down when the surrounding scope
 * closes.
 *
 * @since 4.0.0
 * @category Destructors
 ...
 * **Details**
 *
 * `shutdownOnEnd` indicates whether the PubSub should be shut down when the channel ends. By
 ...
 * **Details**
 *
 * Each element inside emitted non-empty arrays is published as an individual PubSub message.
 * `shutdownOnEnd` indicates whether the PubSub should be shut down when the channel ends. By
 * default this is `true`.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * Each element inside emitted non-empty arrays is published as an individual PubSub message. Use
 * `options.shutdownOnEnd` to shut down the PubSub when channel execution ends.
 *
 * @since 4.0.0
 * @category Destructors
 ...
 * **Details**
 *
 * Emitted non-empty arrays are published as output `Take` values. When the channel ends, its final
 * `Exit` is published so subscribers can observe completion or failure.
 *
 * @since 4.0.0
 * @category Destructors
 */
