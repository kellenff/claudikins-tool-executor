 ...
 *
 * An `ErrorReporter` receives `Cause` values from `Effect.withErrorReporting`, manual `report`
 * calls, or built-in reporting boundaries. It forwards each non-interruption error to a callback,
 * so applications can send failures to logging, monitoring, or error-tracking systems. This module
 * also includes layers for installing reporters and symbols for marking errors as ignored or
 ...
 * **Details**
 *
 * This marker is part of the runtime representation of `ErrorReporter` implementations. Most code
 * should create reporters with `make` and register them with `layer`.
 *
 * @since 4.0.0
 * @category Type IDs
 */
...
/**
 * An `ErrorReporter` receives reported failures and forwards them to an external system such as a
 * logging service or error tracker.
 *
 * **When to use**
 *
 * Use as the interface for custom reporters that forward reported Effect failures to logging,
 * monitoring, or error-tracking systems.
 *
 * **Details**
 *
 * Reporting is triggered by `Effect.withErrorReporting`, `ErrorReporter.report`, or built-in
 * boundaries in the HTTP and RPC server modules. Use {@link make} to create a reporter; it handles
 * deduplication and per-error annotation extraction automatically.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use to define how reported failures are forwarded to a logging, monitoring, or error-tracking
 * backend.
 *
 * **Details**
 *
 * The returned reporter automatically deduplicates causes and individual errors (the same object is
 * never reported twice), skips interruptions, and resolves the `ignore`, `severity`, and
 * `attributes` annotations on each error before invoking your callback.
 *
...
/**
 * Context reference that holds the set of active error reporters for the current fiber. Defaults to
 * an empty set (no reporting).
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Reporters can be plain `ErrorReporter` values or effectful `Effect<ErrorReporter>` values that
 * are resolved when the layer is built. By default the provided reporters **replace** any
 * previously registered reporters. Set `mergeWithExisting: true` to add them alongside existing
 * ones.
 *
 ...
 * **Details**
 *
 * All three annotation properties are optional: `[ErrorReporter.ignore]` prevents reporting when
 * set to `true`, `[ErrorReporter.severity]` overrides the default `"Info"` severity, and
 * `[ErrorReporter.attributes]` adds extra key/value pairs forwarded to reporters. The global
 * `Error` interface is augmented with `Reportable`, so these properties are available on `Error`
 ...
 * **Details**
 *
 * Set this property to `true` on an error class or object error to prevent it from being forwarded
 * to reporters. This is useful for expected failures such as HTTP 404 responses.
 *
 * @since 4.0.0
 * @category Annotations
 */
...
/**
 * Defines the runtime property key used to mark an object error as ignored by error reporting.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Set `error[ErrorReporter.ignore]` to `true` to prevent the error from being forwarded to
 * reporters. This is useful for expected failures such as HTTP 404 responses.
 *
 ...
 * **When to use**
 *
 * Use to check whether an error value is annotated to be skipped before forwarding it to error
 * reporting code.
 *
 * @since 4.0.0
 * @category Annotations
 ...
 * **Details**
 *
 * When set to a valid `LogLevel.Severity`, the reporter callback receives this value as `severity`.
 ...
 * **Details**
 *
 * Set `error[ErrorReporter.severity]` to a valid `LogLevel.Severity` value. Missing or invalid
...
/**
 * Reads the `ErrorReporter.severity` annotation from an error object, falling back to `"Info"` when
 * the annotation is unset or invalid.
 *
 * **When to use**
 *
...
/**
 * Defines the string property key used to attach extra key/value metadata to an object error
 * report.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Reporters receive these attributes alongside the error, making it easy to include contextual
 * information such as user IDs, request IDs, or other domain-specific debugging data.
 *
 * @since 4.0.0
 * @category Annotations
 */
...
/**
 * Defines the runtime property key used to attach extra key/value metadata to an object error
 * report.
 *
 * **When to use**
 *
 * Use to attach domain metadata to object errors so reporter callbacks receive it with the reported
 * failure.
 *
 * **Details**
 *
 * Set `error[ErrorReporter.attributes]` to a record of metadata that should be forwarded to
 * reporters alongside the error.
 *
...
/**
 * Reads the `ErrorReporter.attributes` annotation from an error object, returning an empty record
 * when unset.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * Returns the value stored under `ErrorReporter.attributes`, or the module's shared empty record
