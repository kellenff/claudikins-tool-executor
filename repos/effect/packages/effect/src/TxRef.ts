/**
 * Transactional references for coordinating mutable state with Effect transactions. A `TxRef`
 * stores a current value, but reads and writes inside `Effect.tx` are recorded in a transaction
 * journal and committed together only when the outermost transaction succeeds.
 *
 * This is the basic building block behind the other transactional collections in Effect. The module
 * provides effectful and unsafe constructors plus the core operations for reading, setting,
 * updating, and modifying a transactional value while returning a separate result.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Accessed values are tracked by the transaction in order to detect conflicts and in order to track
 * changes, a transaction will retry whenever a conflict is detected or whenever the transaction
 * explicitely calls to `Effect.txRetry` and any of the accessed TxRef values change.
 *
 ...
 * **When to use**
 *
 * Use to update a `TxRef` and return a computed result from the same transaction step.
 *
