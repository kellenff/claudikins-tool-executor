 ...
 *
 * An `Option<A>` is `Some<A>` when a value is available and `None` when it is not. This lets code
 * handle missing values explicitly instead of relying on `null` or `undefined`. The module includes
 * helpers for creating, checking, transforming, combining, and extracting optional values, plus
 * conversions to and from common nullable or result-like shapes. It also includes `Option.gen` for
 * writing small generator-based computations that stop at the first `None`.
 *
 * @since 2.0.0
 */
...
/**
 * The `Option` data type represents optional values. An `Option<A>` is either `Some<A>`, containing
 * a value of type `A`, or `None`, representing absence.
 *
 * **When to use**
 *
...
/**
 * Iterator protocol used to yield an `Option` inside {@link gen}, returning the contained value
 * type back to the generator.
 *
 * **When to use**
 *
 * Use when defining or typing `[Symbol.iterator]()` for `Option` values so `yield*` can pass the
 * contained value type back into `Option.gen`.
 *
 * @since 4.0.0
 * @category Generators
 ...
 * **Details**
 *
 * This is used by Effect's `Unify` machinery to preserve the contained value type when generic code
 * returns or combines `Option` values. Users normally do not need to reference this interface
 * directly.
 *
 * @since 2.0.0
 * @category Models
 ...
 * **Details**
 *
 * This supports type-level unification behavior for `Option`. Users normally do not need to
 ...
 * **When to use**
 *
 * Use when defining higher-kinded abstractions that must accept optional-value types as one of
 * their type-lambda inputs.
 *
 * @since 2.0.0
 ...
 * **When to use**
 *
 * Use when you need to handle both `Some` and `None` in one expression and transform an `Option`
 * into a plain value.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to turn an `Option`-returning parser into a type-narrowing predicate, such as
 ...
 * **When to use**
 *
 * Use when you need to safely extract the head of a collection, including generators or lazy
 * iterables.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need a lazy fallback `Option`, such as when building priority chains of optional
 * values.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need JavaScript nullish values to become absence at an API boundary while all other
 * values, including falsy ones, remain present.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you want to treat only `undefined` as absent while preserving `null` as a meaningful
 * value.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you want to treat only `null` as absent while preserving `undefined` as a meaningful
 * value.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need fail-fast unwrapping of an `Option` for unexpected absence and want to provide
 * a descriptive debugging error.
 *
 * **Details**
 *
 * - `Some` → returns the inner value
 ...
 * **When to use**
 *
 * Use to apply a pure transformation to an `Option`'s present value, especially when chaining
 ...
 * **When to use**
 *
 * Use when you need to replace a present `Option` value while preserving whether it was `Some` or
 * `None`.
 *
 ...
 * **When to use**
 *
 * Use when you need to discard a present `Option` value while preserving whether it was `Some` or
 * `None`.
 *
...
/**
 * Applies a function that returns an `Option` to the value of a `Some`, flattening the result.
...
/**
 * Chains a second computation onto an `Option`. The second value can be a plain value, an `Option`,
 * or a function returning either.
 *
 * **When to use**
 *
 * Use when you need to chain an `Option` with a next step that may be another `Option`, a plain
 * value, or a function.
 *
 * **Details**
 *
...
/**
 * Combines {@link flatMap} with {@link fromNullishOr}: applies a function that may return
 * `null`/`undefined` to the value of a `Some`.
 *
 * **When to use**
 *
 * Use when you need to chain optional computations that use `null` or `undefined` instead of
 * `Option`, such as nested property access.
 *
 * **Details**
 *
 * - `None` → `None`
 ...
 *
 * console.log(
 *   Option.some(emp).pipe(Option.flatMapNullishOr((e) => e.company?.address?.street?.name)),
 * );
 ...
 * **When to use**
 *
 * Use when you need to compose two functions that each return an `Option`, so `None` short-circuits
...
/**
 * Runs a side-effecting `Option`-returning function on the value of a `Some`, returning the
 * original `Option` if the function returns `Some`, or `None` if it returns `None`.
 *
 * **When to use**
 *
 * Use to validate an `Option`'s present value without transforming it, such as adding a
 * side-condition check in a pipeline.
 *
 * **Details**
 *
 * - `None` → `None`
 * - `Some` → calls `f(value)`; if result is `Some`, returns original `self`; if `None`, returns
 *   `None`
 *
 ...
 * **When to use**
 *
 * Use when you need to require two `Option` values to both be `Some` and keep both values as a
 * tuple.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need several `Option` values of the same type to all be `Some` and return them as a
 * non-empty tuple.
 *
 * **Details**
 *
...
/**
 * Combines a structure of `Option`s (tuple, struct, or iterable) into a single `Option` containing
 * the unwrapped structure.
 *
 * **When to use**
 *
 * Use when you need to combine multiple `Option` values into one while preserving the input shape,
 * with any `None` making the result `None`.
 *
 * **Details**
 *
 ...
 * import { Option } from "effect";
 *
 * const person = Option.zipWith(Option.some("John"), Option.some(25), (name, age) => ({
 ...
 * **When to use**
 *
 * Use when you need to pass an `Option` to array-based APIs or spread optional values into
 * collections.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when you need to split an optional value into "left" and "right" channels using a
 ...
 * **Details**
 *
 * The callback returns a `Result`: `Result.succeed` keeps and transforms the value, while
...
/**
 * Filters an `Option` using a predicate. Returns `None` if the predicate is not satisfied or the
 * input is `None`.
 *
 * **When to use**
 *
 * Use when you need to discard an `Option`'s present value when it does not meet a condition, while
 ...
 * **When to use**
 *
 * Use when you need equality to treat two `None` values as equal and compare two `Some` values with
 * a supplied equality rule.
 *
 * **Details**
 *
 * - `None` vs `None` → `true`
 ...
 * **When to use**
 *
 * Use when you need to sort `Some` and `None` values, with `None` ordered before present values and
 * present values compared by a supplied ordering rule.
 *
 * **Details**
 *
...
/**
 * Lifts a `Predicate` or `Refinement` into the `Option` context: returns `Some(value)` when the
 * predicate holds, `None` otherwise.
 *
 * **When to use**
 *
...
/**
 * Checks whether an `Option` contains a value equivalent to the given one, using a custom
 * `Equivalence`.
 *
 * **When to use**
 *
...
/**
 * Checks whether an `Option` contains a value equal to the given one, using default structural
 * equality.
 *
 * **When to use**
 *
...
/**
 * Gives a name to the value of an `Option`, creating a single-key record inside `Some`. Starting
 * point for the do notation pipeline.
 *
 * **When to use**
 *
...
export {
   ...
   * **When to use**
   *
   * Use when you need to bind a derived non-`Option` value in an `Option` do notation pipeline.
   *
...
/**
 * Adds an `Option` value to the do notation record under a given name. If the `Option` is `None`,
 * the whole pipeline short-circuits to `None`.
 *
 * **When to use**
 *
...
/**
 * Provides an `Option` containing an empty record `{}`, used as the starting point for do notation
 * chains.
 *
 * **When to use**
 *
...
/**
 * Provides generator-based syntax for `Option`, similar to `async`/`await` but for optional values.
 * Yielding a `None` short-circuits the generator to `None`.
 *
 * **When to use**
 *
 * Use when you need generator syntax for a sequence of `Option` steps that should short-circuit on
 * `None`.
 *
 * **Details**
 *
...
/**
 * Creates a `Reducer` for `Option<A>` that prioritizes the first non-`None` value and combines
 * values when both are `Some`.
 *
 * **When to use**
 *
 * Use to build an `Option` reducer that falls back to the first available value when either side
 * may be absent.
 *
 * **Details**
 *
...
/**
 * Creates a `Combiner` for `Option<A>` with fail-fast semantics: returns `None` if either operand
 * is `None`.
 *
 * **When to use**
 *
 ...
 * **When to use**
 *
 * Use when you need to reduce `Option` values with fail-fast semantics, where any `None` aborts the
 * entire result instead of being skipped.
 *
 * **Details**
 *
