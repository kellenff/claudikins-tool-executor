/**
 * The `JsonPatch` module computes and applies deterministic patch documents for JSON values. A
 * patch is an ordered list of `add`, `remove`, and `replace` operations addressed by JSON Pointer
 * paths. Use it to describe the structural difference between two JSON documents, serialize that
 * difference, and replay it without mutating the original input.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to manually construct patch operations, accept patch operations from callers, or type-check
 * patch operation structures.
 *
 * **Details**
 *
 * Represents one transformation step in a JSON Patch document. This is a subset of RFC 6902,
 * restricted to operations that can be applied deterministically without additional context. All
 * fields are readonly, paths use JSON Pointer syntax, and the empty string `""` refers to the root
 * document. Operations are discriminated by the `op` field, and the optional `description` field
...
export type JsonPatchOperation =
  | {
      readonly op: "add";
      /**
       * JSON Pointer to the target location. For arrays, the last token may be `-` to append.
       *
       * **When to use**
       *
 ...
 * **Details**
 *
 * Represents a complete transformation as a readonly sequence of immutable operations. Operations
 * are applied sequentially from first to last, and later operations observe the document state
 * produced by earlier operations. An empty array represents a no-op patch and returns the original
 * document.
 *
 ...
 * **When to use**
 *
 * Use to compute a JSON Patch from before and after JSON documents, detect structural changes, or
 * create deterministic update operations.
 *
 * **Details**
 *
 * Generates a structural diff between two JSON values, producing a patch that yields `newValue`
 * when applied to `oldValue`. It returns an empty array when values are identical, recursively
 * diffs nested structures, emits root `replace` operations for primitive changes, and processes
 * object keys in sorted order for stable output.
 *
 * **Gotchas**
 *
 * Arrays are compared by index position, with no move or copy detection. Array removals are emitted
 * from highest to lowest index to prevent index shifting. The output is deterministic but not
 ...
 * **When to use**
 *
 * Use to execute patches generated by {@link get}, transform documents with manually constructed
 * patches, or process patch operations from external sources.
 *
 * **Details**
 *
 * Executes patch operations sequentially, so later operations see changes made by earlier
 * operations. It never mutates the input document; array and object operations copy the affected
 * containers. An empty patch returns the original reference, and a root replace (`path: ""`)
 * returns the provided value directly.
 *
 * **Gotchas**
 *
