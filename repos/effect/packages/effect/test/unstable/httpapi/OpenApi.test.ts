...
const makeSecurityApi = (first: Context.Key<any, any>, second: Context.Key<any, any>) =>
...
describe("OpenApi", () => {
    ...
    const Api = HttpApi.make("Api").add(
      HttpApiGroup.make("test").add(
        HttpApiEndpoint.post("create", "/create", {
          payload: [
            Schema.Struct({ a: Schema.String }).pipe(
              HttpApiSchema.asJson({ contentType: profileA }),
            ),
            Schema.Struct({ b: Schema.String }).pipe(
              HttpApiSchema.asJson({ contentType: profileB }),
            ),
          ],
        }),
      ),
  ...
  it("emits buffered and stream successes with the same status", () => {
    ...
    assert.strictEqual(streamExtension.failureEvent, "effect/httpapi/stream/failure");
  ...
  it("rejects equivalent templated method and path pairs", () => {
    ...
    assert.throws(
      () => OpenApi.fromApi(Api),
      /Duplicate OpenAPI operation for GET \/users\/\{userId\}/,
    );
  });
