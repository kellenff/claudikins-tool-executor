...
describe("HttpApiClient", () => {
  describe("streaming responses", () => {
    ...
    it.effect("keeps StreamSse parser state isolated between responses", () =>
      Effect.gen(function* () {
        const bodies = ["event: first\ndata: one\n\n", "event: second\ndata: two\n\n"];
        let index = 0;
        const client = yield* HttpApiClient.makeWith(StreamingApi, {
          baseUrl: "http://test",
          httpClient: clientFromResponse(
            () => new Response(textStream([bodies[index++]!]), { status: 200 }),
          ),
        });
    ...
    it.effect("decodes StreamSse reserved failure events as full causes", () =>
      Effect.gen(function* () {
        ...
        const client = yield* HttpApiClient.makeWith(StreamingApi, {
          baseUrl: "http://test",
          httpClient: clientFromResponse(
            () => new Response(textStream([failureEvent]), { status: 200 }),
          ),
        });
    ...
    it.effect("returns StreamUint8Array response bytes incrementally", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          first.map((chunk) => Array.from(chunk)),
          [[1, 2]],
        );
      }),
    );
    ...
    it.effect("decodes StreamUint8Array successes at the annotated status", () =>
      Effect.gen(function* () {
        const client = yield* HttpApiClient.makeWith(AnnotatedStreamingApi, {
          baseUrl: "http://test",
          httpClient: clientFromResponse(
            () => new Response(byteStream([new Uint8Array([4, 5])]), { status: 206 }),
          ),
        });
    ...
    it.effect(
      "decodes non-success responses through endpoint error schemas before returning a stream",
      () =>
        Effect.gen(function* () {
          const client = yield* HttpApiClient.makeWith(StreamingApi, {
            baseUrl: "http://test",
    ...
    it.effect("preserves response-only raw response stream access", () =>
      Effect.gen(function* () {
        const client = yield* HttpApiClient.makeWith(StreamingApi, {
          ...
          httpClient: clientFromResponse(
            () =>
              new Response(byteStream([new Uint8Array([1]), new Uint8Array([2, 3])]), {
                status: 200,
              }),
          ),
        });
    ...
    it.effect(
      "selects a buffered response by content type when a stream uses the same status",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "selects a stream response by content type when buffered success uses the same status",
      () =>
        Effect.gen(function* () {
          const client = yield* HttpApiClient.makeWith(MixedSuccessApi, {
            baseUrl: "http://test",
  ...
  describe("urlBuilder", () => {
    ...
    it("omits missing optional path parameters", () => {
      ...
      strictEqual(builder.files.download({ params: {} }), "https://api.example.com/files");
      strictEqual(
    ...
    it("stores __proto__ identifiers as own properties", () => {
      const Api = HttpApi.make("Api").add(
        HttpApiGroup.make("__proto__").add(HttpApiEndpoint.get("__proto__", "/proto")),
      );
      const builder = HttpApiClient.urlBuilder(Api);
...
class EndpointError extends Schema.TaggedErrorClass<EndpointError>()(
  "EndpointError",
  {
...
const TextResponseError = Schema.String.pipe(HttpApiSchema.asText(), HttpApiSchema.status(400));
...
const clientFromResponse = (response: () => Response): HttpClient.HttpClient =>
  HttpClient.make(
    (request): Effect.Effect<HttpClientResponse.HttpClientResponse, never, never> =>
