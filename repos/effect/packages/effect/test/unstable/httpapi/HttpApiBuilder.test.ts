...
it.effect("reuses response schema transformations by source AST", () => {
  ...
  const GroupLive = HttpApiBuilder.group(Api, "test", (handlers) =>
    handlers
...
it.layer(TestServices)("HttpApiBuilder payload content types", (it) => {
  it.effect("round trips mixed-case media types with declared and received parameters", () =>
    Effect.gen(function* () {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("create", ({ payload }) => Effect.succeed(payload)),
      );
...
it.layer(TestServices)("HttpApiBuilder streaming success responses", (it) => {
  it.effect(
    "emits StreamUint8Array handler responses as streamed bytes with the declared content type",
    () =>
      Effect.gen(function* () {
        const Api = HttpApi.make("Api").add(
          HttpApiGroup.make("test").add(
  ...
  it.effect(
    "renders successful StreamSse events incrementally with the declared content type",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "renders StreamSse failures as one reserved event containing an encoded full cause",
    () =>
      Effect.gen(function* () {
        const Events = Schema.Struct({
        ...
        const GroupLive = HttpApiBuilder.group(Api, "test", (handlers) =>
          handlers.handle("events", () => Effect.succeed(Stream.fail({ reason: "boom" }))),
        );
    ...
    Effect.gen(function* () {
      ...
      const Api = HttpApi.make("Api").add(
        HttpApiGroup.make("test").add(
          HttpApiEndpoint.get("chat", "/test", {
            ...
            success: [Buffered, HttpApiSchema.StreamSse({ data: EventData, error: StreamError })],
          }),
        ),
      );
      ...
      const GroupLive = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("chat", ({ query }) =>
          Effect.succeed(
            query.stream === "true" ? Stream.make({ text: "hello" }) : { message: "done" },
          ),
        ),
      );

      const client = yield* HttpApiTest.groups(Api, ["test"]).pipe(Effect.provide(GroupLive));
      const bufferedResponse = yield* client.test.chat({
        query: { stream: "false" },
      ...
      const streamResponse = yield* client.test.chat({
        query: { stream: "true" },
        responseMode: "response-only",
      });
  ...
  it.effect("rejects unknown endpoint identifiers", () =>
    Effect.gen(function* () {
      ...
      for (const identifier of ["missing", "toString"] as const) {
        const GroupLive = HttpApiBuilder.group(Api, "users", (handlers) =>
          handlers.handle(identifier as "getUser", () => Effect.succeed("missing")),
        );
  ...
  it.effect("rejects duplicate handle and handleAll registrations", () =>
    Effect.gen(function* () {
      ...
      const HandleLive = HttpApiBuilder.group(Api, "users", (handlers) => {
        handlers.handle("getUser", () => Effect.succeed("first"));
        return handlers.handle("getUser", () => Effect.succeed("second"));
      });
      const HandleAllLive = HttpApiBuilder.group(Api, "users", (handlers) => {
  ...
  it.effect("ignores inherited handleAll properties", () =>
    Effect.gen(function* () {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "users", (handlers) =>
        handlers.handleAll(implementations),
      );
  ...
  it.effect("does not try another security scheme after the handler fails", () =>
    Effect.gen(function* () {
      class HandlerFailure extends Schema.TaggedErrorClass<HandlerFailure>()(
        "HandlerFailure",
        {
      ...
      class M extends HttpApiMiddleware.Service<M>()("Security/HandlerFailure", {
        error: Schema.String.pipe(HttpApiSchema.status(401), HttpApiSchema.asText()),
        security: {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("protected", () =>
          Effect.fail(new HandlerFailure({ message: "handler failed" })),
        ),
      );
