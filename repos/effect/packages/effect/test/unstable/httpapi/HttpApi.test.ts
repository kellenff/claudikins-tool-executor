...
describe("HttpApi", () => {
  ...
  it("stores __proto__ group and endpoint identifiers as own properties", () => {
    const group = HttpApiGroup.make("__proto__").add(HttpApiEndpoint.get("__proto__", "/proto"));
  ...
  it("does not mutate groups from the added API", () => {
    ...
    const child = HttpApi.make("Child").annotate(OpenApi.Description, "Child API").add(group);
    ...
    assert.strictEqual(
      Context.getUnsafe(parent.groups.users.annotations, OpenApi.Description),
      "Child API",
    );
  });
  ...
  it("keeps annotations from API variants isolated", () => {
    ...
    assert.strictEqual(
      Context.getUnsafe(publicParent.groups.users.annotations, OpenApi.Description),
      "Public API",
    );
    assert.strictEqual(
      Context.getUnsafe(internalParent.groups.users.annotations, OpenApi.Description),
      "Internal API",
    );
  ...
  it("preserves group annotation precedence", () => {
    ...
    const child = HttpApi.make("Child").annotate(OpenApi.Title, "API title").add(group);

    const parent = HttpApi.make("Parent").addHttpApi(child);

    assert.strictEqual(
      Context.getUnsafe(parent.groups.users.annotations, OpenApi.Title),
      "Group title",
    );
  });
});
