          ...
          Effect.suspend(() => {
            ...
            if (command === "SCRIPT") {
              ...
              if (scriptLoadAttempts === 1) {
                return Effect.fail(
                  new Redis.RedisError({ cause: new Error("ERR transient script load failure") }),
                );
              }
        ...
        send: (command, ...args) =>
          Effect.suspend(() => {
            ...
            if (command === "EVALSHA" && evalShaAttempts === 1) {
              return Effect.fail(
                new Redis.RedisError({ cause: new Error("NOSCRIPT No matching script") }),
              );
            }
            return Effect.succeed("ok");
          }),
