...
describe("Command help output", () => {
  ...
  it.effect("hides subcommands marked with withHidden from help output", () =>
    Effect.gen(function* () {
      const visible = Command.make("visible").pipe(Command.withDescription("A visible subcommand"));
      const secret = Command.make("experimental-foo").pipe(
        Command.withDescription("Should not appear"),
        Command.withHidden,
      );
      const root = Command.make("tool").pipe(Command.withSubcommands([visible, secret]));
  ...
  it.effect("hidden subcommand name does not leak through unknown-subcommand suggestions", () =>
    Effect.gen(function* () {
      ...
      yield* run(["experimental-fo"]).pipe(Effect.catchTag("ShowHelp", () => Effect.void));
