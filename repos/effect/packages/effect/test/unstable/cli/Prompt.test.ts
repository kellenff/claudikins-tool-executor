...
const TestLayer = Layer.mergeAll(FileSystemLayer, PathLayer, TerminalLayer);
...
describe("Prompt.file", () => {
  const FilePromptLayer = Layer.mergeAll(
    FileSystem.layerNoop({
      ...
      stat: (path) =>
        Effect.succeed(
          path.endsWith(".txt") ? ({ type: "File" } as any) : ({ type: "Directory" } as any),
        ),
    }),
    PathLayer,
    TerminalLayer,
...
describe("Prompt.custom", () => {
  it.effect("receive handles events from external dequeue", () =>
    Effect.gen(function* () {
      ...
      const prompt = Prompt.custom({ count: 0 }, Queue.asDequeue(eventQueue), {
  ...
  it.effect("falls back to process when no events are pending", () =>
    Effect.gen(function* () {
      ...
      const prompt = Prompt.custom({ keys: 0 }, Queue.asDequeue(eventQueue), {
  ...
  it.effect("Input variant exposes the input field with UserInput", () =>
    Effect.gen(function* () {
      ...
      const prompt = Prompt.custom({ captured: "" }, Queue.asDequeue(eventQueue), {
