  ...
  describe("run", () => {
    it.effect("should invoke the wizard programmatically from a command handler", () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* Command.runWith(command, { version: "1.0.0" })([]).pipe(
          Effect.forkChild,
        );
    ...
    it.effect("should start wizard mode at a selected subcommand", () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* Command.runWith(command, { version: "1.0.0" })([
          "child",
          "--wizard",
    ...
    it.effect("should reject --completions without a shell", () =>
      Effect.gen(function* () {
        ...
        yield* Command.runWith(command, { version: "1.0.0" })(["--completions"]).pipe(
          Effect.ignore,
        );
        ...
        assert.isTrue(
          stderr.some((line) =>
            String(line).includes(
              "Missing value for flag --completions. Expected: bash | zsh | fish | sh",
            ),
          ),
        );
      }).pipe(Effect.provide(TestLayer)),
    ...
    for (const { expected, flag, name } of missingValueCases) {
      it.effect(`should reject --${name} without a ${expected} value`, () =>
        Effect.gen(function* () {
          ...
          assert.isTrue(
            stderr.some((line) =>
              String(line).includes(`Missing value for flag --${name}. Expected: ${expected}`),
            ),
          );
        }).pipe(Effect.provide(TestLayer)),
      );
    ...
    it.effect("should render invalid choice values without doubling the Expected prefix", () =>
      Effect.gen(function* () {
        ...
        yield* Command.runWith(command, { version: "1.0.0" })(["--size", "bogus"]).pipe(
          Effect.ignore,
        );
    ...
    it.effect("should configure built-in global flags per run", () =>
      Effect.gen(function* () {
        ...
        assert.isTrue(
          errors.some((line) => String(line).includes("Unrecognized flag: --log-level")),
        );
    ...
    it.effect("should honor -- while still applying parent flags", () =>
      Effect.gen(function* () {
        ...
        const root = Command.make(
          ...
          (config) =>
            Effect.sync(() => captured.push({ global: config.global, rest: config.rest })),
        );
    ...
    it.effect("should keep variadic argument order when options are interleaved", () =>
      Effect.gen(function* () {
        ...
        const cmd = Command.make(
          ...
          (config) =>
            Effect.sync(() => captured.push({ files: config.files, verbose: config.verbose })),
        );
