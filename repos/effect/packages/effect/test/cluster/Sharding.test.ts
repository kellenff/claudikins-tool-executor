...
describe.concurrent("Sharding", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.provide(
          TestShardingWithoutStorage.pipe(
            Layer.provideMerge(
              Layer.effect(MessageStorage.MemoryDriver)(MessageStorage.MemoryDriver),
            ),
            Layer.updateService(MessageStorage.MessageStorage, (storage) => ({
              ...storage,
              unprocessedMessagesById(messageIds) {
  ...
  it.effect("defects when a durable request has no MessageStorage", () =>
    Effect.gen(function* () {
      ...
      Effect.provide(TestShardingWithoutStorage.pipe(Layer.provide(MessageStorage.layerNoop))),
    ),
  );
