...
describe("Cache", () => {
  ...
  describe("basic operations", () => {
    describe("get", () => {
      ...
      it.effect(
        "concurrent access - interrupting the first consumer does not interrupt the second",
        () =>
          Effect.gen(function* () {
            ...
            const first = yield* Cache.get(cache, "K1").pipe(
              Effect.forkChild({ startImmediately: true }),
            );
            yield* lookupStarted.await;
            const second = yield* Cache.get(cache, "K1").pipe(
              Effect.forkChild({ startImmediately: true }),
            );
      ...
      it.effect("concurrent access - interrupting the last consumer interrupts the lookup", () =>
        Effect.gen(function* () {
          ...
          const fiber = yield* Cache.get(cache, "K1").pipe(
            Effect.forkChild({ startImmediately: true }),
          );
          yield* lookupStarted.await;
