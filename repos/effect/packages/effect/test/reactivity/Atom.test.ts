...
describe.sequential("Atom", () => {
  ...
  it("effect initial", async () => {
    const count = Atom.make(Effect.succeed(1).pipe(Effect.delay(100)), { initialValue: 0 }).pipe(
      Atom.keepAlive,
    );
    const r = AtomRegistry.make();
  ...
  it("keeps parent child links when a parent is read more than once", () => {
    ...
    const unsubscribe = registry.subscribe(derived, () => {}, { immediate: true });
  ...
  it("disposed lifetime apis are no-ops", () => {
    ...
    const result = Atom.make<AsyncResult.AsyncResult<number, never>>(AsyncResult.success(1)).pipe(
      Atom.keepAlive,
    );
  ...
  it("stream initial", async () => {
    const count = Atom.make(Stream.range(1, 2).pipe(Stream.tap(() => Effect.sleep(50))), {
      initialValue: 0,
    });
    const r = AtomRegistry.make();
    const unmount = r.mount(count);
  ...
  test(`swr preserves writable set semantics`, () => {
    ...
    const source = Atom.writable<AsyncResult.AsyncResult<number, never>, number>(
      (get) =>
        Option.getOrElse(get.self<AsyncResult.AsyncResult<number, never>>(), () =>
