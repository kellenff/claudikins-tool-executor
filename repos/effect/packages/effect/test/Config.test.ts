      ...
      await assertFailure(
        Config.fail(
          new Schema.SchemaError(
            new SchemaIssue.Forbidden(Option.none(), { message: "failure message" }),
          ),
        ),
        ...
        s === ""
          ? Effect.fail(
              new Config.ConfigError(
                new Schema.SchemaError(
                  new SchemaIssue.InvalidValue(Option.some(s), { message: "empty" }),
                ),
              ),
            )
      ...
      it("does not recover from filter failures", async () => {
        const schema = Schema.String.check(
          Schema.makeFilter((s) =>
            s === "a"
              ? undefined
              : new SchemaIssue.InvalidValue(Option.none(), { message: `must be "a"` }),
          ),
        );
...
Expected "Infinity" | "-Infinity" | "NaN", got "abc"
  ...
  at ["a"][1]`,
    ...
    it("Array(Finite)", async () => {
      ...
      // ensure array
      await assertSuccess(config, ConfigProvider.fromEnv({ env: { a: "1,2,3" } }), {
        a: [1, 2, 3],
      });
      await assertSuccess(config, ConfigProvider.fromEnv({ env: { a_0: "1", a_1: "2" } }), {
        a: [1, 2],
