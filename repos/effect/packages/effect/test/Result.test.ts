...
describe("Result", () => {
  ...
  describe("Getters", () => {
    ...
    it("getOrThrowWith", () => {
      ...
      throws(() =>
        Result.getOrThrowWith(Result.fail("e"), (e) => new Error(`Unexpected Err: ${e}`)),
      );
    });
  ...
  describe("Sequencing", () => {
    it("flatMap", () => {
      ...
      assertFailure(
        Result.flatMap(Result.fail("maError"), flow(Str.length, Result.succeed)),
        "maError",
      );
    });
    ...
    it("all", () => {
      ...
      assertSuccess(Result.all({ a: Result.succeed(1), b: Result.succeed(true) }), {
        a: 1,
        b: true,
      });
