...
describe("Chunk", () => {
      ...
      it("should throw", () => {
        fc.assert(
          fc.property(fc.array(fc.anything()), fc.array(fc.anything()), (arr1, arr2) => {
            const chunk: Chunk.Chunk<unknown> = Chunk.appendAll(Chunk.fromIterable(arr2))(
  ...
  it("partition (identity)", () => {
    ...
    assertTuple(
      Chunk.partition(Chunk.make(Result.succeed(1), Result.fail("a"), Result.succeed(2)), identity),
  ...
  it("separate", () => {
    ...
    assertTuple(
      Chunk.separate(Chunk.make(Result.succeed(1), Result.fail("e"), Result.succeed(2))),
