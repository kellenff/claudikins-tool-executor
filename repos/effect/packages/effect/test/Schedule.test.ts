  ...
  describe("combining", () => {
    ...
    it.effect("max - stops when any schedule completes", () =>
      Effect.gen(function* () {
        ...
        const inputs = Array.makeBy(3, constUndefined);
        const outputs = yield* runDelays(schedule, inputs);
        assert.deepStrictEqual(outputs, [Duration.seconds(5), Duration.zero]);
      }),
    );
        ...
        const schedule = Schedule.spaced(Duration.millis(250)).pipe(
          Schedule.modifyDelay((metadata: Schedule.Metadata<number, string>) =>
            Effect.sync(() => {
              ...
              return Duration.sum(
                metadata.duration,
                Duration.millis(metadata.elapsedSincePrevious),
              );
            }),
          ),
        );
  ...
  describe("upTo", () => {
    it.effect("limits by times", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          [first, second, third],
          [["a", Duration.zero], ["b", Duration.zero], "c"],
        );
      }),
    );
    ...
    it.effect("limits by the first exhausted option", () =>
      Effect.gen(function* () {
        const schedule = Schedule.identity<string>().pipe(
          Schedule.upTo({ duration: "1 hour", times: 1 }),
        );
        const step = yield* Schedule.toStep(schedule);
