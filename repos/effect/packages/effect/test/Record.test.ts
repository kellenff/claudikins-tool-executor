...
describe("Record", () => {
  ...
  describe("string only APIs", () => {
    ...
    it("makeEquivalence", () => {
      deepStrictEqual(
        Record.makeEquivalence(Equivalence.strictEqual<number>())({ a: 1 }, { a: 1 }),
        true,
      );
      deepStrictEqual(
        Record.makeEquivalence(Equivalence.strictEqual<number>())({ a: 1 }, stringRecord),
        true,
      );
      deepStrictEqual(
        Record.makeEquivalence(Equivalence.strictEqual<number>())({ a: 1 }, { a: 2 }),
        false,
      );
      deepStrictEqual(
        Record.makeEquivalence(Equivalence.strictEqual<number>())({ a: 1 }, { b: 1 }),
        false,
      );
      const noPrototypeObject = Object.create(null);
      deepStrictEqual(
        Record.makeEquivalence(Equivalence.strictEqual<number>())(noPrototypeObject, { b: 1 }),
        false,
      );
    });
