  ...
  describe("constructors", () => {
    describe("make", () => {
      ...
      it.effect("lookup function context is preserved", () =>
        Effect.gen(function* () {
          class TestService extends Context.Service<TestService, { value: number }>()(
            "TestService",
          ) {}
      ...
      it.effect(
        ...
        () =>
          Effect.gen(function* () {
            ...
            const resultsFiber = yield* Effect.all(effects, { concurrency: "unbounded" }).pipe(
              Effect.forkChild,
            );
      ...
      it.effect("concurrent access - race between get and invalidate", () =>
        Effect.gen(function* () {
          ...
          const cache = yield* ScopedCache.make({
            capacity: 10,
            lookup: (_key: string) =>
              Effect.delay(Duration.millis(50))(Effect.sync(() => ++lookupCount)),
          });
    ...
    describe("invalidateWhen", () => {
      ...
      it.effect("resource cleanup when predicate matches", () =>
        Effect.gen(function* () {
          ...
          const result = yield* ScopedCache.invalidateWhen(cache, "test", (value) =>
  ...
  describe("service context", () => {
    describe("Service Dependency Injection", () => {
      it.effect("services are available in lookup functions", () =>
        Effect.gen(function* () {
          class ConfigService extends Context.Service<ConfigService, { multiplier: number }>()(
            "ConfigService",
          ) {}
      ...
      it.effect("requireServicesAt: 'lookup' provides services at lookup time", () =>
        Effect.gen(function* () {
          class CounterService extends Context.Service<CounterService, { value: number }>()(
            "CounterService",
          ) {}
      ...
      it.effect("concurrent refresh operations don't interfere", () =>
        Effect.gen(function* () {
          ...
          const cache = yield* ScopedCache.make({
            capacity: 10,
            lookup: (key: string) =>
              Effect.delay(Duration.millis(50))(Effect.sync(() => `${key}-${++counter}`)),
          });
