...
describe("SchemaParser", () => {
  ...
  describe("make", () => {
    it("should throw an error when the input is invalid", () => {
      ...
      throws(
        () => SchemaParser.make(schema)(null as any),
        (e) => {
    ...
    it("should throw an error when the cause contains both an Issue and a defect", () => {
      const schema = Schema.Struct({
        a: Schema.String.pipe(
          Schema.withConstructorDefault(Effect.failCause(makeMixedSchemaErrorCause())),
        ),
      });
  ...
  describe("makeOption", () => {
    it("should throw an error when the cause is not an Issue", () => {
      ...
      throws(
        () => SchemaParser.makeOption(schema)({}),
        (e) => {
          assertTrue(e instanceof Error);
  ...
  describe("decodeUnknownSync / encodeUnknownSync", () => {
    ...
    it("should throw an error when the cause contains both an Issue and a defect", () => {
      ...
      throws(
        () => SchemaParser.decodeUnknownSync(decodeSchema)("a"),
        (e) => {
          assertTrue(e instanceof Error);
      ...
      throws(
        () => SchemaParser.encodeUnknownSync(encodeSchema)("a"),
        (e) => {
          assertTrue(e instanceof Error);
          strictEqual(e.message, "Sync adapter can only throw schema issues");
          assertTrue(Cause.hasDies(e.cause as Cause.Cause<never>));
  ...
  describe("decodeUnknownPromise / encodeUnknownPromise", () => {
    it("should reject with an error when the input is invalid", async () => {
      const schema = Schema.String;
      const r1 = await SchemaParser.decodeUnknownPromise(schema)(null).then(
        Result.succeed,
        Result.fail,
      );
      ...
      const r2 = await SchemaParser.encodeUnknownPromise(schema)(null).then(
        Result.succeed,
        Result.fail,
      );
    ...
    it("should reject with an error when the cause contains both an Issue and a defect", async () => {
      ...
      const r1 = await SchemaParser.decodeUnknownPromise(decodeSchema)("a").then(
        Result.succeed,
        Result.fail,
      );
      assertTrue(Result.isFailure(r1));
      ...
      const r2 = await SchemaParser.encodeUnknownPromise(encodeSchema)("a").then(
        Result.succeed,
        Result.fail,
      );
      assertTrue(Result.isFailure(r2));
  ...
  describe("decodeUnknownOption / encodeUnknownOption", () => {
    ...
    it("should throw an error when the cause is not an Issue", () => {
      ...
      throws(
        () => SchemaParser.decodeUnknownOption(decodeSchema)("a"),
        (e) => {
          assertTrue(e instanceof Error);
          strictEqual(e.message, "Option adapter can only return none for schema issues");
          assertTrue(Cause.hasDies(e.cause as Cause.Cause<never>));
      ...
      throws(
        () => SchemaParser.encodeUnknownOption(encodeSchema)("a"),
        (e) => {
          assertTrue(e instanceof Error);
          strictEqual(e.message, "Option adapter can only return none for schema issues");
          assertTrue(Cause.hasDies(e.cause as Cause.Cause<never>));
  ...
  describe("decodeUnknownResult / encodeUnknownResult", () => {
    it("should throw an error when the cause is not an Issue", () => {
      ...
      throws(
        () => SchemaParser.decodeUnknownResult(decodeSchema)("a"),
        (e) => {
          assertTrue(e instanceof Error);
      ...
      throws(
        () => SchemaParser.encodeUnknownResult(encodeSchema)("a"),
        (e) => {
          assertTrue(e instanceof Error);
          strictEqual(e.message, "Result adapter can only return schema issues");
          assertTrue(Cause.hasDies(e.cause as Cause.Cause<never>));
