...
const InnerCategory = Schema.Struct({
  ...
  children: Schema.Array(
    Schema.suspend(
      (): Schema.Codec<Category> => InnerCategory.annotate({ identifier: "Category" }),
    ),
  ),
});
...
describe("toCodeDocument", () => {
  ...
  function assertMultiDocument(
    ...
    const codeDocument = SchemaRepresentation.toCodeDocument(input.multiDocument, {
    ...
    describe("checks", () => {
      ...
      it("isGUID with annotations", () => {
        assertSchema(
          ...
          {
            codes: makeCode(
              `Schema.String.check(Schema.isGUID({ "message": "message" }))`,
              "string",
            ),
          },
        );
  ...
  describe("Union", () => {
    ...
    it("union with oneOf mode", () => {
      ...
      assertSchema(
        {
          schema: Schema.Union([Schema.String, Schema.Number], { mode: "oneOf" }).annotate({
            description: "aa",
          }),
        },
        {
    ...
    it("union with multiple types", () => {
      ...
      assertSchema(
        {
          schema: Schema.Union([Schema.String, Schema.Number, Schema.Boolean]).annotate({
            description: "a",
          }),
        },
        {
        ...
        assertSchema(
          {
            schema: Schema.Date.check(
              Schema.isBetweenDate({ minimum: new Date(0), maximum: new Date(1) }),
            ),
          },
          {
            codes: makeCode(
