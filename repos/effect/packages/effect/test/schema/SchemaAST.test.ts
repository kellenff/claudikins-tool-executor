...
describe("SchemaAST", () => {
  ...
  describe("getIndexSignatureKeys", () => {
    it("String", () => {
      ...
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, Schema.String.ast, SchemaAST.defaultParseOptions),
        ["a", "b"],
      );
    });
    ...
    it("transformed String with decoded checks", () => {
      ...
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, schema.ast, SchemaAST.defaultParseOptions),
        ["a_b", "x_y"],
      );
    });
    ...
    it("Symbol", () => {
      ...
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, Schema.Symbol.ast, SchemaAST.defaultParseOptions),
        [a, b],
      );
    });
    ...
    it("Number", () => {
      ...
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, Schema.Number.ast, SchemaAST.defaultParseOptions),
        ["1", "1.5", "-2", "NaN"],
      );
    });
    ...
    it("Number with checks", () => {
      const input = { "1": 1, "1.5": 2, "-2": 3, a: 4, NaN: 5 };
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, Schema.Int.ast, SchemaAST.defaultParseOptions),
        ["1", "-2"],
      );
    });
  ...
  describe("record", () => {
    ...
    it("ignores Never arms in union parameters", () => {
      const ast = SchemaAST.record(
        Schema.Union([Schema.String, Schema.Never]).ast,
        Schema.Number.ast,
        undefined,
      );
  ...
  describe("IndexSignature", () => {
    it("accepts valid parameters on both type and encoded side", () => {
      doesNotThrow(
        () => new SchemaAST.IndexSignature(Schema.String.ast, Schema.Number.ast, undefined),
      );
      doesNotThrow(
        () =>
          new SchemaAST.IndexSignature(Schema.NumberFromString.ast, Schema.Number.ast, undefined),
      );
      doesNotThrow(
        () =>
