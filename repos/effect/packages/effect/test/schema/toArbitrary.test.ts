...
function verifyGeneration<S extends Schema.ConstraintCodec<unknown, unknown>>(
  schema: S,
  numRuns?: number,
) {
...
describe("Arbitrary generation", () => {
  ...
  it("should keep noNaN and noInfinity as separate number constraints", () => {
    ...
    const noInfinity = Schema.makeFilter(
      (n: number) =>
        n !== globalThis.Number.POSITIVE_INFINITY && n !== globalThis.Number.NEGATIVE_INFINITY,
      {
        arbitrary: {
          constraint: {
            noInfinity: true,
  ...
  describe("report and candidates", () => {
    it("should use filter candidates with the merged constraint context", () => {
      ...
      FastCheck.assert(
        FastCheck.property(result.value, (s) => s === "candidate"),
        { numRuns: 20 },
      );
    });
    ...
    it("should report opaque filters", () => {
      const schema = Schema.Struct({
        a: Schema.String.check(
          Schema.makeFilter((s: string) => s.length > 0, { expected: "a custom string" }),
        ),
      });
      const result = Schema.toArbitrary(schema, { report: true });
      ...
      FastCheck.assert(
        FastCheck.property(result.value, (a) => a.a.length > 0),
        { numRuns: 5 },
      );
    });
    ...
    it("should not report child filters when a filter group provides arbitrary metadata", () => {
      ...
      deepStrictEqual(result.report.warnings, []);
      FastCheck.assert(
        FastCheck.property(result.value, (s) => s === "a"),
        { numRuns: 20 },
      );
    });
    ...
    it("should not report warnings for constructive built-in filters", () => {
      const schema = Schema.Struct({
        ...
        object: Schema.Record(Schema.String, Schema.Number).check(
          Schema.isMinProperties(1),
          Schema.isMaxProperties(3),
        ),
  ...
  describe("object property counts", () => {
    it("enforces minProperties on optional-only structs", () => {
      ...
      FastCheck.assert(
        FastCheck.property(
          Schema.toArbitrary(schema),
          (o) => globalThis.Object.keys(o).length >= 2,
        ),
        { numRuns: 100 },
      );
      verifyGeneration(schema);
    ...
    it("enforces maxProperties on optional-only structs", () => {
      ...
      FastCheck.assert(
        FastCheck.property(
          Schema.toArbitrary(schema),
          (o) => globalThis.Object.keys(o).length <= 1,
        ),
        { numRuns: 100 },
      );
      verifyGeneration(schema);
    ...
    it("fails fast when optional keys cannot satisfy minProperties", () => {
      assertUnsupportedSchema(
        Schema.Struct({ r: Schema.String, a: Schema.optionalKey(Schema.String) }).check(
          Schema.isMinProperties(3),
        ),
        "Unable to derive an arbitrary for object property constraints",
      );
    });
    ...
    it("generates dense optional-key structs without excessive filtering", () => {
      ...
      FastCheck.assert(
        FastCheck.property(
          Schema.toArbitrary(schema),
          (o) => globalThis.Object.keys(o).length === 64,
        ),
        { numRuns: 100 },
      );
    });
  ...
  describe("suspend", () => {
    ...
    it("purely recursive tuple rest made non-empty without finite generation path", () => {
      const Rec = Schema.suspend((): Schema.Codec<unknown> => schema);
      const schema: any = Schema.TupleWithRest(Schema.Tuple([Schema.String]), [Rec]).check(
    ...
    it("non-empty recursive tuple rest with a finite union branch", () => {
      const Rec = Schema.suspend((): Schema.Codec<unknown> => schema);
      const schema: any = Schema.TupleWithRest(Schema.Tuple([Schema.String]), [
    ...
    it("should use filter candidates in recursive terminal paths", () => {
      ...
      const Rec = Schema.suspend((): Schema.Codec<unknown> => schema);
      const schema: any = Schema.Union([Leaf, Schema.Array(Rec).check(Schema.isMinLength(1))]);
    ...
    it("non-empty recursive collections with finite union branches", () => {
      ...
      const setSchema: any = Schema.ReadonlySet(Schema.Union([Schema.String, SetRec])).check(
        Schema.isMinSize(1),
      );
      ...
      const chunkSchema: any = Schema.Chunk(Schema.Union([Schema.String, ChunkRec])).check(
        minSizeOne(Chunk.size),
      );
      verifyGeneration(chunkSchema, 20);

      const MapRec = Schema.suspend((): Schema.Codec<unknown> => mapSchema);
      const mapSchema: any = Schema.ReadonlyMap(
        Schema.String,
        Schema.Union([Schema.Number, MapRec]),
      ).check(Schema.isMinSize(1));
      verifyGeneration(mapSchema, 20);

      const HashMapRec = Schema.suspend((): Schema.Codec<unknown> => hashMapSchema);
      const hashMapSchema: any = Schema.HashMap(
        Schema.String,
        Schema.Union([Schema.Number, HashMapRec]),
      ).check(minSizeOne(HashMap.size));
    ...
    it("custom generic declaration with an explicit terminal branch", () => {
      ...
      const schema: any = CustomArray(Rec, ([value]) => (fc, ctx) => {
        ...
        return {
          arbitrary:
            ctx.recursion === undefined ? arbitrary : fc.oneof(ctx.recursion, terminal, arbitrary),
          terminal,
        };
      });
  ...
  describe("recursion × constraint guards", () => {
    it("recursive array with a finite branch honors minLength", () => {
      const Rec = Schema.suspend((): Schema.Codec<unknown> => schema);
      const schema: any = Schema.Array(Schema.Union([Schema.String, Rec])).check(
        Schema.isMinLength(2),
      );
    ...
    it("recursive struct with required + optional self-ref honors minProperties", () => {
      ...
      assertInvariant(
        schema,
        (o) =>
          globalThis.Object.keys(o).length >= 1 &&
          globalThis.Object.prototype.hasOwnProperty.call(o, "a"),
      );
    });
    ...
    it("recursive ReadonlySet with a finite branch honors minSize", () => {
      const Rec = Schema.suspend((): Schema.Codec<unknown> => schema);
      const schema: any = Schema.ReadonlySet(Schema.Union([Schema.String, Rec])).check(
        Schema.isMinSize(1),
      );
    ...
    it("recursive ReadonlyMap with a finite branch honors minSize", () => {
      const Rec = Schema.suspend((): Schema.Codec<unknown> => schema);
      const schema: any = Schema.ReadonlyMap(
        Schema.String,
        Schema.Union([Schema.Number, Rec]),
      ).check(Schema.isMinSize(1));
    ...
    it("merges multiple minLength filters (takes the maximum)", () => {
      const schema = Schema.Array(Schema.String).check(
        Schema.isMinLength(2),
  ...
  describe("checks", () => {
    ...
    it("isMinLength(2) & isMaxLength(4)", () => {
      verifyGeneration(
        Schema.String.pipe(Schema.check(Schema.isMinLength(2), Schema.isMaxLength(4))),
      );
      verifyGeneration(
    ...
    it("isBetweenDate", () => {
      verifyGeneration(
        Schema.Date.check(Schema.isBetweenDate({ minimum: new Date(0), maximum: new Date(10) })),
      );
    });
    ...
    it("isBetweenBigInt", () => {
      verifyGeneration(
        Schema.BigInt.check(Schema.isBetweenBigInt({ minimum: BigInt(0), maximum: BigInt(10) })),
      );
    });
    ...
    it("non-natural BigDecimal order", () => {
      ...
      verifyGeneration(
        Schema.BigDecimal.check(Schema.makeIsGreaterThan({ order })(BigDecimal.make(0n, 0))),
      );
    });
  });
  ...
  it("DateTimeUtc with ordered DateTime constraints", () => {
    ...
    verifyGeneration(
      Schema.DateTimeUtc.check(Schema.makeIsGreaterThan({ order: DateTime.Order })(start)),
    );
  });
  ...
  it("DateTimeUtc with non-natural DateTime order", () => {
    ...
    verifyGeneration(
      Schema.DateTimeUtc.check(Schema.makeIsGreaterThan({ order })(DateTime.makeUnsafe(0))),
    );
  });
  ...
  it("DateTimeZoned with ordered DateTime constraints", () => {
    ...
    verifyGeneration(
      Schema.DateTimeZoned.check(Schema.makeIsGreaterThan({ order: DateTime.Order })(start)),
    );
  });
  ...
  describe("constraint behavior", () => {
    ...
    it("ordered lower bounds keep the strongest bound", () => {
      verifyGeneration(
        Schema.Number.check(Schema.isGreaterThan(1), Schema.isGreaterThanOrEqualTo(3)),
      );
    });

    it("ordered lower bounds preserve exclusivity for equal bounds", () => {
      verifyGeneration(
        Schema.Number.check(Schema.isGreaterThan(1), Schema.isGreaterThanOrEqualTo(1)),
      );
    });
    ...
    it("isValidDate & isGreaterThanOrEqualToDate", () => {
      verifyGeneration(
        Schema.Date.check(Schema.isDateValid(), Schema.isGreaterThanOrEqualToDate(new Date(0))),
      );
    });
    ...
    it("isGreaterThanOrEqualToBigDecimal", () => {
      verifyGeneration(
        Schema.BigDecimal.check(Schema.isGreaterThanOrEqualToBigDecimal(BigDecimal.make(0n, 0))),
      );
    });

    it("isGreaterThanBigDecimal", () => {
      verifyGeneration(
        Schema.BigDecimal.check(Schema.isGreaterThanBigDecimal(BigDecimal.make(0n, 0))),
      );
    });

    it("isLessThanOrEqualToBigDecimal", () => {
      verifyGeneration(
        Schema.BigDecimal.check(Schema.isLessThanOrEqualToBigDecimal(BigDecimal.make(10n, 0))),
      );
    });

    it("isLessThanBigDecimal", () => {
      verifyGeneration(
        Schema.BigDecimal.check(Schema.isLessThanBigDecimal(BigDecimal.make(10n, 0))),
      );
    });
