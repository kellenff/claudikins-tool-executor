...
/**
 * This suite intentionally avoids re-testing generic JSON Patch behavior (pointer parsing,
 * add/replace/remove mechanics, ordering, etc).
 *
 * It focuses on Schema.toDifferJsonPatch-specific guarantees: - reference preservation when patch
 * is a no-op - reference replacement for root replace - immutability of inputs - schema-specific
 * diff/patch encodings (Number / Date / Defect / DateTimeUtcFromMillis) - property-based roundtrip
 * across a broad set of codecs
 */
...
describe("Schema.toDifferJsonPatch", () => {
  ...
  describe("roundtrip (property-based)", () => {
    it("patch(diff(a,b)) round-trips for a wide set of codecs", () => {
      ...
      roundtrip(
        Schema.TupleWithRest(Schema.Tuple([Schema.Number]), [Schema.String, Schema.Boolean]),
      );
