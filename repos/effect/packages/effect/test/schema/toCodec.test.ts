  ...
  describe("toCodecJson", () => {
    ...
    describe("schemas without encoding", () => {
      ...
      it("Error", async () => {
        ...
        await encoding.succeed(new Error("a", { cause: new Error("b") }), {
          name: "Error",
          message: "a",
          cause: { name: "Error", message: "b" },
        });
        await encoding.succeed(new Error("a", { cause: "b" }), {
          name: "Error",
          message: "a",
          cause: "b",
        });
        ...
        await encoding.succeed(selfCause, {
          name: "Error",
          message: "a",
          cause: "[Circular]",
        });
        ...
        await encoding.succeed(new Error("a", { cause: cyclicCause }), {
          name: "Error",
          message: "a",
          cause: {},
        });
        ...
        await decoding.succeed({ message: "a", cause: null }, new Error("a", { cause: null }));
      });
      ...
      it("Error with stack", async () => {
        ...
        const decoding = asserts.decoding();
        await decoding.succeed({ message: "a", stack: "stack" }, error);
        await decoding.succeed(
      ...
      it.skipIf(isDeno)("FormData", async () => {
        ...
        await encoding.succeed(formData, [
          ...
          [
            ...
            {
              _tag: "File",
              value: { data: "aGk=", type: "text/plain", name: "note.txt", lastModified: 123 },
            },
          ],
        ]);
      ...
      describe("Redacted", () => {
        it("Redacted(Option(String))", async () => {
          ...
          await encoding.succeed(Redacted.make(Option.some("a")), { _tag: "Some", value: "a" });
  ...
  at ["label"]`,
    ...
    describe("schemas with encoding", () => {
      ...
      it("Defect", async () => {
        ...
        await encoding.succeed(new Cause.NoSuchElementError(), {
...
Expected "Infinity" | "-Infinity" | "NaN", got "a"`,
  ...
  describe("toCodecArrayFromSingle", () => {
    it("accepts string and array inputs for a top-level array", async () => {
      const serializer = Schema.toCodecArrayFromSingle(
        Schema.toCodecStringTree(Schema.Array(Schema.Finite)),
      );
      strictEqual(serializer.ast._tag, "Arrays");
      ...
      await decoding.fail(
        "1,2",
        `Expected a string representing a finite number, got "1,2"\n  at [0]`,
      );
      await decoding.succeed(["1", "2"], [1, 2]);
    });
    ...
    it("accepts string and array inputs for tuples with optional elements", async () => {
      const schema = Schema.toCodecArrayFromSingle(
        Schema.toCodecStringTree(Schema.Tuple([Schema.optionalKey(Schema.String)])),
      );
      const asserts = new TestSchema.Asserts(schema);
    ...
    it("applies recursively to nested arrays", async () => {
      const schema = Schema.toCodecArrayFromSingle(
        Schema.toCodecStringTree(Schema.Array(Schema.Array(Schema.Finite))),
      );
      const asserts = new TestSchema.Asserts(schema);
