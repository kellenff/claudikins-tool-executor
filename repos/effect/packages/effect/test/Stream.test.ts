...
describe("Stream", () => {
  ...
  describe("constructors", () => {
    ...
    it.effect(
      "fromReadableStream - errored streams fail with the mapped error, not a finalizer defect",
      () =>
        Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4).pipe(
          Stream.filterMapEffect((n) =>
            Effect.succeed(n % 2 === 0 ? Result.succeed(n + 1) : Result.failVoid),
          ),
          Stream.runCollect,
        );
      ...
      Effect.gen(function* () {
        ...
        const result = yield* Stream.paginate(s, ([chunk, nums]) =>
          nums.length === 0
            ...
            : Effect.succeed([
                chunk,
                Option.some([nums.slice(0, pageSize), nums.slice(pageSize)] as const),
              ] as const),
        ).pipe(Stream.runCollect);
  ...
  describe("error handling", () => {
    ...
    it.effect("catchIf with refinement orElse", () =>
      Effect.gen(function* () {
        ...
        const result = yield* (
          Stream.fail({ _tag: "ErrorB" as const }) as Stream.Stream<never, ErrorA | ErrorB>
        ).pipe(
          Stream.catchIf(
            (error): error is ErrorA => error._tag === "ErrorA",
    ...
    it.effect("catchIf with filter input", () =>
      Effect.gen(function* () {
        const matched = yield* Stream.catchFilter(
          Stream.fail("boom" as string | number),
          (e: string | number) =>
            typeof e === "string" ? Result.succeed(e.length) : Result.fail(`fallback: ${e}`),
      ...
      Effect.gen(function* () {
        const recovered = yield* Stream.fail("boom").pipe(
          Stream.catchCauseFilter(
            (cause) =>
              Cause.hasFails(cause) ? Result.succeed(Cause.squash(cause)) : Result.fail(cause),
        ...
        const stream = pipe(
          ...
          Stream.tap((n) =>
            pipe(
              ...
              Effect.andThen(
                pipe(Deferred.succeed(latch, void 0), Effect.when(Effect.sync(() => n === 999))),
              ),
            ),
          ),
          Stream.rechunk(999),
  ...
  describe("zipping", () => {
    ...
    it.effect("zipWith - prioritizes failures", () =>
      Effect.gen(function* () {
        const result = yield* Stream.zipWith(Stream.never, Stream.fail("Ouch"), (a, b) => [
          a,
          b,
        ]).pipe(Stream.runCollect, Effect.exit);
    ...
    it.effect("zipWith - handles empty streams", () =>
      Effect.gen(function* () {
        const result1 = yield* Stream.zipWith(Stream.empty, Stream.make(1, 2, 3), (a, b) => [
          a,
          b,
        ]).pipe(Stream.runCollect);

        const result2 = yield* Stream.zipWith(Stream.make(1, 2, 3), Stream.empty, (a, b) => [
          a,
          b,
        ]).pipe(Stream.runCollect);
    ...
    describe("zipFlatten", () => {
      ...
      it.effect("propagates errors from left stream", () =>
        Effect.gen(function* () {
          const stream1 = Stream.make([1, "a"] as const, [2, "b"] as const).pipe(
            Stream.concat(Stream.fail("boom")),
          );
          const stream2 = Stream.make("x", "y", "z");
    ...
    it.effect("retry - cleans up resources before restarting the stream", () =>
      Effect.gen(function* () {
        ...
        const stream = pipe(
          ...
          Effect.as(
            pipe(Stream.fromEffect(Ref.get(ref)), Stream.concat(Stream.fail(Option.none()))),
          ),
          Stream.unwrap,
        );
        const result = yield* pipe(
    ...
    it.effect("fans out to a fixed number of streams", () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const result = yield* Effect.all([Stream.runCollect(left), Stream.runCollect(right)], {
            concurrency: "unbounded",
          });
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const result = yield* Effect.all(
            [Stream.runCollect(left).pipe(Effect.exit), Stream.runCollect(right).pipe(Effect.exit)],
            { concurrency: "unbounded" },
          );
