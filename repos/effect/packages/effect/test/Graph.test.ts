...
describe("Graph", () => {
  ...
  describe("equality and hashing", () => {
    ...
    it("keeps parallel edges paired by edge index", () => {
      const left = makeGraph("undirected", [
        [0, 1, "first"],
        [0, 1, "second"],
      ]);
      const reversed = makeGraph("undirected", [
        [1, 0, "first"],
        [1, 0, "second"],
      ]);
      const reordered = makeGraph("undirected", [
        [1, 0, "second"],
        [1, 0, "first"],
      ]);
  ...
  describe("set operations", () => {
    ...
    it("intersection keeps shared nodes and shared edges", () => {
      const graph = Graph.intersection(makeLeft(), makeRight(), {
    ...
    it("symmetricDifference keeps edges present in exactly one graph", () => {
      const graph = Graph.symmetricDifference(makeLeft(), makeRight(), {
        nodeIdentity: (node) => node.id,
      });
    ...
    it("uses edge data from that for equivalent edges", () => {
      const left = Graph.directed<string, { readonly id: string; readonly label: string }>(
        (mutable) => {
          const a = Graph.addNode(mutable, "A");
          const b = Graph.addNode(mutable, "B");
      ...
      const right = Graph.directed<string, { readonly id: string; readonly label: string }>(
        (mutable) => {
          const a = Graph.addNode(mutable, "A");
          const b = Graph.addNode(mutable, "B");
      ...
      strictEqual(
        Array.from(Graph.edges(Graph.compose(left, right, options)))[0][1].data.label,
        "right",
      );
      strictEqual(
        Array.from(Graph.edges(Graph.intersection(left, right, options)))[0][1].data.label,
        "right",
      );
    });
  ...
  describe("removeEdge", () => {
    ...
    it("should handle multiple edges between same nodes", () => {
      const result = Graph.directed<string, number>((mutable) => {
        ...
        assertSome(
          Graph.getEdge(mutable, edge2),
          new Graph.Edge({ source: nodeA, target: nodeB, data: 20 }),
        );
      });
  ...
  describe("getEdge", () => {
    ...
    describe("successors and predecessors", () => {
      ...
      it("should throw for undirected graphs", () => {
        ...
        expect(() => Graph.successors(graph as any, 0)).toThrow(
          "Cannot get successors of undirected graph",
        );
        expect(() => Graph.predecessors(graph as any, 0)).toThrow(
          "Cannot get predecessors of undirected graph",
        );
      });
    });
  ...
  describe("toGraphViz", () => {
    ...
    it("should quote graph names", () => {
      ...
      strictEqual(
        Graph.toGraphViz(graph, { graphName: 'My "Graph"' }),
        'digraph "My \\"Graph\\"" {\n}',
      );
    });
  ...
  describe("Iterator Base Methods", () => {
    ...
    it("should only include start nodes when radius is zero", () => {
      ...
      assert.deepStrictEqual(
        Array.from(Graph.indices(Graph.dfs(graph, { start: [0], radius: 0 }))),
        [0],
      );
      assert.deepStrictEqual(
        Array.from(Graph.indices(Graph.bfs(graph, { start: [0], radius: 0 }))),
        [0],
      );
    });
    ...
    it("should reject cycles introduced before topological iteration", () => {
      const mutable = Graph.beginMutation(
        Graph.directed<string, number>((graph) => {
    ...
    it("should reject initials with incoming edges", () => {
      ...
      expect(() => Array.from(Graph.topo(graph, { initials: [1] }))).toThrow(
    ...
    it("should throw for undirected graphs", () => {
      ...
      expect(() => Graph.topo(graph as any)).toThrow(
    ...
    it("should traverse undirected edges in reverse storage direction", () => {
      ...
      expect(
        Array.from(Graph.indices(Graph.dfs(graph, { start: [0], direction: "incoming" }))),
      ).toEqual([0, 1, 2]);
      expect(Array.from(Graph.indices(Graph.bfs(graph, { start: [0] })))).toEqual([0, 1, 2]);
      expect(Array.from(Graph.indices(Graph.dfsPostOrder(graph, { start: [0] })))).toEqual([
        2, 1, 0,
      ]);
    });
  ...
  describe("Graph Element Iterators", () => {
    ...
    describe("externals", () => {
      ...
      it("should work with manual iterator control", () => {
        ...
        const iterator = Graph.indices(Graph.externals(graph, { direction: "outgoing" }))[
          Symbol.iterator
        ]();
  ...
  describe("GraphIterable abstraction", () => {
    ...
    it("should preserve the receiver for iterable iterator methods", () => {
      ...
      assert.deepStrictEqual(Array.from(walker), [
        [0, "A"],
        [1, "B"],
      ]);
    });
    ...
    it("should create fresh iterators for generator-backed walkers", () => {
      ...
      assertRepeated(walker, [
        [0, "A"],
        [1, "B"],
      ]);
      assert.deepStrictEqual(Array.from(walker.visit((index, data) => `${index}:${data}`)), [
        "0:A",
        "1:B",
      ]);
      assert.deepStrictEqual(Array.from(walker.visit((index, data) => `${index}:${data}`)), [
      ...
      assert.deepStrictEqual(Array.from(Graph.values(walker)), ["A", "B"]);
      assert.deepStrictEqual(Array.from(Graph.entries(walker)), [
        [0, "A"],
        [1, "B"],
      ]);
      assert.deepStrictEqual(Array.from(Graph.entries(walker)), [
