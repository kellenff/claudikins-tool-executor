...
describe("Iterable", () => {
  ...
  it("filter", () => {
    ...
    deepStrictEqual(
      toArray(Iter.filter([Option.some(3), Option.some(2), Option.some(1)], Option.isSome)),
      [Option.some(3), Option.some(2), Option.some(1)],
    );
    deepStrictEqual(
      toArray(Iter.filter([Option.some(3), Option.none(), Option.some(1)], Option.isSome)),
  ...
  it("getFailures", () => {
    ...
    deepStrictEqual(
      toArray(Iter.getFailures([Result.succeed(1), Result.fail("err"), Result.fail("nope")])),
      ["err", "nope"],
    );
  });
  ...
  it("getSuccesses", () => {
    ...
    deepStrictEqual(
      toArray(Iter.getSuccesses([Result.fail("err"), Result.succeed(1), Result.succeed(2)])),
      [1, 2],
    );
  });
