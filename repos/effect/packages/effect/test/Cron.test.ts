...
describe("Cron", () => {
  ...
  it("parse", () => {
    ...
    deepStrictEqual(
      invalidMinuteStep.failure.message,
      "Expected step value to be less than or equal to 59",
    );
  ...
  it("make treats weekday 7 as Sunday when stepping", () => {
    ...
    deepStrictEqual(
      Cron.next(cron, new Date("2024-01-05T00:00:00.000Z")),
      new Date("2024-01-07T00:00:00.000Z"),
    );
    deepStrictEqual(
      Cron.prev(cron, new Date("2024-01-08T00:00:00.000Z")),
      new Date("2024-01-07T00:00:00.000Z"),
    );
  });
  ...
  it("next adheres to day-of-month and weekday semantics", () => {
    ...
    deepStrictEqual(
      next(Cron.parseUnsafe("0 0 */2 * MON", utc), after),
      new Date("2024-01-15T00:00:00.000Z"),
    );
    deepStrictEqual(
      next(Cron.parseUnsafe("0 0 1-31/2 * MON", utc), after),
      new Date("2024-01-03T00:00:00.000Z"),
    );
    deepStrictEqual(
      next(Cron.parseUnsafe("0 0 *,5 * MON", utc), after),
      new Date("2024-01-08T00:00:00.000Z"),
    );
    deepStrictEqual(
      next(Cron.parseUnsafe("0 0 5,* * MON", utc), after),
  ...
  it("next respects day-of-month or weekday constraints when entering a restricted month", () => {
    ...
    deepStrictEqual(
      next(Cron.parseUnsafe("0 0 29 2 MON", tz), after),
      new Date("2024-02-05T00:00:00.000Z"),
    );
    deepStrictEqual(
      next(Cron.parseUnsafe("0 0 30 2 MON", tz), after),
      new Date("2024-02-05T00:00:00.000Z"),
    );
    deepStrictEqual(
      next(Cron.parseUnsafe("0 0 31 2 MON", tz), after),
      new Date("2024-02-05T00:00:00.000Z"),
    );
  });
  ...
  it("handles transition into daylight savings time", () => {
    ...
    const next = (): DateTime.Zoned =>
      DateTime.makeZonedUnsafe(sequence.next().value, { timeZone: "Europe/Berlin" });
