...
const assertUnknownError = <A>(
  exit: Exit.Exit<A, Cause.UnknownError>,
  cause: unknown,
  message: string,
) => {
...
describe("Effect", () => {
  ...
  describe("fromOption", () => {
    ...
    it("from a none", () =>
      Option.none().pipe(
        ...
        Effect.flip,
        Effect.tap((error) =>
          Effect.sync(() => assert.ok(error instanceof Cause.NoSuchElementError)),
        ),
        Effect.runPromise,
      ));
  ...
  describe("transposeOption", () => {
    ...
    it.effect("transposes a some containing a failure", () =>
      Effect.gen(function* () {
        const error = yield* Effect.transposeOption(Option.some(Effect.fail("error"))).pipe(
          Effect.flip,
        );
  ...
  describe("tryPromise", () => {
    ...
    it.effect("maps promise rejections to UnknownError in direct-thunk form", () =>
      Effect.gen(function* () {
        ...
        const exit = yield* Effect.tryPromise<number>(() => Promise.reject(rejected)).pipe(
          Effect.exit,
        );
  ...
  describe("forEach", () => {
    ...
    it("bounded", () =>
      Effect.gen(function* () {
        const results = yield* Effect.forEach([1, 2, 3, 4, 5], (_) => Effect.succeed(_), {
          concurrency: 2,
        });
    ...
    it.effect("returns zero for an empty iterable", () =>
      Effect.gen(function* () {
        const result = yield* Effect.reduce(
          [],
          () => 42,
          (acc, value: number) => Effect.succeed(acc + value),
        );
  ...
  describe("filter", () => {
    ...
    it.live("iterable", () =>
      Effect.gen(function* () {
        const results = yield* Effect.filter(new Set([1, 2, 3, 4, 5]), (value) =>
          Effect.succeed(value % 2 === 1),
        );
        assert.deepStrictEqual(results, [1, 3, 5]);
      }),
    ...
    it.effect("filter match receives pass value and exit", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.onExitFilter(
          Effect.succeed(42),
          (exit) =>
            Exit.isSuccess(exit) ? Result.succeed(`value:${exit.value}`) : Result.fail(exit),
          (value, exit) =>
            Effect.sync(() =>
              finalized.push(
  ...
  describe("catch", () => {
    ...
    it.effect("first argument as failure", () =>
      Effect.gen(function* () {
        const result = yield* Effect.flip(
          Effect.catch(Effect.fail("e1" as const), () => Effect.fail("e2" as const)),
        );
        assert.deepStrictEqual(result, "e2");
      }),
    );
