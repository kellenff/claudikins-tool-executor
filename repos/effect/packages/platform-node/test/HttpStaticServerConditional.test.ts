...
const makeHandler = async () => {
  const fileSystem = FileSystem.makeNoop({
    stat: (path) =>
      path === filePath ? Effect.succeed(fileInfo) : Effect.fail(notFoundError(path)),
  });
...
describe("HttpStaticServer", () => {
  it("304 with If-None-Match exact, weak, and wildcard", async () => {
    ...
    const exact = await handler(
      new Request("http://localhost/file.txt", { headers: { "If-None-Match": etag } }),
    );
    const weak = await handler(
      new Request("http://localhost/file.txt", { headers: { "If-None-Match": `W/${etag}` } }),
    );
    ...
    const any = await handler(
      new Request("http://localhost/file.txt", { headers: { "If-None-Match": "*" } }),
    );
