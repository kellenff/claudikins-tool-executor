    ...
    it.live(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.retry(
            Effect.flatMap(Ref.get(received), (xs) =>
              xs.length >= 2 ? Effect.void : Effect.fail("not enough ticks yet"),
            ),
          ...
          assert.isTrue(
            ghostExit._tag === "Failure",
            "Ghost call should fail with the routing miss",
          );
          ...
          assert.isUndefined(
            tickerStatus,
            "Ticker stream must still be running after the unknown-tag failure",
          );
          assert.isAbove(
            ticksAfterGhost,
            ticksBeforeGhost,
