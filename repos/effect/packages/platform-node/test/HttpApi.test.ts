...
describe("HttpApi", () => {
    ...
    it.effect("security middleware can be implemented with layerClient", () => {
      ...
      const MLive = Layer.succeed(M)({
        apiKey: (effect, options) =>
          Effect.provideService(effect, CurrentToken, Redacted.value(options.credential)),
      });
    ...
    describe("GET", () => {
      it.effect("query parameters", () => {
        ...
        return Effect.gen(function* () {
          ...
          yield* assertServerJson(yield* HttpClient.get("/a?required=1&optional=1"), 200, {
            required: 1,
            optional: 1,
          });
          yield* assertServerJson(yield* HttpClient.get("/a?required=1"), 200, { required: 1 });
          // TODO: fix this test
          yield* assertServerJson(yield* HttpClient.get("/a?required=1&optional="), 200, {
            required: 1,
            optional: 0,
          });
    ...
    it.effect("array of schemas with different encodings", () => {
      ...
      return Effect.gen(function* () {
        ...
        yield* assertServerJson(
          yield* HttpClient.post("/a", { body: HttpBody.jsonUnsafe({ a: "text" }) }),
          200,
          {
            a: "text",
          },
        ...
        yield* assertClientJson(client.group.a({ payload: new Uint8Array([1, 2, 3]) }), {
          0: 1,
          1: 2,
          2: 3,
        });
  ...
  describe("success option", () => {
    ...
    it("no content via toWebHandler", async () => {
      ...
      const { handler, dispose } = HttpRouter.toWebHandler(Layer.mergeAll(ApiLive), {
        disableLogger: true,
      });
      try {
