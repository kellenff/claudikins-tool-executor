 ...
 *
 * Server requests and Node client responses both arrive as Node readable streams with raw header
 * objects, socket metadata, and one-shot body consumption. This module's `NodeHttpIncomingMessage`
 * class keeps the original Node message available while presenting Effect's `HttpIncomingMessage`
 * shape: typed headers, remote address lookup, stream access, and text, JSON, URL-encoded, and
 * array-buffer body readers.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to implement Node HTTP request or response adapters that expose the Effect HTTP
 * incoming-message interface.
 *
 * **Details**
 *
 * The adapter exposes headers, remote address, stream access, and cached body decoders. Subclasses
 * provide the error mapping for unknown Node errors.
 *
 * @since 4.0.0
