 ...
 *
 * `NodeMultipart` adapts a Node `Readable` plus incoming HTTP headers into Effect's shared
 * multipart model. It can expose form parts as a stream or collect a complete persisted form by
 * writing file uploads to scoped temporary files through the current `FileSystem` and `Path`
 * services. `fileToReadable` returns the underlying Node readable stream for file parts produced by
 * this parser.
 *
 * @since 4.0.0
 */
...
/**
 * Parses multipart data from a Node readable request body and headers into a stream of
 * `Multipart.Part` values, converting parser failures to `MultipartError`.
 *
 * @since 4.0.0
 ...
 * `FileSystem`, `Path`, and `Scope` services.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
> =>
  Multipart.toPersisted(stream(source, headers), (path, file) =>
    Effect.tryPromise({
      try: (signal) =>
        NodeStreamP.pipeline((file as FileImpl).file, NFS.createWriteStream(path), { signal }),
      catch: (cause) => Multipart.MultipartError.fromReason("InternalError", cause),
    }),
  );

/**
 * Returns the underlying Node readable stream for a multipart file produced by the Node multipart
 * parser.
 *
 * @since 4.0.0
 * @category Converting
