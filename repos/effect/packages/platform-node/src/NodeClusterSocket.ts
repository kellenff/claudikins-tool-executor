 ...
 *
 * The main `layer` builds a sharding layer for socket transport, choosing serialization, runner
 * health checks, runner storage, message storage, and optional client-only mode from the supplied
 * options. This module also re-exports the shared socket client and server protocol layers, and
 * provides Kubernetes-aware Undici dispatcher and HTTP client layers for runner health checks.
 *
 * @since 4.0.0
 */
import {
...
export {
  ...
  layerClientProtocol,
  /**
   * Provides the socket server used by Node cluster runners through the shared socket server
   * implementation.
   *
   * @since 4.0.0
   * @category Re-exports
...
/**
 * Builds the Node cluster socket sharding layer, configuring RPC serialization, message storage,
 * runner health checks, and optional client-only mode.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Provides an Undici dispatcher for Kubernetes API calls, using the service account CA certificate
 * when it is available and falling back to the default dispatcher otherwise.
 *
 * @since 4.0.0
 * @category Layers
 */
export const layerDispatcherK8s: Layer.Layer<NodeHttpClient.Dispatcher> = Layer.effect(
