 ...
 *
 * This module supplies Node runtime backends for the platform-independent Effect HTTP client API.
 * It re-exports the fetch-based `Fetch`, `RequestInit`, and `layerFetch` APIs, defines an
 * Undici-backed client with dispatcher services and request options, and defines a lower-level
 * `node:http` / `node:https` client with scoped HTTP agent layers.
 *
 * @since 4.0.0
 */
 ...
 * @category Dispatcher
 */
export const makeDispatcher: Effect.Effect<Undici.Dispatcher, never, Scope.Scope> =
...
/**
 * Provides the `Dispatcher` service from Undici's process-global dispatcher, without creating or
 * owning a new agent.
 *
 * @since 4.0.0
 * @category Dispatcher
 */
export const dispatcherLayerGlobal: Layer.Layer<Dispatcher> = Layer.sync(Dispatcher)(() =>
  Undici.getGlobalDispatcher(),
);

/**
 * Fiber reference containing default Undici request options applied to requests sent by
 * `makeUndici`.
 *
 * @since 4.0.0
 * @category Undici
...
/**
 * Creates an `HttpClient` that sends requests through the current Undici `Dispatcher`, converts
 * Effect HTTP bodies to Undici bodies, and maps transport and decode failures to
 * `HttpClientError`.
 *
 * @since 4.0.0
 * @category Undici
...
/**
 * Service tag for the paired Node `http` and `https` agents used by the node:http-backed HTTP
 * client.
 *
 * @since 4.0.0
 * @category HttpAgent
...
/**
 * Acquires Node `http` and `https` agents with the supplied options and destroys both agents when
 * the enclosing scope is finalized.
 *
 * @since 4.0.0
 * @category HttpAgent
 */
...
/**
 * Provides the `HttpAgent` service using scoped Node `http` and `https` agents configured with the
 * supplied options.
 *
 * @since 4.0.0
 * @category HttpAgent
...
/**
 * Creates an `HttpClient` backed by Node `http` and `https`, using the current `HttpAgent`,
 * streaming request bodies, and wrapping Node responses as `HttpClientResponse` values.
 *
 * @since 4.0.0
 * @category Node:http
 */
...
/**
 * Provides a node:http-backed `HttpClient` together with default scoped Node `http` and `https`
 * agents.
 *
 * @since 4.0.0
 * @category Node:http
