 ...
 *
 * This module re-exports the shared Node socket support for TCP connections, Unix domain socket
 * connections, and Node `Duplex` streams. It also provides WebSocket constructor layers: one that
 * uses `globalThis.WebSocket` when present and falls back to `ws`, one that always uses `ws`, and
 * one that creates a `Socket.Socket` layer for a WebSocket URL.
 *
 * @since 4.0.0
 */
...
/**
 * Provides a `Socket.WebSocketConstructor`, using `globalThis.WebSocket` when available and falling
 * back to the `ws` package otherwise.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Creates a `Socket.Socket` layer for a WebSocket URL using the Node WebSocket constructor layer,
 * honoring protocol, open-timeout, and close-code error options.
 *
 * @since 4.0.0
 * @category Layers
 */
