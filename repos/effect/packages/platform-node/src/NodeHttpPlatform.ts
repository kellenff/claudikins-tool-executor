 ...
 *
 * This module connects the portable `HttpPlatform` file response helpers to Node runtime
 * primitives. It serves local files through Node readable streams, supports byte ranges, converts
 * Web `File` values to readable streams, and fills in content type and content length headers when
 * needed.
 *
 * @since 4.0.0
 */
...
/**
 * Creates the Node `HttpPlatform`, serving file responses from Node readable streams and adding
 * MIME type and content-length headers when needed.
 *
 * @since 4.0.0
    ...
    return ServerResponse.raw(Readable.fromWeb(file.stream() as any), {
      headers: Headers.merge(
        ...
        Headers.fromRecordUnsafe({
          "content-type":
            headers["content-type"] ?? Mime.getType(file.name) ?? "application/octet-stream",
...
/**
 * Provides the Node `HttpPlatform` together with the filesystem and ETag services it needs for file
 * responses.
 *
 * @since 4.0.0
 * @category Layers
