 ...
 *
 * This module adapts a supplied Node `http.Server` into Effect's platform-independent HTTP server
 * service. It starts the server with Node `listen` options, converts `request` events into
 * `HttpServerRequest` values, writes `HttpServerResponse` bodies through Node's `ServerResponse`,
 * and handles `upgrade` events by exposing the upgraded socket through `HttpServerRequest.upgrade`.
 * It also exports request and upgrade handler constructors plus layers for the server alone, HTTP
 * support services, the combined server, configurable options, and tests.
 *
 * @since 4.0.0
 */
...
/**
 * Creates a scoped `HttpServer` from a Node `http.Server`, starts listening with the supplied
 * options, registers request and upgrade handling, and closes the server during scope finalization
 * with optional graceful-shutdown control.
 *
 * @since 4.0.0
...
/**
 * Creates a Node `request` event handler for an Effect HTTP application, injecting a
 * `HttpServerRequest` and interrupting the request fiber if the client closes the response before
 * it finishes.
 *
 * @since 4.0.0
 * @category Handlers
...
/**
 * Creates a Node `upgrade` event handler for an Effect HTTP application, exposing the upgraded
 * WebSocket as the request's `upgrade` effect and interrupting the request fiber when the socket
 * closes early.
 *
 * @since 4.0.0
 * @category Handlers
...
/**
 * Provides an `HttpServer` by creating and managing a scoped Node `http.Server` with the supplied
 * listen and shutdown options.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Provides the Node HTTP support services used by `NodeHttpServer`, including the HTTP platform,
 * ETag generator, and core Node platform services.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Provides a Node `HttpServer` together with the Node HTTP platform, ETag, and core platform
 * services required to serve requests.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Provides a Node `HttpServer` together with the Node HTTP platform, ETag, and core Node platform
 * services, reading the listen and shutdown options from a `Config` value.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Provides a test HTTP server listening on an ephemeral port together with a Fetch-backed
 * `HttpClient` configured for server integration tests.
 *
 * @since 4.0.0
 * @category Testing
 */
  ...
  <E>(originalCause: Cause.Cause<E>) =>
    Effect.flatMap(causeResponse(originalCause), ([response, cause]) => {
      ...
      return Effect.failCause(
        headersSent ? Cause.combine(originalCause, Cause.die(originalResponse)) : cause,
      );
    });
