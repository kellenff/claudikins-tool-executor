 ...
 *
 * This module defines a `Permissions` service backed by `navigator.permissions`. The service
 * exposes `query`, which returns the browser `PermissionStatus` for a permission name. Failed
 * browser operations are represented as `PermissionsError` values with `InvalidStateError` or
...
/**
 * Wrapper on the Permission API (`navigator.permissions`) with methods for querying status of
 * permissions.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * `query` delegates to `navigator.permissions.query({ name })` and wraps rejected browser
 * operations in `PermissionsError`.
 *
 * @since 4.0.0
 * @category Layers
 */
