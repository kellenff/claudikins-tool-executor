 ...
 *
 * An {@link IndexedDbTable} is the schema-backed description of one IndexedDB object store. It
 * carries the store name, row schema, key path, index key paths, auto-increment mode, and
 * transaction durability used by database versions, migrations, and typed queries. The `make`
 * constructor also derives the read, array, and auto-increment write schemas used by the query
 * builder.
 *
 * @since 4.0.0
...
/**
 * Typed IndexedDB table definition containing its name, schema, key path, indexes, auto-increment
 * setting, and transaction durability.
 *
 * @since 4.0.0
 * @category Interface
 */
...
/**
 * Creates a typed IndexedDB table definition from its name, schema, optional key path, indexes,
 * auto-increment flag, and durability.
 *
 * **When to use**
 *
 * Use to define a typed object-store descriptor for inclusion in an `IndexedDbVersion` and for
 * migration or query APIs.
 *
 * **Details**
 *
 * `autoIncrement` defaults to `false` and `durability` defaults to `"relaxed"`. Tables without a
 * key path get a read schema that includes an out-of-line `key`, while auto-increment tables use a
 * write schema where the generated key may be omitted.
 *
 * **Gotchas**
 *
 * Tables without a key path cannot define a `key` field in their row schema. Key paths and index
 * paths must point to encoded fields whose values are valid IndexedDB keys, and declared indexes
 * still need to be created during database migrations.
 *
 * @since 4.0.0
...
> => {
  ...
  (Table as any).autoincrementSchema = options.autoIncrement
    ? Schema.Struct(
        Struct.omit((options.schema as Schema.Struct<{}>).fields, [options.keyPath!] as any),
      )
    : options.schema;
