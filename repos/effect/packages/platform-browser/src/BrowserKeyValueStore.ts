 ...
 *
 * This module provides browser implementations of the unstable persistence `KeyValueStore` service.
 * Use {@link layerLocalStorage} for small origin-scoped values that should survive reloads and
 * browser restarts, use {@link layerSessionStorage} for tab / page-session state, and use
 * {@link layerIndexedDb} when the store should be asynchronous and backed by IndexedDB. The
 * IndexedDB layer requires the browser `IndexedDb` service and accepts an optional database name.
 *
 * @since 4.0.0
 */
...
/**
 * Creates a `KeyValueStore` layer that uses the browser's `localStorage` API and stores values
...
/**
 * Creates a `KeyValueStore` layer that uses the browser's `sessionStorage` API and stores values
 ...
 * **When to use**
 *
 * Use when you need persistent asynchronous IndexedDB storage for a browser `KeyValueStore` instead
 * of the synchronous Web Storage APIs.
 *
 * **Details**
 *
 * The database name defaults to `"effect_key_value_store"`. The layer requires the `IndexedDb`
 * service and stores string and `Uint8Array` values in the same backing object store.
 *
 * **Gotchas**
 *
 * IndexedDB may be unavailable or blocked by browser settings, private browsing, quota limits, or
 * restricted contexts. The string and `Uint8Array` accessors do not coerce values stored with the
    ...
    Effect.gen(function* () {
      ...
      return KeyValueStore.make({
        clear: Effect.suspend(() => {
          ...
          return idbRequest({ method: "clear", message: "Failed to clear backing store" }, () =>
            store.clear(),
          );
        }),
        get: (key: string) =>
