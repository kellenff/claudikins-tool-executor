 ...
 *
 * `make` builds a `WorkerRunnerPlatform` over a `MessagePort` or `Window`. `layer` provides the
 * platform from the global worker `self`, and `layerMessagePort` provides it from an explicit
 * endpoint. The platform receives parent or client messages, runs Effect handlers, and posts
 * responses back through the browser messaging channel.
 *
 * @since 4.0.0
 */
...
/**
 * Creates a `WorkerRunnerPlatform` service that runs worker handlers over a `MessagePort` or
 * `Window`.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use when you need a browser worker entry point to use the ambient `self` object as the worker
 * transport.
 *
 * **Details**
 *
 * Delegates to `make(self)` and provides the runner-side platform used by protocols such as
 * `RpcServer.layerProtocolWorkerRunner`.
 *
 * **Gotchas**
 *
 * This layer depends on the browser worker global `self`. Use `layerMessagePort` when the transport
 * is an explicit `MessagePort` or `Window`.
 *
 * @since 4.0.0
 * @category Layers
