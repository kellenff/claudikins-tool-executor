 ...
 *
 * This module exposes HTTP client layers for code that runs in a browser. It re-exports the
 * fetch-based `Fetch`, `RequestInit`, and `layerFetch` APIs for the common case where requests use
 * the platform `fetch` implementation. It also provides an `XMLHttpRequest`-backed client,
 * including controls for the XHR response type, an overridable `XMLHttpRequest` constructor
 * service, and the `layerXMLHttpRequest` layer.
 *
 * @since 4.0.0
 */
...
/**
 * Context reference for the `XMLHttpRequest.responseType` used by the browser XHR HTTP client,
 * defaulting to `"text"`.
 *
 * **When to use**
 *
 * Use when you need XHR-backed HTTP requests to receive response bodies as text or raw
 * `ArrayBuffer` values.
 *
 * @since 4.0.0
 * @category References
...
/**
 * Runs an effect with `CurrentXHRResponseType` set to `"arraybuffer"` so the XHR HTTP client
 * receives response bodies as `ArrayBuffer` values.
 *
 * @since 4.0.0
 * @category References
 */
