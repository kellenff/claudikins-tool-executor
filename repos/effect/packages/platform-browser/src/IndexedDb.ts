 ...
 *
 * This module is the low-level bridge used by the platform-browser IndexedDB integration. It
 * provides an `IndexedDb` service around the browser `indexedDB` factory and `IDBKeyRange`
 * constructor, a `layerWindow` layer for wiring those primitives from `window`, and schemas for the
 * key shapes accepted by IndexedDB object stores and indexes.
 *
 * @since 4.0.0
 */
...
/**
 * Schema for IndexedDB keys: strings, non-NaN numbers, valid dates, buffer sources, or arrays of
 * those flat key values.
 *
 * @since 4.0.0
 * @category Schemas
 */
 ...
 * **When to use**
 *
 * Use when you need to define numeric key-path fields for `IndexedDbTable` definitions that use
 * IndexedDB auto-increment keys.
 *
 * **Details**
 *
 * The schema accepts integer values from `1` through `2 ** 53`, matching the range used for
 * generated IndexedDB auto-increment keys.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Layer that provides `IndexedDb` from `window.indexedDB` and `window.IDBKeyRange`, failing with a
 * config error when they are unavailable.
 *
 * @since 4.0.0
 * @category Constructors
 */
