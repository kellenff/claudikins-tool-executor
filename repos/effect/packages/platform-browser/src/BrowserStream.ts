 ...
 *
 * This module provides typed constructors that turn `window.addEventListener` and
 * `document.addEventListener` events into Effect `Stream` values. Both helpers accept the usual
 * listener options and an optional stream buffer size.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * By default, the underlying buffer is unbounded in size. You can customize the buffer size by
 * passing an object as the second argument with the `bufferSize` field.
 *
 * @since 4.0.0
 * @category Streams
 */
