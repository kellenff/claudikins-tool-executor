 ...
 *
 * This module defines the `Clipboard` service, the `ClipboardError` raised by failed browser
 * operations, a `make` constructor for custom implementations, and a browser-backed `layer` that
 * uses `navigator.clipboard`. The service supports reading and writing text, reading and writing
 * `ClipboardItem` payloads, writing one `Blob`, and clearing the clipboard.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when an application needs clipboard operations through an Effect service so browser failures
 * stay in the error channel.
 *
 * **Details**
 *
 * `read` and `write` work with `ClipboardItem` arrays. `readString` and `writeString` use text,
 * `writeBlob` writes one `Blob`, and `clear` writes an empty string.
 *
 * **Gotchas**
 *
 * Clipboard access generally requires a secure context and may require user activation,
 * permissions, or a focused document. `ClipboardItem` and non-text MIME type support varies by
 * browser. Failed browser operations are surfaced as `ClipboardError`.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Builds a `Clipboard` service from primitive read and write operations, deriving `clear` and
 * `writeBlob` helpers.
 *
 * @since 4.0.0
