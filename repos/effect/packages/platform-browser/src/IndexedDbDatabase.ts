 ...
 *
 * This module turns an `IndexedDbVersion` migration chain into an `IndexedDbDatabase` layer. The
 * layer opens the browser database, runs any pending upgrade migrations, provides a query builder
 * for the current schema, and exposes a `rebuild` effect that deletes and reopens the database.
 * Migration transactions can create or delete object stores and indexes, and database failures are
 * represented as `IndexedDbDatabaseError` values.
 *
 * @since 4.0.0
 */
...
/**
 * String union describing the failure categories for IndexedDB database opening, migration, and
 * schema operations.
 *
 * @since 4.0.0
 * @category Errors
...
/**
 * Tagged error for IndexedDB database operations, carrying a database error reason and the original
 * cause.
 *
 * @since 4.0.0
 * @category Errors
...
/**
 * Service tag for an open IndexedDB database, its `IDBKeyRange` constructor, reactivity service,
 * and rebuild effect.
 *
 * **When to use**
 *
 * Use when you need access to the live database service after an `IndexedDbSchema` layer has been
 * provided, especially for `rebuild` or lower-level database primitives.
 *
 * **Details**
 *
 * `database` is a mutable reference to the current `IDBDatabase`. `IDBKeyRange` and `reactivity`
 * are shared with query builders created from the schema.
 *
 * **Gotchas**
 *
 * `rebuild` closes and deletes the browser database, then reopens it and reruns migrations. Records
 * not recreated by migrations are removed.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Describes an IndexedDB schema version and its migrations, and acts as an effect that yields a
 * query builder for the target version.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Query builder available during a database migration, extended with object-store and index
 * management helpers for the active `IDBTransaction`.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Type-erased `IndexedDbSchema` covering any source version, target version, and migration error
 * type.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Creates the initial `IndexedDbSchema` from a version and an initialization migration run during
 * database upgrade.
 *
 * @since 4.0.0
