 ...
 *
 * An `IndexedDbQueryBuilder` is created from an open database and a version's table descriptors,
 * then exposes `from(tableName)` as the entry point for table operations. Query objects can select,
 * count, delete, insert, upsert, clear tables, stream paged reads, react to invalidations, and run
 * multiple effects in a shared `IDBTransaction` with `withTransaction`. Reads decode stored rows
 * with the table schema, and writes encode input values before sending them to IndexedDB.
 *
 * @since 4.0.0
 */
...
/**
 * String union describing IndexedDB query failure categories such as decoding, encoding, and
 * transaction errors.
 *
 * @since 4.0.0
 * @category Errors
...
/**
 * Tagged error for IndexedDB query operations, carrying a query error reason and the original
 * cause.
 *
 * **Details**
 *
 * `reason` is the query failure category, `cause` preserves the underlying schema, IndexedDB
 * request, transaction, or user callback failure, and `message` is set to the reason.
 *
 * @since 4.0.0
 * @category Errors
...
/**
 * Typed query builder for an IndexedDB version, with helpers for table queries, database access,
 * clearing data, and running effects in a shared transaction.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Valid key-path type for a table schema, using encoded fields whose values are IndexedDB-valid
 * keys.
 *
 * @since 4.0.0
 * @category Models
...
export declare namespace IndexedDbQuery {
  /**
   * Decoded row type returned by select queries, adding a `key` field when the table does not
   * define a key path.
   *
   * @since 4.0.0
   * @category Models
  ...
  /**
   * Input type for insert and upsert operations, adjusted for auto-increment keys and out-of-line
   * keys.
   *
   * @since 4.0.0
   * @category Models
  ...
  /**
   * Value type accepted by range comparisons for a table key path or index, including partial
   * tuples for compound indexes.
   *
   * @since 4.0.0
   * @category Models
   */
  ...
  /**
   * Query entry point for a table, exposing clear, select, count, delete, insert, and upsert
   * operations.
   *
   * @since 4.0.0
   * @category Models
   ...
   * @category Models
   */
  export interface Clear<Table extends IndexedDbTable.AnyWithProps> extends Effect.Effect<
    void,
    IndexedDbQueryError
  > {
  ...
  /**
   * Effect model for counting table rows, optionally constrained by an index and key-range
   * comparisons.
   *
   * @since 4.0.0
   * @category Models
  ...
  /**
   * Intermediate delete builder used to choose a key range or limit before producing an executable
   * delete query.
   *
   * @since 4.0.0
   * @category Models
  ...
  /**
   * Effect model for deleting rows from a table, with optional key-range, limit, filter, and
   * reactivity invalidation helpers.
   *
   * @since 4.0.0
   * @category Models
   */
  ...
  /**
   * Effect model for selecting rows from a table, with chainable range, paging, filtering,
   * streaming, and reactive query helpers.
   *
   * @since 4.0.0
   * @category Models
   */
  ...
  /**
   * Effect model for selecting the first matching row, failing with `NoSuchElementError` when no
   * row is found.
   *
   * @since 4.0.0
   * @category Models
  ...
  /**
   * Effect model for inserting or upserting one row, returning the resulting IndexedDB key and
   * supporting reactivity invalidation.
   *
   * @since 4.0.0
   * @category Models
   */
  ...
  /**
   * Effect model for inserting or upserting multiple rows, returning the resulting IndexedDB keys
   * and supporting reactivity invalidation.
   *
   * @since 4.0.0
   * @category Models
   */
...
/**
 * Service tag for the active `IDBTransaction` used to share a transaction across IndexedDB query
 * effects.
 *
 * @since 4.0.0
 * @category Models
    ...
    predicate || keyPath === undefined || query.offsetValue !== undefined
      ? yield* Effect.callback<Array<any>, IndexedDbQueryError>((resume) => {
          ...
          cursorRequest.onsuccess = () => {
            ...
            if (predicate === undefined || predicate(cursor.value)) {
              results.push(
                keyPath === undefined ? { ...cursor.value, key: cursor.key } : cursor.value,
              );
              count += 1;
            }
...
const applyFirst = Effect.fnUntraced(function* (query: IndexedDbQuery.First<any, never>) {
  ...
  const data = yield* Effect.callback<any, IndexedDbQueryError | Cause.NoSuchElementError>(
    (resume) => {
...
const applyModifyAll = Effect.fnUntraced(
  ...
  Effect.catchIf(SchemaIssue.isIssue, (issue) =>
    Effect.fail(
      new IndexedDbQueryError({ reason: "EncodeError", cause: new Schema.SchemaError(issue) }),
    ),
  ),
);
...
const QueryBuilderProto: Omit<
  ...
  "fromCache" | "tables" | "database" | "IDBKeyRange" | "IDBTransaction" | "reactivity"
> = {
  ...CommonProto,
...
/**
 * Creates an `IndexedDbQueryBuilder` from an open database reference, key-range constructor, table
 * map, and reactivity service.
 *
 * @since 4.0.0
