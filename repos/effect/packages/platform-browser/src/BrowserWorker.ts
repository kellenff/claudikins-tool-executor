 ...
 *
 * `layerPlatform` provides the `WorkerPlatform` used to communicate with a browser `Worker`,
 * `SharedWorker`, or `MessagePort` through Effect's worker protocol. `layer` combines that platform
 * with a `Spawner` built from a callback that creates or returns the worker endpoint for each
 * worker id.
 *
 * @since 4.0.0
 */
...
/**
 * Creates browser worker layers by combining the default `WorkerPlatform` with a spawner for
 * `Worker`, `SharedWorker`, or `MessagePort` instances.
 *
 * **When to use**
 *
 ...
 * **Details**
 *
 * The `spawn` callback receives the numeric worker id and may return a `Worker`, `SharedWorker`, or
 * `MessagePort`.
 *
 * **Gotchas**
 *
 * Scope finalization sends the worker close protocol over the port. Dedicated workers created by
 * `spawn` are not terminated by this layer.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Layer that provides the browser `WorkerPlatform` for `Worker`, `SharedWorker`, and `MessagePort`
 * communication.
 *
 * @since 4.0.0
 * @category Layers
