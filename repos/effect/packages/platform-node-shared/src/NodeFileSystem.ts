 ...
 *
 * This module adapts Node's `node:fs`, `node:os`, and `node:path` APIs into a `FileSystem` layer
 * for Effect programs running on Node-compatible runtimes. Platform packages use it to provide file
 * and directory I/O, permissions, links, metadata, temporary files and directories, and file
 * watching through the shared `FileSystem` service.
 *
 * @since 4.0.0
 */
...
const makeTempDirectoryFactory = (method: string): FileSystem.FileSystem["makeTempDirectory"] => {
  ...
  return (options) =>
    Effect.suspend(() => {
      ...
      const directory =
        typeof options?.directory === "string" ? Path.join(options.directory, ".") : OS.tmpdir();
...
/**
 * Provides the `FileSystem` service backed by Node filesystem APIs, including file operations,
 * directory operations, links, metadata, and file watching.
 *
 * @since 4.0.0
 * @category Layers
 */
