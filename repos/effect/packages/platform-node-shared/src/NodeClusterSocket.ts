 ...
 *
 * This module provides the shared Node layers used by socket-based cluster transports.
 * `layerClientProtocol` opens TCP sockets to peer runner addresses and wraps them in the current
 * RPC serialization protocol. `layerSocketServer` exposes the socket server that receives incoming
 * runner RPC traffic.
 *
 * @since 4.0.0
 */
...
/**
 * Provides the cluster `RpcClientProtocol` by opening TCP sockets to runner addresses and using the
 * current RPC serialization service.
 *
 * @since 4.0.0
 * @category Layers
 */
