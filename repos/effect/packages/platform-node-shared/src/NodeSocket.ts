 ...
 *
 * This module opens `node:net` connections or wraps existing Node `Duplex` streams and presents
 * them as `Socket.Socket` values, socket channels, or layers. It also exposes the current
 * underlying `NetSocket` service for code running inside a socket handler and re-exports the `ws`
 * package namespace.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Supports `openTimeout` and closes or destroys the underlying socket when the enclosing scope is
 * finalized.
 *
 * @since 4.0.0
...
/**
 * Adapts a Node `Duplex` into a `Socket.Socket`, wiring data events to socket handlers, providing a
 * scoped writer, and mapping open, read, write, and close failures to `SocketError`.
 *
 * @since 4.0.0
 * @category Constructors
 */
      ...
      latch.whenOpen(
        Effect.callback<void, Socket.SocketError>((resume) => {
          ...
          if (Socket.isCloseEvent(chunk)) {
            conn.destroy(
              chunk.code > 1000 ? new Error(`closed with code ${chunk.code}`) : undefined,
            );
...
/**
 * Creates a `Channel` over a TCP socket, reading arrays of `Uint8Array` chunks and writing arrays
 * of bytes, strings, or socket close events.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Provides a `Socket.Socket` by opening a TCP connection with the supplied Node `net` connection
 * options.
 *
 * @since 4.0.0
 * @category Layers
