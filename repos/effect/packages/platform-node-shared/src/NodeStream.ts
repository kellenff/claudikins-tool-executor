 ...
 *
 * This module is the stream boundary for Node APIs. It wraps `Readable` and `Duplex` values as
 * Effect `Stream`s and `Channel`s, pipes Effect streams through Node duplex streams, exposes an
 * Effect `Stream` back to Node as a `Readable`, and collects readable payloads into strings, array
 * buffers, or `Uint8Array`s with optional byte limits.
 *
 * @since 4.0.0
 */
...
/**
 * Converts a Node readable stream into an Effect `Stream`, reading chunks with an optional chunk
 * size, mapping stream errors with `onError`, and destroying the readable on completion unless
...
/**
 * Creates a `Channel` that pulls chunks from a Node readable stream, mapping errors with `onError`
 * and destroying the readable on completion unless `closeOnDone` is `false`.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Creates a `Channel` over a Node `Duplex`, writing upstream chunks with backpressure while
 * emitting chunks read from the duplex and optionally ending the writable side when upstream
 * completes.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Pipes an Effect `Stream` through a Node `Duplex`, writing the stream's chunks to the duplex and
 * emitting chunks read back from it.
 *
 * @since 4.0.0
...
/**
 * Pipes a stream of strings or bytes through a Node `Duplex` using default options and
 * `Cause.UnknownError` for stream failures.
 *
 * @since 4.0.0
 * @category Combinators
 */
...
/**
 * Converts an Effect `Stream` into a Node `Readable`, using the caller's Effect context to run the
 * stream and destroying the readable if the stream fails.
 *
 * @since 4.0.0
 * @category Converting
 */
...
/**
 * Consumes a Node readable stream into a string using the selected encoding, failing through
 * `onError` on stream errors or when `maxBytes` is exceeded and destroying the stream on
...
/**
 * Consumes a Node readable stream into an `ArrayBuffer`, failing through `onError` on stream errors
 * or when `maxBytes` is exceeded and destroying the stream on interruption or failure.
 *
 * @since 4.0.0
 * @category Converting
 */
...
/**
 * Consumes a Node readable stream into a `Uint8Array`, using the same error mapping and `maxBytes`
 * handling as `toArrayBuffer`.
 *
 * @since 4.0.0
 * @category Converting
 */
export const toUint8Array = <E = Cause.UnknownError>(
  ...
  Effect.map(toArrayBuffer(readable, options), (buffer) => new Uint8Array(buffer));
