 ...
 *
 * This module turns `node:net` TCP or Unix-domain servers and `ws` WebSocket servers into scoped
 * `SocketServer.SocketServer` services. Use the TCP constructors when handlers should receive a
 * `Socket.Socket` backed by a Node `net.Socket`; use the WebSocket constructors when handlers
 * should receive a `Socket.Socket` backed by `ws` and have access to the per-connection WebSocket
 * and `IncomingMessage` services.
 *
 * @since 4.0.0
 */
...
/**
 * Service tag for the Node `IncomingMessage` associated with the current WebSocket server
 * connection.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Creates a scoped TCP `SocketServer` from a Node `net.Server`, starts listening with the supplied
 * options, queues pending connections until `run` is called, and closes the server when the scope
 * ends.
 *
 * @since 4.0.0
...
/**
 * Provides a TCP `SocketServer` by creating and managing a scoped Node `net.Server` with the
 * supplied server and listen options.
 *
 * @since 4.0.0
 * @category Layers
 */
...
/**
 * Creates a scoped WebSocket `SocketServer` backed by the `ws` package, providing the WebSocket and
 * its Node `IncomingMessage` to connection handlers and closing the server when the scope ends.
 *
 * @since 4.0.0
 * @category Constructors
 */
...
/**
 * Provides a WebSocket `SocketServer` backed by the `ws` package and managed with the supplied
 * server options.
 *
 * @since 4.0.0
 * @category Layers
