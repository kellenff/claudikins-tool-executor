 ...
 *
 * `fromWritable` creates a `Sink`, `fromWritableChannel` creates a lower-level `Channel`, and
 * `pullIntoWritable` writes from an existing pull loop. All three adapters respect writable-stream
 * backpressure, map writable errors with the supplied `onError` function, and can end the writable
 * when the upstream data is done.
 *
 * @since 4.0.0
 */
...
/**
 * Creates a `Sink` that writes chunks to a Node writable stream, respecting backpressure, mapping
 * writable errors with `onError`, and ending the stream on completion unless `endOnDone` is
 * `false`.
 *
 * @since 4.0.0
...
/**
 * Creates a `Channel` that pulls chunks from upstream and writes them to a Node writable stream,
 * respecting backpressure and optionally ending the writable when upstream is done.
 *
 * @since 4.0.0
 * @category Constructors
 */
 ...
 * **When to use**
 *
 * Use to implement custom Node stream adapters that already have an upstream pull and need direct
 * control over a writable stream.
 *
 * **Details**
 *
 * The loop waits for `drain` when needed, fails on writable errors, and ends the writable on
 * upstream completion unless `endOnDone` is `false`.
 *
 * @since 4.0.0
 * @category Converting
 */
