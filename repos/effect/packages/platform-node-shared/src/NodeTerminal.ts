 ...
 *
 * `NodeTerminal` adapts Node's `readline` APIs plus the current process `stdin` and `stdout`
 * streams into {@link Terminal.Terminal}. The service can display output, read a line, stream key
 * input, and read terminal dimensions. `make` manages readline and TTY raw mode in a scope, while
 * `layer` provides the default service that ends key input on Ctrl+C or Ctrl+D.
 *
 * @since 4.0.0
 */
...
/**
 * Creates a scoped process-backed `Terminal` using Node `readline`, enabling TTY raw mode while in
 * scope and using the supplied predicate to decide when key input should end.
 *
 * @since 4.0.0
