 ...
 *
 * This module builds the service from `node:crypto`, using `randomBytes` for random data and
 * `createHash` for supported digest algorithms. It exports `make` as the concrete service value and
...
const digest: EffectCrypto.Crypto["digest"] = (algorithm, data) =>
  Effect.try({
    try: () =>
      Uint8Array.from(NodeCrypto.createHash(toHashAlgorithm(algorithm)).update(data).digest()),
    catch: (cause) =>
