...
const signatureTypeFormatFlags =
...
function getPackageExportMatch(exports: unknown, subpath: string): PackageExportMatch | undefined {
  if (!isRecord(exports)) {
    return subpath === "."
      ? { key: ".", value: exports, star: null, specificity: 1, prefixLength: 1 }
      : undefined;
  }
...
function resolveBarrelImport(
  ...
  { readonly barrel: ParsedJSDocBarrelImport | null; readonly flatNames: ReadonlyArray<string> },
  string
> {
  ...
  while (isPathInside(metadata.sourceRoot, directory)) {
    ...
    if (indexPath !== filename && fs.existsSync(indexPath)) {
      ...
      const exportTarget =
        barrelSubpath === "" ? "./src/index.ts" : `./src/${barrelSubpath}/index.ts`;
      if (isPackageSubpathExported(metadata.exports, exportSubpath, exportTarget)) {
        ...
        if (namespace !== undefined) {
          return {
            _tag: "Success",
            value: {
              barrel: { type: "namespace", module: moduleSpecifier, name: namespace },
              flatNames,
            },
          };
        }
...
function resolveJSDocImports(cwd: string, filename: string): Result<ParsedJSDocImports, string> {
 ...
 * **Details**
 *
 * The block must have one short description paragraph, optional `When to use`, `Details`, and
 * `Gotchas` sections in that order, then any example sections before tags. Example titles must be
 * unique after trimming and lowercasing, each example must contain exactly one non-empty TypeScript
 * code fence, and boundaries between sections, examples, and tags must use exactly one blank line.
 *
...
export function parseJSDoc(raw: string): Result<ParsedCoreJSDoc, JSDocParseError> {
  ...
  if (block.internal) {
    return {
      _tag: "Failure",
      error: { diagnostics: [diagnostic("internal", "Internal JSDoc blocks are ignored")] },
    };
  }
...
function parseDescriptionContent(lines: Array<string>): ParsedContentResult {
  ...
  while (index < lines.length) {
    ...
    if (heading !== undefined) {
      if (examplesStarted) {
        diagnostics.push(
          diagnostic("section-after-example", `${heading} must appear before examples`),
        );
      }
      const order = standardHeadings.indexOf(heading);
      if (order <= currentSectionOrder) {
        diagnostics.push(
          diagnostic("section-out-of-order", `${heading} is out of order or duplicated`),
        );
      }
...
function parseSection(
  ...
  if (joinBody(bodyLines).trim() === "") {
    diagnostics.push(
      diagnostic("empty-section", `${lines[headingIndex].trim()} must have a non-empty body`),
    );
  }
...
function parseExample(
  ...
  const prose = nonEmptyBody
    ? joinBody(bodyLines.slice(0, bodyLines.at(-1)?.trim() === "" ? -1 : undefined))
    : null;
...
function buildTags(
  ...
  ParsedModuleTags | ParsedDeclarationTags | ParsedNamespaceTags | ParsedMemberTags,
  JSDocParseError
> {
  const diagnostics: Array<JSDocDiagnostic> = [];
  const allowed =
  ...
  const singletonTags =
    scope === "member" ? ["deprecated", "default", "since"] : ["deprecated", "category", "since"];
  ...
  if (
    (scope === "declaration" || scope === "namespace" || scope === "namespace-declaration") &&
    since === null
  ) {
    diagnostics.push(
      diagnostic(
  ...
  if (scope === "member") {
    ...
    if (defaultValue === "")
      diagnostics.push(diagnostic("empty-tag", "@default must include a value"));
    if (diagnostics.length > 0) return { _tag: "Failure", error: { diagnostics } };
    return {
      _tag: "Success",
      value: { since, default: defaultValue, deprecated, see: see.map(parseSeeTag) },
    };
  }
...
export function getProgram(tsconfigPath: string): ProgramCacheEntry {
  ...
  if (result.error !== undefined || result.parsed === undefined) {
    const entry = {
      error: result.error ?? `Unable to parse TypeScript config ${tsconfigPath}`,
      reported: false,
    };
...
function malformedInlineLinks(block: JSDocBlock): Array<string> {
  ...
  while (index !== -1) {
    ...
    const end = source.indexOf("}", index + "{@link".length);
    const text =
      end === -1 ? source.slice(index).split(/\r?\n/, 1)[0] : source.slice(index, end + 1);
...
export function computeJSDocInputHash(options: ExtractJSDocsOptions): string {
  ...
  for (const filename of globSync([...options.include], {
    cwd,
    absolute: true,
    nodir: true,
    ignore: ["**/node_modules/**"],
  ...
  for (const filename of globSync(
    ["package.json", "packages/**/package.json", "tsconfig*.json", "packages/**/tsconfig*.json"],
    {
      cwd,
      absolute: true,
      nodir: true,
...
function createJSDocApiSeeLinkResolver(apis: ReadonlyArray<JSDocApi>) {
  ...
  const resolveSymbol = (link: ParsedInlineLink): JSDocApiSeeLinkResolution | undefined => {
    ...
    if (unique.length === 1)
      return { _tag: "Resolved", apiId: unique[0].id, apiFqn: unique[0].apiFqn };
    if (unique.length > 1) {
      return {
        _tag: "Unresolved",
        reason: "ambiguous",
    ...
    const uniqueByName = Array.from(
      new Map(sameFileByName.map((candidate) => [candidate.id, candidate])).values(),
    );
  ...
  return (
    ...
    for (const target of targets) {
      ...
      if (sameModule !== undefined)
        return { _tag: "Resolved", apiId: sameModule.id, apiFqn: sameModule.apiFqn };
...
function createExampleImportPolicy(files: ReadonlyArray<JSDocModelFile>): ExampleImportPolicy {
  ...
  for (const file of files) {
    ...
    const rootValues = file.declarations
      .filter((declaration) => declaration.bucket === "value" && isIdentifierName(declaration.name))
      ...
      diagnostics.push(
        diagnostic(
          ...
          `JSDoc examples should import ${Array.from(flatModule.names)
            .map((name) => `\`${name}\``)
            .join(", ")} from "${flatModule.barrelModule}" instead of "${source}"`,
        ),
      );
      continue;
    }
...
function parseDocumentedTs(
  ...
  if (block === undefined) {
    if (required) {
      diagnostics.push({
        ...diagnostic(
          "missing-jsdoc",
          scope === "member" ? "Member JSDoc is required" : "Public JSDoc is required",
        ),
  ...
  if (linkContext !== undefined)
    addJSDocLinkDiagnostics(node.getSourceFile(), block, diagnostics, linkContext);
  return {
    core: block.parsed,
    tags:
...
function displaySignature(text: string): string {
  return text.replace(
    /import\((?:"[^"]+"|'[^']+')\)\.([A-Za-z_$][A-Za-z0-9_$]*(?:\.[A-Za-z_$][A-Za-z0-9_$]*)*)/g,
    "$1",
  );
}
...
function signatureFunctionThisKind(
  type: ts.Type,
  checker: ts.TypeChecker,
  location: ts.Node,
): string {
...
function signatureParameterKind(
  ...
  if ((type.flags & ts.TypeFlags.Object) !== 0 || checker.getPropertiesOfType(type).length > 0)
    return "object";
  return "other";
}

function signatureTailKind(
  ...
  const tailKinds = signature.parameters
    .slice(1)
    .map((parameter) => signatureParameterKind(parameter, checker, location));
  if (firstKind === "options") {
    const hasTransformTail =
      tailKinds.length > 1 && tailKinds.slice(1).every(isSignatureFunctionKind);
...
function signatureShapeKey(
  signature: ts.Signature,
  checker: ts.TypeChecker,
  location: ts.Node,
): string {
  const hasRest = signature.parameters.some(
    (parameter) => signatureParameterDeclaration(parameter)?.dotDotDotToken !== undefined,
  );
...
${callSignatures.map((text) => `  ${text};`).join("\n")}
  ...
  const typeText = normalizeSignature(
    cwd,
    checker.typeToString(type, location, signatureTypeFormatFlags),
  );
  if (typeText === null) return null;
  return (
    parseableSignature(cwd, `declare const ${name}: ${typeText}`) ??
...
function declarationSignature(
  ...
  if (ts.isVariableStatement(node)) {
    ...
    return symbol === undefined
      ? null
      : symbolSignature(name, bucket, symbol, declaration.name, checker, cwd);
  }
  const nameNode =
    ts.isFunctionDeclaration(node) ||
    ts.isClassDeclaration(node) ||
  ...
  return symbol === undefined
    ? null
    : symbolSignature(name, bucket, symbol, nameNode, checker, cwd);
}
...
function exportSpecifierSignature(
  ...
  const symbol =
    ...
    (specifier.propertyName === undefined
      ? undefined
      : checker.getSymbolAtLocation(specifier.propertyName));
  return symbol === undefined
    ? null
    : symbolSignature(name, bucket, symbol, specifier.name, checker, cwd);
}
...
export function extractJSDocsSync(options: ExtractJSDocsOptions): JSDocModel {
  ...
  const sourceFilesByFile = new Map(
    program.getSourceFiles().map((file) => [normalizeFile(cwd, file.fileName), file]),
  );
  ...
  for (const filename of files) {
    ...
    if (sourceFile === undefined) {
      modelFiles.push({
        ...
        diagnostics: [
          {
            ...diagnostic("tsconfig", `${file} is not included in ${options.tsconfig}`),
            range: [0, 0],
          },
        ],
        declarations: [],
...
export function readJSDocModel(filename: string): Result<JSDocModel, string> {
  ...
  try {
    ...
    if (!Array.isArray(parsed.files))
      return { _tag: "Failure", error: "Invalid jsdocs model: files must be an array" };
    if (!Array.isArray(parsed.apis))
      return { _tag: "Failure", error: "Invalid jsdocs model: apis must be an array" };
