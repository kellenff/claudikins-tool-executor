 ...
 *
 * This module provides the `BarrelGenerator` service used by the codegen CLI to find files
 * annotated with `@barrel` comments and rewrite the generated export section beneath each
 * annotation. The generator resolves matching modules relative to each annotated barrel file,
 * copies each module's top-level `@since` tag into a minimal JSDoc block, and normalizes export
 * paths so the produced TypeScript is stable across platforms.
 *
 * @since 4.0.0
 */
...
/**
 * Metadata for a barrel file discovered from a `@barrel` annotation, including the file path, glob
 * pattern, and insertion offset for generated exports.
 *
 * @since 4.0.0
 * @category Models
 */
...
/**
 * Builds the `BarrelGenerator` service, discovering files with `@barrel` annotations and rewriting
 * their generated export sections from matching modules.
 *
 * @since 4.0.0
 * @category Layers
 */
...
> = Effect.gen(function* () {
  ...
  const toPosix =
    path.sep === "/" ? (file: string) => file : (file: string) => file.split(path.sep).join("/");
