 ...
 * @category Layers
 */
export const layer: Layer.Layer<SpecFetcher, never, FileSystem.FileSystem | HttpClient.HttpClient> =
  Effect.gen(function* () {
    ...
    const fetchViaStainlessStats = Effect.fn("fetchViaStainlessStats")(function* (
      ...
      const statsContent = yield* httpClient.get(statsUrl).pipe(
        Effect.flatMap((response) => response.text),
        Effect.mapError(
          (cause) => new SpecFetchError({ provider, source: `stats:${statsUrl}`, cause }),
        ),
      );
