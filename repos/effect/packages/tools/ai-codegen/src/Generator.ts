 ...
 *
 * Stainless-generated specs encode an open enum as `anyOf: [{ type: "string" }, { const: "a" }, {
 * const: "b" }]` - "any string is valid, and these are the known values". Left as-is, each const
 * becomes its own `Schema.Literal` member, so consumers cannot recover the literal union from the
 * generated schema (the union's `Type` is just `string`).
 *
 * Rewriting the const branches to `anyOf: [{ type: "string" }, { enum: ["a", "b"] }]` emits
 * `Schema.Union([Schema.String, Schema.Literals(["a", "b"])])` instead, which decodes any string
 * while still exposing the known values as `members[1]` for autocomplete.
 */
 ...
 * @category Layers
 */
export const layerSchema: Layer.Layer<CodeGenerator, never, FileSystem.FileSystem | Path_.Path> =
