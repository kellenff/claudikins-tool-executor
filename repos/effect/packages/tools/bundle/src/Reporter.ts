 ...
 *
 * The reporter coordinates fixture discovery with the Rollup service to turn measured fixture
 * bundles into Markdown tables or visualization output. It is used by the bundle CLI to compare the
 * current workspace against a checked-out base directory, to print a one-off report for selected
 * entry files, and to generate visualizations when a size change needs inspection.
 *
 * Reports compare files by basename and display gzipped Rollup output sizes in decimal kilobytes.
 * Base fixtures are bundled only when the matching file exists; if a current fixture has no
 * matching basename in the base directory it is reported as unchanged. Visualization artifacts are
 * named from entry file stems in the requested output directory, so duplicate names can make the
 * output misleading.
 *
 * @since 4.0.0
 */
...
/**
 * Options for generating a bundle size comparison report against fixture files from a base
 * directory.
 *
 * @since 4.0.0
 * @category Options
...
/**
 * Options for generating a bundle size comparison report for explicit entry files against a base
 * checkout.
 *
 * @since 4.0.0
 * @category Options
...
/**
 * Context service for producing bundle size reports and visualizations from Rollup-generated
 * fixture stats.
 *
 * @since 4.0.0
 * @category Services
