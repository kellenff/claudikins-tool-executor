 ...
 *
 * This module provides the service used by the bundle CLI and reporter to turn fixture or selected
 * TypeScript entrypoints into ESM Rollup output, optionally write a minified artifact, and return
 * the gzipped byte count used in bundle-size comparisons.
 *
 * Bundles are generated in memory so the emitted code can be streamed to both gzip measurement and
 * optional file output. Only Rollup `chunk` outputs are included; assets are ignored, and when
 * Rollup creates multiple chunks (for example because of dynamic imports or shared chunks) their
 * code is streamed together for measurement. The optional output file is named from the entrypoint
 * stem, so it is best treated as an inspection artifact rather than a complete Rollup output
 * directory.
 *
 * @since 4.0.0
 */
...
/**
 * Options for bundling one entry file, optionally writing a minified output and generating a
 * visualization.
 *
 * @since 4.0.0
 * @category Options
...
/**
 * Options for bundling multiple entry files with shared visualization and output-directory
 * settings.
 *
 * @since 4.0.0
 * @category Options
...
export class Rollup extends Context.Service<Rollup>()("@effect/bundle/Rollup", {
  make: Effect.gen(function* () {
    ...
    const createVisualizationOutputs = (
      options: BundleOptions,
    ): ReadonlyArray<VisualizationOutput> => {
    ...
    const bundleAll = Effect.fn("Rollup.bundleAll")(function* (options: BundleAllOptions) {
      return yield* Effect.forEach(
        options.paths,
        (path) =>
          bundle({ path, visualize: options.visualize, outputDirectory: options.outputDirectory }),
