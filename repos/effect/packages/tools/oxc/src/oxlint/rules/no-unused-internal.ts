  ...
  const visit = (directory: string) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        normalizePathName(entryPath).includes("/src/")
      ) {
...
function isNonReferenceName(identifier: ts.Identifier): boolean {
  const parent = identifier.parent;
  return (
    isImportOrExportBinding(identifier) ||
  ...
  for (const statement of sourceFile.statements) {
    if (ts.isImportDeclaration(statement) && ts.isStringLiteral(statement.moduleSpecifier)) {
      ...
      if (importClause.name !== undefined) {
        fileInfo.imports.set(importClause.name.text, {
...
function findInternalReferences(
  ...
  const visit = (current: ts.Node) => {
    if (ts.isPropertyAccessExpression(current) && ts.isIdentifier(current.expression)) {
      ...
      const importedExports = getExports(
        fileInfos,
        imported?.fileName,
...
function scanPublicSignature(
  ...
  if (ts.isFunctionDeclaration(node)) {
    ...
    for (const specifier of node.exportClause.elements) {
      for (const internal of getReExportedInternals(
        fileInfos,
        fileInfo,
        workspacePackages,
        specifier,
...
const rule: CreateRule = {
  meta: {
    ...
    docs: {
      description:
        "Disallow unused @internal exports, public re-exports, and references from public type signatures",
    },
  },
  create(context) {
