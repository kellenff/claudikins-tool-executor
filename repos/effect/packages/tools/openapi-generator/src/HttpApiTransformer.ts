 ...
 *
 * This module maps normalized OpenAPI metadata, tags, request bodies, responses, and security
 * requirements into import declarations, HttpApi groups, endpoint definitions, schema annotations,
 * and middleware classes. The rendered TypeScript can be emitted by the OpenAPI generator as an
 ...
 * **Details**
 *
 * The schema namespace import is named by the caller so generated code can avoid collisions with
 * symbols already present in the output module. Multipart support is included only when the parsed
 * OpenAPI document needs it.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * The generated implementation contains security declarations, reusable middleware classes, HttpApi
 * groups, endpoint definitions, and OpenAPI annotations derived from the parsed operation
 * metadata.
 *
 * @since 4.0.0
 * @category Code generation
...
const renderMediaSchema = (media: ParsedOperationMediaTypeSchema): string => {
  if (media.effectStream === "sse") {
    const options =
      media.contentType === "text/event-stream"
