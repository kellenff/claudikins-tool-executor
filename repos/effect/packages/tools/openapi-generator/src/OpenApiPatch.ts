 ...
 *
 * Handles parsing and applying JSON Patch documents (RFC 6902) to OpenAPI specs. Supports patches
 * from: - JSON files (.json) - YAML files (.yaml, .yml) - Inline JSON strings
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * This error aggregates all application errors so users can see every failing operation at once
 ...
 * **Details**
 *
 * Supports the subset of RFC 6902 operations that Effect's JsonPatch module implements: `add`,
 * `remove`, and `replace`.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Details**
 *
 * A JSON Patch document is an ordered list of operations to apply to a JSON document. Operations
 * are applied in sequence, with each operation seeing the result of previous operations.
 *
 ...
 * **Details**
 *
 * The input is first checked as a file path. If the file exists, it is read and parsed based on its
 * extension (.json, .yaml, .yml). Otherwise, the input is parsed as inline JSON.
 *
 ...
 * **Details**
 *
 * Patches are applied in order, with each patch operating on the result of the previous one. All
 * operations are attempted, and if any fail, the errors are accumulated and reported together so
 * users can fix all issues at once.
 *
