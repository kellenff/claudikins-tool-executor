/**
 * The `OpenApiGenerator` module orchestrates converting OpenAPI and Swagger documents into
 * generated Effect source.
 *
 * It normalizes Swagger 2.0 input, resolves local references, builds the parsed operation model,
 * registers request and response schemas, applies HttpApi-specific adaptations such as multipart
 * helpers and security metadata, emits warnings for unsupported or lossy OpenAPI features, and then
 * delegates final rendering to the HttpClient or HttpApi code generators.
 *
 * @since 4.0.0
 */
...
/**
 * Service for turning OpenAPI or Swagger specifications into generated Effect HTTP client or
 * HttpApi source code.
 *
 * @since 4.0.0
 * @category Services
...
/**
 * Describes a non-fatal issue encountered while mapping an OpenAPI operation to generated Effect
 * source.
 *
 * @since 4.0.0
 * @category Models
...
const parseOpenApi = (
  ...
  for (const [path, methods] of Object.entries(spec.paths)) {
    for (const method of methodNames) {
      ...
      const description =
        Utils.nonEmptyString(operation.description) ?? Utils.nonEmptyString(operation.summary);
      ...
      if (
        isHttpApi &&
        hasUnsupportedSuccessfulSseResponse(resolvedResponses, hasExplicitSuccessResponse)
      ) {
        warnForOperation(emitWarning, op, {
        ...
        if (isHttpApi) {
          for (const [contentType, mediaType] of Object.entries(content ?? {})) {
            ...
            const streamEncoding =
              !Number.isNaN(statusMajorNumber) && statusMajorNumber < 4
...
const isMultipartBinaryFiles = (value: Record<string, unknown>, singleFileRef: string): boolean => {
  ...
  return (
    isMultipartBinaryFile(items) || (Predicate.isObject(items) && items.$ref === singleFileRef)
  );
};
...
const getEffectStreamEncoding = (mediaType: object): "uint8array" | "sse" | undefined => {
  ...
  return stream.encoding === "uint8array" || stream.encoding === "sse"
    ? stream.encoding
    : undefined;
};
    ...
    for (const [contentType, mediaType] of Object.entries(content)) {
      ...
      if (streamEncoding === "sse") {
        if (
          ...
          Predicate.isUndefined(getEffectStreamErrorSchema(mediaType))
        ) {
          return true;
        }
        continue;
...
const remapDefaultResponseStatusForHttpApi = (
  status: string,
  hasExplicitSuccessResponse: boolean,
): string => (status === "default" ? (hasExplicitSuccessResponse ? "500" : "200") : status);
 ...
 * @category Layers
 */
export const layerTransformerSchema: Layer.Layer<OpenApiGenerator> = Layer.effect(
  OpenApiGenerator,
  make,
);
