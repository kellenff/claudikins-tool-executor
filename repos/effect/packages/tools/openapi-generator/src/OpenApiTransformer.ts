 ...
 *
 * This module defines the `OpenApiTransformer` service used to render parsed OpenAPI operations
 * into Effect `HttpClient` modules. It provides both schema-backed and type-only transformer
 * implementations, including generated imports, public client interfaces, operation
 * implementations, typed API errors, optional response handling, and helpers for server-sent events
 * and binary response streams.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * A transformer owns the code-generation dialect for imports, public client types, and the
 * implementation body. The generator swaps implementations to choose between schema-backed clients
 * and type-only clients.
 *
 * @since 4.0.0
 ...
 * **Details**
 *
 * Generated clients import Effect Schema values and use them at runtime to decode successful
 * responses and typed API errors. Request parameters and payloads are typed against each schema's
 ...
 * **Details**
 *
 * Generated clients reference emitted TypeScript types directly and do not import schema decoders
 * for JSON response bodies. Responses are typed as the declared operation result while preserving
 * generated error and stream method shapes.
 *
 * @since 4.0.0
 * @category Code generation
 */
...
export const make = (
  ...
  const operationToImpl = (operation: ParsedOperation) => {
    ...
    const errorCodes =
      operation.errorSchemas.size > 0 && Object.fromEntries(operation.errorSchemas.entries());
    const configAccessor = resolveConfigAccessor(operation, "options", "config");
    pipeline.push(
 ...
 * **When to use**
 *
 * Use when you use this layer for the `httpclient-type-only` generator format, where the generated
 * client relies on TypeScript types instead of runtime Schema decoding.
 *
 * @since 4.0.0
 * @category Code generation
 */
