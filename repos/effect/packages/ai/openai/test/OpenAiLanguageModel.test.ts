      ...
      describe("user messages", () => {
        ...
        it.effect("handles PDF base64 data with filename", () =>
          Effect.gen(function* () {
            ...
            assert.isTrue(
              userMessage.content[0].file_data.startsWith("data:application/pdf;base64,"),
            );
          }).pipe(Effect.provide(makeTestLayer())),
        );
      });
    ...
    describe("tool preparation", () => {
      ...
      it.effect("converts dynamic tools to function type", () =>
        Effect.gen(function* () {
          ...
          const tool = body.tools?.find(
            (entry: any) => entry.type === "function" && entry.name === "DynamicTool",
          );
          assert.isDefined(tool);
  ...
  describe("streamText", () => {
    ...
    it.effect("handles reasoning summary events when reasoning state is missing", () =>
      Effect.gen(function* () {
        ...
        const parts = globalThis.Array.from(partsChunk);
        assert.isDefined(
          parts.find((part) => part.type === "reasoning-start" && part.id === "rs_missing:1"),
        );
        assert.isDefined(
          parts.find((part) => part.type === "reasoning-end" && part.id === "rs_missing:1"),
        );
...
const makeUsage = (overrides: Partial<TestResponseUsage> = {}): TestResponseUsage => ({
  input_tokens: 10,
  output_tokens: 20,
  total_tokens: 30,
