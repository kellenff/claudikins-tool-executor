/**
 * The `OpenAiSchema` module defines the request, response, streaming, and embedding schemas used by
 * the handwritten OpenAI client. These schemas are the transport boundary for JSON sent to and
 * decoded from the Responses and embeddings endpoints.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * These values request additional response fields such as image URLs, encrypted reasoning content,
 * output logprobs, code interpreter outputs, or web search sources. This schema enumerates the
 * include values supported by this client path.
 *
 * @since 4.0.0
 * @category Schemas
 */
 ...
 * **Details**
 *
 * Accepted values are `"in_progress"`, `"completed"`, and `"incomplete"`. This item-level status is
 * used by message, reasoning, and tool-call shapes.
 *
 * @since 4.0.0
 * @category Schemas
 */
 ...
 * **Details**
 *
 * Accepts annotation objects discriminated by `type`: `file_citation`, `url_citation`,
 * `container_file_citation`, or `file_path`.
 *
 * @since 4.0.0
 * @category Schemas
 */
 ...
 * **Details**
 *
 * Accepted annotation variants are `file_citation`, `url_citation`, `container_file_citation`, and
 * `file_path`.
 *
 * @since 4.0.0
 * @category Models
...
/**
 * Schema for a reasoning output item containing encrypted content, summaries, and optional
 * reasoning text.
 *
 * **When to use**
 *
 * Use when decoding or encoding OpenAI Responses reasoning items that may be carried into later
 * request input.
 *
 * **Details**
 *
 * Reasoning items represent model reasoning content. `summary` is required, while `content` and
 * `status` are optional.
 *
 * **Gotchas**
 *
 * `encrypted_content` is populated only when `reasoning.encrypted_content` is requested through
 * `include`.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Details**
 *
 * Accepted item families include request/output messages, function call and function call output,
 * reasoning items, item references, shell and local shell calls and outputs, apply-patch output,
 * and MCP approval responses.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **When to use**
 *
 * Use when validating or encoding the `tools` array for a Responses request, including
 * provider-defined tool records with provider-specific fields.
 *
 * **Details**
 *
 * Accepted variants are function tools, custom tools, and provider-defined OpenAI tools.
 * Provider-defined `type` literals include `apply_patch`, `code_interpreter`, `file_search`,
 * `image_generation`, `local_shell`, `mcp`, `shell`, `web_search`, and `web_search_preview`.
 *
 * **Gotchas**
 *
 * Provider-defined tools use `Schema.StructWithRest`, so this schema checks the provider tool
 * `type` and permits additional provider fields rather than fully validating every
 ...
 * **When to use**
 *
 * Use when validating or encoding the `tool_choice` field that constrains model tool use separately
 * from the tool definitions themselves.
 *
 * **Details**
 *
 * Accepted forms are `"none"`, `"auto"`, `"required"`, an allowed-tools set, a named function or
 * custom tool, or a provider-defined tool choice.
 *
 * @since 4.0.0
 * @category Schemas
...
/**
 * Schema for text output format configuration, including plain text, JSON object, and JSON Schema
 * responses.
 *
 * **When to use**
 *
 * Use when validating or encoding the `text.format` setting for a Responses request, especially
 * when choosing structured JSON Schema output.
 *
 * **Details**
 *
 ...
 * **Details**
 *
 * Validates the Responses API request payload, including input content, model selection,
 * instructions, reasoning options, text output format, tools, `tool_choice`, streaming, storage,
 * response continuation, sampling options, and optional response fields requested through
 * `include`.
 *
 * **Gotchas**
 *
 ...
 * **Details**
 *
 * The required counters are `input_tokens`, `output_tokens`, and `total_tokens`. Provider-specific
 * token detail objects are preserved through `input_tokens_details`, `output_tokens_details`, and
 * additional fields.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Details**
 *
 * Includes total input, output, and combined token counts, with provider-specific token detail
 * fields preserved when present.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * Response objects include the response id, model, creation time, output items, optional token
 * usage, optional incomplete details, and optional service tier.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **Details**
 *
 * Response objects include metadata, output items, optional token usage, and optional incomplete
 * details.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **Details**
 *
 * Known event variants include response lifecycle events, output item events, text and reasoning
 * deltas, tool-call deltas, partial image events, and error events.
 *
 * **Gotchas**
 *
 * Future event types decode through the fallback only when their `type` is not one of the known
 * event types. Malformed known events still fail to decode.
 *
 * @since 4.0.0
 * @category Schemas
 ...
 * **When to use**
 *
 * Use when validating individual embedding entries at the OpenAI client boundary before assuming
 * the embedding payload is a numeric vector.
 *
 * **Details**
 *
 * An embedding item contains its `index`, optional `object` marker, and an `embedding` represented
 * either as a numeric vector or as a string.
 *
 * **Gotchas**
 *
 * Callers that need numeric vectors must account for string embeddings, such as base64-encoded
 * embeddings returned for string encoding formats.
 *
 * @since 4.0.0
 * @category Schemas
 */
 ...
 * **Details**
 *
 * Contains the item index and embedding payload. The embedding payload may be a numeric vector or a
 * string.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when validating or encoding embeddings requests before sending them to OpenAI, while leaving
 * model-specific limits to the provider.
 *
 * **Details**
 *
 * Requires `input` and `model`. `input` may be a string, an array of strings, a token array, or an
 * array of token arrays. Optional fields configure the embedding encoding format, requested
 * dimensions, and user identifier.
 *
 * **Gotchas**
 *
 * This schema validates the transport shape, but OpenAI still enforces provider-side constraints
 * such as non-empty input, integer token ids, input size limits, positive dimensions, and
 * model-specific dimension support.
 *
 * @since 4.0.0
 * @category Schemas
 */
 ...
 * **When to use**
 *
 * Use when you need to validate embeddings responses at an OpenAI client boundary before trusting
 * item shapes, especially when numeric and string embeddings are both allowed.
 *
 * **Details**
 *
 * The response contains an array of `Embedding` items, the model name, an optional `object: "list"`
 * marker, and optional token usage counts for prompt and total tokens.
 *
 * **Gotchas**
 *
 * Each `Embedding` may contain either a numeric vector or a string embedding. Callers that require
 * numeric vectors must account for string embeddings.
 *
 * @since 4.0.0
 * @category Schemas
