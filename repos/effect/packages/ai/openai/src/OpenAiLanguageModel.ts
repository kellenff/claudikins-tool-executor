/**
 * The `OpenAiLanguageModel` module provides the OpenAI Responses API implementation of Effect AI's
 * `LanguageModel` service. It translates Effect AI prompts, files, tools, structured output
 * requests, reasoning metadata, and provider options into OpenAI response requests, then converts
 * OpenAI non-streaming or streaming response results back into Effect AI response content and
 * metadata.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need to provide OpenAI Responses API request defaults through Effect context for
 * language model operations.
 *
 * **Details**
 *
 * Config values are merged with the config object passed to `model`, `make`, or `layer`, with
 * scoped context values taking precedence.
 *
 * @since 4.0.0
 * @category Services
...
export class Config extends Context.Service<
  ...
  Simplify<
    ...
    > & {
      /**
       * File ID prefixes used to identify file IDs in Responses API. When undefined, all file data
       * is treated as base64 content.
       *
       * Examples: - OpenAI: ['file-'] for IDs like 'file-abc123' - Azure OpenAI: ['assistant-'] for
       * IDs like 'assistant-abc123'
       */
...
declare module "effect/unstable/ai/Response" {
  ...
  export interface TextPartMetadata extends ProviderMetadata {
    ...
    readonly openai?: {
      ...
      readonly itemId?: string | null;
      /**
       * If the model emits a refusal content part, the refusal explanation from the model will be
       * contained in the metadata of an empty text part.
       */
 ...
 * **When to use**
 *
 * Use when you want an OpenAI language model value that carries provider and model metadata and can
 * be supplied directly to an Effect program.
 *
 * @since 4.0.0
...
/**
 * Creates an OpenAI `LanguageModel` service from a model identifier and optional request defaults.
 *
 * **When to use**
 *
 * Use to construct an OpenAI Responses API language model service backed by `OpenAiClient`.
 *
 * **Details**
 *
 * The returned effect requires `OpenAiClient`. Request defaults from the `config` option are merged
 * with any `Config` service in the context, with context values taking precedence. The service
 * supports both `generateText` and `streamText`.
 *
 * @since 4.0.0
 * @category Constructors
...
/**
 * Creates a layer that provides the OpenAI `LanguageModel.LanguageModel` service.
 *
 * **When to use**
 *
 * Use when composing application layers and you want OpenAI to satisfy
 * `LanguageModel.LanguageModel` while supplying `OpenAiClient` from another layer.
 *
 * **Details**
 *
 * The `config` option supplies request defaults for the selected model. Scoped values from
 * `withConfigOverride` are merged when each request is built and take precedence over these
 * defaults.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **When to use**
 *
 * Use to apply OpenAI Responses API config overrides around one or more language model operations
 * without changing the defaults passed to `model`, `make`, or `layer`.
 *
 * **Details**
 *
 * The override is dual, so it can be used in pipe form or as `withConfigOverride(effect,
 * overrides)`. Overrides are merged with any existing `Config` service in the current context, and
          ...
          switch (part.type) {
            ...
            case "tool-call": {
              ...
              if (Predicate.isNotUndefined(localShellTool) && toolName === "local_shell") {
                const params = yield* Schema.decodeUnknownEffect(localShellTool.parametersSchema)(
                  part.params,
                ).pipe(
              ...
              if (Predicate.isNotUndefined(shellTool) && toolName === "shell") {
                const params = yield* Schema.decodeUnknownEffect(shellTool.parametersSchema)(
                  part.params,
                ).pipe(
                  Effect.mapError((error) =>
                    AiError.make({
...
const makeResponse = Effect.fnUntraced(function* <Tools extends ReadonlyArray<Tool.Any>>({
  ...
  const webSearchTool = options.tools.find(
    (tool) =>
      ...
      (tool.name === "OpenAiWebSearch" || tool.name === "OpenAiWebSearchPreview"),
