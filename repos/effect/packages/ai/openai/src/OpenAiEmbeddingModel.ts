/**
 * The `OpenAiEmbeddingModel` module provides the OpenAI implementation of Effect AI's
 * `EmbeddingModel` service. It sends embedding requests through `OpenAiClient`, exposes
 * constructors for layers and `AiModel` values, supports scoped request configuration overrides,
 * and checks that OpenAI returns one numeric vector for each requested input.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need scoped OpenAI request defaults or overrides for embedding requests from Effect
 * context.
 *
 * **Details**
 *
 * The service stores the OpenAI create-embedding request payload without `input`, carrying options
 * such as `model`, `dimensions`, `encoding_format`, and `user`.
 *
 * @since 4.0.0
 * @category Services
 ...
 * **When to use**
 *
 * Use to provide an OpenAI `EmbeddingModel` and its `Dimensions` service to an Effect program.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use to construct the `EmbeddingModel.Service` effectfully when `OpenAiClient` is already
 * available in the environment.
 *
 * **Details**
 *
 * The `model` option is sent with each embedding request. Constructor `config` supplies
 * create-embedding request fields other than `model` and `input`, and scoped overrides from
 * `withConfigOverride` are merged last for each request.
 *
 * **Gotchas**
 *
 * The service expects numeric embedding vectors. It fails with `InvalidOutputError` when the
 * provider returns base64 embeddings, out-of-range indexes, duplicate indexes, or an unexpected
 ...
 * `EmbeddingModel.EmbeddingModel` while supplying `OpenAiClient` from another layer.
 *
 * **Gotchas**
 *
 * Use the default floating-point embedding format. The service expects numeric vectors and fails
 * with `InvalidOutputError` if OpenAI returns base64 embeddings.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **When to use**
 *
 * Use when you need scoped OpenAI embedding request defaults for a single effect or workflow
 * without rebuilding the embedding model service.
 *
 * **Details**
 *
 * Supports both data-first and data-last forms. Existing scoped config is read first, then the
 * provided overrides are applied so override fields take precedence.
 *
 * @since 4.0.0
