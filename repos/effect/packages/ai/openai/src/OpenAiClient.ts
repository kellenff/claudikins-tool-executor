/**
 * The `OpenAiClient` module defines the low-level Effect service used by the OpenAI integration for
 * Responses API and embedding requests. It builds a configured HTTP client with authentication and
 * OpenAI organization or project headers, exposes helpers for non-streaming responses, SSE response
 * streams, WebSocket response streams, and embeddings, and maps transport or decoding failures into
 * `AiError`.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Provides the configured HTTP client plus helpers for Responses API calls, streaming Responses
 * events, and embeddings. Transport and schema decoding failures are mapped to `AiError`.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * `https://api.openai.com/v1`, adds the bearer token and optional OpenAI organization/project
 * headers, accepts JSON responses, filters for successful HTTP statuses, and applies
  ...
  function* (options: Options): Effect.fn.Return<Service, never, HttpClient.HttpClient> {
            ...
            Effect.flatMap((response) =>
              decodeResponse(response).pipe(
                Effect.map(
                  (
                    body,
                  ): [typeof OpenAiSchema.Response.Type, HttpClientResponse.HttpClientResponse] => [
                    body,
                    response,
                  ],
                ),
    ...
    const buildResponseStream = (
      ...
      const stream = response.stream.pipe(
        ...
        Stream.takeUntil(
          (event) =>
            event.data.type === "response.completed" || event.data.type === "response.incomplete",
        ),
 ...
 * want to provide `OpenAiClient` as a `Layer`.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **When to use**
 *
 * Use when you need direct access to the WebSocket-backed response streaming service rather than
 * wrapping an effect with WebSocket mode.
 *
 * **Details**
 *
 * `createResponseStream` sends a `response.create` message over the WebSocket connection and
 * returns an HTTP response together with a stream of `ResponseStreamEvent` values.
 *
 * **Gotchas**
 *
        ...
        .runRaw((msg) => {
          ...
          try {
            ...
            if (event.type === "error" && "status" in event) {
              ...
              const error =
                "error" in event ? (event.error as typeof ErrorEvent.Type.error) : event;
 ...
 * **When to use**
 *
 * Use to provide WebSocket mode through layer composition for effects that use OpenAI response
 * streaming.
 *
 * **Gotchas**
 *
