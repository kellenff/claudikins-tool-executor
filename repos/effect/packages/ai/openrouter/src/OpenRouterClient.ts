/**
 * The `OpenRouterClient` module provides an Effect service for calling OpenRouter's chat
 * completions API. It wraps the generated OpenRouter HTTP client with Effect-native constructors,
 * layers, authentication and optional site ranking headers, typed errors, and streaming support.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Provides methods for interacting with OpenRouter's Chat Completions API, including both
 * synchronous and streaming message creation.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * **Details**
 *
 * The payload contains streamed choices, model metadata, optional usage, and may include an
 * OpenRouter error object for a streamed response.
 *
 * @since 4.0.0
 * @category Models
 */
 ...
 * `https://openrouter.ai/api/v1`, adds the bearer token and optional `HTTP-Referer` and `X-Title`
 * headers, accepts JSON responses, and applies `transformClient` when provided.
 *
 * **Gotchas**
 *
 * Scoped `OpenRouterConfig.withClientTransform` applies to generated client request methods.
 * Streaming chat completion requests are sent directly by this module and do not read that scoped
 * transform.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use when you already have the OpenRouter client options in code and want to provide
 * `OpenRouterClient` as a layer.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **When to use**
 *
 * Use when you need client settings for OpenRouter to be read from Effect `Config` values while
 * providing `OpenRouterClient` as a layer.
 *
 * **Details**
 *
