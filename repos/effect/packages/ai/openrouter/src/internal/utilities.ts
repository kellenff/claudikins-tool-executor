...
/**
 * Tracks ReasoningDetailUnion entries and deduplicates them based on a derived canonical key.
 *
 * This is used when converting messages to ensure the API request only contains unique reasoning
 * details, preventing "Duplicate item found with id" errors in multi-turn conversations.
 *
 * The canonical key logic matches the OpenRouter API's deduplication exactly (see
 * openrouter-web/packages/llm-interfaces/reasonings/duplicate-tracker.ts): - Summary: key = summary
 * field - Encrypted: key = id field (if truthy) or data field - Text: key = text field (if truthy)
 * or signature field (if truthy)
 *
 * @internal
 */
