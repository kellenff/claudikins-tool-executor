/**
 * The `OpenRouterLanguageModel` module provides the OpenRouter implementation of Effect AI's
 * `LanguageModel` service. It translates provider-neutral prompts, tools, files, structured output
 * requests, reasoning metadata, cache-control hints, and provider options into OpenRouter chat
 * completion requests, records GenAI telemetry around those calls, and converts normal or streaming
 * results back into Effect AI response content and metadata.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use to provide scoped OpenRouter chat completion defaults or per-operation overrides for an
...
declare module "effect/unstable/ai/Prompt" {
   ...
   * **Details**
   *
   * These options are used when translating system instructions into OpenRouter chat messages.
   *
   * @since 4.0.0
   * @category Request
   */
   ...
   * **Details**
   *
   * These options are used when translating user content into OpenRouter chat messages.
   *
   * @since 4.0.0
   * @category Request
   */
   ...
   * **Details**
   *
   * Preserves reasoning metadata when assistant messages are replayed in later OpenRouter requests.
   *
   * @since 4.0.0
   * @category Request
   */
   ...
   * **Details**
   *
   * These options are used when converting tool results into OpenRouter chat messages.
   *
   * @since 4.0.0
   * @category Request
   */
   ...
   * **Details**
   *
   * Preserves provider reasoning blocks so reasoning-aware conversations can continue across
   * OpenRouter requests.
   *
   * @since 4.0.0
   * @category Request
  ...
  export interface FilePartOptions extends ProviderOptions {
    ...
    readonly openrouter?: {
      /**
       * The name to give to the file. Will be prioritized over the file name associated with the
       * file part, if present.
       */
   ...
   * **Details**
   *
   * Preserves reasoning details associated with tool calls when a conversation is sent back to
   * OpenRouter.
   *
   * @since 4.0.0
   * @category Request
...
declare module "effect/unstable/ai/Response" {
   ...
   * **Details**
   *
   * Associates tool calls with provider reasoning details when the model emits reasoning and tool
   * calls together.
   *
   * @since 4.0.0
   * @category Response
   ...
   * **Details**
   *
   * Includes citation text and offsets returned by providers that support URL annotations.
   *
   * @since 4.0.0
   * @category Response
   */
   ...
   * **Details**
   *
   * Exposes provider response details that are not represented by the common Effect AI finish part
   * fields.
   *
   * @since 4.0.0
   * @category Response
 ...
 * **When to use**
 *
 * Use when you want an OpenRouter language model value that carries provider and model metadata and
...
/**
 * Creates an OpenRouter `LanguageModel` service from a model identifier and optional request
 * defaults.
 *
 * **When to use**
 *
 * Use when you need to construct a `LanguageModel.Service` value backed by `OpenRouterClient`
 * inside an Effect.
 *
 * **Details**
 *
 * The returned effect requires `OpenRouterClient`. Request defaults from the `config` option are
 * merged with any `Config` service in the context, with context values taking precedence. The
 * service supports both `generateText` and `streamText`.
 *
 * **Gotchas**
 *
 * Provider-defined tools are not supported by this provider integration; requests that include them
 * fail with an `InvalidUserInputError`.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * `LanguageModel.LanguageModel` while supplying `OpenRouterClient` from another layer.
 *
 * @since 4.0.0
 * @category Layers
 * @see {@link make} for constructing the language model service effectfully
 ...
 * **When to use**
 *
 * Use to apply OpenRouter request configuration to one effect without changing the model's default
 * configuration.
 *
 * **Details**
 *
 ...
 * pipe form and `withConfigOverride(effect, overrides)`.
 *
 * @since 4.0.0
              ...
              if (part.mediaType.startsWith("audio/")) {
                ...
                if (Predicate.isUndefined(format)) {
                  return yield* AiError.make({
                    ...
                    reason: new AiError.InvalidUserInputError({
                      description:
                        `Detected unsupported media type for audio file: '${part.mediaType}' ` +
                ...
                if (part.data instanceof URL) {
                  return yield* AiError.make({
                    ...
                    reason: new AiError.InvalidUserInputError({
                      description:
                        "Detected URL data for audio file - OpenRouter requires " +
...
const tryToolJsonSchema = <T extends Tool.Any>(
  tool: T,
  method: string,
  transformer: LanguageModel.CodecTransformer,
) =>
