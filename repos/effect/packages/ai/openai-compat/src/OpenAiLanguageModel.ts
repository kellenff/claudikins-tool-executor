/**
 * The `OpenAiLanguageModel` module adapts OpenAI-compatible chat completions providers to Effect
 * AI's `LanguageModel` service. It builds a model service from a model id, translates prompts,
 * files, tools, structured output schemas, and provider-specific options into `OpenAiClient`
 * requests, and maps normal or streaming chat completion results back into Effect AI response
...
type ConfigOptions = Simplify<
  Partial<Omit<CreateResponse, "input" | "tools" | "tool_choice" | "stream" | "text">> & {
    /**
     * File ID prefixes used to identify file IDs in Responses API. When undefined, all file data is
     * treated as base64 content.
     *
 ...
 * **When to use**
 *
 * Use as the context service for OpenAI-compatible language model request configuration, especially
 * when a scoped operation should override the defaults supplied to `model`, `make`, or `layer`.
 *
 * @since 4.0.0
 * @category Context
 ...
 * **When to use**
 *
 * Use when you want an OpenAI-compatible language model value that carries provider and model
 * metadata and can be supplied directly to an Effect program.
 *
 * @since 4.0.0
...
/**
 * Creates an OpenAI-compatible `LanguageModel` service from a model identifier and optional request
 * defaults.
 *
 * **When to use**
 *
 * Use to construct an OpenAI-compatible chat-completions language model service backed by
 * `OpenAiClient`.
 *
 * **Details**
 *
 ...
 * **When to use**
 *
 * Use when composing application layers and you want OpenAI-compatible APIs to satisfy
 ...
 * **When to use**
 *
 * Use to override request configuration for a single language model effect without changing the
 * defaults supplied to `model`, `make`, or `layer`.
 *
 * **Details**
 *
 * Existing `Config` values from the Effect context are merged with `overrides`, and the override
...
const toChatToolChoice = (
  ...
  const functionName =
    Predicate.hasProperty(toolChoice, "name") && typeof toolChoice.name === "string"
      ? toolChoice.name
      : toolChoice.type;
...
const getUsage = (usage: CreateResponse200["usage"]): Response.Usage => {
  ...
  const reasoningTokens =
    getUsageDetailNumber(usage.completion_tokens_details, "reasoning_tokens") ?? 0;
