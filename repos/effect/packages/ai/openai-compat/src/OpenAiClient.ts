/**
 * The `OpenAiClient` module provides an Effect service for OpenAI-compatible chat completions and
 * embeddings APIs. It builds on the Effect HTTP client, adds authentication and OpenAI organization
 * or project headers, and exposes typed helpers for non-streaming chat completions, streaming chat
 * completions, and embedding requests.
 *
 * @since 4.0.0
 */
 ...
 * **Details**
 *
 * Exposes the configured HTTP client plus helpers for non-streaming chat completions, streaming
 * chat completions, and embeddings. Transport and schema decoding failures are mapped to
 * `AiError`.
 *
 * @since 4.0.0
 * @category Models
 ...
 * **When to use**
 *
 * Use when building effects that depend on the low-level OpenAI-compatible client through context
 * rather than receiving the client as a value.
 *
 * **Details**
 *
 * The tagged service is the `Service` interface produced by `make` and provided by `layer` or
 * `layerConfig`.
 *
 * @since 4.0.0
 * @category Services
 ...
 * `https://api.openai.com/v1`, adds authentication and OpenAI organization/project headers, accepts
 * JSON responses, and applies `transformClient` when provided.
 *
 * **Gotchas**
 *
 * A scoped `OpenAiConfig.withClientTransform` is applied when request helpers run, after the
 * `transformClient` option supplied to `make`.
 *
 * @since 4.0.0
        ...
        pipe(
          ...
          Effect.flatMap((response) =>
            Effect.map(
              ...
              (body): [CreateResponse200, HttpClientResponse.HttpClientResponse] => [
                body,
                response,
              ],
            ),
      ...
      const stream = response.stream.pipe(
        ...
        Stream.catchTags({
          ...
          HttpClientError: (error) =>
            Stream.fromEffect(Errors.mapHttpClientError(error, "createResponseStream")),
        }),
      ) as any;
 ...
 * **When to use**
 *
 * Use to install `OpenAiClient` in an application layer when the client options are already
 * available as values rather than loaded from `Config`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates a layer that loads OpenAI-compatible client settings from `Config` values before
 * constructing the service.
 *
 * **When to use**
 *
 * Use when you need client settings for OpenAI-compatible APIs to be read from Effect `Config`
 * values while providing `OpenAiClient` as a layer.
 *
 * **Details**
 *
 * Only config values supplied in `options` are loaded. Omitted fields are passed to `make` as
 * `undefined`, and `transformClient` is forwarded as a plain option.
 *
 * @since 4.0.0
 * @category Layers
 * @see {@link make} for constructing the client service effectfully
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const organizationId =
        options?.organizationId !== undefined ? yield* options.organizationId : undefined;
...
/**
 * Reasoning output item containing encrypted reasoning content, summaries, and optional reasoning
 * text.
 *
 * @since 4.0.0
 * @category Response
 ...
 * **Details**
 *
 * Supports input messages, output messages, tool calls, tool outputs, reasoning items, custom tool
 * interactions, and item references.
 *
 * @since 4.0.0
 * @category Request
 */
...
/**
 * Responses-style response object returned by compatible providers or embedded in response stream
 * lifecycle events.
 *
 * @since 4.0.0
 * @category Response
 ...
 * **Details**
 *
 * The embedding can be returned either as a numeric vector or as a base64-encoded string. The
 * `index` field identifies the input item that produced this embedding.
 *
 * @since 4.0.0
 * @category Response
 */
