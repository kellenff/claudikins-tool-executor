/**
 * The `OpenAiEmbeddingModel` module adapts OpenAI-compatible embeddings endpoints to Effect's
 * embedding model service. It sends embedding requests through {@link OpenAiClient}, exposes
 * constructors for layers and `AiModel` values, supports scoped request configuration overrides,
 * and checks that the provider returns one numeric vector for each requested input.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need to provide shared default request options for OpenAI-compatible embedding
 * operations through the Effect context, such as `dimensions`, `encoding_format`, or `user`.
 *
 * **Details**
 *
 * The service stores the embedding request payload without `input`. Requests combine the selected
 * model, layer or constructor config, and scoped context config, with scoped context config taking
 * precedence.
 *
 * @since 4.0.0
 * @category Context
...
/**
 * Creates an `AiModel` for an OpenAI-compatible embedding model with its configured vector
 * dimensions.
 *
 * **When to use**
 *
 * Use to provide an OpenAI-compatible `EmbeddingModel` and its `Dimensions` service to an Effect
 * program.
 *
 * @since 4.0.0
 ...
 * **When to use**
 *
 * Use when you need to build or provide an `EmbeddingModel` service directly from an existing
 * `OpenAiClient`.
 *
 * **Details**
 *
 * The service sends embedding requests through `OpenAiClient.createEmbedding`. Request config is
 * merged as the selected model, constructor config, then scoped `Config`, so scoped overrides take
 * precedence. Provider usage `prompt_tokens` is exposed as `usage.inputTokens`.
 *
 * **Gotchas**
 *
 * Provider responses must contain one numeric vector for every requested input with unique,
 * in-range `index` values; otherwise embedding operations fail with `AiError.InvalidOutputError`.
 *
 * @since 4.0.0
 * @category Constructors
 ...
 * **When to use**
 *
 * Use when composing application layers and you want an OpenAI-compatible embeddings endpoint to
 * satisfy `EmbeddingModel.EmbeddingModel` while supplying `OpenAiClient` from another layer.
 *
 * @since 4.0.0
 * @category Layers
 ...
 * **When to use**
 *
 * Use to apply embedding request options to one effect without changing the model's default
 * configuration.
 *
 * **Details**
 *
