...
describe("OpenAiLanguageModel", () => {
  ...
  describe("generateObject", () => {
    ...
    it.effect("uses OpenAI codec transformer for optional structured fields", () =>
      Effect.gen(function* () {
        ...
        const requestBody = yield* getRequestBody(capturedRequest);
        assert.deepStrictEqual(requestBody.response_format.json_schema.schema.required, [
          "name",
          "nickname",
        ]);
  ...
  describe("streamText", () => {
    ...
    it.effect("preserves fragmented stream tool call ids and names", () =>
      Effect.gen(function* () {
        ...
        const start = parts.find(
          (part) => part.type === "tool-params-start" && part.id === "patch_call_2",
        );
    ...
    it.effect(
      "assembles streamed tool args when continuation fragments have function.name: null",
      () =>
        Effect.gen(function* () {
            ...
            choices: [
              {
                index: 0,
                delta: {
                  tool_calls: [{ index: 0, id: "call_1", type: "function", function: fnDelta }],
                },
              },
            ],
          });
    ...
    it.effect("closes an open reasoning part at stream end", () =>
      Effect.gen(function* () {
                  ...
                  sseResponse(request, [
                    {
                      ...
                      created: 1,
                      choices: [
                        { index: 0, delta: { reasoning: "Only thoughts." }, finish_reason: "stop" },
                      ],
                    },
                    "[DONE]",
                  ]),
        ...
        assert.deepStrictEqual(
          parts.map((part) => part.type),
          ["response-metadata", "reasoning-start", "reasoning-delta", "reasoning-end", "finish"],
        );
      }),
    );
  });
