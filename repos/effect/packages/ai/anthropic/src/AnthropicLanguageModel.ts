/**
 * The `AnthropicLanguageModel` module provides the Anthropic implementation of Effect AI's
 * `LanguageModel` service. It translates Effect AI prompts, tools, files, reasoning content, and
 * Anthropic-specific options into Messages API requests, then converts normal and streaming
 * Anthropic responses back into Effect AI response content with provider metadata.
 *
 * @since 4.0.0
 */
 ...
 * **When to use**
 *
 * Use when you need scoped Anthropic model request defaults or per-operation overrides from Effect
 * context.
 *
 * **Details**
 *
 * The service stores request fields that are merged into Anthropic Messages API requests. Scoped
 * configuration overrides defaults supplied to `model`, `make`, or `layer`.
 *
 * @since 4.0.0
...
export class Config extends Context.Service<
  ...
  Simplify<
    ...
    > & {
       ...
       * **Details**
       *
       * Only applies to models that support structured outputs. Defaults to `true` when structured
...
declare module "effect/unstable/ai/Prompt" {
   ...
   * **Details**
   *
   * These options are used when translating system messages into Anthropic request content.
   *
   * @since 4.0.0
   * @category Request
   */
   ...
   * **Details**
   *
   * These options are used when translating user messages into Anthropic request content.
   *
   * @since 4.0.0
   * @category Request
   */
   ...
   * **Details**
   *
   * These options are used when replaying assistant messages in Anthropic conversation history.
   *
   * @since 4.0.0
   * @category Request
   */
   ...
   * **Details**
   *
   * These options are used when converting tool results into Anthropic user content blocks.
   *
   * @since 4.0.0
   * @category Request
   */
   ...
   * **Details**
   *
   * Preserves Claude thinking metadata when reasoning content is sent back to Anthropic in later
   * turns.
   *
   * @since 4.0.0
   * @category Request
    ...
    readonly anthropic?: {
      readonly info?:
        | {
            ...
            /**
             * Thinking content as an encrypted string, which is used to verify that thinking content
             * was indeed generated by Anthropic's API.
             */
        ...
        | {
            ...
            /**
             * Thinking content which was flagged by Anthropic's safety systems, and was therefore
             * encrypted.
             */
   ...
   * **Details**
   *
   * Controls document metadata, citations, and prompt caching for files sent to Anthropic.
   *
   * @since 4.0.0
   * @category Request
   */
  export interface FilePartOptions extends ProviderOptions {
    readonly anthropic?: {
      ...
      /**
       * A custom title to provide to the document. If omitted, the file part's `fileName` property
       * will be used.
       */
      readonly documentTitle?: string | null;
      /**
       * Additional context about the document that will be forwarded to the large language model,
       * but will not be used towards cited content.
       *
       * **When to use**
       *
   ...
   * **Details**
   *
   * Carries Anthropic tool caller metadata, MCP metadata, and cache control for tool use blocks.
   *
   * @since 4.0.0
   * @category Request
   */
...
declare module "effect/unstable/ai/Response" {
   ...
   * **Details**
   *
   * Includes Claude thinking metadata needed to continue reasoning-aware conversations.
   *
   * @since 4.0.0
   * @category Response
   */
   ...
   * **Details**
   *
   * Identifies Anthropic caller details and MCP tool metadata emitted by the provider.
   *
   * @since 4.0.0
   * @category Response
   */
   ...
   * **Details**
   *
   * Identifies MCP tool metadata associated with provider-executed tool results.
   *
   * @since 4.0.0
   * @category Response
   */
  ...
  export interface UrlSourcePartMetadata extends ProviderMetadata {
    readonly anthropic?:
      | {
          readonly source: "url";
          /** Up to 150 characters of the text content that was referenced from the URL source material. */
          readonly citedText: string;
          /**
           * An internal reference that must be passed back to the Anthropic API during multi-turn
           * conversations.
           */
   ...
   * **Details**
   *
   * Includes container state, context management information, stop details, and token usage
 ...
 * **When to use**
 *
 * Use when you want an Anthropic Claude model value that carries provider and model metadata and
 * can be supplied directly to an Effect program.
 *
 * @since 4.0.0
...
/**
 * Creates an Anthropic `LanguageModel` service from a model identifier and optional request
 * defaults.
 *
 * **When to use**
 *
 * Use when you need to construct a `LanguageModel.Service` value backed by `AnthropicClient` inside
 * an Effect.
 *
 * **Details**
 *
 * The returned effect requires `AnthropicClient`. Request defaults from the `config` option are
 * merged with any `Config` service in the context, with context values taking precedence.
 *
 * @since 4.0.0
 * @category Constructors
...
export const make = Effect.fnUntraced(function* ({
  ...
  const makeConfig: Effect.Effect<typeof Config.Service & { readonly model: string }> = Effect.gen(
    function* () {
 ...
 * `LanguageModel.LanguageModel` while supplying `AnthropicClient` from another layer.
 *
 * @since 4.0.0
 * @category Layers
 * @see {@link make} for constructing the language model service effectfully
 ...
 * **When to use**
 *
 * Use to apply Anthropic request configuration to one effect without changing the model's default
 * configuration.
 *
 * **Details**
 *
 * The overrides are merged with any existing `Config` service for the duration of the supplied
 * effect. Fields in `overrides` take precedence over existing config, and the helper supports both
 * `effect.pipe(withConfigOverride(overrides))` and `withConfigOverride(effect, overrides)`.
 *
 * @since 4.0.0
 * @category Configuration
          ...
          switch (message.role) {
            case "user": {
              for (let j = 0; j < message.content.length; j++) {
                ...
                const cacheControl =
                  getCacheControl(part) ?? (isLastPart ? getCacheControl(message) : null);
      ...
      case "assistant": {
        ...
        for (let j = 0; j < group.messages.length; j++) {
          ...
          for (let k = 0; k < message.content.length; k++) {
            ...
            // message for cache control.
            const cacheControl =
              getCacheControl(part) ?? (isLastPart ? getCacheControl(message) : undefined);
 ...
 * **When to use**
 *
 * Use when you need to type or inspect the provider-specific request payload for a custom Anthropic
 * tool.
 *
 * **Details**
 *
 * This type aliases the encoded `Generated.BetaTool` schema used for Effect user-defined and
 * dynamic tools after conversion. It contains the tool `name`, optional `description`, and
 * `input_schema`, plus Anthropic-specific fields such as `strict` and `cache_control`.
 *
 * @since 4.0.0
 * @category Tools
 ...
 * **Details**
 *
 * These include Anthropic's built-in tools like computer use, code execution, web search, and text
 * editing.
 *
 * @since 4.0.0
 * @category Tools
