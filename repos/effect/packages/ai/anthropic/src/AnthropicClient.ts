/**
 * The `AnthropicClient` module defines the low-level Effect service for Anthropic's Messages API.
 * It builds a generated Anthropic HTTP client with authentication headers, API version headers,
 * response decoding, and error mapping, then exposes helpers for regular and streaming message
 * requests.
 *
 * @since 4.0.0
 */
...
/**
 * Represents the Anthropic client service with methods for the Messages API, including regular and
...
export interface Service {
  ...
  /**
   * Executes a low-level streaming HTTP request and decodes the Server-Sent Events response using
   * the provided schema.
   */
  ...
  /**
   * Creates a message using the Anthropic Messages API and maps all errors to the unified `AiError`
   * type.
   */
  ...
  /**
   * Creates a streaming message using the Anthropic Messages API and maps all errors to the unified
   * `AiError` type.
   *
   * **Details**
   *
   * The returned Effect yields the HTTP response and a stream of events as the model generates its
   * response. The stream automatically terminates when a `message_stop` event is received.
   */
 ...
 * **When to use**
 *
 * Use when the Anthropic client settings are already available as values and should be passed
 * directly to `make` or `layer`.
 *
 * **Details**
 *
 * These options configure the base Anthropic URL, the `x-api-key` authentication header, the
 * `anthropic-version` header, and an optional transformation of the underlying `HttpClient`.
 *
 * @since 4.0.0
 * @category Options
...
export type Options = {
  /**
   * The Anthropic API key for authentication. Requests are made without authentication when this is
   * omitted, which is useful for proxied setups or testing.
   */
  readonly apiKey?: Redacted.Redacted<string> | undefined;

  /**
   * The base URL for the Anthropic API. Override this to use a proxy or a different API-compatible
   * endpoint.
   *
  ...
  /**
   * Optional transformer for the underlying HTTP client, such as middleware, logging, or custom
 ...
 * **When to use**
 *
 * Use when you have explicit configuration values and need an `Effect` that constructs the
 * Anthropic client service, rather than providing it as a `Layer`.
 *
 * **Details**
 *
 * The client handles API key authentication via the `x-api-key` header, API versioning via the
 * `anthropic-version` header, error mapping to the unified `AiError` type, and request/response
 * transformations via `AnthropicConfig`. It requires an `HttpClient` in the context.
 *
 * @since 4.0.0
      ...
      const stream = response.stream.pipe(
        ...
        Stream.catchTags({
          ...
          HttpClientError: (error) =>
            Stream.fromEffect(Errors.mapHttpClientError(error, "createMessageStream")),
 ...
 * **When to use**
 *
 * Use when you already have explicit `Options` values, such as an API key or custom API URL, and
 * want to provide `AnthropicClient` as a `Layer`.
 *
 * @since 4.0.0
 * @category Layers
...
/**
 * Creates a layer for the Anthropic client, loading the requisite configuration via Effect's
 * `Config` module.
 *
 * **When to use**
 *
 * Use when you want to provide the Anthropic client as a `Layer` with configuration loaded from
 * Effect's `Config` module, such as from environment variables or a secrets provider.
 *
 * @since 4.0.0
 * @category Layers
